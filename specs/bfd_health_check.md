...
+        <xsd:element name='ignore-initial-failure' type='xsd:boolean' required='optional' operations='CRUD' default='false'
+             description='Ignore initial failure'/>
...
A new boolean "ignore-initial-failure" has also been added to health check
properties. This is not set by default. If set, it indicates that the very
first failure from health check should be ignored. This is specifically useful
for BFD, if other end is not configured at all for BFD. In such cases, even
though initially BFD would notify that the session is down (as it could not be
brought up), agent would ignore and keep the interface as still up. However, if
the BFD session ever comes up once and later goes down (toggles from up to
down), then the interface shall be brought down and taken out of the forwarding
path.
