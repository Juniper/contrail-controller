...
ip_version | **/!\\ Not supported by Contrail FW /!\\**
...
action | `action-list.simple-action` **/!\\ Missing action `reject` /!\\**
...
audited | `audited` **/!\\ Not yet implemented /!\\**
...
ingress_firewall_policy_id | `firewall-policy` reference with direction property set to `<`
egress_firewall_policy_id | `firewall-policy` reference with direction property set to `>`
...
User coould use the OpenStack Neutron FWaaS v2 API extension.

## 3.4 UI changes
No change for the Contrail WebUI. OpenStack dashboard proposes new views to
administrate and use the penStack Neutron FWaaS v2 API extension.
