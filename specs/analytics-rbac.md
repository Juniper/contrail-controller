...
* For every UVE, UUID of corresponding config object will be retrieved from
  ...
  cloud-admin roles only and won't check object level permissions. Here is the
  list of UVEs which will be visible to a tenant user:
...
* UVEs list queries (e.g. analytics/uve/virtual-networks)
  ...
  will look like:
            ...
            "virtual-machine" : ("ObjectVMTable", virtual_network),
            "virtual-machine-interface" : ("ObjectVMITable", virtual_network),
...
* Alarms
    ...
    Alarms are binned based on alarm-key (same as object type e.g. Virtual-network) and uve-key.
    As part of Objectlog, mapping of object log type is added. This provides a list of alarm-keys
    which are supported for non-admin users. For every such alarm-key, get list of permitted uves
    from ApiServer. Now, for every alarm of an alarm-key, check if uve-key is in permitted uves.
    Discard all other alarms

* Alarm/uve-stream

    From the previous section, there is a list of object-log types which are supported for non-admin users.
    This list is passed as ‘tablefilt’ to UveStreamer. In addition, ContrailConfig structure (which
    has details of owner and all the domains/tenants with which that UVE is shared) is checked in every
    UVE to check whether the user has permissions to ‘read’ that UVE.
    If a tablefilt is passed in original query, then it is compared against the supported list.
