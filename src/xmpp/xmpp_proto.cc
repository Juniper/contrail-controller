...
int XmppProto::EncodeStream(const XmppStreamMessage &str, string &to,
                            string &from, string &xmlns, uint8_t *buf,
                            size_t size) {
    int len = 0;
...
int XmppProto::EncodeOpen(uint8_t *buf, string &to, string &from, string &xmlns,
                          size_t max_size) {
...
int XmppProto::SetXmlns(string &xmlns, XmlBase *doc) {
    if (!doc) return -1;
