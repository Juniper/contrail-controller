...
bool XmppSession::Match(Buffer buffer, int *result, bool NewBuf) {
    ...
    xmsm::XmSubState sub_state = connection->GetStateMcSubState();
    ...
    *result = 0;
    do {
        ...
        if (state == xmsm::ACTIVE || state == xmsm::IDLE) {
            ...
            m = MatchRegex(tag_known_ ? stream_res_end_:stream_patt_);
        } else if ((state == xmsm::OPENCONFIRM) && (IsSslEnabled())) {
            if (connection->IsClient()) {
                if (sub_state == xmsm::OPENCONFIRM_FEATURE_NEGOTIATION) {
                    m = MatchRegex(tag_known_ ? end_patt_: proceed_patt_);
                } else if (sub_state == xmsm::OPENCONFIRM_TLSHANDSHAKE_SUCCESS) {
                    m = MatchRegex(tag_known_ ? stream_res_end_:stream_patt_);
                } else {
                ...
                if (sub_state == xmsm::OPENCONFIRM_TLSHANDSHAKE_SUCCESS) {
                    m = MatchRegex(tag_known_ ? stream_res_end_:stream_patt_);
                } else {
