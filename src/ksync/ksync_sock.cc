...
AgentSandeshContext *KSyncSock::agent_sandesh_ctx_[IoContext::MAX_WORK_QUEUES];
...
KSyncSock::KSyncSock() :
    ...
    bulk_sandesh_context_() , tx_count_(0), ack_count_(0), err_count_(0) {
    TaskScheduler *scheduler = TaskScheduler::GetInstance();
    uint32_t task_id = 0;
    for(int i = 0; i < IoContext::MAX_WORK_QUEUES; i++) {
        task_id = scheduler->GetTaskId(IoContext::io_wq_names[i]);
        receive_work_queue[i] = new WorkQueue<KSyncRxData>
            (task_id, i, boost::bind(&KSyncSock::ProcessKernelData, this,
                                     &bulk_sandesh_context_[i], _1));
        char name[128];
...
bool KSyncSock::ValidateAndEnqueue(char *data, KSyncBulkMsgContext *context) {
    ...
    if (context) {
        q_id = context->io_context_type();
    } else {
        uint32_t seqno = GetSeqno(data);
        if ((seqno & KSYNC_DEFAULT_Q_ID_SEQ) == KSYNC_DEFAULT_Q_ID_SEQ) {
            q_id = IoContext::KSYNC_QUEUE_1;
        } else {
            q_id = IoContext::UVE_Q_ID;
        }
    }
    receive_work_queue[q_id]->Enqueue(KSyncRxData(data, context));
    return true;
}
...
void KSyncSock::SendAsync(KSyncEntry *entry, int msg_len, char *msg,
    ...
    // Get the IoContext type for the KSync entry. If unspecified, then use
    // default
    IoContext::IoContextWorkQId qid = IoContext::KSYNC_QUEUE_1;
    int id = entry->IoContextType();
    if (id > 0) {
        qid = IoContext::IoContextWorkQId(id % IoContext::MAX_WORK_QUEUES);
    }
...
(uint32_t seqno, IoContext::IoContextWorkQId io_context_type) {
    if (read_inline_) {
        if (bulk_seq_no_ == kInvalidBulkSeqNo) {
...
bool KSyncSock::TryAddToBulk(KSyncBulkMsgContext *bulk_message_context,
    ...
    if (bulk_message_context->io_context_type() != ioc->GetWorkQId())
        return false;
...
bool KSyncSock::SendAsyncImpl(IoContext *ioc) {
    KSyncBulkMsgContext *bulk_message_context =
        LocateBulkContext(ioc->GetSeqno(), ioc->GetWorkQId());
    ...
    bulk_message_context = LocateBulkContext(ioc->GetSeqno(), ioc->GetWorkQId());
...
KSyncIoContext::KSyncIoContext(KSyncEntry *sync_entry, int msg_len, char *msg,
    ...
    IoContext(msg, msg_len, seqno, KSyncSock::GetAgentSandeshContext(type),
              type),
...
/////////////////////////////////////////////////////////////////////////////
KSyncBulkMsgContext::KSyncBulkMsgContext(IoContext::IoContextWorkQId type) :
    io_context_list_(), io_context_type_(type), rx_buffer_index_(0),
    vr_response_count_(0), io_context_list_it_() {
}
