...
bool KSyncSock::TryAddToBulk(KSyncBulkMsgContext *bulk_message_context,
    ...
    if ((bulk_buf_size_ + ioc->GetMsgLen()) >= max_bulk_buf_size_) {
        LOG(ERROR, "max_bulk_buff_size is less. max_bulk_buf_size_ = " << max_bulk_buf_size_ <<
	" bulk_buf_size_ = " << bulk_buf_size_ << " ioc->GetMsgLen() = " << ioc->GetMsgLen() <<
        " CEM-22441");
        return false;
    }

    if (bulk_msg_count_ >= max_bulk_msg_count_) {
        LOG(ERROR, "max_bulk_msg_count is less. max_bulk_msg_count_ = " << max_bulk_msg_count_ <<
	"bulk_msg_count_ = " << bulk_msg_count_ << " CEM-22441");
        return false;
    }

    if (bulk_message_context->io_context_type() != ioc->type()) {
        LOG(ERROR, "ioc->type() is not matching. ioc->type() = " << ioc->type() <<
	" bulk_message_context->io_context_type() = " << bulk_message_context->io_context_type()
	<< " CEM-22441");
        return false;
    }
    ...
    if (use_wait_tree_) {
        if (bulk_message_context->work_queue_index() != ioc->index()) {
            LOG(ERROR, "ioc->index() is not matching. ioc->index() = " << ioc->index() <<
            "bulk_message_context->work_queue_index() = " << bulk_message_context->work_queue_index()
	    << " CEM-22441");
            return false;
        }
    }
