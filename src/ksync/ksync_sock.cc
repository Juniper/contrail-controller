...
void KSyncSockTcp::Receive(mutable_buffers_1 buf) {
    ...
    boost::system::error_code ec;
    while (1) {
        mutable_buffers_1 buffer = static_cast<mutable_buffers_1>(buf + bytes_read);
        bytes_read += session_->socket()->receive(buffer, 0, ec);
        if (ec != 0) {
            assert(0);
        }
        const struct nlmsghdr *nlh =  buffer_cast<struct nlmsghdr *>(buf);
        if (nlh->nlmsg_type == NLMSG_ERROR) {
            LOG(ERROR, "Netlink error for seqno " << nlh->nlmsg_seq
                    << " len " << nlh->nlmsg_len);
            assert(0);
        }
        if (bytes_read == nlh->nlmsg_len) {
            return;
        }
    }
}
