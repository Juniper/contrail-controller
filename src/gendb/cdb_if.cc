...
CdbIf::CdbIf(DbErrorHandler errhandler,
        ...
        std::string name, bool only_sync, std::string cassandra_user,
...
bool CdbIf::Db_FindTablespace(const std::string& tablespace) {
    CDBIF_LOG(INFO, "FindTablespace login succeeded " << cassandra_user_ );
    /*if (!cassandra_user_.empty() || !cassandra_password_.empty()) {
       ...
       CDBIF_BEGIN_TRY {
           client_->login(authRequest);
           CDBIF_LOG(INFO, "FindTablespace login succeeded " );
    ...
    CDBIF_BEGIN_TRY {
        ...
        CdbIfStats::CDBIF_STATS_ERR_NO_ERROR,
        CdbIfStats::CDBIF_STATS_CF_OP_NONE)
    CDBIF_LOG(INFO, "abt to come out of find table space");
    return true;
}
