...
DnsAgentXmpp::DnsAgentXmppDecode(const pugi::xml_node dns,
    ...
    if (strcmp(resp.name(), "response") == 0) {
        ...
        code >> resp_code;
        return DecodeDns(resp.first_child(), type, true, data, compute_ip);
    } else
        return DecodeDns(resp, type, false, data, compute_ip);
}
...
DnsAgentXmpp::DecodeDns(const pugi::xml_node node,
    ...
    if (strcmp(node.name(), "update") == 0) {
        ...
        return DecodeDnsItems(node, data, compute_ip);
