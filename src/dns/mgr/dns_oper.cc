...
bool VirtualDnsConfig::HasChanged() {
    if (rec_.domain_name == old_rec_.domain_name &&
        ...
        rec_.negative_cache_ttl_seconds == old_rec_.negative_cache_ttl_seconds )
        return false;
    return true;
}
...
void VirtualDnsConfig::VirtualDnsTrace(VirtualDnsTraceData &rec) {
    ...
    rec.flags = flags_;
    rec.negative_cache_ttl_seconds = rec_.negative_cache_ttl_seconds;
}
...
autogen::VirtualDnsType VirtualDnsRecordConfig::GetVDns() const {
    ...
    data.negative_cache_ttl_seconds = 0;
    if (virt_dns_)
