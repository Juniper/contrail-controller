...
void DnsManager::VdnsServersMsgHandler(std::string key, std::string context) {
    ...
    // Previous page link
    if((key != (vdns.begin())->first) && key != "AllEntries" ) {
        for (int i=0; i <= (sandesh_msg_limit-1); i++) {
            vdns_it--;
        }
...
void ShowVirtualDnsRecords::HandleRequest() const {
        DnsManager::VdnsRecordsMsgHandler(get_virtual_dns_server(), context());
}

void DnsManager::VdnsRecordsMsgHandler(std::string key, std::string context, bool show_all) {
    ...
    // Previous page link
    if((next_iterator_key != *vdns_config->virtual_dns_records_.begin()) && key != "AllEntries" ) {
        for (int i=0; i <= (sandesh_msg_limit-1); i++) {
...
void DnsManager::BindPendingMsgHandler(std::string key, std::string context) {
    ...
    //Next page link
    if(map_iter != map.end() && key != "AllEntries") {
        std::stringstream ss;
        ss << map_iter->first;
    ...
    // Previous page link
    if((index != (map.begin())->first) && key != "AllEntries" ) {
        for (int i=0; i <= (sandesh_msg_limit-1); i++) {
            map_it--;
        }
        std::stringstream ss;
...
void PageReq::HandleRequest() const {
    ...
    if(req_name == "VdnsServersReq") {
        ...
        DnsManager::VdnsRecordsMsgHandler(search_key, context(), true);
    }
    else {
