...
bool DnsManager::ResendRecordsinBatch() {
    static uint16_t start_index = pending_map_.begin()->first;

    PendingListMap::iterator start_it = pending_map_.lower_bound(start_index);
    PendingListMap::iterator stop_it =
        pending_map_.upper_bound(start_index+record_send_count_-1);

    PendingListMap::iterator it = start_it;
    for (; it != stop_it; ) {
         if (it == pending_map_.end()) break;
