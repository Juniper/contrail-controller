...
bool DnsManager::SendRetransmit(uint16_t xid, BindUtil::Operation op,
    ...
    if (it != pending_map_.end()) {
        if (it->second.successfully_sent) {
            DNS_BIND_TRACE(DnsBindTrace, "DNS timer fired while cleaning "
                           << "up entry, xid:" << xid);
...
void DnsManager::DeletePendingList(uint16_t xid) {
    PendingListMap::iterator it = pending_map_.find(xid);
    if (it != pending_map_.end()) {
        // Check as timer may have been fired
        it->second.successfully_sent = true;
        if (it->second.retry_pending_timer->Cancel()) {
            TimerManager::DeleteTimer(it->second.retry_pending_timer);
            pending_map_.erase(it);
        }
    }
...
void ShowBindPendingList::HandleRequest() const {
    ...
    if (dns_manager) {
        ...
        DnsManager::PendingListMap map = dns_manager->GetPendingListMap();
        ...
        for (DnsManager::PendingListMap::iterator it = map.begin();
            ...
            entry.set_successfully_sent(it->second.successfully_sent);
