...
void DnsManager::MakeSandeshPageReq(PageReqData *req, VirtualDnsConfig::DataMap &vdns, VirtualDnsConfig::DataMap::iterator vdns_it,
    ...
    // Previous page link
    if((key != (vdns.begin())->first) && key != "AllEntries" && key != "") {
...
void DnsManager::VdnsRecordsMsgHandler(const std::string &key,
    ...
    // Previous page link
    if((next_iterator_key != *vdns_config->virtual_dns_records_.begin()) && key != "AllEntries"  && key != "") {
...
void DnsManager::BindPendingMsgHandler(const std::string &key,
    ...
    // Previous page link
    if((index != (map.begin())->first) && key != "AllEntries" && key != "") {
        for (int i=0; i <= (sandesh_msg_limit-1); i++) {
            map_it--;
        }
        std::stringstream ss;
