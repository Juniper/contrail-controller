...
DnsManager::DnsManager()
    : bind_status_(boost::bind(&DnsManager::BindEventHandler, this, _1)),
      end_of_config_(false), record_send_count_(KMinBindRecordsSend),
...
bool DnsManager::SendRetransmit(uint16_t xid, BindUtil::Operation op,
...
bool DnsManager::ResendRecordsinBatch() {
    ...
    DNS_BIND_TRACE(DnsBindTrace, "*** ResendRecordsinBatch:" << record_send_count_);
    PendingListMap::iterator it = pending_map_.begin();
    for (; it != pending_map_.end(); ) {
         ...
         if (it->second.retransmit_count > 1)  retransmit = true;
         if (++send_count >= record_send_count_) break;
    }
    if (retransmit) {
        record_send_count_ = (record_send_count_/2 < KMinBindRecordsSend)? 
                                 KMinBindRecordsSend : record_send_count_/2;
    } else {
        record_send_count_ = ((record_send_count_* 2) > KMaxBindRecordsSend)?
                                 KMaxBindRecordsSend : record_send_count_* 2; 
    }

    if (it != pending_map_.end()) {
        pending_timer_->Reschedule(kPendingRecordReScheduleTime);
...
void DnsManager::AddPendingList(uint16_t xid, const std::string &view,
    ...
    pending_map_.insert(PendingListPair(xid, PendingList(xid, view, zone,
                                                         items, op)));
    StartPendingTimer(kPendingRecordRetransmitTime);
