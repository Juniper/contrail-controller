...
bool DnsManager::ResendRecordsinBatch() {
    ...
    if (current_it_ == pending_map_.end()) {
        current_it_ = pending_map_.begin();
    }

    PendingListMap::iterator it = current_it_;
    for (; it != pending_map_.end(); ) {
         ...
         // Limit the number of records sent to named based on hardware thread
         // count, i.e number of udp listeners on named
