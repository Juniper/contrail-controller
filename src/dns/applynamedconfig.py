...
def parse_contrail_dns_conf():
    ...
    subprocess.call(["sed -e 's/^[ \t]*//g' < ../contrail-dns.conf > contrail-dns-temp.conf"], shell=True)
    # remove comments preceeding with #
    subprocess.call(["sed -i 's/[;#].*$//g' contrail-dns-temp.conf"], shell=True)

    # parse contrail-dns.conf
    dns_config = ConfigParser.SafeConfigParser()
    dns_config.read('contrail-dns-temp.conf')
    
    # remove the temp file
    os.remove("contrail-dns-temp.conf")
