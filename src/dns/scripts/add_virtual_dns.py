...
class AddVirtualDns(object):
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument("--name", help = "Virtual DNS Name",required=True)
        parser.add_argument("--domain_name", help = "Domain Name",required=True)
        parser.add_argument("--dns_domain", help = "DNS Domain name",required=True)
        parser.add_argument("--dyn_updates", help = "Enable Dynamic DNS updates", action="store_true",required=True)
        parser.add_argument("--record_order", choices=['fixed', 'random', 'round-robin'], help = "Order used for DNS resolution",required=True)
        parser.add_argument("--ttl", type = int, help = "Time to Live for DNS records",required=True)
        parser.add_argument("--next_vdns", help = "Next Virtual DNS Server",required=True)
        parser.add_argument("--api_server_ip", help = "IP address of api server",required=True)
        parser.add_argument("--api_server_port", type = int, help = "Port of api server",required=True)
