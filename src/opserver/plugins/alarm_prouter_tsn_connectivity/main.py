...
class ProuterConnectivity(AlarmBase):
    """Prouter connectivity to controlling tor agent does not exist
       we look for non-empty value for tsn_connected_agent_list"""

    _RULES = '(ContrailConfig.elements.virtual_router_refs != null) AND ' +\
        '(ProuterData.tsn_connected_agent_list size!= 1)'
    ...
    def __call__(self, uve_key, uve_data):
        ...
        if v1 is not None:
            v2 = v1.get("tsn_connected_agent_list",None)
        if v2 is None:
            or_list.append(AllOf(all_of=[AlarmElement(\
                rule=AlarmTemplate(oper="==",
                    operand1=Operand1(keys=["ProuterData","tsn_connected_agent_list"]),
