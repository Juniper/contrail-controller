...
class Controller(object):
    ...
    def run(self):
        self.gevs = [ gevent.spawn(controller.run_cpu_mon),
                      gevent.spawn(controller.run_uve_processing)]

        if controller.disc:
            sp1 = ServicePoller(controller._logger, CollectorTrace,
                                controller.disc,
                                COLLECTOR_DISCOVERY_SERVICE_NAME,
                                controller.disc_cb_coll, controller._sandesh)

            sp1.start()
            self.gevs.append(sp1)

            sp2 = ServicePoller(controller._logger, AlarmgenTrace,
                                controller.disc, ALARM_GENERATOR_SERVICE_NAME,
                                controller.disc_cb_ag, controller._sandesh)
            sp2.start()
...
def main(args=None):
    ...
    controller.hub.signal(signal.SIGTERM, controller.sigterm_handler)
    controller.run()
