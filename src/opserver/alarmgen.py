...
class AlarmStateMachine:
    ...
    def set_alarms(self):
        ...
        if self.uas.state == UVEAlarmState.Soak_Idle:
            self.uas.state = UVEAlarmState.Active
            AlarmStateMachine.tab_alarms_timer[self.deleteTimeout].remove\
                    (self)
        elif self.uas.state == UVEAlarmState.Idle:
            if not self.uac.ActiveTimer:
                self.uas.state = UVEAlarmState.Active
                update_alarms = True
            elif self.uac.FreqExceededCheck:
                # log the timestamp
                if len(self.uas.alarm_timestamp) <= self.uas.head_timestamp:
                    self.uas.alarm_timestamp.append(self.uai.timestamp)
                else:
                    self.uas.alarm_timestamp[self.uas.head_timestamp] = \
                                    self.uai.timestamp
                self.uas.head_timestamp = (self.uas.head_timestamp + 1) % \
                                (self.uac.FreqCheck_Times + 1)
                #check if frquency exceeded
                if self.is_alarm_frequency_exceeded():
                    self.uas.state = UVEAlarmState.Active
                    update_alarms = True
            if self.uas.state != UVEAlarmState.Active:
                ...
                if not timeout_value in AlarmStateMachine.tab_alarms_timer:
                    AlarmStateMachine.tab_alarms_timer[timeout_value] = []
                AlarmStateMachine.tab_alarms_timer[timeout_value].append\
                        (self)
    ...
    def clear_alarms(self):
        ...
        if self.uas.state == UVEAlarmState.Soak_Active:
            ...
            AlarmStateMachine.tab_alarms_timer[self.activeTimeout].remove(self)
            if not to_value in AlarmStateMachine.tab_alarms_timer:
                AlarmStateMachine.tab_alarms_timer[to_value] = []
            AlarmStateMachine.tab_alarms_timer[to_value].append(self)
            ...
            if not self.uac.IdleTimer:
                ...
                self.uas.state = UVEAlarmState.Soak_Idle
                self.deleteTimeout = cur_time + self.uac.IdleTimer
    ...
    def is_alarm_frequency_exceeded(self):
        ...
        if (self.uas.alarm_timestamp[end] - self.uas.alarm_timestamp[start]) \
                / 1000000 <= self.uac.FreqCheck_Seconds:
    ...
    def run_uve_soaking_timer(self):
        ...
        if self.uas.state == UVEAlarmState.Soak_Active:
            if curr_time >= self.activeTimeout:
                ...
                timeout_value = -1
                update_alarm = True
                AlarmStateMachine.tab_alarms_timer[curr_time].remove(self)
        elif self.uas.state == UVEAlarmState.Soak_Idle:
            deleteTimerExpired = 0
            if self.deleteTimeout:
                deleteTimerExpired = curr_time - self.deleteTimeout
            if deleteTimerExpired >= 0:
                self.send_state_change_trace(self.uas.state, UVEAlarmState.Idle)
                self.uas.state = UVEAlarmState.Idle
                self.idleTimeout = curr_time + self.uac.IdleTimer
                timeout_value = self.idleTimeout
                update_alarm = True
                AlarmStateMachine.tab_alarms_timer[curr_time].remove(self)
                if not timeout_value in AlarmStateMachine.tab_alarms_timer:
                    AlarmStateMachine.tab_alarms_timer[timeout_value] = []
                AlarmStateMachine.tab_alarms_timer[timeout_value].append(self)
        elif self.uas.state == UVEAlarmState.Idle:
            idleTimerExpired = 0
            ...
            if idleTimerExpired >= 0:
                delete_alarm = True
            if ((self.tab, self.uv, self.nm) in AlarmStateMachine.tab_alarms_timer[curr_time]):
                AlarmStateMachine.tab_alarms_timer[curr_time].remove(self)
    ...
    @staticmethod
    def run_timers(curr_time):
        inputs = namedtuple('inputs', ['asm',
                            'delete_alarm', 'timeout_val', 'old_to'])
        delete_alarms = []
        update_alarms = []
        if curr_time in AlarmStateMachine.tab_alarms_timer:
            asmObjects = AlarmStateMachine.tab_alarms_timer[curr_time]
            update_timers = []
            for asm in asmObjects:
                ...
                update_timers.append(inputs(asm=asm, 
                                    delete_alarm=delete_alarm,
                                    timeout_val=timeout_val,
                                    old_to=curr_time))
            for timer in update_timers:
                if timer.timeout_val is not None or timer.delete_alarm:
                    AlarmStateMachine.update_tab_alarms_timer(timer.asm,
                                timer.old_to, timer.timeout_val)
        return delete_alarms, update_alarms

    @staticmethod
    def update_tab_alarms_timer(asm, curr_index, timeout_val):
        del_timers = []
        ...
        if timeout_val >= 0:
            if not timeout_val in AlarmStateMachine.tab_alarms_timer:
                AlarmStateMachine.tab_alarms_timer[timeout_val] = []
            if asm not in AlarmStateMachine.tab_alarms_timer[timeout_val]:
                AlarmStateMachine.tab_alarms_timer[timeout_val].append(asm)
...
class Controller(object):
    ...
    def run_alarm_timers(self, curr_time):
        delete_alarms, update_alarms = AlarmStateMachine.run_timers(curr_time)
