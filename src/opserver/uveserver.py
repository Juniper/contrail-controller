...
class UVEServer(object):
    ...
    def __init__(self, redis_uve_server, logger, api_port=None, redis_password=None):
        ...
        for t in _STAT_TABLES:
            ...
            squery = None 
            if t.obj_table and t.obj_table!='NONE':
                squery = {}
                squery['table'] = STAT_VT_PREFIX + "." + stat_id
                squery['where'] = [[{"name":STAT_OBJECTID_FIELD, "value":"", "op":7}]]
                squery['select_fields'] = ["COUNT(%s)" % t.stat_attr]
                 
            for aln in t.attributes:
                if aln.datatype in ['int','double']:
                    if squery:
                        squery['select_fields'].append("SUM(%s)" % aln.name)
                else:
                    if squery:
                        squery['select_fields'].append("%s" % aln.name)
            if squery:
                if not self._UVE_STATS.has_key(t.obj_table):
                    self._UVE_STATS[t.obj_table] = []
                self._UVE_STATS[t.obj_table].append(\
                        (t.stat_type, t.stat_attr, squery))
    ...
    def get_uve(self, key, flat, filters=None, multi=False, is_alarm=False):
        ...
        for redis_uve in self._redis_uve_list:
            ...
            try:
                ...
                if sfilter is None and mfilter is None and not multi \
                        and self._UVE_STATS.has_key(table) \
                        and not is_alarm and self._api_port:
                    url = OpServerUtils.opserver_query_url(
                        self._local_redis_uve[0],
                        str(self._api_port))
                    statdict['__STATS__'] = {}
                    if not self._UVE_STATS.has_key(table):
                        self._UVE_STATS[table] = []
                    for qtuple in self._UVE_STATS[table]:
                        q_type, q_attr, q_query = qtuple
                        if state[key].has_key(q_type) and \
                                state[key][q_type].has_key(q_attr):
                            try:
                                qdict = copy.deepcopy(q_query)
                                qdict["end_time"] = \
                                        OpServerUtils.utc_timestamp_usec()
                                qdict["start_time"] = \
                                        qdict["end_time"] - (3600 * 1000000)
                                json_str = json.dumps(qdict)
                                resp = OpServerUtils.post_url_http(\
                                        url, json_str, True)
                                if resp is not None:
                                    edict = json.loads(resp)
                                    edict = edict['value']
                                    statdict['__STATS__'][qdict[\
                                            'table']] = []
                                    statdict['__STATS__'][qdict[\
                                            'table']].append(edict) 
                            except Exception as e:
                                print "Stats Query Exception:" + str(e)
                    if len(statdict['__STATS__']) == 0:
                        del statdict['__STATS__']
