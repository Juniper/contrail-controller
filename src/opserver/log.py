...
class LogQuerier(object):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--object", help = "Logs of object type", choices=OBJECT_TABLE_LIST)
        parser.add_argument("--object-id", help = "Logs of object name")
        parser.add_argument("--object-select-field", help = "Select field to filter the log", \
        ...
        if self._args.last != None:
            self._args.last = '-' + self._args.last
            self._start_time = OpServerUtils.convert_to_utc_timestamp_usec(self._args.last)
            self._end_time = OpServerUtils.convert_to_utc_timestamp_usec('now')
        else:
            try:
                if self._args.start_time.isdigit() and self._args.end_time.isdigit():
                    self._start_time = int(self._args.start_time)
                    self._end_time = int(self._args.end_time)
                else: 
                    self._start_time = OpServerUtils.convert_to_utc_timestamp_usec(self._args.start_time)
                    self._end_time = OpServerUtils.convert_to_utc_timestamp_usec(self._args.end_time)
            except:
                print 'Incorrect start-time (%s) or end-time (%s) format' % (self._args.start_time, 
                                                                              self._args.end_time)   
                return -1
        return 0        
    #end parse_args
    ...
    # Public functions    
    def query(self):
        start_time, end_time = OpServerUtils.get_start_end_time(self._start_time,
                                                                self._end_time)
        messages_url = OpServerUtils.opserver_query_url(self._args.opserver_ip,
        ...
        if self._args.object is not None or self._args.object_id is not None or \
           ...
           if self._args.object_select_field is not None:
               if self._args.object_select_field != VizConstants.OBJECT_LOG and \
                  self._args.object_select_field != VizConstants.SYSTEM_LOG:
                   print 'Invalid object-select-field. Valid values are "%s" or "%s"' \
               ...
               self._args.object_select_field = obj_sel_field
                ...
                self._args.object_select_field = obj_sel_field = [VizConstants.OBJECT_LOG, VizConstants.SYSTEM_LOG]
           ...
           select_list = [ 
            ...
            if self._args.module is None:
                ...
                return None
            trace_buf_match = OpServerUtils.Match(name = VizConstants.CATEGORY,
            ...
            sandesh_type_filter = OpServerUtils.Match(name = VizConstants.SANDESH_TYPE,
    ...
    def display(self, result):
        ...
        for messages_dict in messages_dict_list:
            ...
            if VizConstants.TIMESTAMP in messages_dict:
                message_dt = datetime.datetime.fromtimestamp(int(messages_dict[VizConstants.TIMESTAMP])/OpServerUtils.USECS_IN_SEC)
                message_dt += datetime.timedelta(microseconds=(int(messages_dict[VizConstants.TIMESTAMP]) % OpServerUtils.USECS_IN_SEC))
            ...
            if self._args.object is None:
                ...
                if VizConstants.LEVEL in messages_dict:
                    level = SandeshLevel._VALUES_TO_NAMES[messages_dict[VizConstants.LEVEL]]
                else:
                    level = 'Level: NA'        
                ...
                if VizConstants.DATA in messages_dict:
                    ...
                    if self._args.raw:
                        ...
                        OpServerUtils.messages_xml_data_to_dict(messages_dict, VizConstants.DATA)
                        if isinstance(messages_dict[VizConstants.DATA], dict):
                            data_dict = messages_dict[VizConstants.DATA]
                            data_str = OpServerUtils.messages_data_dict_to_str(data_dict, message_type, sandesh_type)
                        else:
                            data_str = messages_dict[VizConstants.DATA]
                else:
                    data_str = 'Data not present'
                if self._args.trace is not None:
                    print '{0} {1}:{2} {3}'.format(message_ts, message_type, seq_num, data_str)
                else:
                    print '{0} {1} [{2}:{3}][{4}] : {5}:{6} {7}'.format(message_ts, source, module, category, level, message_type, seq_num, data_str)
            else:
                for obj_sel_field in self._args.object_select_field:
                   if obj_sel_field in messages_dict:
                       if self._args.raw:
                           ...
                           # Convert XML data to dict
                           OpServerUtils.messages_xml_data_to_dict(messages_dict, obj_sel_field)
                           if isinstance(messages_dict[obj_sel_field], dict):
                               ...
                               data_str = messages_dict[obj_sel_field]
                       if data_str:
                           print '{0} [{1}:{2}] : {3}: {4}'.format(message_ts, source, module, message_type, data_str)
    #end display    
