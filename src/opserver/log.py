    ...
    def run(self):
        try:
            ...
            if self._args.tail:
                ...
                end_time = OpServerUtils.convert_to_utc_timestamp_usec(self._end_time)
                start_time = OpServerUtils.convert_to_utc_timestamp_usec(self._start_time)
                result_list = []
                while int(end_time) - int(start_time) > 0:
                    ...
                    t = Thread(target=self.display, args=(result_list,))
                    t.start()
                    result = self.query()
                    if result == -1:
                        return
