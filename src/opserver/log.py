...
class LogQuerier(object):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--source", nargs="*", help="Logs from source address")
        ...
        parser.add_argument("--message-type", nargs="*", help="Logs of message type")
    ...
    def query(self):
        ...
        if self._args.module is not None:
            if not where_or_list:
                ...
                where_or_list = [where_msg + [module] for where_msg in where_or_list for module in module_list]
            ...
            if not where_or_list:
                for message_type in self._args.message_type:
                    ...
                    message_type_match = OpServerUtils.Match(
                        ...
                        value=val, op=oper)
                    where_or_list.append([])
                    where_or_list[-1].append(message_type_match.__dict__)
            else:
                message_type_list = []
                for message_type in self._args.message_type:
                    if message_type.endswith('*'):
