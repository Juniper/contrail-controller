...
class LogQuerier(object):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--tail-log","-f", help="Tail logs from now", action="store_true")
        ...
        parser.add_argument("--output-file", help="redirect output to file")
    ...
    # Public functions
    def query(self):
        if self._args.tail_log and (self._args.send_syslog or self._args.reverse or
               self._args.start_time or self._args.end_time):
            invalid_combination = " --tail-log"
            ...
            if self._args.object_values is False:
                if self._args.object_select_field is not None:
                    ...
                    if not isinstance(self._args.object_select_field, list):
                         obj_sel_field = [self._args.object_select_field]
                    if VizConstants.OBJECT_LOG or VizConstants.SYSTEM_LOG in obj_sel_field:
    ...
    def _output(self, log_str, sandesh_level):
        ...
        if self._args.output_file is not None:
            ...
            try:
                with open(self._args.output_file, "a") as output_log:
                    output_log.write(log_str)
                    output_log.write("\n")
                    return
            except:
                print "Exception occured when creating/opening file %s" % self._args.output_file
                return -1
        if self._args.send_syslog:
