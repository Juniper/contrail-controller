...
class TestAlarmGenConfigHandler(unittest.TestCase):
    ...
    def _alarm_config_change_callback(self, table_alarm_map):
        logging.info('alarm_config_change_callback: table_alarm_map: %s' %
            str(table_alarm_map))
    ...
    def test_handle_config_update(self):
        ...
        for test in tests:
            ...
            if test.output.table_alarm_map:
                alarmgen_config_handler._alarm_config_change_callback.\
                    assert_called_with(test.output.table_alarm_map)
            else:
                alarmgen_config_handler._alarm_config_change_callback.\
                    assert_not_called()
