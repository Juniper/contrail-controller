...
class AnalyticsTest(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def test_03_session_query(self):
        ...
        #generator_obj1.generate_session_samples()
        generator_object = [generator_obj]
        for obj in generator_object:
            assert vizd_obj.verify_session_samples(obj)
        #assert vizd_obj.verify_session_table(generator_object)
        assert vizd_obj.verify_session_series_aggregation_binning(generator_object)
        return True
