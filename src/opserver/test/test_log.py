...
class LogQuerierTest(unittest.TestCase):
    ...
    def test_6_long_query(self):
        ...
        del expected_result_dict['start_time']
        del expected_result_dict['end_time']
        self.assertEqual(self.query_expectations.times_called,len(query_list))
        self.assertEqual(self.display_expectations.times_called,len(query_list) + 1)
        self.assertEqual(self.process_start_expectations.times_called,len(query_list))
        ...
        self.assertEqual(int(query_list[len(query_list) - 1]['end_time']) - int(query_list[len(query_list) - 1]['start_time']),10*60*pow(10,6) - len(query_list) + 1)
        del query_list[len(query_list) - 1]['start_time']
        del query_list[len(query_list) - 1]['end_time']
        self.assertEqual(expected_result_dict, query_list[len(query_list) - 1])
