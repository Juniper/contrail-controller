...
sys.path.insert(1, vizdtestdir + '/../../../../src/analytics/test/utils/mockredis')
sys.path.insert(1, vizdtestdir + '/../../../../src/analytics/test/utils/mockcassandra')
sys.path.insert(1, vizdtestdir + '/../../../../build/debug/sandesh/library/python')
sys.path.insert(1, vizdtestdir + '/utils')
sys.path.insert(1, vizdtestdir + '/../opserver')
sys.path.insert(1, vizdtestdir + '/../opserver/sandesh')
...
class AnalyticsTest(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def test_03_vm_uve(self):
        ...
        vizd_obj = self.useFixture(AnalyticsFixture(logging, \
            vizdtestdir + '/../../../../build/debug', 0))
        assert vizd_obj.verify_on_setup()
        generator_obj = self.useFixture(GeneratorFixture("VRouterAgent", 
    ...
    def test_04_startup(self):
        ...
        assert vizd_obj.verify_on_setup()
        assert vizd_obj.verify_collector_obj_count()
        generator_obj = self.useFixture(GeneratorFixture("VRouterAgent", ("127.0.0.1", vizd_obj.get_collector_port()), logging, vizd_obj.get_opserver_port(), start_time))
        assert generator_obj.verify_on_setup()
