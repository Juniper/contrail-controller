...
class VerificationOpsSrv (VerificationUtilBase):
    ...
    def post_purge_query_json(self, json_str, sync=True):
        ...
        res = None
        try:
            purge_request_url = OpServerUtils.opserver_database_purge_query_url(self._ip, str(self._port))
            print purge_request_url
            print "query is: ", json_str
            resp = OpServerUtils.post_url_http(purge_request_url, json_str, sync)
            if resp is not None:
                res = json.loads(resp)
                res = res['status']
        except Exception as e:
