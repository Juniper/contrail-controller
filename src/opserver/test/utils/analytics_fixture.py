...
class AnalyticsFixture(fixtures.Fixture):
    ...
    def verify_collector_db_info(self, collector,
        ...
        try:
            ...
            #self.logger.info('collector read db_info_dict:%s' % (json.dumps(db_info_dict)))
            ...
            disk_usage_percentage_level = int(db_info['disk_usage_percentage_level'])
            pending_compaction_tasks_level = int(db_info['pending_compaction_tasks_level'])
            self.logger.info('collector exp  disk_usage_percentage:%u pending_compaction_tasks:%u'
                             ' disk_usage_percentage_level:%u pending_compaction_tasks_level:%u' % 
                             (disk_usage_percentage_out, pending_compaction_tasks_out,
                              disk_usage_percentage_level_out, pending_compaction_tasks_level_out))
            self.logger.info('collector read disk_usage_percentage:%u pending_compaction_tasks:%u'
                             ' disk_usage_percentage_level:%u pending_compaction_tasks_level:%u' % 
                             (disk_usage_percentage, pending_compaction_tasks,
                              disk_usage_percentage_level, pending_compaction_tasks_level))
            if ((disk_usage_percentage == disk_usage_percentage_out) and
                (pending_compaction_tasks == pending_compaction_tasks_out) and
                (disk_usage_percentage_level == disk_usage_percentage_level_out) and
                (pending_compaction_tasks_level == pending_compaction_tasks_level_out)):
                return True
        except Exception as err:
            self.logger.error('Exception: %s' % err)
        return False
    ...
    def set_opserver_db_info(self, opserver,
        ...
        try:
            ...
            #self.logger.info('opserver db_info_dict:%s' % (json.dumps(db_info_dict)))
            db_info = db_info_dict['DbInfo']
            disk_usage_percentage = int(db_info['disk_usage_percentage'])
            pending_compaction_tasks = int(db_info['pending_compaction_tasks'])

            self.logger.info('opserver exp  disk_usage_percentage:%u pending_compaction_tasks:%u' %
                             (disk_usage_percentage_out, pending_compaction_tasks_out))
            self.logger.info('opserver read disk_usage_percentage:%u pending_compaction_tasks:%u' %
                             (disk_usage_percentage, pending_compaction_tasks))
            if ((disk_usage_percentage == disk_usage_percentage_out) and
