...
class AnalyticsFixture(fixtures.Fixture):
    ...
    def verify_collector_db_info(self, collector):
        ...
        try:
            ...
            self.logger.info('db_info_dict:%s' % (json.dumps(db_info_dict)))
            db_info = (db_info_dict['db_info'])['DbInfo']
            db_usage = int(db_info['db_usage'])
            pending_compaction_tasks = int(db_info['pending_compaction_tasks'])
            self.logger.info('read db_usage:%u pending_compaction_tasks:%u' % (db_usage, pending_compaction_tasks))
