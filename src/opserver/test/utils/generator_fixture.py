...
class GeneratorFixture(fixtures.Fixture):
    ...
    def __init__(self, name, collector, logger, opserver_port, start_time=None):
        self._name = name
        self._logger = logger
    ...
    def send_flow_stat(self, flow, flow_bytes, flow_pkts, ts=None):
        ...
        flow_data = FlowDataIpv4(flowuuid=flow.flowuuid, direction_ing=flow.direction_ing,
    ...
    def generate_flow_samples(self):
        ...
        for i in range(self.flow_cnt):
            self.flows.append(FlowDataIpv4(flowuuid=str(uuid.uuid1()), 
                                        ...
                                        sourceip=0x0A0A0A01, destip=0x0A0A0A02,
                                        sport=i+10, dport=i+100, protocol=i/2))
        ...
        # 'psize'    - packet size
        flow_template = [
                            {'duration':60, 'tdiff':5, 'pdiff':1, 'psize':50},
                            {'duration':30, 'tdiff':4, 'pdiff':2, 'psize':100},
                            {'duration':20, 'tdiff':3, 'pdiff':3, 'psize':25},
                            {'duration':10, 'tdiff':2, 'pdiff':4, 'psize':75},
                            {'duration':5,  'tdiff':1, 'pdiff':5, 'psize':120}
                        ]
        ...
        self.flow_end_time = self.flow_start_time + (max_duration*self._KSECINMSEC)
    ...
    def send_vm_uve(self, vm_id, num_vm_ifs, msg_count):
        ...
        for num in range(num_vm_ifs):
            ...
            vm_if_stats.in_bytes = self._INITIAL_PKT_COUNT * self._BYTES_PER_PACKET
        ...
        for num in range(msg_count):     
            ...
            if num != 0:
                for vm_if_stats in vm_if_stats_list:
                    ...
                    vm_if_stats.in_bytes = vm_if_stats.in_pkts * self._BYTES_PER_PACKET
            ...
            self._logger.info('Sent UveVirtualMachineAgentTrace:%s .. %d' % (vm_id, num))
    #end send_uve_vm
