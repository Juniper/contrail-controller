...
curfile = sys.path[0]
sys.path.insert(0, sys.path[0] + '/../../../../build/debug/opserver/opserver')
from uveserver import UVEServer
from uveserver import ParallelAggregator
...
This function returns a mock sandesh dict
        ...
        3: optional list<string>            connected_networks (aggtype="union")
        4: optional i32                     total_virtual_machines (aggtype="sum")
...
def MakeUVEVirtualNetwork(
    ...
    if attached_policies is not None:
        if (not result['UVEVirtualNetwork'].has_key('attached_policies')):
    ...
    if connected_networks is not None:
        if (not result['UVEVirtualNetwork'].has_key('connected_networks')):
    ...
    if total_virtual_machines is not None:
        if (not result['UVEVirtualNetwork'].has_key('total_virtual_machines')):
    ...
    if total_acl_rules is not None:
        if (not result['UVEVirtualNetwork'].has_key('total_acl_rules')):
    ...
    if in_tpkts is not None:
        if (not result['UVEVirtualNetwork'].has_key('in_tpkts')):
    ...
    if in_stats is not None:
        if (not result['UVEVirtualNetwork'].has_key('in_stats')):
