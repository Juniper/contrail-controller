        ...
        for uve in uves:
            try:
                ...
                for node_dburl in node_dburls:
                    ...
                    db1 = json.loads(json.dumps({'Hostname': node_dburl['name']}))
                    if "DatabaseUsageInfo" in uve:
                        db2 = stats[uve]['database_usage'][0]
                    elif "DatabasePurgeInfo" in uve:
                        db2 = stats[uve]['stats'][0]
                        db2['request_time'] = time.strftime("%Hh:%Mm:%Ss",
                                      time.localtime(db2['request_time']))
    ...
    def start_purge(self, params):
        ...
        try:
            ...
            if ('purge_start_time' in res.keys()):
                res['purge_start_time'] = time.strftime("%Hh:%Mm:%Ss",
                                time.localtime(int(res['purge_start_time'])))
            for key in res:
