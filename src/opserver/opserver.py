...
class OpServer(object):
        ...
        if 'where' in request.json:
            ...
            if len(where_clause_list) == 1:
                if where_clause_list[0] == default_clause:
                    ...
                    request.json['where'].remove(default_clause)

        name_where_clause = []
        for where_clause in where_clause_list:
            for and_query in where_clause:
                if and_query['name'] == name:
                    name_where_clause.append(where_clause)
                    break
        for where_clause in name_where_clause:
            where_clause_list.remove(where_clause)
        for obj in obj_list:
            new_clause =  [{
                'name' : name,
                'value' : obj if not prefix else prefix + obj,
                'op' : op
            }]
            if len(name_where_clause) == 0 or only_default_clause or \
                    new_clause in name_where_clause:
                where_clause_list.append(new_clause)
        request.json['where'] = where_clause_list
    #end update_where_clause

    def get_uuid_from_uve(self, uve):
        if 'ContrailConfig' in uve.keys():
            if isinstance(uve['ContrailConfig']['elements'], dict):
                if 'uuid' in uve['ContrailConfig']['elements']:
                    uuid = uve['ContrailConfig']['elements']['uuid']
            if isinstance(uve['ContrailConfig']['elements'], list):
                if 'uuid' in uve['ContrailConfig']['elements'][0]:
                    uuid = uve['ContrailConfig']['elements'][0]['uuid']
            if not uuid:
                self._logger.error("not able to retrieve uuid from %s" %uves)
                return ""
            uuid = uuid.split('"')[1]
            return uuid
        else:
            self._logger.error("ContrailConfig not present in uve")
        return None
    #end get_uuid_from_uve
    ...
    def validate_user_token_check_perms(self, uves):
        if self._args.aaa_mode == AAA_MODE_RBAC and \
            ...
            if not user_token:
                return False
            uuid = self.get_uuid_from_uve(uves)
            if uuid == "":
                return False
            if not uuid:
                if not self._vnc_api_client.is_role_cloud_admin(user_token):
                    return False
            elif not self._vnc_api_client.is_read_permission(user_token, uuid):
                return False
        elif self._args.aaa_mode == AAA_MODE_CLOUD_ADMIN \
            ...
            if prg < 0:
                ...
                if request.get_header('Expect') == '202-accepted' or\
                   request.get_header('Postman-Expect') == '202-accepted':
                    ...
                    self._logger.error("in async mode %s" % href)
                    ...
                    for gen in self._sync_query(request, qid):
                        #self._logger.error("yielding %s " %gen)
                        yield gen
    # end _query
    ...
    def dyn_list_http_get(self, tables):
        ...
        try:
            ...
            if user_resources is not None:
                uve_list = set()
                for uve in self._uve_server.multi_uve_get(uve_tbl, True, \
                            filters, base_url=base_url):
                    if uve['name'] in user_resources:
                        uve_list.add(uve['name'])
            else:
