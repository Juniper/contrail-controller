...
class OpServer(object):
    ...
    def validate_user_token_get_perms(self, uves):
        ...
        if self._args.auth_conf_info.get('cloud_admin_access_only') and \
            ...
            if not user_token or not \
                ...
                return False
        return True
    #end validate_user_token_get_perms
    ...
    def dyn_http_get(self, table, name):
        ...
        if name.find('*') != -1:
            ...
            num = 0
            byt = 0
            uve_list = self._uve_server.multi_uve_get(uve_tbl, flat,
                                                      filters, base_url)
            for gen in uve_list:
                if self.validate_user_token_get_perms(gen):
                    dp = json.dumps(gen)
                    byt += len(dp)
                    if first:
                        yield u'' + dp
            ...
            _, rsp = self._uve_server.get_uve(uve_name, flat, filters,
                                           base_url=base_url)
            if self.validate_user_token_get_perms(rsp):
                dp = json.dumps(rsp)
