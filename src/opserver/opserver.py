...
class OpServer(object):
    ...
    def process_purge_request(self):
        ...
        for key, value in bottle.request.json.iteritems():
            if( (key == "purge_input") and (type(value) is int) and (value < 100) ):
                purge_input = value
            else:
                return bottle.HTTPError(_ERRORS[errno.EBUSY],
                    'purge_input not specified')

        if (self._analytics_db.number_of_purge_requests == 1):
    ...
    def db_purge_operation(self, purge_input, purge_id):
        try:
            ...
            return json.dumps({'status': 'started', 'Purge_id': purge_id})
        except Exception as e:
            self._logger.error("Exception: Purging failed %s" % e)
            return json.dumps({'status': 'failed', 'reason': e})
