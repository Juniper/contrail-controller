...
from sandesh.viz.constants import _TABLES, _OBJECT_TABLES, _OBJECT_TABLE_SCHEMA, _OBJECT_TABLE_COLUMN_VALUES
...
def redis_query_start(host, port, qid, inp):
    ...
    redish.hset("QUERY:" + qid, 'enqueue_time' , OpServerUtils.utc_timestamp_usec())
    redish.lpush("QUERYQ", qid)

    k,res = redish.blpop("REPLY:" + qid, 10)

    if k == None or res == None:
        return None
...
class OpStateServer(object):
    ...
    def redis_publisher_init(self):
        ...
        self._redis_pub_client = redis.StrictRedis(host=self._host, port=int(self._port), db=0)
...
class OpServer(object):
    """
    This class provides ReST API to get operational state of Contrail VNS system.
    ...
    def __new__(cls, *args, **kwargs):
        ...
        bottle.route('/analytics/virtual-networks', 'GET', obj.uve_list_http_get)
        bottle.route('/analytics/virtual-machines', 'GET', obj.uve_list_http_get)
        bottle.route('/analytics/service-instances', 'GET', obj.uve_list_http_get)
        ...
        bottle.route('/analytics/virtual-network/<name>', 'GET', obj.uve_http_get)
        bottle.route('/analytics/virtual-machine/<name>', 'GET', obj.uve_http_get)
        bottle.route('/analytics/service-instance/<name>', 'GET', obj.uve_http_get)
        bottle.route('/analytics/service-chain/<name>', 'GET', obj.uve_http_get)
        ...
        bottle.route('/analytics/query/<queryId>/chunk-final/<chunkId>', 'GET', obj.query_chunk_get)
        ...
        bottle.route('/analytics/table/<table>/schema', 'GET', obj.table_schema_process)
        for i in range(0, len(_TABLES)):
            if len(_TABLES[i].columnvalues) > 0:
                bottle.route('/analytics/table/<table>/column-values', 'GET', obj.column_values_process)
                bottle.route('/analytics/table/<table>/column-values/<column>', 'GET', obj.column_process)
        bottle.route('/analytics/send-tracebuffer/<source>/<module>/<name>', 'GET', obj.send_trace_buffer)
        bottle.route('/request-analyzer/<name>', 'GET', obj.analyzer_add_request)
        bottle.route('/delete-analyzer/<name>', 'GET', obj.analyzer_delete_request)
        bottle.route('/analyzers', 'GET', obj.analyzer_show_request)
        bottle.route('/request-mirroring/<name>', 'GET', obj.mirror_add_request)
        bottle.route('/delete-mirroring/<name>', 'GET', obj.mirror_delete_request)
        bottle.route('/mirrors', 'GET', obj.mirror_show_request)

        bottle.route('/documentation/<filename:path>', 'GET', obj.documentation_http_get)

        for uve in UVE_MAP: 
            bottle.route('/analytics/uves/' + uve + 's', 'GET', obj.uve_list_http_get)
            bottle.route('/analytics/uves/' + uve + '/<name>', 'GET', obj.uve_http_get)
    ...
    def __init__(self):
        ...
        self._homepage_links.append(LinkObject('documentation', '/documentation/index.html'))
        ...
        for t in _OBJECT_TABLES:
           obj = query_table(name = t, display_name = _OBJECT_TABLES[t].objtable_display_name,
               schema = _OBJECT_TABLE_SCHEMA, columnvalues = _OBJECT_TABLE_COLUMN_VALUES)
        ...
        bottle.route('/analytics/virtual-networks', 'GET', self.uve_list_http_get)
        bottle.route('/analytics/virtual-machines', 'GET', self.uve_list_http_get)
        bottle.route('/analytics/service-instances', 'GET', self.uve_list_http_get)
        bottle.route('/analytics/service-chains', 'GET', self.uve_list_http_get)
        ...
        bottle.route('/analytics/virtual-network/<name>', 'GET', self.uve_http_get)
        bottle.route('/analytics/virtual-machine/<name>', 'GET', self.uve_http_get)
        bottle.route('/analytics/service-instance/<name>', 'GET', self.uve_http_get)
        bottle.route('/analytics/service-chain/<name>', 'GET', self.uve_http_get)
        ...
        bottle.route('/analytics/query/<queryId>', 'GET', self.query_status_get)
        bottle.route('/analytics/query/<queryId>/chunk-final/<chunkId>', 'GET', self.query_chunk_get)
        ...
        bottle.route('/analytics/table/<table>/schema', 'GET', self.table_schema_process)
        for i in range(0, len(self._VIRTUAL_TABLES)):
            if len(self._VIRTUAL_TABLES[i].columnvalues) > 0:
                bottle.route('/analytics/table/<table>/column-values', 'GET', self.column_values_process)
                bottle.route('/analytics/table/<table>/column-values/<column>', 'GET', self.column_process)
        bottle.route('/analytics/send-tracebuffer/<source>/<module>/<name>', 'GET', self.send_trace_buffer)
        bottle.route('/request-analyzer/<name>', 'GET', self.analyzer_add_request)
        bottle.route('/delete-analyzer/<name>', 'GET', self.analyzer_delete_request)
        bottle.route('/analyzers', 'GET', self.analyzer_show_request)
        bottle.route('/request-mirroring/<name>', 'GET', self.mirror_add_request)
        bottle.route('/delete-mirroring/<name>', 'GET', self.mirror_delete_request)
        ...
        for uve in UVE_MAP: 
            bottle.route('/analytics/uves/' + uve + 's', 'GET', self.uve_list_http_get)
            bottle.route('/analytics/uves/' + uve + '/<name>', 'GET', self.uve_http_get)
    #end __init__
    ...
    def homepage_http_get(self):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc

        for link in self._homepage_links:
            json_links.append( \
                         {'link': obj_to_dict(LinkObject(link.name, base_url+link.href))}
                         )
    ...
    def documentation_http_get(self, filename):
        return bottle.static_file(filename, root='/usr/share/doc/python-vnc_opserver/html')
    ...
    def _query_status(self, request, qid):
        ...
        try:
            ...
            return bottle.HTTPResponse(json.dumps(resp), resp_code, resp_header)
    #end _query_status
    ...
    def _query_chunk(self, request, qid, chunk_id):
        try:  
            ...
            self._logger.info("Query [%s] chunk #%d read at time %d" % (qid, chunk_id, time.time()))
    #end _query_chunk

    def _query(self, request):
        reply = {}
        try:
            remote_addr, = struct.unpack('I', \
            ...
            if prg < 0:
                cod = -prg
                self._logger.error("Query Failed. Found Error %s" % errno.errorcode[cod])
                reply = bottle.HTTPError(status=501)
                reply = bottle.HTTPError(status=_ERRORS[cod])
                yield reply
                return
            else:
                self._logger.info("Query Accepted at time %d , Progress %d" % (time.time(), prg))
                ...
                if request.get_header('Expect') == '202-accepted' or \
                    ...
                    yield bottle.HTTPResponse(resp_data, 202, {'Content-type': 'application/json'})
                else:
    ...
    def _sync_query(self, request, qid):
        # In Sync mode, Keep polling query status until final result is available
        try:
            ...
            while not done:
                ...
                self._logger.info("Query Progress is %s time %d" % (str(resp), time.time()))
            ...
            # In Sync mode, its time to read the final result. Status is in "resp"
            done = False
            ...
            '''
            final_res = {}
            prg, final_res['value'] = redis_query_result_dict(host=self._args.redis_server_ip,
                    port=int(self._args.redis_query_port),
                    qid=qid)
            ...
            self._logger.info("Query Result available at time %d" % time.time())
        return
    #end _sync_query
    ...
    def show_queries(self):
        ...
        try:
            ...
            processing_queries = redish.lrange('ENGINE:'+socket.gethostname(), 0, -1)
    ...
    def _uve_filter_set(req):   
        ...
        return any_filter,sfilter,mfilter,tfilter,afilter
    ...
    def uve_http_get(self, name):
        ...
        try:
            ...
            any_filter, sfilter, mfilter, tfilter, afilter = OpServer._uve_filter_set(req)
            if any_filter:
                flat = True
            ...
            if name.find('*') != -1:
                ...
                for gen in self._uve_server.multi_uve_get(uve_name, flat,\
                        sfilter, mfilter, tfilter, afilter):
                    if first:
                ...
                rsp = self._uve_server.get_uve(uve_name, flat, \
                        sfilter, mfilter, tfilter, afilter)
    ...
    def uve_list_http_get(self):
        ...
        try:
            ...
            uve_list = self._uve_server.get_uve_list(uve_tbl, sfilter, mfilter, tfilter, afilter)
            base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/uves/%s/' % (uve_type)
            uve_links = [obj_to_dict(LinkObject(uve, base_url+uve+"?"+uve_filters)) for uve in uve_list]
    ...
    def analytics_http_get(self):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/'
        analytics_links = [obj_to_dict(LinkObject(link, base_url+link)) for link in self._analytics_links]
    ...
    def uves_http_get(self):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/uves/'
        uvetype_links = [obj_to_dict(LinkObject(uvetype+'s', base_url+uvetype+'s')) for uvetype in UVE_MAP]
        return json.dumps(uvetype_links)
    #end uves_http_get

    def analyzer_init(self):
        # TODO: Analyzer table and mirror table should be stored in cassandra/Redis
        # for persistency
        self._analyzer_tbl = {}
        self._mirror_tbl = {}
    #end analyzer_init
    ...
    def analyzer_add_request(self, name):
        ...
        try:
            ...
            response['vnc'] = 'http://%s:%s' % (analyzer['ip'], analyzer['vnc_port'])
    ...
    def mirror_add_request(self, name):
        ...
        if not req.apply_vn or not req.analyzer_name:
            ...
            response['error'] = '"apply_vn" and/or "analyzer_name" not specified'
            return json.dumps(response)
        try:
        ...
        if req.src_ip_prefix_len:
            try:
                ...
                print 'Invalid src ip prefix length [%s]' % (req.src_ip_prefix_len)
        ...
        if req.dst_ip_prefix_len:
            try:
                ...
                print 'Invalid dst ip prefix length [%s]' % (req.dst_ip_prefix_len)
        ...
        # If the prefix_len is not 32, then set the "destination source" to "*".
        # If the src_ip/dst_ip is not provided, set the "destination source" to "*".
        # We may want to optimize this by specifying the list of destinations which hosts
        # the VMs in the specified VN.
        # destination format is "source:module", where source is the hostname 
        dest = '*:'+ModuleNames[Module.VROUTER_AGENT]
        if self._state_server.redis_publish(msg_type='Request', destination=dest, msg=mirror_req):
    ...
    def tables_process(self):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/table/'
        json_links = []
        for i in range(0, len(self._VIRTUAL_TABLES)):
            link = LinkObject(self._VIRTUAL_TABLES[i].name, base_url + self._VIRTUAL_TABLES[i].name)
            tbl_info = obj_to_dict(link)
            tbl_info['type'] = self._VIRTUAL_TABLES[i].schema.type
            if (self._VIRTUAL_TABLES[i].display_name != None):
                tbl_info['display_name'] = self._VIRTUAL_TABLES[i].display_name
    ...
    def table_process(self, table):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/table/' + table + '/'
    ...
    def table_schema_process(self, table):
        ...
        for i in range(0, len(self._VIRTUAL_TABLES)):
            if (self._VIRTUAL_TABLES[i].name == table):
                return json.dumps(self._VIRTUAL_TABLES[i].schema, default=lambda obj: obj.__dict__)
    ...
    def column_values_process(self, table):
        ...
        base_url = bottle.request.urlparts.scheme + '://' + bottle.request.urlparts.netloc + '/analytics/table/' + table + '/column-values/'
        ...
        if ((column == 'ModuleId') or (column == 'Source')):
            try:
                redish = redis.StrictRedis(db=0,
                    host=self._args.redis_server_ip,
    ...
    def start_webserver(self):
        ...
        bottle.run(app = pipe_start_app, host = self._args.rest_api_ip, port = self._args.rest_api_port, server = 'gevent')
    #end start_webserver
    ...
    def cpu_info_logger(self):
        ...
        while True:
            ...
            mod_cpu_info.cpu_info = opserver_cpu_info.get_cpu_info(system=False)
