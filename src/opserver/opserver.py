...
class OpServer(object):
    ...
    def handle_db_info(self, disk_usage_percentage = None, pending_compaction_tasks = None):
        ...
        if self._state_server.redis_publish(msg_type='db-info',
            ...
            self._logger.info("redis-publish success for db_info usage(%u)"
                              ...
                              req.disk_usage_percentage, req.pending_compaction_tasks);
        else:
    ...
    def _auto_purge(self):
        ...
        while True:
            ...
            for node in db_node_usage:
                if (int(db_node_usage[node]) > int(self._args.db_purge_threshold)):
                    ...
                    self._logger.info("Database usage of %d on %s does not exceed threshold",
                            db_node_usage[node], node)
            # get max disk-usage value from dict
            disk_usage_percentage = None
            if (len(db_node_usage)):
                disk_usage_percentage = int(math.ceil(max(db_node_usage.values())))
            ...
            if ((disk_usage_percentage != None) or (pending_compaction_tasks != None)):
                self.handle_db_info(disk_usage_percentage, pending_compaction_tasks)
