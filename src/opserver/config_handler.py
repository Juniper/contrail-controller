...
class ConfigHandler(object):
    ...
    def _rabbitmq_subscribe_callback(self, notify_msg):
        ...
        fq_name = self._fqname_to_str(notify_msg['obj_dict']['fq_name'])
        if notify_msg['oper'] == 'CREATE' or notify_msg['oper'] == 'UPDATE':
            ...
            if cfg_obj:
                ...
                self._logger('config object %s not found in api-server' %
                    (fq_name), SandeshLevel.SYS_ERR)
        elif notify_msg['oper'] == 'DELETE':
            self._handle_config_update(cfg_type, fq_name, None,
                notify_msg['oper'])
