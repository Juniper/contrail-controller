...
class FlowQuerier(object):
    ...
    def __init__(self):
        ...
        self._ANALYZER_NAME = VizConstants.FlowRecordNames[
            FlowRecordFields.FLOWREC_ANALYZER_NAME]
        self._SEC_ANALYZER_NAME = VizConstants.FlowRecordNames[
    ...
    def output(self, output_dict):
        ...
        analyzer_name = output_dict['analyzer_name']
        sec_analyzer_name = output_dict['sec_analyzer_name']
        ...
        print '[SRC-VR:{0}{1}] {2} {3} {4} ({5} -- {6}) {7} '\
               ...
               nw_ace_uuid, tunnel_info, flow_uuid, analyzer_name, mirror_packets,
               mirror_bytes, sec_analyzer_name, sec_mirror_packets, sec_mirror_bytes)
    ...
    def display(self, result):
        ...
        for flow_dict in flow_dict_list:
            ...
            # Mirror Info
            if self._ANALYZER_NAME in flow_dict and\
                flow_dict[self._ANALYZER_NAME] is not None:
                analyzer_name = flow_dict[self._ANALYZER_NAME]
            else:
                analyzer_name = 'Mirror Analyzer: NA'
            ...
            if self._SEC_ANALYZER_NAME in flow_dict and\
                flow_dict[self._SEC_ANALYZER_NAME] is not None:
                sec_analyzer_name = flow_dict[self._SEC_ANALYZER_NAME]
            else:
                sec_analyzer_name = 'SMirror Analyzer: NA'
            ...
            output_dict['analyzer_name'] = analyzer_name
            output_dict['mirror_bytes'] = mirror_bytes
            output_dict['mirror_packets'] = mirror_packets
            output_dict['sec_analyzer_name'] = sec_analyzer_name
