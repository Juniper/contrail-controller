...
class AnalyticsDb(object):
    ...
    def connect_db(self):
        try:
            ...
            self._logger.error("Exception: Connection Failure %s" % e)
        try:
    ...
    def _update_analytics_start_time(self, start_time):
        try:
            ...
            self._logger.error("Exception: update_analytics_start_time Failure %s" % e)
    ...
    def purge_old_data(self, purge_time, purge_id):
        ...
        try:
            ...
            self._logger.error("Exception: Purge_id %s Unable to retrieve server and port%s" % (purge_id, e))
        ...
        for table in table_list:
            ...
            if (table not in excluded_table_list):
                ...
                try:
                    ...
                    self._logger.error("purge_id %s Failure in fetching columnfamily %s" % e)
                b = cf.batch()
                try:
