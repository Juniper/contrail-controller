...
class AnalyticsDb(object):
    def __init__(self, logger, cassandra_server_list,
                    redis_query_port, redis_password, cassandra_name,
                    cassandra_password):
        self._logger = logger
        ...
        self._pool = None
        self._cassandra_name = cassandra_name
    ...
    def connect_db(self):
        try:
             creds=None
             if self._cassandra_name is not None:
                 creds =  {'username':self._cassandra_name,'password':self._cassandra_password}
    ...
    def _get_sysm(self):
        ...
        for server_and_port in self._cassandra_server_list:
            try:
                sysm = pycassa.system_manager.SystemManager(server_and_port,credentials=creds)
            except Exception as e:
