...
class AnalyticsDb(object):
    ...
    def set_analytics_db_purge_status(self, purge_id, purge_input):
        ...
        redish.hset('ANALYTICS_DB_PURGE' + purge_id, 'status', 'running')
        redish.hset('ANALYTICS_DB_PURGE' + purge_id, 'purge_input', purge_input)
        redish.hset('ANALYTICS_DB_PURGE' + purge_id,
                    'purge_start_time', UTCTimestampUsec())
        redish.hset('ANALYTICS_DB_PURGE' + purge_id, 'purge_id', purge_id)
        redish.lpush('ANALYTICS_DB_PURGE_ID', purge_id)
    # end set_analytics_db_purge_status

    def delete_db_purge_status(self, purge_id):
        redish = redis.StrictRedis(db=0, host='127.0.0.1',
                                   port=self._redis_query_port)
        redish.delete('ANALYTICS_DB_PURGE' + purge_id)
        redish.delete('ANALYTICS_DB_PURGE_ID')
    # end delete_db_purge_status_on_restart
        ...
        for redis_ip_port in redis_list:
            ...
            if (res != []):
                if (redish.exists('ANALYTICS_DB_PURGE' + str(res[0]))):
                    return redish.hgetall('ANALYTICS_DB_PURGE' + str(res[0]))
        return None
