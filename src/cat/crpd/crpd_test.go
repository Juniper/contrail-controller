...
set protocols bgp group test allow 0.0.0.0/0`
    ...
    for i := 0; i < 3; i++ {
        retrieved_conf, err := sut.ShellCommandWithRetry(3, 3, "sudo", "--non-interactive", "docker", "exec", "-i", cr.Name, "cli", "show", "configuration", "|", "display", "set")
