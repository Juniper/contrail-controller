...
func (o *VirtualNetwork) AddChild(uuidTable *UUIDTableType, obj *ContrailConfigObject) {
    switch obj.Type{
...
func (o *VirtualNetwork) AddRef(uuidTable *UUIDTableType, obj *ContrailConfigObject) {
    switch obj.Type{
        case "network_ipam":
            u, _ := uuid.NewUUID()
...
func NewVirtualNetwork(fqNameTable *FQNameTableType, uuidTable *UUIDTableType, name string) (*VirtualNetwork, error) {
    co, err := createContrailConfigObject(fqNameTable, "virtual_network", name, "project", []string{"default-domain", "default-project", name})
    if err != nil {
        return nil, err
    }
    o := &VirtualNetwork{
