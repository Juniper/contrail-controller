...
func setup(c *cat.CAT, desc string, nc, na, ncrpd int, ConNodes map[string]int) error {
	...
	if desc == "ZIUTestcase" {
		i := 1
		for key, value := range ConNodesDS {
			if _, err := c.AddControlNode(desc, key, fmt.Sprintf("%d.0.0.%d", ipOctet, i), controlnode.GetConfFile(), value); err != nil {
				return err
			}
			i = i+1
		}
