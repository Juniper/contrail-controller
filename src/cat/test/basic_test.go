...
type ConfigMapType map[string]*config.ContrailConfigObject
...
func createVirtualNetwork(fqNameTable *config.FQNameTableType, uuidTable *config.UUIDTableType, configMap ConfigMapType, name, target string, network_ipam *config.ContrailConfigObject) (*config.VirtualNetwork, *config.RoutingInstance, error) {
    ...
    vn.AddChild(uuidTable, ri.ContrailConfigObject)
    return vn, ri, err
}
...
func generateConfiguration(cat *cat.CAT, configMap ConfigMapType, control_nodes []*controlnode.ControlNode) error {
    ...
    for i := range bgp_routers {
        for j := range bgp_routers {
            if i != j {
                bgp_routers[i].AddRef(&uuidTable, bgp_routers[j].ContrailConfigObject)
            }
        }
    }
    ...
    vmi1.AddRef(&uuidTable, vn1.ContrailConfigObject)
    vmi1.AddRef(&uuidTable, ri1.ContrailConfigObject)
    ...
    instance_ip1.AddRef(&uuidTable, vn1.ContrailConfigObject)
    instance_ip1.AddRef(&uuidTable, vmi1.ContrailConfigObject)
