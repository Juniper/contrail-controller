...
sys.path.append('../fabric-ansible/ansible-playbooks/manual_config')
...
class TestCliSyncFilters(test_case.JobTestCase):
    ...
    def mockCreateFile(self):
        try:
            path = "/root/contrail/build/debug/config/fabric-ansible/ansible" \
                   "-playbooks/manual_config/1.1.1.1"
            if not os.path.exists(path):
                os.makedirs(path)
            dirName = "ansible-playbooks/manual_config/1.1.1.1/"
