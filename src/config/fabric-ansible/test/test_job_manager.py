...
class TestJobManager(test_case.JobTestCase):
    ...
    def test_execute_job(self):
        # create job template
        play_info = PlaybookInfoType(playbook_uri='/build/supriyas/contrail-mainline/controller/src/config/fabric-ansible/job_manager_test.yaml',vendor='Juniper',device_family='MX')
        playbooks_list = PlaybookInfoListType(playbook_info=play_info)
        job_template = JobTemplate(job_template_type='device', job_template_job_runtime='ansible',
                                   job_template_multi_device_job=False, job_template_playbooks=playbooks_list,
                                   name='Test_template')
        ...
        job_input_json = {"job_template_id":job_template_uuid, "input":{"playbook_data": "some playbook data"}, 
                     "execution_id":"327870a6-b7cf-43cd-b4de-e151d477a073"}
    ...
    def mock_play_book_executor(self):
        facts_dict = {"localhost": {"output": {"status": "Success", "message": "Playbook executed successfully", 
