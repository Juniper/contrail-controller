...
# Definitions used by fabric_ansible_logger below
DEFAULT_ANSIBLE_LOG_PATH = '/tmp/ansible.log'
LOGGING_FORMAT = '%(asctime)s.%(msecs)03d %(name)s:  %(message)s'
DATE_FORMAT = "%m/%d/%Y %H:%M:%S"

# Context attribute along with it's abbeviation for logging.
# format is <context_attr> : <attr_abbreviation>
...
#
# fabric_ansible_logger()
#
# This is the common debug logger init method used by
# ansible internals and custom fabric ansible modules.
# It initializes the logger level and header format,
# including optional job context fields specified in
# the extra_list above. Note that setting
# logger=fabric_ansible_logger below is used to override
# the default logging display format specified in
# ansible/utils/display.py.
#
def fabric_ansible_logger(name, ctx=None):
    name = name
    ctx = ctx
    debug = C.DEFAULT_DEBUG
    verbosity = C.DEFAULT_VERBOSITY
    logfile = C.DEFAULT_LOG_PATH or DEFAULT_ANSIBLE_LOG_PATH
...
#
# This overrides the default Display() class defined in
# ansible/utils/display.py.
# Using this Display() we are able to customize the CLI and logging output.
# Note this this Display uses the customer "logger" defined above.
#
class Display:
        ...
        if self.b_cowsay:
            try:
                cmd = subprocess.Popen(
                    [self.b_cowsay, "-l"], stdout=subprocess.PIPE,
    ...
    def display(
        ...
        Note: msg *must* be a unicode string to
        prevent UnicodeError tracebacks.
        """
        ...
        if not log_only:
            ...
            msg2 = to_bytes(
                msg2, encoding=self._output_encoding(
                    stderr=stderr))
    ...
    def verbose(self, msg, host=None, caplevel=2):
        if self.verbosity > caplevel:
            # JPW
            if True:  # if host is None:
                self.display(msg, color=C.COLOR_VERBOSE)
            else:
                self.display(
                    "<%s> %s" %
                    (host, msg), color=C.COLOR_VERBOSE, screen_only=True)
    ...
    def deprecated(self, msg, version=None, removed=False):
        ...
        if not removed:
            if version:
                new_msg = "[DEPRECATION WARNING]: %s. This feature will be " \
                ...
                new_msg = "[DEPRECATION WARNING]: %s. " \
                          ...
                          "in a future release." % (
                              msg)
            new_msg = new_msg + " Deprecation warnings can " \
    ...
    def banner(self, msg, color=None, cows=True):
        '''
        Prints a header-looking line with cowsay or stars wit hlength
        ...
        if self.b_cowsay and cows:
            try:
                ...
                self.warning(
                    "somebody cleverly deleted cowsay or something "
                    "during the PB run.  heh.")
