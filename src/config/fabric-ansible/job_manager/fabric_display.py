...
# Include attributes in this list if you want them to appear in log message header
extra_list = {
...
def fabric_ansible_logger(name, ctx=None):
    ...
    # If log file is writable, init normal logger, otherwise use null logger to avoid logging errors
    if (logfile is not None and os.path.exists(logfile) and
                os.access(logfile, os.W_OK)) or os.access(os.path.dirname(logfile), os.W_OK):
        logging.basicConfig(filename=logfile, level=level, format=LOGGING_FORMAT, datefmt=DATE_FORMAT)
