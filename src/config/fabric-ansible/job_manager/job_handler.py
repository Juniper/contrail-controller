...
class JobHandler(object):
    ...
    PLAYBOOK_EXECUTION_TIMEOUT = 600
    ...
    def run_playbook_process(self, playbook_info):
        try:
            ...
            p = subprocess.Popen(["python", playbook_exec_path, "-i",
                                 ...
                                 close_fds=True, cwd='/')
            p.wait(timeout=self.PLAYBOOK_EXECUTION_TIMEOUT)
            ...
            msg = "Playbook execution timed out after %s sec for %s" %\
                  (str(self.PLAYBOOK_EXECUTION_TIMEOUT), playbook_info['uri'])
            raise JobException(msg, self._execution_id)
        except JobException:
            raise
        except Exception as e:
