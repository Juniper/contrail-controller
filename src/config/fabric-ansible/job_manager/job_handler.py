...
class JobHandler(object):
    ...
    def handle_job(self, result_handler, job_percent_per_task, device_id=None):
        try:
            ...
            # run the playbook
            self.run_playbook(playbook_info, result_handler.percentage_completed)
    ...
    def read_pr_object_log(self, exec_id, start_time, end_time):
        ...
        if resp.status_code == 200:
            ...
            for PRObjectLog in PRouterOnboardingLog:
                ...
                self._job_log_utils.send_prouter_job_uve(self._job_template.fq_name,
    ...
    def run_playbook_process(self, playbook_info, percentage_completed):
        ...
        try:
                                ...
                                for marker in markers:
                                    if marker in total_output:
                                        ...
                                        if marker == JOB_PROGRESS:
                                            ...
                                            self._job_progress = marked_jsons.get(JOB_PROGRESS)
                                            self.current_percentage += float(self._job_progress)
                                            self._job_log_utils.send_job_execution_uve(
                                                self._fabric_fq_name,
                                                self._job_template.fq_name, exec_id,
            ...
            self.read_pr_object_log(exec_id, pr_object_log_start_time, pr_object_log_end_time)
