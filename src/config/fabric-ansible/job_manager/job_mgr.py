...
def parse_logger_args(args_str=None):
    ...
    parser.add_argument("--push_delay_per_kb",
                        help="time delay between two successful "
                             "commits per kb config size")
...
def initialize_sandesh_logger():
    ...
    try:
        if args is not None:
            ...
            logger.info("Job Manager process is starting. "
                        "Sandesh is initialized.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # TODO the logs before the sandesh is initialized should go to some log file
    ...
    try:
        ...
        pass
        # print "Job input data is not passed to job manager.
        #  Aborting job. %s" % repr(e)
        # print "%s" % traceback.print_stack()
    ...
    try:
        if logger is not None and vnc_api is not None:
            ...
            logger.error("Failed to start job due to issue in "
                         "logger/api intialization")
    except Exception as e:
