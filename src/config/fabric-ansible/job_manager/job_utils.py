...
class JobUtils(object):
    ...
    def send_job_log(self, message, status, result=None, timestamp=None):
        try:
            ...
            self._logger.debug("Created job log for job template: %s, "
                               ...
                               " message: %s" % (
                self._job_template_id, self._job_execution_id,  status, result,
                message))
    ...
    def send_job_execution_uve(self, timestamp=None, percentage_completed=None):
        try:
            if timestamp is None:
                timestamp = strftime("%Y-%m-%d %H:%M:%S", gmtime())
            job_exe_data = JobExecution(name=self._job_template_id,
                                        execution_id=self._job_execution_id,
