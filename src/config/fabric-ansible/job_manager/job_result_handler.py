...
from job_utils import send_job_log, JobStatus
...
class JobResultHandler(object):
    ...
    def __init__(self, job_template, execution_id, logger):
    ...
    def update_job_status(self, output, device_id=None):
        try:
            ...
            if status == "Success":
                ...
                job_status = JobStatus.FAILURE
                err_msg = " No completion status received from the playbook. Marking the job as failed."
                if message is not None:
                    message += err_msg
                else:
                    message = err_msg

            if self._job_result_status is None or self._job_result_status==JobStatus.SUCCESS:
                self._job_result_status = job_status

            # update the result
            if device_id is None:
                device_id = self.DUMMY_DEVICE_ID
            self._job_result.update({device_id: self.DeviceJobResult(message, output)})
        except Exception as e:
            msg = "Error while updating the job status after playbook execution : %s" % repr(e)
            self._logger(msg)
    ...
    def create_job_summary_log(self, timestamp):
        ...
        # create the job log
        send_job_log(self._job_template.get_uuid(), self._execution_id, self._logger,
                     self._job_result_message, self._job_result_status, timestamp=timestamp)
        ...
        try:
            ...
            for entry in self._job_result:
                if entry == self.DUMMY_DEVICE_ID:
                    result_summary += self._job_result[self.DUMMY_DEVICE_ID].message
                else:
                    result_summary += "%s:%s \n" % (entry, self._job_result[self.DUMMY_DEVICE_ID].message)
            job_summary_message += result_summary

            return job_summary_message
        except Exception as e:
            msg = "Error while generating the job summary message : %s" % repr(e)
            self._logger(msg)
            self._logger(traceback.print_stack())
            raise JobException(msg)
