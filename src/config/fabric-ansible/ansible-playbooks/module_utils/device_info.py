    ...
    def initial_processing(self, concurrent):
        ...
        try:
            ...
            total_task_percentage = self.module.calculate_job_percentage(
                ...
                task_weightage_array=self.job_ctx.get('task_weightage_array'))[0]
            ...
            self.per_greenlet_percentage = self.module.calculate_job_percentage(
                concurrent,
        ...
        try:
            ...
            self.module.results['msg'] = "Failed to connect to API server due to " \
                                     "error: %s"\
                % str(ex)
        ...
        if oid_mapped:
            if self.serial_num_flag:
                if oid_mapped.get('serial-number') not in \
                        self.all_serial_num:
                    self.logger.info("Serial number {} for host {} not present "
                                     "in fabric_namespace, nothing to update "
                                     "in DB".format(oid_mapped.get(
                        'serial-number'), host))
                    return
            ...
            if return_code is True:
                self.vncapi.ref_update(
                    ...
                    None, "ADD")
                self.logger.info("Fabric updated with physical router info for "
                             "host: {}".format(host))
                temp = {}
