...
class FabricAnsibleModule(AnsibleModule):
    def __init__(self, argument_spec={}, **kwargs):
        ...
        self.job_utils = JobUtils(logger=self.logger)
    ...
    def calculate_per_greenlet_percentage(self, num_tasks, buffer_task_percent=False,
                                 task_seq_number=None, total_percent=100,
                                 task_weightage_array=None):
        return self.job_utils.calculate_job_percentage(
            num_tasks, buffer_task_percent, task_seq_number, total_percent,
            task_weightage_array)

    def job_log_percentage_to_file(self, job_log, percentage):
        write_to_file = {}

        if percentage:
            percentage_completed = percentage
        else:
            percentage_completed = self.calculate_job_percentage()

        write_to_file_log = "\n"
        try:
            write_to_file['percentage'] = percentage_completed
            write_to_file['message'] = job_log
            write_to_file_log = "Attempting to create or open file.. \n"
            with open("/tmp/"+self.job_ctx.get('job_execution_id'), "a") as f:
                write_to_file_log += "Opened file in /tmp ... \n"
                line_in_file = self.job_ctx.get('unique_pb_id') + \
                               'JOB_PROGRESS##' + json.dumps(write_to_file) \
                               + 'JOB_PROGRESS##'
            ...
            self.logger.info(write_to_file_log)
    ...
    def prouter_object_log_to_file(self, prouter_log_data):
        write_to_file = {}
        write_to_file_log = "\n"
        try:
            ...
            with open("/tmp/"+self.job_ctx.get('job_execution_id'), "a") as f:
                write_to_file_log += "Opened file in /tmp ... \n"
                line_in_file = self.job_ctx.get('unique_pb_id') + \
                               'PROUTER_LOG##' + json.dumps(write_to_file) + \
                               'PROUTER_LOG##'
                f.write(line_in_file + '\n')
                write_to_file_log += "Written line %s to the /tmp/exec-id" \
                                     " file \n" % line_in_file
