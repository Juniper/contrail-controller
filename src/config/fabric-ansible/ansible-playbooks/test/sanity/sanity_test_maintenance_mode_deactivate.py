...
class SanityTestHitless(SanityBase):
    ...
    def __init__(self, cfg):
        SanityBase.__init__(self, cfg, 'sanity_test_deactivate_maintenance_mode')
        self.fabric = cfg['fabric']
        self.advanced_params = cfg['advanced_params']
        self.device_name = cfg['mm_device']

    def test_maintenance_mode_deactivate(self):
        try:
            ...
            self.deactivate_maintenance_mode(device_uuid, fabric,
                                             self.advanced_params,
                                             [self.device_name])
        except Exception as ex:
            self._exit_with_error(
                "Deactivate maintenance mode failed due to unexpected error: %s"
                % str(ex))

if __name__ == "__main__":
    SanityTestHitless(config.load('sanity/config/test_config.yml',
                             'sanity/config/image_config.yml')).test_maintenance_mode_deactivate()
