...
class FilterModule(object):
    def filters(self):
        return {
            'runtime_interfaces_filter': self.rt_intf_filter,
            'configured_interfaces_filter': self.cf_intf_filter,
        }

    def rt_intf_filter(self, interface_list_new, prouter_name, regex_str=".*"):
        if not regex_str:
            regex_str = ".*"
    ...
    def cf_intf_filter(self, interface_list, prouter_name, regex_str=".*"):
        if not regex_str:
            regex_str = ".*"
