...
class FilterModule(object):
    ...
    def filters(self):
        return {
            ...
            'get_config_templates': self.get_config_templates
        }
    # end filters

    def _load_role_to_feature_mapping(self):
        group_vars = yaml.load(open('./group_vars/all.yml'))
    ...
    def _load_abstract_config(self):
        ...
        f = open(self.abstract_config_path)
        ab_cfg = f.read()
        f.close()
        self.abstract_config = json.loads(ab_cfg)

    def _prepare_build_env(self):
        self.build_config_path = self.conf_dir+'/build_config'
        self.final_config_path = self.conf_dir+'/final_config'
        shutil.rmtree(self.build_config_path, ignore_errors=True)
        ...
        os.mkdir(self.build_config_path)

    def _initialize(self):
        self.final_config = ""
        self.config_templates = []
        os.chdir('opt/contrail/fabric_ansible_playbooks')
        ...
        self.is_ztp = input['is_ztp']
        self.input = input

        self.conf_dir = '/opt/contrail/fabric_ansible_playbooks/config/' + self.device_mgmt_ip
    ...
    def _render_config(self):
        ...
        for feature in feature_list:
            ...
            if not feature_template:
                continue
            if 'underlay' in feature and not self.is_ztp:
                continue
        ...
        # Write to file
        with open(self.build_config_path + '/fabric_config.conf', 'w') as f:
    ...
    def _get_templates(self):
        ...
        for feature in feature_list:
            ...
            if 'underlay' in feature and not self.is_ztp:
                continue
            self.config_templates.append({
                'feature': feature,
                'template': feature_template,
                'vf_feature': vf_feature,
                'cfg_group': '__contrail_' + feature + '__',
                'feature_empty': False if feature in self.dev_feature_list
                    else True
        ...
        templates_output = {
            ...
            'device_username': self.device_username,
            'device_password': self.device_password,
            'device_abstract_config': self.abstract_config,
            'is_delete': self.is_delete,
            'results': {},
            'status': "success",
    ...
    def get_config_templates(self, job_ctx):
        self.job_ctx = job_ctx
        self._initialize()
        self._get_feature_list()
...
def _mock_job_ctx(is_delete, is_ztp):
    return {
...
def _parse_args():
    ...
    arg_parser.add_argument('-z', '--ztp',
                            action='store_true', help='ZTP fabric config')
    arg_parser.add_argument('-t', '--templates',
                            action='store_true', help='Templates only')
...
def __main__():
    ...
    elif parser.templates:
        results = fab_filter.get_config_templates(_mock_job_ctx(False, False))
