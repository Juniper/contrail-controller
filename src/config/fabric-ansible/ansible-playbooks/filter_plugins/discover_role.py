...
class FilterModule(object):
    ...
    def discover_role(self, job_ctx, prouter_name, prouter_uuid, prouter_vendor_name, prouter_product_name):
        role_discovery_log = "\n"
        try:
            vnc_lib = FilterModule._init_vnc_api(job_ctx)

            # form the hardware fqname from prouter_vendor_name and prouter_product_name
            hw_fq_name = [ (prouter_vendor_name + '-' + prouter_product_name).lower() ]
            ...
            role_discovery_log += "Getting all the node-profile back refs for the hardware: %s" % hw_fq_name[-1]
            np_back_refs = hw_obj.get_node_profile_back_refs()
            role_discovery_log += "done\n"

            # get the fabric object fq_name to check if the node-profile is in the same fabric
            role_discovery_log += "Fetching the fabric fq_name ..."
            fabric_fqname = job_ctx.get('fabric_fqname')
            fab_fq_name = fabric_fqname.split(":")
            role_discovery_log += "done\n"

            # read the fabric_object to get a list of node-profiles under this fabric_object
            ...
            role_discovery_log += "Getting the list of node-profile-uuids under this fabric object .... "
            ...
            # check to see which of the node-profile back refs are in the present fabric
            # Assumption: at present there is only a single node-profile that can match a hardware under the
            # current given fabric

            role_discovery_log += "Checking to see if any node-profile is under given fabric .... \n"
            upd_resp = self._do_role_discovery(np_back_refs, prouter_name, vnc_lib, prouter_uuid, node_profile_obj_uuid_list)
            ...
            self._logger.info(role_discovery_log)
            self._logger.error("Object not present in database: " + str(no_id_exc))
    ...
    def _do_role_discovery(self, np_back_refs, prouter_name, vnc_lib, prouter_uuid, node_profile_obj_uuid_list):
        upd_resp = ""
        for np_back_ref in np_back_refs or []:
            if np_back_ref['uuid'] in node_profile_obj_uuid_list:
                upd_resp += "Creating ref between %s and %s ...." % (prouter_name, np_back_ref['to'][-1])
                ref_upd_resp = vnc_lib.ref_update("physical_router", prouter_uuid, "node_profile",
                             np_back_ref['uuid'], None, 'ADD')
...
def _mock_pb_input_discover_role():
    ...
    return {
        "args": "{\"fabric_ansible_conf_file\": [\"/etc/contrail/contrail-keystone-auth.conf\", \"/etc/contrail/contrail-fabric-ansible.conf\"], \"collectors\": [\"10.155.75.102:8086\"]}",
...
if __name__ == '__main__':
    ...
    if parser.discover_role:
        ...
        results = fabric_filter.discover_role(_mock_job_ctx_discover_role(), pb_input['device_fqname'][-1],
                                              pb_input['device_id'], pb_input['vendor'], pb_input['product_name'])
    print results
