...
class FilterModule(object):
    ...
    show lldp neighbors command (lldp_neighbor_info). The lldp_neighbor_info can be
    ...
    The field: neighbor_map_info maintains the mapping of the local_interface_fqname
    to the remote_interface_info and the field: remote_neigbors_list will maintain
    ...
    corresponding (remote neighboring) prouters from a bulk query to vnc-db if
    lldp_neighbor_info is of format A.
    ...
    def lldp_neighbormap_filter(self, lldp_neighbor_info, prouter_fqname):
        ...
        needs_parsing = True
        # pilot sequence to determine what the structure of each lldp_neighbor_info is like
        ...
        for lldp_neighbor in lldp_neighbor_info:
            ...
            local_phy_int = lldp_neighbor.get('lldp-local-port-id') or lldp_neighbor.get('lldp-local-interface')
            phy_int_fqname.append(local_phy_int.replace(":", "_"))

            if needs_parsing:
                # ensure local interface is not a logical interface, remote interface need not be checked as port no
                # won't be imported
                ...
                lldp_remote_neighbor_port_desc = lldp_neighbor.get('lldp-remote-port-description').split("interface")[-1].strip()
                lldp_neighbor_fqname = []
                lldp_neighbor_fqname.extend(remote_prouter_fqname)
                lldp_neighbor_fqname.append(lldp_remote_neighbor_port_desc.replace(":", "_"))
                # ensure local interface and remote interfaces are not logical interfaces
                if '.' not in phy_int_fqname[-1] and '.' not in lldp_neighbor_fqname[-1]:
        ...
        return {"remote_neighbors_list": list(remote_neighbors_set), "neighbor_map_info_list": neighbor_map_info_list, "do_more_parsing": needs_parsing}
    ...
    def get_port_id_fqname_mapping(self, bulk_query_prouter_resp):
        ...
        for remote_phy_intfs in all_remote_phy_intfs:
            ...
            for phy_intf in phy_intfs:
                port_id = phy_intf.get('physical-interface-port-id')
    ...
    This filter takes inputs as the port_id_fqname_prouter_map and neighbor_map_info_list
    and returns the payload for bulk-ref-update (in vnc-module).
    ...
    and the remote_phy_int_fqname (fq_name has been chosen to maintain uniformity).
