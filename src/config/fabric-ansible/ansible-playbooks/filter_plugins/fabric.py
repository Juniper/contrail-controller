...
class FilterModule(object):
    ...
    def _read_device_obj(self, vnc_api, device_uuid, fields=None):
        device_obj = None
        try:
            device_obj = vnc_api.physical_router_read(
                id=device_uuid,
                fields=fields if fields else ['logical_router_back_refs']
            )
        except NoIdError:
            _task_done(
                'Device %s not found'
    ...
    def _read_service_appliance_obj(self, vnc_api, sa_uuid, fields=None):
        sa_obj = None
        try:
            sa_obj = vnc_api.service_appliance_read(
                id=sa_uuid,
                fields=fields if fields else ['physical_interface_refs']
            )
        except NoIdError:
            _task_done(
    ...
    def _get_virtual_network_refs(self, device_obj, fabric_obj):
        """
        ...
        for vn_ref in device_obj.get_virtual_network_refs() or []:
            ...
            if not self._auto_generated_virtual_network(vn_fq_name,
                                                        fabric_obj.name):
                vn_list.append(vn_fq_name[0]+':'+
                               vn_fq_name[1]+':'+
                               vn_fq_name[2])
        return vn_list
    # end _check_virtual_network_refs

    def _get_logical_router_refs(self, device_obj):
        """
        :param device_obj: <vnc_api.gen.resource_client.PhysicalRouter>
        :param fabric_obj: <vnc_api.gen.resource_client.Fabric>
        :return:
        ...
        for lr_ref in device_obj.get_logical_router_back_refs() or []:
            ...
            lr_list.append(lr_fq_name[0]+':'+
                           lr_fq_name[1]+':'+
                           lr_fq_name[2])
        return lr_list
    # end _check_logical_router_refs

    def _get_virtual_port_group_refs(self, vnc_api, fabric_obj):
        """
        :param vnc_api: <vnc_api.VncApi>
        :param fabric_obj: <vnc_api.gen.resource_client.Fabric>
        :return:
