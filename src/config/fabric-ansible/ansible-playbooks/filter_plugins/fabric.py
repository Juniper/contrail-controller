...
class FilterModule(object):
    ...
    def _check_pnf_service_refs(self, vnc_api, fabric_obj):
        """
        :param vnc_api: <vnc_api.VncApi>
        :param fabric_obj: <vnc_api.gen.resource_client.Fabric>
        :return:
    ...
    def _validate_fabric_deletion(self, vnc_api, fabric_obj, force_delete):
        ...
        if force_delete:
            _task_log('Validating no virtual networks, logical routers, or '
                      'virtual port groups created on the fabric')
            vn_list = []
            lr_list = []
            ...
            pnf_list = self._check_pnf_service_refs(vnc_api, fabric_obj)
            ...
            if pnf_list:
                err_msg += 'Service Appliances: {}, '.format(pnf_list)
            ...
            raise ValueError(
                'Failed to delete fabric %s due to references from '
