...
class FilterModule(object):
    ...
    def assign_roles(self, job_ctx):
        ...
        try:
            ...
            for device_roles in role_assignments:
                ...
                device_obj, log = self._add_logical_interfaces_for_fabric_links(
    ...
    def _add_logical_interfaces_for_fabric_links(
        ...
        :return: (device_obj, log):
            (<vnc_api.gen.resource_client.PhysicalRouter>, string)
        """
        device_obj = vnc_api.physical_router_read(
        ...
        fabric_name = str(fabric_back_refs[0].get('to')[-1])

        # get network-ipam object for the fabric network
        fabric_network_name = _fabric_network_name(
            fabric_name, NetworkType.FABRIC_NETWORK
        )
        fabric_network_obj = vnc_api.virtual_network_read(
            fq_name=['default-domain', 'default-project', fabric_network_name]
        )
        ...
        for link in self._get_device_fabric_links(vnc_api, device_obj) or []:
            ...
            try:
                ...
                log += 'Creating logical interface %s for physical link from' \
                       ' %s to %s ...' % (local_li.name, str(local_pi.fq_name),
                                          str(remote_pi.fq_name))
            ...
            if not iip_refs:
                ...
                try:
                    log += 'Create instance ip for logical interface %s ...' \
                           % local_li.fq_name
                    vnc_api.instance_ip_create(iip_obj)
                    log += 'done\n'
                except NoIdError:
                    log += 'instance ip already exists for logical interface' \
                           ' %s, hence updating it ...' % local_li.fq_name
    ...
    def _build_li_name(
        ...
        :return:
        """
        if device_obj.physical_router_vendor_name and device_obj.physical_router_vendor_name.lower() == 'juniper':
    ...
    def _assign_device_roles(self, vnc_api, log, device_roles):
        ...
        device_obj.physical_router_role \
            = device_roles.get('physical_router_role')
        device_obj.routing_bridging_roles \
            = device_roles.get('routing_bridging_roles')
        log += 'Assign roles to device "%s" ...' % device_obj.fq_name
