...
class FilterModule(object):
    ...
    def _get_logical_router_refs_for_fabric(self, fabric_obj):
        lr_list = set()
        for lr_ref in fabric_obj.get('logical_router_back_refs') or []:
            lr_fq_name = lr_ref.get('to')
            lr_list.add(':'.join(lr_fq_name))
        return lr_list
    ...
    def _check_object_references(self, vnc_api, fabric_obj, device_uuids,
        ...
        if not is_device_delete:
            ...
            master_lr_fqname = self.get_master_lr_in_list(
                vnc_api,
                lr_list,
                del_m_lr_from_list_if_exis=True)
            if master_lr_fqname:
                _task_log("Found master-LR during fabric deletion and "
