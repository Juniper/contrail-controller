...
class FilterModule(object):
    ...
    def _validate_mgmt_subnets(self, fab_fq_name, mgmt_subnets, brownfield):
        ...
        mgmt_tag = self.vnc_api.tag_read(fq_name=['label=fabric-management-ip'])
        for ref in mgmt_tag.get_fabric_namespace_back_refs() or []:
            namespace_obj = self.vnc_api.fabric_namespace_read(id=ref.get('uuid'))
    ...
    def _add_node_profiles(self, fabric_obj, node_profiles):
        ...
        for node_profile in node_profiles:
            ...
            np_fq_name = [GSC, name]
            node_profile_obj = self.vnc_api.node_profile_read(fq_name=np_fq_name)
    ...
    def _validate_fabric_deletion(self, fabric_obj):
        ...
        if fabric_obj and fabric_obj.get_physical_router_back_refs():
            vn_refs = self.vnc_api.virtual_networks_list().get('virtual-networks')
