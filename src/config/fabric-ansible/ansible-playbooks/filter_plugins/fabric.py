...
class FilterModule(object):
    ...
    def _validate_fabric_rr_role_assigned(
        ...
        for dev in fabric_devices:
            ...
            device_obj = vnc_api.physical_router_read(id=dev.uuid)
            phys_role = device_obj.get_physical_router_role()
            rb_roles = device_obj.get_routing_bridging_roles()
            if phys_role or rb_roles:
                no_role_assigned = False
            if phys_role == 'spine' and 'Route-Reflector' in (rb_roles or []):
                return
        ...
        raise ValueError(
            'No "Route-Reflector" role assigned to any spine devices in "%s". '
            'Please assign "Route-Reflector" role to at least one spine router '
            'and retry!' % fabric_fq_name[-1]
        )
