...
class FilterModule(object):
    ...
    def filters(self):
        ...
        return {
            ...
            'validate_device_functional_groups': self.validate_device_functional_group,
    ...
    def assign_roles(self, job_ctx):
        ...
        try:
            ...
            for device_roles in role_assignments:
                ...
                device_obj = device_roles.get('device_obj')
                self.update_physical_router_annotations(job_ctx,
                                                        device_obj.uuid)
        except Exception as ex:
            errmsg = str(ex)
            _task_error_log('%s\n%s' % (errmsg, traceback.format_exc()))
...
# ***************** validate dfg filter ***********************************
                ...
                if device_map[device_name].get('device_functional_group') != None:
                    ...
                    for item in existing_device_functional_list:
                        if item['fq_name'][2] == given_dfg:
                            dfg_uuid = item['uuid']
                            # delete the previous dfg reference before creating new one
                            self._delete_dfg_ref(job_ctx, prouter_uuid)
    ...
    def update_annotations(self, job_ctx, pr_uuid, role_assgn_dict):
        # Call function to validate supported phy and rb roles 
        self._check_rb_role_compatibity(job_ctx,pr_uuid,role_assgn_dict)
        fabric_info = job_ctx.get('job_input')
        fabric_fq_name = fabric_info.get('fabric_fq_name')
        vnc_api = JobVncApi.vnc_init(job_ctx)
        ...
        # Acquire lock to make sure no overlapping entries happen 
        acquired = False
...
# ***************** update physical router annotations filter # ****************************
    def update_physical_router_annotations(self, job_ctx, pr_uuid):
        ...
        if dfg_refs!= None:
            ...
            # Match the image
            if dfg_item_dict.get('device_functional_os_version')!= None:
                pr_os_version = pr_obj.physical_router_os_version
                if pr_os_version!= dfg_item_dict.get(
                        'device_functional_os_version'):
                    set_dfg_flag = True
            # Match physical role
            if dfg_item_dict.get('physical_role_refs')!= None:
                ...
                if pr_physical_role!= dfg_physical_role:
                    set_dfg_flag = True
            # Match rb roles -lo wer case nnd also compare rb roles
            if dfg_item_dict.get('device_functional_group_routing_bridging_roles')!= None:
                dfg_rb_roles_dict = dfg_item_dict.get('device_functional_group_routing_bridging_roles')
    ...
    # end update_physical_router_annotations
    def _delete_dfg_ref(self, job_ctx, pr_uuid):
        vnc_api = JobVncApi.vnc_init(job_ctx)
        try:
            pr_obj= vnc_api.physical_router_read(id=pr_uuid)
            dfg_refs = pr_obj.get_device_functional_group_refs()
    ...
    # Check rb and phy role compatibility
    def _check_rb_role_compatibity(self, job_ctx, pr_uuid, role_assgn_dict):
        vnc_api = JobVncApi.vnc_init(job_ctx)
        pr_obj = vnc_api.physical_router_read(id=pr_uuid)
        ...
        if not node_profile_refs:
            ...
            supported_roles_np = node_profile_obj.get_node_profile_roles().get_role_mappings()
