...
class FilterModule(object):
    ...
    def onboard_fabric(self, job_ctx):
        ...
        try:
            ...
            self._add_fabric_vn(vnc_api, fabric_obj, 'management', mgmt_subnets)
            onboard_log += "done\n"

            onboard_log += "Creating management network for the fabric ... "
            self._add_fabric_vn(vnc_api, fabric_obj, 'loopback', loopback_subnets)
            onboard_log += "done\n"

            onboard_log += "Creating management network for the fabric ... "
            peer_subnets = self._carve_out_peer_subnets(fabric_subnets)
            self._add_fabric_vn(vnc_api, fabric_obj, 'ip-fabric', peer_subnets)
            onboard_log += "done\n"
            ...
            return {
                'status': 'success',
                'fabric_uuid': '12345',
                'fabric': vnc_api.obj_to_dict(fabric_obj)
            }
    ...
    def _add_fabric_vn(self, vnc_api, fabric_obj, network_type, subnets):
        # create vn and ipam
        network_name = '%s-%s-network' % (fabric_obj.name, network_type)
        network = self._add_virtual_network(vnc_api, network_name)
        ipam_name = '%s-%s-network-ipam' % (fabric_obj.name, network_type)
...
if __name__ == '__main__':
    ...
    results = fabric_filter.onboard_fabric(job_ctx)
    print results
