...
class NodeProfileCache(object):
    def __init__(self, node_profile):
        self.node_profile_json = {
            "node_profile_device_family":
                node_profile.node_profile_device_family,
            "display_name": node_profile.display_name,
            "node_profile_vendor": node_profile.node_profile_vendor,
            "node_profile_hitless_upgrade":
                node_profile.node_profile_hitless_upgrade
        }
...
class PhysicalRouterCache(object):
    ...
    def __init__(self, pr, remote_vnc_admin_pwd):

        self.node_profile_fq_name_list = list()
        self.physical_interface_list = list()
        self.logical_interface_list = list()

        # read PR credentials as clear text
        user_name = None
        password = None
        user_credentials = pr.physical_router_user_credentials.get_password()
        if user_credentials is not None:
            user_name = pr.physical_router_user_credentials.get_username()
            password = pr.physical_router_user_credentials.get_password()
            if password is not None:
                password = JobVncApi.decrypt_password(
                    encrypted_password=password,
                    admin_password=remote_vnc_admin_pwd)
        ...
        self.pr_json = {
            ...
            # "physical_router_management_mac": pr.physical_router_management_mac
            }
...
class DataCache(object):
    ...
    # the topology
    """
    Mapping of the PI fqname to another PI fqname
    {
        "default-global-system-config:5a12-qfx11:xe-0/0/46":
...
class FilterModule(object):
    ...
    def migrate_fabric_data(self, job_ctxt):
        ...
        try:
            ...
            self.remote_vnc_params = vnc_params
            remote_job_ctxt = {
    ...
    def _migrate_all_fabric(self, remote_vnc_api, local_vnc_api,
        ...
        try:
            result = remote_vnc_api.fabrics_list(fields=['fq_name']) or\
                     {"fabrics": []}
            fabric_list = result.get("fabrics")
            if fabric_list is None:
                raise Exception("Fabric list is None")
        except Exception as exception:
        ...
        for fabric in fabric_list:
            ...
            try:
                ...
                # if the fabric has refs to new node profiles, create them
                FilterModule._migrate_node_profiles(
                    fabric_obj, remote_vnc_api, local_vnc_api)
                ...
                self._migrate_fabric_devices(fabric_obj, local_fabric_obj,
    ...
    @staticmethod
    def _migrate_node_profiles(fabric_obj, remote_vnc_api,
                               local_vnc_api):
        _task_log("Migrating node profiles for %s " % fabric_obj.fq_name)
        node_profile_refs = fabric_obj.get_node_profile_refs()
        if node_profile_refs is not None:
            for node_profile_ref in node_profile_refs:
                # check if node profile exists in the local server
                try:
                    local_vnc_api.node_profile_read(
                        fq_name=node_profile_ref.get("to"))
                except NoIdError:
                    # create node profile
                    _task_log("Migrating node profile %s " %
                              node_profile_ref.get("to"))
                    remote_node_profile = remote_vnc_api.node_profile_read(
                        fq_name=node_profile_ref.get("to"))
                    cls = JobVncApi.get_vnc_cls("physical_router")
                    node_profile_json =\
                        NodeProfileCache(remote_node_profile).node_profile_json
                    new_node_profile = cls.from_dict(**node_profile_json)
                    local_node_profile = local_vnc_api.node_profile_create(
                        new_node_profile)

                    # create refs to role config
                    role_config_refs = remote_node_profile.get_role_configs()
                    if role_config_refs is not None:
                        for role_config in role_config_refs:
                            local_vnc_api.ref_update(
                                "node_profile", local_node_profile.uuid,
                                "role_config", None, role_config.get("to"),
                                'ADD')

                    # create refs to hardware
                    hardware_refs = remote_node_profile.get_hardware_refs()
                    if hardware_refs is not None:
                        for hardware_ref in hardware_refs:
                            local_vnc_api.ref_update(
                                "node_profile", local_node_profile.uuid,
                                "hardware", None, hardware_ref.get("to"),
                                'ADD')
                    _task_done()
        _task_done()
    # end _migrate_node_profiles
    ...
    def _get_fabric_credentials(self, fabric):
        credential_list = []

        if fabric.get_fabric_credentials() is not None and\
                fabric.get_fabric_credentials().get_device_credential()\
                is not None:
            device_credentials = \
                fabric.get_fabric_credentials().get_device_credential()
            for credential in device_credentials:
                password = credential.get_credential().get_password()
                user_name = credential.get_credential().get_username()
                if password is not None:
                    password = JobVncApi.decrypt_password(
                        encrypted_password=password,
                        admin_password=self.remote_vnc_params.get(
                            "admin_password"))
                credential_dict = {
                    "username": user_name,
                    "password": password
                }
                credential_list.append(credential_dict)
        ...
        if namespaces is not None:
            for namespace in namespaces:
                ...
                if namespace_type == "management-subnets":
                    ...
                    fabric_asn_pool = self._get_asn_range_list(
                        remote_vnc_api, namespace_uuid)
        node_profiles = FilterModule._get_node_profiles(fabric)
        device_credentials = self._get_fabric_credentials(fabric)
        job_input_json = {
    ...
    def _migrate_fabric_devices(self, remote_fabric_obj, local_fabric_object,
                                remote_vnc_api, local_vnc_api):
        ...
        if pr_refs is not None:
            for pr_ref in pr_refs:
                try:
                    ...
                    _task_log("Importing physical router")
                    # import PR
                    self._import_physical_router(pr_cache, local_fabric_object,
                                                 local_vnc_api)
                    _task_done()
                except Exception as exception:
    ...
    def _export_physical_router(self, remote_vnc_api, pr_uuid, fabric_fqname):
        ...
        pr_cache = PhysicalRouterCache(
            pr, self.remote_vnc_params.get('admin_password'))

        # read the ref to node profile
        node_profile_refs = pr.get_node_profile_refs()
        if node_profile_refs is not None:
            for node_profile_ref in node_profile_refs:
                pr_cache.node_profile_fq_name_list.append(
                    node_profile_ref.get("to"))
    ...
    def _read_pr_role_info(self, pr, fabric_fqname):
        ...
        if pr.get_physical_router_role() is not None and rb_roles is not None\
                and len(rb_roles) > 0:
            pr_role_dict = dict()
            ...
            _task_debug_log("Role info for the PR"
                            " %s " % json.dumps(pr_role_dict))
        else:
            _task_error_log("Role info not complete for PR %s " % pr.fq_name)
    ...
    def _import_physical_router(self, pr_cache, local_fabric, local_vnc_lib):
        pr_obj = FilterModule._create_pr(pr_cache.pr_json, local_vnc_lib)
    ...
    @staticmethod
    def _create_pr(pr_json, local_vnc_lib):
    ...
    def _create_node_profile_ref(pr_obj, pr_cache, local_vnc_lib):
        for node_profile_fq_name in pr_cache.node_profile_fq_name_list:
            local_vnc_lib.ref_update(
                "physical_router", pr_obj.uuid, "node_profile", None,
                node_profile_fq_name, 'ADD')
    ...
    def _assign_roles(self, fabric_fq_name, local_vnc_api):
        ...
        if job_input is None:
            msg = "Role assignment data not found for %s " % fabric_fq_name
            _task_info_log(msg)
            _task_done(msg)
            return
