...
class FilterModule(object):
    ...
    def get_hitless_upgrade_plan(self, job_ctx, image_upgrade_list,
        ...
        try:
            ...
            upgrade_plan, first_batch = self._get_hitless_upgrade_plan()
            return upgrade_plan, first_batch
    ...
    def _get_hitless_upgrade_plan(self):
        ...
        # Create the first device_json batch to return to job manager
        first_batch = self._create_first_batch()
        return upgrade_plan, first_batch
    ...
    # Create device_json for first batch
    def _create_first_batch(self):
        first_batch = {}
        if len(self.batches) > 0:
            for device_uuid in self.batches[0]['device_list']:
                first_batch[device_uuid] = self.device_table[device_uuid]['basic']
        return first_batch
    # end _create_first_batch
...
def __main__():
    ...
    upgrade_plan, first_batch = hitless_filter.get_hitless_upgrade_plan(job_ctx,
                                                      mock_image_upgrade_list,
                                                      batch_limit=4)
    print json.dumps({
