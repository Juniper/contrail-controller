...
class FilterModule(object):
    ...
    def filters(self):
        return {
            ...
            'hitless_pop_batch': self.pop_batch,
        }
    # end filters
    ...
    # Pop the next batch off the batch list
    def pop_batch(self, job_ctx, upgrade_plan):
        batch = {}
        idx = upgrade_plan['batch_idx']
        if len(upgrade_plan['batches']) >= idx+1:
            for device_uuid in upgrade_plan['batches'][idx]['device_list']:
                batch[device_uuid] = upgrade_plan['device_table'][device_uuid]['basic']
                upgrade_plan['batch_idx'] += 1
        return batch
    # end pop_batch
...
def _parse_args():
    ...
    arg_parser.add_argument('-b', '--pop_batch',
                            action='store_true', help='Pop Next Batch')
...
def __main__():
    ...
    elif parser.pop_batch:
        batch = hitless_filter.pop_batch(mock_job_ctx, mock_upgrade_plan)
