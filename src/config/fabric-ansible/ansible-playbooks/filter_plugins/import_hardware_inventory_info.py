...
class FilterModule(object):
    ...
    def _create_hardware_inventory(self,
        ...
        hardware_iven_obj = prouter_vendor+ "_" +pr_product_name
        json_data = hardware_inventory_payload['inventory_info']
        hardware_inventory_payload_final = {
            ...
            "fq_name": [
                ...
                prouter_name,
                hardware_iven_obj],
            "hardware_inventory_inventory_info": json_data
        }
    ...
    # try to update the existing hardware existing object else create new one.
        try:
            try:
                ...
                inventory_id = vnc_lib.hardware_inventory_create(inventory_obj)

        except Exception as exc:
            _task_error_log(str(exc))
            _task_error_log(traceback.format_exc())
            warn_info = dict()
            warn_info['failed_operation'] = 'Device hardware inventory Creation'
            warn_info['failure_message'] = 'Error while creating hardware ' \
                                           'object ' + str(exc)
