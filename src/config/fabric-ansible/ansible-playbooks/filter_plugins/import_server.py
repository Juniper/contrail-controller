...
class FilterModule(object):
    ...
    def generate_node_name(self, port_list):
        ...
        if generated_hostname == "":
            ...
            if len(port_list) > 0:
                first_mac = port_list[0]['mac_address']
                generated_hostname = "auto-" + first_mac.replace(":", "")[6:]
    ...
    def import_nodes(self, data, cc_node_obj):
        ...
        if isinstance(data, dict) and "nodes" in data:
            ...
            for node_dict in node_list:
                node_payload, node_name, port_list = \
                    self.create_cc_node(node_dict, cc_node_obj)
                if node_name == "":
                    self._logger.warn("IGNORING Creation of : " +
                                      pformat(node_dict))
                    continue
    ...
    def import_nodes_from_file(self, job_ctx):
        """
        Job for importing nodes/ports from file.
        ...
        job_ctx:
            ...
            'job_input': { 'file_format': <json/yaml>,
                           'contrail_command_host': <contrail-command-host>,
                           'encoded_file': <base64 encoded yaml/json data>
                         }
        """
        try:
