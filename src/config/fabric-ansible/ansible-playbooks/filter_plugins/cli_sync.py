...
class FilterModule(object):
    ...
    def _instantiate_filter_log_instance(self, device_name):
        FilterLog.instance("Cli sync filter",
                           device_name)

    # Function to update cli object with cli diff list info
    def create_cli_obj(self, job_ctx, device_mgmt_ip, device_name):
        directory_path = "/opt/contrail/fabric_ansible_playbooks/manual_config/"+device_mgmt_ip
        ...
        try:
            try:
                ...
                existing_cli_obj = vnc_lib.cli_config_read(fq_name =
                                                           cli_config_payload.get('fq_name'))
                updated_cli_obj = self._cli_config_obj_update(job_ctx, None,
                                                              existing_cli_obj,
                                                              file_list)
            except NoIdError:
            ...
            raise Exception ("Either cli object creation or updation has failed with "
                             "exception: %s",str(exc))
    ...
    def _cli_config_obj_update(self, job_ctx, new_cli_uuid, existing_cli_dict,
        ...
        try:
            ...
            raise Exception ("Either cli object updation has failed with "
                             "exception: %s",str(exc))
        ...
        try:
            ...
            for dict_item in devices_cli:
                ...
                #Read the referenced cli object from PR
                    ...
                    cli_fq_name = ['default-global-system-config', prouter_name,
                                   cli_obj_name]
                    ...
                    for pr_commit_item in pr_commit_diff_list:
                        for cli_item in device_cli_objects:
                            if cli_item.get('time') == pr_commit_item.get('time'):
                                pr_commit_processed_list.append(pr_commit_item)
                                action_to_be_performed = cli_item.get('status')
                                if action_to_be_performed == "accept":
                                    partial_accepted_config = self._accept_routine(pr_commit_item,
                                                          device_mgmt_ip)
                                else:
                                    self._reject_routine(pr_commit_item,
                                                          device_mgmt_ip)
            ...
            data_dict = { "pr_commit_diff_list": pr_commit_diff_list,
    ...
    def merge_files(self, device_mgmt_ip):
        path_to_files = "/opt/contrail/fabric_ansible_playbooks/manual_config" \
                        "/" + device_mgmt_ip
    ...
    def _process_file_for_delete_commands(self, partial_acc_config,
        ...
        path_to_file = "/opt/contrail/fabric_ansible_playbooks/manual_config" \
    ...
    def final_set_command_approve_case(self, lcommands, leaf, device_mgmt_ip,
        ...
        try:
            ...
            file_to_write.close()
        if ("__cli_contrail_group__" in final_command):
        ...
        for i in range(len(l)):
            ...
            if (elem.startswith('!')):
                ...
                if ("inactive" in clean_elem):
                    ...
                    accept_config = self.final_set_command_approve_case(linactive,
                                                           clean_elem,
            ...
            if (elem.startswith('+')):
                ...
                if ';' in clean_elem:
                    if group:
                        line_out[0] = "delete"
                        accept_config = self.final_set_command_approve_case(line_out,
                                                             clean_elem.split(
                            ";")[0],device_mgmt_ip, accept_config)
                        group_out = line_out[:]
                        ...
                        accept_config = self.final_set_command_approve_case(group_out,
                                                             clean_elem.split(
                            ";")[0],device_mgmt_ip, accept_config)
                    else:
                        accept_config = self.final_set_command_approve_case(line_out,
            ...
            if (elem.startswith('-')):
                ...
                if ';' in clean_elem:
                    accept_config = self.final_set_command_approve_case(line_out,
                                        clean_elem.split(";")[0],device_mgmt_ip, accept_config)
                elif clean_elem == '}':  # Up one level remove parent
                    line_out.pop()
                else:
                    line_out.append(clean_elem)
        ...
        for i in range(len(l)):
            ...
            if (elem.startswith('!')):
                ...
                if ("inactive" in clean_elem):
                    ...
                    linactive = list(line_out)
                    linactive[0] = "activate"
                    self.final_set_command_reject_case(linactive, clean_elem,
                                           device_mgmt_ip)
                elif ("active" in clean_elem):
                    clean_elem = clean_elem.replace("active: ", "")
                    linactive = list(line_out)
