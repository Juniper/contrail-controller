...
class FilterModule(object):
    ...
    def _add_fabric_annotations(self):
        jt_refs = self.vncapi.job_templates_list().get('job-templates')
        for jt_ref in jt_refs:
            jt = jt_ref.get('job-template')
            job_template_fqname = jt.get('fq_name')
            ja = JobAnnotations(self.vncapi)
            def_job_input = ja.generate_default_json(job_template_fqname[-1])
            ja.cache_job_input(self.fabric_uuid, job_template_fqname[-1],
                               def_job_input)
        return
