...
def _single_greenlet_processing(deviceinfo, retry_queue):
    ...
    if host_params.get('time') == 0 or elapsed_time >= 30:
        ...
        if deviceinfo.ping_sweep(host_params.get('host')):
            ...
            snmp_result = snmp_walk(host_params.get('host'), '2vc', 'public')

            if snmp_result.get('error') is False:
                oid_mapped = deviceinfo.oid_mapping(host_params.get('host'), snmp_result)
                if oid_mapped:
                    logger.info("HOST {}: SNMP SUCCEEDED".format(host_params.get('host')))
            else:
                logger.info(
                    "SNMP failed for host {} with error {}".format(
                    host_params.get('host'), snmp_result['error_msg']))
            ...
            logger.debug("HOST {}: NOT REACHABLE".format(host_params.get('host')))
            if deviceinfo.total_retry_timeout: 
                logger.info("HOST {}: not reachable, "
                            "add in retry queue".format(host_params.get('host')))
                host_params['time'] = datetime.now()
                retry_queue.put(host_params)
...
def module_process(module):
    ...
    for host in all_hosts:
        host_retry_map_dict = {'host': str(host), 'time': 0, 'time_duration': deviceinfo.total_retry_timeout}
