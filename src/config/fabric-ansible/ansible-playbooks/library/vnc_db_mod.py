...
def vnc_crud(module):
     ...
     class_name = CamelCase(object_type)
     ...
     if object_op == 'create':
         ...
         try: 
            if obj_uuid:
                ...
                results['msg'] = "Either uuid or fq_name should be present for delete"
                results['failed'] = False
         ...
         try: 
            if obj_uuid:
                ...
                results['msg'] = "Either uuid or fq_name should be present for read"
                results['failed'] = False
                return results
         ...
         try:
             if object_op == 'ref_update':
                 obj_uuid = method(obj_type, obj_uuid, ref_type, ref_uuid, ref_fqname, 'ADD')
             else:
                 obj_uuid = method(obj_type, obj_uuid, ref_type, ref_uuid, ref_fqname, 'DELETE')
             results['uuid'] = obj_uuid
         except Exception as ex:
            results['msg'] = str(ex)
            results['failed'] = True
    ...
    module = AnsibleModule(
        argument_spec = dict(
            ...
            object_op = dict(required=True, type='str', 
                             choices=['create', 'delete', 'update', 'read', 'list',
                                      'ref_update', 'ref_delete']),
            object_dict = dict(required=True, type='dict'),
