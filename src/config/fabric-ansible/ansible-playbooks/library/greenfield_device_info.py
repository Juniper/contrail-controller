...
def _device_info_processing(host_params, vncapi, module, fabric_uuid, retry_queue):
    ...
    if host_params.get('time') == 0 or elapsed_time >= 30:
        ...
        if deviceinfo.ping_sweep(host_params.get('host')):
            ...
            if oid_mapped:
                #if fabric.serial_number and oid_mapped['serial-number']\
                #    not in fabric.serial_number:
                #    raise ValueError("Serial number {} not present in fabric {}"
                #                     .format(oid_mapped['serial-number'],
                #                             fabric_uuid))
                fq_name = [
