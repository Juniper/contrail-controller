    ...
    for cred in sorted_len:
        if (('device_family' in cred and 'vendor' not in cred) or
            ...
            if success:
                ...
                host {} with credentials {}".format(host), cred['credential']['username'], \
                cred['credential']['password'])
...
def ping_sweep(host):
    try:
        ...
        logging.debug("ERROR: SUBPROCESS.POPEN failed with error {}".format( \
                      str(e))
        return False
...
def oid_mapping_or_ssh(host, pysnmp_output, module):
    ...
    if pysnmp_output['error']:
        ...
        if not vendor:
            '''
            job object log indicating not in the list of supported vendors
            exit
            '''
            logging.debug( \
              "Host {} is from the vendor {} which is not supported".format( \
              host, vendor))
        ...
        if mapped_dict:
            mapped_value = next(item for item in mapped_dict['snmp_probe'] if \
                            item['oid'] == pysnmp_output['ansible_sysobjectid'])
...
def device_info_processing (host, module):
    ...
    if ping_sweep(host):
        ...
        #logging.debug("DEVICE_INFO_PROCESSING is DONE with value {} in thread id {}".format(mapped_value, id(getcurrent())))
        #mapped_value['host'] = host + id(getcurrent()) 
...
def main():
    ...
    [output.append(thread.value) for thread in threads if thread.value != None]
