...
def _ssh_connect(ssh_conn, username, password, hostname, command, oid_mapped,
                logger):
    try:
        ssh_conn.connect(
            username=username,
...
def _pr_object_create(vncapi, oid_mapped, fq_name, logger, object_log, fabric_uuid):
    try:
...
def _device_info_processing(host, vncapi, module, object_log, fabric_uuid):
    ...
    if _ping_sweep(host, logger):
        ...
        if oid_mapped:
            ...
            return_code = _pr_object_create(vncapi, oid_mapped, fq_name, logger, object_log, fabric_uuid)
                ...
                if phy_router.get('physical_router_management_ip') == \
                   oid_mapped.get('host'):
                    ...
                    fq_name = [
                        ...
                        '_' +
                        oid_mapped.get('host')]
                    return_code = _pr_object_create(vncapi, oid_mapped, \
...
def main():
    ...
    try:
        ...
        for host in all_hosts:
            threadpool.start(Greenlet(_device_info_processing,
