...
def module_process(module, args):
    ...
    if module.params['subnets']:
        for subnet in module.params['subnets']:
            try:
                ...
                _exit_with_error("ERROR: Invalid subnet \"%s\" (%s)" % \
                                 (subnet, str(ex)))
    ...
    if module.params['hosts']:
        for host in module.params['hosts']:
            try:
                ...
                _exit_with_error("ERROR: Invalid ip address \"%s\" (%s)" % \
                                 (host, str(ex)))

    # Verify that we receive a community when using snmp v2
    if module.params['version'] == "v2" or module.params['version'] == "v2c":
        if module.params['community'] is None:
            _exit_with_error("ERROR: Community not set when using \
                             snmp version 2")

    if module.params['version'] == "v3":
        _exit_with_error("ERROR: Donot support snmp version 3")
    ...
    try:
        ...
        _exit_with_error("Failed to connect to API server due to error: %s"\
            % str(ex))
