...
class ParserModule(object):
    ...
    def parse_juniper_junos_qfx(self, device_data, prouter_name, regex_str):
        return self.parse_juniper_device_data(device_data, prouter_name, regex_str)

    def parse_juniper_junos(self, device_data, prouter_name, regex_str):
        return self.parse_juniper_device_data(device_data, prouter_name, regex_str)

    def parse_juniper_device_data(self, device_data, prouter_name, regex_str):
        ...
        rt_interfaces = self.junos_rt_intf_filter(rt_interfaces_list, prouter_name, regex_str)
        _task_log("parsing configured interfaces")
        cf_interfaces = self.junos_cf_intf_filter(cf_interfaces_list, prouter_name, regex_str)
