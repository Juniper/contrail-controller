...
class DeviceConf(object):
    ...
    @classmethod
    def plugin(cls, name, params={}, logger=None):
    ...
    def verify_plugin(self, name):
        if name == self._product:
            return True
        return False
    # end verify_plugin
    ...
    def register(cls, plugin_info={}):
        if not all (k in plugin_info for k in ("name", "product", "vendor")):
            mesg = "Missing Configurations %s" % str(plugin_info)
            raise Exception("Plugin can not be registered : " + mesg)
        DeviceConf._plugins[plugin_info['name']] = plugin_info
    # end register
