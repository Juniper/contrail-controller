...
class DeviceManagerLogger(ConfigServiceLogger):
    ...
    def sandesh_feature_flag_build(self, feature_flag):
        ...
        return sandesh.FeatureFlag(name=fflag.get('feature_name'),
    ...
    def sandesh_feature_flag_handle_request(self, req):
        ...
        if req.name_or_uuid is None:
           for feature_flag in list(FeatureFlagDM.values()):
               sandesh_feature_flag = self.sandesh_feature_flag_build(feature_flag)
               resp.feature_flags.append(sandesh_feature_flag)
        else:
           feature_flag = FeatureFlagDM.find_by_name_or_uuid(req.name_or_uuid)
           if feature_flag:
               sandesh_feature_flag = self.sadesh_feature_flag_build(feature_flag)
               resp.feature_flags.append(sandesh_feature_flag)
        resp.response(req.context())
