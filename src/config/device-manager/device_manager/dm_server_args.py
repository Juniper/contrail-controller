...
#
"""
Device Manager Server arguments.
"""
...
def default_options():
    return {
        ...
        # `kombu_health_check_interval` or `amqp_health_check_interval`
        # for example.
...
def add_parser_arguments(parser):
    ...
    parser.add_argument(
        "--api_server_use_ssl", help="Use SSL to connect with API server")
    parser.add_argument(
        "--analytics_server_ip", help="IP address of Analytics server")
    parser.add_argument(
        "--analytics_server_port", help="Port of Analytics server")
    parser.add_argument(
        "--analytics_username", help="Username for Analytics server")
    parser.add_argument(
        "--analytics_password", help="Password for Analytics server")
    parser.add_argument(
        "--zk_server_ip", help="IP address:port of zookeeper server")
    parser.add_argument(
        "--collectors",
        help="List of VNC collectors in ip:port format",
        nargs="+")
    parser.add_argument(
        "--http_server_port", help="Port of Introspect HTTP server")
    parser.add_argument(
        "--http_server_ip", help="IP of Introspect HTTP server")
    parser.add_argument(
        "--log_local",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "messages")
    parser.add_argument(
        "--log_file", help="Filename for the logs to be written to")
    parser.add_argument(
        "--use_syslog", action="store_true", help="Use syslog for logging")
    parser.add_argument(
        "--syslog_facility", help="Syslog facility to receive log lines")
    ...
    parser.add_argument(
        "--cluster_id", help="Used for database keyspace separation")
    parser.add_argument(
        "--logging_conf",
    ...
    parser.add_argument(
        "--repush_interval", help="time interval for config re push")
    parser.add_argument(
        "--repush_max_interval", help="max time interval for config re push")
    parser.add_argument(
        "--push_delay_per_kb",
    ...
    parser.add_argument(
        "--job_status_retry_timeout",
        help="Timeout between job status check retries")
    parser.add_argument(
        "--job_status_max_retries", help="Max number of job status retries")
    parser.add_argument(
        "--max_job_count",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+")
    parser.add_argument(
        "--dnsmasq_conf_dir", help="Path of the dnsmasq config directory")
    parser.add_argument("--tftp_dir", help="Path of the tftp directory")
    parser.add_argument(
        "--dhcp_leases_file", help="Path of the dhcp leases file")
    parser.add_argument(
        "--ztp_timeout", help="Timeout for the DHCP Lease lookup during ZTP")
    parser.add_argument(
