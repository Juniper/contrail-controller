...
class DeviceManager(object):
    ...
    def __init__(self, args=None):
        ...
        ok, pr_list = self._cassandra._cassandra_physical_router_list(None, None, None)
        if not ok:
            self.config_log('physical router list returned error: %s' % pr_list)
        else:
            vn_set = set()
    ...
    def _vnc_subscribe_callback(self, oper_info):
        try:
            ...
            obj_class = self._OBJ_TYPE_MAP.get(obj_type)
            ...
            if oper_info['oper'] == 'CREATE':
                ...
                dependency_tracker = DependencyTracker(self._OBJ_TYPE_MAP)
