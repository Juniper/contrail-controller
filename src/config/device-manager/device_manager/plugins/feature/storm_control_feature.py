...
class StormControlFeature(FeatureBase):
    ...
    def _get_port_profile_list(self, vpg_obj):
        if self._is_enterprise_style(self._physical_router):
            return vpg_obj.port_profiles
        else:
            for vmi_uuid in vpg_obj.virtual_machine_interfaces or []:
                vmi_obj = VirtualMachineInterfaceDM.get(vmi_uuid)
                if vmi_obj and vmi_obj.port_profiles:
                    return vmi_obj.port_profiles

        return []
    # end _get_port_profile_list
    ...
    def _build_storm_control_interface_config(self):
        ...
        for vpg_uuid in pr.virtual_port_groups or []:
            ...
            if not vpg_obj:
                continue
            pp_list = self._get_port_profile_list(vpg_obj)
