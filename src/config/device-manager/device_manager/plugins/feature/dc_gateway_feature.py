...
from abstract_device_api.abstract_device_xsd import Feature, Firewall, \
    FirewallFilter, From, RoutingInstance, NatRules, NatRule, Term, Then
import copy
import gevent
...
class DcGatewayFeature(FeatureBase):
    ...
    def _add_routing_instance(self, ri_conf):
        ...
        if ri.get_routing_instance_type() != 'virtual-switch' and \
            ...
            self._logger.debug("PARAG: Adding rib: %s, l2: %s l2-l3 %s "
                               "ri_type %s vn_mode %s, public %s" %
                               (ri.get_name(), str(is_l2), str(is_l2_l3),
                                ri.get_routing_instance_type(),
                                ri.get_virtual_network_mode(),
                                router_external))
        else:
            self._logger.debug("PARAG: Skipping rib: %s, l2: %s l2-l3 %s "
                               "ri_type %s vn_mode %s, public %s" %
                               (ri.get_name(), str(is_l2), str(is_l2_l3),
                                ri.get_routing_instance_type(),
                                ri.get_virtual_network_mode(),
        ...
        if router_external and is_l2 is False:
            self._logger.debug("PARAG: CALLING FIREWALL rib: %s, l2: %s "
                               "l2-l3 %s ri_type %s vn_mode %s, public %s" %
                               (ri.get_name(), str(is_l2), str(is_l2_l3),
                                ri.get_routing_instance_type(),
                                ri.get_virtual_network_mode(),
                                router_external))
    ...
    def _build_ri_config_for_dc(self):
        ...
        pr.evaluate_vn_irb_ip_map(set(vn_dict.keys()), 'l2_l3', 'irb', False)
    ...
    def feature_config(self, **kwargs):
        ...
        rb_roles = self._physical_router.routing_bridging_roles
        if not rb_roles or 'DC-Gateway' not in rb_roles:
