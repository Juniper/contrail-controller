...
class TelemetryFeature(FeatureBase):
    ...
    def _build_sflow_interface_config(self, interface,
        ...
        if TelemetryFeature._check_interface_for_sflow(
            ...
            self._build_telemetry_config(telemetry_profile_name,
                                         sflow_profile_name,
                                         sflow_profile_params)
            interface.set_telemetry_profile(telemetry_profile_name)
    # end _build_sflow_interface_config

    @staticmethod
    def _check_interface_for_sflow(interface_name,
                                   interface_type,
                                   sflow_interface_type,
        ...
        if sflow_interface_type == "all":
            ...
            for custom_intf in enabled_custom_interface_list:
                if interface_name == custom_intf.get_name():
                    return True
    ...
    def _build_telemetry_config(self, tp_name, sflow_name, sflow_params):
        ...
        if scf:
            sflow_profile_obj.set_sample_rate(scf.get_sample_rate())
            sflow_profile_obj.set_polling_interval(
                scf.get_polling_interval())
        agent_id = str(sflow_params.get('agent_id', ''))
        if agent_id:
        ...
        if enbld_intf_params:
            enbld_intf_name = enbld_intf_params.get_name()
            stats_coll_freq = \
                enbld_intf_params.get_stats_collection_frequency()
            stats_sampl_rt = stats_coll_freq.get_sample_rate()
            stats_poll_intvl = stats_coll_freq.get_polling_interval()

            enbld_intf_params_obj = EnabledInterfaceParams(
                name=enbld_intf_name,
                sample_rate=stats_sampl_rt,
                polling_interval=stats_poll_intvl
            )
            sflow_profile_obj.set_enabled_interface_params(
    ...
    def feature_config(self, **kwargs):
        ...
        for interface in pr.physical_interfaces:
