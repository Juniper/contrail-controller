...
class OverlayDcGatewayFeature(FeatureBase):
            ...
            for int_ri in self.internal_vn_ris:
                ...
                for vn in vn_list:
                    ...
                    if irb_name in self.irb_interfaces:
                        self._logger.info(
                            "ODC set_internal_vn_irb_config, %s %s=" %
                            (int_ri.get_name(), irb_name))
                        self._add_ref_to_list(int_ri.get_routing_interfaces(),
                                              irb_name)
    # end _set_internal_vn_irb_config

    def _set_dci_vn_irb_config(self):
    ...
    def _set_default_li(self, li_map, name, unit):
        if name in li_map:
            li = li_map[name]
        else:
    ...
    def _add_irb_config(self, ri_conf):
        ...
        if gateways is not None:
            intf_unit = self._set_default_li(li_map,
                                             'irb.' + str(network_id),
                                             network_id)
    ...
    def _attach_irb(self, ri_conf, ri):
        ...
        if (is_l2 and vni is not None and
            ...
            if is_l2_l3:
                ...
                self._logger.info("ODC attach_irb, %s irb.%s=" % (
                    ri_conf.get("ri_name"), str(network_id)))
    # end _attach_irb
    ...
    def _add_routing_instance(self, ri_conf):
        ...
        if fip_map is not None:
            ...
            bExist = False
            for existingF in self.firewall_config.get_firewall_filters():
                if existingF.get_name() == f.get_name() and \
                        existingF.get_comment() == f.get_comment():
                    self._logger.info(
                        "FirewallFilter Already Exist %s-%s, "
                        "skipping it.." % (f.get_name(), f.get_comment()))
                    bExist = True
                    break
            if bExist == False:
                self.firewall_config.add_firewall_filters(f)
                ...
                if not is_internal_vn:
                    ...
                    for interface in interfaces:
                        self._add_ref_to_list(vlan.get_interfaces(),
                                              interface.name)
                    if is_l2_l3 and self._is_gateway():
    ...
    def _build_ri_config(self):
        ...
        for vn_id, interfaces in self._get_sorted_key_value_pairs(vn_dict):
            ...
            if (vn_obj is None or vn_obj.get_vxlan_vni() is None or
                ...
                if vn_obj:
                    self._logger.info(
                        "SKIPPING VN: %s %s MX %s for PR %s" %
                        (vn_obj.name, vn_obj.router_external,
                         isMxRouter, self._physical_router.name))
                continue
            ...
            if export_set and\
                ...
                if not self._physical_router \
                    ...
                    self._logger.error("DCFeature: DM can't allocate service "
                                       "interfaces for (vn, vn-id)=(%s,%s)" %
                                       (vn_obj.fq_name,
                                        vn_obj.vn_network_id))
                else:
    ...
    def feature_config(self, **kwargs):
        feature_config_param = Feature(name=self.feature_name())

        self._build_ri_config()
        self._set_internal_vn_irb_config()
        self._set_dci_vn_irb_config()
        ...
        if len(self.ri_map):
            isMxRouter = self._physical_router.is_junos_service_ports_enabled()
            for ri in self._get_values_sorted_by_key(self.ri_map):
                if ri.get_is_public_network() == False and isMxRouter == False:
                    self._logger.info(
                        "SKIPPING non-public RI: %s %s" %
                        (ri.get_name(), ri.get_comment()))
                    continue
                feature_config_param.add_routing_instances(ri)

        if len(pis) > 0:
            feature_config_param.set_physical_interfaces(pis)
        if self.evpn:
            feature_config_param.set_evpn(self.evpn)
        if self.firewall_config:
            feature_config_param.set_firewall(self.firewall_config)
        if len(self.vlan_map) > 0:
            feature_config_param.set_vlans(self._get_values_sorted_by_key(
                self.vlan_map))

        return feature_config_param
    # end feature_config
