        ...
        for vpg_uuid in pr.virtual_port_groups or []:
            ...
            for pi_uuid in vpg_obj.physical_interfaces or []:
                if pi_uuid not in pr.physical_interfaces:
                    # This Physical Interface UUID is related to different Physical Router (PR)/Leaf.
                    multi_homed = True
                    continue
                ...
                if ae_id is not None:
                    ...
                    if ae_link_members.has_key(ae_intf_name):
                        ...
                        ae_link_members[ae_intf_name].append(pi.name)
                # PRADEEP: Below code may not be required as it is not being used in LAG.
                else:
                    pi_obj = db.PhysicalInterfaceDM.get(pi_uuid)
                    if pi_obj:
                        lag = LinkAggrGroup(description="Virtual Port Group : %s" %
                                                        vpg_obj.name)
                        intf, _ = self._add_or_lookup_pi(self.pi_map, pi_obj.name, 'regular')
            ...
            for ae_intf_name, link_members in ae_link_members.iteritems():
                ...
                lag = LinkAggrGroup(lacp_enabled=True,
                                    ...
                                    description="Virtual Port Group : %s" %
                                                vpg_obj.name)
                # PRADEEP - Found equivalent method and using this.
                intf, _ = self._add_or_lookup_pi(self.pi_map, ae_intf_name, 'lag')
                intf.set_link_aggregation_group(lag)
