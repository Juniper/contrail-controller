        ...
        for pnf_pi, intf_type in svc_app_obj.physical_interfaces.iteritems():
            ...
            for spine_pi in pnf_pi_obj.physical_interfaces:
                ...
                if spine_pi_obj.fq_name == left_fq_name:
                    ...
                    li_fq_name = ['default-global-system-config', pr.name, spine_pi_obj.name, li_name]
                    li_uuid = pr._object_db.fq_name_to_uuid("logical_interface", li_fq_name)
        ...
        if self.is_service_chained():
            ...
            for pt in pr.port_tuples:
                ...
                if si_obj.rp_ip_addr:
                    ...
                    li_fq_name = ['default-global-system-config', pnf_pr_obj.name, 'lo0', li_name]
                    li_uuid = pr._object_db.fq_name_to_uuid("logical_interface", li_fq_name)
                    li_obj = LogicalInterfaceDM.get(li_uuid)
                    if li_obj:
