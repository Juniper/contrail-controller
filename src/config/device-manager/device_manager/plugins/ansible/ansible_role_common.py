...
class AnsibleRoleCommon(AnsibleConf):
    ...
    def build_l2_evpn_interface_config(self, interfaces, vn, vlan_conf):
        ...
        for ifd_name, interface_list in ifd_map.items():
            untagged = [i for i in interface_list if i.is_untagged()]
            if len(untagged) > 1:
                self._logger.error(
                    "Only one untagged interface is allowed on a ifd %s" %
                    ifd_name)
                continue
            tagged = [i for i in interface_list if not i.is_untagged()]
            if (self._is_enterprise_style() and len(untagged) > 0 and
               len(tagged) > 0):
                self._logger.error(
