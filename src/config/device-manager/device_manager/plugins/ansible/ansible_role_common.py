...
class AnsibleRoleCommon(AnsibleConf):
            ...
            for int_ri in self.internal_vn_ris:
                ...
                for vn in vn_list:
                    ...
                    if irb_name in self.irb_interfaces:
                        self.add_to_list(int_ri.get_routing_interfaces(), irb_name)
    ...
    def add_bogus_lo0(self, ri, network_id, vn):
        ...
        self.add_to_list(ri.get_loopback_interfaces(), intf_name)
    # end add_bogus_lo0
    ...
    def add_routing_instance(self, ri_conf):
        ...
        if not is_l2:
            ...
            if fip_map is None:
                for interface in interfaces:
                    self.add_to_list(ri.get_interfaces(), interface.name)
                if prefixes:
                    for prefix in prefixes:
        ...
        if fip_map is not None:
            self.add_to_list(ri.get_interfaces(), interfaces[0].name)
            ...
            for public_vrf, fips in public_vrf_ips.items():
                ...
                self.add_to_list(ri_public.get_interfaces(), interfaces[1].name)
                floating_ips = []
                for fip in fips:
        ...
        if fip_map is not None:
            ...
            self.add_to_list(ri.get_routing_interfaces(), intf_name)
        ...
        if (is_l2 and vni is not None and
            ...
            if encapsulation_priorities[0] == "VXLAN":
                ...
                for interface in interfaces:
                    self.add_to_list(vlan.get_interfaces(), interface.name)
                if is_l2_l3:
                    # network_id is unique, hence irb
                    irb_intf = "irb." + str(network_id)
                    self.add_to_list(vlan.get_interfaces(), irb_intf)
            elif (any(x in encapsulation_priorities for x in ["MPLSoGRE", "MPLSoUDP"])):
                ...
                for interface in interfaces:
                    self.add_to_list(self.evpn.get_interfaces(), interface.name)
        ...
        if fip_map is not None:
            ...
            self.add_to_list(ri.get_ingress_interfaces(), interfaces[0].name)
            self.add_to_list(ri.get_egress_interfaces(), interfaces[1].name)
    ...
    def build_l2_evpn_interface_config(self, interfaces, vn, vlan_conf):
        ...
        for ifd_name, interface_list in ifd_map.items():
            ...
            if interface_list[0].is_untagged():
                ...
                if vlan_conf:
                    self.add_to_list(vlan_conf.get_interfaces(), unit_name)
            else:
