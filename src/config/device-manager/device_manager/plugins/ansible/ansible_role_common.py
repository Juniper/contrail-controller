...
class AnsibleRoleCommon(AnsibleConf):
    ...
    def add_inet_filter_term(self, ri_name, prefixes, inet_type):
        ...
        for prefix in prefixes:
            ...
            from_.add_destination_prefix_list(Subnet(
                prefix=prefix_parts[0], prefix_len=int(prefix_parts[1])))
        then_ = Then()
    ...
    def add_routing_instance(self, ri_conf):
        ...
        if (not is_l2 and not is_l2_l3 and gateways):
            ...
            lo_intf = PhysicalInterface(name="lo0")
            self.interfaces_config.append(lo_intf)
            intf_unit = LogicalInterface(name=ifl_num, comment=DMUtils.l3_lo_intf_comment(vn))
            lo_intf.add_logical_interfaces(intf_unit)
            ...
            ri.add_loopback_interfaces(LogicalInterface(
                name="lo0." + ifl_num, comment=DMUtils.lo0_ri_intf_comment(vn)))
    ...
    def add_ri_vlan_config(self, ri, vni):
        ...
        ri.set_virtual_network_id(str(vni))
    ...
    def build_esi_config(self):
        ...
        for esi, ae_id in self.physical_router.ae_id_map.items():
            intf = PhysicalInterface(name="ae" + str(ae_id),
