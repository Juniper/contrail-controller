...
class AnsibleRoleCommon(AnsibleConf):
    ...
    def build_pnf_security_zone_config(self, svc_params):
    ...
    def build_pnf_ri_config(self, svc_params):
        if svc_params.get('svc_inst_name'):
            ...
            if svc_params.get('lo_li'):
                ri.add_routing_interfaces(LogicalInterface(name=svc_params.get('lo_li')))
            if svc_params.get('left_li'):
            ...
            if svc_params.get('right_li_ip') and svc_params.get('right_asns'):
                ...
                bgp_name = right_bgp_name
                peers = {}
                for peer_ip in svc_params.get('peer_right_li_ips') or []:
                    peer = Bgp(name=right_bgp_name,
                               ip_address=peer_ip,
                               autonomous_system=svc_params.get('right_asns')[1])
                    peers[right_bgp_name] = peer
                if peers:
                    bgp.set_peers(self.get_values_sorted_by_key(peers))
                    self.bgp_map[bgp_name] = bgp
                ri.add_protocols(bgp)

    def build_pnf_intfs_config(self, svc_params):
             ...
             if pt_obj:
                 ...
                 if si_obj:
                     ...
                     if sa_obj:
                         ...
                         self.build_pnf_ri_config(svc_params)
                         self.build_pnf_intfs_config(svc_params)
                         self.build_pnf_security_zone_config(svc_params)
