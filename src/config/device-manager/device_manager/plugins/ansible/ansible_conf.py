...
class AnsibleConf(AnsibleBase):
    ...
    def device_send(self, job_template, job_input, is_delete, retry):
        ...
        try:
            ...
            expanded_job_input = job_input.copy()
            expanded_job_input['device_abstract_config'] = \
                json.loads(expanded_job_input['device_abstract_config'])
            self._logger.error("Abstract config: %s" %
                               json.dumps(expanded_job_input, indent=4,
                                          sort_keys=True))
            self.commit_stats[
