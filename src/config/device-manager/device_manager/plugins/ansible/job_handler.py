...
import gevent
import json
from opeserver_util import OpServerUtils
...
class JobHandler(object):
    ...
    START_TIME = 'now-5m'
    END_TIME = 'now'
    FIELDS = [VizConstants.TIMESTAMP, VizConstants.MESSAGE_TYPE]
    ...
    def _get_opserver_query(cls, job_execution_id, status):
        ...
        match = OpServerUtils.Match(name=VizConstants.OBJECT_ID,
    ...
    def _check_job_status(self, url, job_execution_id, status):
        ...
        resp = OpServerUtils.post_url_http(url,
                                           json.dumps(query.__dict__),
                                           self._analytics_config.username,
                                           self._analytics_config.password)
        if resp is not None:
