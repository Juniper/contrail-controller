...
class JobHandler(object):
    ...
    def push(self, timeout, max_retries):
        try:
            ...
            job_execution_id = str(int(round(time.time() * 1000))) + '_' + \
                               str(uuid.uuid4())
            ...
            self._logger.debug("job handler: Published job request "
                               "(%s, %s, %s)" % (self._device_list,
                               str(self._job_template_name),
                               str(job_execution_id)))
    ...
    def _start_job_status_listener(self, job_execution_id):
        # TODO - Restarting the worker might not scale and have issue in parallel use amongst the greenlets
