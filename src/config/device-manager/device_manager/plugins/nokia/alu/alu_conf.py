...
class AluConf(JuniperConf):
    ...
    def push_e2_conf(self, is_delete=False):
        if not self.e2_manager:
            self.e2_manager = ALUE2Services(self._logger)
        self.e2_manager.config_e2_services()
        ...
        for sindex, sep_uuid in enumerate(self.service_endpoints):
            ...
            for scm_id in sep.service_connection_modules:
                    ...
                    if 'resources' in skey:
                        ...
                        for res_entry in sval:
                            ...
                            for res_key, res_value in res_entry.iteritems():
                                #if 'rkey' in res_key and res_value == 'mtu':
                                    #mtu_present = True
                                if 'rvalue' in res_key and mtu_present == True:
                                    scm.mtu = res_value
                                    phy_mtu = scm.mtu
                                    mtu_present = False
                                #if 'rkey' in res_key and res_value == 'control-word':
                                    #cw_present = True
                ...
                if service_type == 'fabric-interface' and service_fabric == False:
                    ...
                    li.vlan_tag, service_type, node_role, phy_mtu, li_mtu, peer_is_nokia, ps_config)
                    self.config_manager.add_e2_lo0_config(node_role, router_lpbk_id)
                    self.config_manager.add_e2_routing_options(node_role, router_id)
                    service_fabric == True
                    ...
                    self.config_manager.add_e2_services_l2vpn_config_xml(node_role, 
                            li.name, li.vlan_tag, as_number, service_index,
                    ...
                    if 'ps' in ifname[0] and ifname[0] in ps_config_dict:
                        ...
                        ps_config = None
                        li_name = ifname[0] + '.0'
                        ps_config = self.config_manager.add_e2_phy_logical_interface(li_name, \
                                0, service_type, node_role, phy_mtu, \
                                li_mtu, peer_is_nokia, ps_config)
                    else:
                        ps_config = None
                    ps_config = self.config_manager.add_e2_phy_logical_interface(li.name, 
                            li.vlan_tag, service_type, node_role, phy_mtu, 
                            li_mtu, peer_is_nokia, ps_config)
                    ...
                    if not ifname[0] in ps_intf_list:
                        li_name = ifname[0] + '.0'
                        self.config_manager.add_e2_services_pwht_config_mx_xml(node_role, \
