...
class DeviceJobManager(object):
    ...
    JOB_STATUS_TTL = 5*60
    # request_params = {"fabric_fq_name": "default-global-system-config:fab01", "auth_token": None, "job_execution_id": "1543319968210_9fd37365-c59a-4ad8-8c43-b00484f6da7e", "job_template_fq_name": ["default-global-system-config", "existing_fabric_onboard_template"], "job_template_id": "67b7ba49-8b57-4d00-8028-42c419233e49", "job_concurrency": "fabric", "input": {"fabric_asn_pool": [{"asn_max": 65000, "asn_min": 64000}, {"asn_max": 65100, "asn_min": 65000}], "fabric_subnets": ["30.1.1.1/24"], "fabric_fq_name": ["default-global-system-config", "fab01"], "loopback_subnets": ["20.1.1.1/24"], "management_subnets": [{"cidr": "10.84.130.94/32", "gateway": "10.84.130.254"}], "node_profiles": [{"node_profile_name": "juniper-qfx5k"}, {"node_profile_name": "juniper-qfx10k"}], "device_auth": [{"username": "root", "password": "Embe1mpls"}]}, "api_server_host": ["10.155.75.171"]}

    _instance = None

    def __init__(self, db_conn, amqp_client, zookeeper_client, args, dm_logger):
        ...
        self._amqp_client.add_exchange(self.JOB_REQUEST_EXCHANGE,
                                       type='direct')
        self._amqp_client.add_consumer(self.JOB_REQUEST_CONSUMER,
    ...
    def handle_execute_job_request(self, body, message):
        ...
        try:
            ...
            if job_template_id is None:
                job_template_id = self.get_job_template_id(job_template_fq_name)
            ...
            if is_delete is None or is_delete == False:
                ...
                if job_concurrency is not None and job_concurrency == "fabric":
                    ...
                    if existing_job:
                        ...
                        self.mark_failure(msg, job_template_fq_name,
                                          job_execution_id,
            ...
            self.job_status.update({job_execution_id:JobStatus.STARTING.value})
            ...
            job_mgr_path = os.path.dirname(__file__) + \
            ...
            self._logger.info("Current number of job_mgr processes running %s"
                              % self._job_mgr_statistics.get(
                'running_job_count'))
        except Exception as e:
