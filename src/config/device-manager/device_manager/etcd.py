...
class DMEtcdDB(VncObjectDBClient, DeviceMapperDBMixin):
    ...
    def __init__(self, args, vnc_lib, logger=None):
        logger_log = None
        _etcd_args = etcd_args(args)
        if logger:
            logger.log(
                "VncObjectEtcdClient arguments. {}".format(_etcd_args))
            logger_log = logger.log
        super(DMEtcdDB, self).__init__(logger=logger_log, **_etcd_args)
    ...
    def get_si_pr_set(self, si_id):
        ...
        pr_set = set()
        for pt_uuid in si_obj.port_tuples:  # set of tuples
    ...
    def pnf_vlan_int_pool(self, pr_id):
    ...
    def pnf_unit_int_pool(self, pi_id):
    ...
    def _fetch_pnf_resources(self, si_id):
        ...
        value = self.dm_object_db_instance.get_value(
    ...
    def add(self, path, key, value):
        ...
        try:
            self.dm_object_db_instance.put_kv(
    ...
    def delete(self, path, key, columns=None):
        ...
        # simply delete a key
        if not columns:
            self.dm_object_db_instance.delete_kv(etcd_key)
            return True
        # delete some fields in dict stored under etcd_key
        value = self.dm_object_db_instance.get_value(etcd_key)
        try:
        ...
        if entries:
            ...
            self._objdm_object_db_instanceect_db.put_kv(
    ...
    def get_pnf_resources(self, vmi_obj, pr_id):
        ...
        # NOTE used in PhysicalRouterDM.allocate_pnf_resources
    ...
    def delete_pnf_resources(self, si_id):
        ...
        for pr_uuid in pr_set:
            self._vnc_lib.deallocate_int(
                self.pnf_vlan_int_pool(pr_uuid), int(pnf_resources['vlan_id']))
        ...
        for pt_uuid in si_obj.port_tuples:
            ...
            for vmi_uuid in pt_obj.virtual_machine_interfaces:
                ...
                if vmi_obj.physical_interface:
                    self._vnc_lib.deallocate_int(
                        self.pnf_unit_int_pool(vmi_obj.physical_interface),
                        int(pnf_resources['unit_id']))

        # remove local dict and etcd resource
        del self.pnf_resources_map[si_id]
        self.dm_object_db_instance.delete_kv(self._pnf_resources_path(si_id))
    ...
    def init_pr_map(self):
        pr_entries = self.dm_object_db_instance.list_kv(
    ...
    def init_dci_asn_map(self):
        dci_entries = self.dm_object_db_instance.list_kv(
    ...
    # FIXME here finished 2019-01-23
