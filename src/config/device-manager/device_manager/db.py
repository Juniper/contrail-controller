...
class PhysicalRouterDM(DBBaseDM):
    ...
    def allocate_pnf_resources(self, vmi):
        resources = self._cassandra.get_pnf_resources(
            vmi.service_instance, self.uuid)
        ...
        ip = ip + "/30"
        return (ip, str(resources['vlan_id']))
    # end
...
class DMCassandraDB(VncCassandraClient):
    ...
    def __init__(self, manager, zkclient):
        ...
        self.pnf_netork_allocator = IndexAllocator(
    ...
    def get_pnf_resources(self, si_id, pr_id):
        if si_id in self.pnf_resources_map:
            return self.pnf_resources_map[si_id]

        network_id = self.pnf_netork_allocator.alloc(si_id)
        vlan_alloc = self.pnf_vlan_allocator_map.get(
            pr_id,
            IndexAllocator(
                self._zkclient,
                self._zk_path_pfx+pr_id+'/'+self._PNF_VLAN_ALLOC_PATH,
                self._PNF_MAX_VLAN)
        )
        ...
        for other_pr_uuid in pr_set:
            if other_pr_uuid != pr_id:
                self.pnf_vlan_allocator_map.get(
                    other_pr_uuid,
                    IndexAllocator(
                        self._zkclient,
                        self._zk_path_pfx+other_pr_uuid+'/' +
                        self._PNF_VLAN_ALLOC_PATH,
                        self._PNF_MAX_VLAN)
                ).reserve(vlan_id)
        pnf_resources = {
            "network_id": str(network_id), "vlan_id": str(vlan_id)}
    ...
    def delete_pnf_resourecs(self, si_id):
        pnf_resources = self.pnf_resources_map[si_id]
        self.pnf_netork_allocator.delete(int(pnf_resources['network_id']))

        pr_set = self.get_si_pr_set(si_id)
        for pr_uuid in pr_set:
            if pr_uuid in self.pnf_vlan_allocator_map:
                self.pnf_vlan_allocator_map[
                    pr_uuid].delete(int(pnf_resources['vlan_id']))
    # end
