...
class AccessControlListDM(DBBaseDM):
    ...
    def update(self, obj=None):
        if obj is None:
            obj = self.read_obj(self.uuid)
        self.name = obj.get('fq_name')[-1]
        self.vnc_obj = self.read_vnc_obj(uuid=self.uuid)
        self.is_ingress = True if 'ingress-' in self.name else False
        self.security_group = None
        if obj.get('parent_type') == "security-group":
            self.security_group = self.get_parent_uuid(obj)
            sg = SecurityGroupDM.get(self.security_group)
            if sg:
                sg.update_acl(self)
        return obj
    # end update
...
class SecurityGroupDM(DBBaseDM):
    ...
    def update(self, obj=None):
        ...
        self.name = self.fq_name[-1]
        self.update_multiple_refs('virtual_machine_interface', obj)
        self.access_control_lists = set([acl['uuid'] for acl in
                                        obj.get('access_control_lists', [])])
    # end update

    def update_acl(self, acl_uuid):
        self.access_control_lists.add(acl_uuid)
    # end update_acl

    def remove_acl(self, acl_uuid):
        self.access_control_lists.remove(acl_uuid)
    # end remove_acl
