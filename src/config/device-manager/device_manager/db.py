...
class PhysicalRouterDM(DBBaseDM):
    ...
    def __init__(self, uuid, obj_dict=None):
        ...
        self.config_manager = DeviceConf.plugin(self.product, plugin_params, self._logger)
    ...
    def update(self, obj=None):
        ...
        if not self.config_manager:
            ...
            if self.config_manager.verify_plugin(self.product):
    ...
    def init_device_config(self):
        if not self.device_config and self.config_manager:
            self.device_config = self.config_manager.dv_get()
    ...
    def push_config(self):
        ...
        if self.product.lower() not in model.lower():
    ...
    def uve_send(self, deleted=False):
        ...
        if self.is_vnc_managed():
            ...
            pr_trace.last_commit_time = commit_stats.get('last_commit_time', '')
            pr_trace.last_commit_duration = commit_stats.get('last_commit_duration', 0)
            pr_trace.commit_status_message = commit_stats.get('commit_status_message', '')
            pr_trace.total_commits_sent_since_up = commit_stats.get('total_commits_sent_since_up', 0)
        else:
