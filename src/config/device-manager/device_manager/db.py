...
class PhysicalRouterDM(DBBaseDM):
    ...
    def _set_internal_vn_routed_bgp_info(self, ri, vn_obj, routed_param):
        ...
        peer_bgp = Bgp(name=bgp_info.get('peer_ip_address'),
                                autonomous_system=\
                                bgp_info.get('peer_autonomous_system'),
                ...
                if self.uuid == route_param.get('physical_router_uuid'):
                    if route_param.get('routing_protocol') == 'bgp':
                        self._set_internal_vn_routed_bgp_info(ri, vn_obj,
                                                             route_param)
                    elif route_param.get('routing_protocol') == 'static-routes':
...
class VirtualNetworkDM(DBBaseDM):
    ...
    def get_prefixes(self, pr_uuid=None):
        ...
        vn_list = lr.get_connected_networks(include_internal=False, pr_uuid=pr_uuid)
        prefix_set = set()
