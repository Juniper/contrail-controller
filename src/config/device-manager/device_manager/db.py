    ...
    def evaluate_vn_ip_map(self, vn_set, ip_map, ip_used_for,
        ...
        for vn_subnet in create_set:
            ...
            if use_gateway_ip:
                ...
                if DMUtils.is_ipv6_ll_subnet(sub) is True and \
                   vn.ipv6_ll_vn_id is not None:
                    vn_uuid = vn.ipv6_ll_vn_id
...
class VirtualNetworkDM(DBBaseDM):
    ...
    def read_ipv6_object(self):
        ...
        net_obj =self._manager._vnc_lib.virtual_network_read(fq_name=
                                                             nw_fq_name);
        if net_obj is None:
            self._logger.error("virtual network '%s' does not exist"
                                % (nw_fq_name[-1]))
            return (None, None)
        gateways = {}
...
class DMCassandraDB(VncObjectDBClient):
    ...
    def get_ipv6_ll_subnet(self, key):
        ...
        return self.ni_ipv6_ll_map.get(key);
