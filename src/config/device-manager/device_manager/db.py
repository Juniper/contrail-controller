...
class ServiceInstanceDM(DBBaseDM):
    ...
    def update(self, obj=None):
        ...
        if bindings:
            kvp = bindings.get('key_value_pair') or []
            kvp_dict = dict((kvp['key'], kvp['value']) for kvp in kvp)
            self.left_vlan = kvp_dict.get('left-vlan') or None
            self.right_vlan = kvp_dict.get('right-vlan') or None
        self.port_tuples = set(
