...
from dm_utils import DMIndexer, DMUtils, PushConfigState
...
class DeviceMapperDBMixin(object):
    """Methods common for DMCassandrdaDB and DMEtcdDB classes."""

    def __init__(self, **kwargs):
        super(DeviceMapperDBMixin, self).__init__()
        self.pr_vn_ip_map = {}
        self.pr_dci_ip_map = {}
        self.pr_ae_id_map = {}
        self.pr_asn_map = {}
    ...
    def add_to_pr_map(self, pr_uuid, vn_subnet, ip_used_for):
        # NOTE used in PhysicalRouterDM.evaluate_vn_irb_ip_map
        if pr_uuid in self.__pr_vn_ip_map:
            self.__pr_vn_ip_map[pr_uuid].add((vn_subnet, ip_used_for))
        else:
            self.__pr_vn_ip_map[pr_uuid] = set()
            self.__pr_vn_ip_map[pr_uuid].add((vn_subnet, ip_used_for))
    # end add_to_pr_map
    ...
    def delete_from_pr_map(self, pr_uuid, vn_subnet, ip_used_for):
        if pr_uuid in self.__pr_vn_ip_map:
            self.__pr_vn_ip_map[pr_uuid].remove((vn_subnet, ip_used_for))
            if not self.__pr_vn_ip_map[pr_uuid]:
                del self.__pr_vn_ip_map[pr_uuid]
    ...
    def populate_pr_ae_map(self, pr_entries):
        for key in pr_entries.keys():
            key_data = key.split(':', 1)
