...
class VirtualNetworkDM(DBBase):
    ...
    def update(self, obj=None):
        ...
        self.update_vmi_public_network_map()
        self.prefixes = set()
        self.gateways = set()
    ...
    def update_vmi_public_network_map(self):
        for vmi_uuid in self.virtual_machine_interfaces:
            ...
            if vmi is not None and vmi.floating_ip is not None:
                ...
                if self.vmi_public_network_map.get(vmi_uuid) is not None and self.vmi_public_network_map[vmi_uuid] != fip.public_network:
    ...
    def set_vmi_public_network(self, vmi_uuid, public_network_uuid):
        if public_network_uuid is not None:
            ...
            self.vmi_public_network_map[vmi_uuid] = public_network_uuid
        else:
            if self.vmi_public_network_map is None:
                return
            if vmi_uuid in self.vmi_public_network_map:
                old_public_network = self.vmi_public_network_map[vmi_uuid]
                ...
                del self.vmi_public_network_map[vmi_uuid]
