...
class DataCenterInterconnectDM(DBBaseDM):
    ...
    def get_intrafabric_properties(self, obj):
        ...
        self._logger.debug(
            ...
            (self.name, self.dci_type, self.src_lr_uuid, len(self.dst_lr_pr),
            ...
            for rpname, rpobj in self.rp_vrf_import_list.items():
                ...
                for term in rp_entries.get_terms():
                    ...
                    if actionl:
                        ...
                        if not action:
                            self._logger.error(
                                "DCI %s for vrf import RP %s with "
                                "routeFilter action is not defined. "
                                "skipping it" % (self.name, rpname))
                            continue
                        if action and action != 'accept':
                            self._logger.error(
                                "DCI %s for vrf import RP %s with "
                                "routeFilter action %s found. "
                                "skipping it" % (self.name, rpname,
                                                 action))
                            continue
    ...
    def set_intrafabric_dci_config(cls, curpr, internal_vn_ris):
        ...
        for int_ri in internal_vn_ris or []:
            if DMUtils.get_lr_internal_vn_prefix() not in int_ri.name:
                continue
                ...
                if curlr_in_dstlr:
                    ...
                    if curpr_in_srclr:
                        ...
                        if ribname not in rib_map:
                            ...
                            if len(trplist) < 1:
                                cls._logger.error(
                                    "PARAG BUG: DCI %s %s RIB RP len 0" %
                                    (dci.name, curpr))
                            ...
                            for k, v in trplist.items():
                                if k in rp_list:
                                    cls._logger.error(
                                        "PARAG BUG: Duplicate RP %s %s RIB "
                                        "RP %s" % (dci.name, curpr, k))
                                rp_list[k] = v
                        ...
                        if len(tvrflist) < 1:
                            cls._logger.error(
                                "PARAG BUG: DCI %s %s VRF Import RP len 0" %
                                (dci.name, curpr))
                        for k, v in tvrflist.items():
                            if k in rp_list:
                                cls._logger.error(
                                    "PARAG BUG: Duplicate RP %s %s VRF Import"
                                    " RP %s" % (dci.name, curpr, k))
                            rp_list[k] = v
                            if k not in int_ri.get_vrf_import():
                                int_ri.add_vrf_import(k)
                    continue
                curlr_in_srclr = True if (lr_uuid == dci.src_lr_uuid and
                                          curpr in dci.get_src_lr_prlist()) \
                    else False
                if curlr_in_srclr:
        ...
        for int_ri, dcis in src_ri_dci_map.items():
            for dci in dcis:
                ...
                if ribname in rib_map:
                    ...
                    if dci.name in vrf_dst_dci_list:
                        cls._logger.error(
                            "PARAG BUG: DCI %s %s For SRC RIBname %s AND VRF "
                            "DST RP BOTH EXISTs." %
                            (dci.name, curpr, ribname))
                    ...
                    if bool(dstlrprs.difference(srclrprs)) == False:
                        cls._logger.debug(
                            "PARAG SKIPPING: DCI %s srcPR %s dstPr %s " %
                            (dci.name, srclrprs, dstlrprs))
                        continue
                    cls._logger.debug(
                        "PARAG doing Both RIB + VRF-export: DCI %s srcPR "
                        "%s dstPr %s " % (dci.name, srclrprs, dstlrprs))
                if dci.name in vrf_dst_dci_list:
                    cls._logger.error(
                        "PARAG BUG: DCI %s %s For SRC VRF Expost and DST VRF "
                        "Import BOTH EXISTs." % (dci.name, curpr))
                if dci.name not in vrf_dst_dci_list:
                    ...
                    if len(tvrflist) < 1:
                        cls._logger.debug(
                            "PARAG BUG: DCI %s %s VRF Export RP len 0" %
                            (dci.name, curpr))
                    for k, v in tvrflist.items():
                        if k in rp_list:
                            cls._logger.error(
                                "PARAG BUG: Duplicate RP %s %s VRF Export"
                                " RP %s" % (dci.name, curpr, k))
                        rp_list[k] = v
                        if k not in int_ri.get_vrf_export():
                            ...
                            cls._logger.error(
                                "PARAG BUG: rib Duplicate RP %s %s VRF Import"
                                " RP %s" % (dci.name, curpr, k))
...
class RoutingPolicyDM(DBBaseDM):
    ...
    def update(self, obj=None):
        ...
        if rp_entries:
            ...
            self.term_type = rp_entries.get('term_type', 'vrouter')
        self.name = obj['fq_name'][-1]
        self.fq_name = obj['fq_name']
        self.update_multiple_refs('virtual_network', obj)
        self.update_multiple_refs('data_center_interconnect', obj)
    ...
    def create_abstract_rpterm(cls, o_term):
        ...
        if o_term_action_list is not None:
            ...
            as_path_expand = o_term_action_list.get(
                'as_path_expand', None)
            as_path_prepend = o_term_action_list.get(
                'as_path_prepend', None)
        tcond = None
        taction = None
