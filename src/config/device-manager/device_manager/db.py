...
class DBBaseDM(DBBase):
    ...
    def _generate_job_transaction(self, oper_type, request_id=None,
        ...
        self._logger.debug("ENTER GEN_JOB_TRANS: {}: {}: {}: {}: {}: {}".\
                           format(oper_type, request_id,
                                  name, obj_descr, trans_descr,
                                  fabric_id))
        ...
        if fabric_id:
            ...
            if trans_id:
                trans_descr = t_descr
            self._logger.debug("GEN_JOB_TRANS: tid={}, descr={}".format(trans_id, trans_descr))
        if not trans_id:
            trans_id = request_id
...
class PhysicalRouterDM(DBBaseDM):
    ...
    def update(self, obj=None, request_id=None):
        if obj is None:
            obj = self.read_obj(self.uuid)
        self._logger.debug("JPW: PR_UPDATE: {}".format(obj))
    ...
    def _role_assignment_changed(self, obj):
        ...
        self._logger.debug("JPW: ROLE1: {}: {}".format(pr_role, new_pr_role))
        ...
        new_rb_roles = set(obj.get('routing_bridging_roles', {}).\
            get('rb_roles', []))
        self._logger.debug("JPW: ROLE2: {}: {}".format(rb_roles, new_rb_roles))
        ...
        self._logger.debug("JPW: ROLE3: {}: {}".format(physical_role, new_physical_role))
        ...
        self._logger.debug("JPW: ROLE4: {}: {}".format(overlay_roles, new_overlay_roles))
    ...
    def upd_job_trans(self, obj, request_id):
        ...
        f_trans_id, f_trans_descr = self._get_fabric_trans_info(self.fabric)

        # if trans_descr and \
        #         ('Fabric Deletion' in trans_descr or
        #             'Device Deletion' in trans_descr) and \
        #         trans_id != self.transaction_id:
        #     update = True
        if f_trans_id:
            trans_id = f_trans_id
            trans_descr = f_trans_descr
            update = True
...
class LogicalRouterDM(DBBaseDM):
    ...
    def do_update_trans(self, obj):
        if not self.updated or not self.data_center_interconnect:
            return True
        ...
        if obj.get("logical_router_gateway_external", False) != \
