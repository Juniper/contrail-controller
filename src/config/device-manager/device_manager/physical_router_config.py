...
class PhysicalRouterConfig(object):
    ...
    def add_routing_instance(self, name, import_targets, export_targets,
        ...
        if vni is not None:
            ...
            vtep = etree.Element("vtep-source-interface")
            etree.SubElement(vtep, "name").text = "lo0.0"

            interfaces_config = self.interfaces_config or etree.Element("interfaces")
            irb_intf = etree.SubElement(interfaces_config, "irb")
            etree.SubElement(irb_intf, "gratuitous-arp-reply")
            if gateways is not None:
                intf_unit = etree.SubElement(irb_intf, "unit")
                intf_unit.text = str(vni)
                family = etree.SubElement(intf_unit, "family")
                family.text = "inet"
                etree.SubElement(family, "address").text = ';'.join(gateways)

            lo_intf = etree.SubElement(interfaces_config, "lo0")
            intf_unit = etree.SubElement(lo_intf, "unit")
            intf_unit.text = "0"
            family = etree.SubElement(intf_unit, "family")
            family.text = "inet"
            etree.SubElement(family, "address").text = self.bgp_params['address']
            for interface in interfaces:
