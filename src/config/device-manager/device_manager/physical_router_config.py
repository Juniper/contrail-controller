...
"""
This file contains implementation of inetconf interface for physical router configuration manager
"""
...
class PhysicalRouterConfig(object):
    ...
    def add_routing_instance(self, name, route_target, prefixes=[], interfaces=[]):
    ...
    def delete_routing_instance(self, name):
        if name not in self.routing_instances:
            return
        ri_config = etree.Element("routing-instances")
    ...
    def set_bgp_config(self, params):
        ...
        etree.SubElement(self.bgp_config, "local-address").text = params['address']
    ...
    def delete_bgp_config(self):
        if not self.bgp_config:
            return
        for routing_instance in self.routing_instances:
            self.delete_routing_instance(routing_instance)
