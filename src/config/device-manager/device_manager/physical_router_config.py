...
class PhysicalRouterConfig(object):
    ...
    def add_routing_instance(self, ri_conf):
        ...
        if fip_map is not None:
            ...
            intf_unit = Unit(name=str(network_id), comment=DMUtils.vn_irb_fip_inet_comment(vn))
        ...
        if (is_l2 and vni is not None and
            ...
            if is_l2_l3:
                ...
                if gateways is not None:
                    intf_unit = Unit(name=str(network_id), comment=DMUtils.vn_irb_comment(vn, False, is_l2_l3))
                    irb_intf.add_unit(intf_unit)
                    family = Family()
            ...
            fam_inet = FamilyInet(address=[Address(name=self.bgp_params['address'] + "/32",
                                                   primary='', preferred='')])
            intf_unit = Unit(name="0", family=Family(inet=fam_inet), comment=DMUtils.lo0_unit_0_comment())
        ...
        if (not is_l2 and not is_l2_l3 and gateways):
            ...
            ri.add_interface(Interface(name="lo0." + ifl_num, comment=DMUtils.lo0_ri_intf_comment(vn)))
        ...
        if fip_map is not None:
            ...
            interfaces_config = self.interfaces_config or Interfaces(comment=DMUtils.interfaces_comment())
            si_intf = Interface(name=interfaces[0].ifd_name, comment=DMUtils.service_ifd_comment())
            interfaces_config.add_interface(si_intf)

            intf_unit = Unit(name=interfaces[0].unit, comment=DMUtils.service_intf_comment("Ingress"))
            si_intf.add_unit(intf_unit)
            family = Family(inet=FamilyInet())
            intf_unit.set_family(family)
            intf_unit.set_service_domain("inside")

            intf_unit = Unit(name=interfaces[1].unit, comment=DMUtils.service_intf_comment("Egress"))
            si_intf.add_unit(intf_unit)
            family = Family(inet=FamilyInet())
            intf_unit.set_family(family)
