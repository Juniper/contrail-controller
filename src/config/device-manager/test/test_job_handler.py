...
class TestJobHandler(unittest.TestCase):
    ...
    def test_check_status_throws(self):
        ...
        self.assertEqual(self.sleep_mock.call_args_list[0], mock.call(self.TIMEOUT))
        self.assertEqual(len(self.sleep_mock.call_args_list), self.MAX_RETRIES - 1)
        self.assertEqual(self.vnc_api_obj.job_status.call_count, 2 * self.MAX_RETRIES)
        self.assertTrue(self.job_handler.is_job_done())
        self.assertEqual(self.job_handler.get_job_status(), JobStatus.FAILED)
