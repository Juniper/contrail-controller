...
class TestAnsibleSecurityGroupsDM(TestAnsibleCommonDM):
    ...
    def test_02_multiple_vpgs_same_vlan(self):
	#create SG and ACL
        sg1_obj = self.security_group_create('sg-1', ['default-domain',
                                                      'default-project'])
        self.wait_for_get_sg_id(sg1_obj.get_fq_name())
        ...
        pr1, fabric, pi_obj_1, pi_obj_2, vn_obj, _, _ = self.create_vpg_dependencies()
        vmi_obj_1 = self.create_vpg_and_vmi(sg1_obj, pr1, fabric, pi_obj_1, vn_obj)
        vmi_obj_2 = self.create_vpg_and_vmi(sg1_obj, pr1, fabric, pi_obj_2, vn_obj, None, None, None, vpg_nm=2)
        ...
        phy_interfaces = device_abstract_config.get(
            'physical_interfaces', [])
        i = 1
        while i < len(phy_interfaces):
            phy_intf = phy_interfaces[i]
            i+=1
            log_intfs = phy_intf.get('logical_interfaces', [])
            for log_intf in log_intfs:
                if "xe-0/0/0" in log_intf.get('name'):
                    if log_intf.get('unit') == '10':
                        filter = log_intf.get('firewall_filters')[0]
                        self.assertEqual(filter, 
                                         "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid)
                    else:
                        raise Exception("SG firewall filter not configured on intf")
	...
	#create SG and ACL
        ...
        logical_interfaces = device_abstract_config.get(
            'physical_interfaces', [])[0].get('logical_interfaces', [])
        self.assertEqual(logical_interfaces[0].get('name'), "ae0.10")
        self.assertEqual(logical_interfaces[0].get('firewall_filters')[0], 
                         "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid)
	...
	#create SG and ACL
        ...
        firewall = device_abstract_config_1.get(
            'firewall', {})
        print device_abstract_config_1
        if len(firewall) == 0:
            raise Exception("No firewalls configured")
        firewall_filters = firewall.get('firewall_filters', [])
        if len(firewall_filters) == 0:
        ...
        logical_interfaces = device_abstract_config_1.get(
            'physical_interfaces', [])[0].get('logical_interfaces', [])
        self.assertEqual(logical_interfaces[0].get('name'), "ae0.10")
        self.assertEqual(logical_interfaces[0].get('firewall_filters')[0], 
                         "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid)
        ...
        logical_interfaces = device_abstract_config_2.get(
            'physical_interfaces', [])[0].get('logical_interfaces', [])
        self.assertEqual(logical_interfaces[0].get('name'), "ae0.10")
        self.assertEqual(logical_interfaces[0].get('firewall_filters')[0], 
                         "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid)
    ...
    def test_05_vpg_mh_and_single(self):
        # create objects
        sg1_obj = self.security_group_create('sg-1', ['default-domain',
                                                      'default-project'])
        self.wait_for_get_sg_id(sg1_obj.get_fq_name())
        sg1_obj = self._vnc_lib.security_group_read(sg1_obj.get_fq_name())
        prefix1 = '6.6.6.0/24'
        rule1 = self.build_acl_rule(0, 65535, 'egress', prefix1, 'icmp', 'IPv4')
        sg_rule1 = self._security_group_rule_build(rule1,
                                                   sg1_obj.get_fq_name_str())
        self._security_group_rule_append(sg1_obj, sg_rule1)
        self._vnc_lib.security_group_update(sg1_obj)
        ...
        prefix2 = '7.7.7.0/24'
        rule2 = self.build_acl_rule(0, 65535, 'egress', prefix2, 'icmp', 'IPv4')
        ...
        vmi_obj1 = self.create_vpg_and_vmi(sg1_obj, pr1, fabric, pi_obj_1, vn_obj,
                                          pi_obj2=pi_obj_1_pr2, pr2=pr2)
        vmi_obj2 = self.create_vpg_and_vmi(sg2_obj, pr1, fabric,
        ...
        acl_uuid1 = self.get_acl_uuid(sg1_obj)
        sg1_obj_fqname = sg1_obj.get_fq_name()
        acl_uuid2 = self.get_acl_uuid(sg2_obj)
        sg2_obj_fqname = sg2_obj.get_fq_name()
...
#        firewall_filter = firewall_filters[0]
        for firewall_filter in firewall_filters:
            ...
            destaddr = acl.get('fromxx', {}).get('destination_address', {})
            destprefix = destaddr[0].get('prefix')
            if firewall_filter.get('name') == "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid1:
                self.assertEqual(destprefix, prefix1.split('/')[0])
            else:
                self.assertEqual(destprefix, prefix2.split('/')[0])
              
        logical_interfaces = device_abstract_config_1.get(
            'physical_interfaces', [])[0].get('logical_interfaces', [])
        for log_intf in logical_interfaces:
            if "xe-0/0/1" in log_intf.get('name'):
                self.assertEqual(log_intf.get('firewall_filters')[0], 
                                 "sg-filter-"+ sg2_obj_fqname[-1] + "-" + acl_uuid2)
            else:
                self.assertEqual(log_intf.get('name'), "ae0.10")
                self.assertEqual(log_intf.get('firewall_filters')[0], 
                                 "sg-filter-"+ sg1_obj_fqname[-1] + "-" + acl_uuid1)
    ...
    def create_vpg_and_vmi(self, sg_obj, pr1, fabric, pi_obj,
        ...
        if pp_obj_2:
            vmi_obj_2.set_security_group(sg_obj)
            self._vnc_lib.virtual_machine_interface_create(vmi_obj_2)

            li2 = LogicalInterface('li2.0', parent_obj = pi_obj2)
            li2.set_logical_interface_vlan_tag(100)
            li2.set_virtual_machine_interface(vmi_obj_2)
