...
class TestAnsibleSecurityGroupsDM(TestAnsibleCommonDM):
	...
	#create SG and ACL
        ...
        firewall = device_abstract_config.get(
            'features', {}).get('firewall', {}).get('firewall', {})
        if len(firewall) == 0:
	...
	#create SG and ACL
        ...
        phy_interfaces = device_abstract_config.get(
            'features', {}).get('firewall', {}).get('physical_interfaces', [])
        i = 1
	...
	#create SG and ACL
        ...
        logical_interfaces = device_abstract_config.get(
            'features', {}).get('firewall', {}).get('physical_interfaces', [])[0].get(
            'logical_interfaces', [])
...
#        firewall_filter = firewall_filters[0]
        ...
        self.assertEqual(device_abstract_config_2.get('system', {}).get(
            'management_ip'), "3.3.3.2")
        firewall = device_abstract_config_2.get(
            'features', {}).get('firewall', {}).get('firewall', {})
        if len(firewall) == 0:
            raise Exception("No firewalls configured")
        firewall_filters = firewall.get('firewall_filters', [])
