...
class TestE2DM(test_case.DMTestCase):
    ...
    def test_dm_e2_vrs(self):
        ...
        # Verify VRR config
        gevent.sleep(2)
        xml_config_str = '<config xmlns:xc="urn:ietf:params:xml:ns:netconf:base:1.0"><configuration><groups operation="replace"><name>__contrail-e2__</name><system><host-name>test.test_dm_e2.TestE2DM.test_dm_e2_vrs-e2vrr</host-name><services><extension-service><request-response><grpc><clear-text/><skip-authentication/></grpc></request-response><notification><allow-clients><address>0.0.0.0/0</address></allow-clients></notification></extension-service></services></system><routing-options><router-id>10.92.36.9</router-id><autonomous-system><as-number>100</as-number></autonomous-system></routing-options><policy-options><policy-statement><name>filter-global-comms</name><term><name>block-all</name><from><community>block-all-comm</community></from><then><reject/></then></term><then><next>policy</next></then></policy-statement><policy-statement><name>import-from-all-inst</name><term><name>from-any</name><from><instance-any/></from><then><accept/></then></term><then><reject/></then></policy-statement><community><name>block-all-comm</name><members>0:100</members></community><community><name>to-all-comm</name><members>100:100</members></community><community><name>to-wildcard-comm</name><members>^1:[0-9]*$</members></community></policy-options></groups><apply-groups insert="first">__contrail-e2__</apply-groups></configuration></config>'
        self.check_netconf_config_mesg('10.92.36.9', xml_config_str)
