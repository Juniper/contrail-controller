...
#
# All E2 related DM test cases should go here
#
class TestE2DM(TestCommonDM):
    ...
    def check_if_xml_is_generated(self):
        ...
        if not xml_conf or 'config' not in xml_conf or 'apply-groups' \
           not in xml_conf:
    ...
    def test_dm_unsupported_device(self):
        # product name, vendor is still configured as mx-e2, juniper
        # but actual device is not mx
        bgp_router, pr = self.create_router('router1' + self.id(),
                                            '199.199.199.199',
                                            product=self.product)
    ...
    def test_dm_instance(self):
        ...
        kill_device_manager(TestE2DM._dm_greenlet)
        self.check_dm_instance()
        TestE2DM._dm_greenlet = gevent.spawn(launch_device_manager,
            self._cluster_id, TestE2DM._api_server_ip,
            TestE2DM._api_server_port)
    ...
    def test_dm_plugins(self):
        ...
        FakeDeviceConnect.reset()
        FakeNetconfManager.set_model('mx-e2')
        pr.physical_router_vendor_name = "juniper"
        pr.physical_router_product_name = "mx-e2"
        self._vnc_lib.physical_router_update(pr)
        self.check_dm_plugin()

        FakeDeviceConnect.reset()
        FakeNetconfManager.set_model('alu')
        pr.physical_router_vendor_name = "nokia"
        pr.physical_router_product_name = "alu"
        self._vnc_lib.physical_router_update(pr)
        self.check_dm_plugin()
