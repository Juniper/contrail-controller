...
class TestAnsibleStormControlDM(TestAnsibleCommonDM):
    ...
    def test_storm_control_profile_update(self):
        ...
        pp_obj = self.create_port_profile('port_profile')
    ...
    def create_vpg_and_vmi(self, pp_obj):
        ...
        jt = self.create_job_template('job-template-sc')
        fabric = self.create_fabric('fab-sc')
        np, rc = self.create_node_profile('node-profile-sc',
        ...
        vmi_profile = "{\"local_link_information\":[{\"switch_id\":\"11:11:11:11:11:11\",\"port_id\":\"%s\",\"switch_info\":\"%s\",\"fabric\":\"%s\"}]}" % ('xe-0/0/0', 'device-sc', 'fab-sc')
        ...
        vn_obj = VirtualNetwork(fq_name=["default-domain",
            "admin", "vn-sc"], parent_type='project')
