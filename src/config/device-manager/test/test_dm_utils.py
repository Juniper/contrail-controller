...
class FakeJobHandler(object):
    ...
    @classmethod
    def send(cls, plugin, job_template, job_input):
        cls.params = {
            'plugin': plugin,
...
def fake_job_handler_push(plugin, job_template, job_input):
    return FakeJobHandler.send(plugin, job_template, job_input)
