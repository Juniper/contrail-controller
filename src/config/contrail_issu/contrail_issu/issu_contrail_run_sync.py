...
class ICRMQMain():
    ...
    def start(self):
        ...
        self.amqp_new_version_handle = VncKombuClient(
                rabbit_hosts=self.new_rabbit.amqp_ip,
                rabbit_port=self.new_rabbit.amqp_port,
                rabbit_user=self.new_rabbit.amqp_user,
                rabbit_password=self.new_rabbit.amqp_pwd,
                rabbit_vhost=self.new_rabbit.amqp_vhost,
                rabbit_ha_mode=self.new_rabbit.amqp_ha,
                q_name=self.new_rabbit.amqp_q,
                subscribe_cb=self.logger,
                logger=self.logger,
        ...
        self.amqp_old_version_handle = ICKombuClient(
            rabbit_ip=self.old_rabbit.amqp_ip,
            rabbit_port=self.old_rabbit.amqp_port,
            rabbit_user=self.old_rabbit.amqp_user,
            rabbit_password=self.old_rabbit.amqp_pwd,
            rabbit_vhost=self.old_rabbit.amqp_vhost,
            rabbit_ha_mode=self.old_rabbit.amqp_ha,
            q_name=self.old_rabbit.amqp_q,
            subscribe_cb=self.issu_amqp_callback_handler,
            logger=self.logger,
            cassandra_inst=self.issu_cass_config_db_uuid_handle,
            keyspace=self.keyspace_info,
            new_api_info=self.new_api_info,
