...
public:
    ...
    ConfigClientManager(EventManager *evm, std::string hostname, 
                        ...
                        boost::function<void(const contrail_rapidjson::Document &)>);
...
//    ConfigJsonParser *config_json_parser() const;
    ...
    void EnqueueUUIDRequest(std::string oper, std::string obj_type,
                            std::string uuid_str);
    void NotifyChange(const contrail_rapidjson::Document &jdoc);
...
//    boost::scoped_ptr<ConfigJsonParser> config_json_parser_;
    ...
    boost::function<void(const contrail_rapidjson::Document &)> cfg_change_callback_;
};
