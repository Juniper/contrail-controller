...
void ConfigCass2JsonAdapter::AddOneEntry(Value *jsonObject,
    ...
    if ((is_map = boost::starts_with(c.key, map_prop_prefix)) ||
        ...
        string wrapper = cassandra_client_->mgr()-> \
                 config_client_json_parser()->GetWrapperFieldName(type_,
                                                       prop_map);
    ...
    if (boost::starts_with(c.key, ref_prefix)) {
        ...
        bool link_with_attr =
            cassandra_client_->mgr()->config_client_json_parser()->\
...
void ConfigCass2JsonAdapter::CreateJsonString(const string &obj_type,
    ...
    if (type_.empty()) {
        CONFIG_CLIENT_WARN_LOG(ConfigurationMissingTypeWarningLog, Category::CONFIG_CLIENT,
                       uuid_, obj_type);
        CONFIG_CLIENT_TRACE(ConfigurationMissingTypeWarningTrace, uuid_, obj_type);
        return;
    }
