...
class OpenstackDriver(vnc_plugin_base.Resync):
    ...
    def _resync_projects_forever(self):
        ...
        while True:
            try:
                # compare new and old set, optimize for common case where nothing
                # has changed, so track the project-ids in a set
                # add '-', keystone gives uuid without...
                new_project_ids = set([str(uuid.UUID(proj.id)) for proj in kc.tenants.list()])
                ...
                for new_proj_id in new_project_ids - old_project_ids:
                    try:
                        ...
                        # project exists, no-op for now, sync any attr changes in future
