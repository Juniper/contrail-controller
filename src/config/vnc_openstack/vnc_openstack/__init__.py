...
class OpenstackDriver(vnc_plugin_base.Resync):
    def __init__(self, api_server_obj):
        ...
        self._config_sections = api_server_obj._args.conf_sections
        self._sandesh_logger = api_server_obj._sandesh.logger()

        fill_keystone_opts(self, self._conf_sections)
        self._ks = None
    ...
    def _get_keystone_conn(self):
        ...
        ConnectionState.update(conn_type=ConnType.OTHER, name='Keystone',
                               ...
                               server_addrs=[self._auth_url])
        # TODO(ethuleau): sync domain/project of the creds used by that
        # keystone admin client connection
    ...
    def _ksv3_sync_project_to_vnc(self, id=None):
        ...
        dom_obj_dict = self._api_server_obj.internal_request_read('domain', domain_uuid)[1]['domain']
        ...
        try:
            old_id = self._api_server_obj.internal_request_fq_name_to_uuid('project', fq_name)[1]['uuid']
