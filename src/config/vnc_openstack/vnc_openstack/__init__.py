...
def fill_discovery_opts(obj, conf_sections):
    try:
        ...
        obj._disc_server_api = conf_sections.get('DEFAULTS', 'disc_server_api')
        if obj._disc_server_api[0] == '/':
            obj._disc_server_api = obj._disc_server_api[1:]
        obj._disc_server_api_url = "http://%s:%s/%s" % (
            obj._disc_server_ip, obj._disc_server_port, obj._disc_server_api)
        obj._disc_api_headers = { 'Content-type': 'application/json' }
    except ConfigParser.NoOptionError:
        obj.__disc_server_api_url = None
...
class ResourceApiDriver(vnc_plugin_base.ResourceApi):
    ...
    def post_dsa_rule_create(self, obj_dict):
        if self._disc_server_api_url is None:
            return
        body = { 'op':'create', 'obj_type': 'dsa_rule' }
        requests.put(self._disc_server_api_url, data=json.dumps(body),
