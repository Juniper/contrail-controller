...
class OpenstackDriver(vnc_plugin_base.Resync):
    ...
    def _get_keystone_conn(self):
        ...
        if self._ks.version == 'v3':
            ...
            self.ks_project_get = self._ksv3_project_get
            ...
            self.ks_project_get = self._ksv2_project_get
    ...
    def _ksv2_project_get(self, id=None, name=None):
        # public method - need to ensure connections
    ...
    def _ksv2_sync_project_to_vnc(self, id=None):
        self._get_keystone_conn()
        self._get_vnc_conn()
        ks_project = self.ks_project_get(id=id.replace('-', ''))
    ...
    def _ksv3_sync_project_to_vnc(self, id=None, name=None):
        ...
        if id:
            ks_project = \
                self.ks_project_get(id=id.replace('-', ''))
            display_name = ks_project['name']
            project_id = id
        elif name:
            ks_project = \
                self.ks_project_get(name=name)
    ...
    def _resync_domains_projects_forever(self):
        # because this class is used only with OpenStack
        # - it should initiate connection to keystone first
        while True:
            try:
                self._get_keystone_conn()
            except Exception as e:
        ...
        while True:
            try:
                ...
                # all data was synced. let's continue.
                break
            except Exception as e:
                self.log_exception()
                self._cgitb_error_log()
...
class ResourceApiDriver(vnc_plugin_base.ResourceApi):
    def __init__(self, api_server_ip, api_server_port, conf_sections, sandesh,
                 propagate_map_exceptions=False, resync_extension_manager=None):
        if api_server_ip == '0.0.0.0':
            self._vnc_api_ip = '127.0.0.1'
        else:
            self._vnc_api_ip = api_server_ip
    ...
    def pre_project_read_fqname(self, fq_name):
        ...
        if not self._keystone_sync_on_demand or fq_name == None:
            ...
            return
        name = fq_name[-1]
        id = self._resync_extension_manager.map_method(ks_project_get, id=None, name=name).get('id')
        if not id:
        ...
        try:
            ...
            pass
        self._resync_extension_manager.map_method(_vnc_project_ids.add, id)
