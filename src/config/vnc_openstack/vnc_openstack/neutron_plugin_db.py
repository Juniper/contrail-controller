...
class DBInterface(object):
    ...
    def _firewall_group_neutron_to_vnc(self, firewall_group, id=None):
        ...
        if (not ingress == attr_not_specified or
                not egress == attr_not_specified):
    ...
    def _application_policy_set_vnc_to_neutron(self, aps, fields=None):
        ...
        if len(fp_refs) > 1:
            msg = ("Application Policy Set %s(%s) corresponding to a Neutron "
                   ...
                   "reference, use first one and remove others." %
                   (aps.get_fq_name_str(), aps.uuid))
            self.logger.warning(msg)
            for fp_ref in fp_refs[1:]:
                self._vnc_lib.ref_update(
                    ...
                    'DELETE')
        if fp_refs:
            fwg['ingress_firewall_policy_id'] = fp_refs[0]['uuid']
            fwg['egress_firewall_policy_id'] = fp_refs[0]['uuid']
