...
class DBInterface(object):
    ...
    def _router_set_external_gateway(self, router_obj, external_network):
        # Set logical gateway virtual network
        router_obj.set_virtual_network(external_network)
    ...
    def remove_router_interface(self, router_id, port_id=None, subnet_id=None):
        ...
        if port_id:
            ...
            for intf in router_obj.get_virtual_machine_interface_refs() or []:
                port_id = intf['uuid']
                port_db = self.port_read(port_id)
                if 'fixed_ips' not in port_db or not port_db['fixed_ips']:
                    continue
