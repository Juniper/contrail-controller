...
class DBInterface(object):
    ...
    def _network_neutron_to_vnc(self, network_q, oper):
        ...
        physical_network = ''
        if 'provider:physical_network' in network_q:
            physical_network = network_q['provider:physical_network']

        segmentation_id = -1
        if 'provider:segmentation_id' in network_q:
            segmentation_id = network_q['provider:segmentation_id']

        if segmentation_id != -1 or physical_network != '':
            net_obj.set_provider_properties(ProviderDetails(segmentation_id, physical_network))
    ...
    def _port_neutron_to_vnc(self, port_q, net_obj, oper):
        ...
        if not bindings:
            kvp = []
            vnic_type = port_q.get('binding:vnic_type', 'normal')
            kvp.append(KeyValuePair('vnic_type', vnic_type))
            port_obj.virtual_machine_interface_bindings = KeyValuePairs(kvp)
            bindings = port_obj.get_virtual_machine_interface_bindings()
        ...
        if (not host_id_modify) and (port_q.has_key('binding:host_id')):
            bindings.add_key_value_pair(KeyValuePair('host_id', port_q['binding:host_id']))
        if (not profile_modify) and (port_q.has_key('binding:profile')):
            bindings.add_key_value_pair(KeyValuePair('profile', port_q['binding:profile']))


        port_obj.set_virtual_machine_interface_bindings(bindings)
