...
class DBInterface(object):
    ...
    def _get_router_gw_interface_for_neutron(self, context, router):
        ...
        # name
        sorted_vm_ref = sorted(si.get_virtual_machine_back_refs() or [],
                        key=lambda ref: ref['to'][-1])
        if len(sorted_vm_ref) >= 1:
            ...
            vmis = self._virtual_machine_interface_list(
                back_ref_id=[sorted_vm_ref[0]['uuid']], filters=filters)
            sorted_vmis = sorted(vmis, key=lambda vmi: vmi.name)
            return sorted_vmis[0] if len(sorted_vmis) >= 1 else None
