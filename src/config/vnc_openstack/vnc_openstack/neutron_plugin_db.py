...
_SG_NO_RULE_FQ_NAME = ['default-domain', 'default-project', _SG_NO_RULE_NAME]
...
class DBInterface(object):
    ...
    def _create_no_rule_sg(self):
        domain_obj = Domain(_SG_NO_RULE_FQ_NAME[0])
        proj_obj = Project(_SG_NO_RULE_FQ_NAME[1], domain_obj)
        proj = self._vnc_lib.project_read(fq_name=_SG_NO_RULE_FQ_NAME[:2])
        ...
        return self._vnc_lib.security_group_read(sg_uuid)
    # end _create_no_rule_sg

    def _get_no_rule_security_group(self, proj_obj):
        try:
            sg_obj = self._vnc_lib.security_group_read(fq_name=_SG_NO_RULE_FQ_NAME)
        except NoIdError:
    ...
    def _port_vnc_to_neutron(self, port_obj, port_req_memo=None):
        ...
        for sg_ref in sg_refs or []:
            if 'to' in sg_ref and sg_ref['to'] == _SG_NO_RULE_FQ_NAME:
        ...
        for project_sgs in all_sgs:
            for sg_obj in project_sgs:
                ...
                if not self._filters_is_present(filters, 'name',
                    ...
                    continue
                if sg_obj.get_fq_name() == _SG_NO_RULE_FQ_NAME:
