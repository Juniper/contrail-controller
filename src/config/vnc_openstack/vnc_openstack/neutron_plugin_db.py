...
class DBInterface(object):
    ...
    def _trunk_neutron_to_vnc(self, trunk_q, oper, id=None):
        ...
        def _set_sub_ports(trunk, trunk_port, sp_id):
            sub_port = self._vnc_lib.virtual_machine_interface_read(id=sp_id)
        ...
        if oper == CREATE:
            ...
            trunk_port = self._vnc_lib.virtual_machine_interface_read(id=port_id)
    ...
    def _trunk_vnc_to_neutron(self, trunk_obj, context=None):
	    ...
	    'sub_ports': trunk_obj.get_virtual_machine_interface_refs()
                         if trunk_obj.get_virtual_machine_interface_refs() else
                         [],
    ...
    def trunk_read(self, id, fields=None, context=None):
        ...
        return self._trunk_vnc_to_neutron(trunk, context=context)
    ...
    def trunk_list(self, context, filters, fields):
        ...
        for trunk in trunks:
            if (shared and trunk.get_perms2().owner.replace('-', '') ==
                    context['tenant_id']):
                continue
            results.append(self._trunk_policy_vnc_to_neutron(trunk, context))
    ...
    def trunk_remove_subports(self, context, id, sub_ports):
        ...
        :param sub_ports: sub ports info that needs to be removed from a
         trunk object.
        """
        trunk_obj = self._trunk_neutron_to_vnc(sub_ports, DELETE, id)
        self._resource_update('virtual_port_group', trunk_obj)
        ret_trunk_q = self._trunk_vnc_to_neutron(
            self._vnc_lib.virtual_port_group_read(id=id))
        return ret_trunk_q
