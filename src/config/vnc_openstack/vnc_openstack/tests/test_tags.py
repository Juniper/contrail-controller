...
class TestNeutronTags(test_case.NeutronBackendTestCase):
    ...
    def _create_fip_pool(self, name):
        vn = VirtualNetwork('vn-fip-{}'.format(name),
                            parent_obj=self.project)
        vn.add_network_ipam(NetworkIpam(), VnSubnetsType([
            IpamSubnetType(SubnetType('1.1.1.0', 28)),
        ]))
        vn.uuid = self.api.virtual_network_create(vn)
        fip_pool_uuid = self.api.floating_ip_pool_create(
            FloatingIpPool('fip-pool-{}'.format(name), parent_obj=vn))
        fip_pool = self.api.floating_ip_read(id=fip_pool_uuid)
        return fip_pool

    def _pre_create_floating_ips(self):
        # TODO: fix it
        fip_pool = self._create_fip_pool(self.id())
        self.fips = {}
        for tag in [self.TAG_RED, self.TAG_BLUE, self.TAG_GREEN]:
            fip = FloatingIp('fip-{}-{}'.format(tag, self.id()),
                             parent_obj=fip_pool)
            fip.add_tag(self._get_tag(tag))
            fip.uuid = self.api.floating_ip_create(fip)
            self.fips[tag] = self.api.floating_ip_read(id=fip.uuid)

        fip_notag = FloatingIp('fip-{}-{}'.format(self.NO_TAG, self.id()),
                               parent_obj=fip_pool)
        fip_notag.uuid = self.api.floating_ip_create(fip_notag)
        self.fips[self.NO_TAG] = self.api.floating_ip_read(id=fip_notag.uuid)
    ...
    def test_query_floating_ip_with_one_tag(self):
        ...
        self.assertEqual(expected_count, len(result))
        # check if virtual network uuid match
        self.assertEqual(self.fips[self.TAG_RED].uuid, result[0]['id'])
