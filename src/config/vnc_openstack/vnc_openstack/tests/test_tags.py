...
class TestSubnetNeutronTags(NeutronTagsTestCase):
    ...
    def test_query_subnet_by_one_tags_performance(self):
        """
        Test performance of querying VN by one tag.
        Average time is about 7.1 milliseconds per one request.
        """
        start = int(round(time.time() * 1000))
        for _ in range(1000):
            self.list_resource(
        ...
        expected_time_milliseconds = 7200
        self.assertLessEqual(end - start, expected_time_milliseconds)
    ...
    def test_query_subnet_by_multiple_tags_performance(self):
        """
        Test performance of querying VN by multiple tag (full match).
        Average time is about 9.7 ms per one request.
        """
        start = int(round(time.time() * 1000))
        for _ in range(1000):
            self.list_resource(
        ...
        expected_time_milliseconds = 9800
        self.assertLessEqual(end - start, expected_time_milliseconds)
    ...
    def test_query_subnet_by_multiple_any_tags_performance(self):
        """
        Test performance of querying VN by multiple tag (match any).
        Average time is about 21.6 milliseconds per one request.
        """
        start = int(round(time.time() * 1000))
        for _ in range(1000):
            self.list_resource(
        ...
        expected_time_milliseconds = 22000
        self.assertLessEqual(end - start, expected_time_milliseconds)
