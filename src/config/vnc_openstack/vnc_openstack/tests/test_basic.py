...
class TestBasic(test_case.NeutronBackendTestCase):
    ...
    def test_list_router_gw_interfaces_with_not_owned_public_network(self):
        # Project which will owne the public network
        proj1_id = str(uuid.uuid4())
        proj1_name = 'proj1-%s' % self.id()
        test_case.get_keystone_client().tenants.add_tenant(proj1_id,
                                                           proj1_name)
        project1 = self._vnc_lib.project_read(id=proj1_id)

        # Private project which will own private network and router
        proj2_id = str(uuid.uuid4())
        proj2_name = 'proj-%s' % self.id()
        test_case.get_keystone_client().tenants.add_tenant(proj2_id,
                                                           proj2_name)
        project2 = self._vnc_lib.project_read(id=proj2_id)

        # public network/subnet on project1
        public_net = self.create_resource(
            'network', project1.uuid, 'public-%s' % self.id(),
            extra_res_fields={'router:external': True},
        )
        self.create_resource(
            'subnet', project1.uuid, 'public-%s' % self.id(),
            extra_res_fields={
        ...
        self._vnc_lib.virtual_network_read(id=public_net['id'])

        # private network/subnet on classic project
        privat_net = self.create_resource('network', project2.uuid,
                                          name='private-%s' % self.id())
        private_subnet = self.create_resource(
            'subnet', project2.uuid, 'private-%s' % self.id(),
            extra_res_fields={
        ...
        router = self.create_resource(
            'router', project2.uuid, name='router-%s' % self.id(),
            extra_res_fields={
                'external_gateway_info': {
                    'network_id': public_net['id'],
                }
    ...
    def test_list_router_gw_interfaces_with_owned_public_network(self):
        ...
        test_case.get_keystone_client().tenants.add_tenant(proj_id, proj_name)
        project = self._vnc_lib.project_read(id=proj_id)

        # public network/subnet
        public_net = self.create_resource(
            'network', project.uuid, 'public-%s' % self.id(),
            extra_res_fields={'router:external': True},
        )
        self.create_resource(
            'subnet', project.uuid, 'public-%s' % self.id(),
            extra_res_fields={
                'network_id': public_net['id'],
                'cidr': '80.0.0.0/24',
                'ip_version': 4,
        ...
        privat_net = self.create_resource('network', project.uuid,
                                          name='private-%s' % self.id())
        private_subnet = self.create_resource(
            'subnet', project.uuid, 'private-%s' % self.id(),
            extra_res_fields={
                'network_id': privat_net['id'],
                'cidr': '10.0.0.0/24',
                'ip_version': 4,
        ...
        # Router on project with a gateway on the public network
        # (owned by the same project)
        router = self.create_resource(
            'router', project.uuid, name='router-%s' % self.id(),
            extra_res_fields={
                'external_gateway_info': {
                    'network_id': public_net['id'],
                }
        ...
        # Add router interface on the private network/subnet
        self.update_resource(
            'router', router['id'], project.uuid, is_admin=True,
            operation='ADDINTERFACE',
            extra_res_fields={'subnet_id': private_subnet['id']},
        )
