...
class TestBasic(test_case.NeutronBackendTestCase):
    ...
    def test_list_router_interfaces(self):
        ...
        def fake_vm_obj(orig_method, *args, **kwargs):
            if 'id' in kwargs and kwargs['id'] == 'fake_vm_uuid':
                vm = mock.Mock()
                vm.get_virtual_machine_interface_back_refs.return_value = \
                    [{'to': 'fake_vmi_name', 'uuid': 'fake_vmi_uuid'}]
                return vm
            return orig_method(*args, **kwargs)

        def return_router_gw_interface(orig_method, *args, **kwargs):
            if 'fq_name' in kwargs and kwargs['id'] == 'fake_vmi_uuid':
        ...
        with test_common.patch(neutron_db_obj._vnc_lib, 'logical_routers_list',
                ...
                test_common.patch(neutron_db_obj._vnc_lib,
                                  'virtual_machine_read', fake_vm_obj), \
                test_common.patch(neutron_db_obj._vnc_lib,
