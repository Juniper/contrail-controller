...
class TestServicePolicy(STTestCase, VerifyServicePolicy):
    ...
    def test_evpn_service_policy_with_v4_v6_subnets(self):
        ...
        left_int_vn_obj = self._vnc_lib.virtual_network_read(fq_name=left_int_vn_fq_name)
        ...
        right_int_vn_obj = self._vnc_lib.virtual_network_read(fq_name=right_int_vn_fq_name)
        ...
        sc_ri_name = 'service-' + sc + '-default-domain_default-project_' + \
                     service_name
        self.check_ri_ref_present(self.get_ri_name(left_int_vn_obj),
                                  self.get_ri_name(left_int_vn_obj, sc_ri_name))
        self.check_ri_ref_present(self.get_ri_name(right_int_vn_obj, sc_ri_name),
        ...
        sci = ServiceChainInfo(
            ...
            service_instance='default-domain:default-project:' +
                             service_name,
            source_routing_instance=':'.join(self.get_ri_name(right_int_vn_obj)))
        self.check_evpn_service_chain_info(self.get_ri_name(right_int_vn_obj, sc_ri_name),
                                      sci)
        sci.prefix = ['1000::/16']
        sci.service_chain_address = '::0.255.255.251'
        self.check_evpn_v6_service_chain_info(self.get_ri_name(right_int_vn_obj, sc_ri_name),
                                         sci)
        ...
        sci = ServiceChainInfo(
            ...
            service_chain_address='0.255.255.252',
            service_instance='default-domain:default-project:' + service_name,
            source_routing_instance=':'.join(self.get_ri_name(left_int_vn_obj)))
        self.check_evpn_service_chain_info(self.get_ri_name(left_int_vn_obj, sc_ri_name),
                                      sci)
        sci.prefix = ['2000::/16']
        sci.service_chain_address = '::0.255.255.252'
        self.check_evpn_v6_service_chain_info(self.get_ri_name(left_int_vn_obj, sc_ri_name),
                                         sci)
