...
class SchemaTransformerEtcd(vnc_object_db.VncObjectEtcdClient):
    ...
    def allocate_service_chain_vlan(self, service_vm, service_chain):
        if service_vm not in self._sc_vlan_allocator_dict:
            ...
            self._sc_vlan_allocator_dict[service_vm] = int_pool
            try:
                self._vnc_lib.create_int_pool(int_pool,
                                              self._SERVICE_CHAIN_MIN_VLAN,
                                              self._SERVICE_CHAIN_MAX_VLAN)
            except RefsExistError:
                pass  # int pool already allocated

        alloc_kv = self._sc_vlan_allocator_dict[service_vm] + service_chain
        try:
        ...
        vlan_int = self._vnc_lib.allocate_int(self._sc_vlan_allocator_dict[service_vm])
    ...
    def free_service_chain_vlan(self, service_vm, service_chain):
        if service_vm in self._sc_vlan_allocator_dict:
            int_pool = self._sc_vlan_allocator_dict[service_vm]
            alloc_kv = int_pool + service_chain
