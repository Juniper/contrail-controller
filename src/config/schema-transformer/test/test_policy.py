...
class TestPolicy(STTestCase, VerifyPolicy):
    ...
    def test_provider_network(self):
        ...
        Verify:
            ...
            2. Check is_provider_network property of a
               provider-VN can be set as True
            3. Check is_provider_network property of provider-VN
               can not be set as False
            4. Check is_provider_network property of non provider-VN
               can be set as False
            5. Check is_provider_network property of non provider-VN
               can  not be set as True
            6. Check db_resync sets is_provider_network property
               of provider-VN as True (simulating upgrade case)
            7. Check non provider VNs can be added to
               provider VN
            8. Check the provider-VN can be added to a VN
            9. Check non provider-VN can not be added to a VN
            10. Check many VNs can be linked to the provider-VN
            11. Check (provider-vn -> any-VN),DENY acl rule is added to
               the provider-VN
            12. Check (VN -> provider-VN),DENY acl rule is added to
               the VN
            13. Adding a (VN -> provider-VN),PASS acl rule at VN removes
        ...
        self.assertRaises(RefsExistError, self._vnc_lib.virtual_network_update, provider_vn)
        ...
        self.assertRaises(RefsExistError, self._vnc_lib.virtual_network_update, vn4_obj1)
        ...
        self._vnc_lib.virtual_network_update(provider_vn)
        provider_vn = self._vnc_lib.virtual_network_read(fq_name=provider_vn.get_fq_name())
        self.assertEqual(len(provider_vn.virtual_network_refs), 2)
        linked_uuids = [ref['uuid'] for ref in provider_vn.virtual_network_refs]
        ...
        provider_vn = self._vnc_lib.virtual_network_read(fq_name=provider_vn.get_fq_name())
        vn3_obj1 = self._vnc_lib.virtual_network_read(fq_name=vn3_obj1.get_fq_name())
        self.assertEqual(len(provider_vn.virtual_network_refs), 2)
        linked_uuids = [ref['uuid'] for ref in provider_vn.virtual_network_refs]
        self.assertIn(vn3_obj1.uuid, linked_uuids)
        self.assertIn(vn4_obj1.uuid, linked_uuids)
