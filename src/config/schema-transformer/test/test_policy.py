    ...
    def test_provider_network(self):
        ...
        Verify:
            1. Check is_provider_network property of a
               provider-VN is True
            2. Check a non provider VN can be added to
               provider VN
            3. Check db_resync sets is_provider_network property
               of provider-VN as True (simulating upgrade case)
            4. Check the provider-VN can be added to a VN
            5. Check non provider-VN can not be added to a VN
            6. Check many VNs can be linked to the provider-VN
            7. Check (provider-vn -> any-VN),DENY acl rule is added to
               the provider-VN
            8. Check (VN -> provider-VN),DENY acl rule is added to
               the VN
            9. Adding a (VN -> provider-VN),PASS acl rule at VN removes
        ...
        self.assertEqual(provider_vn.virtual_network_refs[0]['to'],
                         vn3_obj1.get_fq_name())
        self.assertEqual(vn3_obj1.virtual_network_refs[0]['to'],
                         provider_fq_name)
