...
from vnc_api.vnc_api import (VirtualNetwork, SequenceType, VirtualNetworkType,
        ...
        PolicyBasedForwardingRuleType, PortTuple, Project, ServiceScaleOutType,
        ServiceInstanceInterfaceType, ServiceInstanceType, ServiceInstance)
...
class TestServicePolicy(STTestCase, VerifyServicePolicy):
    ...
    def test_service_policy_vmi_with_multi_port_tuples(self):
        ...
        si1_sc_ri_name = 'service-'+si1_sc_ri_uuid+'-default-domain_default-project_' + si1[0]

        #Creating a simple allow_all network policy between VN2 and VN3. Service
        ...
        # Creating a SI between VN2 and VN3. Making the right VMI to be
        # the same as the right VMI of SI1
        si1_st = self._vnc_lib.service_template_read(fq_name=['default-domain', 
                                                              si1[0] + 'template'])
        scale_out = ServiceScaleOutType()
        if_list = [ServiceInstanceInterfaceType(virtual_network=vn2_name),
                   ServiceInstanceInterfaceType(virtual_network=vn3_name)]
        si_props = ServiceInstanceType(auto_policy = False,
                                       interface_list = if_list,
                                       scale_out = scale_out)
        si2 = ServiceInstance(name = si2_name,
                              service_instance_properties = si_props)
        si2.add_service_template(si1_st)
        self._vnc_lib.service_instance_create(si2)

        si2_fqdn = si2.get_fq_name_str()
        si2_obj = self._vnc_lib.service_instance_read(fq_name_str=si2_fqdn)

        proj = Project()
        pt = PortTuple('pt-'+si2_name, parent_obj=si2)
        ...
        vn2_obj = self._vnc_lib.virtual_network_read(fq_name_str=vn2_obj.get_fq_name_str())
        ...
        # Updating the network policy created above to use the SI2 created to be
        # applied between VN2 and VN3.
        np2_entries= np2.get_network_policy_entries()
        np2_policy_rules = np2_entries.get_policy_rule()
        action_list = np2_policy_rules[0].get_action_list()
        action_list.set_apply_service(['default-domain:default-project:' + self.id() + '_s2'])
        np2.set_network_policy_entries(np2_entries)
        self._vnc_lib.network_policy_update(np2)
        ...
        si2_sc_ri_uuid = self.wait_to_get_sc(left_vn=vn2_obj.get_fq_name_str(), 
                                             ...
                                             check_create=True)
        si2_sc_ri_name = 'service-'+si2_sc_ri_uuid+'-default-domain_default-project_' + si2_name

        # Checking the VRF assign rules.
        self.check_acl_action_assign_rules(vn1_obj.get_fq_name(), vn1_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn1_obj, si1_sc_ri_name)))
        self.check_acl_action_assign_rules(vn1_obj.get_fq_name(), vn3_obj.get_fq_name_str(),
                                  vn1_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn1_obj, si1_sc_ri_name)))
        self.check_acl_action_assign_rules(vn3_obj.get_fq_name(), vn1_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn3_obj, si1_sc_ri_name)))
        self.check_acl_action_assign_rules(vn3_obj.get_fq_name(), vn1_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn3_obj, si1_sc_ri_name)))

        self.check_acl_action_assign_rules(vn2_obj.get_fq_name(), vn2_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn2_obj, si2_sc_ri_name)))
        self.check_acl_action_assign_rules(vn2_obj.get_fq_name(), vn3_obj.get_fq_name_str(),
                                  vn2_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn2_obj, si2_sc_ri_name)))
        self.check_acl_action_assign_rules(vn3_obj.get_fq_name(), vn2_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn3_obj, si2_sc_ri_name)))
        self.check_acl_action_assign_rules(vn3_obj.get_fq_name(), vn2_obj.get_fq_name_str(),
                                  vn3_obj.get_fq_name_str(), ':'.join(self.get_ri_name(vn3_obj, si2_sc_ri_name)))
