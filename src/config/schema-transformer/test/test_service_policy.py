...
class VerifyServicePolicy(VerifyPolicy):
    ...
    @retries(10)
    def check_vrf_assign_table(self, vmi_fq_name=None, floating_ip=None, is_present = True, vmi=None, src_port=None, dst_port=None):
        if not vmi:
            vmi = self._vnc_lib.virtual_machine_interface_read(vmi_fq_name)
        ...
        if is_present:
            ...
            try:
                if floating_ip:
                    self.assertEqual(vmi.get_vrf_assign_table().vrf_assign_rule[1].match_condition.src_address.subnet.ip_prefix, floating_ip)
                if src_port:
                    assertEqual([vmi.get_vrf_assign_table().vrf_assign_rule[1].match_condition.src_port], src_port)
                if dst_port:
                    self.assertEqual([vmi.get_vrf_assign_table().vrf_assign_rule[1].match_condition.dst_port], dst_port)
...
class TestServicePolicy(STTestCase, VerifyServicePolicy):
    ...
    def test_vrf_assign_rules(self):
        ...
        self.check_vrf_assign_table(vmi=left_vmi, src_port=prule[0].dst_ports, dst_port=prule[0].src_ports)
        self.check_vrf_assign_table(vmi=right_vmi, src_port=prule[0].src_ports, dst_port=prule[0].dst_ports)
