...
#class TestRouteTable(test_case.STTestCase):
    def test_add_delete_route(self):
        ...
        service_name = self.id() + 's1'
        np = self.create_network_policy(lvn, rvn, [service_name], auto_policy=True, service_mode="in-network-nat")
        ...
        @retries(5)
        def _match_no_route_in_service_ri():
