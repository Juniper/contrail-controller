...
class TestPolicy(test_case.STTestCase):
    ...
    @retries(5, hook=retry_exc_handler)
    def check_pi_is_present(self, fq_name):
        self._vnc_lib.physical_interface_read(fq_name)
    ...
    @retries(5, hook=retry_exc_handler)
    def check_vmi_is_deleted(self, fq_name):
        try:
            self._vnc_lib.virtual_machine_interface_read(fq_name)
            print "retrying ... ", test_common.lineno()
            raise Exception
    ...
    @retries(5, hook=retry_exc_handler)
    def check_pi_is_deleted(self, fq_name):
        try:
            self._vnc_lib.physical_interface_read(fq_name)
            print "retrying ... ", test_common.lineno()
            raise Exception
    ...
    @retries(5, hook=retry_exc_handler)
    def check_li_is_deleted(self, fq_name):
        try:
            self._vnc_lib.logical_interface_read(fq_name)
            print "retrying ... ", test_common.lineno()
            raise Exception
    ...
    def test_fip(self):
        ...
        bare_metal_service.enable = True
        ...
        vmi1_name =  vn1_name + '_' + vn2_name + '_' + 'ge-0/0/0' + '_vmi_0'
        try:
        ...
        pi_fq_name = list(pr_mx.get_fq_name())
        pi_fq_name.append('ge-0/0/0')
        try:
            self.check_pi_is_present(fq_name=pi_fq_name)
        except NoIdError, e:
            print "failed : pi is not present ... ", test_common.lineno()
            self.assertTrue(False)

	li1_fq_name = list(pi_fq_name)
        li1_fq_name.append('ge-0/0/0.0')
        try:
        ...
        vmi2_name =  vn1_name + '_' + vn2_name + '_' + 'ge-0/0/0' + '_vmi_1'
        try:
	...
	li1_fq_name = list(pi_fq_name)
        li1_fq_name.append('ge-0/0/0.0')
        try:
            self.check_li_is_deleted(fq_name=li2_fq_name)
        except Exception, e:
            print "failed : li1 is still present ... ", test_common.lineno()
            self.assertTrue(False)
	...
	li2_fq_name = list(pi_fq_name)
        ...
        try:
            ...
            print "failed : li2 is still present ... ", test_common.lineno()
            self.assertTrue(False)

        vmi1_name =  vn1_name + '_' + vn2_name + '_' + 'ge-0/0/0' + '_vmi_0'
        try:
            self.check_vmi_is_deleted(fq_name=[u'default-project', vmi1_name])
        except Exception, e:
            print "failed : vmi1 is still present ... ", test_common.lineno()
            self.assertTrue(False)

        vmi2_name =  vn1_name + '_' + vn2_name + '_' + 'ge-0/0/0' + '_vmi_1'
        try:
            self.check_vmi_is_deleted(fq_name=[u'default-project', vmi2_name])
        except Exception, e:
            print "failed : vmi2 is still present ... ", test_common.lineno()
            self.assertTrue(False)

        try:
            self.check_pi_is_deleted(fq_name=pi_fq_name)
        except Exception, e:
            print "failed : pi is still present ... ", test_common.lineno()
            self.assertTrue(False)

        pool_ri_name = vn1_name + "_" + fip_pool_name + "_nat"
        try:
            self.check_ri_is_deleted(fq_name=[u'default-domain', u'default-project', vn2_name, pool_ri_name])
        except Exception, e:
            print "failed : pool ri is still present ... ", test_common.lineno()
            self.assertTrue(False)
