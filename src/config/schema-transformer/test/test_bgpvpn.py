...
class TestBgpvpnWithLogicalRouter(STTestCase, VerifyRouteTarget):
    def test_associating_bgpvpn(self):
        ...
        lr, vns, _ = self.create_logical_router('lr-%s' % self.id())
    ...
    def test_route_target_removed_when_resource_deleted(self):
        ...
        lr1, vns1, _ = self.create_logical_router('lr1-%s' % self.id())
        lr2, vns2, _ = self.create_logical_router('lr2-%s' % self.id())
        ...
        # network and its primary routing instance used to applied BGP VPN's
        # route target
        self._vnc_lib.logical_router_delete(id=lr2.uuid)
        # Chech that routing instance still there but the BGP VPN route target
        ...
        # Chech the routing instance still there but the BGP VPN route target
        # were not anymore referenced
    ...
    def test_updating_bgpvpn(self):
        ...
        # We attached only one virtual network to the logical router
        vn = vns[0]
        # Create one bgpvpn without any route target associated to one logical
        # router
        bgpvpn = Bgpvpn('bgpvpn-%s' % self.id())
        bgpvpn_id = self._vnc_lib.bgpvpn_create(bgpvpn)
        bgpvpn = self._vnc_lib.bgpvpn_read(id=bgpvpn_id)
