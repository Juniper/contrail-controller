            ...
            if verbose >= 2: 
                ...
                for p in i['props']:
                    if p['name'] not in vnc_viewer.skip:
                        show_list.append(p)
                #propstr2 = ['%s=%s'%(p['name'],p['value']) for p in i['props']]
                propstr2 = ['%s=%s'%(p['name'],p['value']) for p in show_list]
                propstr2 = '\n'.join(propstr2)
                propstr2 = propstr2.split('\n')
                propstr2 = prop_fmt.join(propstr2)
        print '    '*indent + '%s = '%(i['type']) + str(i['name']) + propstr
...
def parse_config(soap_config):
    ...
    for r_i in root.findall('*/*/*/resultItem'):
        ...
        type1 = res.group(1); name1 = res.group(3)
        id1 = ident1.split(':')
        # strip contrail, type
        id1 = id1[2:]
        ...
        if ident2:
            ...
            type2 = res.group(1); name2 = res.group(3)
            id2 = ident2.split(':')
            # strip contrail, type
            id2 = id2[2:]
        ...
        for name in id1:
            match, n = find_node(name, node, None)
            if n == None:
                node.append({'type': type1, 'name': name1, 'value': [], 'props' : [], 'refs': []})
                match = node[-1]
                node = node[-1]['value']
                break
            node = n
        ...
        if ident2:
            ...
            if n == None:
                match = {'type': type2, 'name': name2, 'value': [], 'props': [], 'refs':[]}
                node1.append(match)
            ...
            if ident2:
                if meta_name in link_name_to_xsd_type:
                    ...
                    obj_json = json.dumps(obj,
                        default=lambda o: dict((k, v) for k,v in o.__dict__.iteritems()), indent=4)
                    outfile.write('Attaching Reference %s to Id %s\n' %(meta_name, ident2))
                    outfile.write('JSON %s = %s\n' % (meta_name, obj_json))
                    match['refs'].append({'name':'%s'%(meta_name), 'value':obj_json})
            else:
                if meta_name in vnc.prop_name_to_xsd_type:
                    ...
                    obj_json = json.dumps(obj,
                        default=lambda o: dict((k, v) for k,v in o.__dict__.iteritems()), indent=4)
                    outfile.write('Attaching Property %s to Id %s\n' %(meta_name, ident1))
                    outfile.write('JSON %s = %s\n' % (meta_name, obj_json))
                    match['props'].append({'name':'%s'%(meta_name), 'value':obj_json})
...
class IfmapClient():
    ...
    def ifmap_read(self, ifmap_id, srch_meta, result_meta, field_names = None):
		start_id = str(Identity(name = ifmap_id, type = 'other', other_type = 'extended'))
		srch_result = self._search(start_id, srch_meta, result_meta, max_depth = 10)
		return srch_result
	#end ifmap_read			
...
class VncViewer():
    def parse_args(self):
        ...
        parser = argparse.ArgumentParser(description = "Display IFMAP configuration")
        ...
        parser.add_argument('ifmap_username', help = "Username known to ifmap server")
        parser.add_argument('ifmap_password', help = "Password known to ifmap server")
        ...
        parser.add_argument('-n', '--node', default=None,
            help = "Start node (fully qualified name such as default-domain:admin:vn2")
        ...
        self.skip = ['id-perms']
        print 'MAP server connection  = %s:%s' % (self._args.ifmap_server_ip, self._args.ifmap_server_port)
        print 'MAP server credentials = %s:%s' % (self._args.ifmap_username,  self._args.ifmap_password)
...
""" sample search metas
 ...
 srch_meta = ' or '.join(['contrail:config-root-domain', 'contrail:config-root-virtual-router']) (domain or virtual-router)
 srch_meta = 'contrail:config-root-domain or contrail:config-root-virtual-router' (same as above)
...
srch_meta = None
result_meta = 'all'
soap_result = vnc_viewer._db_conn.ifmap_read('contrail:config-root:root', srch_meta, result_meta)
