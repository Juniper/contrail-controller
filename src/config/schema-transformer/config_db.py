...
class BgpRouterST(DBBaseST):
    ...
    def skip_fabric_bgp_router_peering_add(self, router):
        if self.cluster_id or router.cluster_id:
            pr_obj_ref = self.obj.get_physical_router_back_refs()
            pr_peer_obj_ref = router.obj.get_physical_router_back_refs()
            ...
            if pr_obj_ref == None and pr_peer_obj_ref == None:
                return False

            pr_obj = self._vnc_lib.physical_router_read(id=pr_obj_ref[0]['uuid'])
            pr_peer_obj = self._vnc_lib.physical_router_read(id=pr_peer_obj_ref[0]['uuid'])
