...
class SecurityLoggingObjectST(DBBaseST):
    ...
    prop_fields = ['_security_logging_object_rate']

    def __init__(self, name, obj=None):
        self.name = name
        self.network_policys = set()
        self.security_groups = set()
        self._security_logging_object_rate = None
        self._security_logging_object_rules = []
        self.update(obj)
        self.uuid = self.obj.uuid
    # end __init__

    def evaluate(self):
        slo_rule_entries = list(set(self.populate_rules('network_policy') +
                                    self.populate_rules('security_group')))
        if (len(slo_rule_entries) != len(self._security_logging_object_rules)) or\
           (set(slo_rule_entries) - set(self._security_logging_object_rules)):
            self.obj.set_security_logging_object_rules(
                            SecurityLoggingObjectRuleListType(slo_rule_entries))
            self._vnc_lib.security_logging_object_update(self.obj)
            self._security_logging_object_rules = slo_rule_entries
    ...
    def populate_rules(self, rule_type):
        ...
        if hasattr(self.obj, rule_type_refs):
            for np_sg in getattr(self.obj, rule_type_refs):
                if np_sg['attr'].rule:
                    np_sg_rule_list = np_sg['attr'].rule
                else:
                    ...
                    vnc_np_sg = getattr(self._vnc_lib,
                                        rule_type+'_read')(id=np_sg['uuid'])
                    np_sg_rule_list = getattr(vnc_np_sg,
                                              'get_'+rule_type+'_entries')().get_policy_rule()
                for rule_entry in np_sg_rule_list:
                    rate_to_use = getattr(rule_entry, 'rate',
                                          self._security_logging_object_rate)
    ...
    def handle_st_object_req(self):
        ...
        resp.properties = [
            sandesh.PropList('rule', str(rule)) for rule in self._security_logging_object_rules
        ]
        return resp
