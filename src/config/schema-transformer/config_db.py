...
class VirtualNetworkST(DBBaseST):
    ...
    def _get_routing_instance_from_route(self, next_hop):
        ...
        if sc is None:
            self._logger.error("Service chain between %s and %s not "
            ...
            for daddr in daddr_list:
                ...
                if dvn in [self.name, 'any']:
                    ...
                    if prule.action_list.apply_service:
                        self._logger.error("service chains not allowed in cidr only rules"
...
class VirtualMachineInterfaceST(DBBaseST):
    ...
    def rebake(self):
        ...
        vm_obj = VirtualMachineST.get(self.virtual_machine)
        if vm_obj is None:
            self._logger.error("Virtual machine %s not found " %
            ...
            for prule in policy.rules:
                if prule.action_list is not None:
                    if si_name in prule.action_list.apply_service:
                        network_set |= policy.virtual_networks
