...
class BgpAsAServiceST(DBBaseST):
    ...
    def reinit(cls):
        for bgpaas_obj in cls.list_vnc_obj():
            try:
                cls.locate(bgpaas_obj.get_fq_name_str(), bgpaas_obj)
            except Exception as e:
                cls._logger.error("Error in reinit for %s %s: %s" % (
                    cls.obj_type, bgpaas_obj.get_fq_name_str(), str(e)))

            shared = bgpaas_obj.get_bgpaas_shared()
            if shared:
                if not bgpaas_obj.bgpaas_ip_address:
                    return
                vmi_refs = bgpaas_obj.get_virtual_machine_interface_refs()
                if not vmi_refs:
                    return
            ...
            if not bgp_peer_found:
                client_bgpr.obj.add_bgp_router(server_router.obj)
                self._vnc_lib.bgp_router_update(client_bgpr.obj)
    # end reinit
