...
class SecurityLoggingObjectST(DBBaseST):
    ...
    prop_fields = ['_security_logging_object_rate',
                   '_security_logging_object_rules']
    ...
    def evaluate(self):
        self._security_logging_object_rules = []
        self.populate_rules('network_policy')
            ...
            for np_sg in getattr(self.obj, rule_type_refs):
                if np_sg['attr'].rule:
                    ...
                    np_sg_rule_list = getattr(vnc_np_sg,
                                              'get_'+rule_type+'_entries')().policy_rule
                for rule_entry in np_sg_rule_list:
                    rate_to_use = getattr(rule_entry, 'rate',
                                          self._security_logging_object_rate)
                    self._security_logging_object_rules.append(
                                        {
                                            'rule_uuid': rule_entry.rule_uuid,
                                            'rate': rate_to_use
                                        }
                                    )
    ...
    def delete_obj(self):
        self.update_multiple_refs('network_policy', {})
