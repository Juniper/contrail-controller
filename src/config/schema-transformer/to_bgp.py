...
class VirtualNetworkST(DictST):
    ...
    def __init__(self, name, obj=None, acl_dict=None, ri_dict=None):
        self.obj = obj or _vnc_lib.virtual_network_read(
            fq_name_str=name, fields=['routing-instances',
...
class RouteTargetST(DictST):
    ...
    def delete(cls, rt_key):
        if rt_key not in cls._dict:
            return
        _vnc_lib.route_target_delete(self.obj)
...
class ServiceChain(DictST):
    ...
    def create(self):
        ...
        for service in self.service_list:
            ...
            tmp_vm_obj = VirtualMachineST('', service)
            mode = tmp_vm_obj.get_service_mode()
            if mode is None:
                return None
...
class VirtualMachineInterfaceST(DictST):
    ...
    def recreate_vrf_assign_table(self):
        ...
        if vm_obj is None:
            _sandesh._logger.error(
                "virtual machine %s not found: %s",
                self.virtual_machine, str(e))
            return
        smode = vm_obj.get_service_mode()
...
class SchemaTransformer(object):
    ...
    def reinit(self):
        ...
        rt_list = _vnc_lib.security_groups_list()['route-targets']
        for rt in rt_list:
