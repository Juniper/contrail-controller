...
from schema_transformer.resources.acl_rule_list import AclRuleListST
...
class VirtualNetworkST(DBBaseST):
    ...
    def update(self, obj=None):
        ...
        for policy_name in DBBaseST.get_obj_type_map().get('network_policy').get_internal_policies(self.name):
            self.add_policy(policy_name)
        for policy_name in old_policies - set(self.network_policys.keys()):
            policy = DBBaseST.get_obj_type_map().get('network_policy').get(policy_name)
            if policy:
    ...
    def _update_primary_ri_to_service_ri_connection(self, sc, si_name,
            ...
            service_ri = DBBaseST.get_obj_type_map().get('routing_instance').get(service_ri_name)
            ...
            if (multi_policy_enabled and
                ...
                rt_obj = DBBaseST.get_obj_type_map().get('route_target').get(self.get_route_target())
        ...
        for sc_list in self.service_chains.values():
            for sc in sc_list:
                ...
                if sc.left_vn == self.name:
                    ...
                    right_si = DBBaseST.get_obj_type_map().get('service_instance').get(other_si_name)
                elif sc.right_vn == self.name:
                    si_name = sc.service_list[-1]
                    other_vn_name = sc.left_vn
                    other_si_name = sc.service_list[0]
                    right_si = DBBaseST.get_obj_type_map().get('service_instance').get(si_name)
                else:
                    continue
    ...
    def get_primary_routing_instance(self):
        return DBBaseST.get_obj_type_map().get('routing_instance').get(self._default_ri_name)
    ...
    def add_service_chain(self, svn, dvn, proto, prule):
        ...
        for remote_vn in remote_vns:
            ...
            if self.me(dvn):
                service_chain = DBBaseST.get_obj_type_map().get('service_chain').find_or_create(
    ...
    def allocate_service_chain_ip(self, sc_fq_name):
        ...
        ip_dict = {}
        sc_ipam_obj = DBBaseST.get_obj_type_map().get('service_chain')._get_service_chain_ipam()
    ...
    def get_route_target(self):
        ...
        rtgt_name = "target:%s:%d" % (
                    DBBaseST.get_obj_type_map().get('global_system_config').get_autonomous_system(),
                    self._route_target)
        return rtgt_name
    ...
    def set_route_target(self, rtgt_name):
        ...
        if int(asn) != DBBaseST.get_obj_type_map().get('global_system_config').get_autonomous_system():
            return
    ...
    def _ri_needs_external_rt(vn_name, ri_name):
        sc_id = DBBaseST.get_obj_type_map().get('routing_instance')._get_service_id_from_ri(ri_name)
        if sc_id is None:
            return False
        sc = DBBaseST.get_obj_type_map().get('service_chain').get(sc_id)
        if sc is None:
            return False
    ...
    def set_route_target_list(self, obj):
        ...
        for rt in itertools.chain(rt_add, rt_add_export, rt_add_import):
            DBBaseST.get_obj_type_map().get('route_target').locate(rt)
        ...
        for ri_name in self.routing_instances:
            if self._ri_needs_external_rt(self.name, ri_name):
                service_ri = DBBaseST.get_obj_type_map().get('routing_instance').get(ri_name)
    ...
    def _get_routing_instance_from_route(self, next_hop):
        si = DBBaseST.get_obj_type_map().get('service_instance').get(next_hop)
        if si is None:
    ...
    def get_analyzer_vn_and_ip(analyzer_name):
        vn_analyzer = None
        ip_analyzer = None
        si = DBBaseST.get_obj_type_map().get('service_instance').get(analyzer_name)
        if si is None:
            return (None, None)
        ...
        if not vm_analyzer and not pt_list:
            return (None, None)
        if pt_list:
            vm_pt = DBBaseST.get_obj_type_map().get('port_tuple').get(pt_list[0])
        else:
            vm_pt = DBBaseST.get_obj_type_map().get('virtual_machine').get(vm_analyzer[0])
        if vm_pt is None:
            return (None, None)
    ...
    def address_list_policy_to_acl(self, addr):
        if addr.network_policy:
            pol = DBBaseST.get_obj_type_map().get('network_policy').get(addr.network_policy)
            if not pol:
                self._logger.error(
        ...
        for saddr in saddr_list:
            ...
            for daddr in daddr_list:
                ...
                if self.me(dvn):
                    ...
                    dp_obj = DBBaseST.get_obj_type_map().get('network_policy').get(dpol)
                    ...
                    sp_obj = DBBaseST.get_obj_type_map().get('network_policy').get(spol)
    ...
    def update_pnf_presence(self):
        ...
        for ri_name in self.routing_instances:
            ...
            if ri.obj.get_routing_instance_has_pnf():
                has_pnf = True
                break
        default_ri = DBBaseST.get_obj_type_map().get('routing_instance').get(self._default_ri_name)
        if (default_ri and
    ...
    def evaluate(self):
                            ...
                            if self.multi_policy_service_chains_enabled:
                                ...
                                for sc in sc_list:
                                    if sc is not None and sc.created:
                                        ...
                                        right_si = DBBaseST.get_obj_type_map().get('service_instance').get(
                                            right_si_name)
        ...
        # analyzer
        for policy in DBBaseST.get_obj_type_map().get('network_policy').values():
            for prule in policy.rules:
        ...
        if primary_ri:
            ...
            for rp in new_rp_set - old_rp_set:
                rp_obj = DBBaseST.get_obj_type_map().get('routing_policy').get(rp)
