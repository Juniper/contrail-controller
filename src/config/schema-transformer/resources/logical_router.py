...
#from schema_transformer.resources.bgp_vpn import DBBaseST.get_obj_type_map().get('bgpvpn')
...
class LogicalRouterST(DBBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        if not rt_ref:
            ...
            rt_key = "target:%s:%d" % (
                DBBaseST.get_obj_type_map().get('global_system_config').get_autonomous_system(), rtgt_num)
            rtgt_obj = DBBaseST.get_obj_type_map().get('route_target').locate(rt_key)
            self.obj.set_route_target(rtgt_obj.obj)
            self._vnc_lib.logical_router_update(self.obj)

        if old_rt_key:
            DBBaseST.get_obj_type_map().get('route_target').delete_vnc_obj(old_rt_key)
    ...
    def update_virtual_networks(self):
        ...
        for vmi in self.virtual_machine_interfaces:
            vmi_obj = DBBaseST.get_obj_type_map().get('virtual_machine_interface').get(vmi)
    ...
    def set_virtual_networks(self, vn_set):
        ...
        for vn in self.virtual_networks - vn_set:
            vn_obj = DBBaseST.get_obj_type_map().get('virtual_network').get(vn)
    ...
    def set_route_target_list(self):
        ...
        for bgpvpn_name in self.bgpvpns:
            bgpvpn = DBBaseST.get_obj_type_map().get('bgpvpn').get(bgpvpn_name)
    ...
    def delete_route_targets(route_targets=None):
        for rt in route_targets or []:
            try:
                DBBaseST.get_obj_type_map().get('route_target').delete_vnc_obj(rt)
