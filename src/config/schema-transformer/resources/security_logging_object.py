...
#from schema_transformer.resources.network_policy import DBBaseST.get_obj_type_map().get('network_policy')
#from schema_transformer.resources.security_group import DBBaseST.get_obj_type_map().get('security_group')
    ...
    def populate_rules(self, rule_type, refs):
        ...
        for np_sg, attr in refs.items():
            if attr and attr.rule:
                ...
                np_sg_st_obj = (DBBaseST.get_obj_type_map().get('network_policy').get(np_sg) or
                                DBBaseST.get_obj_type_map().get('security_group').get(np_sg))
