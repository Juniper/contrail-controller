...
#from schema_transformer.resources.bgp_as_a_service import DBBaseST.get_obj_type_map().get('bgp_as_a_service')
...
#from schema_transformer.resources.physical_router import DBBaseST.get_obj_type_map().get('physical_router')
...
class BgpRouterST(DBBaseST):
    ...
    def evaluate(self):
        if self.router_type == 'bgpaas-client':
            bgpaas = DBBaseST.get_obj_type_map().get('bgp_as_a_service').get(self.bgp_as_a_service)
    ...
    def update_bgpaas_client(self, bgpaas):
        ...
        if bgpaas.bgpaas_shared:
            if (bgpaas.virtual_machine_interfaces and
                ...
                vmis = [DBBaseST.get_obj_type_map().get('virtual_machine_interface').get(vmi_name)
            ...
            if vmi is None or vmi.virtual_network is None:
                del bgpaas.bgpaas_clients[vmi_name]
                return -1
            vn = DBBaseST.get_obj_type_map().get('virtual_network').get(vmi.virtual_network)
    ...
    def skip_fabric_bgp_router_peering_add(self, router):
        ...
        if phy_rtr_name and phy_rtr_peer_name:
            phy_rtr = DBBaseST.get_obj_type_map().get('physical_router').get(phy_rtr_name)
            fabric = phy_rtr.fabric

            phy_rtr_peer = DBBaseST.get_obj_type_map().get('physical_router').get(phy_rtr_peer_name)
    ...
    def update_peering(self):
        if not DBBaseST.get_obj_type_map().\
        ...
        if self.physical_router:
            phy_rtr = DBBaseST.get_obj_type_map().get('physical_router').get(self.physical_router)
            fabric = phy_rtr.fabric

        global_asn = int(DBBaseST.get_obj_type_map().\
                        get('global_system_config').get_autonomous_system())
