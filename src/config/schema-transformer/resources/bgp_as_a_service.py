...
class BgpAsAServiceST(DBBaseST):
    ...
    def set_bgpaas_clients(self):
        if (self.bgpaas_shared and self.bgp_routers):
            bgpr = DBBaseST.get_obj_type_map().get('bgp_router').get(list(self.bgp_routers)[0])
            self.bgpaas_clients[self.obj.name] = bgpr.obj.get_fq_name_str()
        else:
            for bgp_router in self.bgp_routers:
                bgpr = DBBaseST.get_obj_type_map().get('bgp_router').get(bgp_router)
                if bgpr is None:
                    continue
    ...
    def create_bgp_router(self, name, shared=False):
        if shared:
            ...
            vmis = [DBBaseST.get_obj_type_map().get('virtual_machine_interface').get(':'.join(ref['to']))
                       for ref in vmi_refs]
        else:
            # only current vmi will have link to this bgp-router
            vmi = DBBaseST.get_obj_type_map().get('virtual_machine_interface').get(name)
            if not vmi:
                self.virtual_machine_interfaces.discard(name)
                return
            vmis = [vmi]
        vn = DBBaseST.get_obj_type_map().get('virtual_network').get(vmis[0].virtual_network)
        if not vn:
            return
        ...
        server_router = DBBaseST.get_obj_type_map().get('bgp_router').get(server_fq_name)
        if not server_router:
            ...
            DBBaseST.get_obj_type_map().get('bgp_router').locate(server_fq_name, server_router)
        else:
        ...
        bgpr = DBBaseST.get_obj_type_map().get('bgp_router').get(router_fq_name)
        create = False
        update = False
        src_port = None
        if not bgpr:
        ...
        if create:
            ...
            bgpr = DBBaseST.get_obj_type_map().get('bgp_router').locate(router_fq_name)
        elif update:
