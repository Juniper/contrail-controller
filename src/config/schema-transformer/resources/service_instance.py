...
class ServiceInstanceST(DBBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        self.update(obj)
        self.network_policys = DBBaseST.get_obj_type_map().get('network_policy').get_by_service_instance(self.name)
        self.route_tables = DBBaseST.get_obj_type_map().get('route_table').get_by_service_instance(self.name)
    ...
    def get_allocated_interface_ip(self, side, version):
        ...
        for vm_name in self.virtual_machines or []:
            vm_pt_list.append(DBBaseST.get_obj_type_map().get('virtual_machine').get(vm_name))
        for pt_name in self.port_tuples or []:
            vm_pt_list.append(DBBaseST.get_obj_type_map().get('port_tuple').get(pt_name))
    ...
    def add_properties(self, props):
        ...
        policy = DBBaseST.get_obj_type_map().get('network_policy').locate(policy_name, policy_obj)
        policy.virtual_networks = set([self.left_vn_str, self.right_vn_str])

        policy.set_internal()
        vn1 = DBBaseST.get_obj_type_map().get('virtual_network').get(self.left_vn_str)
        if vn1:
            vn1.add_policy(policy_name)
        vn2 = DBBaseST.get_obj_type_map().get('virtual_network').get(self.right_vn_str)
        if vn2:
    ...
    def delete_properties(self):
        ...
        for vn_name in policy.virtual_networks:
            vn = DBBaseST.get_obj_type_map().get('virtual_network').get(vn_name)
            if vn is None:
                continue
            del vn.network_policys[policy_name]
        # end for vn_name
        DBBaseST.get_obj_type_map().get('network_policy').delete(policy_name)
