...
#from schema_transformer.resources.instance_ip import DBBaseST.get_obj_type_map().get('instance_ip')
...
#from schema_transformer.resources.floating_ip import DBBaseST.get_obj_type_map().get('floating_ip')
#from schema_transformer.resources.alias_ip import DBBaseST.get_obj_type_map().get('alias_ip')
...
class VirtualMachineInterfaceST(DBBaseST):
    ...
    def get_any_instance_ip_address(self, ip_version=0):
        for ip_name in self.instance_ips:
            ip = DBBaseST.get_obj_type_map().get('instance_ip').get(ip_name)
    ...
    def update_routing_instances(self, ri_refs):
        ...
        for ri_name in old_ri_set - new_ri_set:
            ri = DBBaseST.get_obj_type_map().get('routing_instance').get(ri_name)
            if ri:
    ...
    def get_virtual_machine_or_port_tuple(self):
        if self.port_tuples:
            pt_list = [DBBaseST.get_obj_type_map().get('port_tuple').get(x) for x in self.port_tuples if x is not None]
            return pt_list
        elif self.virtual_machine:
            vm = DBBaseST.get_obj_type_map().get('virtual_machine').get(self.virtual_machine)
    ...
    def _add_pbf_rules(self):
        ...
        for vm_pt in vm_pt_list:
            if vm_pt.get_service_mode() != 'transparent':
                return
            for service_chain in DBBaseST.get_obj_type_map().get('service_chain').values():
                if vm_pt.service_instance not in service_chain.service_list:
                    continue
                if not service_chain.created:
                    continue
                if self.is_left():
                    vn_obj = DBBaseST.get_obj_type_map().get('virtual_network').locate(service_chain.left_vn)
                    vn1_obj = vn_obj
                else:
                    vn1_obj = DBBaseST.get_obj_type_map().get('virtual_network').locate(service_chain.left_vn)
                    vn_obj = DBBaseST.get_obj_type_map().get('virtual_network').locate(service_chain.right_vn)
    ...
    def set_virtual_network(self):
        lr = DBBaseST.get_obj_type_map().get('logical_router').get(self.logical_router)
        if lr is not None:
