...
class GlobalSystemConfigST(DBBaseST):
    ...
    def update_autonomous_system(cls, new_asn):
        ...
        # by the route target
        for route_tgt in DBBaseST.get_obj_type_map().get('route_target').values():
            ...
            new_rtgt_obj = DBBaseST.get_obj_type_map().get('route_target').locate(new_rtgt_name)
            old_rtgt_name = "target:%d:%s" % (cls._autonomous_system, target)
            old_rtgt_obj = RouteTarget(old_rtgt_name)

            route_tgt.obj = DBBaseST.get_obj_type_map().get('route_target').read_vnc_obj(
            ...
            for ri_ref in route_tgt.obj.get_routing_instance_back_refs() or []:
                rt_inst = DBBaseST.get_obj_type_map().get('routing_instance').get(':'.join(ri_ref['to']))
                if rt_inst:
                    ri = rt_inst.obj
                else:
                    continue
            ...
            for router_ref in route_tgt.obj.get_logical_router_back_refs() or []:
                lr = DBBaseST.get_obj_type_map().get('logical_router').\
            ...
            DBBaseST.get_obj_type_map().get('route_target').\
    ...
    def evaluate(self):
        for router in DBBaseST.get_obj_type_map().get('bgp_router').values():
            router.update_global_asn(self._autonomous_system)
            router.update_peering()
        # end for router
        for router in DBBaseST.get_obj_type_map().get('logical_router').values():
