...
#from schema_transformer.resources.virtual_network import DBBaseST.get_obj_type_map().get('virtual_network')
#from schema_transformer.resources.service_instance import DBBaseST.get_obj_type_map().get('service_instance')
#from schema_transformer.resources.routing_policy import DBBaseST.get_obj_type_map().get('routing_policy')
#from schema_transformer.resources.route_aggregate import DBBaseST.get_obj_type_map().get('route_aggregate')
#from schema_transformer.resources.route_target import DBBaseST.get_obj_type_map().get('route_target')
...
#from schema_transformer.resources.route_table import DBBaseST.get_obj_type_map().get('route_table')
#from schema_transformer.resources.logical_router import DBBaseST.get_obj_type_map().get('logical_router')
#from schema_transformer.resources.virtual_machine_interface import\
#    DBBaseST.get_obj_type_map().get('virtual_machine_interface')
...
class RoutingInstanceST(DBBaseST):
    ...
    def __init__(self, name, obj=None):
        ...
        if self.is_default:
            vn = DBBaseST.get_obj_type_map().get('virtual_network').get(self.virtual_network)
            if vn is None:
                return
    ...
    def update_routing_policy_and_aggregates(self):
        if not self.service_chain:
            return
        sc = DBBaseST.get_obj_type_map().get('service_chain').get(self.service_chain)
        if sc is None:
            return
        for si_name in sc.service_list:
            if not self.name.endswith(si_name.replace(':', '_')):
                continue
            si = DBBaseST.get_obj_type_map().get('service_instance').get(si_name)
            if si is None:
                return
            ...
            for rp_name in self.routing_policys:
                if rp_name not in rp_dict:
                    rp = DBBaseST.get_obj_type_map().get('routing_policy').get(rp_name)
                    if rp:
            ...
            for ra_name in self.route_aggregates - ra_set:
                ra = DBBaseST.get_obj_type_map().get('route_aggregate').get(ra_name)
                if ra:
    ...
    def import_default_ri_route_target_to_service_ri(self):
        ...
        if sc is None or not sc.created:
            return update_ri
        left_vn = DBBaseST.get_obj_type_map().get('virtual_network').get(sc.left_vn)
        right_vn = DBBaseST.get_obj_type_map().get('virtual_network').get(sc.right_vn)
        ...
        right_si = DBBaseST.get_obj_type_map().get('service_instance').get(right_si_name)
        ...
        if service_ri_name == self.name:
            ...
            if rt not in self.stale_route_targets:
                rt_obj = DBBaseST.get_obj_type_map().get('route_target').get(rt)
    ...
    def locate_route_target(self):
        ...
        rt_key = "target:%s:%d" % (DBBaseST.get_obj_type_map().get('global_system_config').get_autonomous_system(),
                                   rtgt_num)
        rtgt_obj = DBBaseST.get_obj_type_map().get('route_target').locate(rt_key).obj
        if self.is_default:
            inst_tgt_data = InstanceTargetType()
        elif DBBaseST.get_obj_type_map().get('virtual_network')._ri_needs_external_rt(self.virtual_network, self.name):
            ...
            if self.obj.parent_uuid != vn.obj.uuid:
                ...
                if inst_tgt_data:
                    for rt in vn.rt_list | vn.bgpvpn_rt_list:
                        if rt not in self.stale_route_targets:
                            rtgt_obj = DBBaseST.get_obj_type_map().get('route_target').locate(rt)
        ...
        if 0 < old_rtgt < common.BGP_RTGT_MIN_ID:
            rt_key = "target:%s:%d" % (
                DBBaseST.get_obj_type_map().get('global_system_config').get_autonomous_system(), old_rtgt)
            DBBaseST.get_obj_type_map().get('route_target').delete_vnc_obj(rt_key)
    ...
    def update_route_target_list(self, rt_add=None, rt_add_import=None,
        ...
        for rt in rt_add or []:
            if rt not in self.stale_route_targets:
                rtgt_obj = DBBaseST.get_obj_type_map().get('route_target').locate(rt).obj
    ...
    def update_static_routes(self):
        ...
        si_set = DBBaseST.get_obj_type_map().get('service_instance').get_vn_si_mapping(self.virtual_network)
        for si in si_set or []:
            mode = si.get_service_mode()
            if mode is None or mode != 'in-network-nat':
                self._logger.debug("service mode for %s-%s, skip" % (si.name, mode))
                continue
            route_tables = DBBaseST.get_obj_type_map().get('route_table').get_by_service_instance(si.name)
            ...
            for route_table_name in route_tables:
                route_table = DBBaseST.get_obj_type_map().get('route_table').get(route_table_name)
                ...
                for lr_name in route_table.logical_routers:
                    lr = DBBaseST.get_obj_type_map().get('logical_router').get(lr_name)
                    if lr is None:
        ...
        vn_obj = DBBaseST.get_obj_type_map().get('virtual_network').get(self.virtual_network)
        for rt_name in vn_obj.route_tables:
            route_table = DBBaseST.get_obj_type_map().get('route_table').get(rt_name)
    ...
    def delete_obj(self):
        ...
        if vn_obj is not None and service_chain is not None:
            ...
            for vmi_name in list(self.virtual_machine_interfaces):
                vmi = DBBaseST.get_obj_type_map().get('virtual_machine_interface').get(vmi_name)
                if vmi:
        ...
        for rp_name in self.routing_policys:
            ...
            if rp:
                ...
                try:
                    rp_obj = DBBaseST.get_obj_type_map().get('routing_policy').read_vnc_obj(fq_name=rp_name)
                    if rp_obj:
        ...
        for ra_name in self.route_aggregates:
            ...
            if ra:
                ...
                try:
                    ra_obj = DBBaseST.get_obj_type_map().get('route_aggregate').read_vnc_obj(fq_name=ra_name)
                    if ra_obj:
        ...
        bgpaas_server = DBBaseST.get_obj_type_map().get('bgp_router').get(bgpaas_server_name)
        if bgpaas_server:
            try:
                self._vnc_lib.bgp_router_delete(id=bgpaas_server.obj.uuid)
            except NoIdError:
                pass
            DBBaseST.get_obj_type_map().get('bgp_router').delete(bgpaas_server_name)
        try:
        ...
        for rtgt in rtgt_list or []:
            try:
                DBBaseST.get_obj_type_map().get('route_target').delete_vnc_obj(rtgt['to'][0])
            except RefsExistError:
