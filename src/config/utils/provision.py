...
class VncProvisioner(object):
    def __init__(self, args_str = None):
        ...
        self._bgp_sess_attrs = [BgpSessionAttributes(address_families = self._bgp_addr_fams)]
        self._bgp_sessions = [BgpSession(attributes = self._bgp_sess_attrs)]
        self._bgp_peering_attrs = BgpPeeringAttributes(session = self._bgp_sessions)
        ...
        gsc_obj = vnc_lib.global_system_config_read(fq_name = ['default-global-system-config'])
        ...
        rt_inst_obj = vnc_lib.routing_instance_read(fq_name = ['default-domain', 'default-project',
    ...
    def add_vrouter(self, name, ip, bgps):
        ...
        vrouter_obj = VirtualRouter(name, gsc_obj, virtual_router_ip_address = ip)
        for bgp in bgps:
            bgp_router_fq_name = copy.deepcopy(self._fab_rt_inst_obj.get_fq_name())
            bgp_router_fq_name.append(bgp)
            bgp_router_obj = vnc_lib.bgp_router_read(fq_name = bgp_router_fq_name)
    ...
    def add_bgp_router(self, name, ip):
        ...
        gsc_obj = self._global_system_config_obj
        router_params = BgpRouterParams(autonomous_system = gsc_obj.get_autonomous_system(),
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument("--prov_data_file", help = "File name of provision data in json")
