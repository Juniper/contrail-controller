...
#if chmod.args.service_uuid:
#    print 'Service uudi = ', chmod.args.service_uuid
#else:
#    print "--service-uuid is needed"
#    sys.exit(1)
...
def read_owner():
    ...
    if chmod.args.project_uuid:
        proj = vnc.project_read(id=chmod.args.project_uuid)
        print 'XXXXXXXXXXXXX', proj.uuid, proj.get_perms2().owner
...
def update_owner():
    ...
    if chmod.args.project_uuid:
        proj = vnc.project_read(id=chmod.args.project_uuid)
        print 'XXXXXXXXXXXXX', proj.uuid
        update_owner_field(proj.fq_name, proj, proj.uuid)
        proj.perms2.owner = proj.uuid
    else:
        for project in project_list['projects']:
            proj = vnc.project_read(id=project['uuid'])
            print 'XXXXXXXXXXXXX', proj.uuid
