...
def build_dsa_rule_entry(rule_str):
    ...
    publisher = DiscoveryPubSubEndPointType(service_prefix = pfx_pub,
                    service_type = pubspec[1], service_id = pubspec[2],
                    service_version = pubspec[3])
    subscriber = [DiscoveryPubSubEndPointType(service_prefix = pfx_sub, 
                     service_type = subspec[1], service_id = subspec[2], 
                     service_version = subspec[3])]
...
def match_pubsub_ep(ep1, ep2):
    if ep1.service_prefix.ip_prefix != ep2.service_prefix.ip_prefix:
        return False
    if ep1.service_prefix.ip_prefix_len != ep2.service_prefix.ip_prefix_len:
        return False
    if ep1.service_type != ep2.service_type:
        return False
    if ep1.service_id != ep2.service_id:
        return False
    if ep1.service_version != ep2.service_version:
        return False
    return True
...
def print_dsa_rule_entry(entry, prefix = ''):
    ...
    pub_str = '%s/%d:%s:%s:%s' % \
        (pub.service_prefix.ip_prefix, pub.service_prefix.ip_prefix_len,
        pub.service_type, pub.service_id, pub.service_version)
    sub_str = '%s/%d:%s:%s:%s' % \
        (sub.service_prefix.ip_prefix, sub.service_prefix.ip_prefix_len,
        sub.service_type, sub.service_id, sub.service_version)
