...
class BgpProvisioner(object):
    ...
    def add_bgp_router(self, router_type, router_name, router_ip, router_asn):
        ...
        router_params = BgpRouterParams(vendor = router_type, autonomous_system = int(router_asn),
        ...
        try:
            ...
            print ("BGP Router " + pformat(fq_name) + " already exists with uuid " + existing_obj.uuid)
            return
        except NoIdError:
            pass
    ...
    def add_route_target(self, rt_inst_fq_name, router_asn, route_target_number):
        vnc_lib = self._vnc_lib
    ...
    def del_route_target(self, rt_inst_fq_name, router_asn, route_target_number):
        vnc_lib = self._vnc_lib
        ...
        if rtgt_val not in net_obj.get_route_target_list().get_route_target():
            print "%s not configured for VN %s" %(rtgt_val, rt_inst_fq_name[:-1])
            return
