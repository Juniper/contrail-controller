...
class FabricVPGValidator(object):
            ...
            if annotation_info_dict['validation'] == 'enterprise':
                ...
                if ((vn_uuid, vlan_id) in fabric_vn_vlan_set) or \
                   ...
                   (vlan_id not in fabric_vlan_set)):
                    ...
                    if vn_uuid in self.vn_to_vpg_map:
    ...
    def _report_within_vpg_errors(self, local_errors, vpg_key):
        ...
        for existing_vmi, other_vmi, vn, vlan in local_errors:
            existing_vmi_fq = \
                self._get_vnc_lib().virtual_machine_interface_read(
                    id=existing_vmi).get_fq_name()
            existing_vmi_fq_name = ':'.join(x for x in existing_vmi_fq)
            other_vmi_fq = self._get_vnc_lib().virtual_machine_interface_read(
                id=other_vmi).get_fq_name()
            other_vmi_fq_name = ':'.join(x for x in other_vmi_fq)
            self._logger.info(
                _create_error_msg(
                    vpg_key[0], vpg_key[1],
    ...
    def _report_across_fabric_errors(self, local_errors, vpg_key):
        ...
        for vmi, other_vpg, vn, vlan in local_errors:
            vmi_fq = self._get_vnc_lib().virtual_machine_interface_read(
                id=vmi
            ).get_fq_name()
            vmi_fq_name = ':'.join(x for x in vmi_fq)
            other_vpg_vmi_fq = \
                self._get_vnc_lib().virtual_machine_interface_read(
                    id=other_vpg[2]).get_fq_name()
            other_vmi_name = ':'.join(x for x in other_vpg_vmi_fq)
            self._logger.info(
                _create_error_msg(
                    vpg_key[0], vpg_key[1],
                    vmi, vmi_fq_name,
                    other_vpg[1], other_vpg[0],
    ...
    def _report_untagged_vlan_errors(self, local_errors, vpg_key):
        ...
        for vmi_untag, untag_vlan, other_vmi, other_vlan in local_errors:
            other_vmi_fq = self._get_vnc_lib().virtual_machine_interface_read(
                id=other_vmi).get_fq_name()
            other_vmi_fq_name = ':'.join(x for x in other_vmi_fq)
            untag_vmi_fq = self._get_vnc_lib().virtual_machine_interface_read(
                id=vmi_untag).get_fq_name()
            untag_vmi_fq_name = ':'.join(x for x in untag_vmi_fq)
