...
class ClusterPeer(object):
    ...
    def peer_controls_in_cluster(self, bgp_router_objs):
        for current_obj in bgp_router_objs:
            ...
            revised_peer_set = set(bgp_router_objs) - set([current_obj])
            peer_list = map(lambda x: x.get_fq_name(), revised_peer_set)
            revised_peered_uuids = set(x.get_uuid() for x in revised_peer_set)
            bgp_peering_attr_list = [bgp_peering_attrs for x in peer_list]
            if revised_peered_uuids != currently_peered_uuids:
                print "Peering %s %s" %(current_obj.fq_name, revised_peered_uuids)
                current_obj.set_bgp_router_list(peer_list,
                                      [bgp_peering_attrs]*len(peer_list))
