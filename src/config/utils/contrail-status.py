...
class IntrospectUtil(object):
    ...
    def web_invoke(self, cacert, certfile, keyfile, httplink):
        cmd = 'curl -s -tlsv1 --cert ' + certfile + ' --key ' + keyfile + ' --cacert ' + cacert + ' ' + httplink
        output = None
        try:
    ...
    def _load(self, path):
	    ...
	    #resp = self.web_invoke(self._cacert, self._certfile, self._keyfile, url)
            resp = requests.get(url, verify=self._cacert, cert=(self._certfile, self._keyfile))
        if resp.status_code == requests.codes.ok:
                return etree.fromstring(resp.text)
        else:
