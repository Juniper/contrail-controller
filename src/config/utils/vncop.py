...
class VncOp():
    def parse_args(self):
        # Eg. python vnc_op.py VirtualNetwork domain:default-project:default-virtual-network
        ...
        parser.add_argument('oper', help = "Operation eg. read, write, create, delete")
        parser.add_argument('--type', help = "object type eg. virtual-network")
        parser.add_argument('--name', help = "colon seperated fully qualified name")
...
vnc = VncApi(username, password, tenant_name, server[0], server[1], user_info = ui)
...
if vnc_op.args.oper == 'create':
	...
	if len(fq_name) >= 3:
		obj = network = VirtualNetwork(name = fq_name[2], project_obj = project)
	method_name = vnc_op.args.type.replace('-', '_')
	method = getattr(vnc, "%s_create" % (method_name))
	#obj.set_network_ipam(ipam, vns)
	id = method(obj)
	print 'Created %s "%s", uuid = %s' %(vnc_op.args.type, vnc_op.args.name, id)
	obj.dump()
