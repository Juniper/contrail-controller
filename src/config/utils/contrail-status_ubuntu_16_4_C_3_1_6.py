...
#for contrail 3.1 or 3.2 return true 
def is_contrail_3_1_2():
    retval = False
    p1 = subprocess.Popen(['contrail-version', 'contrail-vrouter-agent'], stdout=subprocess.PIPE)
    p2 = subprocess.Popen(['tail', '-1'], stdin=p1.stdout, stdout=subprocess.PIPE)
    versioncmdout = p2.communicate()[0]
    arr = versioncmdout.split()
    if  len(arr) >=2 and ('3.1' in arr[1] or '3.2' in arr[1]):
        retval = True
    return retval
...
def is_xenial_or_above():
    ...
    if distribution == 'ubuntu' and \
        ...
        if is_contrail_3_1_2(): #hack for contrail 3.1
            #pretend we are a low version if contrail 3.1
            is_xenial_or_above =  False
        else:
...
def get_http_server_port_from_conf(svc_name, debug):
    ...
    if svc_name in ServicesDefaultConfigurationFile:
        default_conf_files = ServicesDefaultConfigurationFile[svc_name]
    else:
