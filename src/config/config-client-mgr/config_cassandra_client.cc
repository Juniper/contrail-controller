...
bool ConfigCassandraClient::ReadObjUUIDTable(set<string> *uuid_list) {
    ...
    if (dbif_->Db_GetMultiRow(&col_list_vec, kUuidTableName, keys,
        ...
        HandleCassandraConnectionStatus(false);
        CONFIG_CLIENT_WARN(ConfigClientGetRowError, "GetMultiRow failed for table",
                   kUuidTableName, "");
        //
    ...
    BOOST_FOREACH(string uuid_key, *uuid_list) {
        CONFIG_CLIENT_WARN(ConfigClientGetRowError, "Missing row in the table", kUuidTableName,
                   uuid_key);
...
void ConfigCassandraClient::GenerateAndPushJson(
    ...
    ConfigCass2JsonAdapter ccja(uuid_key, this, obj_type,
                                cass_data_vec);
    mgr()->config_client_json_parser()->Receive(ccja, add_change);
}

void ConfigCassandraClient::HandleObjectDelete(const string &uuid, bool add_change) {
    if (!add_change) {
...
bool ConfigCassandraClient::IsPropsEmpty(const string &uuid_key,
      const string &lookup_key) {
    return GetPartition(uuid_key)->IsPropsEmpty(uuid_key, lookup_key);
}
...
bool ConfigCassandraClient::FQNameReader() {
    for (ConfigClientManager::ObjectTypeList::const_iterator it =
         mgr()->config_client_json_parser()->ObjectTypeListToRead().begin();
         it != mgr()->config_client_json_parser()->ObjectTypeListToRead().end(); it++) {
        string column_name;
        while (true) {
            ...
            if (dbif_->Db_GetRow(&col_list, kFqnTableName, key,
                ...
                HandleCassandraConnectionStatus(false);
                CONFIG_CLIENT_WARN(ConfigClientGetRowError, "GetRow failed for table",
                           kFqnTableName, *it);
                usleep(GetInitRetryTimeUSec());
            }
        }
...
void ConfigCassandraPartition::HandleObjectDelete(const string &uuid, bool add_change){
...
bool ConfigCassandraPartition::IsPropsEmpty(const string &uuid_key,
      const string &lookup_key) {
    string key;
    ...
    FieldDetailMap::iterator lower_bound_it =
        uuid_iter->second.lower_bound(JsonAdapterDataType(key, ""));
...
void ConfigCassandraPartition::ListMapPropReviseUpdateList(
    ...
    for (std::set<std::string>::iterator it =
        ...
        FieldDetailMap::iterator field_iter =
            uuid_iter->second.lower_bound(JsonAdapterDataType(*it, ""));
...
bool ConfigCassandraPartition::StoreKeyIfUpdated(const string &uuid,
    ...
    FieldDetailMap::iterator field_iter 
                  = uuid_iter->second.find(JsonAdapterDataType(key, value));
    if (field_iter == uuid_iter->second.end()) {
        // seeing field for first time
        uuid_iter->second.insert(make_pair(JsonAdapterDataType(key, value),
