...
def vnc_etcd_factory(host='etcd-host-01', port='2379', prefix='/contrail',
    ...
    return VncEtcd(host=host, port=port, prefix=prefix, logger=logger,
                   ...
                   credential=credential, ssl_enabled=ssl_enabled,
                   ca_certs=ca_certs)


class VncEtcdTest(unittest.TestCase):
    def test_create_etcd_client_instance(self):
        vnc_etcd = vnc_etcd_factory(obj_cache_exclude_types=['tag_type'])
    ...
    def test_key_prefix(self):
        vnc_etcd = vnc_etcd_factory(obj_cache_exclude_types=[])
    ...
    @mock.patch('etcd3.client')
    def test_object_all_should_return_from_cache(self, etcd_client):
        example_resource = [('{"uuid":"aa3442c8a3dd"}', None),
                            ('{"uuid":"bb3442c8a3ee"}', None),
                            ('{"uuid":"cc3442c8a3ff"}', None)]
    ...
    def test_object_list_for_parent_uuid(self, etcd_client):
        ...
        with mock.patch.object(vnc_etcd, '_get_childes') as childes:
            childes.return_value = [{'uuid': 'ddd'}]
            ok, result, marker = vnc_etcd.object_list(
                obj_type='test_obj', parent_uuids=parent_uuids,
                obj_uuids=['ddd'])

        self.assertTrue(ok)
        self.assertListEqual(result, ['ddd'])
        self.assertIsNone(marker)

    @mock.patch('etcd3.client')
    def test_object_list_for_back_ref_uuid(self, etcd_client):
        back_ref_uuids = ['aaa', 'bbb', 'ccc']
        ...
        with mock.patch.object(vnc_etcd, '_get_childes') as childes:
            childes.return_value = [{'uuid': 'ddd'}]
            ok, result, marker = vnc_etcd.object_list(
                obj_type='test_obj', back_ref_uuids=back_ref_uuids,
                obj_uuids=['ddd'])
    ...
    def test_object_list_should_return_all_resources(self, etcd_client):
        ...
        with mock.patch.object(vnc_etcd, '_get_childes') as childes:
            childes.return_value = [{'uuid': 'ddd'}, {'uuid': 'eee'}]
            ok, result, marker = vnc_etcd.object_list(
                obj_type='test_obj', obj_uuids=['aaa', 'ccc'])

        self.assertTrue(ok)
        # get childes are called twice which case MagicMock to return
    ...
    @mock.patch('etcd3.client')
    def test_get_childes(self, etcd_client):
        parents = [{'uuid': 'aaa',
        ...
        vnc_etcd.object_read = mock.MagicMock(
            return_value=(True, {'uuid': 'aaa_child'}))

        result = vnc_etcd._get_childes(parents, obj_type='test',
                                       field='children')
    ...
    @mock.patch('etcd3.client')
    def test_get_childes_should_return_generator(self, etcd_client):
        vnc_etcd = vnc_etcd_factory()
        result = vnc_etcd._get_childes(parents=[], obj_type='test',
                                       field='test')
    ...
    @mock.patch('etcd3.client')
    def test_filter_return_emtpy(self, etcd_client):
        objs = []
