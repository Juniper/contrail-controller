...
class DBBase(object):
    ...
    def update_vnc_obj(self, obj=None):
        ...
        for field in self.ref_fields or []:
            old_field = getattr(old_obj, field+'_refs', None) or getattr(old_obj, field+'_back_refs', None)
            new_field = getattr(self.obj, field+'_refs', None) or getattr(self.obj, field+'_back_refs', None)
