...
=======
            ...
            action='store_true', default=False)
>>>>>>> def8ea9c9... [Config] Modified db_manage.py to support to read directly from JSON dump and cleaned up JSON Import Export Logic
    ...
    def db_import(self):
        ...
        self.zk_ignore_list = ['consumers', 'config', 'controller',
        ...
        for path_value_ts in json.loads(self.import_data['zookeeper'] or "{}"):
            ...
            if path.endswith('/'):
                path = path[:-1]
            if path.split('/')[1] in self.zk_ignore_list:
                continue
...
>>>>>>> def8ea9c9... [Config] Modified db_manage.py to support to read directly from JSON dump and cleaned up JSON Import Export Logic
        ...
        # end pre_db_export
    def db_export(self):
        db_contents = self.pre_db_export()
        def get_nodes(path):
            try:
    ...
    def db_import(self):
        ...
        def build_tree_from_json(root, json_dict):
            ...
            for json_key, json_val in json_dict.items():
                if json_key in self.zk_ignore_list:
                    continue
