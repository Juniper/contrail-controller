    ...
    def request(self, path, fqdn_uuid, user_token=None,
        ...
        for index in range(client_count):
            try:
                ...
                resp = None
                if (self._analytics_api_ssl_params and self._analytics_api_ssl_params['ssl_enable']):
                    endpoint = "https://%s:%s" % (analytics_ip,self.analytics_api_port)
                    certfile = self._analytics_api_ssl_params['certfile']
                    keyfile = self._analytics_api_ssl_params['keyfile']
                    self._cert = (certfile,keyfile)
                    url = urlparse.urljoin(endpoint, path + fqdn_uuid)
                    if (self._analytics_api_ssl_params['insecure_enable']):
                        resp = requests.get(url, cert=self._cert, 
                                verify=False, **req_params)
                    else:
                        resp = requests.get(url, cert=self._cert, 
                                verify=self._analytics_api_ssl_params['ca_cert'], 
                                **req_params)
                else:
