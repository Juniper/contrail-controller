...
# To avoid performance degradation when using `batch` we should never
# pass to them queries using different partition key. When this flag
# is enabled, such situation will be fixed by commiting current
# queries before adding new one, + this will log a message for
# developers to fix the code. For single node this can be turned off.
WARNING_AND_FLUSH_BATCH_FOR_KEY = True
...
# Because we use `Batch` with queries within same CFs and partition
# keys we can avoid writting LOG, that is improving performance.
BATCH_TYPE = connector.query.BatchType.UNLOGGED
...
class CassandraDriverCQL(datastore_api.CassandraDriver):
    ...
    def _Init_Cluster(self):
        ...
        for address in self._server_list:
            try:
                ...
                endpoints.append(address.split(':', 1)[0])
    ...
    def _Get_CF_Batch(self, cf_name, keyspace_name=None):
        ...
        class Batch(connector.query.BatchStatement):
            def __init__(self, session, cf_name, logger):
                ...
                super(Batch, self).__init__(
                    session=session,
                    consistency_level=DEFAULT_CONSISTENCY_LEVEL,
                    # Because we use batch by CF (in oposite to have
                    # batch quering several CFs) and because
                    # add/remove based on partition key we can avoid
                    # to write logs which add performances
                    batch_type=BATCH_TYPE)
            def add(self, partition_key, *args, **kwargs):
                if WARNING_AND_FLUSH_BATCH_FOR_KEY:
                    if self.partition_key is None:
                        self.partition_key = partition_key
                    elif self.partition_key != partition_key:
                        self.logger("Adding in batch a query using "
                                    "different partition key, this implies "
                        ...
                        self.send()
                        self.partition_key = partition_key
                super(Batch, self).add(*args, **kwargs)
            def send(self):
                self._session.execute(self)
                self.clear()
        return Batch(session=self.get_cf(cf_name),
                     cf_name=cf_name,
    ...
    def _Insert(self, key, columns, keyspace_name=None, cf_name=None,
        ...
        for column, value in columns.items():
            ...
            batch.add(key, cql, [StringType(key),
                                 StringType(column),
                                 StringType(value)])
