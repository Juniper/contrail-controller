...
CQL_VERSION = None
# The driver will try to connect server from the higher version to the
# lower. The minimun supported Cassandra is 2.0.
SUPPORTED_VERSIONS = (4, 3, 2)
...
class CassandraDriverCQL(datastore_api.CassandraDriver):
    ...
    def _Init_Cluster(self):
        ...
        for protocol_version in SUPPORTED_VERSIONS:
            # We are going from the best high to the lower supported
            # protocol by cluster.
            try:
                ...
                self.options.logger("cassandra enpoinds {} do not support "
                                    "protocol {}".format(endpoints,
                                                         protocol_version),
                                    level=SandeshLevel.SYS_DEBUG)
            else:
                self.options.logger("connected to cassandra enpoinds {}, "
                                    "protocol {}".format(endpoints,
                                                         protocol_version),
