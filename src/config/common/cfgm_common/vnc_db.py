...
class FeatureFlagBase(object):
    ...
    def set_version(cls, version):
        cls._dict['release_version'] = version
    ...
    def is_feature_enabled(cls, feature_id, version=None):
        ...
        if not version:
            version = cls._dict['release_version']
        version_fflags = cls._dict['feature_flags'].get(version, {})
    ...
    def _execute_callbacks(cls, feature_id):
        feature_cbs = cls._dict['callbacks'].get(feature_id)
        if not feature_cbs:
           return
    ...
    def feature_flag_update(cls, feature_id, version, old_fflag, new_fflag, execute_cbs=True):
        ...
        if old_fflag and \
           (old_fflag.get('feature_version_id') != new_fflag.get('feature_version_id')):
              return update
        cls._dict['feature_flags'].setdefault(version, {})
        cls._dict['feature_flags'][version][feature_id] = new_fflag
    ...
    def delete_feature_flag(cls, feature_id, version):
        ...
        cls._dict['feature_flags'][version].pop(feature_id)
        cls._dict['callbacks'].pop(feature_id)
