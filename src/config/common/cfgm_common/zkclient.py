...
class IndexAllocator(object):
    ...
    def get_children_wrapper(self, path):
        if self._db_manager is not None:
            return self._db_manager.zk_get_children(path)
        else:
            return self._zookeeper_client.get_children(path)
   
    def _create(self, path, value):
        if self._db_manager is not None:
            self._db_manager.zk_create(path, value)
        else:
            self._zookeeper_client.create_node(path, value)
