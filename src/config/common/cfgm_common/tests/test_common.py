...
try:
    ...
    if not hasattr(vnc_cfg_api_server, 'main'):
        from vnc_cfg_api_server import vnc_api_server
except ImportError:
    vnc_api_server = 'vnc_api_server could not be imported'
...
def launch_api_server(test_id, listen_ip, listen_port, http_server_port,
    ...
    with tempfile.NamedTemporaryFile() as conf, \
        ...
        server = vnc_api_server.VncApiServer(args_str)
        gevent.getcurrent().api_server = server
        vnc_api_server.main(args_str, server)
