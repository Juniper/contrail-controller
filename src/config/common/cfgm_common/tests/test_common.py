...
class TestCase(testtools.TestCase, fixtures.TestWithFixtures):
    ...
    def _get_golden_json(cls):
        ...
        if not os.path.exists(dirpath):
            os.makedirs(dirpath)
        filename_seed = os.path.join(dirpath, 'db-dump.json')

        # every process gets it's own file to dump db into
        filename_proc = os.path.join(dirpath, ('db-dump-%s.json' % id(cls)) )
        try:
            os.stat(filename_proc)
            return filename_proc
        except OSError:
            # this is the first time we are being run
            open(filename_proc, 'a').close()
            return filename_seed
    # end get_golden_json
