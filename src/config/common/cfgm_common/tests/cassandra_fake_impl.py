...
class CassandraFakeServer(object):
    ...
    def create_keyspace(self, name):
        self.__keyspaces__[name] = _CassandraFakeServerKeyspace(name)

        return self.__keyspaces__[name]

    def drop_keyspace(self, keyspace):
        if name in self.__keyspaces__:
            del self.__keyspaces__[name]
...
class _CassandraFakeServerTable(object):
    ...
    def get(self, key, columns=None, column_start=None, column_finish=None,
        ...
        if decode_json is None:
            decode_json = self.keyspace.endswith(
                datastore_api.UUID_KEYSPACE_NAME)

        def decode(obj, decode_json, include_timestamp):
            if not decode_json:
                return obj
            if include_timestamp:
                return (json.loads(obj[0]), obj[1])
            return json.loads(obj)

        sorted_col_dict = OrderedDict(
            (k, decode(col_dict[k], decode_json, include_timestamp)) for k in sorted(col_dict))
    ...
    def patch_cf(self, new_contents=None):
        orig_contents = CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name]
        try:
            CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name] = new_contents
            yield
        finally:
            CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name] = orig_contents
