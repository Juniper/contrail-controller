    ...
    def execute(self, prepare):
        ...
        if query.conditions:
            ...
            if query.method == "get":
                ...
                try:
                    return [(c, v[0], v[1]) for c, v in self.get(**args).items()]
...
class _CassandraFakeServerTable(_TableCQLSupport):
    ...
    def patch_cf(self, new_contents=None):
        orig_contents = CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name].__rows__
        try:
            CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name].__rows__ = new_contents
            yield
        finally:
            CassandraFakeServer.__keyspaces__[self.keyspace].__tables__[self.name].__rows__ = orig_contents
