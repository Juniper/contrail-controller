...
_VN_NAME_RE = "contrail:virtual-network:%s:%s:%s" %(_TENANT_GRP, _VPC_GRP, _VN_GRP)
_SG_NAME_RE = "contrail:security-group:%s:%s:%s" %(_TENANT_GRP, _VPC_GRP, _SG_GRP)
...
def parse_result_items(result_items, my_imid = None):
    ...
    for (ident_1, ident_2, meta) in all_result_list:
        if (((ident_2 != None) and (ident_2.attrib['name'] == my_imid)) or
...
def subscribe_root(ssrc_mapc):
    ...
    ident = str(Identity(name = _ROOT_IMID, type = "other",
                         other_type = "extended"))
    subreq = SubscribeRequest(ssrc_mapc.get_session_id(),
                 operations = str(SubscribeUpdateOperation("root",
                                      ident,
                                  {
                                      "max-depth":"255", #TODO -hard-code
                                  })))
    ...
    for result in poll_results:
        ...
        for child in children:
            ...
            for item in item_list:
                ...
                idents[ident1_type] = get_fq_name_str_from_ifmap_id(ident1_imid)
                if ident2 is not None:
                    ...
                    if ident1_type == ident2_type:
                        idents[ident1_type] = [idents[ident1_type], get_fq_name_str_from_ifmap_id(ident2_imid)]
                    else:
                        idents[ident2_type] = get_fq_name_str_from_ifmap_id(ident2_imid)
