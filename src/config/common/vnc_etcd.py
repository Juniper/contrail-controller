...
class VncEtcdClient(object):
    ...
    def _new_client(self):
        ...
        return etcd3.client(timeout=2, **kwargs)
...
class VncEtcd(VncEtcdClient):
    ...
    def object_all(self, obj_type):
        ...
        :return: (gen) Generator with resources dict
        """
        key = self._key_prefix(obj_type)
        if key in self._obj_cache:
            resource = self._obj_cache[key].resource
        else:
            response = self._client.get_prefix(key)
            resource = []
            kv_meta = []
            for res in response:
                resource.append(json.loads(res[0]))
                kv_meta.append(res[1])
        ...
        if not parent_uuids and not back_ref_uuids:
            ...
            if obj_uuids:
                ...
                _, parents = self.object_read(obj_type, obj_uuids,
                                              ret_readonly=True)
                for key in child_keys.keys():
                    childes = self._get_childes(parents, obj_type,
                                                child_keys[key])
                    filtered = self._filter(childes, filters=filters)
                    results.extend(filtered)
            else:
    ...
    def fq_name_to_uuid(self, obj_type, fq_name):
        ...
        if key in self._cache:
            ...
            results = []  # must have only one element
            for resp in response:
                resp = json.loads(resp)
                resp_fq_name = utils.encode_string(':'.join(resp['fq_name']))
                if resp_fq_name == fq_name_str:
                    results.append(resp)

            if not results:
                msg = '{} {}'.format(obj_type, fq_name_str)
                raise NoIdError(msg)
            if len(results) > 1:
                msg = 'Multi match {} for {}'.format(fq_name_str, obj_type)
                raise VncError(msg)

            record = EtcdCache.Record(resource=results[0])
    ...
    def _filter(self, objs, obj_uuids=None, filters=None):
        ...
        for obj in objs:
            if not filters:
                ...
                if yield_obj:
                    yield obj  # yield only full match object
