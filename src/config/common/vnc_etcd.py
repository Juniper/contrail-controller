...
def with_etcd_args(parser):
    ...
    parser.add_argument('--etcd_use_ssl', type=_str_to_bool,
...
class VncEtcd(VncEtcdClient):
    ...
    def fq_name_to_uuid(self, obj_type, fq_name):
        ...
        if key in self._cache:
            ...
            record = None
            fq_name_str = self._fq_name_str(fq_name)
            for data, _ in response:
                obj = json.loads(data)
                obj_fq_name = self._fq_name_str(obj['fq_name'])
    ...
    def _cache_get_fq_name_key(self, obj_type, fq_name):
        prefix = self._key_prefix(obj_type)
        key = "{}{}".format(prefix, self._fq_name_str(fq_name))
        return key
    ...
    def _fq_name_str(self, fq_name):
        """Make fq_name string from fq_name list
        Format of output string is 'root:parent:child'

        :param (List[str]) fq_name: list of strings
        :return: (str) fq_name string
        """
        return utils.encode_string(':'.join(fq_name))
