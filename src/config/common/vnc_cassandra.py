...
class VncCassandraClient(object):
    ...
    def get_shared(self, obj_type, share_id, share_type = 'tenant'):
        result = []
        method_name = obj_type.replace('-', '_')
    ...
    # share object 'obj_id' with <share_type:share_id>
    # column name:data (share_type, share_id, obj_id):rwx
    def set_shared(self, obj_type, obj_id, share_id, share_type = 'tenant', rwx = 7):
        col_name = '%s:%s:%s' % (share_type, share_id, obj_id)
        method_name = obj_type.replace('-', '_')
        self._obj_shared_cf.insert(method_name, {col_name : json.dumps(rwx)})

    # delete share of 'obj_id' object with <group:id>
    def del_shared(self, obj_type, obj_id, share_id, share_type = 'tenant'):
        col_name = '%s:%s:%s' % (share_type, share_id, obj_id)
        method_name = obj_type.replace('-', '_')
