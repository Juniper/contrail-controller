...
class VncCassandraClient(object):
    ...
    def get(self, cf_name, key, columns=[], suffixed=False):
        results = OrderedDict()
        cf = self.get_cf(cf_name)
        if columns:
            for column in columns:
                start = finish = column
                if suffixed and not start.endswith(':'):
                    start += ':'
                if suffixed and not start.endswith(';'):
                    finish += ';'
                for col, val in cf.xget(key,
                    ...
                    try:
                        results[col] = json.loads(val)
                    except ValueError as e:
                        msg = ("Cannot json load the value of cf: %s, key:%s, "
                               "columns:%s (error: %s). Use it as is: %s" %
                               (cf_name, key, column, str(e), val))
    ...
    def get_one_col(self, cf_name, key, column, suffixed=False):
        col = self.get(cf_name, key, columns=[column], suffixed=suffixed)
        if not col:
            raise NoIdError(key)
        elif len(col) > 1:
    ...
    def object_read(self, res_type, obj_uuids, field_names=None):
        ...
        for obj_uuid in obj_uuids:
            ...
            if obj_type != json.loads(obj_rows['type'][0]):
                continue
            ok, result = self._object_construct(
                obj_uuid, OrderedDict(obj_rows), field_names)
            if not ok:
                return (False, result)
    ...
    def _object_construct(self, obj_uuid, obj_cols, field_names=None):
    ...
    def object_list(self, res_type, parent_uuids=None, back_ref_uuids=None,
        ...
        if back_ref_uuids:
            ...
            for back_ref_uuid in back_ref_uuids:
                ...
                obj_row = OrderedDict(obj_row)
                if obj_row:
                    obj_rows[back_ref_uuid] = obj_row
            if not obj_rows:
                return ((True, 0) if count else (True, []))
