...
class VncCassandraClient(object):
        ...
        for ks, cf_dict in itertools.chain(self._rw_keyspaces.items(),
            ...
            for cf_name in cf_dict:
                ...
                self._cf_dict[cf_name] = ColumnFamily(
                    ...
                    dict_class=dict,
                    **cf_kwargs)
                for col in cf_dict[cf_name].get('indexes', []):
                    ...
                    for index in cf_def.column_metadata:
                        ...
                        self.sys_mgr.create_index(keyspace, cf_name, col,
                                                  'UTF8Type',
                                                  index_name='index_%s' % col)
    ...
    def object_raw_read(self, type, uuids=None, fq_names=None,
        ...
        hit_obj_dicts, miss_uuids, miss_fq_names =\
            self._obj_cache_mgr.read(uuids, fq_names, prop_names)
        ...
        for col_name, col_value in obj_uuid_cf.xget(obj_uuid):
            if self._is_prop(col_name):
                ...
                if prop_name == 'id_perms':
                    ...
                    # get it from request dict (or from db if not in request
                    # dict)
