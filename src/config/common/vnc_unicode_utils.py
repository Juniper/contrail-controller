...
def encode_str_list(str_list, encoding='utf-8'):
    if not str_list:
        return
    for i in range(len(str_list)):
        if '%' not in str_list[i]:
            str_list[i] = encode_string(str_list[i], encoding)


def decode_str_list(str_list, encoding='utf-8'):
    if not str_list:
        return
    for i in range(len(str_list)):
        if '%' in str_list[i]:
            str_list[i] = decode_string(str_list[i], encoding)


def check_and_encode_string(enc_str, encoding='utf-8'):
    if '%' not in enc_str:
        enc_str = encode_string(enc_str, encoding)
    return enc_str
