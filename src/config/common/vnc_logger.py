...
class ConfigServiceLogger(object):
    ...
    def __init__(self, discovery, module, module_pkg, args=None):
        ...
        cpu_info = vnc_cpu_info.CpuInfo(
                self._module_name, self._instance_id, sysinfo_req,
                self._sandesh, 60)
    ...
    def log(self, log_msg, level=SandeshLevel.SYS_DEBUG):
        if self._module_name == 'contrail-svc-monitor':
            vn_log = sandesh.SvcMonitorLog(
                    level=level, log_msg=log_msg, sandesh=self._sandesh)
    ...
    def sandesh_init(self):
        ...
        self.sandesh.set_logging_params(
