...
class BadParameter(BadRequest):
    ...
    def __init__(self, 
        ...
        if message is None:
            if bad_value is not None:
                self.message = \
                    'Bad request for value {} of property {} - Current Body: {}' \
                    .format(bad_value, bad_property \
                        if bad_property is not None \
                        else 'Unknown', current_body \
                        if current_body is not None \
                        else 'Unknown')
            else:
                self.message = \
                    'Bad request for property {} with no value - Current Body: {}' \
                    .format(bad_property \
                        if bad_property is not None \
                        else 'Unknown', current_body \
                        if current_body is not None \
    ...
    def __str__(self):
        if self.bad_value is not None:
            return \
                'Bad request for value {} of property {} - Current Body: {}' \
                .format(self.bad_value, self.bad_property \
                    if self.bad_property is not None \
                    else 'Unknown', self.current_body \
                    if self.current_body is not None \
                    else 'Unknown')
        else:
            return \
                'Bad request for property {} with no value - Current Body: {}' \
                .format(self.bad_property \
                    if self.bad_property is not None \
                    else 'Unknown', self.current_body \
                    if self.current_body is not None \
                    else 'Unknown')
