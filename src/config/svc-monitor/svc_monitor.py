        ...
        for inst_count in range(0, max_instances):
            try:
                ...
                self._svc_syslog("Cannot find enough virtual routers to "
                                 ...
                                 "service instance" % (si_obj.name, inst_count))
                break
            vrs.remove(vr_fq_name_selected)
            vr_obj_selected = self._vnc_lib.virtual_router_read(fq_name=vr_fq_name_selected)

            vm_fq_name = proj_fq_name + [("%s_%s") % (si_obj.uuid, inst_count)]
