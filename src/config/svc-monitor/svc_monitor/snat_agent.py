...
class SNATAgent(Agent):
    ...
    def _create_snat_vn(self, project_obj, vn_name, vn_subnet):
        vn_obj = VirtualNetwork(name=vn_name, parent_obj=project_obj)
        ...
        VirtualNetworkSM.locate(vn_obj.uuid, vn_obj.serialize_to_json())
    ...
    def _get_snat_vn(self, project_obj, si_obj):
        ...
        try:
            self._vnc_lib.fq_name_to_id('virtual-network', vn_fq_name)
        except NoIdError:
            snat_subnet = svc_info.get_snat_left_subnet()
            self._create_snat_vn(project_obj, vn_name, snat_subnet)
    ...
    def _diff_virtual_interfaces(self, router_obj):
        uuids = set([uuid for uuid in
                     router_obj.virtual_machine_interfaces])
        last_uuids = set([uuid for uuid in
                     router_obj.last_virtual_machine_interfaces])
    ...
    def _update_snat_instance(self, router_obj):
        ...
        if to_del or to_add:
            project_obj = self._vnc_lib.project_read(
                id=router_obj.parent_uuid)
    ...
    def _delete_snat_instance(self, router_obj):
        ...
        if rt_obj:
            ...
            uuids = [ref['uuid'] for ref in
                     rt_obj.get_virtual_network_back_refs()]
