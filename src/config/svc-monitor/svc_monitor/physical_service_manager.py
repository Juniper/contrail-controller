...
from config_db import ServiceApplianceSetSM, ServiceApplianceSM, PhysicalInterfaceSM, ServiceInstanceSM
...
class PhysicalServiceManager(InstanceManager):
    ...
    def create_service(self, st, si):
        ...
        if si.max_instances > len(service_appliances):
            self.logger.log_info("There are not enough Service appliance for that Service instance")
            return
        idx = 0;
            ...
            for i in range(0,len(ports)):
                if i <= len(pi_uuid_set):
                    self._vnc_lib.ref_update('virtual-machine-interface', 
                        ports[i].uuid, 'physical_interface_refs', pi_uuid_set[i], None, 'ADD')
