...
class ServiceMonitorDB(object):
    ...
    @staticmethod
    def get_vm_db_prefix(inst_count):
...
class ServiceMonitorCass(VncObjectDBClient):
    ...
    def __init__(self, args, logger):
        ...
        cred = None
        if args.cassandra_user is not None and args.cassandra_password is not None:
...
class ServiceMonitorEtcd(VncObjectEtcdClient):
    ...
    def _path_key(rsrc_type, uuid):
        return "{prefix}/{type}/{uuid}".format(prefix=ServiceMonitorEtcd.PREFIX, type=rsrc_type, uuid=uuid)
    ...
    def _db_get(self, rsrc_type, key, column):
        try:
            ...
            self._db_logger.log("DB: %s %s get failed. Exception %s" %
                                (inspect.stack()[1][3], key, e))
            return None
    ...
    def _db_insert(self, rsrc_type, uuid, entry):
        ...
        try:
            ...
            self._db_logger.log("DB: %s %s insert failed. Exception %s" %
                                (inspect.stack()[1][3], key, e))
            return False
        return True
    ...
    def _db_remove(self, rsrc_type, uuid, columns=None):
        ...
        try:
            ...
            self._db_logger.log("DB: %s %s remove failed. Exception %s" %
                                (inspect.stack()[1][3], key, e))
            return False
        return True
    ...
    def _db_list(self, rsrc_type):
        try:
            ...
            self._db_logger.log("DB: %s list failed. Exception %s" %
                                (inspect.stack()[1][3], e))
            return None
        return entries
