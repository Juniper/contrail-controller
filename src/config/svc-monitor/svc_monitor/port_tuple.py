...
class PortTupleAgent(Agent):
    ...
    def _get_vmi_obj(self, vmi):
        proj = ProjectSM.get(vmi.parent_key)
        proj_obj = Project()
        proj_obj.uuid = proj.uuid
        proj_obj.name = proj.name
        proj_obj.fq_name = proj.fq_name

        vmi_obj = VirtualMachineInterface(parent_obj=proj_obj, name=vmi.name)
