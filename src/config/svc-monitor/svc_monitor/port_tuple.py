...
class PortTupleAgent(Agent):
    ...
    def _allocate_health_check_iip_for_family(self, health_id, si, iip_family, port, vmi):
    ...
    def _allocate_health_check_iip(self, health_id, si, port, vmi):
        ...
        for health_id in list(vmi.service_health_checks):
            ...
            if health and health.service_instances:
                for pt_id in vmi.port_tuples:
                    ...
                    if not si_temp:
                        continue
                    if health.uuid in si_temp.service_health_checks.keys():
                        pt_present = True
                        break
            if pt_present == False:
                self._vnc_lib.ref_update('virtual-machine-interface', vmi.uuid,
        ...
        for health_id in list(vmi.service_health_checks):
            ...
            if health and health.service_instances:
                for pt_id in vmi.port_tuples:
                    ...
                    if not si:
                        continue
                    if health.uuid in si.service_health_checks.keys():
                        pt_present = True
                        break
            if pt_present == False:
                self._vnc_lib.ref_update('virtual-machine-interface', vmi.uuid,
