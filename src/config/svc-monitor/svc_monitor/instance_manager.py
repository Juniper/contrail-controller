...
class InstanceManager(object):
    ...
    def _set_static_routes(self, nic, si):
        ...
        rt_obj = self._vnc_lib.interface_route_table_read(id=rt_id)
        InterfaceRouteTableSM.locate(rt_obj.uuid, self._vnc_lib.obj_to_dict(rt_obj))
        return rt_obj
    ...
    def link_si_to_vm(self, si, st, instance_index, vm_uuid):
        ...
        try:
            ...
            vm_res = self._vnc_lib.virtual_machine_update(vm_obj)
            vm_id = vm_res['virtual-machine']['uuid']
        vm_obj = self._vnc_lib.virtual_machine_read(id = vm_id)

        VirtualMachineSM.locate(vm_obj.uuid,self._vnc_lib.obj_to_dict(si_obj))
        self.logger.info("Info: VM %s updated SI %s" %
                             (vm_obj.get_fq_name_str(), self._vnc_lib.obj_to_dict(si_obj)))
    ...
    def create_port_tuple(self, si, st, instance_index, pt_uuid):
        ...
        pt_obj = self._vnc_lib.port_tuple_read(id=pt_id or pt_uuid)

        PortTupleSM.locate(pt_uuid, pt_obj.to_dict())
        self.logger.info("Info: PT %s updated SI %s" %
                             (pt_obj.get_fq_name_str(), self._vnc_lib.obj_to_dict(si_obj)))
        return pt_obj
    ...
    def create_service_vn(self, vn_name, vn_subnet, vn_subnet6,
        ...
        VirtualNetworkSM.locate(vn_obj.uuid, self._vnc_lib.obj_to_dict(vn_obj))
    ...
    def _check_create_netns_vm(self, instance_index, si, st, vm):
        ...
        if not vm:
            ...
            vm = VirtualMachineSM.locate(vm_obj.uuid, self._vnc_lib.obj_to_dict(vm_obj))
    ...
    def _create_svc_vm_port(self, nic, instance_name, si, st,
        ...
        if vmi_create:
            ...
            try:
                ...
                vmi_res = self._vnc_lib.virtual_machine_interface_update(vmi_obj)
                vmi_id = vmi_res['virtual-machine-interface']['uuid']
            if pi:
        ...
        vmi_obj = self._vnc_lib.virtual_machine_interface_read(id=vmi_id or vmi_obj.uuid)
        VirtualMachineInterfaceSM.locate(vmi_obj.uuid, self._vnc_lib.obj_to_dict(vmi_obj))
