...
class SvcMonitor(object):
    ...
    def sync_sm(self):
        ...
        if not ok:
            ...
            for fq_name, uuid in vm_list:
                ...
                if not vm.service_instance:
                    VirtualMachineSM.delete(uuid)
    ...
    def _delete_service_instance(self, vm):
        self.logger.log("Deleting VM %s %s" % (vm.proj_name, vm.uuid))

        if vm.virtualization_type == svc_info.get_vm_instance_type():
            self.vm_manager.delete_service(vm.uuid, vm.proj_name)
        elif vm.virtualization_type == svc_info.get_netns_instance_type():
            self.netns_manager.delete_service(vm.uuid)
        elif vm.virtualization_type == 'vrouter-instance':
            self.vrouter_manager.delete_service(vm.uuid)
