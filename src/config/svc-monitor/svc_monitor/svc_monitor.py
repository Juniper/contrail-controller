...
class SvcMonitor(object):
    ...
    def _create_svc_instance_netns(self, st_obj, si_obj):
        ...
        for inst_count in range(0, max_instances):
            ...
            # NetNS was not schedule to a vrouter
            row_entry = {}
            ...
            if vr_obj_selected:
                row_entry['vrouter_name'] = vrs_fq_name_selected[inst_count][-1]
            self._svc_vm_cf.insert(vm_obj.uuid, row_entry)
    ...
    def _schedule_virtual_routers(self, vrouter_count=1):
        ...
        while vrouter_count:
            ...
            if self._vrouter_running(vr_fq_name_selected[-1]):
                vrs_fq_name_selected.append(vr_fq_name_selected)
                vrouter_count -= 1

        return vrs_fq_name_selected

    def _vrouter_running(self, vrouter_name):
        """
        From the WebUI code:
        ...
        fqdn_uuid = "%s:Compute:VRouterAgent:0?flat" % vrouter_name

        try:
            vrouter_stats = self._analytics.request(path, fqdn_uuid).json()
        except OpencontrailAPIFailed:
            return False
    ...
    for vm_uuid, si in vm_list:
        try:
            ...
            if si['instance_type'] == 'virtual-machine':
                ...
                try:
                    if not monitor._vrouter_running(si['vrouter_name']):
