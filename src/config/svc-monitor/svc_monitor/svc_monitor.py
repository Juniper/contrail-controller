...
def cleaner_callback(monitor):
    monitor.logger.log_debug("SI cleanup starts")
...
def check_status_callback(monitor):
    monitor.logger.log_debug("SI health check starts")
...
def launch_timer(monitor, method, time):
    while True:
        gevent.sleep(time)
        try:
            method(monitor)
...
def run_svc_monitor(args=None):
    ...
    cleanup_task = gevent.spawn(launch_timer,
                                monitor,
                                cleaner_callback,
                                monitor._get_cleanup_delay())
    check_status_task = gevent.spawn(launch_timer,
                                     monitor,
                                     check_status_callback,
                                     monitor._get_check_status_delay())
