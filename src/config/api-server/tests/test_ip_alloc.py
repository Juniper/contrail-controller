...
class TestIpAlloc(test_case.ApiServerTestCase):
    ...
    def test_update_delete_notify(self):
        #                                                       #
        # This test check network and ipam delete_notify method #
        #                                                       #
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'][vn1.uuid]), 2)
        ...
        # exec update req (should not update _subnet_objs)
        addr_mgmt.net_update_req(vn1_dict['fq_name'], vn1_dict, self._vnc_lib.obj_to_dict(vn1))
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'][vn1.uuid]), 1)
        self.assertEqual(addr_mgmt.__dict__['_subnet_objs'][vn1.uuid].keys()[0], '192.168.150.0/24')
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'][ipam2.uuid]), 2)
        ...
        # exec update req (should not update _subnet_objs)
        addr_mgmt.ipam_update_req(ipam2_dict['fq_name'], ipam2_dict,
                self._vnc_lib.obj_to_dict(ipam2), ipam2.uuid)
        ...
        # check that it remains only one subnet
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'][ipam2.uuid]), 1)
        self.assertEqual(addr_mgmt.__dict__['_subnet_objs'][ipam2.uuid].keys()[0], '11.1.2.0/23')
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'].keys()), 2)

        # exec delete request should not modify '_subnet_objs'
        addr_mgmt.net_delete_req(vn1_dict)
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'].keys()), 1)

        # check that ipam2 still remains in '_subnet_objs'
        self.assertEqual(addr_mgmt.__dict__['_subnet_objs'].keys()[0],
            ipam2.uuid)
        ...
        self.assertEqual(len(addr_mgmt.__dict__['_subnet_objs'].keys()), 0)
    ...
    def test_flat_subnet_ipam_user_defined_network(self):
        ...
        # exec ipam_update_notify to update AddrMgmt obj after ipam update
        addr_mgmt = AddrMgmt(self._api_server)
        addr_mgmt.ipam_update_notify(ipam.uuid)
