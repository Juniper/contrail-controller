...
class TestIpAlloc(test_case.ApiServerTestCase):
    ...
    def test_floating_ip_pool_subnet_create(self):
        ...
        subnet_list = [FloatingIpPoolSubnetType(subnet_uuid=self.id())]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'user-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
        ...
        subnet_list = [FloatingIpPoolSubnetType(subnet_uuid = svc_subnet_uuid)]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'user-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
    ...
    def test_floating_ip_pool_subnet_prefix_create(self):
        """
        Test Floating-Ip-Pool create by specifiying subnet uuid and subnet
        prefix within an Ipam in a virtual-network. This testcase covers both
        valid and invalid subnet uuid and prefix specification scenarios.
        """
        def _get_ipam_subnet_uuid(ipam_ref, ipam_obj):
            """
            Convenience function to return subnet uuid in a ipam ref on
        ...
        # Create a virtual-network and add flat-subnet IPAM to it.
        vn_obj = VirtualNetwork('flat-vn-%s' %(self.id()), proj_obj)
        vn_obj.add_network_ipam(ipam_obj, VnSubnetsType([]))
        vn_obj.set_virtual_network_properties(\
            VirtualNetworkType(forwarding_mode='l3'))
        self._vnc_lib.virtual_network_create(vn_obj)
        ...
        # Create a Floating-ip-pool that points to an valid subnet id but
        # invalid subnet prefix. Verify, floating-ip-pool create fails.
        subnet_list = [FloatingIpPoolSubnetType(\
            subnet_uuid = svc_subnet_uuid, subnet=SubnetType('20.2.2.0', 30))]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'flat-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
        with ExpectedException(cfgm_common.exceptions.BadRequest,
                 'Specified prefix does not belong to requested subnet') as e:
            self._vnc_lib.floating_ip_pool_create(fip_pool_obj)

        # Create a Floating-ip-pool that points to an valid subnet id and
        # valid subnet prefix. Verify, floating-ip-pool create succeeds.
        subnet_list = [FloatingIpPoolSubnetType(\
            subnet_uuid = svc_subnet_uuid, subnet=SubnetType('10.1.1.0', 30))]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'flat-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
        ...
        # invalid subnet prefix. Verify, floating-ip-pool create fails.
        subnet_list = [FloatingIpPoolSubnetType(\
            subnet_uuid = svc_subnet_uuid, subnet=SubnetType('22.2.2.0', 30))]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'user-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
        ...
        # valid subnet prefix. Verify, floating-ip-pool create succeeds.
        subnet_list = [FloatingIpPoolSubnetType(\
            subnet_uuid = svc_subnet_uuid, subnet=SubnetType('11.1.1.0', 30))]
        fip_prefixes = FloatingIpPoolType(subnet_list=subnet_list)
        fip_pool_obj = FloatingIpPool(
            'user-fip-pool-%s' %(self.id()), parent_obj=vn_obj,
            floating_ip_pool_prefixes = fip_prefixes)
        self._vnc_lib.floating_ip_pool_create(fip_pool_obj)
    # end test_floating_ip_pool_subnet_prefix_create
