...
def set_perms(obj, owner=None, owner_access=None, share=None, global_access=None):
    ...
    if global_access is not None:
        ...
        perms.globally_shared = global_access != PERMS_NONE
...
def all(ip='127.0.0.1', port=8082, domain_name='default-domain',
    ...
    # allow permission to create objects (no delete permission)
    for user in [alice, bob]:
    ...
    print
    print 'Enable virtual networks in alice project for global sharing'
    ...
    set_perms(vn, global_access = PERMS_NONE)
    try:
