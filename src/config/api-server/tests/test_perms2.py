...
class User(object):
   ...
   def set_role(self, new_role):
       role_dict = {role.name:role for role in self.kc.roles.list()}
       if new_role not in role_dict:
           logger.info('role %s missing from keystone ... creating' % new_role)
           self.kc.roles.create(new_role)
           role_dict = {role.name:role for role in self.kc.roles.list()}
       self.kc.roles.add_user_role(self.user, role_dict[new_role], self.tenant)

       token = token_from_user_info(self.name, self.project,
            'default-domain', new_role, self.project_uuid)
...
class TestPermissions(test_case.ApiServerTestCase):
    ...
    def setUp(self):
        ...
        self.kc = kc = keystone.Client(username='admin', password='contrail123',
                       tenant_name='admin',
                       auth_url='http://127.0.0.1:5000/v2.0')

        # prepare token before vnc api invokes keystone
        self.alice = User(ip, port, kc, 'alice', 'alice123', 'alice-role', 'alice-proj')
        self.bob =   User(ip, port, kc, 'bob', 'bob123', 'bob-role', 'bob-proj')
        self.admin = User(ip, port, kc, 'admin', 'contrail123', 'cloud-admin', 'admin')
        self.admin1 = User(ip, port, kc, 'admin1', 'contrail123', 'admin', 'admin1')
        self.admin2 = User(ip, port, kc, 'admin2', 'contrail123', 'admin', 'admin2')
