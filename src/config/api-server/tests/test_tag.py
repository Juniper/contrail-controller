...
class TestTagType(TestTagBase):
    ...
    def test_tag_type_is_unique(self):
        ...
        with ExpectedException(exceptions.RefsExistError):
            new_tag_type = TagType(name='tag-type-%s' % self.id())
            self.api.tag_type_create(new_tag_type)

    def test_tag_type_id_cannot_be_set(self):
        tag_type = TagType(name='tag-type-%s' % self.id(), id='0xBEEF')
        with ExpectedException(exceptions.BadRequest):
            self.api.tag_type_create(tag_type)

    def test_tag_type_id_cannot_be_updated(self):
        tag_type = TagType(name='tag-type-%s' % self.id())
        tag_type_id = self.api.tag_type_create(tag_type)
        tag_type = self.api.tag_type_read(id=tag_type_id)
        with ExpectedException(exceptions.BadRequest):
            tag_type.id = '0xbeef'
    ...
    def test_deallocate_tag_type_id(self):
        ...
        tag_type = self.api.tag_type_read(id=tag_type_id)
        zk_id = int(tag_type.id, 0)
        self.api.tag_type_delete(id=tag_type_id)
...
class TestTag(TestTagBase):
    def test_tag_name_is_composed_with_type_and_value(self):
        ...
        tag = Tag(name=name, display_name=name, type=type, value=value)
        tag_id = self.api.tag_create(tag)
        tag = self.api.tag_read(id=tag_id)
    ...
    def test_tag_type_cannot_be_updated(self):
        ...
        with ExpectedException(exceptions.BadRequest):
            tag.type = 'new_fake_type-%s' % self.id()
            tag_id = self.api.tag_update(tag)
    ...
    def test_tag_value_cannot_be_updated(self):
        ...
        with ExpectedException(exceptions.BadRequest):
            tag.value = 'new_fake_type-%s' % self.id()
            tag_id = self.api.tag_update(tag)

    def test_tag_id_cannot_be_set(self):
        type = 'fake_type-%s' % self.id()
        value = 'fake_value-%s' % self.id()
        tag = Tag(type=type, value=value, id='0xDEADBEEF')
        with ExpectedException(exceptions.BadRequest):
            self.api.tag_create(tag)
    ...
    def test_tag_type_reference_cannot_be_set(self):
        value = 'fake_value-%s' % self.id()
        tag_type = TagType(name='tag-type-%s' % self.id())
        tag = Tag(type=tag_type.name, value=value)
    ...
    def test_tag_type_reference_cannot_be_removed(self):
        ...
        tag_type_id = tag.get_tag_type_refs()[0]['uuid']
    ...
    def test_tag_is_unique_in_same_scope(self):
        ...
        self.api.tag_create(tag)
        with ExpectedException(exceptions.RefsExistError):
            new_tag = Tag(type=type, value=value)
    ...
    def test_allocate_tag_id(self):
        ...
        zk_id = int(tag.id, 0) & 0x0000ffff
        self.assertEqual(tag.get_fq_name_str(),
                         mock_zk.get_tag_value_from_id(tag.type, zk_id))
    ...
    def test_deallocate_tag_id(self):
        ...
        zk_id = int(tag.id, 0) & 0x0000ffff
        self.api.tag_delete(id=tag_id)
        self.assertIsNone(mock_zk.get_tag_value_from_id(tag.type, zk_id))
    ...
    def test_create_project_scoped_tag(self):
        ...
        type = 'fake_type-%s' % self.id()
        value = 'fake_value-%s' % self.id()
        tag = Tag(parent_obj=project, type=type, value=value)
        tag_id = self.api.tag_create(tag)
        tag = self.api.tag_read(id=tag_id)
    ...
    def test_tag_duplicable_between_global_and_project(self):
        ...
        global_tag = Tag(type=type, value=value)
        global_tag_id = self.api.tag_create(global_tag)

        project = Project('project-%s' % self.id())
        self.api.project_create(project)
        scoped_tag = Tag(parent_obj=project, type=type, value=value)
        scoped_tag_id = self.api.tag_create(scoped_tag)
        self.assertNotEquals(global_tag_id, scoped_tag_id)
    ...
    def test_tag_duplicable_between_project(self):
        ...
        project1_tag = Tag(parent_obj=project1, type=type, value=value)
        project1_tag_id = self.api.tag_create(project1_tag)

        project2 = Project('project2-%s' % self.id())
        self.api.project_create(project2)
        project2_tag = Tag(parent_obj=project2, type=type, value=value)
        project2_tag_id = self.api.tag_create(project2_tag)

        self.assertNotEquals(project1_tag_id, project2_tag_id)
    ...
    def test_associate_global_tag_to_a_resource(self):
        ...
        self.api.project_create(project)
        vn = VirtualNetwork('vn-%s' % self.id(), parent_obj=project)
        vn_id = self.api.virtual_network_create(vn)
    ...
    def test_associate_scoped_tag_to_a_resource(self):
        ...
        type = 'fake_type-%s' % self.id()
        value = 'fake_value-%s' % self.id()
        scoped_tag = Tag(type=type, value=value, parent_obj=project)
    ...
    def test_only_one_value_for_a_type_can_be_associate_to_a_resource(self):
        ...
        global_tag = Tag(type=type, value=global_value)
        self.api.tag_create(global_tag)
        ...
        scoped_tag = Tag(type=type, value=scoped_value, parent_obj=project)
        self.api.tag_create(scoped_tag)
    ...
    def test_unset_tag_from_a_resource(self):
        ...
        type = 'fake_type-%s' % self.id()
        value = 'fake_value-%s' % self.id()
        tag = Tag(type=type, value=value, parent_obj=project)
        self.api.tag_create(tag)

        self.api.set_tag(vn, type, value)
        for system_type in constants.TagTypeNameToId:
            tmp_tag = Tag(type=system_type, value=value)
            self.api.tag_create(tmp_tag)
        ...
        self.api.unset_tags(vn, types=[type])
    ...
    def test_resource_exists_before_disassociate_tag(self):
        ...
        with ExpectedException(exceptions.BadRequest):
            self.api.unset_tags(vn, ['fake_type-%s' % self.id()])
    ...
    def test_set_unset_multi_value_of_authorized_type_on_one_resource(self):
        ...
        label_tag1 = Tag(type=type, value=value1, parent_obj=project)
        self.api.tag_create(label_tag1)
        value2 = '%s-label2' % self.id()
        label_tag2 = Tag(type=type, value=value2, parent_obj=project)
        self.api.tag_create(label_tag2)
        value3 = '%s-label3' % self.id()
        label_tag3 = Tag(type=type, value=value3, parent_obj=project)
