...
class TestLogicalRouter(test_case.ApiServerTestCase):
    ...
    def test_same_network_not_attached_to_lr(self):
        ...
        # Add Router External Interface
        lr.add_virtual_network(net_obj)
        logger.debug("Trying adding same network of interafce as external network to LR object")
        with ExpectedException(cfgm_common.exceptions.PermissionDenied) as e:
            self._vnc_lib.logical_router_update(lr)
        logger.debug("Adding same network of interafce as external network to LR object failed as expected")
