<<<<<<< HEAD   (97725c Path NextHop can be NULL, added NULL check for the same.)
=======
#
...
class TestCrud(test_case.ApiServerTestCase):
    ...
    def test_show_allowed_address_pair_with_leading_spaces(self):
       ...
       self._vnc_lib.virtual_machine_interface_create(vmi)
       # read vmi
       ok, vmi_list = self._api_server._db_conn._cassandra_db.object_read(
       ...
       vmi_aap['allowed_address_pair'][0]['ip']['ip_prefix'] = (
           expected_address)
       self._api_server._db_conn._cassandra_db.object_update(
           'virtual-machine-interface', vmi.uuid, vmi_dict)

       # reading at DB to ensure DB update was successful
       ok, vmi_list2 = self._api_server._db_conn._cassandra_db.object_read(
           'virtual-machine-interface', [vmi.uuid])
...
>>>>>>> CHANGE (3d32b4 Fix for displaying IP addresses with leading spaces)
