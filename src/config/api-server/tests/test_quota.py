...
class TestQuota(test_case.ApiServerTestCase):
    ...
    def test_create_vmi_with_quota_in_parallels(self):
        ...
        #cleanup
        for i in xrange(self._port_quota):
            self._vnc_lib.virtual_machine_interface_delete(id=vmi[i].uuid)
    ...
    def test_create_fip_with_quota_in_parallels(self):
        ...
        #cleanup
        for i in xrange(self._fip_quota):
            self._vnc_lib.floating_ip_delete(id=fip[i].uuid)
