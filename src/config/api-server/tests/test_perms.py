...
class TestPerms():
    def parse_args(self):
        ...
        parser = argparse.ArgumentParser(description = "Test API server resource permissions")
        parser.add_argument('api_server_ip', help = "IP address of API server")
        parser.add_argument('--api-server-port',  type=int, 
            default = 8082, help = "API server port (default 8082)")

        self.args = parser.parse_args()
        print 'API server = %s : %d' %(self.args.api_server_ip, self.args.api_server_port)
    #end parse_args
...
def keystone_create_users(user_list):
    ...
    kc = client.Client(username='admin', password='contrail123', 
	    tenant_name='admin', auth_url='http://127.0.0.1:5000/v2.0')
...
def all (ip='127.0.0.1', port=8082, domain_name='my-domain',
    ...
    keystone_create_users([('alice', 'alice123', 'staff'), ('bob', 'bob123', 'staff')])

    # user=admin, role=admin
    vnc_lib = VncApi(username='admin', password='contrail123', tenant_name='admin', 
	api_server_host = ip, api_server_port = port)

    # user=bob, role=staff
    vnc_lib_bob = VncApi(username='bob', password='bob123', tenant_name='admin', 
	api_server_host = ip, api_server_port = port)

    # user=alice, role=staff
    vnc_lib_alice = VncApi(username='alice', password='alice123', tenant_name='admin', 
	api_server_host = ip, api_server_port = port)
    ...
    print
    print '############################# READ #######################################'
    print 'Reading VN as bob/staff ... should go through because of other permissions'
    ...
    try:
    	net_obj = vnc_lib_alice.virtual_network_read(fq_name = vn.get_fq_name())
    ...
    try:
        ...
        print ' -> name=%s, perms=%s' %(net_obj.get_fq_name(), print_perms(net_obj.get_id_perms()))
        testpass += 1
    except PermissionDenied as e:
