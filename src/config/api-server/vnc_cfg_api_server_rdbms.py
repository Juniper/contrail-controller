...
class VncRDBMSDbClient(VncDbClient):
    ...
    def get_shared_objects(self, obj_type, obj_uuid):
        ...
        l1 = self._object_db.get_shared(obj_type, share_id = obj_uuid, share_type = 'tenant')
        if l1:
            shared.extend(l1)
...
class VncRDBMSApiServer(vnc_cfg_api_server.VncApiServer):
    ...
    def http_resource_read(self, obj_type, id):
        ...
        # check visibility
        if (result.get('id_perms') and not result['id_perms'].get('user_visible', True) and
