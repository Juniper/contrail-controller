...
class VirtualNetworkServer(Resource, VirtualNetwork):
    ...
    @classmethod
    def _is_multi_policy_service_chain_supported(cls, obj_dict, read_result={}):
        read_result.update(obj_dict)
        if read_result.get('multi_policy_service_chains_enabled'):
            import_export_targets = read_result.get('route_target_list', {})
            import_targets = read_result.get('import_route_target_list', {})
            export_targets = read_result.get('export_route_target_list', {})
            if (import_export_targets.get('route_target', []) or
                    import_targets.get('route_target', []) and
                    export_targets.get('route_target', [])):
