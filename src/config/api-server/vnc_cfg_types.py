        ...
        if virtual_network_refs:
            ...
            if virtual_router_refs:
                ...
                if vr_refs:
                    ...
                    vrouter_uuid = vr_refs[0].get('uuid')
                    (ok, vrouter_dict) = db_conn.dbe_read(
...
class VirtualRouterServer(Resource, VirtualRouter):
        ...
        if ipam_refs != None:
            ...
            for ref in ipam_refs:
                ...
                for vr_pool in vr_pools:
                    pool_name = vr_pool['start'] + '/' + vr_pool['end']
    ...
    def _check_vr_alloc_pool_delete(self, pool_set, vr_dict, db_conn):
        ...
        for iip in iip_list:
            ...
            inst_ip_addr = int(IPAddress(iip_addr))
            for alloc_pool in pool_set:
                alloc_pool = alloc_pool.split('/')
                pool_start = int(IPAddress(alloc_pool[0]))
                pool_end = int(IPAddress(alloc_pool[1]))
                if inst_ip_addr in IPRange(pool_start, pool_end):
                    return (False,
