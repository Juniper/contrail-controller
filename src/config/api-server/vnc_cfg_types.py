...
class VirtualNetworkServer(Resource, VirtualNetwork):
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        for backref_obj_type in backref_fields:
            backref_res_type = backref_field_types[backref_obj_type][0]
            def drop_ref(obj_uuid):
                # drop ref from ref_uuid to ri_uuid
                cls.server.internal_request_ref_update(
                    backref_res_type, obj_uuid, 'DELETE', 'routing-instance',
                    ri_uuid)
            # end drop_ref
            for backref in ri_obj_dict.get(backref_obj_type, []):
                drop_ref(backref['uuid'])

        children_field_types = RoutingInstance.children_field_types
        for child_obj_type in children_fields:
            child_res_type = children_field_types[child_obj_type][0]
            for child in ri_obj_dict.get(child_obj_type, []):
