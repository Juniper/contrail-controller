...
class LogicalRouterServer(Resource, LogicalRouter):
    ...
    def check_for_configured_vni(cls, db_conn, obj_dict):
        ok, global_vrouter_config = db_conn.dbe_list('global_vrouter_config', 
                                                    is_detail=True)
        if not ok:
    ...
    def post_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        if vxlan_routing == True:
            vn_int_name = LogicalRouterServer.get_lr_internal_vn_name(
                                             obj_dict.get('uuid'))
            proj_obj = Project(name=proj_dict['display_name'],
                               kwargs_parent_type='domain',
                               kwargs_fq_name=proj_dict['fq_name'])
            ...
            ok, gvc_dict = db_conn.dbe_list('global_vrouter_config',
                                            is_detail=True)
            if not ok:
                return ok, gvc_dict
