...
class TagTypeServer(Resource, TagType):
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        if obj_dict.get('id') is not None:
        ...
        obj_dict['id'] = "0x{:04x}".format(type_id)
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        if obj_dict.get('display_name') or obj_dict.get('id'):
            msg = "Tag value or ID cannot be updated"
            return False, (400, msg)
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        cls.vnc_zk_client.free_tag_type_id(int(obj_dict['id'], 0))

    @classmethod
    def dbe_delete_notification(cls, obj_id, obj_dict):
        # Deallocate in memory tag-type ID
        cls.vnc_zk_client.free_tag_type_id(int(obj_dict['id'], 0), notify=True)

    @classmethod
    def get_all_tag_types(cls):
        # NOTE(ethuleau): could it be more efficient to use zk to obtain that
        #                 list?
    ...
    def get_tag_type_id(cls, type_str):
        ...
        return True, int(tag_type['id'], 0)


class TagServer(Resource, Tag):
    @classmethod
    def pre_dbe_alloc(cls, obj_type, obj_dict):
        type_str = obj_dict.get('type')
        value_str = obj_dict.get('value')
        ...
        obj_dict['type'] = type_str
        obj_dict['value'] = value_str

        return True, ''

    @classmethod
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        type_str = obj_dict['type']
        value_str = obj_dict['value']
        ...
        # As a Tag can be own by the config-root or a project, a tag type/value
        # can be create with different fq_name. Check Tag not already exists
        # TODO(ethuleau): Fails to filter tag list with 'name' resource prop
        ok, result, _ = cls.db_conn.dbe_list(
            'tag', filters={'display_name': [obj_dict['name']]})
        if not ok:
            return False, result
        existing_tags = result
        if len(existing_tags) >= 1:
            if obj_dict.get('parent_type') == 'project':
                msg = ("Global tag '%s' already exists with UUID: %s" %
                       (obj_dict['name'], existing_tags[0]['uuid']))
            else:
                msg = ("Project scoped tag '%s' already exists on project %s "
                       " with UUID: %s" %
                       (obj_dict['name'],
                        ':'.join(existing_tags[0]['fq_name'][:-1]),
                        existing_tags[0]['uuid']))
            return False, (400, msg)
        ...
        obj_dict['id'] = "{}{:04x}".format(tag_type['id'], value_id)

        return True, ""

    @classmethod
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        # User can't update display_name, type, value or id once created
        if (obj_dict.get('display_name') is not None or
                obj_dict.get('type') is not None or
                obj_dict.get('value') is not None or
                obj_dict.get('id') is not None):
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        if obj_dict['type'] in constants.TagTypeNameToId:
            return True, ''

        # Deallocate ID for tag value
        value_id = int(obj_dict['id'], 0) & 0x0000ffff
        cls.vnc_zk_client.free_tag_value_id(obj_dict['type'], value_id)

        # Try to delete referenced tag-type and ignore RefExistError which
        # means it's still in use
        if 'tag_type_refs' in obj_dict and obj_dict['tag_type_refs']:
...
class FirewallRuleServer(Resource, FirewallRule):
    ...
    def _frs_fix_endpoint_tag(cls, obj_dict, ep, db_conn):
        ...
        for tag_name in tag_set:
            ...
            ep['tag_ids'].append(int(tag_dict['id'], 0))
