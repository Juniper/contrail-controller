...
from vnc_cfg_ifmap import VncIfmapDb
...
class VncDbClient(object):
    def __init__(self, api_svr_mgr, cass_srv_list,
    ...
    def _dbe_resync(self, obj_type, obj_uuids):
        ...
        for obj_dict in obj_dicts:
            try:
                ...
                (ok, result) = VncIfmapDb.object_alloc(
    ...
    def dbe_alloc(self, obj_type, obj_dict, uuid_requested=None):
        ...
        (ok, result) = VncIfmapDb.object_alloc(obj_class, parent_res_type,
                                               obj_dict['fq_name'])
        if not ok:
