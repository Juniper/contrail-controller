...
class VncServerKombuClient(VncKombuClient):
    ...
    def _dbe_update_notification(self, obj_info):
        ...
        from pprint import pformat
        print(Back.BLUE+pformat(obj_info)+Style.RESET_ALL)
        obj_type = obj_info['type']
        obj_uuid = obj_info['uuid']
...
class VncZkClient(object):
    ...
    def subnet_to_vn(subnet_fq_name):
        ...
        return ':'.join(subnet_fq_name.split(':', 3)[:-1])
    # end subnet_to_vn
    ...
    def delete_subnet_allocator(self, obj_name, notified=True, rm_all=False):
        obj_fq_name = self.subnet_to_vn(obj_name)

        try:
            if rm_all:
                if notified:
                    del self._subnet_allocators[obj_name]
                else:
                    for key in self._subnet_allocators[obj_name]:
                       IndexAllocator.delete_all(self._zk_client,
                                      self._subnet_path+'/'+key+'/')
            else:
                if notified:
                    self._subnet_allocators[obj_fq_name].pop(obj_name, None)
                else:
                    IndexAllocator.delete_all(self._zk_client,
                                   self._subnet_path+'/'+obj_name+'/')
        except KeyError:
            pass
...
class VncDbClient(object):
    ...
    def subnet_delete_allocator(self, subnet, notified=True, rm_all=False):
        return self._zk_db.delete_subnet_allocator(subnet, notified, rm_all)
