                ...
                for each_entity in search_list:
                    ...
                    for each_result in result_items:
                        ...
                        if len(metadata) != 0:
                            ...
                            if len(perms) != 0:
                                continue
                        raise Exception("%s not found in IFMAP" % each_entity)

                # If we had unpublised the IFMAP server to discovery server earlier
                # publish it back now since it is valid now.
                if not self._disc_published:
                    self._get_api_server().publish_ifmap_to_discovery()
                    self._disc_published = True
