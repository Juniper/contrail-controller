...
from cfgm_common.imid import escape, entity_is_present,\
    get_ifmap_id_from_fq_name, get_fq_name_from_ifmap_id, ifmap_wipe
...
class VncIfmapClient(object):
    ...
    def _init_conn(self):
        ...
        while not connected:
            try:
                ...
                msg = 'Failed to establish IF-MAP connection'
            ...
            if not err_codes:
                ...
                self.config_log(msg, level=SandeshLevel.SYS_WARN)
                endreq = EndSessionRequest(self._mapclient.get_session_id())
                self._mapclient.call('endSession', endreq)
                time.sleep(3)
    ...
    def _publish_to_ifmap_enqueue(self, oper, oper_body, do_trace=True):
        # If ifmap connection is not up and running, don't try to push request
        # in queue.
        ...
        if not oper_body:
            return
        try:
            self._queue.put((oper, oper_body, do_trace), timeout=1)
        except Full:
            if self._conn_state != ConnectionStatus.UP or self._syncing:
                return
            self._conn_state = ConnectionStatus.DOWN
            self._ifmap_queue_full()
    # end _publish_to_ifmap_enqueue

    def _ifmap_queue_full(self):
        # Set ifmap server connection to down and when it comes up again
        # reset and repopulate it
        msg = ("Connection to IF-MAP down. Publish queue is full "
               "(queue size: %d)." %
               self._get_api_server()._args.ifmap_queue_size)
        self.config_log(msg, level=SandeshLevel.SYS_ERR)
        ConnectionState.update(
            conn_type=ConnType.IFMAP, name='IfMap',
            status=ConnectionStatus.DOWN, message=msg,
            server_addrs=["%s:%s" % (self._ifmap_srv_ip,
                                     self._ifmap_srv_port)])

        self.reset()
        self._get_api_server().publish_ifmap_to_discovery('down', msg)
        self._is_ifmap_up = False
        self._conn_state = ConnectionStatus.INIT
        self._syncing = True
    ...
    def _publish_to_ifmap_dequeue(self):
        def _publish(requests, traces, publish_discovery=False):
            ...
            while not ok:
                # when connection was setted down, the sending queue was
                # exceeded => abort to send actual requests
                ...
                if not ok:
                    msg = ("Cannot publish message %s. "
                           "IF-MAP queue size: %d/%d" %
    ...
    def _health_checker(self):
        while True:
            try:
                ...
                if self._conn_state == ConnectionStatus.DOWN and self._syncing:
                    continue
