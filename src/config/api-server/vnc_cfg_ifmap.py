    ...
    def build_shared_index(oper):
        def wrapper1(func):
            def wrapper2(self, obj_type, obj_ids, obj_dict):
                ...
                # don't build sharing indexes for operation (create/update) failed
                ...
                if cur_perms2['global_access'] != global_access:
                    if global_access:
                        self._cassandra_db.set_shared(obj_type, obj_uuid, '*', share_type = '*', 
                            rwx = global_access)
                    else:
                        self._cassandra_db.del_shared(obj_type, obj_uuid, '*', share_type = '*')
