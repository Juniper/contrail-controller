...
class VncIfmapClient(object):
    ...
    def _object_set(self, res_type, my_imid, existing_metas, obj_dict):
        ...
        for prop_field in obj_class.prop_fields:
            ...
            if is_simple:
                ...
                prop_cls = self._db_client_mgr.str_to_class(prop_type)
        ...
        for ref_field in obj_class.ref_fields:
            ...
            for ref in refs:
                ...
                if ref_data:
                    buf = cStringIO.StringIO()
                    attr_cls = self._db_client_mgr.str_to_class(ref_link_type)
    ...
    def _object_read_to_meta_index(self, res_type, ifmap_id, field_names=None):
        obj_type = res_type.replace('-', '_')
        ...
        all_metas = obj_cls.prop_field_metas.values() + \
                    obj_cls.ref_field_metas.values() + \
                    obj_cls.children_field_metas.values()
        if not field_names:
...
class VncDbClient(object):
    ...
    def str_to_class(self, obj_type):
        return self._api_svr_mgr.str_to_class(obj_type)
    # end str_to_class
