...
class QuotaHelper(object):
    ...
    def get_security_group_rule_count(cls, db_conn, proj_uuid):
        ...
        (ok, res_list) = db_conn.dbe_list(
                'security_group', parent_uuids=[proj_uuid])
        if not ok:
    ...
    def get_floating_ip_pool_count(cls, db_conn, proj_uuid):
        ...
        (ok, vn_list) = db_conn.dbe_list(
               'virtual_network', parent_uuids=[proj_uuid])
        if not ok:
            raise cfgm_common.exceptions.NoIdError

        quota_count = 0
        for vn_res in vn_list:
            (ok, fip_pool_list) = db_conn.dbe_list(
                    'floating_ip_pool', parent_uuids=[vn_res[1]], count=True)
            if not ok:
                raise cfgm_common.exceptions.NoIdError
