...
class EventDispatcher(object):
    ...
    def __init__(self, spawn_dispatch_greenlet=False):
        if spawn_dispatch_greenlet:
            self._dispatch_greenlet = vnc_greenlets.VncGreenlet("Event Dispatcher", self.dispatch)
    # end __init__
    ...
    def initialize(self, resource_type):
        return [self.pack(event="init", data=resource) for resource in self._db_conn.dbe_list(resource_type)]
    # end initialize
