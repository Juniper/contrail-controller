...
class GlobalSystemConfigServer(Resource, GlobalSystemConfig):
    ...
    @classmethod
    def _find_dci_ipam(cls, fq_name, ipam_refs):
        if not ipam_refs:
            return None
    ...
    def _create_dci_lo0_network_ipam(cls, db_conn, subnet_list):
        ...
        if ipam_ref:
            ok, ipam_obj = cls.dbe_read(db_conn, obj_type, ipam_ref.get("uuid"))
            if not ok:
                return ok, ipam_obj
        ...
        # create ipam object for the first ttime
        if not ipam_obj:
    ...
    def post_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        status, err = cls._create_dci_lo0_network_ipam(db_conn,
