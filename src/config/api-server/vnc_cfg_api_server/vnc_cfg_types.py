...
class LogicalRouterServer(Resource, LogicalRouter):
    ...
    def _revert_vxlan_id_update_for_lr(cls, obj_dict, read_result):
        ...
        #Don't need to check for difference between old and new vxlan id
        #because this function is added to undo stack only if they were different
        #in the first place. 
        ...
        #Second, check if new_vxlan_id is not None, if so, it means that
        # it's updated in the Zookeeper. Delete it from Zookeeper
        if(new_vxlan_id != None):
...
class VirtualNetworkServer(Resource, VirtualNetwork):
    ...
    @classmethod
    def _revert_vxlan_id_update(cls, obj_dict, fq_name, read_result):
        new_vxlan_id = None
        old_vxlan_id = None
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        if(new_vxlan_status and new_vxlan_id != old_vxlan_id ):
            ...
            if(new_vxlan_id != None):
                ...
                def undo_vxlan_id():
                    cls._revert_vxlan_id_update(obj_dict, fq_name, read_result)
                # end undo
                get_context().push_undo(undo_vxlan_id)
