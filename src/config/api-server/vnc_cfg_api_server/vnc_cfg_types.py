...
class SecurityResourceMixin(object):
    ...
    def _get_dedicated_draft_policy_management(cls, obj_dict):
        ...
        if parent_type == PolicyManagementServer.resource_type:
            if (obj_dict.get('fq_name')[-2] ==
                ...
                # Already own by the dedicated draft policy management, probably
                # an internal draft security resource creation, ignore it
                return True, ''
    ...
    @classmethod
    def pending_update(cls, obj_dict, delta_obj_dict={}):
        ...
        already_exists = True
        try:
            cls.db_conn.fq_name_to_uuid(cls.object_type, obj_dict['fq_name'])
        except cfgm_common.exceptions.NoIdError as e:
        ...
        # pending resource is pending create and now deleting, we can purge
        # cloned resource
        if not already_exists and not delta_obj_dict:
            ...
            return False, (400, msg)
        elif not already_exists and delta_obj_dict.get('pending_delete', False):
            cls.server.internal_request_delete(cls.resource_type,
                                               pending_obj_dict['uuid'])
        elif pending_obj_dict.get('pending_delete'):
...
class ServiceGroupServer(Resource, ServiceGroup, SecurityResourceMixin):
    @classmethod
    def pre_dbe_alloc(cls, obj_dict):
        return cls.pending_update(obj_dict)
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        return cls.pending_update(db_obj_dict, obj_dict)

    @classmethod
    def pre_dbe_delete(cls, id, obj_dict, db_conn):
        return cls.pending_update(obj_dict, {'pending_delete': True})
