...
class PhysicalInterfaceServer(Resource, PhysicalInterface):
    ...
    def _check_interface_name(cls, obj_dict, db_conn, vlan_tag):
        ...
        if product_name.lower().startswith("qfx") and vlan_tag != None:
            reserved_l2_vlan_ids = [1, 2, 4094]
            li_type = obj_dict.get('logical_interface_type', 'l2').lower()
            if li_type !='l3' and vlan_tag in reserved_l2_vlan_ids:
                return (False, (403, "Vlan ids " + str(reserved_l2_vlan_ids) + " are not allowed on QFX"
