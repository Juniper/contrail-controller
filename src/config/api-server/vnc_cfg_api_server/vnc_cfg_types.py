...
class GlobalSystemConfigServer(Resource, GlobalSystemConfig):
    ...
    @classmethod
    def _create_dci_lo0_network_ipam(cls, db_conn, subnet_list):
        ...
        if ipam_ref:
            ...
            if not ok:
                return ok, (400, 'Could not find dci network ipam object')
    ...
    def post_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        return cls._create_dci_lo0_network_ipam(db_conn,
...
class DataCenterInterconnectServer(Resource, DataCenterInterconnect):
    ...
    def _make_sure_lrs_belongs_to_different_fabrics(cls, db_conn, dci):
        ...
        for lr_uuid in lr_list:
            ...
            if not ok:
                return False, (400, 'Could not read Logical router object: ' + lr_uuid)
            ...
            if ok:
                ...
                for pr_ref in read_result.get('physical_router_refs') or []:
                    ...
                    if not status:
                        return False, (400, 'Could not read physical router: ' + pr_uuid)
                    ...
                    if pr_result.get("fabric_refs"):
                        ...
                        return False, (400, 'Logical router is not associated to any fabric: ' + lr_uuid)
        return True, ''
    ...
    def _ensure_lr_dci_association(cls, db_conn, lr):
        ...
        for pr_ref in lr.get('physical_router_refs') or []:
            ...
            status, pr_result = cls.dbe_read(db_conn, 'physical_router',
                                                     pr_uuid, obj_fields=['fabric_refs'])
            if not status:
                return False, (400, 'could not read physical router object: ' + pr_uuid)
            if pr_result.get("fabric_refs"):
                fab_id = pr_result["fabric_refs"][0].get('uuid')
