...
class PortTupleServer(Resource, PortTuple):
    ...
    @classmethod
    def check_phys_intf_belongs_to_same_sa(cls, obj_dict, db_conn):
        # Validate if left and right physical interfaces have references
        # to another physical interface, which belongs to the PNF device
        # representing this service instance
        ...
        for phys_intf_ref in obj_dict.get('physical_interface_refs') or []:
             ...
             for ref in read_result.get('physical_interface_refs'):
                  ...
                  if read_result.get('parent_type') == 'physical-router':
                      ...
                      sa_uuid = cls.get_sa_from_pt(obj_dict, db_conn)
                      if sa_uuid is not None:
                          if phys_router_uuid != sa_uuid:
                              msg = ("Referenced physical interface(%s) belongs to different SA" % %\
                                     (phys_intf_ref['uuid']), sa_uuid)
                              return (False, (400, msg))
    ...
    @classmethod
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ok, result = cls.check_phys_intf_belongs_same_sa(obj_dict, db_conn)
        if not ok:
            return ok, result
    ...
    @classmethod
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ok, result = cls.check_phys_intf_belongs_to_same_sa(obj_dict, db_conn)
        if not ok:
            return ok, result
