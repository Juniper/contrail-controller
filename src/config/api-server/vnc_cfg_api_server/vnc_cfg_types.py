...
class LogicalRouterServer(Resource, LogicalRouter):
    ...
    @classmethod
    def add_snat_instance(cls, db_conn, obj_dict, obj_id=None):
        if obj_id is not None:
            ok, db_obj_dict = db_conn.dbe_read('logical_router', obj_id)
            if not ok:
                return False, (400, 'Logical router not found')
    ...
    @classmethod
    def delete_snat_instance(cls, db_conn, obj_dict, del_refs=True):
        api_server = db_conn.get_api_server()
        ...
        if rt_ref:
            ...
            api_server.internal_request_delete('route-table', rt_ref['uuid'])
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        if obj_dict.get('virtual_network_refs'):
            return cls.add_snat_instance(db_conn, obj_dict, obj_id=id)
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        if obj_dict.get('service_instance_refs'):
            return cls.delete_snat_instance(db_conn, obj_dict, del_refs=False)
        return True, ''
    # end post_dbe_delete
