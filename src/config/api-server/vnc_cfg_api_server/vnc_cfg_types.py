...
class EndSystemServer(Resource, EndSystem):
    ...
    def remove_port_group_ref(db_conn, port_uuid, port_fq_name):
        api_server = db_conn.get_api_server()
        db_conn.config_log("Removing port-group refs", level=SandeshLevel.SYS_DEBUG)
        ...
        for pg in range(len(results.get('port_group_back_refs'))):
            ...
            pg_fq_name = results['port_group_back_refs'][pg]['to']
    ...
    def process_node_profile(cls, id, fq_name, obj_dict, db_conn):
        ...
        if obj_dict.get('node_profile_refs'):
            ...
            db_conn.config_log("process NP port-groups " + pformat(port_groups), level=SandeshLevel.SYS_DEBUG)
            end_system_fq_name = db_conn.uuid_to_fq_name(obj_dict['uuid'])
            db_conn.config_log("process NP port-groups " + str(end_system_fq_name), level=SandeshLevel.SYS_DEBUG)
            db_conn.config_log("process NP port-groups {0}".format(type(end_system_fq_name)), level=SandeshLevel.SYS_DEBUG)
            for pg in port_groups:
                create_port_group = False
                pg_fq_name = end_system_fq_name + [pg]
                db_conn.config_log("process NP port-groups 1 " + str(pg_fq_name), level=SandeshLevel.SYS_DEBUG)
                try:
                    pg_uuid = cls.db_conn.fq_name_to_uuid('port-group', pg_fq_name)
                except cfgm_common.exceptions.NoIdError as e:
                ...
                if create_port_group:
                    ...
                    ok, port_group_obj = api_server.internal_request_create('port-group',
                                                        json.loads(pg_dict))
                    if not ok:
                        db_conn.config_log("port-group creation failed", level=SandeshLevel.SYS_DEBUG)

                    pg_uuid = port_group_obj['port-group'].get('uuid')
                    msg = ("port-group-obj : %s", pformat(port_group_obj))
                    db_conn.config_log(str(msg), level=SandeshLevel.SYS_DEBUG)
                    db_conn.config_log(str(pg_fq_name), level=SandeshLevel.SYS_DEBUG)

                for interface in port_groups[pg]:
                    port_fq_name = end_system_fq_name + [ interface ]
                    port_uuid = cls.db_conn.fq_name_to_uuid('port', port_fq_name)
                    msg = ("ref_create for %s, UUID %s, fq_name %s", interface, port_uuid, port_fq_name)
                    db_conn.config_log(str(msg), level=SandeshLevel.SYS_DEBUG)
                    #cls.remove_port_group_ref(db_conn, port_uuid, port_fq_name)
                    ok, result = api_server.internal_request_ref_update('port-group', pg_uuid, 'ADD',
                                      'port', port_uuid, port_fq_name)
                    msg = ("ref-update status ", str(ok), ": result ", pformat(result))
                    db_conn.config_log(str(msg), level=SandeshLevel.SYS_DEBUG)
