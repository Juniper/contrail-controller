...
class SecurityResourceBase(Resource):
    ...
    def _pending_update(cls, obj_dict, delta_obj_dict={}):
        ...
        if (obj_dict['fq_name'][-2] ==
                constants.POLICY_MANAGEMENT_NAME_FOR_SECURITY_DRAFT):
            orig_fq_name = list(obj_dict['fq_name'])
            orig_fq_name.pop(-2)
            if len(orig_fq_name) == 1:
                # In global case the origin owner is not the grant parent of the
                # pending resource
                orig_fq_name.insert(
                    0, 'default-%s' % PolicyManagementServer.resource_type)
        else:
            orig_fq_name = obj_dict['fq_name']
        try:
            cls.db_conn.fq_name_to_uuid(cls.object_type, orig_fq_name)
        except cfgm_common.exceptions.NoIdError as e:
            already_exists = False

        # check if pending resource already exists
        if new_parent_dict['fq_name'] != obj_dict['fq_name'][:-1]:
            ...
            if not ok and result[0] == 404:
                ...
                ok, result = cls.locate(fq_name=new_fq_name, **obj_dict)
                if not ok:
                    return False, result
                return True, (202, result)
            elif not ok:
                return False, result

            # A pending resource was already there
            pending_obj_dict = result
        else:
            # Updating a pending resource
            pending_obj_dict = obj_dict

        # Pending creation (it does not yet exist/enforce)
        if not already_exists:
            # Trying to re-create it (no updates), forbidden
            if not delta_obj_dict:
                msg = ("%s named %s was already created (%s) and it is pending "
                       "to be committed or reverted. You cannot create it "
                       "again" %  (cls.object_type.replace('_', ' ').title(),
                                   ':'.join(pending_obj_dict['fq_name']),
                                   pending_obj_dict['uuid']))
                return False, (400, msg)
            # Abort pending creation
            elif delta_obj_dict.get('pending_delete', False):
                # Let classic delete operated
                return True, ''
        # Pending update (an original version of the resource is enforced)
        else:
            # Resource already in pending delete
            if pending_obj_dict.get('pending_delete', False):
                # Try to pending delete a resource already in pending deleted
                # nothing to do
                if delta_obj_dict.get('pending_delete', False):
                    return True, (202, pending_obj_dict)
        ...
        # Finally, we can update the pending resource
        try:
...
class ApplicationPolicySetServer(SecurityResourceBase, ApplicationPolicySet):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        return cls.check_associated_firewall_resource_in_same_scope(id, fq_name,
            obj_dict, FirewallPolicyServer.object_type)
