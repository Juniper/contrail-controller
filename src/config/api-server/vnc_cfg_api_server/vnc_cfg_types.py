                    ...
                    if scope == GlobalSystemConfigServer.resource_type:
                        ...
                        msg = ("Cannot disable security draft mode on scope "
                               ...
                               "need to be reviewed:\n" % (
                                    ...
                                    parent_uuid))
                    childs_types = set(['%ss' % type for type in
                                        constants.SECURITY_OBJECT_TYPES])
                    if not childs_types & set(draft_pm.keys()):
                        return False, (e.status_code, e.content)
                    for type in childs_types:
                        if draft_pm.get('%ss' % type, []):
                            msg += "\t- %s: " % type.replace('_', ' ').title()
                            for child in draft_pm.get('%ss' % type, []):
                                msg += "%s (%s), " % (':'.join(child['to']),
                                                      child['uuid'])
                            if msg.endswith(', '):
                                msg = msg[:-2]
                            msg += '\n'
