...
class VncApiServer(object):
    ...
    def http_resource_read(self, obj_type, id):
        ...
        if 'draft' in get_request().query:
            draft_version = 'true' in get_request().query.draft.lower()
        else:
            draft_version = False
        ...
        if draft_version:
            no_draft_msg = ("%s %s (%s) does not have draft version" %
                            (obj_type.replace('_', ' ').title(),
                             ':'.join(fq_name), id))
            if hasattr(r_class, 'get_pending_resource'):
                # Look if the resource have a pending version
                try:
                    ok, result = db_conn.dbe_read(
                        obj_type, id, ['parent_type', 'parent_uuid'])
                except NoIdError as e:
                    # Not present in DB
                    raise cfgm_common.exceptions.HttpError(404, str(e))
                if not ok:
                ...
                obj_dict = result
                ok, result = r_class.get_pending_resource(obj_dict, [])
                if ok and isinstance(result, dict):
                    obj_dict = result
                    id = obj_dict['uuid']
                    fq_name = obj_dict['fq_name']
                elif not ok and result[0] != 404:
                    raise cfgm_common.exceptions.HttpError(404, no_draft_msg)
                elif not ok:
                    self.config_object_error(id, None, obj_type, 'http_get',
                                                result[1])
                    raise cfgm_common.exceptions.HttpError(result[0], result[1])
    ...
    def __init__(self, args_str=None):
        ...
        self.route('/security-policy-draft', 'POST', self.security_policy_draft)
        self._homepage_links.append(
   ...
   # end
    ...
    def security_policy_draft(self):
        ...
        try:
            ...
            msg = ("Cannot find scope where pending security resource are own: "
                   "%s" % str(e))
        ...
        ok, result = pm_class.locate(fq_name=pm_fq_name, create_it=False,
                                    fields=fields)
        if not ok and result[0] == 404:
        ...
        scope_lock = self._db_conn._zk_db._zk_client.lock(
            '%s/%s/%s' % (
                self.security_lock_prefix, scope_type, ':'.join(scope_fq_name)),
            'api-server-%s %s' % (socket.gethostname(), action),
        )
        try:
            scope_lock.acquire(timeout=0.1)
                    ...
                    if action == 'revert':
                        ...
                        try:
                            uuid = self._db_conn.fq_name_to_uuid(
                                r_class.object_type, fq_name)
                        except NoIdError:
                            # No original version found, new resource created
                            uuid = None
                        # Purge pending resource
                        self.internal_request_delete(r_class.object_type,
                                                    child['uuid'])
                        if uuid:
                            if draft_obj.get('pending_delete', False):
                                # The resource is removed, we can purge original
                                # resource
                                self.internal_request_delete(r_class.object_type,
                                                            uuid)
                            else:
                                # Update orginal resource with pending resource
                                draft_obj.pop('fq_name', None)
                                draft_obj.pop('uuid', None)
                                if 'id_perms' in draft_obj:
                                    draft_obj['id_perms'].pop('uuid', None)
                                draft_obj['parent_type'] = scope_class.resource_type
                                draft_obj['parent_uuid'] = scope_uuid
                                self.internal_request_update(r_class.resource_type,
                                                            uuid, draft_obj)
                        else:
                            # Create new resource with pending value and re-use the
                            # UUID of the deleted pending resource
                            draft_obj.pop('id_perms', None)
                            draft_obj.pop('perms2', None)
                            draft_obj['fq_name'] = fq_name
                            draft_obj['parent_type'] = scope_class.resource_type
                            draft_obj['parent_uuid'] = scope_uuid
                            self.internal_request_create(r_class.resource_type,
                                                        draft_obj)
            # Remove draft policy mode on the scope
            self.internal_request_update(
                scope_class.resource_type,
                scope_uuid,
                {'enable_security_policy_draft': False},
            )
        except kazoo_exceptions.LockTimeout:
            action_in_progress = scope_lock.contenders()
            if len(action_in_progress) > 0 and action_in_progress[0]:
                _, _, action_in_progress = action_in_progress[0].partition(' ')
