   ...
   # end
    ...
    def _get_network(fqn):
        if isinstance(fqn, list) and len(fqn) == 3:
            return '{"domain":"%s", "project":"%s", "name":"%s"}' % (fqn[0], fqn[1], fqn[2])
        return ''

    # hbs network template
    def _get_hbs_network(self, vn_uuid, vn_k8s_str, namespace):
        hbs_nw = {
        ...
        vn_class = self.get_resource_class('virtual_network')
        ok, result = vn_class.locate(uuid=vn_uuid,
                                     fields=['network_ipam_refs'],
                                     create_it=False)
        if not ok:
            raise cfgm_common.exceptions.HttpError(result[0], result[1])
        network_ipam_refs = result.get('network_ipam_refs', [])
        vn_fq_name = result['fq_name']
        hbs_nw['metadata']['annotations']['opencontrail.org/network'] = self._get_network(vn_fq_name)
    ...
    def _get_hbs_ds(self, hbs_info):
            ...
            'spec': {'selector': {'matchLabels': {'name': 'hbf'}},
                     'template':
                         {'metadata': {'annotations': {'k8s.v1.cni.cncf.io/networks':
                                       ...
                                       'labels': {'name': 'hbf'}},
                          'spec': {'containers': [{'env': [{'name': 'CSRX_FORWARD_MODE',
                                                            'value': 'wire'}],
        ...
        if hbs_info['vnmgmt']['fq_name']:
            ds_template['metadata']['annotations']['k8s.v1.cni.cncf.io/networks'] = \
                '[{"name":"left"}, {"name":"right"},{"name":"management"}]'
        ds_template_spec = ds_template['spec']
        ds_template_spec['containers'][0]['image'] = hbs_info['service']['image']
        ds_template_spec['imagePullSecrets'][0]['name'] = hbs_info['service']['imagePullSecrets']
        return hbs_ds_template

    # end _get_hbs_ds

    # hbs object daemon set generation 
    def _get_hbs_info(self, hbs_fq_name, hbs_uuid):
        hbs_info = {
        ...
        # get hbs object
        hbs_class = self.get_resource_class('host_based_service')
        ok, result = hbs_class.locate(fq_name=hbs_fq_name,
                                      uuid=hbs_uuid,
                                      create_it=False)
        if not ok:
            raise cfgm_common.exceptions.HttpError(result[0], result[1])
        ...
        project_uuid = hbs_obj['parent_uuid']
        project_class = self.get_resource_class('project')
        ok, result = project_class.locate(uuid=project_uuid,
                                          create_it=False)
        if not ok:
            raise cfgm_common.exceptions.HttpError(result[0], result[1])
        project_obj = result
        project_fq_name = project_obj['fq_name']

        # Get namespace information from project annotations
        kvs = project_obj.get('annotations', {}).get('key_value_pair', [])
        namespace = None
        cluster = None
        for kv in kvs:
            if kv['key'] == 'namespace': namespace = kv['value']
            if kv['key'] == 'cluster': cluster = kv['value']
        if not namespace:
            raise cfgm_common.exceptions.HttpError(
        ...
        vn_refs = hbs_obj.get('virtual_network_refs', [])
        vn_left = vn_right = None
        for vn in vn_refs:
            attr = vn.get('attr', {})
        ...
        kvs = hbs_obj.get('annotations', {}).get('key_value_pair', [])
        for kv in kvs:
    ...
    def hbs_get(self):
        ...
        # get hbs info
        hbs_info = self._get_hbs_info(hbs_fq_name, hbs_uuid)
        namespace_template = self._get_hbs_namespace(hbs_info['namespace'])
        ds_template = self._get_hbs_ds(hbs_info)
        left_vn_template = self._get_hbs_network(hbs_info['vnleft']['uuid'],
                                                 'left',
                                                 hbs_info['namespace'])
        right_vn_template = self._get_hbs_network(hbs_info['vnright']['uuid'],
                                                  'right',
                                                  hbs_info['namespace'])
        hbs = {'hbs': [namespace_template, left_vn_template, right_vn_template]}
        if hbs_info['vnmgmt']['fq_name']:
            mgmt_vn_template = self._get_hbs_network(hbs_info['vnmgmt']['uuid'],
                                                     'management',
                                                     hbs_info['namespace'])
            hbs['hbs'].append(mgmt_vn_template)
        hbs['hbs'].append(ds_template)
        return hbs
