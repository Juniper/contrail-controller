...
class VncApiServer(object):
    ...
    def job_mgr_signal_handler(self, signalnum, frame):
        try:
            ...
            status,prouter_info = self._extracted_file_output(signal_var.get(
                'exec_id'))

            for exec_id in self.job_status:
                if exec_id.has_key(signal_var.get('exec_id')):
                    exec_id[signal_var.get('exec_id')] = status
    ...
    def job_status_http_get(self):
        execution_id = (get_request().json).get('execution_id')

        for each_job in self.job_status:
                if each_job.has_key(execution_id):
                    return each_job.get(execution_id)
    ...
    def __init__(self, args_str=None):
        ...
        # job status for every job for DM
        self.job_status = []
