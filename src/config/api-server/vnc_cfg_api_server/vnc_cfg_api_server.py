...
class VncApiServer(object):
    ...
    def get_device_credentials_http_get(self):
        ...
        obj_fq_name = input.get('fq_name')
        obj_uuid = input.get('uuid')
        obj_type = input.get('obj_type')
        admin_password = self._auth_svc.args.admin_password
        ...
        try:
            ...
            if obj_type == 'physical_router':
                try:
                    ...
                    pass
                if result is not None and result.get('physical_router_user_credentials') is not None and result.get('physical_router_user_credentials', {}).get('password') is not None:
                    padded_text = (result.get('physical_router_user_credentials', {}).get('password')).rjust(32)
                ...
                try:
                    (ok, result) = self._db_conn.dbe_read(obj_type, obj_uuid,
                        ...
                        pass
                if result is not None and result.get('fabric_credentials') is not None:
                    for dev_cred in result['fabric_credentials']['device_credential']:
                        padded_text = (dev_cred.get('credential', {}).get('password')).rjust(32)
                        password = base64.b64encode(cipher.encrypt(padded_text))
