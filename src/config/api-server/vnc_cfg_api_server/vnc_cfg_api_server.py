            ...
            for backref_field in set(obj_dict.keys()) & backref_fields:
                ...
                for backref in copy.deepcopy(obj_dict.get(backref_field, [])):
                    ...
                    if obj_type == AddressGroup.object_type:
                        # Is not allowed to diresctly create Address Group
                        # reference to a Firewall Rule, use its endpoints
                        # address-group property
                        ok, result = self.get_resource_class(backref_type).locate(backref['to'], backref['uuid'], create_it=False, fields=['endpoint_1', 'endpoint_2'])
                        if not ok:
                        ...
                        for ep_type in ['endpoint_1', 'endpoint_2']:
                            if ep_type in fr and fr[ep_type].get('address_group').split(':') == obj_dict['fq_name']:
                                tmp = FirewallRuleEndpointType(address_group=':'.join(fq_name))
                                updates.append(('update', (FirewallRule.resource_type, fr['uuid'],
                                                {ep_type: vars(FirewallRuleEndpointType(address_group=':'.join(fq_name)))})))
                                break
                    else:
                        held_refs.append(
