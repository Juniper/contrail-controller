...
class VncApiServer(object):
    ...
    def execute_job_http_post(self):
        ...
        try:
            ...
            request_params = self.read_config_params(request_params)
    ...
    def read_config_params(self, request_params):
        ...
        try:
            ...
            raise cfgm_common.exceptions.HttpError(
                500, "Sandesh collector information required by Job Manager "
                     "missing in the API config")

        # set to a default value and check if the conf file for the updated val
        try:
            job_args['fabric_ansible_conf_file'] = \
                self._args.fabric_ansible_conf_file
        except AttributeError:
            raise cfgm_common.exceptions.HttpError(
                500, "Config param fabric_ansible_conf_file required by Job"
                     " Manager missing in the API config")
