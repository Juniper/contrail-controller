...
* 1.2:
  - Re-define the way the script recovers the route target inconsistencies.
    ...
    be re-initialized. If missing RT persists in Config or/and in
    Schema DB (foundable with command 'check_route_targets_id') that means
    zookeeper have a stale lock which needs to be manually deleted.
* 1.1:
...
exceptions = [
    ...
    'SchemaRTgtIdMissingError',
    'ConfigRTgtIdExtraError',
    'ConfigRTgtIdMissingError',
    'OrphanResourceError',
    'ZkSubnetPathInvalid',
...
class DatabaseChecker(DatabaseManager):
    ...
    def check_route_targets_id(self):
        ...
        msg = ("Missing Route Target ID %d in %s DB for '%s'. The script will "
               "not fix it but schema-transformer config service will probably "
               "do when it'll initializing. If still persists after, that "
               "means there is a lock leak in zookeeper for that Route Target "
               "ID. You can  manually remove it (%s)")
        for id, res_fq_name_str in zk_set - schema_set:
            id_str = "%(#)010d" % {'#': id}
            zk_path = '%s/%s' % (self.base_rtgt_id_zk_path, id_str)
            errmsg = msg % (id, 'Schema', res_fq_name_str, zk_path)
            ret_errors.append(SchemaRTgtIdMissingError(errmsg))

        for id, res_fq_name_str in zk_set - config_set:
            id_str = "%(#)010d" % {'#': id}
            zk_path = '%s/%s' % (self.base_rtgt_id_zk_path, id_str)
            errmsg = msg % (id, 'Config', res_fq_name_str, zk_path)
            ret_errors.append(ConfigRTgtIdMissingError(errmsg))
