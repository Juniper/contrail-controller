...
def _parse_args(args_str):
    ...
    parser.add_argument(
        "--backup", help="Take a backup before heal and clean operations and store in json backups directory." + \
        ...
        if self.using_json is True:
            ...
            if columns:
                return OrderedDict({
                            ...
                            table_to_check.keys() else json.dumps({})) for k in columns
                       })
            elif column_start:
                return OrderedDict({
                            (k if k in table_to_check.keys() else k):(table_to_check[k] if k in
                            table_to_check.keys() else json.dumps({}))for k in
                            filter(lambda k: column_start <= k <= column_finish, table_to_check.keys())
                       })
            else:
                return OrderedDict({
                            (k if k in table_to_check.keys() else k):(table_to_check[k] if k in
                            table_to_check.keys() else json.dumps({})) for k in table_to_check.keys()
                       })
                ...
                for col, _ in lr_cols.items():
                    if col.startswith('ref:service_instance:'):
                        ...
                        snat_ri_uuid = snat_ri_fq_name_uuid_str.popitem()[
                            0].rpartition(':')[-1]
                        try:
                            self.cassandra_remove(ri_uuidssnat_ri_uuid)
