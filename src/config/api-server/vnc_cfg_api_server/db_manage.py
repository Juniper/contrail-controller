...
db_json_exim_supported = True
try:
    from cfgm_common.db_json_exim import DatabaseJSONExim
except ImportError:
    db_json_exim_supported = False
...
def _parse_args(args_str):
    ...
    parser.add_argument(
        "--skip_backup", help="Don't Take a backup before heal and clean" + \
            " operations and store in json backups directory." + \
            " all backups are stored under /var/tmp/json_backups/ encoded" + \
            " with IP address and timestamp",
        action='store_true', default=False)
    parser.add_argument(
...
class DatabaseManager(object):
    ...
    def __init__(self, args='', api_args=''):
        ...
        if not db_json_exim_supported:
            self._logger.info(
                "JSON-JSON functionality is not" + \
    ...
    ''' Wrapper functions implemented to support KazooClient/Zookeeper
    Operations '''
    ...
    def cassandra_get_range(self, table, columns=None, column_count=None):
        if self.using_json is True:
            ...
            if key_list:
                ...
                row_table = {}
            row_table_keys = list(row_table.keys())
    ...
    def audit_subnet_uuid(self):
        ...
        for key, cols in self.cassandra_get_range(ua_kv_cf):
            ...
            if mch:  # subnet key -> uuid
                ...
                try:
                    reverse_map = self.cassandra_get(ua_kv_cf, subnet_id)
                ...
                try:
                    reverse_map = self.cassandra_get(ua_kv_cf, subnet_key)
                except (pycassa.NotFoundException, KeyError):
                ...
                if vn_fq_name_str:
                    ...
                    for key in ip_cols.keys():
                        if 'parent:%s' % parent_type in key:
                            ...
                            parent_cols = dict(self.cassandra_xget(obj_uuid_table, parent_id))
        ...
        for obj_uuid, _ in self.cassandra_get_range(obj_uuid_table, column_count=1):
            ...
            for col_name in cols:
                if col_name.startswith('parent:%s:' % parent_type):
                    ...
                    break
            try:
                self.cassandra_get(obj_uuid_table, parent_uuid)
            except (pycassa.NotFoundException, KeyError):
            ...
            if len(lr_uuids) > 1:
                ...
                for col, _ in list(lr_cols.items()):
                    if col.startswith('ref:service_instance:'):
                        ...
                        try:
                            si_cols = self.cassandra_get(uuid_table,
    ...
    def audit_aggregated_ethernet_id(self):
        ...
        try:
            prouters_with_ae_id = self.zk_get_children(base_path)
        except kazoo.exceptions.NoNodeError:
            prouters_with_ae_id = None

        for prouter_name in prouters_with_ae_id or []:
            prouter_path = base_path + '/' + prouter_name
            for ae_id in self.zk_get_children(prouter_path) or []:
                vpg_name = self.zk_get(prouter_path + '/' + ae_id)[0]
    ...
    def clean_stale_fq_names(self, res_type=None):
        ...
        for obj_type, _ in iterator:
            ...
            for fq_name_str_uuid, _ in self.cassandra_xget(obj_fq_name_table, obj_type):
                obj_uuid = fq_name_str_uuid.split(':')[-1]
                try:
                    self.cassandra_get(obj_uuid_table, obj_uuid)
            ...
            if stale_cols:
                if not self._args.execute:
                    ...
                    logger.info("Removing stale %s fq_names: %s", obj_type,
                                stale_cols)
                    self.cassandra_remove(obj_fq_name_table, obj_type, columns=stale_cols)
    ...
    def clean_obj_missing_mandatory_fields(self):
        ...
        for obj_uuid, _ in self.cassandra_get_range(obj_uuid_table, column_count=1):
            ...
            if not self._args.execute:
                ...
                self.cassandra_remove(obj_uuid_table, obj_uuid)
        ...
        if stale_vm_uuids:
            for vm_uuid in stale_vm_uuids:
                if not self._args.execute:
                    ...
                    self.cassandra_remove(obj_uuid_table, vm_uuid)
    ...
    def _remove_stale_from_uuid_table(self, dangle_prefix):
        ...
        for obj_uuid, _ in self.cassandra_get_range(obj_uuid_table, column_count=1):
            ...
            for col_name in cols:
                ...
                try:
                    self.cassandra_get(obj_uuid_table, dangle_check_uuid)
                except (KeyError, pycassa.NotFoundException):
        ...
        for uuid in uuids:
            ...
            for ref in mandatory_refs:
                ...
                for col in cols:
                    ...
                    try:
                        self.cassandra_get(obj_uuid_table, ref_uuid)
                    except (KeyError, pycassa.NotFoundException):
                        continue
                    break
                else:
...
class DatabaseHealer(DatabaseManager):
    ...
    def heal_children_index(self):
        ...
        for obj_uuid, cols in self.cassandra_get_range(obj_uuid_table, column_count=1):
            ...
            try:
                _ = self.cassandra_get(obj_uuid_table, parent_uuid)
            except (pycassa.NotFoundException, KeyError):
