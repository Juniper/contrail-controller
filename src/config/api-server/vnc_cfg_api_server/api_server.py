...
class VncApiServer(object):
    ...
    def watch(self):
        try:
            ...
            def msg_unpack(event):
                self.data = json.loads(event['data'])
                key = None
                err_msg = None
                for key, value in self.data.items():
                    err_msg = value.get('error')
                return err_msg
            # end meg_unpack
            ...
            for resource in resource_fields:
                try:
                    ...
                    err_msg = "Initialization failed for %s with exception %s" % (
                        resource, e)
                    yield bottle.HTTPError(500, err_msg)
            ...
            while True:
                ...
                if (crud_event['event'] == "error"):
                    err_msg = msg_unpack(crud_event)
                    yield bottle.HTTPError(500, "Error in queue data %s" % err_msg)
