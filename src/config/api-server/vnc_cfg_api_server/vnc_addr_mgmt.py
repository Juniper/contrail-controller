...
class AddrMgmt(object):
    ...
    def get_subnet_quota_counter(self, obj_dict, proj_dict, db_vn_dict=None):
        ...
        self.config_log('quota_limit: %s' % quota_limit, level=SandeshLevel.SYS_INFO)
        self.config_log('default_quota: %s' % QuotaHelper.default_quota, level=SandeshLevel.SYS_INFO)
        if quota_limit >= 0:
            path_prefix = (
