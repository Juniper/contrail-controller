...
class FirewallDraftModeBase(object):
    ...
    def test_show_draft_version_of_a_pending_resource_require_fq_name(self):
        self.set_scope_instance()
        fp_name = 'fp-%s' % self.id()
        fp = FirewallPolicy(fp_name, parent_obj=self._scope)
        self.api.firewall_policy_create(fp)
        ...
        self.assertTrue(
            isinstance(
                self.api.firewall_policy_read(id=fp.uuid, draft=True), str))
        self.assertTrue(
            isinstance(
                self.api.firewall_policy_read(
                    fq_name=list(future_fq_name), draft=True),
                FirewallPolicy,
            ),
        )
        self.assertTrue(
            isinstance(
                self.api.firewall_policy_read(
                    fq_name_str=':'.join(future_fq_name), draft=True),
                FirewallPolicy,
            ),
        )

    def test_show_draft_version_of_a_pending_create_security_resource(self):
        self.set_scope_instance()
        fp_name = 'fp-%s' % self.id()
        fp = FirewallPolicy(fp_name, parent_obj=self._scope)
        self.api.firewall_policy_create(fp)
        ...
        try:
            self.api.firewall_policy_read(id=fp.uuid, draft=False)
        except NoIdError:
            self.fail("Cannot read the pending created Firewall Policy")
        try:
            self.api.firewall_policy_read(fq_name=future_fq_name, draft=True)
        except NoIdError:
            self.fail("Cannot read the draft version of the pending created"
                      "Firewall Policy")

    def test_show_draft_version_of_a_pending_update_security_resource(self):
        self.set_scope_instance(draft_enable=False)
        fp = FirewallPolicy('fp-%s' % self.id(), parent_obj=self._scope)
        self.api.firewall_policy_create(fp)
        fp = self.api.firewall_policy_read(id=fp.uuid)
        ...
        fp = self.api.firewall_policy_read(id=fp.uuid, draft=False)
        try:
            fp_draft = self.api.firewall_policy_read(fq_name=list(fp.fq_name),
                                                     draft=True)
        except NoIdError:
            self.fail("Cannot read the draft version of the pending created"
                      "Firewall Policy")
        self.assertNotEqual(fp.uuid, fp_draft.uuid)
        self.assertNotEqual(fp.fq_name, fp_draft.fq_name)
        self.assertEqual(fp.display_name, old_fp_name)
        self.assertEqual(fp_draft.display_name, new_fp_name)

    def test_show_draft_version_of_a_pending_delete_security_resource(self):
        self.set_scope_instance(draft_enable=False)
        fp = FirewallPolicy('fp-%s' % self.id(), parent_obj=self._scope)
        self.api.firewall_policy_create(fp)
        self._scope.enable_security_policy_draft = True
        ...
        self.assertTrue(fp_draft.pending_delete)
