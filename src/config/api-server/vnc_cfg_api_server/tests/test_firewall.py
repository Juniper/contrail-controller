...
class FirewallDraftModeBase(object):
    ...
    def test_show_draft_version_of_a_pending_update_security_resource(self):
        ...
        try:
            fp_draft = self.api.firewall_policy_read(id=fp.uuid, draft=True)
        except NoIdError:
            self.fail("Cannot read the draft version of the pending created"
                      "Firewall Policy")
    ...
    def test_revert_updated_complex_scenario(self):
        ...
        for aborted_uuid in (set(self.updates_security_resources.keys()) -
                             set(self.security_resources.keys())):
            with ExpectedException(NoIdError):
                getattr(self.api, '%s_read' % r.object_type)(id=aborted_uuid)
