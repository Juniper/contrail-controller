...
class TestExecuteJob(test_case.ApiServerTestCase):
    ...
    def test_get_device_credentials(self):
        ...
        dev_creds = self._vnc_lib.get_device_credentials(obj_uuid=fabric_uuid)
        self.assertIsNotNone(dev_creds)

        key = uuid.UUID(str(fabric_uuid)).bytes
        cipher = AES.new(key, AES.MODE_ECB)
        password = cipher.decrypt(base64.b64decode(dev_creds[0].get('password')))
        dev_creds[0]['password'] = password.strip()
        ...
        dev_creds = self._vnc_lib.get_device_credentials(obj_uuid=pr_uuid)
        self.assertIsNotNone(dev_creds)

        key = uuid.UUID(str(pr_uuid)).bytes
        cipher = AES.new(key, AES.MODE_ECB)
        password = cipher.decrypt(base64.b64decode(dev_creds[0].get('password')))
        dev_creds[0]['password'] = password.strip()
