...
class TestPnfPortTuple(TestPortTupleBase):
    def setUp(self):
        ...
        left_vn_obj.add_network_ipam(ipam_obj, VnSubnetsType([IpamSubnetType(SubnetType(left_vn_subnet, 24))]))
        ...
        right_vn_obj.add_network_ipam(ipam_obj, VnSubnetsType([IpamSubnetType(SubnetType(right_vn_subnet, 24))]))
        self.right_vn_uuid = self.api.virtual_network_create(right_vn_obj)
        # Create left/right VMI
        left_vmi_name = 'left_vmi-' + self.id()
        left_vmi = VirtualMachineInterface(name=left_vmi_name,parent_obj=proj_obj)
        left_vmi.add_virtual_network(left_vn_obj)
        left_vmi.set_virtual_machine_interface_device_owner('network:router_interface')
        self.left_vmi_uuid = self.api.virtual_machine_interface_create(left_vmi)
        right_vmi_name = 'right_vmi-' + self.id()
        right_vmi = VirtualMachineInterface(name=right_vmi_name,parent_obj=proj_obj)
        right_vmi.add_virtual_network(right_vn_obj)
        right_vmi.set_virtual_machine_interface_device_owner('network:router_interface')
        self.right_vmi_uuid = self.api.virtual_machine_interface_create(right_vmi)
        ...
        left_pnf_pi_obj = PhysicalInterface('ge-0/0/1-' + self.id(), parent_obj=pnf_obj)
        right_pnf_pi_obj = PhysicalInterface('ge-0/0/2-' + self.id(), parent_obj=pnf_obj)
        lo_pnf_pi_obj = PhysicalInterface('lo0', parent_obj=pnf_obj)
        self.left_pnf_pi_uuid = self.api.physical_interface_create(left_pnf_pi_obj)
        self.right_pnf_pi_uuid = self.api.physical_interface_create(right_pnf_pi_obj)
        self.lo_pnf_pi_uuid = self.api.physical_interface_create(lo_pnf_pi_obj)
        # create left, right spine PI
        left_spine_pi_obj = PhysicalInterface('xe-0/0/1-' + self.id(), parent_obj=spine_obj)
        right_spine_pi_obj = PhysicalInterface('xe-0/0/2-' + self.id(), parent_obj=spine_obj)
        self.left_spine_pi_uuid = self.api.physical_interface_create(left_spine_pi_obj)
        self.right_spine_pi_uuid = self.api.physical_interface_create(right_spine_pi_obj)
        ...
        kvp = KeyValuePair("left-attachment-point",
            default_gsc_name + ':' + 'spine-' + self.id() + ':' + 'xe-0/0/1-' + self.id())
        kvp_array.append(kvp)
        kvp = KeyValuePair("right-attachment-point",
            default_gsc_name + ':' + 'spine-' + self.id() + ':' + 'xe-0/0/2-' + self.id())
        kvp_array.append(kvp)
        kvps = KeyValuePairs()
        kvps.set_key_value_pair(kvp_array)
        ...
        sc_ns_obj = FabricNamespace('servicechain-subnet-' + self.id(), 
                       parent_obj=fab_obj,
                       fabric_namespace_type='IPV4-CIDR', 
                       fabric_namespace_value={'ipv4_cidr': {'subnet':pnf_subnets}})
        peer_subnets = _carve_out_subnets(pnf_cidr, 29)
        self.sc_ns_uuid = self.api.fabric_namespace_create(sc_ns_obj)
        pnf_vn_obj = VirtualNetwork(name='fab-' + self.id() + '-pnf-servicechain-network',
        ...
        pnf_ipam_obj = _get_network_ipam('fab-' + self.id() + '-pnf-servicechain-network-ipam', 
                                            peer_subnets, True)
        ...
        fab_obj = self.api.fabric_read(id=self.fab_uuid)
        fab_obj.add_virtual_network(pnf_vn_obj, FabricNetworkTag(network_type='pnf-servicechain'))
        ...
        lo_ns_obj = FabricNamespace('loopback-subnet-' + self.id(), 
                       parent_obj=fab_obj,
                       fabric_namespace_type='IPV4-CIDR', 
                       fabric_namespace_value={'ipv4_cidr': {'subnet':lo_subnets}})
        self.lo_ns_uuid = self.api.fabric_namespace_create(lo_ns_obj)
        lo_vn_obj = VirtualNetwork(name='fab-' + self.id() + '-loopback-network',
                         virtual_network_properties=VirtualNetworkType(forwarding_mode='l3'), 
                         address_allocation_mode='flat-subnet-only')
        self.lo_vn_uuid = self.api.virtual_network_create(lo_vn_obj)
        lo_ipam_obj = _get_network_ipam('fab-' + self.id() + '-loopback-network-ipam', 
                                            peer_subnets, False)
        ...
        fab_obj = self.api.fabric_read(id=self.fab_uuid)
        fab_obj.add_virtual_network(lo_vn_obj, FabricNetworkTag(network_type='loopback'))
        self.api.fabric_update(fab_obj)
        pass
    ...
    def tearDown(self):
        ...
        lo_vn_obj.del_network_ipam(pnf_ipam_obj)
        self.api.virtual_network_update(pnf_vn_obj)
        self.api.virtual_network_update(lo_vn_obj)
    ...
    def test_valid_pt(self):
        ...
        left_pnf_pi_obj = self.api.physical_interface_read(id=self.left_pnf_pi_uuid)
        right_pnf_pi_obj = self.api.physical_interface_read(id=self.left_pnf_pi_uuid)
        left_spine_pi_obj = self.api.physical_interface_read(id=self.left_spine_pi_uuid)
        right_spine_pi_obj = self.api.physical_interface_read(id=self.right_spine_pi_uuid)
        lo_pnf_pi_obj = self.api.physical_interface_read(id=self.lo_pnf_pi_uuid)
        ...
        left_pnf_li_uuid = left_pnf_pi_obj.get_logical_interfaces()[0].get('uuid')
        right_pnf_li_uuid = right_pnf_pi_obj.get_logical_interfaces()[0].get('uuid')
        left_spine_li_uuid = left_spine_pi_obj.get_logical_interfaces()[0].get('uuid')
        right_spine_li_uuid = right_spine_pi_obj.get_logical_interfaces()[0].get('uuid')
        ...
        right_pnf_li_obj = self.api.logical_interface_read(id=left_pnf_li_uuid)
        left_spine_li_obj = self.api.logical_interface_read(id=left_spine_li_uuid)
        right_spine_li_obj = self.api.logical_interface_read(id=right_spine_li_uuid)
        ...
        self.assertEqual(len(right_spine_li_obj.get_instance_ip_back_refs()), 1)
        self.assertEqual(len(lo_pnf_li_obj.get_instance_ip_back_refs()), 1)

        left_pnf_iip_uuid = left_pnf_li_obj.get_instance_ip_back_refs()[0].get('uuid')
        right_pnf_iip_uuid = right_pnf_li_obj.get_instance_ip_back_refs()[0].get('uuid')
        left_spine_iip_uuid = left_spine_li_obj.get_instance_ip_back_refs()[0].get('uuid')
        right_spine_iip_uuid = right_spine_li_obj.get_instance_ip_back_refs()[0].get('uuid')
        lo_pnf_iip_uuid = lo_pnf_li_obj.get_instance_ip_back_refs()[0].get('uuid')

        self.assertIsNotNone(self.api.instance_ip_read(id=left_pnf_iip_uuid).instance_ip_address)
        self.assertIsNotNone(self.api.instance_ip_read(id=right_pnf_iip_uuid).instance_ip_address)
        self.assertIsNotNone(self.api.instance_ip_read(id=left_spine_iip_uuid).instance_ip_address)
        self.assertIsNotNone(self.api.instance_ip_read(id=right_spine_iip_uuid).instance_ip_address)
        self.assertIsNotNone(self.api.instance_ip_read(id=lo_pnf_iip_uuid).instance_ip_address)
