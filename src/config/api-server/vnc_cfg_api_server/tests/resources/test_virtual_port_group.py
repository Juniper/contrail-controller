...
class TestVirtualPortGroup(TestVirtualPortGroupBase):
    ...
    def test_vmi_bindings_key_value_pair(self):
        ...
        #
        # vmis = self.api.resource_list(VirtualMachineInterface.object_type,
        #                               obj_uuids=[vmi.fq_name], detail=True)
        vmi_obj = self.api.virtual_machine_interface_read(id=vmi.uuid)
        self.assertItemsEqual(kv_pairs['key_value_pair'],
                              vmi_obj.virtual_machine_interface_bindings)
