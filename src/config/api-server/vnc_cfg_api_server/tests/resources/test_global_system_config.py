...
import mock
from vnc_api.vnc_api import GlobalSystemConfig, VirtualNetwork, RouteTargetList
...
class TestGlobalSystemConfig(test_case.ApiServerTestCase):
    ...
    def test_change_4_byte_to_2_byte_asn_if_any_rt_uses_4_byte(self):
        """
        Test scenario.
        ...
        rt_name = 'target:%d:%d' % (
            self.ASN_4_BYTES, get_bgp_rtgt_min_id(self.ASN_4_BYTES) + 1000)
        vn.set_route_target_list(RouteTargetList([rt_name]))
        self.api.virtual_network_create(vn)

        # Set enable_4byte_as to false
        # and change global ASN to 2 bytes numbers
        gsc.enable_4byte_as = False
        gsc.autonomous_system = self.NEW_ASN
        self.api.global_system_config_update(gsc)
