...
class TestFabricNamespace(TestFabricNamespaceBase):
    ...
    def _create_and_check_fn(self, fn_subnets):
        ...
        for subnet in fn_subnets:
            ...
            namespace = FabricNamespace(
                ...
                fq_name= ["default-global-system-config", "fabric", "overlay-loopback-subnets"],
                parent_type='fabric',
        ...
        vn = None
        try:
            vn = self.api.virtual_network_read(fq_name=
                                               self._get_loopback_vn_fq_name(
                                               'fabric'))
        except:
            self.assertTrue(False,
                            'Loopback VN not created')
        ...
        for subnet in subnets.get_subnets():
            ...
            if cidr not in fn_subnet_list:
                self.assertTrue(create_exception,
    ...
    def _delete_fabric_namespace(self, fn_uuid):
        ...
        try:
            self.api.virtual_network_read(fq_name=
    ...
    def test_loopback_routed_vn_instance_ip_alloc_dealloc(self):
        ...
        fn_uuid = self._create_and_check_fn(loopback_subnet)
        # Read loopback VN
        vn = self.api.virtual_network_read(fq_name=self._get_loopback_vn_fq_name('fabric'))
        ...
        if '1.1.1.5' not in ip_list:
            self.assertTrue(True,
                            'Loopback IP not found in VN instance ip back refs')
