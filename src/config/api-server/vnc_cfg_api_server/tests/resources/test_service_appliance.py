...
class TestPnfServiceAppliance(TestServiceApplianceBase):
    def setUp(self):
        ...
        self.gsc_obj = self.api.global_system_config_read(GlobalSystemConfig().fq_name)
        ...
        self.sas_obj.set_service_appliance_set_virtualization_type('physical-device')
        ...
        pnf_uuid = self.api.physical_router_create(self.pnf_obj)
        # Create spine Physical Router object
        self.spine_obj = PhysicalRouter('spine-' + self.id(), self.gsc_obj)
        spine_uuid = self.api.physical_router_create(self.spine_obj)
        # create left, right PNF PI
        self.left_pnf_pi_obj = PhysicalInterface('ge-0/0/1-' + self.id(), parent_obj=self.pnf_obj)
        self.right_pnf_pi_obj = PhysicalInterface('ge-0/0/2-' + self.id(), parent_obj=self.pnf_obj)
        ...
        self.left_spine_pi_obj = PhysicalInterface('xe-0/0/1-' + self.id(), parent_obj=self.spine_obj)
        self.right_spine_pi_obj = PhysicalInterface('xe-0/0/2-' + self.id(), parent_obj=self.spine_obj)
    ...
    def test_valid_sa(self):
        ...
        kvp = KeyValuePair("left-attachment-point",
            self.default_gsc_name + ':' + 'spine-' + self.id() + ':' + 'xe-0/0/1-' + self.id())
        kvp_array.append(kvp)
        kvp = KeyValuePair("right-attachment-point",
            self.default_gsc_name + ':' + 'spine-' + self.id() + ':' + 'xe-0/0/2-' + self.id())
        kvp_array.append(kvp)
        kvps = KeyValuePairs()
        ...
        self.left_pnf_pi_obj = self.api.physical_interface_read(id=self.left_pnf_pi_obj.uuid)
        self.right_pnf_pi_obj = self.api.physical_interface_read(id=self.right_pnf_pi_obj.uuid)

        # Check if spine PI <-> PNF PI link has been created
        self.assertEqual(self.left_pnf_pi_obj.physical_interface_refs[0].get('uuid'),
                   self.left_spine_pi_obj.uuid)
        self.assertEqual(self.right_pnf_pi_obj.physical_interface_refs[0].get('uuid'),
        ...
        self.left_pnf_pi_obj = self.api.physical_interface_read(id=self.left_pnf_pi_obj.uuid)
        self.right_pnf_pi_obj = self.api.physical_interface_read(id=self.right_pnf_pi_obj.uuid)
        self.assertFalse(hasattr(self.left_pnf_pi_obj, "physical_interface_refs"))
        self.assertFalse(hasattr(self.right_pnf_pi_obj, "physical_interface_refs"))
    ...
    def test_sa_with_invalid_kvp(self):
        ...
        self.assertRaises(BadRequest, self.api.service_appliance_create, sa_obj)
    ...
    def test_sa_with_invalid_pr_role(self):
        ...
        # Create SA should raise exception
        self.assertRaises(HttpError, self.api.service_appliance_create, sa_obj)
