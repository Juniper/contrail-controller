...
class TestSubClusterBase(ApiServerTestCase):
    ...
    def setUpClass(cls, *args, **kwargs):
        ...
        # Create a pool of 1 sub-cluster ID to use for the tests
        start = 42
        if cls._global_asn < 0xFFFF:
            start += 1 << 16
...
class SubClusterTestSuite(object):
    ...
    def test_cluster_id_automatically_assigned(self):
        ...
        self.assertEqual(
            zk_db.get_sub_cluster_from_id(
                self._global_asn, sc.get_sub_cluster_id()),
            sc.get_fq_name_str(),
        )
    ...
    def test_manually_allocate_sub_cluster_id(self):
        ...
        self.assertEqual(
            zk_db.get_sub_cluster_from_id(
                self._global_asn, sub_cluster_id),
            sc.get_fq_name_str(),
        )
