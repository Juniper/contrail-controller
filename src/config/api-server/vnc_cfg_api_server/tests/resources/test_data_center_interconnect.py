...
class TestDataCenterInterconnect(test_case.ApiServerTestCase):
    ...
    def _validate_dci_lr_refs_error(self, dci, srclr, update=False):
        ...
        with ExpectedException(
            cfgm_common.exceptions.BadRequest,
                ...
                "intra_fabric type data_center_interconnect.") as e:
                func_call(dci)
        if update == True:
            return
        ...
        with ExpectedException(
            ...
            "type data_center_interconnect.") as e:
            func_call(dci)
    ...
    def _validate_destination_pr_list_properties(self, dci, dict_lrs,
        ...
        with ExpectedException(
                ...
                "destination_physical_router_list.") as e:
            self._vnc_lib.data_center_interconnect_create(dci)

        lr_list[0].logical_router_uuid = dict_lrs[src_lrname].get_uuid()
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "%s logical_router_uuid exist in destination_physical_router_list"
                " is marked as a  source LR." %
                dict_lrs[src_lrname].get_uuid()) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
        ...
        with ExpectedException(
                ...
                dict_lrs[dst_lrname3].get_uuid()) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
        ...
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "%s logical_router_uuid in destination_physical_router_list does "
                "not have  any physical_router" % lruuid) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
        ...
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "%s logical_router_uuid in destination_physical_router_list "
                "specified more than once." % lruuid) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
        ...
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "No destination LR exist in destination_physical_router_list") \
                as e:
            self._vnc_lib.data_center_interconnect_create(dci)
        ...
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "destination LR list count %s in destination_physical_router_list"
                " does not match logical_router_Refs expected %s count" %
                ((len(lr_list) - 1), len(lr_list))) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
    ...
    def _validate_dci_rp_and_vn_refs(self, dci, srcrp_names,
        ...
        with ExpectedException(
                cfgm_common.exceptions.BadRequest,
                "Provide either routing_policy_refs or virtual_network_refs. Both"
                " has been provided for intra_fabric type "
                "data_center_interconnect") as e:
            self._vnc_lib.data_center_interconnect_create(dci)
    ...
    def _validate_master_lr_and_public_lr_cases(self, srcvn_name1,
        ...
        with ExpectedException(
                ...
                " set to be True." % dict_lrs[dst_lrname3].display_name) as e:
            self._vnc_lib.data_center_interconnect_create(dci)
    ...
    def test_dci_intrafabric(self):
        ...
        subnetmask = 24; vn_starting_index = 21
