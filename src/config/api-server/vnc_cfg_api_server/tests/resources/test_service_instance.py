...
class TestPnfServiceInstance(TestServiceInstanceBase):
    def setUp(self):
        ...
        gsc_obj = self.api.global_system_config_read(GlobalSystemConfig().fq_name)
        # Create Service Appliance Set object
        sas_obj = ServiceApplianceSet('sas-' + self.id(), gsc_obj)
        sas_obj.set_service_appliance_set_virtualization_type('physical-device')
    ...
    def test_valid_si(self):
        ...
        st_fqn = ['default-domain', 'st-' + self.id()]
        ...
        # Expected - [key = left-svc-unit, value = 5, key = right-svc-unit, value = 6]
        self.assertEqual(len(si_obj.service_instance_bindings.key_value_pair), 2)
