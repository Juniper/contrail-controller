...
class TestLogicalRouter(test_case.ApiServerTestCase):
    ...
    def test_job_transaction(self):
        ...
        lr_uuid = self._vnc_lib.logical_router_create(lr)
        self.assertEqual("Logical Router '%s' Create" % lr_name,
                         self._get_job_transaction_descr(pr1_uuid))
        lr = self._vnc_lib.logical_router_read(id=lr_uuid)
        logger.debug('Created Logical Router ')
        ...
        self._vnc_lib.logical_router_update(lr)
        self.assertEqual("Logical Router '%s' Update" % lr_name,
                         self._get_job_transaction_descr(pr1_uuid))

        self._vnc_lib.logical_router_delete(id=lr_uuid)
        self.assertEqual("Logical Router '%s' Delete" % lr_name,
                         self._get_job_transaction_descr(pr1_uuid))
        self.assertEqual("Logical Router '%s' Delete" % lr_name,
                         self._get_job_transaction_descr(pr2_uuid))
