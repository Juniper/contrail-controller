...
from vnc_api.vnc_api import NodeProfile
...
class TestNodeProfile(test_case.ApiServerTestCase):
    ...
    def remove_node_and_port(self, node_and_port):
        ...
        for node in node_and_port:
            ...
            port_groups = self.api.port_groups_list(parent_fq_name = ['default-global-system-config', node])
    ...
    def create_tags(self):
        tag_list = {'provisioning': {'tag_type_name': 'label'}, 
        ...
        for tag in tag_list:
            tag_obj = Tag(tag_type_name=tag_list[tag]['tag_type_name'], tag_value=tag) 
    ...
    def create_node_profile(self, node_profile_data):
        for np in node_profile_data:
            ...
            for iface in interface_map:
                ...
                port_info = PortInfoType(name = iface, type = "xe", 
            ...
            card_obj = Card(hardware['card'].get('name'), interface_map = iface_map)
            ...
            node_profile_obj = NodeProfile( np,
                                       node_profile_vendor = node_profile_data[np].get('node_profile_vendor'), 
                                       node_profile_device_family= node_profile_data[np].get('node_profile_device_family'), )
    ...
    def test_create_node_profile(self):
        '''
        1. create end-system (node1), and ports,
        2. create node-profiles qfx1-np and qfx2-np
        3. create tags to be used
        4. associate end-system with qfx1-np, now end-system-ports should ref to tags from node-profile
        5. assoicate end-system with qfx2-np, now end-system-ports should ref to new tags from node-profile
        6. remove ref from end-system, tags from end-system-ports should be removed.
        7. remove ports and end-system, there should not be any error.
        '''
        node_and_port = {'node1':  
                           [{'name':'eth0', 'address': "11:22:33:44:55:55", 
                                 'sw_name': 'unit_test_qfx1', 'port_id': 'xe-0/0/0'},
                            {'name':'eth1', 'address': "11:22:33:44:55:56", 
                                 'sw_name': 'unit_test_qfx1', 'port_id': 'xe-0/0/1'},
                            {'name':'eth2', 'address': "11:22:33:44:55:57", 
                                 'sw_name': 'unit_test_qfx1', 'port_id': 'xe-0/0/2'}]}

        node_profile_data = { 'qfx1-np': {
                                ...
                                'hardware': {
                                  ...
                                  'card': {
                                    'name': 'card1',
                                    'interface-map': {
                                      'eth0': {'labels':["provisioning", "tenant"], 'port_group': 'bond0',
                                               'port_speed': '10G' },
                                      'eth1': {'labels':["tenant"], 'port_group': 'bond0',
                                               'port_speed': '10G' },
                                      'eth2': {'labels':["provisioning", "tenant", "control-data"],
                                               'port_speed': '10G' }
                                    }
                                  }
                                }
        ...
        node_profile_data1 = { 'qfx2-np': {
                                ...
                                'hardware': {
                                  ...
                                  'card': {
                                    'name': 'card2',
                                    'interface-map': {
                                      'eth0': {'labels':["provisioning1", "tenant1"], 'port_group': 'bond1',
                                               'port_speed': '10G' },
                                      'eth1': {'labels':["tenant2"], 'port_group': 'bond1',
                                               'port_speed': '10G' },
                                      'eth2': {'labels':["provisioning1", "tenant3", "control-data1"],
                                               'port_speed': '10G' }
                                    }
                                  }
                                }
        ...
        node_object=self.api.end_system_read(fq_name=['default-global-system-config', 'node1'])
        np_object=self.api.node_profile_read(fq_name=['default-global-system-config', 'qfx1-np'])
        np2_object=self.api.node_profile_read(fq_name=['default-global-system-config', 'qfx2-np'])
        ...
        for node in node_and_port:
            node_object_update=self.api.end_system_read(fq_name=['default-global-system-config', node])
            logger.warn(pprint.pformat(node_object_update.__dict__))
            for port in node_and_port[node]:
                port_obj = self.api.port_read(fq_name=['default-global-system-config', node, port.get('name')])
                logger.warn(pprint.pformat(port_obj.__dict__))
            
        self.api.ref_update('end-system', node_object.uuid, 'node-profile', np2_object.uuid, ['default-global-system-config', 'qfx2-np'], 'DELETE')
        for node in node_and_port:
            node_object_update=self.api.end_system_read(fq_name=['default-global-system-config', node])
            logger.warn(pprint.pformat(node_object_update.__dict__))
            for port in node_and_port[node]:
