            ...
            if prop:  # if( is_a_property )
                ...
                if schemes[obj_type].has_key(prop):
                    try:
                        ...
                        _debug(e)
                        # FIXME: inform the user about the problem
                        continue
    ...
    for obj_type, obj in iterate_over_objects(data):
        try:
            ...
            for k, schema in upgrade.items():
                ...
                # TODO(rado)
                # this is actually pretty easy, as I can use the types in schema
                # as constructors and use them to create nullable values
...
def json_upgrade(file_l, file_r, **kwargs):
    ...
    with open(file_l) as fl, open(file_r) as fr:
        ...
        # TODO(rado): make it clear which interpretation is correct.
        # options are:
        # - old data with new schema
        # - new data with old schema
        fl.seek(0)
        data = json.load(fl)  # load old data
        cassandra_schema_upgrade(data['cassandra'], schm_new)  # conform to new schema
