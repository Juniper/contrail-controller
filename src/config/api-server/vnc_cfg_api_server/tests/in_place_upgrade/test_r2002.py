...
class TestInPlaceUpgradeR2002(test_case.InPlaceUpgradeTestCase):
    ...
    def test_alias_ip_create(self):
        ...
        prop_map = {
            'parent_obj': aip_pool,
            'parent_type': aip_pool.object_type,
    ...
    def test_subnet_create(self):
        prop_map = {
            'subnet_ip_prefix': SubnetType(id_prefix='192.168.0.0',
                                           ip_prefix_len=16),
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_address_group_create(self):
        prop_map = {
            ...
            'address_group_prefix': SubnetListType(subnet=SubnetType(
                id_prefix='192.168.0.0', ip_prefix_len=16)),
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_qos_queue_create(self):
        qos_cfg = QlobalQosConfig(name=self.id(), parent_obj=self.gsc,
    ...
    def test_card_create(self):
        prop_map = {
            'interface_map': InterfaceMapType(port_info=PortInfoType(
                ...
                port_group='some string', labels='some string'
            )),
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_qos_config_create(self):
        qos_forwarding = QosIdForwardingClassPairs(
            qos_id_forwarding_class_pair=QosIdForwardingClassPair(
                control=10, analytics=23, dns=55))
    ...
    def test_global_vrouter_config_create(self):
        prop_map = {
            ...
            'linklocal_services': LinklocalServicesTypes(
                linklocal_service_entry=LinklocalServiceEntryType(
                    linklocal_service_name='some string',
                    linklocal_service_ip='192.168.168.10',
                    linklocal_service_port=8085,
                    ip_fabric_service_port=9090)),
            'encapsulation_priorities': EncapsulationPrioritiesType(
                encapsulation='VXLAN'),
            'vxlan_network_identifier_mode': 'configured',
            'flow_export_rate': 124,
            'flow_aging_timeout_list': FlowAgingTimeoutList(
                flow_aging_timeout=FlowAgingTimeout(
                    key=124, forwarding_class_id=120)),
            'enable_security_logging': True,
            'encryption_mode': 'all',
            'encryption_tunnel_endpoints': EncryptionTunnelEndpointList(
                endpoint=EncryptionTunnelEndpoint(
                    tunnel_remote_ip_address='172.128.23.23')),
            'forwarding_mode': 'l2_l3',
            'port_translation_pools': PortTranslationPools(
                port_translation_pool=PortTranslationPool(protocol='UDP')),
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_floating_ip_create(self):
        fip_pool = FloatingIpPool(
            ...
            floating_ip_pool_subnets=FloatingIpPoolSubnetType(
                subnet_uuid='9b5e5547-a7b2-4cd2-99ed-87eff7ed34da'))
        self.api.floating_ip_pool_crate(fip_pool)

        prop_map = {
            'parent_obj': fip_pool,
            'parent_type': fip_pool.object_type,
            'floating_ip_address': '10.100.0.123',
            'floating_ip_is_virtual_ip': '10.100.100.100',
            ...
            'floating_ip_port_mappings': PortMappings(
                port_mappings=PortMap(protocol='TCP', src_port=9124,
                                      dst_port=9354)),
            ...
            'routing_policy_entries': PolicyStatementType(
                term=PolicyTermType(
                    term_match_condition=TermMatchConditionType(
                        ...
                        community='some string', community_list='some string',
                        ...
                        nlri_route_type=124, prefix_list=PrefixListMatchType(
    ...
    def test_virtual_dns_create(self):
        ...
        domain = Domain(name=self.id(), parent_obj=cr)
        ...
        prop_map = {
            ...
            'virtual_DNS_data': VirtualDnsType(
                domain_name='test domain', dynamic_records_from_client=True,
    ...
    def test_firewall_rule_create(self):
        prop_map = {
            ...
            'match_tags': FirewallRuleMatchTagsType(
                tag_list=TagType(tag_type_id='test tag')),
            'match_tag_types': FirewallRuleMatchTagsTypeIdList(tag_type=1),
            'direction': '<>',
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_bgp_vpn_create(self):
        rt_list = RouteTargetList(route_target='some string')
    ...
    def test_security_group_create(self):
        prop_map = {
            ...
            'configured_security_group_id': 1,
        ...
        prop_map = {
            ...
            'fabric_namespace_value': NamespaceValue(
                ipv4_cidr=SubnetListType(subnet=SubnetType(
                    id_prefix='10.100.0.0', id_prefix_len=16)),
                asn=self.gsc.autonomous_system),
            'annotations': {},
            'display_name': 'some string',
        }
    ...
    def test_network_ipam_create(self):
        prop_map = {
            ...
            'ipam_subnets': IpamSubnets(subnets=IpamSubnetType(
                subnet=SubnetType(id_prefix='10.100.0.0', id_prefix_len=16))),
            'ipam_subnet_method': 'auto-subnet',
    ...
    def test_virtual_port_group_create(self):
        prop_map = {
            ...
            'virtual_port_group_trunk_port_id': 'foo',
    ...
    def test_physical_interface_create(self):
        ...
        prop_map = {
            'parent_obj': pr,
            'parent_type': pr.object_type,
            'ethernet_segment_identifier': 'some string',
