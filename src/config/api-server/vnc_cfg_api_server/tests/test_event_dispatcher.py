...
def TestPack(TestCase):
    def test_pack(self):
        ...
        resource_data = {}
        event = dispatcher.pack(id=resource_id, event=resource_oper, data=resource_data)
        self.assertEquals(
            event,
            {
            'id': resource_id,
...
def TestRegisterClient(TestCase):
    ...
    def test_register_client(self):
        client_queue = Queue()
        watcher_resource_types = ["virtual_network", "virtual_network_interface"]
        self._dispatcher.register_client(client_queue, watcher_resource_types)
        for resource_type in watcher_resource_types:
            self.assertEquals(self._dispatcher._get_client_queues()[resource_type], client_queue)
