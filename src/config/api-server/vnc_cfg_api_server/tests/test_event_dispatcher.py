...
class TestDispatch(TestCase):
    ...
    def test_dispatch_with_dbe_read_failure(self):
        mockVncDBClient = flexmock(
            dbe_read=lambda resource_type, resource_id: (False, {})
        )
        self._dispatcher._set_db_conn(mockVncDBClient)
        ...
        for notification in notifications:
            event = {
                    'event': "error",
                    'data': json.dumps({"error": "dbe_read failure"})
            }
            self.assertEquals(
                event,
                client_queue.get()
...
class TestInitialize(TestCase):
    ...
    def assertExceptionEquals(self, expected, actual):
        self.assertEquals(
    ...
    def test_initialize_with_dbe_list_failure(self):
        mockVncDBClient = flexmock(
            dbe_list=lambda resource_type, is_detail: (False, {}, 0)
        )
        self._dispatcher._set_db_conn(mockVncDBClient)
        watcher_resource_types = [
            "virtual_network",
        ...
        for resource_type in watcher_resource_types:
            try:
                ...
                self.assertExceptionEquals(Exception("deb_list failure"), e)
