...
class TestRegisterClient(TestCase):
    ...
    def test_concurrent_requests_with_subscribe_client(self):
        org_subscribe_client_queue = self._dispatcher._subscribe_client_queue
        ...
        class MockSubscribe():
            ...
            @classmethod
            def mock_subscribe_client_queue(cls, client_queue, resource_type):
                while cls.HOLD_API:
                    gevent.sleep(0.5)
                return cls.org_subscribe_client_queue(client_queue, resource_type)

        def _execute_subscribe_concurrently(
                client_queue_list, watcher_resource_types):
            try:
                self._dispatcher._subscribe_client_queue = MockSubscribe.mock_subscribe_client_queue
                MockSubscribe.HOLD_API = True
                ...
                self._dispatcher._subscribe_client_queue = org_subscribe_client_queue
        ...
        self._dispatcher._client_queues.clear()
    ...
    def test_concurrent_requests_with_subscribe_and_unsubscribe_client(self):
        org_subscribe_client_queue = self._dispatcher._subscribe_client_queue
        org_unsubscribe_client_queue = self._dispatcher._unsubscribe_client_queue
        ...
        class MockClass():
            ...
            @classmethod
            def mock_unsubscribe_client_queue(
                    cls, client_queue, resource_type):
                while cls.HOLD_API:
                    gevent.sleep(0.5)
                return cls.org_unsubscribe_client_queue(client_queue, resource_type)

        def _execute_subscribe_unsubsribe_concurrently(
                method_invocation_list, client_queue_list, watcher_resource_types):
            try:
                self._dispatcher._subscribe_client_queue = MockClass.mock_subscribe_client_queue
                self._dispatcher._unsubscribe_client_queue = MockClass.mock_unsubscribe_client_queue
                MockClass.HOLD_API = True
                for index, method in enumerate(method_invocation_list):
                    gevent.spawn(
                        ...
                        watcher_resource_types)
                gevent.sleep(2)
                MockClass.HOLD_API = False
                gevent.sleep(3)
            except gevent.timeout.Timeout:
                self.assertFalse(False, 'Failed unexpectedly')
            finally:
                self._dispatcher._subscribe_client_queue = org_subscribe_client_queue
                self._dispatcher._unsubscribe_client_queue = org_unsubscribe_client_queue

        # Case 1
        # Client1:unsubscribe(fields:all), Client2:subscribe(fields:X,Z)
        resources_query1 = {}
        resources_query2 = {
            "virtual_network": {
                "fields": [
        ...
        method_invocation_list = [
            ...
            self._dispatcher.subscribe_client]
        _execute_subscribe_unsubsribe_concurrently(
            method_invocation_list, client_queue_list, watcher_resource_types)
    ...
    def test_concurrent_requests_with_unsubscribe_client(self):
        org_unsubscribe_client_queue = self._dispatcher._unsubscribe_client_queue

        class MockUnsubscribe():
            org_unsubscribe_client_queue = self._dispatcher._unsubscribe_client_queue
            HOLD_API = True
        ...
        def _execute_unsubsribe_concurrently(
                client_queue_list, watcher_resource_types):
            try:
                self._dispatcher._unsubscribe_client_queue = MockUnsubscribe.mock_unsubscribe_client_queue
                MockUnsubscribe.HOLD_API = True
                for client_queue in client_queue_list:
                    gevent.spawn(
                        self._dispatcher.unsubscribe_client,
                        client_queue,
                        watcher_resource_types)
                gevent.sleep(2)
                MockUnsubscribe.HOLD_API = False
                gevent.sleep(3)
            except gevent.timeout.Timeout:
                self.assertFalse(
                    False, '%s failed unexpectedly' %
        ...
        self._dispatcher.subscribe_client(
            client_queue2, watcher_resource_types)
        _execute_unsubsribe_concurrently(
            client_queue_list, watcher_resource_types)
        for resource_type in watcher_resource_types:
            result_request_fields = self._dispatcher._get_client_queues()[
                resource_type].get("fields")
