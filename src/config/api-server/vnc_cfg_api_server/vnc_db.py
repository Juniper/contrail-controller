    ...
    def alloc_ae_id(self, fq_name_str, id=None):
        ...
        if id is not None:
            if self.get_ae_from_id(id) is not None:
                self._ae_id_allocator.set_in_use(id - _AE_MIN_ID)
                return id
        elif fq_name_str is not None:
            return self._ae_id_allocator.alloc(fq_name_str) + _AE_MIN_ID

    def free_ae_id(self, id, fq_name_str, notify=False):
        if id is not None and id - _AE_MIN_ID < self._AE_MAX_ID:
            # If fq_name associated to the allocated ID does not correpond to
            # freed resource fq_name, keep zookeeper lock
            ...
            if notify:
                # If notify, the ZK allocation already removed, just remove
                # lock in memory
                self._ae_id_allocator.reset_in_use(id - _AE_MIN_ID)
            else:
                self._ae_id_allocator.delete(id - _AE_MIN_ID)

    def get_ae_from_id(self, id):
        if id is not None and id - _AE_MIN_ID < self._AE_MAX_ID:
            return self._ae_id_allocator.read(id - _AE_MIN_ID)
