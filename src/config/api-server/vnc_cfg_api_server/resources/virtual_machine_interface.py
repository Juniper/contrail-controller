...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
        ...
        if kvps:
            ...
            if vnic_type == 'baremetal':
                ...
                if phy_links and phy_links.get('local_link_information'):
                    ...
                    db_vlan_type = None
                    bindings = read_result.get('virtual_machine_interface_bindings') or {}
                    if bindings and (bindings.get('key_value_pair')):
                        kvp_nums = (bindings.get('key_value_pair')).__len__()
                        for i in range(kvp_nums):
                           if (bindings.get('key_value_pair'))[i].get('key') == 'tor_port_vlan_id':
                               db_vlan_type = True
                           elif old_vlan:
                               db_vlan_type = False
    ...
    def _check_serviceprovider_fabric(
            ...
            vlan_id, vmi_uuid, is_untagged_vlan, db_vlan_type):
        ...
        try:
            ...
            def undo_create_sp_validation_node():
                cls.vnc_zk_client.delete_node(validation_znode, True)
                return True, "", None
            ...
            if value:  # Not STALE
                ...
                if is_untagged_vlan and db_vlan_type:
                    return True, '', None

                # Check if VLAN_type has changed
                if db_vlan_type is not None:
                    # tagged to untagged
                    if (is_untagged_vlan and not db_vlan_type):
                        validation_znode = os.path.join(
                            _DEFAULT_ZK_SERVICE_PROVIDER_FABRIC_VALIDATION_PATH_PREFIX,
                            'virtual-port-group:%s/virtual-network:%s/vlan:%s'
                            % (vpg_uuid, vn_uuid, db_vlan_id))
                    ...
                    # untagged to tagged
                    if db_vlan_type and (not is_untagged_vlan and db_vlan_type):
                        validation_znode = os.path.join(
                            _DEFAULT_ZK_SERVICE_PROVIDER_FABRIC_VALIDATION_PATH_PREFIX,
                            'virtual-port-group:%s/untagged'
                            % vpg_uuid)
                        return True, '', {'delete_validation_znode': validation_znode}
    ...
    def get_vlan_id(cls, obj_dict):
        ...
        links = None
        is_untagged_vlan = False
        bindings = (obj_dict.get('virtual_machine_interface_bindings') or None)
        if not bindings:
    ...
    def _delete_serviceprovider_validations_znodes(cls, obj_dict, db_conn, vpg_uuid):
        ...
        ok, result = cls.get_vlan_id(obj_dict)
        if not ok:
            return ok, result
        vlan_id, is_untagged_vlan, links = result
