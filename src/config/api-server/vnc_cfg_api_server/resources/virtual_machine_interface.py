...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn,
        ...
        if kvps:
            ...
            if vnic_type == 'baremetal':
                ...
                if phy_links and phy_links.get('local_link_information'):
                    links = phy_links['local_link_information']
                    if ((tor_port_vlan_id != 0 and old_vlan != 0) or
                       (tor_port_vlan_id == 0 and old_vlan == 0)):
                        msg = 'Only one of tor_port_vlan_id or '\
                              'sub_interface_vlan_tag can be set and '\
                              'both cannot be zero'
                        return (False, (400, msg))
    ...
    @classmethod
    def _check_vpg_for_enterprise(cls, all_vpgs, vpg_uuid, vn_uuid,
                                  vlan_id, is_tagged_vlan):
    ...
    def _check_vlanid_vn_mapping(cls, all_vpgs, vn_uuid, vlan_id):
        ...
        # if the current VN is already connected to a different VMI of
        # any VPG in this fabric, then it's VLAN ID for that VMI and
        # current VMI should be same.
        for vpg in all_vpgs:
            # Get all VMI for VPG
            ok, read_result = cls.dbe_read(
                db_conn,
    ...
    @classmethod
    def _check_vpg_for_service_provider(cls, vpg_uuid, vn_uuid,
                                        vlan_id, is_tagged_vlan):
        db_conn = cls.db_conn
        # Only one native/untagged VLAN supported
        # A VN can be set only once.
        ...
        for vn_id, vlan_list in all_vns_in_vpg_dict.items():
            if len(vlan_list) > len(set(vlan_list)):
                msg = ("Virtual Network(%s) can only be associated to VPG "
                       "through different VLAN in a service provider "
                       "style fabric" % vn_id)
                return (False, (400, msg))
    ...
    def _manage_vpg_association(cls, vmi_id, api_server, db_conn, phy_links,
        ...
        if 'parent_uuid' in vpg_dict:
            ...
            if fabric_enterprise_style:
                ...
                ok, result = cls._check_vpg_for_service_provider(
                    vpg_uuid,
                    vn_uuid,
                    vlan_id,
                    is_tagged_vlan)
