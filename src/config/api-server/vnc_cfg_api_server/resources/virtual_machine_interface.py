...
from vnc_api.gen.resource_common import LinkAggregationGroup
...
from vnc_cfg_api_server.resources import InstanceIpServer
...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn,
        ...
        if 'virtual_machine_interface_properties' in obj_dict:
            ...
            old_vlan = int(
                read_result.get('virtual_machine_interface_properties',
                                {}).get('sub_interface_vlan_tag', 0))
    ...
    def _manage_lag_interface(cls, vmi_id, api_server, db_conn, phy_links,
        ...
        for uuid in set(phy_interface_uuids):
            ...
            lag_refs = phy_interface_dict.get(
        ...
        if lag_name:  # read the lag object
            ...
            ok, resp = api_server.internal_request_create(
                'link-aggregation-group', lag_obj.serialize_to_json())
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        for lag_back_ref in obj_dict.get('link_aggregation_group_back_refs',
                                         []):
            fqname = lag_back_ref['to']

            # Check if LAG is involved and and it's not referring to any other
            ...
            if not lag_dict.get('virtual_machine_interface_refs'):
                api_server.internal_request_delete('link_aggregation_group',
                                                   lag_uuid)
                uuid = int(fqname[2][2:])
