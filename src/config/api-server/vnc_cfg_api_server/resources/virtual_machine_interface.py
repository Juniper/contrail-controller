...
from vnc_api.gen.resource_xsd import KeyValuePairs, KeyValuePair
...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    @classmethod
    def _check_annotations(cls, api_server,
        obj_uuid, resource_type, annotation_key,
        annotation_value, fail_if_exists=True):
        """Check given annotation exists in the
           system. Consider availability as PASS/FAIL
    ...
    @classmethod
    def _add_annotations(cls, api_server,
        obj_uuid, resource_type, annotation_key,
        annotation_value):
    ...
    @classmethod
    def _check_unique_vn_vlan_in_vpg(cls, db_conn,
        api_server, vpg_uuid, vn_uuid,
        vlan_id, validation):
        ...
        ok, result = cls._check_annotations(api_server,
            vpg_uuid, 'virtual_port_group', ann_key,
    ...
    @classmethod
    def _add_unique_vn_vlan_in_vpg(cls, db_conn,
        api_server, vpg_uuid, vn_uuid,
        vlan_id, validation):
    ...
    @classmethod
    def _check_add_one_untagged_vlan(cls, db_conn,
        api_server, vpg_uuid, vlan_id, is_untagged,
        validation):
    ...
    @classmethod
    def _check_same_vn_vlan_exists(cls, db_conn,
        api_server, vn_uuid, vlan_id, fabric_uuid,
        validation):
        """Verify same VN and same VLAN-ID combination are
           associated to VPGs in a given fabric
           : Args
           :   db_conn: db connection object
           :   api_server: api_server connection object
           :   vn_uuid: UUID of virtual_network
        ...
        ok, fabric = cls.dbe_read(db_conn,
            'fabric', fabric_uuid,
    ...
    @classmethod
    def _check_serviceprovider_fabric(cls, db_conn,
        api_server, zk_vpg_lock_path, vpg_uuid,
        vn_uuid, vlan_id, is_untagged_vlan):
        """Verifies Service Provider Style Fabric's
           restrictions
           : Args
           :   db_conn: db connection object
           :   api_server: api_server connection object
        ...
        zk_vpg_args = {'zookeeper_client': db_conn._zk_db._zk_client,
            'path': zk_vpg_lock_path, 'name': vpg_uuid, 'timeout': 60}
        validation = 'serviceprovider'
        # VPG ZK Lock
        with ZookeeperLock(**zk_vpg_args) as vpg_zk_lock:
            ok, result = cls._check_unique_vn_vlan_in_vpg(db_conn,
                api_server, vpg_uuid, vn_uuid,
                vlan_id, validation)
            if not ok:
                return ok, result
            ok, result = cls._check_add_one_untagged_vlan(db_conn,
                api_server, vpg_uuid, vlan_id, is_untagged_vlan,
                validation)
            if not ok:
                return ok, result
            ok, result = cls._add_unique_vn_vlan_in_vpg(db_conn,
                api_server, vpg_uuid, vn_uuid,
                vlan_id, validation)
            if not ok:
                return ok, result
    ...
    @classmethod
    def _check_enterprise_fabric(cls, db_conn,
        api_server, zk_vpg_lock_path, zk_fabric_lock_path,
        vpg_uuid, fabric_uuid, vn_uuid, vlan_id,
        is_untagged_vlan):
        """Verifies Enterprise Style Fabric's
           restrictions
           : Args
           :   db_conn: db connection object
           :   api_server: api_server connection object
        ...
        zk_fab_args = {'zookeeper_client': db_conn._zk_db._zk_client,
            'path': zk_fabric_lock_path, 'name': vpg_uuid, 'timeout': 60}
        ...
        with ZookeeperLock(**zk_vpg_args) as vpg_zk_lock:
            ...
            if not ok:
                return ok, result
            # fabric ZK lock
            with ZookeeperLock(**zk_fab_args) as fab_zk_lock:
                ok, result = cls._check_same_vn_vlan_exists(db_conn,
                    api_server, vn_uuid, vlan_id, fabric_uuid,
                    validation)
                if not ok:
                    return ok, result
    ...
    def _manage_vpg_association(cls, vmi_id, api_server, db_conn, phy_links,
        ...
        if 'parent_uuid' in vpg_dict:
            ...
            fabric_enterprise_style = fabric.get('fabric_enterprise_style') or False
            fabric_style = ('enterprise' if fabric_enterprise_style else
                'serviceprovider')
            ...
            if fabric_enterprise_style:
                ok, result = cls._check_enterprise_fabric(db_conn,
                    api_server, zk_vpg_lock_path, zk_fabric_lock_path,
                    vpg_uuid, fabric_uuid, vn_uuid,
                    vlan_id, is_untagged_vlan)
                if not ok:
                    return ok, result
            else:
                ok, result = cls._check_serviceprovider_fabric(db_conn,
                    api_server, zk_vpg_lock_path, vpg_uuid,
                    vn_uuid, vlan_id, is_untagged_vlan)
                if not ok:
                    return ok, result
