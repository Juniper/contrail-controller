...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def _check_same_vn_vlan_exists_in_vpg(
        ...
        ok, vpg_annotations = cls.dbe_read(db_conn, 'virtual_port_group',
                                  vpg_uuid, obj_fields=['annotations'])
        if not ok:
            return ok, vpg_annotations

        # check if any vn or vlan matches
        for annotations in (vpg_annotations or {}):
