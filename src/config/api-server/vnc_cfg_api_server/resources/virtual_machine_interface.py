...
class VirtualMachineInterfaceServer(ResourceMixin, VirtualMachineInterface):
    ...
    def _check_serviceprovider_fabric(
        ...
        if is_untagged_vlan:  # untagged vlan
            try:
                self.db_conn._zk_db._zk_client.create_node(
                    untagged_validation_path, value=vmi_uuid, ephemeral=True)
            ...
            try:
                self.db_conn._zk_db._zk_client.create_node(
                    tagged_validation_path, value=vmi_uuid, ephemeral=True)
            except ResourceExistsError:
                value = self.db_conn._zk_db._zk_client.read_node(
        ...
        if result:  # ZNode creation successful
            ...
            (ok, result) = self.db_conn.dbe_create(
                obj_type, vmi_uuid, obj_dict)
            if not ok:
                return (ok, result)
