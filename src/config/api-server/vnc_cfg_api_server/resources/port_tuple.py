...
class PortTupleServer(ResourceMixin, PortTuple):
    ...
    def _delete_logical_interface(cls, dev_name, link, vlan_tag):
        ...
        db_conn.config_log(
                "iip_fqname: %s "%(iip_fqname), level=SandeshLevel.SYS_DEBUG)
        ...
        db_conn.config_log(
                "li_fqname: %s "%(li_fqname), level=SandeshLevel.SYS_DEBUG)
        try:
    ...
    def _create_logical_interface(cls, dev_name, link,
        ...
        try:
            ...
            db_conn.config_log(
                "iip_fqname: %s "%(iip_name), level=SandeshLevel.SYS_DEBUG)
