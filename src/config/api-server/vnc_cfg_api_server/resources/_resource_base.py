...
class ResourceMixin(object):
    ...
    def _pi_id_to_fqname(cls, db_conn, pi_uuid):
        try:
            return db_conn.uuid_to_fq_name('physical_interface', pi_uuid)
        except NoIdError:
            return None
    ...
    def _upd_job_transaction(cls, api_server, db_conn, trans_info,
        ...
        for pr_id in list(pr_id_set):
            ok, pr_dict = db_conn.dbe_read(
                obj_type='physical-router', obj_id=pr_id)
            if ok:
                updated = False
                kvps = None
                ...
                api_server.internal_request_update(
                    'physical_router', pr_id, {'annotations': annotations})
