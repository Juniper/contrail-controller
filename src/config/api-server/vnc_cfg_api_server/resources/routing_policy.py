...
class RoutingPolicyServer(ResourceMixin, RoutingPolicy):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        old_rpe = read_result.get('routing_policy_entries')
        if old_rpe:
            old_term_type = old_rpe.get('term_type')
            new_rpe = obj_dict.get('routing_policy_entries')
            if new_rpe and old_term_type:
                new_term_type = new_rpe.get('term_type')
                if not new_term_type or new_term_type != old_term_type:
                    return False, \
                           (403,
                            "Cannot change term_type of routing policy! "
