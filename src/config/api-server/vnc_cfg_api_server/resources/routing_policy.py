...
class RoutingPolicyServer(ResourceMixin, RoutingPolicy):
    ...
    def _check_network_device_routing_policy_term(cls, term, term_type,
        ...
        if match_condition:
            ...
            if protocol:
                ...
                if protocol not in allowed_protocol:
                    msg = ('%s protocol is not valid value for '
                           'network-device term' % protocol)
                    return False, (400, msg)
                    ...
                    if route_filter.get('route') is None:
                        ...
                        if route_type:
                            if route_type == 'prefix-length-range':
                                ...
                                res = None
                                if route_tvalue:
                                    res = re.match(r'^\/([1-9]|[1-2]\d|3[0-2])$',
                                                   route_tvalue)
                                if not res:
                                    msg = ("route-filter value %s invalid. %s "
                                           "should be in format '/number'" %
                                           (route_tvalue,route_type))
                                    return False, (400, msg)
    ...
    def _validate_term_and_get_asnlist(cls, obj_dict, db_conn):
        ...
        if rp_entries:
            term_type = rp_entries.get('term_type')
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        try:
            ...
            if not ok:
                ecode, emsg = result
                msg = "PARAG: ecode: %s, emsg: %s" % (str(ecode), str(emsg))
                db_conn.config_log(str(msg), level=SandeshLevel.SYS_DEBUG)
                # return False, result
        except Exception as e:
            msg = ("exception is %s" % str(e))
            db_conn.config_log(str(msg), level=SandeshLevel.SYS_DEBUG)
            pass
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ok, read_result = cls.dbe_read(db_conn, 'routing_policy', id,
                                       obj_fields=['term_type'])
        if not ok:
            return ok, read_result
        # do not allow change in term type
        if 'term_type' in obj_dict:
            if obj_dict['term_type'] != read_result.get('term_type'):
                return False, (403, "Cannot change term type of routing policy!")
