...
class LogicalRouterServer(ResourceMixin, LogicalRouter):
    ...
    def _check_annotations(cls, api_server, obj_uuid,
                           ...
                           fabric_uuid, fail_if_exists=True):
        ...
        for vn_ref in vn_refs:
            ...
            if is_shared:
                ok, result = cls._check_annotations(
                    ...
                    fabric_uuid, fail_if_exists=False)
    ...
    def _check_and_associate_logical_router_vn(cls, db_conn, obj_dict,
        ...
        if 'virtual_machine_interface_refs' in obj_dict:
            ...
            if fab_refs:
                fabric_uuid = fab_refs[-1].get('uuid')

                with ZookeeperLock(**zk_lr_args):
                    vn_not_in_another_lr, results = cls.is_vn_not_in_another_lr(
                        db_conn,
                        vn_refs,
                        api_server,
                        fabric_uuid
                    ...
                    ann_key = 'LogicalRouter'
                    exis_annotation_lrs = results.get(fabric_uuid, [])
                    exis_annotation_lrs.append(lr_fq_name_str)
                    results = results.update({fabric_uuid: exis_annotation_lrs})
                    ann_value = json.dumps(results)
                    for vn_ref in vn_refs:
            ...
            with ZookeeperLock(**zk_lr_args):
                if fab_uuid_to_be_del:
                    ...
                    vn_refs_to_be_deleted = set(existing_vn_refs) - set(vn_refs)
                    vn_refs_to_be_deleted = list(vn_refs_to_be_deleted)
                    cls._check_and_delete_vn_annotations(
                        obj_dict,
                        db_conn,
                        fab_uuid_to_be_del,
