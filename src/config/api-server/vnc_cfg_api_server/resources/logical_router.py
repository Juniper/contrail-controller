...
class LogicalRouterServer(ResourceMixin, LogicalRouter):
    ...
    @staticmethod
    def check_for_external_gateway(
          obj_dict,
          logical_router_type, logical_router_type_in_db):

        if(obj_dict.get('virtual_network_refs') and
           logical_router_type != 'snat-routing' and
           logical_router_type_in_db != 'snat-routing'):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        ok, result = cls.check_for_external_gateway(
                     obj_dict,
                     logical_router_type,
                     logical_router_type_in_db)
        if not ok:
            return ok, result

        if logical_router_type == 'vxlan-routing':
            # If we reach here, it means that an existing LR was upated
            # to support VXLAN functionality.
            # We will need to create a new Internal VN and return
    ...
    def post_dbe_update(cls, uuid, fq_name, obj_dict, db_conn,
        ...
        if logical_router_type == 'vxlan-routing':
            # If we reach here, it means that an existing LR used for VXLAN
