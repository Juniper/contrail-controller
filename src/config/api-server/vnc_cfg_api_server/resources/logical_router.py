...
class LogicalRouterServer(ResourceMixin, LogicalRouter):
    ...
    @staticmethod
    def validate_change_from_SNAT(db_conn, obj_dict, read_result):
        if ('virtual_network_refs' in obj_dict and
                obj_dict['virtual_network_refs'] != []):
            msg = 'Virtual_Network REF present even after change from SNAT'
            return (False, msg)

    @classmethod
    def validate_change_from_VXLAN(cls, db_conn, obj_dict, read_result):
        if read_result.get('virtual_network_refs'):
            ...
            ok, proj_dict = cls.get_parent_project(read_result, db_conn)
            ...
            def undo_int_vn_delete():
                return cls.create_intvn_and_ref(read_result, db_conn)
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        if (logical_router_type != logical_router_type_in_db and
            ...
            if logical_router_type_in_db == 'snat-routing':
                cls.validate_change_from_SNAT(db_conn, obj_dict, read_result)

            elif logical_router_type_in_db == 'vxlan-routing':
                cls.validate_change_from_VXLAN(db_conn, obj_dict, read_result)
    ...
    def post_dbe_update(cls, uuid, fq_name, obj_dict, db_conn,
        ...
        if logical_router_type == 'vxlan-routing':
            ...
            ok, result = cls.create_intvn_and_ref(read_result, db_conn)
            if not ok:
                return ok, result
