...
from vnc_cfg_api_server.resources import BgpvpnServer
...
class LogicalRouterServer(ResourceMixin, LogicalRouter):
    ...
    def check_for_external_gateway(cls, db_conn, obj_dict):
        if 'virtual_network_refs' in obj_dict:
            ...
            for ref in obj_dict.get('virtual_network_refs', []):
                if (ref.get('attr') and
                        ref['attr'].get(
                            'logical_router_virtual_network_type') and
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        ok, result = BgpvpnServer.check_router_supports_vpn_type(obj_dict)
        if not ok:
            return ok, result

        # Check if we can reference the BGP VPNs
        return BgpvpnServer.check_router_has_bgpvpn_assoc_via_network(obj_dict)
