...
class DataCenterInterconnectServer(TransactionResourceBase,
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        ok, result = cls._job_transaction_update(db_conn, "Create",
                                    obj_dict['fq_name'][-1],
                                    None, result)
        if not ok:
            return ok, result
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        ok, result = cls._job_transaction_update(db_conn, "Update",
                                    fq_name[-1],
                                    obj_dict, result)
        if not ok:
            return ok, result
    ...
    def pre_dbe_delete(cls, id, obj_dict, db_conn):
        ok, result = cls._job_transaction_update(db_conn, "Delete",
                                    obj_dict['fq_name'][-1],
                                    obj_dict, None)
        if not ok:
            return ok, result
