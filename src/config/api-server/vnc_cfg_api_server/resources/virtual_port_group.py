...
class VirtualPortGroupServer(ResourceMixin, VirtualPortGroup):
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        if obj_dict.get('virtual_port_group_trunk_id'):
            primary_vmi_id = obj_dict.get('virtual_port_group_trunk_id')
            ok, result = db_conn.dbe_read(
                         obj_type='virtual-machine_interface',
	    ...
	    if result.get('virtual_port_group_back_refs'):
		      ...
		      % (primary_vmi_id,
                      result.get('virtual_port_group_back_refs')[0]['to'][-1])
                return (ok, 409, msg)
