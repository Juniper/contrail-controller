...
class GlobalSystemConfigServer(ResourceMixin, GlobalSystemConfig):
    ...
    def _check_vn_route_targets(cls, global_asn):
        ...
        for vn in vn_list:
            ...
            route_targets.extend(vn.get('export_route_target_list', {})
                                 .get('route_target', []))
            for rt in cls._find_used_route_targets(route_targets, global_asn):
                found_vn_using_asn.append((':'.join(vn['fq_name']),
                                           vn['uuid'], rt))
        if not found_vn_using_asn:
            return True, ''
        ...
        for fq_name, uuid, rt in found_vn_using_asn:
    ...
    def _check_lr_route_targets(cls, global_asn):
        ...
        found_lr_using_asn = []
        for lr in lr_list:
            lr_rt_refs = lr.get('route_target_refs', [])
            route_targets = [rt['to'][0] for rt in lr_rt_refs]
            for rt in cls._find_used_route_targets(route_targets, global_asn):
                found_lr_using_asn.append((':'.join(lr['fq_name']),
                                           lr['uuid'], rt))

        if not found_lr_using_asn:
            return True, ''
        msg = ("Logical routers are configured with a route target with this "
               "ASN %d and route target value in the same range as used by "
               "automatically allocated route targets:\n" % global_asn)
        for fq_name, uuid, rt in found_lr_using_asn:
            msg += ("\t- %s (%s) have route target %s configured\n" %
                    (':'.join(fq_name), uuid, rt[7:]))
        return False, (400, msg)
