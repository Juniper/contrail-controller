...
class GlobalSystemConfigServer(ResourceMixin, GlobalSystemConfig):
    ...
    def _check_asn(cls, obj_dict):
        ...
        for vn in vn_list:
            for field in fields:
                route_targets = vn.get(field, {}).get('route_target', [])
                for rt in route_targets:
                    ok, result, _ = cls.server.get_resource_class(
                        'route_target').validate_route_target_range(rt,
                                                                    global_asn)
                    if not ok:
        ...
        for fq_name, uuid, field, rt in found_vn_using_asn:
