...
class PhysicalInterfaceServer(ResourceMixin, PhysicalInterface):
    ...
    def post_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        if interface_name[:2] == 'ae' and interface_name[2:].isdigit():
            ...
            if ae_id < 128 and cls.vnc_zk_client.ae_id_is_free(router_name,
                                                               ae_id):
                ...
                if alloc_id == ae_id:
                    ...
                    cls.vnc_zk_client.free_ae_id(router_name, interface_name,
                                                 alloc_id)
                    msg = ("Aggregated interface %s got wrong AE-ID %d - "
                           "removing allocation" % (interface_name, alloc_id))
    ...
    def post_dbe_delete(cls, id, obj_dict, db_conn):
        ...
        if interface_name[:2] == 'ae' and interface_name[2:].isdigit():
            ae_id = int(interface_name[2:])
            if ae_id < 128 and not cls.vnc_zk_client.ae_id_is_free(router_name,
                                                                   ae_id):
                cls.vnc_zk_client.free_ae_id(router_name, interface_name,
                                             ae_id)
                cls.vnc_zk_client.free_ae_id(router_name, interface_name,
    ...
    def _check_interface_name(cls, obj_dict, db_conn, vlan_tag):
        ...
        if interface_name[:2] == 'ae' and interface_name[2:].isdigit():
            ae_id = int(interface_name[2:])
            if ae_id < 128 and not cls.vnc_zk_client.ae_id_is_free(
                    router_name, ae_id):
                msg = ("Aggregated interface tried to use AE-ID %d which is "
                       "already occupied on this physical router" % ae_id)
                return False, (403, msg)
        try:
