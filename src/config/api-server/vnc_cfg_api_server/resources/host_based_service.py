...
class HostBasedServiceServer(ResourceMixin, HostBasedService):
    ...
    @staticmethod
    def _check_default_vn_delete(req_dict):
        ...
        if 'left' not in vn_type_set:
            msg = ("default hbf-left Virtual network  reference cannot be "
                   "removed")
            return False, (400, msg)
        if 'right' not in vn_type_set:
            msg = ("default hbf-right Virtual network  reference cannot be "
                   "removed")
            return False, (400, msg)
        return True, ''
    ...
    def _check_for_allowed_configure_vn_types(fq_name, uuid, req_dict):
        if is_internal_request():
            return True, ''
        vn_types = ['left', 'right', 'management']
        for ref in req_dict.get('virtual_network_refs', []):
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        ok, result = cls._check_default_vn_delete(obj_dict)
        if not ok:
            return ok, result
