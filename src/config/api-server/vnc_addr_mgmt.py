...
class Subnet(object):
    ...
    def __init__(self, name, prefix, prefix_len,
        ...
        for alloc_pool in alloc_pool_list:
            ...
            if (service_node_address >=start_ip and service_node_address <= end_ip):
                ...
                if ((int(service_node_address) - int(start_ip) >= ip_alloc_unit) and
                    (int(end_ip) - int(service_node_address) >= ip_alloc_unit) and 
                    (abs(int(gw_ip) - int(service_node_address)) > ip_alloc_unit)):
                    block_alloc_unit += 1
