...
class AddrMgmt(object):
    ...
    def _get_subnet_method(self, vn_uuid, vn_dict=None):
        db_conn = self._get_db_conn()
        subnet_type = None
        ...
        for ipam_ref in ipam_refs:
            ...
            if not ok:
                raise VncError(ipam_dict)
            ipam_subnet_type = ipam_dict.get('ipam_subnet_method')
            if (ipam_subnet_type is None):
                ipam_subnet_type = 'user-defined-subnet'
            if subnet_type is None:
                subnet_type = ipam_subnet_type
            else:    
                if (ipam_subnet_type != subnet_type):
    ...
    def ip_alloc_req(self, vn_fq_name, vn_dict=None, sub=None,
        ...
        if not vn_dict:
            #obj_fields=['network_ipam_refs']
            obj_fields=['uuid', 'network_ipam_refs']
            (ok, vn_dict) = self._fq_name_to_obj_dict('virtual_network',
                                                      vn_fq_name, obj_fields)
            if not ok:
                raise VncError(vn_dict)
    ...
    def ip_alloc_notify(self, ip_addr, vn_fq_name):
        ...
        if (self._ipam_ip_alloc_notify(ip_addr, vn_uuid) == False):
            return self._net_ip_alloc_notify(ip_addr, vn_uuid, vn_fq_name)
    # end ip_alloc_notify
    ...
    def ip_free_req(self, ip_addr, vn_fq_name, sub=None):
        ...
        self._net_ip_free_req(ip_addr, vn_uuid, vn_fq_name, sub)
    # end ip_free_req
    ...
    def is_ip_allocated(self, ip_addr, vn_fq_name, vn_uuid=None, sub=None):
        ...
        if self._ipam_is_ip_allocated(ip_addr, vn_uuid, sub):
            return True
        else:
    ...
    def ip_free_notify(self, ip_addr, vn_fq_name):
        ...
        self._net_ip_free_notify(ip_addr, vn_uuid)
    # end _ip_free_notify
    ...
    def ipam_update_req(self, ipam_fq_name, db_ipam_dict, req_ipam_dict,
        ...
        db_subnet_dicts = self._get_ipam_subnet_dicts(ipam_uuid, db_ipam_dict)
        req_subnet_dicts = self._get_ipam_subnet_dicts(ipam_uuid, req_ipam_dict)
