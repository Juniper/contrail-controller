...
class AddrMgmtSubnetUndefined(AddrMgmtError):
    ...
    def __str__(self):
        return "Virtual-Network(%s) has no defined subnet(s)" % (self.vn_fq_name)
    #end __str__
...
class AddrMgmtSubnetExhausted(AddrMgmtError):
    ...
    def __str__(self):
        return "Virtual-Network(%s) has exhausted subnet(%s)" % (self.vn_fq_name, self.subnet_val)
    #end __str__
...
class Subnet(object):
    ...
    incarnations of api server. These are also taken out of free pool to prevent
    ...
    def __init__(self, name, prefix, prefix_len, gw = None, db_conn = None):
        ...
        exclude = [IPAddress(network.first), IPAddress(network.last - 1), network.broadcast]
        if gw:
            gw_ip = IPAddress(gw)
        ...
        free_list = []
        for addr in network:
            if not addr in exclude and not self._inuse[int(addr)-network.first]:
    ...
    def ip_free(self, ipaddr):
        ip = IPAddress(ipaddr)
        if (ip in self._network and 
            ip not in self._free_list and ip not in self._exclude):
            if self._db_conn:
...
class AddrMgmt(object):
            ...
            for ref in refs:
                ...
                for ipam_subnet in ipam_subnets:
                    ...
                    if subnet_name not in self.vninfo[vn_fq_name_str]:
                        subnet = Subnet('%s:%s'%(vn_fq_name_str, subnet_name),
    ...
    def net_check_subnet_overlap(self, obj_dict):
        ...
        for index, subnet in enumerate(new_subnet_list):        
            ...
            while (next_index < size):
                if IPSet([str(subnet)]) & IPSet([str(new_subnet_list[next_index])]):
                    msg = new_subnet_list[index] + 'and' + new_subnet_list[next_index] + 'are overalap IP Blocks'
                    return False, msg
    ...
    def net_check_subnet_delete(self, obj_dict):
        ...
        if instip_refs:
            for ref in instip_refs:
                ...
                try:
                    ...
                    (ok, result) = db_conn.dbe_read('instance-ip', {'uuid': uuid})
                ...
                if not all_matching_cidrs(inst_ip, new_subnet_list):
                    return False, "Cannot Delete Ip Block, IP(%s) is in use" % (inst_ip)
        ...
        if fip_pool_refs:
            for ref in fip_pool_refs:
                ...
                try:
                    db_conn = self._db_conn
                    (ok, result) = db_conn.dbe_read('floating-ip-pool', {'uuid': uuid})
                except cfgm_common.exceptions.NoIdError:
                    msg = 'ID ' + uuid + ' not found'
                    return False, msg
                if not ok:
                ...
                if floating_ips:
                    for floating_ip in floating_ips:
                        ...
                        #new subnet_list
                        (read_ok, read_result) = db_conn.dbe_read('floating-ip', {'uuid': fip_uuid})
                        fip_addr = read_result.get('floating_ip_address', None)
                        if not fip_addr:
                            msg = 'ID' + fip_uuid + 'does not have floating_ip_address'
                            return False, msg

                        if not all_matching_cidrs(fip_addr, new_subnet_list):
                            return False, "Cannot Delete Ip Block, IP(%s) is in use" % (fip_addr)
    ...
    def mac_alloc(self, obj_dict):
        uid = obj_dict['uuid']
        return '02:%s:%s:%s:%s:%s' % (uid[0:2], uid[2:4], uid[4:6], uid[6:8], uid[9:11])
    #end mac_alloc
