...
def subnets_overlap(subnets):
    ...
    list of subnets.
    """
    # Sorted list of alternating start and end addresses
    ranges = []
    for subnet in subnets:
        ...
        # check the overlap conditions and return the overlapping subnet
...
class AddrMgmt(object):
    ...
    def net_check_subnet_overlap(self, req_vn_dict, db_vn_dict={}):
        ...
        overlaps = subnets_overlap(requested_subnets)
        if overlaps:
        ...
        overlaps = subnets_overlap(list(existing_set) + list(new_set))
        if overlaps:
