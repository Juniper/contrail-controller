        ...
        for ipam_ref in ipam_refs:
            ...
            for ipam_subnet in ipam_subnets:
                ...
                for pool in alloc_pools or []:
                    try:
                        start_ip = IPAddress(pool['start'])
                        end_ip = IPAddress(pool['end'])
                    except AddrFormatError:
                        err_msg = "Invalid Ip address in allocation Pool start:%s, end:%s" \
                            %(pool['start'], pool['end'])
                    ...
                    if (start_ip not in network or end_ip not in network):
                        ...
                        return False, err_msg
                    if (end_ip < start_ip):
                        err_msg = " Invalid allocation pool start:%s, end:%s" \
                            %(pool['start'], pool['end'])
                        return False, err_msg
