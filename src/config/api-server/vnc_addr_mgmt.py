...
class AddrMgmt(object):
    ...
    def _ipam_subnet_to_subnet_dict(self, ipam_subnet):
        ...
        subnet_dict['dns_server_address'] = \
            ipam_subnet.get('dns_server_address', None)
        ...
        subnet_dict['dns_nameservers'] = \
            ipam_subnet.get('dns_nameservers', None)
        subnet_dict['addr_start'] = ipam_subnet.get('addr_from_start', False)
        subnet_dict['subnet_uuid'] = ipam_subnet.get('subnet_uuid', None) 
        return subnet_dict
    #end

    def _uuid_to_obj_dict(self, req_obj_type, obj_uuid, req_fields=[]):
    ...
    def _fq_name_to_obj_dict(self, req_obj_type, fq_name, req_fields=[]):
        db_conn = self._get_db_conn()
    ...
    def _get_net_subnet_dicts(self, vn_fq_name, vn_dict=None):
        db_conn = self._get_db_conn()
        ...
        for ipam_ref in ipam_refs:
            ...
            subnet_method = ipam_dict.get('ipam_subnet_method') or None
            if subnet_method == 'flat-subnet':
                #if network is referred to flat-subnet ipam,
                # there will be dummy entry for subnet-uuid
                # which is not a valid configured subnet_dict
                continue

            vnsn_data = ipam_ref['attr']
            ipam_subnets = vnsn_data['ipam_subnets']
            for ipam_subnet in ipam_subnets:
                subnet_dict = self._ipam_subnet_to_subnet_dict(ipam_subnet)
    ...
    def _get_ipam_subnet_dicts(self, ipam_fq_name, ipam_dict=None):
        db_conn = self._get_db_conn()

        # Read in the ipam details if not passed in
        if not ipam_dict:
            (ok, ipam_dict) = self._fq_name_to_obj_dict('network_ipam', 
                                                        ipam_fq_name)
            if not ok:
                raise VncError(ipam_dict)
    ...
    def _create_subnet_objs(self, fq_name_str, ipam_subnets, should_persist):
        for ipam_subnet in ipam_subnets:
            subnet = ipam_subnet['subnet']
            subnet_name = subnet['ip_prefix'] + '/' + str(
            subnet['ip_prefix_len'])
            try:
                subnet_obj = self._subnet_objs[fq_name_str][subnet_name]
            except KeyError:
    ...
    def _create_ipam_subnet_objs(self, ipam_fq_name_str, ipam_dict, should_persist):
        self._subnet_objs.setdefault(ipam_fq_name_str, {})
        ipam_subnets_dict = ipam_dict.get('ipam_subnets')
        if ipam_subnets_dict:
            ipam_subnets = ipam_subnets_dict['subnets']
            self._create_subnet_objs(ipam_fq_name_str, ipam_subnets,
                                     should_persist)
    # end _create_ipam_subnet_objs

    def _create_net_subnet_objs(self, vn_fq_name_str, vn_dict, should_persist):
        self._subnet_objs.setdefault(vn_fq_name_str, {})
        # create subnet for each new subnet
        refs = vn_dict.get('network_ipam_refs', None)
        if refs:
            for ref in refs:
                if (self._ipam_is_flat_subnet(ref)):
                    continue
                ...
                if ipam_subnets:
                    self._create_subnet_objs(vn_fq_name_str, ipam_subnets,
                                             should_persist)
    ...
    def net_create_req(self, obj_dict):
        ...
        self._create_net_subnet_objs(vn_fq_name_str, obj_dict, should_persist=True)
    # end net_create_req
    ...
    def net_create_notify(self, obj_ids, obj_dict):
        ...
        self._create_net_subnet_objs(vn_fq_name_str, vn_dict, should_persist=False)
    ...
    def net_update_req(self, vn_fq_name, db_vn_dict, req_vn_dict, obj_uuid=None):
        ...
        db_subnet_dicts = self._get_net_subnet_dicts(vn_fq_name, db_vn_dict)
        req_subnet_dicts = self._get_net_subnet_dicts(vn_fq_name, req_vn_dict)
        ...
        self._create_net_subnet_objs(vn_fq_name_str, req_vn_dict, should_persist=True)
    # end net_update_req
    ...
    def net_delete_req(self, obj_dict):
        ...
        vn_fq_name_str = ':'.join(vn_fq_name)
        subnet_dicts = self._get_net_subnet_dicts(vn_fq_name)
    ...
    def _ipam_is_flat_subnet(self, ipam_ref):
        db_conn = self._get_db_conn()
        ipam_uuid = ipam_ref['uuid']
    ...
    # return subnet_method used for ip allocation for a given virtual network
    # return None or "flat-subnet" or "user-defined-subnet" as stored in
    # network_ipam or "hybird-subnet" if network is attached to multiple ipams
    # with different subnet type
    def _get_subnet_method(self, vn_fq_name, vn_dict=None):
        db_conn = self._get_db_conn()
        subnet_type = None
        ...
        for ipam_ref in ipam_refs:
            ...
            if not ok:
                raise VncError(ipam_dict)
            ipam_subnet_type = ipam_dict.get('ipam_subnet_method', None)
            if (ipam_subnet_type != None):
                if ((ipam_subnet_type != subnet_type) and \
                    (subnet_type != None)):
                    subnet_type = 'hybrid-subnet'
                else:
                    subnet_type = ipam_subnet_type

        # if ipam is without ipam_subnet_method, consider default
        # 'user-defined-subnet
        if (subnet_type == None):
            subnet_type = 'user-defined-subnet' 
        return subnet_type
    # end _get_subnet_method

    def _check_subnet_in_user_define_ipam(self, vn_fq_name, vn_dict, sub):
        db_conn = self._get_db_conn()
        # Read in the VN details if not passed in
        if not vn_dict:
            obj_fields=['network_ipam_refs']
        ...
        for ipam in ipam_refs:
            ...
            for ipam_subnet in ipam_subnets:
                ipam_subnet_uuid = ipam_subnet.get('subnet_uuid', None)
                if (ipam_subnet_uuid != None) and\
                   (ipam_subnet_uuid == sub):
                    return True;
        return False
    #end _check_subnet_in_user_define_ipam

    def _check_subnet_in_flat_ipam(self, vn_fq_name, vn_dict, subnet_uuid):
        db_conn = self._get_db_conn()
        ipam_refs = vn_dict.get('network_ipam_refs', [])
        #special uuid identifying flat-subet, which is stored in
        #vn->ipam link's VnSubnetType
        for ipam in ipam_refs:
            vnsn_data = ipam['attr']
            ipam_subnets = vnsn_data.get('ipam_subnets', [])
            if len(ipam_subnets) is 0:
            ...
            first_ipam_subnet = ipam_subnets[0]
            subnet = first_ipam_subnet.get('subnet', {})
            if ((len(ipam_subnets) == 1) and ('ip_prefix' not in subnet) and \
                ('subnet_uuid' in first_ipam_subnet)):
                for ipam_subnet in ipam_subnets:
                    ipam_subnet_uuid = ipam_subnet.get('subnet_uuid', None)
                    if (ipam_subnet_uuid != None) and\
                       (ipam_subnet_uuid == subnet_uuid):
                        return True;
        return False
    #end _check_subnet_in_flat_ipam

    def _get_subnet_obj(self, fq_name_str, subnet_name, subnet_dict):
        try:
            subnet_obj = self._subnet_objs[fq_name_str][subnet_name]
        except KeyError:
            if fq_name_str not in self._subnet_objs:
                self._subnet_objs[fq_name_str] = {}

            subnet_obj = Subnet('%s:%s' % (fq_name_str,
                                           subnet_name),
    ...
    def _ipam_ip_alloc_req(self, vn_fq_name, vn_dict=None, sub=None,
        ...
        ipam_refs = vn_dict['network_ipam_refs']
        current_ipam_count = 0
        ipam_refs_count = len(ipam_refs)
        for ipam_ref in ipam_refs:
            current_ipam_count += 1
            ...
            if ('ip_prefix' in subnet):
                ...
                if current_ipam_count < ipam_refs_count:
                    continue
                else:
            ...
            subnet_dicts = self._get_ipam_subnet_dicts(ipam_fq_name)
            current_count = 0
            ...
            for subnet_name in subnet_dicts:
                ...
                subnet_obj = self._get_subnet_obj(ipam_fq_name_str,
                ...
                # ignore exception if it not a last subnet in last ipam_refs
                    if current_count < subnet_count:
                        continue
                    if current_ipam_count < ipam_refs_count:
                        continue
                    self.config_log("In ipam_ip_alloc_req: %s" %(str(e)),
    ...
    def _net_ip_alloc_req(self, vn_fq_name, vn_dict=None, subnet_uuid=None,
                          ...
                          alloc_id=None):
        vn_fq_name_str = ':'.join(vn_fq_name)
        subnet_dicts = self._get_net_subnet_dicts(vn_fq_name, vn_dict)
        ...
        for subnet_name in subnet_dicts:
            ...
            # api-server before
            subnet_obj = self._get_subnet_obj(vn_fq_name_str, subnet_name, subnet_dict)
            if asked_ip_version and asked_ip_version != subnet_obj.get_version():
                continue
            if asked_ip_addr == str(subnet_obj.gw_ip):
                return asked_ip_addr
    ...
    def ip_alloc_req(self, vn_fq_name, vn_dict=None, sub=None,
        ...
        #if subnet_uuid is given, first found out where subnet is present
        # in ipam for flat-subnet or in link for user-defined-subnet
        if sub:
            if (self._check_subnet_in_user_define_ipam(vn_fq_name,
                                                       vn_dict, sub)):
                return self._net_ip_alloc_req(vn_fq_name, vn_dict, sub,
                                              asked_ip_addr, asked_ip_version,
                                              alloc_id)
            if (self._check_subnet_in_flat_ipam(vn_fq_name,
                                                     vn_dict, sub)):
        ...
        subnet_method = self._get_subnet_method(vn_fq_name, vn_dict)
        allocation_method = vn_dict.get('address_allocation_mode') or None
        ...
        # Raise exception if subnet_type and allocation_method
        # are out of sync
            ...
            if allocation_method == 'user-defined-subnet-only':
                raise AddrMgmtSubnetUndefined(vn_fq_name_str)
                 ...
                 return self._ipam_ip_alloc_req(vn_fq_name, vn_dict, sub,
                                                asked_ip_addr, asked_ip_version,
                                                alloc_id)
        # This is a hybrid subnet-method where VN is attached to
        # both types of ipam (flat and user-defined)
        # allocation method will decide which subnets to look for allocation
        if asked_ip_addr != None:
            try:
                return self._ipam_ip_alloc_req(vn_fq_name, vn_dict, sub,
                                               asked_ip_addr, asked_ip_version,
                                               alloc_id)
    ...
    def _ipam_ip_alloc_notify(self, ip_addr, vn_fq_name):
        db_conn = self._get_db_conn()
        ...
        for ipam_ref in ipam_refs:
            ...
            subnet_dicts = self._get_ipam_subnet_dicts(ipam_fq_name, ipam_dict)
            for subnet_name in subnet_dicts:
    ...
    def net_ip_alloc_notify(self, ip_addr, vn_fq_name):
        vn_fq_name_str = ':'.join(vn_fq_name)
        try:
            subnet_dicts = self._get_net_subnet_dicts(vn_fq_name)
        except cfgm_common.exceptions.NoIdError:
    ...
    def ip_alloc_notify(self, ip_addr, vn_fq_name):
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_ip_alloc_notify(ip_addr, vn_fq_name)

        return self._net_ip_alloc_notify(ip_addr, vn_fq_name)
    # end ip_alloc_notify


    def _ipam_ip_free_req(self, ip_addr, vn_fq_name, sub=None):
        db_conn = self._get_db_conn()
        ...
        for ipam_ref in ipam_refs:
            ...
            for subnet_name in subnet_dicts:
                if sub and sub != subnet_name:
                    continue
                try:
                    subnet_obj = self._subnet_objs[ipam_fq_name_str][subnet_name]
                except KeyError:
                    if ipam_fq_name_str not in self._subnet_objs:
    ...
    def _net_ip_free_req(self, ip_addr, vn_fq_name, sub=None):
        vn_fq_name_str = ':'.join(vn_fq_name)
        subnet_dicts = self._get_net_subnet_dicts(vn_fq_name)
        for subnet_name in subnet_dicts:
            if sub and sub != subnet_name:
    ...
    def ip_free_req(self, ip_addr, vn_fq_name, sub=None):
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_ip_free_req(ip_addr, vn_fq_name, sub=None)

        if (subnet_method == "user-defined-subnet"):
            return self._net_ip_free_req(ip_addr, vn_fq_name, sub=None)

        #ip alloc was done for hybrid subnet where network is
        # attached to multiple ipams with different ipam_subnet_method
        # we can not assume allocation from specific set of cidrs
        # we should call both ip free for both cases
        self._ipam_ip_free_req(ip_addr, vn_fq_name, sub=None)
        self._net_ip_free_req(ip_addr, vn_fq_name, sub=None)

    # end ip_free_req

    def _ipam_is_ip_allocated(self, ip_addr, vn_fq_name, sub=None):
        db_conn = self._get_db_conn()
        ...
        for ipam_ref in ipam_refs:
            ...
            for subnet_name in subnet_dicts:
                ...
                try:
                    ...
                    if ipam_fq_name_str not in self._subnet_objs:
                        self._subnet_objs[ipam_fq_name_str] = {}

                    subnet_dict = subnet_dicts[subnet_name]
                    subnet_obj = Subnet('%s:%s' % (ipam_fq_name_str,
                                                   subnet_name),
                                        subnet_dict['ip_prefix'],
                                        subnet_dict['ip_prefix_len'],
                                        gw=subnet_dict['gw'],
                                        ...
                                        addr_from_start = subnet_dict['addr_start'],
                                        should_persist=False)
                    self._subnet_objs[ipam_fq_name_str][subnet_name] = subnet_obj
    ...
    def _net_is_ip_allocated(self, ip_addr, vn_fq_name, sub=None):
        vn_fq_name_str = ':'.join(vn_fq_name)
        subnet_dicts = self._get_net_subnet_dicts(vn_fq_name)
        for subnet_name in subnet_dicts:
            if sub and sub != subnet_name:
    ...
    def is_ip_allocated(self, ip_addr, vn_fq_name, sub=None):
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_is_ip_allocated(ip_addr, vn_fq_name, sub)

        if (subnet_method == "user-defined-subnet"):
            return self._net_is_ip_allocated(ip_addr, vn_fq_name, sub)

        # This is a hybrid case, where we will have multiple ipams
        # and possibly ip might have come from any of the ipams.
        # First try ipam then network cidrs
        if self._ipam_is_ip_allocated(ip_addr, vn_fq_name, sub):
            return True
        else:
            return self._net_is_ip_allocated(ip_addr, vn_fq_name, sub)
    ...
    def _ipam_ip_free_notify(self, ip_addr, vn_fq_name):
        db_conn = self._get_db_conn()
        ...
        for ipam_ref in ipam_refs:
            ...
            for subnet_name in self._subnet_objs.get(ipam_fq_name_str) or []:
                subnet_obj = self._subnet_objs[vn_fq_name_str][subnet_name]
                if Subnet.ip_belongs_to(IPNetwork(subnet_name),
                                        IPAddress(ip_addr)):
    ...
    def _net_ip_free_notify(self, ip_addr, vn_fq_name):
        vn_fq_name_str = ':'.join(vn_fq_name)
    ...
    def ip_free_notify(self, ip_addr, vn_fq_name):
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_ip_free_notify(ip_addr, vn_fq_name)

        return self._net_ip_free_notify(ip_addr, vn_fq_name)
    # end _ip_free_notify
    ...
    def ipam_create_req(self, obj_dict):
        ...
        if subnet_method is not None and subnet_method == 'flat-subnet':
            self._create_ipam_subnet_objs(ipam_fq_name_str, obj_dict,
                                          should_persist=True)
    # end ipam_create_req
    ...
    def ipam_create_notify(self, obj_ids, obj_dict):
        ...
        if not ok:
            db_conn.config_log("Error: %s in ipam_create_notify" %(result),
                               level=SandeshLevel.SYS_ERR)
            return

        ipam_dict = result
        if 'ipam_subnet_method' in ipam_dict:
            if ipam_dict['ipam_subnet_method'] is not None:
                if (ipam_dict['ipam_subnet_method'] == 'flat-subnet'):

                    ipam_fq_name_str = ':'.join(ipam_dict['fq_name'])
                    self._create_ipam_subnet_objs(ipam_fq_name_str, ipam_dict,
                                                  should_persist=False)
    ...
    def ipam_delete_req(self, obj_dict):
        ...
        try:
            del self._subnet_objs[ipam_fq_name_str]
        except KeyError:
            pass
    # end ipam_delete_req
    ...
    def ipam_update_req(self, ipam_fq_name, db_ipam_dict, req_ipam_dict,
        ...
        db_subnet_dicts = self._get_ipam_subnet_dicts(ipam_fq_name, db_ipam_dict)
        req_subnet_dicts = self._get_ipam_subnet_dicts(ipam_fq_name, req_ipam_dict)
        ...
        for subnet_name in del_subnet_names:
            Subnet.delete_cls('%s:%s' % (ipam_fq_name_str, subnet_name))
            try:
                del self._subnet_objs[ipam_fq_name_str][subnet_name]
            except KeyError:
                pass
        ...
        self._create_ipam_subnet_objs(ipam_fq_name_str, req_ipam_dict, should_persist=True)
    # end ipam_update_req
    ...
    def ipam_update_notify(self, obj_ids):
        ...
        ipam_dict = result
        ipam_fq_name_str = ':'.join(ipam_dict['fq_name'])
        self._create_net_subnet_objs(ipam_fq_name_str, ipam_dict,
                                     should_persist=False)
    # end ipam_update_notify

    def _ipam_is_gateway_ip(self, vn_fq_name, vn_dict, ip_addr):
        db_conn = self._get_db_conn()
        ipam_refs = vn_dict['network_ipam_refs']
        for ipam_ref in ipam_refs:
            ipam_fq_name = ipam_ref['to']
    ...
    def _net_is_gateway_ip(self,  vn_fq_name, vn_dict, ip_addr):
        ipam_refs = vn_dict.get('network_ipam_refs', [])
        for ipam in ipam_refs:
    ...
    def is_gateway_ip(self, vn_dict, ip_addr):
        ...
        vn_fq_name = vn_dict['fq_name']
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_is_gateway_ip(vn_fq_name, vn_dict, ip_addr)

        return self._net_is_gateway_ip(vn_fq_name, vn_dict, ip_addr)
    # end is_gateway_ip
# end class AddrMgmt
