...
class AddrMgmt(object):
    ...
    def _validate_allocation_pool_change(self, req_alloc_list,
                                         db_alloc_list):
        first_new_idx = len(db_alloc_list)
        for index in range(first_new_idx, len(req_alloc_list)):
            alloc_pool = req_alloc_list[index]
            start_ip = IPAddress(alloc_pool['start'])
            end_ip = IPAddress(alloc_pool['end'])
            # make sure new alloc pool is not overlapping with any of the
            # db_alloc_pool
            for index in range(len(db_alloc_list)):
                ...
                if (start_ip >= db_start_ip) and (start_ip <= db_end_ip):
                    return False
                if (end_ip >= db_start_ip) and (end_ip <= db_end_ip):
                    return False

        return True
    # end _validate_allocation_pool_change
    ...
    def _validate_subnet_alloc_pools(self, req_subnet, alloc_pool_list=[]):
        ...
        for index in range(len(alloc_pool_list)):
            ...
            for idx in range(index+1, len(alloc_pool_list)):
    ...
    def _check_subnet_alloc_pools(self, req_subnet, db_subnet={}):
        ...
        subnet_with_pool = None
        if (len(req_alloc_list) > len(db_alloc_list)):
            if not (self._validate_allocation_pool_change(req_alloc_list,
                                                          db_alloc_list)):
                return (False, None)
            subnet_name = req_subnet['ip_prefix'] + '/' + str(
                              req_subnet['ip_prefix_len'])
            subnet_with_pool = {'subnet_name': subnet_name,
                                'alloc_pool': req_alloc_list}

        return (True, subnet_with_pool)
                ...
                for subnet_alloc_pool in alloc_pool_change:
                    if subnet_alloc_pool['subnet_name'] == subnet_name:
                        ...
                        for alloc_pool in new_alloc_list:
                            alloc_pool.pop('vrouter_specific_pool', None)
                            alloc_pool['start'] = int(IPAddress(alloc_pool['start']))
                            alloc_pool['end'] = int(IPAddress(alloc_pool['end']))
                        self._db_conn.subnet_change_allocator(
                            allocator_name, new_alloc_list,
