...
class AddrMgmt(object):
    ...
    def _create_subnet_obj_for_subnet_dict(self, fq_name_str, subnet_name,
                                           subnet_dict):
        subnet_obj = Subnet(
    ...
    def _get_ipam_subnet_objs_from_ipam_uuid(self, ipam_fq_name,
                                             ipam_uuid=None, should_persist=False): 
        ...
        subnet_objs = None
        for key in self._subnet_objs:
            if key == ipam_uuid:
                subnet_objs = self._subnet_objs[key]
                break
    ...
    def _create_subnet_objs(self, fq_name_str, obj_uuid, ipam_subnets,
                            should_persist):
        #need to do this before submitting review -- Atul
        for ipam_subnet in ipam_subnets:
            subnet = ipam_subnet['subnet']
            subnet_name = subnet['ip_prefix'] + '/' + str(
            subnet['ip_prefix_len'])
    ...
    def _create_net_subnet_objs(self, vn_fq_name_str, vn_dict, should_persist):
        ...
        if refs:
            for ref in refs:
                ...
                # and vn, if link has only one subnet with one key in 
                # in subnet dict ('subnet_uuid'), that is a flat-subnet ipam
    ...
    def _get_subnet_obj(self, fq_name_str, obj_uuid, subnet_name, subnet_dict):
        try:
            ...
            subnet_obj = self._create_subnet_obj_for_subnet_dict(
                             self, fq_name_str, subnet_name, subnet_dict)
            self._subnet_objs[obj_uuid][subnet_name] = subnet_obj
        return subnet_obj
    # end _get_subnet_obj
    ...
    def _net_ip_free_req(self, ip_addr, vn_fq_name, sub=None):
        ...
        for subnet_name in subnet_dicts:
            ...
            try:
                ...
                if vn_uuid not in self._subnet_objs:
                    self._subnet_objs[vn_uuid] = {}

                subnet_dict = subnet_dicts[subnet_name]
                subnet_obj = self._create_subnet_obj_for_subnet_dict(
                    self, vn_fq_name_str, subnet_name, subnet_dict)
                self._subnet_objs[vn_uuid][subnet_name] = subnet_obj
    ...
    def _ipam_is_ip_allocated(self, ip_addr, vn_fq_name, vn_uuid, sub=None):
        db_conn = self._get_db_conn()
    ...
    def _net_is_ip_allocated(self, ip_addr, vn_fq_name, vn_uuid=None, sub=None):
        db_conn = self._get_db_conn()
        vn_fq_name_str = ':'.join(vn_fq_name)
        subnet_dicts = self._get_net_subnet_dicts(vn_fq_name)
        if vn_uuid is None:
    ...
    def is_ip_allocated(self, ip_addr, vn_fq_name, vn_uuid=None, sub=None):
        ...
        subnet_method = self._get_subnet_method(vn_fq_name)
        if (subnet_method == "flat-subnet"):
            return self._ipam_is_ip_allocated(ip_addr, vn_fq_name, vn_uuid, sub)
        ...
        if self._ipam_is_ip_allocated(ip_addr, vn_fq_name, vn_uuid, sub):
            return True
        else:
            return self._net_is_ip_allocated(ip_addr, vn_fq_name, vn_uuid, sub)
    ...
    def ipam_update_req(self, ipam_fq_name, db_ipam_dict, req_ipam_dict,
        ...
        self._create_ipam_subnet_objs(ipam_fq_name_str, req_ipam_dict,
                                      should_persist=True)
    # end ipam_update_req
