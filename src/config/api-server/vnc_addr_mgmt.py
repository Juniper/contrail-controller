...
class AddrMgmt(object):
    ...
    def _get_ipam_subnet_objs_from_ipam_uuid(self, ipam_fq_name,
        ...
        if (subnet_objs is None and not ipam_dict):
    ...
    def get_ip_free_args(self, vn_fq_name, vn_dict=None):
        ...
        if the api to cassandra connection is down/falpping.
        LP# 1749294
        """
        db_conn = self._get_db_conn()
        kwargs = {}
        if not vn_dict:
            ...
            (ok, vn_dict) = self._uuid_to_obj_dict(
                                 ...
                                 req_fields)
            if not ok:
                return (False, (500, 'Bad VN error : ' + pformat(vn_dict)))
        ...
        for ipam_ref in vn_dict.get('network_ipam_refs', []):
            ...
            if not ok:
                return (False, (500, 'Bad IPAM error : ' + pformat(vn_dict)))
    ...
    def ip_free_req(self, ip_addr, vn_fq_name, alloc_id=None,
            sub=None, vn_dict=None, ipam_dicts=None):
        db_conn = self._get_db_conn()
        if not vn_dict:
            vn_uuid = db_conn.fq_name_to_uuid('virtual_network', vn_fq_name)
        else:
        ...
        if not (self._net_ip_free_req(
            ...
            self._ipam_ip_free_req(ip_addr, vn_uuid, sub, vn_dict, ipam_dicts)
    # end ip_free_req
