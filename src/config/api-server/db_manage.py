...
class DatabaseChecker(object):
    def __init__(self, args_str):
        ...
        keyspace_column_families = {
            'config_db_uuid': ['obj_fq_name_table', 'obj_uuid_table'],
            'useragent': ['useragent_keyval_table'],
        }
        rd_consistency = pycassa.cassandra.ttypes.ConsistencyLevel.QUORUM
        self._cf_dict = {}
        for ks_name in keyspace_column_families.keys():
            pool = pycassa.ConnectionPool(keyspace=ks_name,
                       server_list=self._cassandra_servers, prefill=False)
            for cf_name in keyspace_column_families[ks_name]:
    ...
    def check_cassandra_keyspace_replication(self):
        ...
        for server in self._cassandra_servers:
            ...
            for ks_name in ['config_db_uuid', 'useragent', 'to_bgp_keyspace',
                            'DISCOVERY_SERVER', 'svc_monitor_keyspace']:
                logger.debug("Reading keyspace properties for %s on %s: ",
                             ks_name, server)
                ks_prop = sys_mgr.get_keyspace_properties('config_db_uuid')
    ...
    def check_subnet_addr_alloc(self):
        ...
        self._addr_alloc_process_ip_objects(cassandra_all_vns,
