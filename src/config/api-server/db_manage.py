...
class DatabaseManager(object):
    ...
    def audit_route_targets_id(self):
        ...
        for rtgt_id in self._zk_client.get_children(base_path) or []:
            ri_fq_name_str = self._zk_client.get(base_path+'/'+rtgt_id)[0]
            zk_all_rtgts[int(rtgt_id)] = ri_fq_name_str
        ...
        for rtgt_uuid in rtgt_uuids:
            ...
            rtgt_id = json.loads(rtgt_cols['fq_name'])[0].split(':')[-1]
        ...
        logger.debug("Got %d route-targets with id in cassandra",
            len(cassandra_all_rtgts))
        zk_set = set([(id, ri_fqn) for id, ri_fqn in zk_all_rtgts.items()])
        #cassandra_set = set([(int(id)-self.AUTO_RTGT_START, fqns)
        #    for id, fqns in cassandra_all_rtgts.items()
        #    if int(id) >= self.AUTO_RTGT_START])
        cassandra_set = set([(int(id), fqns)
            for id, fqns in cassandra_all_rtgts.items()])
...
class DatabaseChecker(DatabaseManager):
    ...
    def check_fq_name_uuid_ifmap_match(self):
        ...
        obj_uuid_table = self._cf_dict['obj_uuid_table']
        fq_name_table_all = []
        mandatory_fields_missing_uuids = []
        ...
        for obj_type, cols in obj_fq_name_table.get_range(column_count=MAX_COL):
            logger.info("Checking for type %s" %(obj_type))
            for fq_name_str_uuid in cols:
                ...
                try:
                    ...
                    obj_fq_name_json = obj_cols.get('fq_name')
                    if not obj_fq_name_json:
                        logger.info("Ignoring fq_name_str_uuid %s" %(fq_name_str_uuid))
                        continue
                    obj_fq_name_str = ':'.join(json.loads(obj_fq_name_json))
        ...
        for obj_uuid, cols in obj_uuid_table.get_range(column_count=MAX_COL):
            try:
                ...
                mandatory_fields_missing_uuids.append(obj_uuid)
        ...
        for extra in set(fq_name_table_all) - set(uuid_table_all):
            ...
            if obj_uuid in mandatory_fields_missing_uuids:
                continue
            ret_errors.append(FQNStaleIndexError(
    ...
    def check_obj_mandatory_fields(self):
        ...
        for obj_uuid, cols in obj_uuid_table.get_range(column_count=MAX_COL):
            ...
            for col_name in self.OBJ_MANDATORY_COLUMNS:
                ...
                num_bad_objs += 1
                ret_errors.append(MandatoryFieldsMissingError(
                    'Error, obj %s missing column %s only columns %s' %(obj_uuid, col_name, cols)))
    ...
    def check_route_targets_id(self):
        ...
        extra_rtgt_ids = set([e[0] for e in zk_set]) - set([e[0] for e in cassandra_set])
        for rtgt_id in extra_rtgt_ids:
            errmsg = 'Extra route target ID in zookeeper for RTgt %s' \
                    %(rtgt_id)
            ret_errors.append(ZkRTgtIdExtraError(errmsg))

        extra_rtgt_ids = set([e[0] for e in cassandra_set if e[0] >= self.AUTO_RTGT_START]) - set([e[0] for e in zk_set])
        for rtgt_id in extra_rtgt_ids:
            errmsg = 'Missing route target ID in zookeeper for RTgt %s' \
                    %(rtgt_id)
