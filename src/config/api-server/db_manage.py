...
in that header:
* 1.6:
  - fix TypeError issue releated to StringIO
* 1.5:
...
exceptions = [
    ...
    'ZkRTgtIdExtraError',
    'OrphanResourceError',
    'IfmapMissingIdentifiersError',
    'IfmapExtraIdentifiersError',
    'ZkSubnetPathInvalid',
...
class DatabaseManager(object):
    ...
    def _connect_to_ifmap_servers(self):
        ...
        if self._args.ifmap_servers:
            ...
            if (self._api_args.ifmap_listen_ip is not None and
                    self._api_args.ifmap_listen_port is not None):
                ifmap_ips_ports = [(self._api_args.ifmap_listen_ip,
                                    self._api_args.ifmap_listen_port)]
            else:
        ...
        if self._args.ifmap_credentials:
            ...
            if (self._api_args.ifmap_username is not None and
                    self._api_args.ifmap_password is not None):
                ifmap_user =  self._api_args.ifmap_username
                ifmap_passwd = self._api_args.ifmap_password
            else:
                ifmap_user, ifmap_passwd = self._api_args.ifmap_credentials[0]

        for ifmap_ip, ifmap_port in ifmap_ips_ports:
            mapclient = ifmap_client((ifmap_ip, ifmap_port),
            ...
            if retry >= 3:
                self._logger.warning("Cannot connect to the IF-MAP server: "
                                     "%s/%s", ifmap_ip, ifmap_port)
            else:
                mapclient.set_session_id(
                    newSessionResult(result).get_session_id())
                mapclient.set_publisher_id(
                    newSessionResult(result).get_publisher_id())
                self._mapclients.append(mapclient)

        if not self._mapclients:
            sys.exit("Cannot connect to any of IF-MAP server(s) provided: %s" %
                     ifmap_ips_ports)
...
class DatabaseChecker(DatabaseManager):
    ...
    def check_fq_name_uuid_ifmap_match(self):
        ...
        logger.debug("Got %d objects from cassandra UUID table",
                     len(uuid_table_all))
