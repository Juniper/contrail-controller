...
class VncApiServer(VncApiServerGen):
    ...
    def _set_api_audit_info(self, apiConfig):
        ...
        apiConfig.user = bottle.headers.get('X-User-Name')
        apiConfig.project = bottle.headers.get('X-Project-Name')
        apiConfig.domain = bottle.headers.get('X-Domain-Name')
        if int(bottle.headers.get('Content-Length', 0)) > 0:
