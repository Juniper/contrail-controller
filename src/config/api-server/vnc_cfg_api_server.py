...
from sandesh_common.vns.constants import ModuleNames, Module2NodeType,\
    NodeTypeNames, INSTANCE_ID_DEFAULT, TagTypeNameToId, TagType,\
...
class VncApiServer(object):
    ...
    def __init__(self, args_str=None):
        ...
        # Set/unset Tag actions
        self.route('/set-tag', 'POST', self.set_tag)
        self._homepage_links.append(
            LinkObject('action', self._base_url,  '/set-tag', 'set-tag',
                       'POST'))
        self.route('/unset-tag', 'POST', self.unset_tag)
        self._homepage_links.append(
            LinkObject('action', self._base_url,  '/unset-tag', 'unset-tag',
                       'POST'))
   ...
   # end
    ...
    def _db_init_entries(self):
        ...
        for type_str, type_id in TagTypeNameToId.items():
            tag = TagType(name=type_str, id=type_id)
    ...
    def set_tag(self):
        ...
        for tag_type, attrs in req_dict.items():
            ...
            if tag_type not in TAG_TYPE_NOT_UNIQUE_PER_OBJECT:
                ...
                for value in ((set([value]) | add_values) -
                              set(refs_per_values.keys())):
                ...
                for value in delete_values & set(refs_per_values.keys()):
