...
class VncApiServer(object):
    ...
    def _validate_refs_in_request(self, resource_class, obj_dict):
        for ref_obj_type in resource_class.ref_fields:
            _, ref_link_type, _ = resource_class.ref_field_types[ref_obj_type]
            if ref_link_type == 'None':
                continue
            for ref_dict in obj_dict.get(ref_obj_type) or []:
    ...
    def _validate_perms_in_request(self, resource_class, obj_type, obj_dict):
        for ref_obj_type in resource_class.ref_fields:
            for ref in obj_dict.get(ref_obj_type) or []:
                ref_uuid = self._db_conn.fq_name_to_uuid(ref_obj_type,
                                                         ref['to'])
    ...
    def http_resource_create(self, obj_type):
        ...
        obj_dict = get_request().json[obj_type]
    ...
    def create_default_children(self, object_type, parent_obj):
        ...
        for child_obj_type in r_class.children_fields:
            # Create a default child only if provisioned for
            _, is_derived = r_class.children_field_types[child_obj_type]
            if is_derived:
                continue
            child_cls = self.get_resource_class(child_obj_type)
            ...
            if not ok:
                ...
                err_msg = "DB Create failed creating %s" % child_obj_type
                self.config_log(err_msg, level=SandeshLevel.SYS_ERR)
                return (ok, result)
    ...
    def _generate_resource_crud_methods(cls, obj):
        for object_type, _ in all_resource_types:
    ...
    def __init__(self, args_str=None):
        ...
        for _, resource_type in all_resource_types:
   ...
   # end
    ...
    def ref_update_http_post(self):
        self._post_common(get_request(), None, None)
        # grab fields
        res_type = get_request().json.get('type')
        res_class = self.get_resource_class(res_type)
        obj_uuid = get_request().json.get('uuid')
        ref_res_type = get_request().json.get('ref-type')
        ...
        ref_type = ref_class.object_type
        if not ref_uuid:
            try:
    ...
    def fq_name_to_id_http_post(self):
        self._post_common(get_request(), None, None)
        r_class = self.get_resource_class(get_request().json['type'])
    ...
    def get_resource_class(self, type):
        if type in self._resource_classes:
            return self._resource_classes[type]

        common_name = cfgm_common.utils.CamelCase(type)
    ...
    def list_bulk_collection_http_post(self):
        ...
        return self._list_collection(r_class.obj_type, parent_uuids,
    ...
    def _ensure_perms2_present(self, obj_uuid, obj_dict):
        """
        Called at resource creation to ensure that id_perms is present in obj
        """
        # retrieve object and permissions
    ...
    def _http_post_common(self, request, obj_type, obj_dict):
        ...
        self._ensure_perms2_present(None, obj_dict)
