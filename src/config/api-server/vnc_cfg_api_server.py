...
class VncApiServer(object):
    ...
    def _validate_complex_type(cls, dict_cls, dict_body):
        ...
        for key, value in dict_body.items():
            if key not in dict_cls.attr_fields:
                raise BadHeader(key, None, str(dict_cls))
            ...
            if is_array:
                if not isinstance(value, list):
                    raise BadHeader(key, str(value), str(dict_cls))
                values = value
            else:
                values = [value]
   ...
   # end
        ...
        for req_param in request_params.get('updates') or []:
            ...
            if prop_coll_type == 'list':
                ...
                if ((req_oper == 'add') and field_val is None):
                    raise BadHeader('field value', current_header=req_param)
                    ...
                    raise BadHeader('field position', current_header=req_param)
    ...
    def useragent_kv_http_post(self):
        ...
        if oper is None:
            raise BadHeader('Key/Value API Operation')
        if 'key' not in request_params:
            raise BadHeader('Key/Value API Key')
    ...
    def _ensure_perms2_present(self, obj_type, obj_uuid, obj_dict,
        ...
        if gaccess is not None and shared is not None and shared != (gaccess != 0):
            ...
            return (False, (400, BadParameter('is_shared and global_access', (shared, gaccess))))
        return (True, "")
