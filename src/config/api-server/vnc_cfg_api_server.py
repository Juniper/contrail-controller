...
class VncApiServer(object):
    ...
    def __init__(self, args_str=None):
        ...
        if self._args.multi_tenancy is None:
            ...
            if self.aaa_mode not in vnc_consts.VALID_AAA_MODE_VALUES:
                self.aaa_mode = vnc_consts.AAA_MODE_DEFAULT_VALUE
            self._args.multi_tenancy = self.aaa_mode != 'no-auth'
        else:
            # MT configured by user - ignore aaa-mode
            self.aaa_mode = "admin-only" if self._args.multi_tenancy else "no-auth"
   ...
   # end
    ...
    def aaa_mode_http_put(self):
        ...
        if aaa_mode not in vnc_consts.VALID_AAA_MODE_VALUES:
