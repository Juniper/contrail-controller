...
class VncApiServer(object):
    ...
    def internal_chown(self, obj_uuid, owner):
        req_dict = {'uuid': obj_uuid, 'owner': owner}
        try:
            orig_context = get_context()
            orig_request = get_request()
            b_req = bottle.BaseRequest(
    ...
    def create_default_children(self, object_type, parent_obj):
        ...
        for child_type in child_types:
            ...
            return self.create_default_children(child_obj_type, child_obj)
   ...
   # end
    ...
    def _db_init_entries(self):
        ...
        _, gsc = self.create_singleton_entry(GlobalSystemConfig(
            autonomous_system=64512, config_version=CONFIG_VERSION))
        _, gvc = self.create_singleton_entry(GlobalVrouterConfig(
            parent_obj=gsc))
        self.create_singleton_entry(Domain())

        # Global and default policy resources
        _, pm_obj = self.create_singleton_entry(PolicyManagement())
        global_default_aps = ApplicationPolicySet(parent_obj=pm_obj,
                                                  all_applications=True)
        global_default_aps.add_global_vrouter_config(gvc)
        self.create_singleton_entry(global_default_aps)

        _, ip_fab_vn = self.create_singleton_entry(
        ...
        _, link_local_vn = self.create_singleton_entry(
    ...
    def create_singleton_entry(self, singleton_obj, user_visible=True):
        ...
        # create if it doesn't exist yet, else update
        already_exists = False
        try:
            s_obj.uuid = self._db_conn.fq_name_to_uuid(obj_type, fq_name)
            already_exists = True
            ok, result = self._db_conn.dbe_update(obj_type, s_obj.uuid,
                                                  obj_dict)
            if not ok:
                msg = "Error while updating %s: %s" % (obj_type, result[1])
            ...
            if not ok:
                msg = "Error while allocating %s: %s" % (obj_type, result[1])
                self.config_log(msg, level=SandeshLevel.SYS_ERR)
                return False, None
            s_obj.uuid = result
            # For virtual networks, allocate an ID
            ...
            ok, result = self._db_conn.dbe_create(obj_type, s_obj.uuid,
                                                  obj_dict)
            if not ok:
                msg = "Error while creating %s: %s" % (obj_type, result[1])
                self.config_log(msg, level=SandeshLevel.SYS_ERR)
                return False, None
            ok, result = self.create_default_children(obj_type, s_obj)
            if not ok:
                msg = ("Error while creating default childrens %s: %s" %
                       (obj_type, result[1]))
                self.config_log(msg, level=SandeshLevel.SYS_ERR)
                return False, None

        return already_exists, s_obj
