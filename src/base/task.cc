...
TaskScheduler::CancelReturnCode TaskScheduler::Cancel(Task *t) {
    ...
    if (t->state_ == Task::RUN) {
        ...
        // Remove task accordingly if TaskGroup or TaskEntry is disabled
        if (group->IsDisabled()) {
            ...
            //If the waitq_ is empty, remove TaskEntry from deferq.
            if (!entry->WaitQSize()) {
                ...
                // deferq_ comparison function uses the seqno of the first entry
                // in the waitq_. Therefore, if the task to be cancelled is the
                // first entry in the waitq_, then delete the entry from the
                // deferq_ and add it again.
                disable_entry->DeleteFromDeferQ(*entry);
