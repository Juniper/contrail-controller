...
public:
    virtual void SetUp() {
        monitor_ = new TaskMonitor(NULL, 1000, 20);
    }
    ...
    void SetTimers(uint64_t inactivity_time, uint64_t poll_interval) {
        monitor_->inactivity_time_usec_ = inactivity_time * 1000;
        monitor_->poll_interval_msec_ = poll_interval;
    }

    void UpdateTimers(uint32_t tbb_keepawake_time) {
        monitor_->UpdateTimers(tbb_keepawake_time);
    }
