...
void LoggingInit(std::string filename, long maxFileSize, int maxBackupIndex,
                 bool useSyslog, std::string syslogFacility,
                 std::string ident) {
    ...
    if (useSyslog) {
        std::string syslogident = boost::str(boost::format("%1%[%2%]") % ident % internal::get_process_id());
        if (boost::starts_with(syslogFacility, "LOG_")) {
            syslogFacility = syslogFacility.substr(4);
        }
        props.setProperty(LOG4CPLUS_TEXT("facility"), syslogFacility);
        props.setProperty(LOG4CPLUS_TEXT("ident"), syslogident);
        SharedAppenderPtr syslogappender(new SysLogAppender(props));
        std::auto_ptr<Layout> syslog_layout_ptr(new PatternLayout("[Thread %t]: %m%n"));
