...
try:
    ...
    from sandesh.storage.ttypes import *
        ...
        for line in arr:
                if line != arr[0]:
                    ...
                    if arr1[0] != "total":
                        ...
                        cs_pool.name = self._hostname + \
                            ':' + arr1[0]
                        pool_stats = PoolStats()
                        pool_stats.pool = arr1[0]
                        pool_stats.reads = int(arr1[7])
                        pool_stats.read_kbytes = int(
                            arr1[8])
                        pool_stats.writes = int(
                            arr1[9])
                        pool_stats.write_kbytes = int(
                            arr1[10])
                        cs_pool.info_stats = [
                            pool_stats]
        ...
        for line in arr:
            ...
            if is_active == "ok\n":
                ...
                if osd_prev_stats is None:
                    ...
                    ceph_name = "ceph-" + \
                        osd_name + ".asok"
                    # In this case subprocess is called
                    # once.
                    ...
                    try:
                        ...
                        for line1 in arr1:
                            ...
                            line2 = result.split(":")
                            if len(line2) != 0:
                                if line2[0].find('subop_r_out_bytes') != -1:
                                    osd_stats.read_kbytes += int(
                                        line2[1].rstrip(",").strip(' ')) / 1024
                                elif line2[0].find('op_r_out_bytes') != -1:
                                    osd_stats.read_kbytes += int(
                                        line2[1].rstrip(",").strip(' ')) / 1024
                            ...
                            if len(line2) != 0:
                                if line2[0].find('subop_r_out_bytes') != -1:
                                    self.curr_read_kbytes += int(
                                        line2[1].rstrip(",").strip(' ')) / 1024
                                    temp_osd_stats.read_kbytes = \
                                        self.curr_read_kbytes
                                    osd_stats.read_kbytes = \
                                        self.curr_read_kbytes - \
                                    ...
                                    self.curr_write_kbytes += int(
                                        line2[1].rstrip(",").strip(' ')) / 1024
                                    temp_osd_stats.write_kbytes = \
                                        self.curr_write_kbytes
                                    osd_stats.write_kbytes = \
                                        self.curr_write_kbytes - \
                                ...
                                if line2[0].find('subop_r_latency') != -1:
                                    ...
                                    sum_rlatency = int(
                                        float(
                                            ...
                                            strip().rstrip("},")))
                                    if avgcount != 0:
                                        osd_stats.op_r_latency += (
                                            sum_rlatency * 1000) / avgcount
                                    else:
                                        osd_stats.op_r_latency += 0
                                    ...
                                    sum_wlatency = int(
                                        float(
                                            ...
                                            strip().rstrip("},")))
                                    if avgcount != 0:
                                        osd_stats.op_w_latency += (
                                            sum_wlatency * 1000) / avgcount
                                    else:
                                        osd_stats.op_w_latency += 0
    ...
    def create_and_send_disk_stats(self):
        ...
        for line in disk_list:       # this will have all rows
            ...
            if len(arr1) != 0 and arr1[0].find('sd') != -1:
                    ...
                    if last == 'K':
                        cs_disk.current_disk_usage = \
                            long(
                                float(
                                    disk_usage_obj.
                                    disk_used.strip('K')) * 1024)
                    elif last == 'M':
                        cs_disk.current_disk_usage = \
                            long(
                                float(
                                    disk_usage_obj.
                                    disk_used.strip('M')) * 1024 * 1024)
                    elif last == 'G':
                        cs_disk.current_disk_usage = \
                            long(
                                float(
                ...
                if disk_stats.disk_name in new_dict:
                ...
                if len(arr) > 0 and not arr[0].isalpha():
...
def main(argv=sys.argv):
    ...
    # read the contrail-storage-nodemgr.conf file and populate
    # the node_type, discovery ip and port
    conf_file = open('/etc/contrail/contrail-storage-nodemgr.conf')
    for line in conf_file:
        if len(line) != 0 and line.find('node_type') != -1:
            arr = line.split('=')
            node_type = arr[1].rstrip()
        if len(line) != 0 and line.find('disc_server_port') != -1:
            arr = line.split('=')
            discovery_port = int(arr[1].rstrip())
        if len(line) != 0 and line.find('disc_server_ip') != -1:
            arr = line.split('=')
            discovery_ip = arr[1].rstrip()

    # dump the read values
    sys.stderr.write("node_type: " + node_type + "\n")
    sys.stderr.write("Discovery ip: " + discovery_ip + "\n")
    ...
    if (node_type == 'storage-compute'):
        ...
        _disc = client.DiscoveryClient(discovery_ip, discovery_port, module_name)
