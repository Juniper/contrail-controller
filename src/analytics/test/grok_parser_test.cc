...
TEST_F(GrokParserTest, Add_n_Match) {
    ...
    std::string pattern("CONTRAIL_SYSLOG <%{POSINT}>%{PROG:prog}: %{CONTRAIL_SYSLOG_TIMESTAMP:timestamp} %{HOSTNAME:hostname} \\[%{CONTRAIL_MODULE:module}\\]\\[%{CONTRAIL_LOG_LEVEL:log_level}\\] : %{GREEDYDATA:message}");
    gp.add_base_pattern(pattern);
    EXPECT_TRUE(gp.syntax_add("CONTRAIL_SYSLOG"));
