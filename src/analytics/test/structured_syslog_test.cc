...
vector<ArgSet> PopulateTestMessageStatsInfo(bool no_struct_part) {
    ...
    if (no_struct_part) {
        a1.attribs = map_list_of
            ...
            ("data.facility", DbHandler::Var(static_cast<uint64_t>(1)))
            ("data.severity", DbHandler::Var(static_cast<uint64_t>(6)))
            ("data.body", DbHandler::Var("APPTRACK_SESSION_CLOSE [junos@2636.1.1.1.2.26 reason=\"TCP RST\" source-address=\"4.0.0.1\" source-port=\"13175\""))
...
TEST_F(StructuredSyslogStatWalkerTest, Basic) {
    ...
    const std::string test_structured_syslog ("<14>Dec 17 04:46:29 csp-ucpe-bglr51 RT_FLOW: APPTRACK_SESSION_CLOSE [junos@2636.1.1.1.2.26 reason=\"TCP RST\" source-address=\"4.0.0.1\" source-port=\"13175\"]");
    ...
    std::vector<std::string> tagged_fields = boost::assign::list_of ("source-address") ("prog") ("hostname") ("tag");
    std::vector<std::string> int_fields = boost::assign::list_of ("source-port");
    bool r = structured_syslog::impl::ProcessStructuredSyslog(p, test_structured_syslog.length(), rep,
        boost::bind(&StatCbTester::Cb, &ct, _1, _2, _3, _4, _5), tagged_fields, int_fields);
...
TEST_F(StructuredSyslogStatWalkerTest, ParseError) {
    ...
    const std::string test_structured_syslog ("123 <14>Dec 17 04:46:29 csp-ucpe-bglr51 RT_FLOW: APPTRACK_SESSION_CLOSE [junos@2636.1.1.1.2.26 reason=\"TCP RST\" source-address=\"4.0.0.1\" source-port=\"13175\"]");
    boost::system::error_code ec;
    boost::asio::ip::address raddr(
        boost::asio::ip::address::from_string("127.0.0.1", ec));
    boost::asio::ip::udp::endpoint rep(raddr, 0);
    const uint8_t* p = reinterpret_cast<const uint8_t*>(test_structured_syslog.c_str());
    std::vector<std::string> tagged_fields = boost::assign::list_of ("source-address") ("prog") ("hostname");
    std::vector<std::string> int_fields = boost::assign::list_of ("source-port");
    bool r = structured_syslog::impl::ProcessStructuredSyslog(p, test_structured_syslog.length(), rep,
        boost::bind(&StatCbTester::Cb, &ct, _1, _2, _3, _4, _5), tagged_fields, int_fields);
