...
void XMLOutputStreamTest::ProcessMessage(std::string test_xml) {
    ...
    if (!doc.child("sandesh").child("ModuleCpuStateTrace").empty())
        data_ok_ = true;
}
...
TEST_F(XMLOutputStreamTest, INIConfig) {    
    ...
    SandeshXMLMessage *test_message = new SandeshXMLMessage;
    {
        std::string test_xml;
        load_file("data/logstream_test_sandeshmsg1.xml", test_xml);
...

    {
        ...
        const struct analytics::TCPOutputStream::Stats *stats =
            os.stats();
        WAIT_FOR(stats->messages_sent);
    }

    task_util::WaitForIdle();
    WAIT_FOR(session_->onread_called_);
    WAIT_FOR(messages_received_ == 1);
