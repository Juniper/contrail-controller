...
void
UserDefinedCounters::UDCHandler(RAPIDJSON_NAMESPACE::Document &jdoc,
            ...
            std::map<std::string, std::string> *headers)
{
    if (jdoc.IsObject() && jdoc.HasMember("global-system-configs")) {
        for (RAPIDJSON_NAMESPACE::SizeType j=0;
            ...
            const RAPIDJSON_NAMESPACE::Value& gsc = jdoc["global-system-configs"][j]
                    ["user_defined_log_statistics"]["statlist"];
            if (!gsc.IsArray()) {
                continue;
            }

            for (RAPIDJSON_NAMESPACE::SizeType i = 0; i < gsc.Size(); i++) {
