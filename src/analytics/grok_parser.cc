...
*/
void GrokParser::MakeSandesh(std::map<std::string, std::string> * m_in) {
    if (m_in == NULL)
        return;
    ...
    for (std::vector<std::string>::iterator ait = attribs_.begin(); ait != attribs_.end(); ait++) {
        ...
        for (std::vector<std::string>::iterator kit = keys_.begin(); kit != keys_.end(); kit++) {
            ...
            LOG(DEBUG, __func__ << a_key + "." + cur_keyval << " =======> " << (*m_in)[cur_attrib]);
            cur_keyval += "." + (*m_in)[(*kit)];
            a_key += "." + cur_keyval;
            m_out[a_key] = boost::lexical_cast<uint64_t>((*m_in)[cur_attrib].c_str());
            LOG(DEBUG, __func__ << a_key << " =======> " << (*m_in)[cur_attrib]);
        }
    }
