...
bool SandeshGenerator::Db_Connection_Init() {
    ...
    if (!GetDbHandler()->Init(ToString(), false, instance_)) {
...
bool SandeshGenerator::GetDbStats(uint64_t *queue_count, uint64_t *enqueues,
    ...
    db_handler_->GetSandeshStats(ToString(),drop_level, vdropmstats);
...
void SandeshGenerator::SendDbStatistics() {
    ...
    GenDb::DbErrors dbe;
    db_handler_->GetStats(ToString(), &vdbti, &dbe, &vstats_dbti);
