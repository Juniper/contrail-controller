...
class TCPOutputStream : public OutputStreamHandler {
 ...
 private:
    ...
    // We'll enqueue incoming TCP events in EventQueue and dequeue them in a
    // separate thread, to avoid deadlocks with TcpSession implementation.
    typedef TcpSession::Event TCPEvent;
    typedef WorkQueue<TcpSession::Event> TCPEventQueue;
    ...
    bool TCPEvDequeue(TCPEvent);
    bool isReadyForTCPEvDequeue() { return true; }

    struct ConnectionState state_machine_;
    // State Machine events to be processed. 
    TCPEventQueue tcp_evq_;
