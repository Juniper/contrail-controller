...
bool use_grok;
boost::shared_ptr<GrokParser> gp;
...
=======
  LOG(DEBUG, "full structured_syslog: " << std::string(p + start, p + len) << " len: " << len);
    if (use_grok) {
      ...
      if (gp.get()->match(str, &match_map)) {
          if (match_map["Message Type"] == "APPTRACK_SESSION_CLOSE") {
              gp.get()->send_generic_stat(&match_map);
          }
      }
    }
>>>>>>> 6153e32... Add GenericStat Objectlog for structured_syslog_c
  do {
...
StructuredSyslogServer::StructuredSyslogServer(EventManager *evm,
    ...
    boost::shared_ptr<GrokParser> gp, bool use_grok) {
