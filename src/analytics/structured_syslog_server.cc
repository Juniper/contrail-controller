...
void StructuredSyslogUVESummarizeData(SyslogParser::syslog_m_t v, bool summarize_user) {
    ...
    int64_t sampling_percentage = SyslogParser::GetMapVal(v, "sampling-percentage", 0);
    if (link1_bytes == -1)
    ...
    if (boost::equals(link1, link2)) {
        ...
        sdwanmetric.set_total_bytes(link1_bytes + link2_bytes);
        sdwanmetric.set_input_bytes(link2_bytes);
        sdwanmetric.set_output_bytes(link1_bytes);
        ...
        link_metrics_dial_traffic_type.insert(std::make_pair(linkmetrics_key, sdwanmetric_dial));
...
void StructuredSyslogUVESummarizeAppQoePSMR(SyslogParser::syslog_m_t v, bool summarize_user) {
    ...
    sdwanmetric.set_rtt(SyslogParser::GetMapVal(v, "rtt", 0));
    sdwanmetric.set_rtt_jitter(SyslogParser::GetMapVal(v, "rtt-jitter", 0));
    sdwanmetric.set_egress_jitter(SyslogParser::GetMapVal(v, "egress-jitter", 0));
    sdwanmetric.set_ingress_jitter(SyslogParser::GetMapVal(v, "ingress-jitter", 0));
    sdwanmetric.set_pkt_loss(SyslogParser::GetMapVal(v, "pkt-loss", 0));
    sdwanmetric.set_sampling_percentage(SyslogParser::GetMapVal(v, "sampling-percentage", 0));
...
void StructuredSyslogUVESummarizeAppQoeSMV(SyslogParser::syslog_m_t v, bool summarize_user) {
    ...
    SDWANMetrics_diff sdwanmetric;
    SDWANMetrics_dial sdwanmetric_dial;
    sdwanmetric_dial.set_sampling_percentage(SyslogParser::GetMapVal(v, "sampling-percentage", 0));
...
void StructuredSyslogUVESummarizeAppQoeASMR(SyslogParser::syslog_m_t v, bool summarize_user) {
    ...
    const std::string uvename = tenant + "::" + location + "::" + device_id;
    const std::string traffic_type(SyslogParser::GetMapVals(v, "active-probe-params", "UNKNOWN"));
    int64_t pkt_loss = SyslogParser::GetMapVal(v, "pkt-loss", 0);
    int64_t rtt = SyslogParser::GetMapVal(v, "rtt", 0);
    int64_t rtt_jitter = SyslogParser::GetMapVal(v, "rtt-jitter", 0);
    sdwanmetricrecord.set_name(uvename);
    SDWANMetrics_dial sdwanmetric;
