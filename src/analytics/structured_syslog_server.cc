        ...
        if (hr != NULL) {
            ...
            if (rar != NULL) {
                ...
                if (!tenant_app_service_tags.empty()) {
                    ...
                    prev_pos = DecorateMsg(msg, "app-service-tags", tenant_app_service_tags, prev_pos);
                }
            }
            else if (ar != NULL) {
                ...
                if (!app_service_tags.empty()) {
                    ...
                    prev_pos = DecorateMsg(msg, "app-service-tags", app_service_tags, prev_pos);
                }
            }
        }
        else {
...
public:
    StructuredSyslogServerImpl(EventManager *evm, uint16_t port,
        ...
        TaskScheduler::GetInstance()->GetTaskId("vnc-api http client"))) {
        if (structured_syslog_forward_dst != "") {
            std::vector<std::string> dest;
            boost::split(dest, structured_syslog_forward_dst, boost::is_any_of(":"), boost::token_compress_on);
            LOG(DEBUG, "structured_syslog_forward_dst" << structured_syslog_forward_dst);
            forwarder_ = new StructuredSyslogForwarder (evm, dest[0], atoi(dest[1].c_str()));
        }
        else {
            forwarder_ = NULL;
        }
...
StructuredSyslogServer::StructuredSyslogServer(EventManager *evm,
    uint16_t port, const std::string &structured_syslog_forward_dst,
    boost::shared_ptr<ConfigDBConnection> cfgdb_connection,
...
public:
    //typedef boost::intrusive_ptr<StructuredSyslogTcpForwarderSession> StructuredSyslogTcpForwarderSessionPtr;
    StructuredSyslogTcpForwarder(EventManager *evm) :
        TcpServer(evm),
        session_(NULL) {
    }
    ...
    void Connect(const std::string &ipaddress, int port) {
        ...
        endpoint.address(boost::asio::ip::address::from_string(ipaddress, ec));
        endpoint.port(port);
...
void StructuredSyslogForwarder::Init ()
{
    tcpForwarder_ = new StructuredSyslogTcpForwarder(evm_);
    tcpForwarder_->CreateSession();
    tcpForwarder_->Connect(dst_ipaddress_, port_);
    tcpForwarder_->SetSocketOptions();
}
...
bool StructuredSyslogForwarder::PollTcpForwarder() {
    ...
    if (tcpForwarder_->Connected() ==  false) {
        LOG(DEBUG,"reconnecting to remote syslog server");
        tcpForwarder_->ClearSessions();
        delete tcpForwarder_;
        Init();
    }
    else {
        LOG(DEBUG,"connection to remote syslog server is fine");
    }
    return true;
}
...
bool StructuredSyslogForwarder::Client (StructuredSyslogQueueEntry *sqe) {
    ...
    ret = tcpForwarder_->Send((const u_int8_t*)sqe->data->c_str(), sqe->length, &bytes_written);
