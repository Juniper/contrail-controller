...
class OpServerProxy::OpServerImpl {
    public:
        ...
        void analyticsCallbackProcess(const redisAsyncContext *c, void *r, void *privdata) {
            ...
            RAPIDJSON_NAMESPACE::Document document;	// Default template parameter uses UTF8 and MemoryPoolAllocator.
...
OpServerProxy::UVENotif(const std::string &type,
    ...
    if (deleted) {
        ...
        for (map<string,string>::const_iterator it = value.begin();
                    it != value.end(); it++) {
            RAPIDJSON_NAMESPACE::Value sval(RAPIDJSON_NAMESPACE::kStringType);
            sval.SetString((it->second).c_str(), dd.GetAllocator());
            RAPIDJSON_NAMESPACE::Value skey(RAPIDJSON_NAMESPACE::kStringType);
