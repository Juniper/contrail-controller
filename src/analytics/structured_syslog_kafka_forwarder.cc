...
void
KafkaForwarder::Send(const string& value) {
    ...
    if (producer_) {
        const string skey = value.substr(value.length()/2, 10);
...
KafkaForwarder::KafkaTimer() {
    ...
    if ((((kafka_tick_ms_ - kafka_start_ms_) < kActivityCheckPeriod_ms_) &&
        ...
        if (k_forwarder_dr_cb.count==0) {
            ...
            LOG(INFO, "Got Kafka Callbacks " << k_forwarder_dr_cb.count);
        }
