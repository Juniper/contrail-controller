...
UserDefineSyslogParser::delete_config(std::string name,
    ...
    if (parser.HasMember("querry_tags")) {
        for (contrail_rapidjson::SizeType i = 0; 
                     i < parser["querry_tags"]["querry_tag"].Size(); i++) {
            std::string tag = parser["querry_tags"]["querry_tag"][i].GetString();
    ...
    if (parser.HasMember("metrics")) {
        for (contrail_rapidjson::SizeType i = 0;
                     i < parser["metrics"]["metric"].Size(); i++) {
            std::string metric = parser["metrics"]["metric"].GetString();
            metric_list_[name].erase(metric);
        }
    }
    ...
    if (parser.HasMember("patterns")) {
        ...
        for (contrail_rapidjson::SizeType i = 0;
                         i < parser["patterns"]["pattern"].Size(); i++) {
            std::string pattern = parser["patterns"]["pattern"][i].GetString();
...
UserDefineSyslogParser::add_update_config(std::string name,
    ...
    header_cfg_info.hostname = parser["hostname"].GetString();
    header_cfg_info.appname  = parser["appname"].GetString();
    header_cfg_info.procid   = parser["procid"].GetString();
    header_cfg_info.msgid    = parser["msgid"].GetString();
    if (parser["appname_as_classfier"].GetBool()) {
        domain_id += header_cfg_info.appname;
    }
    domain_id += ":";
    if (parser["msgid_as_classfier"].GetBool()) {
    ...
    if (parser.HasMember("metrics")) {
        for (contrail_rapidjson::SizeType i = 0;
             i < parser["metrics"]["metric"].Size(); i++) {
            std::string metric = parser["metrics"]["metric"][i].GetString();
            metric_list_[name].insert(metric);
        }
    }
...
UserDefineSyslogParser::get_config(std::vector<LogParserConfigInfo> *config_info,
    ...
    for (DomainList::iterator it = domain_list_.begin();
        ...
        for (std::set<string>::iterator iter = it->second.begin();
            ...
            for (it1 = metric_list_[name].begin();
                 it1 != metric_list_[name].end(); it1++) {
