...
StructuredSyslogConfig::AddNetwork(const std::string& key, const std::string& network, const std::string& mask, const std::string& location)
{
    ...
    if (it  != networks_map_.end()) {
        LOG(DEBUG, "VPN name found in Networks MAP while adding network.
             Appending to existing values of VPN key... ");
        ...
        LOG(DEBUG, "VPN name NOT found in Networks MAP while adding network. 
            Creating a new entry in Networks MAP ...");
