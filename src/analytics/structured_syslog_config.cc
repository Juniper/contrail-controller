...
void
StructuredSyslogConfig::HostnameRecordsHandler(RAPIDJSON_NAMESPACE::Document &jdoc,
    ...
    if (jdoc.IsObject() && jdoc.HasMember("structured-syslog-hostname-records")) {
        for (RAPIDJSON_NAMESPACE::SizeType j = 0;
                    j < jdoc["structured-syslog-hostname-records"].Size(); j++) {
                const RAPIDJSON_NAMESPACE::Value& hr = jdoc["structured-syslog-hostname-records"][j];
...
void
StructuredSyslogConfig::ApplicationRecordsHandler(RAPIDJSON_NAMESPACE::Document &jdoc,
            boost::system::error_code &ec,
            const std::string &version, int status, const std::string &reason,
            std::map<std::string, std::string> *headers) {
    if (jdoc.IsObject() && jdoc.HasMember("structured-syslog-application-records")) {
        for (RAPIDJSON_NAMESPACE::SizeType j = 0;
                ...
                const RAPIDJSON_NAMESPACE::Value& ar = jdoc["structured-syslog-application-records"][j];
                ...
                const RAPIDJSON_NAMESPACE::Value& fq_name = ar["fq_name"];
