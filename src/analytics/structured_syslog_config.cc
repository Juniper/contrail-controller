...
StructuredSyslogConfig::AddMessageConfig(const std::string &name,
        ...
        bool process_and_store, const std::string &forward_action, bool process_and_summarize, bool process_and_summarize_user) {
    ...
    Cmc_t::iterator it = message_configs_.find(name);
    if (it  != message_configs_.end()) {
        it->second->Refresh(name, tags, ints, process_and_store, forward, process_before_forward, process_and_summarize, process_and_summarize_user);
    } else {
        boost::shared_ptr<MessageConfig> c(new MessageConfig(
                    name, tags, ints, process_and_store, forward, process_before_forward, process_and_summarize, process_and_summarize_user));
