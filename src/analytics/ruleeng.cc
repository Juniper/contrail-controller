...
static bool ParseDomTags(const std::string& tstr,
    ...
    do {
        ...
        if (sname.compare(g_viz_constants.STAT_KEY_FIELD)==0) {
            ...
            if (!ev.which()) {
                // In case elem is an XML Node
                pugi::xml_node anode_s = boost::get<pugi::xml_node>(ev).child(sname.c_str());
        ...
        if (!pterm.empty()) {
            ...
            if (pattr.compare(g_viz_constants.STAT_KEY_FIELD)==0) {
                ...
                if (!ev.which()) {
                    // In case elem is an XML Node
                    pugi::xml_node anode_p = boost::get<pugi::xml_node>(ev).child(pattr.c_str());
...
static bool DomStatWalker(StatWalker& sw,
    ...
    pugi::xml_node object = boost::get<pugi::xml_node>(elem_chain.at(0).get<1>().first);
    size_t sz = elem_chain.size();
    ...
    if (ParseDomTags(tstr, NULL, &elem_chain, &tagmap)) {
        ...
        if (ev.which()) {
            // This element is a value
            attribs.insert(make_pair(g_viz_constants.STAT_VALUE_FIELD,
                boost::get<DbHandler::Var>(ev)));    
        } else {
            // This element is an XML node
            pugi::xml_node node = boost::get<pugi::xml_node>(ev);
