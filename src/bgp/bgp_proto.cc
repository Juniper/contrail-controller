...
BgpProto::OpenMessage::Capability::GR::Encode(
    ...
    assert(gr_time <= 0x0FFF);
    const uint16_t gr_bytes = (gr_flags << 12) | (gr_time & 0x0FFF);

    vector<uint8_t> restart_cap = vector<uint8_t>();
...
bool BgpProto::OpenMessage::Capability::GR::Decode(GR *gr_params,
    ...
    bool result = false;
    for (cap_it = capabilities.begin(); cap_it != capabilities.end();
            ++cap_it) {
        ...
        size_t offset = 2;
        while (data + offset < data + (*cap_it)->capability.size()) {
