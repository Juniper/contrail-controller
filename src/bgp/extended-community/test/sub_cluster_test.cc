...
TEST_F(SubClusterTest, ByteArrayTypeExperimental4ByteAs_3) {
    ...
    EXPECT_EQ(0x82, sub_cluster.Type());
    EXPECT_EQ(0x11, sub_cluster.Subtype());
    EXPECT_EQ("subcluster:4294967294:65534", sub_cluster.ToString());
}
...
TEST_F(SubClusterTest, GetSubClusterId) {
    SubCluster::bytes_type data =
    { { BgpExtendedCommunityType::Experimental4ByteAs,
        BgpExtendedCommunitySubType::SubCluster,
    ...
    EXPECT_EQ(0x102, sub_cluster.GetId());
}

// Set sub_cluster_asn
TEST_F(SubClusterTest, SetSubClusterId) {
    as_t asn = 65412345;
    ...
    EXPECT_EQ(100, sub_cluster.GetId());
}
