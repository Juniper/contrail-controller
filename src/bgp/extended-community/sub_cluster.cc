...
SubCluster::SubCluster(as_t asn, const uint16_t sub_cluster_asn) {
    ...
    put_value(&data_[6], SubCluster::kSize - 6, sub_cluster_asn);
}

uint16_t SubCluster::SubClusterAsn() const {
...
string SubCluster::ToString() const {
    ...
    uint16_t sub_cluster_asn = get_value(data + 6, 2);
    snprintf(temp, sizeof(temp), "subcluster:%u:%u", asn, sub_cluster_asn);
    return string(temp);
}
...
SubCluster SubCluster::FromString(const string &str,
    ...
    uint64_t sub_cluster_asn = strtol(third.c_str(), &endptr, 10);
    // Check sub-cluster-asn for type 0x82.
    if (sub_cluster_asn == 0 || sub_cluster_asn > 0xFFFF || *endptr != '\0') {
        if (errorp != NULL) {
            *errorp = make_error_code(boost::system::errc::invalid_argument);
        }
        return SubCluster::null_sub_cluster;
    ...
    put_value(&data[offset], SubCluster::kSize - offset, sub_cluster_asn);
