...
int MvpnPrefix::FromProtoPrefix(const BgpProtoPrefix &proto_prefix,
    ...
    case LeafADRoute: {
        ...
        if (proto_prefix.prefix[0] == (uint8_t)SPMSIADRoute) {
            expected_nlri_size = kPrefixBytes + kSPMSIADRouteSize +
                                 kIp4AddrSize;
        } else if (proto_prefix.prefix[0] == (uint8_t)InterASPMSIADRoute) {
        ...
        if (proto_prefix.prefix[0] == (uint8_t)SPMSIADRoute) {
            size_t source_offset = kPrefixBytes + kRdSize + 1;
            prefix->source_ = Ip4Address(get_value(
                &proto_prefix.prefix[source_offset], kIp4AddrSize));

            size_t group_offset = source_offset + kIp4AddrSize + 1;
            prefix->group_ = Ip4Address(get_value(
                &proto_prefix.prefix[group_offset], kIp4AddrSize));
        } else if (proto_prefix.prefix[0] == (uint8_t)InterASPMSIADRoute) {
...
MvpnPrefix MvpnPrefix::FromString(const string &str, error_code *errorp) {
    ...
    case LeafADRoute: {
        ...
        if (src_rt_type == InterASPMSIADRoute) {
            ...
            prefix.rt_key_[0] = (uint8_t)InterASPMSIADRoute;
            prefix.rt_key_[1] = (uint8_t)kInterASPMSIADRouteSize;
        } else if (src_rt_type == SPMSIADRoute) {
            key_size = kPrefixBytes + kSPMSIADRouteSize;
            prefix.rt_key_.resize(key_size);
            prefix.rt_key_[0] = (uint8_t)SPMSIADRoute;
            prefix.rt_key_[1] = (uint8_t)kSPMSIADRouteSize;
        }
        size_t key_offset = 2;
        // Look for RD.
        size_t pos3;
...
string MvpnPrefix::ToString() const {
    ...
    switch (type_) {
        ...
        case LeafADRoute: {
            ...
            if (rt_type == (uint8_t)SPMSIADRoute) {
                repr += "-3";
                repr += "-" + rd.ToString();
                ...
                ip = Ip4Address(get_value (&rt_key_[key_offset], kIp4AddrSize));
                repr += "," + ip.to_string();
            } else if (rt_type == (uint8_t)InterASPMSIADRoute) {
                repr += "-2";
                repr += "-" + rd.ToString();
                key_offset += kRdSize;
...
void MvpnPrefix::SetLeafADPrefixFromSPMSIPrefix(const MvpnPrefix &prefix) {
    ...
    size_t key_offset = 0;
    rt_key_[key_offset++] = (uint8_t)SPMSIADRoute;
