...
BgpPeer::BgpPeer(BgpServer *server, RoutingInstance *instance,
        ...
        : server_(server),
          ...
          peer_type_((config->peer_as() == config->local_as()) ? BgpProto::IBGP : BgpProto::EBGP),
          policy_((config->peer_as() == config->local_as()) ? BgpProto::IBGP : BgpProto::EBGP, RibExportPolicy::BGP, config->peer_as(), -1, 0),
