...
PeerCloseManager *BgpPeerClose::close_manager() {
    return manager_.get();
}

PeerCloseManager *BgpPeerClose::close_manager() const {
    return manager_.get();
}
...
void BgpPeerClose::Close(bool non_graceful) {
    ...
    if (close_manager()->IsCloseInProgress() &&
            !close_manager()->IsInGRTimerWaitState())
        return;
...
bool BgpPeerClose::SetGRCapabilities(BgpPeerInfoData *peer_info) {
    ...
    // identical set.
    if (close_manager()->IsInGRTimerWaitState() &&
