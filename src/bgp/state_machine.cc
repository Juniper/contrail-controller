...
void StateMachine::SendNotificationAndClose(BgpSession *session, int code,
    ...
    // Close the peer session non-gracefully if we are sending an explicit
    // notification. However, For HoldTimerExp, attempt GR/LLGR.
    peer_->Close(code != 0 && code != BgpProto::Notification::HoldTimerExp);
}
