...
static std::string ToStr(int count) {
...
void GracefulRestartTest::AddRoutes() {
    GR_TEST_LOG("Adding Routes for " <<  ToStr(n_agents_) << " agents " <<
                " and " << ToStr(n_peers_) << " peers: BEGIN");
    ...
    GR_TEST_LOG("Adding " << ToStr(n_routes_) << " Bgp Routes");
    AddOrDeleteBgpRoutes(true);

    GR_TEST_LOG("Adding " << ToStr(n_routes_) << " Xmpp Routes");
    AddOrDeleteXmppRoutes(true);

    WaitForIdle();

    GR_TEST_LOG("Verify if received Xmpp route count is " <<
                ToStr(n_instances_ * (n_agents_ + n_peers_) * n_routes_));
...
void GracefulRestartTest::GracefulRestartTestStart () {
    ...
    GR_TEST_LOG("Verifying if Bgp INET_VPN route count per AFI is " << ToStr(n_routes_));
...
void GracefulRestartTest::GracefulRestartTestRun () {
    ...
    GR_TEST_LOG("1. Num instances: " << ToStr(n_instances_));
    GR_TEST_LOG("1. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("1. Total BGP routes: " << ToStr(total_bgp_routes));

    //  Verify that n_agents_ * n_instances_ * n_routes_ routes are received in
    //  agent in each instance
    GR_TEST_LOG("1. Verify that " << ToStr(total_xmpp_routes + total_bgp_routes)
    ...
    GR_TEST_LOG("2. Deleting " << ToStr(instances_to_delete_before_gr_.size())
                << " RIs before GR");
    ...
    total_bgp_routes -= n_routes_ * n_peers_ *
                    instances_to_delete_before_gr_.size();
    GR_TEST_LOG("2. Remaining instances: " << ToStr(remaining_instances));
    GR_TEST_LOG("2. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("2. Total BGP routes: " << ToStr(total_bgp_routes));

    // Subset of agents go down permanently (Triggered from agents)
    GR_TEST_LOG("3. Bringing " << ToStr(n_down_from_agents_.size()) <<
    ...
    GR_TEST_LOG("3. Bringing " << ToStr(n_down_from_peers_.size()) <<
    ...
    GR_TEST_LOG("3. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("3. Total BGP routes: " << ToStr(total_bgp_routes));
    ...
    GR_TEST_LOG("4. Bringing " << ToStr(n_flipped_agents.size())
                << " flipped agents down");
    ...
    GR_TEST_LOG("4. Bringing " << ToStr(n_flipped_peers.size())
                << " flipped peers down");
    ...
    GR_TEST_LOG("4. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("4. Total BGP routes: " << ToStr(total_bgp_routes));

    // Subset of agents flip (Triggered from agents)
    GR_TEST_LOG("5. Bringing " << ToStr(n_flipping_agents.size())
    ...
    // Subset of peers flip (Triggered from peers)
    GR_TEST_LOG("5. Bringing " << ToStr(n_flipping_peers.size())
                << " flipping peers down");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipping_peers) {
        BgpPeerTest *peer = gr_test_param.peer;
        WaitForPeerToBeEstablished(peer);
    ...
    GR_TEST_LOG("5. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("5. Total BGP routes: " << ToStr(total_bgp_routes));
    ...
    // were not deleted.
    GR_TEST_LOG("6. Deleting " << ToStr(instances_to_delete_during_gr_.size())
    ...
    GR_TEST_LOG("6. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("6. Total BGP routes: " << ToStr(total_bgp_routes));

    XmppStateMachineTest::set_skip_tcp_event(TcpSession::EVENT_NONE);

    GR_TEST_LOG("7. Bring up " << ToStr(n_flipped_agents.size()) <<
                " flipped agents");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_agents) {
        test::NetworkAgentMock *agent = gr_test_param.agent;
        TASK_UTIL_EXPECT_FALSE(agent->IsEstablished());
    ...
    GR_TEST_LOG("7. Bring up " << ToStr(n_flipped_peers.size()) <<
                " flipped peers");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_peers) {
        BgpPeerTest *peer = gr_test_param.peer;
        TASK_UTIL_EXPECT_FALSE(peer->IsReady());
    ...
    GR_TEST_LOG("7. Send subscriptions and readvertise routes for " <<
                ToStr(n_flipped_agents.size()) << " flipped agents"); 
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_agents) {
        test::NetworkAgentMock *agent = gr_test_param.agent;
        WaitForAgentToBeEstablished(agent);
    ...
    GR_TEST_LOG("7. Advertise subset of BGP routes for " <<
                ToStr(n_flipped_peers.size()) << " flipped peers");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_peers) {
        BgpPeerTest *peer = gr_test_param.peer;
        WaitForPeerToBeEstablished(peer);
    ...
    GR_TEST_LOG("7. Total XMPP routes: " << ToStr(total_xmpp_routes));
    GR_TEST_LOG("7. Total BGP routes: " << ToStr(total_bgp_routes));

    // Send EoR marker or trigger GR timer for agents which came back up and
    // sent desired routes.
    GR_TEST_LOG("8. Send EOR marker or trigger GR timer for " <<
                ToStr(n_flipped_agents.size()) << " restarted agents");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_agents) {
        test::NetworkAgentMock *agent = gr_test_param.agent;
    ...
    // sent desired routes.
    GR_TEST_LOG("8. Send EOR marker or trigger GR timer for " <<
                ToStr(n_flipped_peers.size()) << " restarted peers");
    BOOST_FOREACH(GRTestParams gr_test_param, n_flipped_peers) {
        BgpPeerTest *peer = gr_test_param.peer;
        if (gr_test_param.should_send_eor())
            peer->SendEndOfRIB();
    ...
    GR_TEST_LOG("8. Bring up half of " << ToStr(n_flipping_agents.size()) <<
    ...
    GR_TEST_LOG("8. Bring up half of " << ToStr(n_flipping_peers.size()) <<
    ...
    GR_TEST_LOG("8. Trigger GR timer for " << ToStr(n_down_from_agents_.size())
    ...
    GR_TEST_LOG("8. Trigger GR timer for " << ToStr(n_down_from_peers_.size())
                << " peers that went down permanently");
    BOOST_FOREACH(BgpPeerTest *peer, n_down_from_peers_) {
        FireGRTimer(bgp_server_peers_[peer->id()]);
    }
    
    GR_TEST_LOG("8. Verify that " << ToStr(total_xmpp_routes + total_bgp_routes) <<
                        "  Xmpp routes are received in agent in each instance");
    VerifyReceivedXmppRoutes(total_xmpp_routes + total_bgp_routes);

    GR_TEST_LOG("8. Verify that " << ToStr(instances_to_delete_before_gr_.size())
                << " RIs were deleted before GR");
    VerifyDeletedRoutingInstnaces(instances_to_delete_before_gr_);

    GR_TEST_LOG("8. Verify that " << ToStr(instances_to_delete_during_gr_.size())
