...
class RoutingInstanceMgrTraceBuffThresholdTest : public ::testing::Test {
protected:
    RoutingInstanceMgrTraceBuffThresholdTest() : server_(&evm_) {
    ...
    size_t GetEnvRoutingInstanceDormantTraceBufferCapacity() {
        return ri_mgr_->GetEnvRoutingInstanceDormantTraceBufferCapacity();
    }
...
TEST_F(RoutingInstanceMgrTraceBuffThresholdTest,
    ...
    TASK_UTIL_EXPECT_EQ(false, HasRoutingInstanceActiveTraceBuf("TestRi#3"));
    TASK_UTIL_EXPECT_EQ(false, HasRoutingInstanceActiveTraceBuf("TestRi#4"));
    ...
    TASK_UTIL_EXPECT_EQ(true, HasRoutingInstanceDormantTraceBuf("TestRi#3"));
    TASK_UTIL_EXPECT_EQ(true, HasRoutingInstanceDormantTraceBuf("TestRi#4"));
    TASK_UTIL_EXPECT_EQ(true, HasRoutingInstanceDormantTraceBuf("TestRi#5"));
...
static void SetUp() {
    ...
    size_t trace_buf_threshold = TEST_DORMANT_TRACE_BUFFER_THRESHOLD_ZERO;
