...
TEST_F(BgpXmppMvpnErrorTest, BadGroupAddress) {
    agent_xa_->AddMvpnRoute("blue", "225.0.0,90.1.1.1", "10.1.1.1", "10-20");
    task_util::WaitForIdle();
...
TEST_F(BgpXmppMvpnErrorTest, BadSourceAddress) {
    agent_xa_->AddMvpnRoute("blue", "225.0.0.1,90.1.1", "10.1.1.1", "10-20");
    task_util::WaitForIdle();
    MvpnTable *blue_table_ = static_cast<MvpnTable *>(
        bs_x_->database()->FindTable("blue.mvpn.0"));
    EXPECT_TRUE(blue_table_->Size() == 0);
...
TEST_F(BgpXmppMvpnErrorTest, BadNexthopAddress) {
    agent_xa_->AddMvpnRoute("blue", "225.0.0.1,90.1.1.1", "10.1", "10-20");
    task_util::WaitForIdle();
    MvpnTable *blue_table_ = static_cast<MvpnTable *>(
        bs_x_->database()->FindTable("blue.mvpn.0"));
    EXPECT_TRUE(blue_table_->Size() == 0);
...
TEST_F(BgpXmppMvpnErrorTest, BadLabelBlock1) {
    agent_xa_->AddMvpnRoute("blue", "225.0.0.1,90.1.1.1", "10.1.1.1", "10,20");
    task_util::WaitForIdle();
    MvpnTable *blue_table_ = static_cast<MvpnTable *>(
        bs_x_->database()->FindTable("blue.mvpn.0"));
    EXPECT_TRUE(blue_table_->Size() == 0);
...
TEST_F(BgpXmppMvpnErrorTest, BadLabelBlock2) {
    agent_xa_->AddMvpnRoute("blue", "225.0.0.1,90.1.1.1", "10.1.1.1", "1-2-3");
    task_util::WaitForIdle();
    MvpnTable *blue_table_ = static_cast<MvpnTable *>(
        bs_x_->database()->FindTable("blue.mvpn.0"));
    EXPECT_TRUE(blue_table_->Size() == 0);
...
TEST_F(BgpXmppMvpnSubscriptionTest, PendingSubscribe) {
    ...
    task_util::WaitForIdle();
    agent_xb_->AddMvpnRoute("blue", mroute, "10.1.1.2", "20000-29999");
    ...
    agent_xa_->AddMvpnRoute("blue", mroute, "10.1.1.1", "10000-19999");
    task_util::WaitForIdle();

    // Verify number of routes on all agents.
    WAIT_FOR(1000, 100, 2 == GetVrfTableSize(bs_x_, "blue"));

    // Add the route again, there should still be only 2 routes
    agent_xb_->AddMvpnRoute("blue", mroute, "10.1.1.2", "20000-29999");
    agent_xa_->AddMvpnRoute("blue", mroute, "10.1.1.1", "10000-19999");
    task_util::WaitForIdle();
    WAIT_FOR(1000, 100, 2 == GetVrfTableSize(bs_x_, "blue"));
...
TEST_F(BgpXmppMvpnSubscriptionTest, SubsequentSubscribeUnsubscribe) {
    ...
    const char *route = "7-127.0.0.1:2,0,10.1.1.10,225.0.0.1";
...
TEST_F(BgpXmppMvpnMultiAgentTest, SourceAndGroup) {
    const char *mroute = "225.0.0.1,90.1.1.1";

    // Add mcast route for all agents.
    agent_xa_->AddMvpnRoute("blue", mroute, "10.1.1.1", "10000-19999");
    agent_xb_->AddMvpnRoute("blue", mroute, "10.1.1.2", "20000-29999");
    agent_xc_->AddMvpnRoute("blue", mroute, "10.1.1.3", "30000-39999");
    task_util::WaitForIdle();

    // Verify number of routes on all agents.
    WAIT_FOR(1000, 100, 3 == GetVrfTableSize(bs_x_, "blue"));
