...
protected:
    ...
    TableState *GetVpnTableState() {
        ...
        TableState *vpn_ts = replicator->FindTableState(vpn_table);
        return vpn_ts;
    }

    void VerifyVpnTableStateExists(bool exists) {
        if (exists)
            TASK_UTIL_EXPECT_TRUE(GetVpnTableState() != NULL);
        else
            TASK_UTIL_EXPECT_TRUE(GetVpnTableState() == NULL);
    }
    ...
    void VerifyVpnTableStateRouteCount(uint32_t count) {
        TableState *vpn_ts = GetVpnTableState();
...
TEST_F(ReplicationTest, TableStateOnVRFWithNoImportExportRT) {
    ...
    const TableState *ts_blue = LookupTableState("blue");
    const TableState *ts_red = LookupTableState("red");
    TASK_UTIL_EXPECT_TRUE(ts_blue != NULL);
    TASK_UTIL_EXPECT_EQ(1, ts_blue->route_count());
    TASK_UTIL_EXPECT_TRUE(ts_red != NULL);
    ...
    ts_blue = LookupTableState("blue");
    ts_red = LookupTableState("red");
