...
class BgpDscpTest : public ::testing::TestWithParam<TestParams>{
public:
...
void BgpDscpTest::InitializeTemplates(bool set_local_as,
...
void BgpDscpTest::RunTest(uint8_t test_dscp_value) {
    ...
    uint8_t   get_dscp_value;
    if (peer_vm1_session) {
        get_dscp_value   = peer_vm1_session->GetDscpValue();
        TASK_UTIL_EXPECT_EQ((test_dscp_value << 2), get_dscp_value);
     }
}

TEST_P(BgpDscpTest, Basic) {
    if (!ebgp_) {
        if (set_local_as_) {
            vm1_as_ = "700";
            vm2_as_ = "800";
...
INSTANTIATE_TEST_CASE_P(BgpDscpTestWithParam, BgpDscpTest,
