...
class BgpCatTestUpgrade : public ::testing::Test {
protected:
    void run(const string &path);
};


void BgpCatTestUpgrade::run(const string &path) {
    pid_t child = 0;
    char *const argv[4] = {
        (char *) "go",
        (char *) "test",
