...
public:
    ...
    bool AttemptGRHelperModeDefault(bool notification, int code,
                                    int subcode) const {
        return BgpPeer::AttemptGRHelperMode(notification, code, subcode);
    }

    virtual bool AttemptGRHelperMode(bool notification, int code,
                                     int subcode) const {
        if (attempt_gr_helper_mode_fnc_.empty())
            return AttemptGRHelperModeDefault(notification, code, subcode);
        return attempt_gr_helper_mode_fnc_(notification, code, subcode);
    }
    ...
    boost::function<bool(bool, int, int)> attempt_gr_helper_mode_fnc_;
