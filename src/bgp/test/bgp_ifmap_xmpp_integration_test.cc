...
class BgpIfmapXmppIntegrationTest : public ::testing::Test {
 ...
 protected:
    ...
    virtual void SetUp() {
        ...
        string ufile = dir + "/conf/" + h + ".txt";

        int xmpp = 0, bgp = 0;
        if (getenv("RESTART")) {
            xmpp = strtoul(getenv("XMPP_PORT"), NULL, 0);
            bgp = strtoul(getenv("BGP_PORT"), NULL, 0);
        }
        server_->session_manager()->Initialize(bgp);
        xmpp_server_test_->Initialize(xmpp, false);

        string b = boost::lexical_cast<string>(server_->session_manager()->GetPort());
        string x = boost::lexical_cast<string>(xmpp_server_test_->GetPort());

        ofstream myfile;
        myfile.open (ufile.c_str());
        string data = "{\"ControllerDetails\": [{ \"ProcessID\": " + boost::lexical_cast<string>(pid) + ", \"BGPPORT\": "+ b + ", \"XMPPPORT\": " + x + ", \"HTTPPORT\": " + h +"}]}" ;
        myfile << data;
        cout << data << endl;
        myfile.close();
