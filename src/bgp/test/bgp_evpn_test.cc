...
"   </routing-instance>"
    ...
    virtual void SetUp() {
        ...
        fabric_evpn_ = new EvpnTable *[instances_set_count_];
...
#if 0
    EvpnRoute *VerifyLeafADEvpnRoute(EvpnTable *table, const string &prefix,
            const PMSIParams &pmsi) {
        EvpnPrefix type4_prefix =
            EvpnPrefix::FromString("4-" + prefix + ",127.0.0.1");
        TASK_UTIL_EXPECT_NE(static_cast<EvpnRoute *>(NULL),
                            table->FindRoute(type4_prefix));
        EvpnRoute *leaf_ad_rt = table->FindRoute(type4_prefix);
    ...
    void VerifyWithNoProjectManager(size_t red1c, size_t blue1c, size_t green1c,
        ...
        // Verify that there is no EvpnManager object created yet.
        for (size_t i = 1; i <= instances_set_count_; i++) {
            TASK_UTIL_EXPECT_EQ(static_cast<EvpnManager *>(NULL),
                                red_[i-1]->GetEvpnManager());
            TASK_UTIL_EXPECT_EQ(static_cast<EvpnManager *>(NULL),
                                blue_[i-1]->GetEvpnManager());
            TASK_UTIL_EXPECT_EQ(static_cast<EvpnManager *>(NULL),
                                green_[i-1]->GetEvpnManager());
    ...
    string prefix6(int index, int gindex = 1) const {
        ostringstream os;
...
TEST_P(BgpEvpnTest, Type3_SPMSI_Without_ErmVpnRoute) {
    ...
    for (size_t i = 1; i <= instances_set_count_; i++)
        for (size_t j = 1; j <= groups_count_; j++)
            AddEvpnRoute(red_[i-1], prefix6(i, j), getRouteTarget(i, "1"));
...
TEST_P(BgpEvpnTest, Type3_SPMSI_With_ErmVpnRoute) {
    ...
    for (size_t i = 1; i <= instances_set_count_; i++) {
        for (size_t j = 1; j <= groups_count_; j++) {
            ...
            lock.release();
            AddEvpnRoute(red_[i-1], prefix6(j), getRouteTarget(i, "1"));
            AddEvpnRoute(green_[i-1], prefix6(j), getRouteTarget(i, "3"));
        }
    }
    ...
    for (size_t i = 1; i <= instances_set_count_; i++) {
        ...
        TASK_UTIL_EXPECT_EQ(groups_count_, master_->Size()); // 
    }

    for (size_t i = 1; i <= instances_set_count_; i++) {
        for (size_t j = 1; j <= groups_count_; j++) {
            // Setup ermvpn route before type 3 spmsi route is added.
            DeleteEvpnRoute(red_[i-1], prefix6(j));
            DeleteEvpnRoute(green_[i-1], prefix6(j));
            {
                tbb::mutex::scoped_lock lock(pmsi_params_mutex);
    ...
    for (size_t i = 1; i <= instances_set_count_; i++) {
        WalkTable(red_[i-1]);
        WalkTable(green_[i-1]);
        WalkTable(master_);
        WalkTable(red_ermvpn_[i-1]);
        TASK_UTIL_EXPECT_EQ(0, red_[i-1]->Size()); // 1 local
        TASK_UTIL_EXPECT_EQ(0, blue_[i-1]->Size()); // 1 local
