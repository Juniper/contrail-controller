...
void BgpRoute::InsertPath(BgpPath *path) {
    ...
    if (table) {
        ...
        if (!path->IsReplicated()) {
            ...
            if (subcluster_id) {
                BgpAttrPtr modified_attr = table->ProcessExtCommunitySubCluster(
                                                attr, subcluster_id);
...
uint16_t BgpRoute::SubClusterId() const {
    BgpTable *table = static_cast<BgpTable *>(get_table());
    const RoutingInstance *ri = table->routing_instance();
    if (!ri) {
        return 0;
    ...
    const BgpProtocolConfig *proto = config_manager_->GetProtocolConfig(ri->name());
    if (!proto) {
        return 0;
    }
