...
bool BgpTable::InputCommon(DBTablePartBase *root, BgpRoute *rt, BgpPath *path,
    ...
    case DBRequest::DB_ENTRY_ADD_CHANGE: {
        ...
        // (3) TODO: We only do resolution for BGP learned paths. Once we
        //     have support for underlay routes being advertised for XMPP
        //     peers, need to relax the check.
        bool fc_enabled = false;
        fc_enabled = (server()->global_config()->fc_enabled() &&
                       !rtinstance_->IsMasterRoutingInstance() &&
                       peer && !peer->IsXmppPeer());
