...
void BgpTable::PrependLocalAs(const RibOut *ribout, BgpAttr *clone,
    ...
    as4_t local_as = ribout->local_as() ?:
...
void BgpTable::RemovePrivateAs(const RibOut *ribout, BgpAttr *attr) const {
    ...
    as4_t peer_asn = peer_loop_check ? ribout->peer_as() : 0;
    as4_t replace_asn = 0;
    if (replace) {
...
void BgpTable::PrependAsToAsPath2Byte(BgpAttr *clone, as4_t asn) const {
...
void BgpTable::PrependAsToAsPath4Byte(BgpAttr *clone, as4_t asn) const {
...
void BgpTable::CreateAsPath2Byte(BgpAttr *attr, as4_t local_as) const {
    ...
    if (attr->aspath_4byte()) {
        ...
        for (size_t i = 0; i < as_path4.path_segments.size(); i++) {
            ...
            for (size_t j = ps4->path_segment.size(); j > 0; j--) {
                as4_t as4 = ps4->path_segment[j-1];
...
void BgpTable::CreateAsPath4Byte(BgpAttr *attr, as4_t local_as) const {
                ...
                for (size_t j = ps->path_segment.size(); j > 0; j--) {
                    ...
                    if (as != AS_TRANS)
                        ...
                        if (as4_index >= ps4->path_segment.size())
                            assert(0);
                        as4_t as4 = ps4->path_segment[as4_index++];
