...
bool BgpTable::InputCommon(DBTablePartBase *root, BgpRoute *rt, BgpPath *path,
    ...
    case DBRequest::DB_ENTRY_ADD_CHANGE: {
        ...
        // (3) Path resolution in this case is needed only for routes
        //     in the VRF routing tables and for INET, INET6 and EVPN
        //     address families.
        bool fc_family = false;
        if (family() == Address::INET || family() == Address::INET6 ||
            family() == Address::EVPN) {
            fc_family = true;
        }
