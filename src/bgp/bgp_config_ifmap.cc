...
bool BgpIfmapGlobalSystemConfig::Update(BgpIfmapConfigManager *manager,
    ...
    if (data_.gr_time() != system->graceful_restart_parameters().time) {
        data_.set_gr_time(system->graceful_restart_parameters(). time);
        changed |= true;
    }

    if (data_.llgr_time() != static_cast<uint32_t>(
            system->graceful_restart_parameters(). long_lived_time)) {
        data_.set_llgr_time(
            system->graceful_restart_parameters().long_lived_time);
        changed |= true;
    }
