...
public:
    ...
    static string SaveContextAndPopLast(const ShowRouteReq *req,
            bool all_done, uint32_t total_count,
            vector<ShowRouteTable> *route_table_list);
...
string ShowRouteHandler::SaveContextAndPopLast(const ShowRouteReq *req,
    ...
    // If we are not done, we must have read one extra entry to use as context
    // for the next page. Save all the values we will need for the next
    // iteration.
    if (!all_done) {
    ...
    // 'total_count' could be less than the number requested if we did not have
    // enough routes matching the input criteria OR
    // 'total_count' will be equal to GetMaxRouteCount().
    // Pop off the last entry only if we read an extra entry. Also, pop off the
    // last entry only after we have captured its values in 'next_batch' above.
    if (ShowRouteHandler::GetMaxRouteCount(req) == total_count) {
...
bool ShowRouteHandler::CallbackS2Common(const ShowRouteReq *req,
    ...
    for (size_t j = 0; j < route_table_list.size(); ++j) {
        total_count += route_table_list[j].routes.size();
    }

    ShowRouteResp *resp = new ShowRouteResp;
    string next_batch = SaveContextAndPopLast(req, all_done, total_count,
                                              &route_table_list);
