...
public:
    ...
    static string SaveContextAndPopLast(const ShowRouteReq *req,
            ShowRouteResp *resp, vector<ShowRouteTable> *route_table_list);
...
bool ShowRouteHandler::CallbackS1Common(const ShowRouteReq *req, int inst_id,
    ...
    // Since max_count is one more than request value or default, the following
    // if() covers both the following cases:
    // 1. the last batch has exactly (max_count - 1)
    // 2. the last batch has less than (max_count - 1)
    if (count < max_count) {
        mydata->done = true;
    }
    return true;
...
string ShowRouteHandler::SaveContextAndPopLast(const ShowRouteReq *req,
    ...
    // save anything since there will be no subsequent iteration.
    if (route_table_list->empty()) {
        return string("");
    }

    ShowRouteTable *route_table =
        &route_table_list->at(route_table_list->size() - 1);
    ShowRoute last_route =
        route_table->routes.at(route_table->routes.size() - 1);
    // Save all the values we will need for the next iteration.
    string next_batch =
        ...
        route_table->get_routing_instance() + kShowRouteIterSeparator +
        route_table->get_routing_table_name() + kShowRouteIterSeparator +
    ...
    // We get back one extra entry to able to do pagination. Pop off the last
    // entry only after we have captured its values in 'next_batch' above for
    // the next iteration.
    if (ShowRouteHandler::GetMaxRouteCount(req) == route_table->routes.size()) {
        route_table->routes.pop_back();
        if (route_table->routes.empty()) {
...
bool ShowRouteHandler::CallbackS2Common(const ShowRouteReq *req,
    ...
    // We read one extra entry for pagination. Save the last entry as context
    // for the next page.
    if (!all_done) {
        string next_batch = SaveContextAndPopLast(req, resp, &route_table_list);
