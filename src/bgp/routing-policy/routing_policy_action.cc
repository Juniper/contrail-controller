...
void UpdateCommunity::operator()(const BgpAttr *attr, BgpAttr *out_attr) const {
    if (!attr) return;
    const Community *comm = attr->community();
    if (comm) {
        BgpAttrDB *attr_db = out_attr->attr_db();
        ...
        out_attr->set_community(new_community);
    }
}
...
void UpdateLocalPref::operator()(const BgpAttr *attr, BgpAttr *out_attr) const {
    out_attr->set_local_pref(local_pref_);
}
