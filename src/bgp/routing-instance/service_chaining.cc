...
ServiceChain<T>::ServiceChain(ServiceChainMgrT *manager,
    ...
    for (vector<string>::const_iterator it = subnets.begin();
        ...
        if (GetFamily() == Address::EVPN) {
            ...
            if (GetSCFamily() == SCAddress::EVPN) {
                if (subnet.family() == Address::INET6) continue;
            } else {
                if (subnet.family() != Address::INET6) continue;
            }
        }
...
BgpTable *ServiceChain<T>::connected_table() const {
    return connected_->GetTable(GetConnFamily());
}
...
void ServiceChain<T>::UpdateServiceChainRouteInternal(const RouteT *orig_route,
    ...
    for (Route::PathList::iterator it =
        ...
        if (bgptable->family() == Address::EVPN) {
            ...
            vector<string> tunnel_encaps = boost::assign::list_of
                ("vxlan");
...
BgpConditionListener *ServiceChainMgr<T>::GetListener() {
    return server_->condition_listener(GetConnFamily());
}
...
bool ServiceChainMgr<T>::RequestHandler(ServiceChainRequestT *req) {
    ...
    BgpConditionListener *listener = GetListener();
    ...
    if (state) {
        ...
        if (state->refcnt() == 0 && state->deleted()) {
            ...
            if (!info->num_matchstate()) {
                if (info->dest_table_unregistered()) {
                    GetListener()->UnregisterMatchCondition(
 ...
 */
template <>
Address::Family ServiceChainMgr<ServiceChainInet>::GetConnFamily() const {
    return Address::INET;
}

template <>
Address::Family ServiceChainMgr<ServiceChainInet6>::GetConnFamily() const {
    return Address::INET6;
}

template <>
Address::Family ServiceChainMgr<ServiceChainEvpn>::GetConnFamily() const {
    return Address::INET;
}

template <>
Address::Family ServiceChainMgr<ServiceChainEvpn6>::GetConnFamily() const {
    return Address::INET6;
}
...
bool ServiceChainMgr<T>::LocateServiceChain(RoutingInstance *rtinstance,
    ...
    if (it != chain_set_.end()) {
        ...
        GetListener()->RemoveMatchCondition(chain->dest_table(), chain, cb);
        GetListener()->RemoveMatchCondition(chain->connected_table(), chain, cb);
        return true;
    }
    ...
    if (config.source_routing_instance == "") {
        ...
        /**
          * TODO: Currently assuming that the presence of a source RI in
          * service-info config as an indication that this SI is the first
          * in the list of service-chains. Need to validate this with the
          * config team. If not, schema-transformer may need to find this
          * information for control-node and add it to the service-info.
          */
    ...
    connected_table = connected_ri->GetTable(GetConnFamily());
...
void ServiceChainMgr<T>::StopServiceChain(RoutingInstance *rtinstance) {
    ...
    BgpConditionListener::RequestDoneCb cb =
        bind(&ServiceChainMgr::StopServiceChainDone, this, _1, _2);
    GetListener()->RemoveMatchCondition(chain->dest_table(), chain, cb);
    GetListener()->RemoveMatchCondition(chain->connected_table(),
                                                  chain, cb);
}
