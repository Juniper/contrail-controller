...
RoutingInstanceMgr::RoutingInstanceMgr(BgpServer *server) :
        ...
        identifier_listener_id_(server->RegisterIdentifierUpdateCallback(
            boost::bind(&RoutingInstanceMgr::IdentifierUpdateCallback,
                this, _1))),
        dormant_trace_buf_size_(GetRoutingInstanceDormantTraceBufferCapacity()),
...
void RoutingInstanceMgr::DisableTraceBuffer(const std::string &name) {
    ...
    if (iter != trace_buffer_active_.end()) {
        ...
        if (trace_buffer_dormant_.size() >= dormant_trace_buf_size_){
            ...
            for (int i = 0; 
                 i < (int)std::min(trace_buffer_dormant_list_.size(),
        ...
        trace_buffer_dormant_.insert(make_pair(name, trace_buf));
...
size_t RoutingInstanceMgr::
       GetRoutingInstanceDormantTraceBufferCapacity() const {
    char *buffer_capacity_str = getenv("ROUTING_INSTANCE_DORMANT_TRACE_BUFFER_SIZE");
    if (buffer_capacity_str) {
        size_t env_buffer_capacity = strtoul(buffer_capacity_str, NULL, 0);
        return env_buffer_capacity;
    } 
    return MAX_ROUTING_INSTANCE_DORMANT_TRACE_BUFFER_SIZE;
}

size_t RoutingInstanceMgr::
       GetRoutingInstanceDormantTraceBufferThreshold() const {
    char *buffer_threshold_str = getenv("ROUTING_INSTANCE_DORMANT_TRACE_BUFFER_THRESHOLD");
    if (buffer_threshold_str) {
        size_t env_buffer_threshold = strtoul(buffer_threshold_str, NULL, 0);
        return env_buffer_threshold;
    } 
    return ROUTING_INSTANCE_DORMANT_TRACE_BUFFER_THRESHOLD;
}

int RoutingInstanceMgr::GetRoutingInstanceActiveTraceBufSize() const {
    tbb::spin_rw_mutex::scoped_lock read_lock(rw_mutex_, false);
    return trace_buffer_active_.size();
}

int RoutingInstanceMgr::GetRoutingInstanceDormantTraceBufSize() const {
    tbb::spin_rw_mutex::scoped_lock read_lock(rw_mutex_, false);
