...
static bool FillServiceChainInfo(Address::Family family, string search_string,
    ...
    smm.set_dest_virtual_network(
            GetVNFromRoutingInstance(sc_config->routing_instance));
    smm.set_service_instance(sc_config->service_instance);

    IServiceChainMgr *service_chain_mgr =
        rit->server()->service_chain_mgr(family);
    if (family == Address::INET) {
        ServiceChain<ServiceChainInet> *service_chain =
            dynamic_cast<ServiceChainMgr<ServiceChainInet> *>(service_chain_mgr)->FindServiceChain(rit);
        if (service_chain != NULL) {
            smm.set_state(service_chain->deleted() ? "Deleted" : "Active");
            service_chain->FillServiceChainInfo(&smm);
            return true;
        }
    } else if (family == Address::INET6) {
        ServiceChain<ServiceChainInet6> *service_chain =
            dynamic_cast<ServiceChainMgr<ServiceChainInet6> *>(service_chain_mgr)->FindServiceChain(rit);
        if (service_chain != NULL) {
            smm.set_state(service_chain->deleted() ? "Deleted" : "Active");
            service_chain->FillServiceChainInfo(&smm);
            return true;
