...
class ContributingRouteState : public ConditionMatchState {
public:
    explicit ContributingRouteState(AggregateRoutePtr info) : info_(info) {
    }
...
private:
    ...
    DISALLOW_COPY_AND_ASSIGN(ContributingRouteState);
};

class AggregateRouteState : public DBState {
public:
    explicit AggregateRouteState(AggregateRoutePtr info) : info_(info) {
    }
    AggregateRoutePtr info() {
        return info_;
    }
...
public:
    ...
    typedef std::set<BgpRoute *> ContributingRouteList;
    ...
    bool IsContributing(const BgpRoute *route) const {
        return (contributors_.find(route) != contributors_.end());
    }

    void AddContributingRoute(BgpRoute *route) {
        contributors_.insert(route);
    }

    void RemoveContributingRoute(BgpRoute *route) {
        contributors_.erase(route);
    }
...
AggregateRoute<T>::AggregateRoute(RoutingInstance *rtinstance,
    ...
    : routing_instance_(rtinstance),
      ...
      nexthop_(nexthop),
      aggregate_route_(NULL), unregistered_(false) {
}
...
bool AggregateRoute<T>::Match(BgpServer *server, BgpTable *table,
    ...
    ContributingRouteState *state = static_cast<ContributingRouteState *>
        (listener->GetMatchState(table, route, this));
    if (!deleted) {
        if (!state) {
            state = new ContributingRouteState(AggregateRoutePtr(this));
...
void AggregateRoute<T>::RemoveAggregateRoute() {
    ...
    BgpRoute *aggregate_route =
        static_cast<BgpRoute *>(partition->Find(&rt_key));
    if (!aggregate_route || aggregate_route->IsDeleted()) return;
...
void AggregateRoute<T>::set_aggregate_route(BgpRoute *aggregate) {
    if (aggregate) {
        AggregateRouteState *state =
            new AggregateRouteState(AggregateRoutePtr(this));
        ...
        if (aggregate_route_) {
            ...
            if (state) {
                aggregate_route_->ClearState(bgp_table(),
                                             manager_->listener_id());
                delete state;
            }
        }
    }
...
template <typename T>
DBTableBase::ListenerId RouteAggregator<T>::listener_id() {
    if (listener_id_ == DBTableBase::kInvalidId) {
...
bool RouteAggregator<T>::ProcessRouteAggregateUpdate() {
    ...
    for (AggregateRouteProcessList::iterator
        ...
        if (aggregate->aggregate_route()) {
            if (aggregate->contribute_route_list()->empty())
                aggregate->RemoveAggregateRoute();
        } else {
            if (aggregate->contribute_route_list()->size())
