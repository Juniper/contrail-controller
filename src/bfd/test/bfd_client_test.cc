...
TEST_F(ClientTest, BasicSendTimerTest1) {
    ...
    boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
    // Detection time = 600mSec, test wait time = 1sec.
    // so the expected Tx packet ~6+1 = 7. And taking some jitter into
    // account shouldn't exceed 9 pkts.
    EXPECT_LE((session_test->Stats().tx_count - old_tx_count), 9);
