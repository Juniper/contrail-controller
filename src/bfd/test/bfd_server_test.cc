...
TEST_F(ServerTest, Test2) {
    ...
    boost::function<ResultCode(const ControlPacket *)> c1 = boost::bind(&Server::ProcessControlPacket, &server1, _1);
    ...
    boost::function<ResultCode(const ControlPacket *)> c2 = boost::bind(&Server::ProcessControlPacket, &server2, _1);
    ...
    ASSERT_LT(abs((s2->detection_time() - (down2 - start)).total_milliseconds()), 100);
