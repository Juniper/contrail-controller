...
#include "bfd/rest_api/bfd_config_server.h"
...
void RESTServer::ClientHandler(const struct RESTData& data) {
    ...
    //    DeleteRESTClientSession(atoi(client_id.c_str()), session, request);
}

void RESTServer::ClientIPConnectionHandler(const struct RESTData& data) {
    const std::string client_id = data.match[1];
    CreateBFDConnection(atoi(client_id.c_str()), data.session, data.request);
}
...
void RESTServer::ClientMonitorHandler(const struct RESTData& data) {
    const std::string client_id = data.match[1];
    MonitorRESTClientSession(atoi(client_id.c_str()), data.session, data.request);
}

const std::vector<RESTServer::HandlerSpecifier> RESTServer::RESTHandlers_ =
    boost::assign::list_of
    ( { boost::regex("/Session"), HTTP_POST, &RESTServer::SessionHandler } )
//  ( { boost::regex("/Session/([[:digit:]]{1,9})", HTTP_DELETE,
//      &RESTServer::ClientHandler } )
    ( { boost::regex("/Session/([[:digit:]]{1,9}/IpConnection", HTTP_POST,
        &RESTServer::ClientIPConnectionHandler } )
    ( { boost::regex("/Session/([[:digit:]]{1,9})/IpConnection/"
                        "([[:digit:]]{1,3}\\.[[:digit:]]{1,3}\\."
                        "[[:digit:]]{1,3}\\.[[:digit:]]{1,3})"),
        HTTP_GET, &RESTServer::ClientIPAddressHandlerGet)
    ( { boost::regex("/Session/([[:digit:]]{1,9})/IpConnection/"
                        "([[:digit:]]{1,3}\\.[[:digit:]]{1,3}\\."
                        "[[:digit:]]{1,3}\\.[[:digit:]]{1,3})"),
        HTTP_DELETE, &RESTServer::ClientIPAddressHandlerDelete)
    ( { boost::regex("/Session/([[:digit:]]{1,9}"),
        HTTP_GET, &RESTServer::ClientMonitorHandler } );
...
RESTClientSession* RESTServer::GetClientSession(ClientId client_id,
    ...
    if (it != client_sessions_.end()) {
        ...
        SendErrorResponse(session, "Unknown client-id: " +
...
void RESTServer::CreateRESTClientSession(HttpSession* session,
    ...
    RESTClientSession* client_session = new ClientSession(bfd_server_, client_id);
...
void RESTServer::DeleteRESTClientSession(ClientId client_id, HttpSession* session,
                                            const HttpRequest* request) {
