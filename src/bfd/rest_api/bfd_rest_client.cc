...
#include "bfd/bfd_config_client.h"
...
bool RESTClient::IsInitializedNonLocking() {
    return !stopped_ && !error_ && client_id_.is_initialized();
}

bool RESTClient::IsIsInitialized() {
    tbb::interface5::unique_lock<tbb::mutex> lock(mutex_);
    return IsInitializedNonLocking();
}
...
void RESTClient::Init() {
    ...
    while (!error_ && !IsInitializedNonLocking()) {
        cond_var_.wait(lock);
    }
}
...
void RESTClient::Stop() {
    ...
    LOG(INFO, "RESTClient: " << ep_ << " stopped");
}
...
BfdJsonConfig RESTClient::DefaultConfig() {
    BfdJsonConfig res;
...
void RESTClient::CreateSessionCallback(const std::string& response,
    ...
    if (!ec) {
        ...
        LOG(ERROR, "CreateSession: Error: " << ec.value() << " " << ec.message());
    }
...
void RESTClient::DeleteSessionCallback(const std::string& response,
    ...
    if (!ec) {
        ...
        LOG(ERROR, "DeleteSession: Error: " << ec.value() << " " << ec.message());
    }
    connection->client()->RemoveConnection(connection);
    cb(ec);
}
