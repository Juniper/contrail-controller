 ...
 public:
    ...
    BFDTestServer(TestCommunicatorManager *communicationManager, EventManager *evm,
        ...
        http->RegisterHandler(HTTP_WILDCARD_ENTRY,
                boost::bind(&RESTServer::HandleRequest, &config_server, _1, _2));
        http->Initialize(port);
        communicationManager->registerServer(addr, boost::bind(&Server::ProcessControlPacket, &bfd_server, _1));
    }
 ...
 public:
    ...
    void WaitForState(RESTClient *client, const boost::asio::ip::address &addr,
        ...
        if (state_changed) {
            ...
            EXPECT_TRUE(client->GetSession(addr,  boost::bind(&BFDConfigTest::GetSessionCallback, this, _1, _2)));
    ...
    void DeleteBFDHost(RESTClient *client, const boost::asio::ip::address &addr) {
...
TEST_F(BFDConfigTest, Test1) {
    RESTClient *client1(new RESTClient(server1.config_ep, evm));
    RESTClient *client2(new RESTClient(server2.config_ep, evm));
...
TEST_F(BFDConfigTest, DoubleClient) {
    RESTClient *client1(new RESTClient(server1.config_ep, evm));
    RESTClient *client1b(new RESTClient(server1.config_ep, evm));
    RESTClient *client2(new RESTClient(server2.config_ep, evm));

    client1->Init();
    TASK_UTIL_EXPECT_TRUE(client1->IsInitialized());
    client1b->Init();
    TASK_UTIL_EXPECT_TRUE(client1b->IsInitialized());
    client2->Init();
    TASK_UTIL_EXPECT_TRUE(client2->IsInitialized());
