...
RESTClientSession::ClientSession(Server* server, ClientId client_id) :
...
ResultCode RESTClientSession::AddBFDConnection(const boost::asio::ip::address& remoteHost,
                                            const SessionConfig& config) {
  if (bfd_sessions_.find(remoteHost) != bfd_sessions_.end()) {
    //TODO(clbw,clmz) update
  ...
  if (NULL == session)
    return kResultCode_Error;
  session->RegisterChangeCallback(client_id_, boost::bind(&RESTClientSession::Notify, this));
  Notify();

  return result;
}

ResultCode RESTClientSession::DeleteBFDConnection(const boost::asio::ip::address& remoteHost) {
  if (bfd_sessions_.find(remoteHost) == bfd_sessions_.end()) {
...
RESTClientSession::~ClientSession() {
  for (Sessions::iterator it = bfd_sessions_.begin(); it != bfd_sessions_.end(); ++it) {
