...
Session(Discriminator localDiscriminator,
    ...
    sm_->SetCallback(boost::optional<StateMachine::ChangeCb>(
        boost::bind(&Session::StateChangeCallback, this, _1)));
}
...
BFDState Session::LocalStateNoLock() {
    return sm_->GetState();
}

BFDState Session::LocalState() {
    tbb::mutex::scoped_lock lock(mutex_);
...
ResultCode Session::ProcessControlPacket(const ControlPacket *packet) {
    ...
    if (remoteSession_.minRxInterval != packet->required_min_rx_interval) {
        // TODO(bw,mz) schedule from previous packet
        ScheduleSendTimer();
...
TimeInterval Session::DetectionTime() {
...
TimeInterval Session::tx_interval() {
    ...
    boost::random::uniform_int_distribution<>
            dist(minInterval.total_microseconds(), maxInterval.total_microseconds());
    return boost::posix_time::microseconds(dist(randomGen));
}

boost::asio::ip::address Session::RemoteHost() {
    tbb::mutex::scoped_lock lock(mutex_);
    return remoteHost_;
}
...
BFDRemoteSessionState Session::RemoteState() {
    tbb::mutex::scoped_lock lock(mutex_);
    return remoteSession_;
}

Discriminator Session::LocalDiscriminator() {
    tbb::mutex::scoped_lock lock(mutex_);
...
void Session::UpdateConfig(const SessionConfig& config) {
    //TODO(bw,mz)
    LOG(WARN, "Session::UpdateConfig not implemented");
}
