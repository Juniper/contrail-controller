...
def _cfg_curl_logging(api_curl_log_file=None):
    ...
    curl_logger.debug("Connecting to the API server")
    return curl_logger
...
class VncApi(object):
	...
	self._curl_logging = False
        if _read_cfg(cfg_parser, 'logcurl', 'is_logging', False):
            self._curl_logging = True
            self._curl_logger=_cfg_curl_logging(_read_cfg(cfg_parser, 'logcurl', 'logfile', '/var/log/contrail/api_log_curl.log'))
