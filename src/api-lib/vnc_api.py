...
def compare_refs(old_refs, new_refs):
    ...
    old_ref_dict = dict((':'.join(ref['to']), ref['attr'])
                        for ref in old_refs or [])
    new_ref_dict = dict((':'.join(ref['to']), ref['attr'])
...
class VncApi(object):
    ...
    def _authenticate(self, response=None, headers=None):
        ...
        if self._authn_token_url:
            ...
            url = "%s://%s:%s%s" % (self._authn_protocol, self._authn_server,
                                    self._authn_port, self._authn_url)
        new_headers = headers or {}
        try:
            if self._ksinsecure:
                response = requests.post(
                        url, data=self._authn_body,
