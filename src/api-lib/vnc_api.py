...
class VncApi(object):
    ...
    def _log_curl(self, op, url, data=None):
        op_str = {OP_GET: 'GET', OP_POST: 'POST',
                  OP_DELETE: 'DELETE', OP_PUT: 'PUT'}
    ...
    def resource_list(self, obj_type, parent_id=None, parent_fq_name=None,
        ...
        if fq_names_str:
            comma_sep_fq_names = ','.join([':'.join(u) for u in fq_names_str])
            query_params['fq_names_str'] = comma_sep_fq_names
            if len(fq_names_str) > self.POST_FOR_LIST_THRESHOLD:
                do_post_for_list = True
