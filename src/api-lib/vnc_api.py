        ...
        while True:
            ...
            try:
                ...
                if ((status == 401) and (not self._auth_token_input) and
                    ...
                    raise NoIdError(error_content['unknown_id'], error_content['resource_type'])
                elif status == 403:
                    error_content = json.loads(content)
                    raise PermissionDenied(error_content['reason'] if 'reason' in error_content.keys() else 'Unknown Reason')
                elif status == 412:
                    error_content = json.loads(content)
                    raise OverQuota(content)
                elif status == 409:
                    error_content = json.loads(content)
                    raise RefsExistError(error_content['existing_resource_fq_name'] if 'existing_resource_fq_name' in error_content.keys() else 'Unknown',
                    ...
                    if retried >= retry_count:
                        ...
                        if error_content['down_service'] == 'Database':
                            ...
                            raise ServiceUnavailableError(error_content['down_service']  if 'down_service' in error_content.keys() else 'Unknown')
                    ...
                    if error_content['error'] == 'BadHeader':
                        raise BadHeader(error_content['bad_parameter'] if 'bad_parameter' in error_content.keys() else None,
                        ...
                        raise BadParameter(error_content['bad_property'] if 'bad_property' in error_content.keys() else None,
                                        error_content['bad_value'] if 'bad_value' in error_content.keys() else None)
                    elif error_content['error'] == 'AccessDeniedError':
                        raise AccessDeniedError(error_content['object_type'] if 'object_type' in error.keys() else None)
                    else:
                        raise BadRequest(error_content['reason'] if 'reason' in error_content.keys() else None)
                else:  # Unknown Error
