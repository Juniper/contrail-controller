...
class VncApi(object):
    ...
    def __init__(self, username=None, password=None, tenant_name=None,
        ...
        if self._authn_type == 'keystone':
            ...
            self._apiinsecure = _read_cfg(cfg_parser, 'global', 'insecure',
                          self._DEFAULT_INSECURE)
            self._apicertfile=_read_cfg(cfg_parser,'global','certfile','')
            self._apikeyfile=_read_cfg(cfg_parser,'global','keyfile','')
            self._apicafile=_read_cfg(cfg_parser,'global','cafile','')
            ...
            self._ksinsecure = _read_cfg(cfg_parser, 'auth', 'insecure',
                          self._DEFAULT_INSECURE)
            self._kscertfile=_read_cfg(cfg_parser,'auth','certfile','')
            self._kskeyfile=_read_cfg(cfg_parser,'auth','keyfile','')
            self._kscafile=_read_cfg(cfg_parser,'auth','cafile','')
    ...
    def _authenticate(self, response=None, headers=None):
        ...
        try:
            if self._ksinsecure:
                ...
                response = requests.post(url, data=self._authn_body,
                                         headers=self._DEFAULT_AUTHN_HEADERS, verify=self._kscafile,
    ...
    def _http_get(self, uri, headers=None, query_params=None):
        ...
        if self._apiinsecure:
             response = self._api_server_session.get(url, headers=headers,
                                                      params=query_params,verify=VncApi._DEFAULT_INSECURE)
        elif not self._apiinsecure and self._use_api_certs:
             response = self._api_server_session.get(url, headers=headers,
                                                     params=query_params,verify=self._apicafile,
    ...
    def _http_post(self, uri, body, headers):
        ...
        if self._apiinsecure:
             response = self._api_server_session.post(url, data=body,
                                                     headers=headers, verify=VncApi._DEFAULT_INSECURE)
        elif not self._apiinsecure and self._use_api_certs:
             response = self._api_server_session.post(url, data=body,
                                                      headers=headers, verify=self._apicafile,
                                                      cert=[self._apicertfile,self._apikeyfile])
        else:
             response = self._api_server_session.post(url, data=body,
                                                      headers=headers)
