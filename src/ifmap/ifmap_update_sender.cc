...
public:
    ...
    virtual bool Run() {
        ...
        for (size_t i = send_scheduled.find_first(); i != BitSet::npos;
            ...
            IFMAP_UPD_SENDER_TRACE(IFMapUpdSender7, "Send scheduled for",
        ...
        if (sender_->queue_active_) {
            ...
            // Reset queue_active_
            IFMAP_UPD_SENDER_TRACE(IFMapUpdSender8, "Queue active for",
...
void IFMapUpdateSender::Send(IFMapMarker *imarker) {
    ...
    if (!blocked_clients.empty()) {
        IFMAP_UPD_SENDER_TRACE(IFMapUpdSender1, "Splitting blocked clients",
        ...
        if (message_->IsFull() ||
            ...
            if (!blocked_set.empty()) {
                ...
                if (blocked_set == marker->mask) {
                    IFMAP_UPD_SENDER_TRACE(IFMapUpdSender2, marker->ToString(),
                ...
                // traversal.
                IFMAP_UPD_SENDER_TRACE(IFMapUpdSender3, "Clients",
    ...
    if (marker != last) {
        ...
        // useless.
        IFMAP_UPD_SENDER_TRACE(IFMapUpdSender4, "Moving", marker->ToString(),
...
IFMapMarker* IFMapUpdateSender::ProcessMarker(IFMapMarker *marker,
    ...
    if (blocked_set.empty() || ready_set.empty()) {
        IFMAP_UPD_SENDER_TRACE(IFMapUpdSender5, "Merging", marker->ToString(),
        ...
        queue_->MarkerMerge(next_marker, marker, marker->mask);
        assert(next_marker->mask == total_set);
        IFMAP_UPD_SENDER_TRACE(IFMapUpdSender6, "Splitting blocked clients",
