...
public:
    ...
    static bool BufferStageCommon(const IFMapTableShowReq *request,
                                  ...
                                  const string &next_table_name, int partition,
...
bool ShowIFMapTable::TableToBuffer(const IFMapTableShowReq *request,
    ...
    for (int i = start_part_id; i < IFMapTable::kPartitionCount; ++i) {
        ...
        while (src) {
            ...
            if (show_data->send_buffer.size() == kMaxElementsPerRound) {
                ...
                // Use src_node, not src which is pointing to the next node.
                show_data->last_node_name = src_node->name();
                show_data->partition_id = i;
                buffer_full = true;
                break;
            }
        }
...
bool ShowIFMapTable::BufferOneTable(const IFMapTableShowReq *request,
        RequestPipeline::InstData *data, int partition,
        const string &last_node_name) {
    ...
    if (table) {
        ...
        TableToBuffer(request, table, sctx->ifmap_server(), partition,
                      last_node_name, show_data);
    } else {
...
bool ShowIFMapTable::BufferAllTables(const IFMapTableShowReq *request,
        ...
        int partition, const string &last_node_name) {
    IFMapSandeshContext *sctx = 
        static_cast<IFMapSandeshContext *>(request->module_context("IFMap"));
    ...
    for (; iter != db->end(); ++iter) {
        ...
        bool buffer_full = TableToBuffer(request, table, sctx->ifmap_server(),
                                         partition, last_node_name, show_data);
        if (buffer_full) {
            break;
        }
    }
...
bool ShowIFMapTable::ConvertReqIterateToReq(
    ...
    *next_table_name = node_info.substr((pos2 + sep_size),
    ...
    // partition
    size_t pos4 = node_info.find(kShowIterSeparator, (pos3 + sep_size));
    if (pos4 == string::npos) {
        return false;
    }
    string partition_str = node_info.substr((pos3 + sep_size),
                                            pos4 - (pos3 + sep_size));
    *partition = atoi(partition_str.c_str());

    // last_node_name
    *last_node_name = node_info.substr(pos4 + sep_size);
...
bool ShowIFMapTable::BufferStageCommon(const IFMapTableShowReq *request,
    ...
    if (request->get_table_name().length()) {
        return BufferOneTable(request, data, partition, last_node_name);
    } else {
        return BufferAllTables(request, data, next_table_name, partition,
                               last_node_name);
    }
}
...
bool ShowIFMapTable::BufferStage(const Sandesh *sr,
    ...
    int partition = 0;
    string last_node_name;
    return BufferStageCommon(request, data, next_table_name, partition,
                             last_node_name);
}
...
bool ShowIFMapTable::BufferStageIterate(const Sandesh *sr,
    ...
    bool success = ConvertReqIterateToReq(request_iterate, request,
                        &next_table_name, &partition, &last_node_name);
    if (success) {
        BufferStageCommon(request, data, next_table_name, partition,
                          last_node_name);
    }
    request->Release();
    return true;
...
void ShowIFMapTable::SendStageCommon(const IFMapTableShowReq *request,
    ...
    if (dest_buffer.size() == kMaxElementsPerRound) {
        next_batch = request->get_table_name() + kShowIterSeparator +
                     ...
                     integerToString(show_data.partition_id) +
                     kShowIterSeparator + show_data.last_node_name;
    }
...
bool ShowIFMapLinkTable::BufferStageCommon(const IFMapLinkTableShowReq *request,
    ...
    if (table) {
        ...
        while (src) {
            ...
            if (show_data->send_buffer.size() == kMaxElementsPerRound) {
                // Use src_link, not src which is pointing to the next node.
