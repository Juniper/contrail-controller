...
bool ConfigCassandraClient::UUIDToFQNameShow(
    const std::string &search_string, const string &last_uuid,
    uint32_t num_entries,
    vector<ConfigDBFQNameCacheEntry> *entries) const {
    uint32_t count = 0;
    bool more = false;
    boost::regex search_expr(search_string);
    tbb::spin_rw_mutex::scoped_lock read_lock(rw_mutex_, false);
    for (FQNameCacheMap::const_iterator it =
        fq_name_cache_.upper_bound(last_uuid);
        it != fq_name_cache_.end(); it++) {
        if (boost::regex_search(it->first, search_expr) ||
            boost::regex_search(it->second.obj_type, search_expr) ||
            boost::regex_search(it->second.obj_name, search_expr)) {
...
bool ConfigCassandraClient::UUIDToObjCacheShow(
    const std::string &search_string, int inst_num, const string &last_uuid,
