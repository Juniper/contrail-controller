        ...
        retry_timer_ = TimerManager::CreateTimer(
                *parent_->client()->event_manager()->io_service(),
                ...
                TaskScheduler::GetInstance()->GetTaskId(
                                "cassandra::Reader"),
                parent_->worker_id_);
        retry_timer_created_ = true;
        IFMAP_WARN(IFMapGetRowError, "created timer \n", uuid, uuid);
    }
    uint32_t retry_time_pow_of_two =
        retry_count_ > kMaxUUIDRetryTimePowOfTwo ?
        kMaxUUIDRetryTimePowOfTwo : retry_count_;
    retry_timer_->Start(((1 << retry_time_pow_of_two) * 1000),
...
void ConfigCassandraPartition::ObjectCacheEntry::DisableCassReadRetry(string uuid) {
    if(retry_timer_created_) {
        retry_timer_->Cancel();
        TimerManager::DeleteTimer(retry_timer_);
        retry_timer_created_ = false;
...
bool ConfigCassandraPartition::ObjectCacheEntry::CassReadRetryTimerExpired(string uuid) {
    parent_->client()->mgr()->EnqueueUUIDRequest("UPDATE", obj_type_, parent_->client()->uuid_str(uuid));
    retry_count_++;
