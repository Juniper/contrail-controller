...
*ConfigCassandraPartition::GetObjectCacheEntry(string uuid) {
...
void ConfigCassandraPartition::ObjectCacheEntry::EnableCassReadRetry(string uuid) {
    if (!retry_timer_) {
        ...
        IFMAP_DEBUG(IFMapCassReadRetry, "created timer \n", uuid);
    }
    retry_timer_->Start(parent_->UUIDRetryTimeInMSec(this),
            ...
            boost::bind(
                ...
                this));
    IFMAP_DEBUG(IFMapCassReadRetry, "start/restart timer due to config\n", uuid);
}
...
void ConfigCassandraPartition::ObjectCacheEntry::DisableCassReadRetry(string uuid) {
    if(retry_timer_created_) {
        ...
        IFMAP_DEBUG(IFMapCassReadRetry, "deleted timer due to config\n", uuid);
    }
}
...
bool ConfigCassandraPartition::ObjectCacheEntry::CassReadRetryTimerExpired(
    ...
    retry_count_++;
    IFMAP_DEBUG(IFMapCassReadRetry, "timer expired \n", uuid);
    return true;
}
...
void ConfigCassandraPartition::ObjectCacheEntry::CassReadRetryTimerErrorHandler() {
     std::string message = "Timer";
        IFMAP_WARN(IFMapGetRowError, 
                "Timer error while retrying read of obj with unresloved ref/parent",
                message, message);
}
...
bool ConfigCassandraPartition::StoreKeyIfUpdated(const string &uuid,
    ...
    if (is_ref || is_parent) {
        ...
        if (ref_name == "ERROR") {
            ...
            IFMAP_DEBUG(IFMapCassReadRetry, "Out of order parent or ref\n", uuid + ":" + field_name);
            return false;
        }
