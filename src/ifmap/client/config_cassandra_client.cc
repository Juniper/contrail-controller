...
void ConfigCassandraPartition::ObjectCacheEntry::EnableCassReadRetry(string uuid) {
    ...
    if (!this->retry_timer_) {
        ...
        this->retry_timer_->Start(((1 << retry_time_pow_of_two) * 1000),
            ...
            IFMAP_WARN(IFMapGetRowError, "created and stated timer \n", uuid, uuid);
    } else {
        this->retry_timer_->Cancel();
        this->retry_timer_->Reschedule((1 << retry_time_pow_of_two)*1000);
            IFMAP_WARN(IFMapGetRowError, "rescheduled timer due to config\n", uuid, uuid);
    }
}
