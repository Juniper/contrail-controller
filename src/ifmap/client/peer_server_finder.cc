...
void PeerIFMapServerFinder::PeerIFMapDSSubscribe() {
    ...
    if (DSExists()) {
        ...
        ds_client_->Subscribe(subscriber_name_, service_name_, num_instances,
...
bool PeerIFMapServerFinder::FirstDSPeerAvailable(size_t *index) {
    bool exists = false;
    if (peer_ifmap_servers_.empty()) {
        exists = false;
    } else {
        // Return the 'first' peer in the db i.e. index 0
        *index = 0;
        peer_ifmap_servers_[0].in_use = true;
        exists = true;
    }
    return exists;
}
...
// 'index' gives the next available peer only if return value is true
bool PeerIFMapServerFinder::NextDSPeerAvailable(size_t *index) {
    bool exists = false;
    if (peer_ifmap_servers_.empty()) {
        exists = false;
    } else {
...
bool PeerIFMapServerFinder::GetBestPeer(PeerIFMapServerInfo *srv_info,
    ...
    if (ValidStaticConfig()) {
        *srv_info = static_peer_;
        ...
        size_t index;
        if (get_first) {
            if (FirstDSPeerAvailable(&index)) {
                *srv_info = peer_ifmap_servers_[index];
                is_valid = true;
            }
        } else {
            if (NextDSPeerAvailable(&index)) {
                *srv_info = peer_ifmap_servers_[index];
                is_valid = true;
            }
        }
...
void PeerIFMapServerFinder::ProcPeerIFMapDSResp(
    ...
    if (PeerExistsInDb(old_peer, &index)) {
        ...
        bool get_first = true;
        bool valid = GetBestPeer(&srv_info, get_first);
...
bool PeerIFMapServerFinder::PeerDown() {
    ...
    bool is_valid = GetBestPeer(&srv_info, get_first);
...
void PeerIFMapServerFinder::GetAllDSPeerInfo(IFMapDSPeerInfo *ds_peer_info) {
    ...
    ds_peer_info->set_subscriber_name(get_subscriber_name());
