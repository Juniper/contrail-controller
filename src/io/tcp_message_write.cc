...
int TcpMessageWriter::AsyncSend(const uint8_t *data, size_t len, error_code *ec) {
    ...
    if ((buffer_queue_.size() + len) > TcpMessageWriter::kMaxPendingBufferSize) {
        /* throttle the sender */
    ...
    if (session_->io_strand_) {
        session_->io_strand_->post(bind(&TcpSession::AsyncWriteInternal, session_,
                                   TcpSessionPtr(session_)));
    }
    return len;
}

void TcpMessageWriter::HandleAsyncWrite() {

    /* Return if there is an async write in progress */
    if (last_write_) return;
    ...
    if (!buffer_queue_.empty()) {
        ...
        int write  = std::min(TcpMessageWriter::kDefaultWriteBufferSize, remaining);
        last_write_ = write;
...
void TcpMessageWriter::FlushBuffer(size_t wrote, bool *send_ready) {
    ...
    if (!buffer_queue_.empty()) {
        ...
        if (buffer_queue_.size() < TcpMessageWriter::kMinPendingBufferSize) {
            ...
            session_->server_->stats_.write_blocked_duration_usecs += 
                                                             blocked_usecs;
            *send_ready = true;
        }
        return;
