...
void
UnixDomainSocketSession::Send(const uint8_t *data,
                              int data_len)
{
  if (!data || !data_len) {
    return;
...
UnixDomainSocketSession::WriteToSocket()
{
  ...
  boost::asio::async_write(socket_, buffer(buffer_cast<const uint8_t *>(head),
...
void
UnixDomainSocketSession::AppendBuffer(const uint8_t *src,
                                      int bytes)
{
