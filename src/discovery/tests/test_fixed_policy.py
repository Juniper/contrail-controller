...
class DiscoveryServerTestCase(test_case.DsTestCase):
    def setUp(self):
        test_id = self.id()
        section = 'foobar'
        if test_id == 'tests.test_fixed_policy.DiscoveryServerTestCase.test_fixed_policy_global':
            section = 'DEFAULTS'
        extra_config_knobs = [
            (section, 'policy', 'fixed'),
        ]
    ...
    def test_fixed_policy(self):
        ...
        for entry in response['services']:
            if entry['service_id'] == 'test_discovery-1:%s' % service_type:
                break
    ...
    def test_fixed_policy_global(self):
        global server_list
        # publish 3 instances of service foobar
        tasks = []
