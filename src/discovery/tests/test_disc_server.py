...
class PolicyTest(unittest.TestCase):
    ...
    def _insert_fake_services(self):
        t = time.time() - 5
    ...
    def test_round_robin_one_instance(self):
        self._insert_fake_services()
    ...
    def test_round_robin_two_instances(self):
	self._insert_fake_services()
        ...
        with mock.patch.object(self.server, 'get_service_config',
            ...
            for expt in expected:
                res = self.server.api_subscribe()
                self.assertEquals(res['IfmapServer'][0]['ip-address'], expt[0])
                self.assertEquals(res['IfmapServer'][1]['ip-address'], expt[1])
