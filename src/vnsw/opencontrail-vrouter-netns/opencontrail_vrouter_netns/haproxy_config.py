...
def build_config(conf_file, barbican_conf_file):
    ...
    conf += _set_frontend(config, conf_dir, barbican_conf_file) + '\n\n'
...
def _set_frontend(config, conf_dir, barbican_conf_file):
    port = config['vip']['port']
    vip_custom_attributes = validator(config, 'vip', barbican_conf_file)
    ssl = ''
