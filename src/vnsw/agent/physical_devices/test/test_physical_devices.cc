...
TEST_F(TestPhysicalDevice, device_1) {
    AgentUtXmlTest test("controller/src/vnsw/agent/physical_devices/test/physical-device.xml");
...
TEST_F(TestPhysicalDevice, phyiscal_port_1) {
    AgentUtXmlTest test("controller/src/vnsw/agent/physical_devices/test/physical-interface.xml");
    AgentUtXmlOperInit(&test);
    AgentUtXmlPhysicalDeviceInit(&test);
    if (test.Load() == true) {
        test.ReadXml();
...
TEST_F(TestPhysicalDevice, logical_port_1) {
    AgentUtXmlTest test("controller/src/vnsw/agent/physical_devices/test/logical-interface.xml");
    AgentUtXmlOperInit(&test);
    AgentUtXmlPhysicalDeviceInit(&test);
    if (test.Load() == true) {
        test.ReadXml();
...
TEST_F(TestPhysicalDevice, physical_device_vn_1) {
    AgentUtXmlTest test("controller/src/vnsw/agent/physical_devices/test/physical-device-vn.xml");
    AgentUtXmlOperInit(&test);
    AgentUtXmlPhysicalDeviceInit(&test);
    if (test.Load() == true) {
        test.ReadXml();
