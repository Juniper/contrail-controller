...
void AgentXmppConnectionStatusReq::HandleRequest() const {
    ...
    while (count < MAX_XMPP_SERVERS) {
        if (!agent->controller_ifmap_xmpp_server(count).empty()) {
            ...
            AgentXmppData data;
            data.set_controller_ip(agent->controller_ifmap_xmpp_server(count));
            AgentXmppChannel *ch = agent->controller_xmpp_channel(count);
            AgentIfMapXmppChannel *ifmap_ch =agent->ifmap_xmpp_channel(count);
            ...
            if (ch) {
                ...
                data.set_last_ready_time(
                    integerToString(UTCUsecToPTime
                        (agent->controller_xmpp_channel_setup_time(count))));
                ...
                if (ifmap_ch) {
                    ...
                    eoc_stats.set_last_start_time(
                        integerToString(UTCUsecToPTime(eoc_timer->
                                                       last_restart_time_)));
                ...
                if (ifmap_ch) {
                    ConfigCleanupTimer *cleanup_timer = ifmap_ch->
                ...
                eor_tx.set_end_of_rib_tx_time
                    (integerToString(UTCUsecToPTime
                         (eor_tx_timer->end_of_rib_tx_time_)));
                eor_tx.set_last_route_published_time
                    (integerToString(UTCUsecToPTime
                ...
                eor_rx.set_end_of_rib_rx_time
                    (integerToString(UTCUsecToPTime
                         (eor_rx_timer->end_of_rib_rx_time_)));
                eor_rx.set_last_start_time
                    (integerToString(UTCUsecToPTime
                         (eor_tx_timer->last_restart_time_)));
                ...
                if (agent->mulitcast_builder() == ch) {
                    data.set_mcast_controller("Yes");
                } else {
                    data.set_mcast_controller("No");
                }

                if (agent->ifmap_active_xmpp_server().compare
                    (agent->controller_ifmap_xmpp_server(count)) == 0) {
                ...
                data.set_dscp(ChannelToDscp(xc));
...
void AgentDnsXmppConnectionStatusReq::HandleRequest() const {
    ...
    while (dns_count < MAX_XMPP_SERVERS) {
        if (!agent->dns_server(dns_count).empty()) {

            AgentXmppDnsData data;
            data.set_dns_controller_ip(agent->dns_server(dns_count));

            AgentDnsXmppChannel *ch = agent->dns_xmpp_channel(dns_count);
            if (ch) {
                XmppChannel *xc = ch->GetXmppChannel();
                if (xc->GetPeerState() == xmps::READY) {
                    data.set_state("Established");
