...
AgentXmppChannel::AgentXmppChannel(Agent *agent,
    ...
    : channel_(NULL), xmpp_server_(xmpp_server), label_range_(label_range),
      ...
      unicast_sequence_number_(0), agent_(agent) {
    bgp_peer_id_.reset();
...
void AgentXmppChannel::SetMulticastPeer(AgentXmppChannel *old_peer,
    ...
    if (new_peer)
        MulticastHandler::GetInstance()->FlushPeerInfo(old_peer->agent()->
...
void AgentXmppChannel::ControllerPeerDelDoneEnqueue() {
    ControllerDeletePeerDataType data(new ControllerDeletePeerData(bgp_peer_id()));
 ...
 * destruction of same.
 */
bool AgentXmppChannel::ControllerPeerDelDone() {
    // Retain the disconnect state for peer as bgp_peer will be freed
    // below.
    bool is_disconnect_walk = bgp_peer_id()->is_disconnect_walk();
    DynamicPeer::ProcessDelete(bgp_peer_id());
    // Delete walk for peer was issued via shutdown of agentxmppchannel
    if (is_disconnect_walk) {
 ...
 */
void AgentXmppChannel::DelPeerRoutes() {
    bgp_peer_id()->DelPeerRoutes(
              boost::bind(&VNController::ControllerPeerDelDoneEnqueue,
                          this, bgp_peer));
}
