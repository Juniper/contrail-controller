...
AgentXmppChannel::AgentXmppChannel(Agent *agent,
    ...
    : channel_(NULL), xmpp_server_(xmpp_server), label_range_(label_range),
      ...
      sequence_number_(0), end_of_rib_seen_(false), end_of_rib_sent_(false),
      end_of_config_(false), end_of_rib_seen_time_(0), end_of_rib_sent_time_(0),
      end_of_config_time_(0), agent_(agent) {
    bgp_peer_id_.reset();
    end_of_rib_timer_.reset(new EndOfRibTimer(agent));
    stale_path_timer_.reset(new StalePathTimer(agent));
    CreateBgpPeer();
}
...
AgentXmppChannel::~AgentXmppChannel() {
    ...
    end_of_rib_timer_.reset();
    stale_path_timer_.reset();
}
...
bool AgentXmppChannel::SetConfigPeer(AgentXmppChannel *peer) {
    ...
    if (AgentXmppChannel::ControllerSendCfgSubscribe(peer)) {
        ...
        agent->controller()->end_of_config_timer().Start(peer);
        return true;
    }
    return false;
}
...
void AgentXmppChannel::HandleAgentXmppClientChannelEvent(AgentXmppChannel *peer,
    ...
    if (state == xmps::READY) {
        ...
        agent->controller()->StartStalePathTimer();
        ...
        peer->set_end_of_rib_seen(false);
        peer->set_end_of_rib_sent(false);
        peer->set_end_of_config(false);
...
void AgentXmppChannel::NotifyRoutes() {
    if ((agent_->controller()->end_of_config_timer().running() == false) &&
        (end_of_rib_timer()->running() == false))
        agent_->controller()->StartEndOfRibWalker(GetXmppServerIdx());
}

ControllerTimer *AgentXmppChannel::end_of_rib_timer() {
    return end_of_rib_timer_.get();
}

ControllerTimer *AgentXmppChannel::stale_path_timer() {
    return stale_path_timer_.get();
}

void AgentXmppChannel::AllConfigPeersGone() {
    agent_->controller()->end_of_config_timer().Cancel();
    agent_->controller()->config_cleanup_timer().Cancel();
}
...
bool AgentXmppChannel::ControllerSendV4V6UnicastRouteCommon(AgentRoute *route,
    ...
    // send data
    SendUpdate(data_,datalen_);
    route_published_time_ = UTCTimestampUsec(); 
    return true;
}
...
void AgentXmppChannel::ReceiveEndOfRib() {
    set_end_of_rib_seen(true);
    set_end_of_rib_seen_time(UTCTimestampUsec());
    stale_path_timer_.get()->Cancel();
...
void AgentXmppChannel::SendEndOfRib() {
    string msg;
    msg += "\n<message from=\"";
    ...
    if (!end_of_rib_sent_) {
        set_end_of_rib_sent(true);
        set_end_of_rib_sent_time(UTCTimestampUsec());
    }
}
