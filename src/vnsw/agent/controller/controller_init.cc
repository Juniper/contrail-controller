...
void VNController::ReConnect() {
    ...
    if (controller_list_chksum_ != 
        ...
        LOG(DEBUG, "ReConnect list[0]:" << controller_list[0] << "\n\n");
        LOG(DEBUG, "ReConnect list[1]:" << controller_list[1] << "\n\n");
        LOG(DEBUG, "ReConnect list[2]:" << controller_list[2] << "\n\n");
        ControllerReConfigDataType cdata(new ControllerReConfigData(
            g_vns_constants.XMPP_SERVER_DISCOVERY_SERVICE_NAME, 
            controller_list));
        base_data = boost::static_pointer_cast<ControllerWorkQueueData>(cdata);
    ...
    if (dns_list_chksum_ != 
        ...
        LOG(DEBUG, "ReConnect dns list[0]:" << dns_list[0] << "\n\n");
        LOG(DEBUG, "ReConnect dns list[1]:" << dns_list[1] << "\n\n");
        LOG(DEBUG, "ReConnect dns list[2]:" << dns_list[2] << "\n\n");
        ControllerReConfigDataType ddata(new ControllerReConfigData(
            g_vns_constants.DNS_SERVER_DISCOVERY_SERVICE_NAME, 
            dns_list));
        base_data = boost::static_pointer_cast<ControllerWorkQueueData>(ddata);
        work_queue_.Enqueue(base_data);
    }
}
...
bool VNController::ApplyControllerReConfigInternal(std::vector<string> resp) {
    ...
    for (iter = resp.begin(); ++count < min_iter; iter++) {
        ...
        uint32_t port;
        std::istringstream converter(server_port);
        converter >> port;
        ...
        if (chnl) {
            ...
            for (uint8_t xs_idx = 0; xs_idx < MAX_XMPP_SERVERS; xs_idx++) {
                ...
                if (agent_->controller_ifmap_xmpp_server(xs_idx).empty()) {
                    ...
                    CONTROLLER_DISCOVERY_TRACE(DiscoveryConnection,
                        "Set Xmpp Channel", xs_idx, server_ip, server_port);
