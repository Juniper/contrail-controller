...
struct UnicastCleanupTimer : public ControllerTimer {
    static const uint32_t kUnicastStaleTimer = (2 * 60 * 1000); 
    UnicastCleanupTimer(Agent *agent)
        : ControllerTimer(agent, "Agent Unicast Stale cleanup timer", 
                          kUnicastStaleTimer) { };
...
struct MulticastCleanupTimer : public ControllerTimer {
    static const uint32_t kMulticastStaleTimer = (5 * 60 * 1000); 
    MulticastCleanupTimer(Agent *agent) 
        : ControllerTimer(agent, "Agent Multicast Stale cleanup timer",
