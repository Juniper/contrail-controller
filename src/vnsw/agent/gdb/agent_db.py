...
def dump_route_paths(route_entry):
   if route_entry:
      route_table = gdb.parse_and_eval(str(route_entry))
      if route_table:
         path_list = route_table['path_']
         count = path_list['data_']['root_plus_size_']['size_']
         print( "Number of paths : %s\n" % (count))
         print_db_entries(route_table, print_route_entry)

def print_ifmap_link_entry(entry_ptr, entry):
...
def dump_ifmap_link_entries(table):
    ...
    print_db_entries(ifmap_table, print_ifmap_link_entry)
