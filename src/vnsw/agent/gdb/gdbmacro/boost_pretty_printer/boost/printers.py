...
@_register_printer
class BoostIntrisiveListHook:
    "Pretty Printer for boost::intrusive::list_member_hook"
    printer_name = 'boost::intrusive::list_member_hook'
    version = '1.40'
    type_name_re = '^boost::intrusive::list_member_hook<(.*)>$'
    ...
    def to_string(self):
        ...
        n = int(self.value['next_'])
        p = int(self.value['prev_'])
        return '<boost::intrusive_hook> next = 0x%x prev = 0x%x' % (n, p)

@_register_printer
class BoostIntrisiveListHook:
    "Pretty Printer for boost::intrusive::set_member_hook"
    printer_name = 'boost::intrusive::set_member_hook'
    version = '1.40'
    type_name_re = '^boost::intrusive::set_member_hook<(.*)>$'
    ...
    def to_string(self):
        #import pdb; pdb.set_trace()
        p = int(self.value['parent_'])
        l = int(self.value['left_'])
        r = int(self.value['right_'])
        return '<boost::intrusive_hook> parent = 0x%x left = 0x%x right = 0x%x' % (p, l, r)

@_register_printer
class BoostIpAddress:
    "Pretty Printer for boost::asio::ip::address"
    printer_name = 'boost::asio::ip::address'
    version = '1.40'
    type_name_re = '^boost::asio::ip::address$'
    ...
    def to_string(self):
        #import pdb; pdb.set_trace()
        family = str(self.value['type_'])
        if 'ipv4' in family:
            v4 = int(self.value['ipv4_address_']['addr_']['s_addr'])
            return '<boost::IPv4> %08x' % (v4)
        else:
            v6 = self.value['ipv6_address_']['addr_']['__in6_u']['__u6_addr32']
            return '<boost::IPv6> %08x::%08x::%08x::%08x' % (int(v6[0]), int(v6[1]), int(v6[2]), int(v6[3]))
#
