...
define nova_intf_entry_format
    set $__nova_intf = (CfgIntEntry *) ((size_t)($Xnode) - (size_t)&(((CfgIntEntry*)0)->node_))
    printf "%p    flags=%-4d    name=%s\n", $__nova_intf, $__nova_intf->flags, $__nova_intf->tap_name_._M_dataplus._M_p
end

define dump_nova_intf_entries
   pdb_table_entries Agent::singleton_->intf_cfg_table_ nova_intf_entry_format
end
...
define pksync_entries
    if $argc != 1 && $argc != 2
        ...
        # set the node equal to first node and end marker
        set $Xtree = &((KSyncDBObject *)$arg1)->tree_.tree_
        set $Xnode = $Xtree->data_.node_plus_pred_.header_plus_size_.header_.left_
        set $Xend = &($Xtree->data_.node_plus_pred_.header_plus_size_.header_)
