...
public:
  ...
  ResourceManager(const std::string &file, const std::string &tmp_file,
          Agent *agent):file_path_(file),file_tmp_path_(tmp_file),
    agent_(agent)  {
...
protected:
  ...
  {
    std::ofstream output;
    output.open(file_tmp_path_.c_str(), std::ofstream::binary | std::ofstream::trunc);
    if (!output.good()) {
        output.close();
        RESOURCEMANAGER_TRACE(Trace, "Write to file failed", file_tmp_path_);
        return; 
    }
    ...
    std::string copy ("cp "  + file_tmp_path_ + "  " + file_path_);
    if (-1 == system(copy.c_str())) {
        RESOURCEMANAGER_TRACE(Trace, "copy command failed to", file_path_);
    }
  }

  void ReadResourceDataFromFile() {
    std::ifstream input (file_path_.c_str(), std::ofstream::binary);
    if (!input.good()) {
        input.close();
        RESOURCEMANAGER_TRACE(Trace, "Read from file failed", file_path_);
        return;
    }
