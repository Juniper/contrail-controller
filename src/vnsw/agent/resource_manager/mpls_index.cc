...
//VM interface
InterfaceIndexResourceKey::InterfaceIndexResourceKey(ResourceManager *rm,
...
void InterfaceIndexResourceKey::Backup(ResourceData *data, bool del) {
    ...
    if (del) {
        rm_->backup_mgr()->sandesh_maps().interface_mpls_index_map_.erase
            (index_data->GetIndex());
    } else {
        rm_->backup_mgr()->sandesh_maps().interface_mpls_index_map_.insert
    ...
    //number internally.
    rm_->backup_mgr()->sandesh_maps().incr_sequence_number();
}
...
void VrfMplsResourceKey::Backup(ResourceData *data, bool del) {
    ...
    if (del) {
        rm_->backup_mgr()->sandesh_maps().vrf_mpls_index_map_.erase
            (index_data->GetIndex());
    } else {
        rm_->backup_mgr()->sandesh_maps().vrf_mpls_index_map_.insert
...
void RouteMplsResourceKey::Backup(ResourceData *data, bool del) {
    ...
    if (del) {
        rm_->backup_mgr()->sandesh_maps().route_mpls_index_map_.erase
            (index_data->GetIndex());
    } else {
        rm_->backup_mgr()->sandesh_maps().route_mpls_index_map_.insert
