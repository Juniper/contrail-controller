...
ResourceBackupManager::ResourceBackupManager(Agent *agent) :
    sandesh_maps_(), agent_(agent) {
    timer_ = TimerManager::CreateTimer(*(agent->event_manager()->io_service()),
                                       "ResourceBackupManager");
    timer_->Start(kResourceSyncTimeout,
                  boost::bind(&ResourceBackupManager::TimerExpiry, this));
    //Read stored resource file and restore resources
    //ReadFromFile();
    sandesh_maps_.RestoreResource(agent);
}
...
bool ResourceBackupManager::TimerExpiry() {
    if (sandesh_maps_.timer_processed_sequence_number() !=
        sandesh_maps_.sequence_number()) {
        WriteToFile();
        sandesh_maps_.update_timer_processed_sequence_number();
    }
    return true;
}

void ResourceBackupManager::WriteToFile() {
    tbb::mutex::scoped_lock lock(mutex_);
    sandesh_maps_.WriteToFile(this);
}

void ResourceBackupManager::ReadFromFile() {
    tbb::mutex::scoped_lock lock(mutex_);
    sandesh_maps_.ReadFromFile(this);
}
...
void ResourceBackupManager::SaveResourceDataToFile(const std::string &file_name,
    ...
    std::string copy("cp "  + file_tmp_name + "  " + file_name);
    system(copy.c_str());
}
