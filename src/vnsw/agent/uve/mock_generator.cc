...
typedef std::map<uint32_t, map<IpAddress, SessionAggInfo> > SessionAggMap;
    ...
    public:
        ...
        bool Run() {
                        ...
                        for (int nsession = 0; nsession < mgen_->num_session_per_vm_;
                            ...
                            for (int i = 0; i < nsport; i++) {
                                ...
                                IpAddress ipaddr(Ip4Address(mgen_->ip_vns_[vn] +
                                    mgen_->ip_start_index_ + nvm));
                                sess_agg_ip_map[ipaddr];
                                uint16_t protoIdx(mgen_->dProtocols(mgen_->rgen_));
                                uint32_t port = mgen_->kPorts[protoIdx];
                                uint32_t proto = mgen_->kProtocols[protoIdx];
                                uint32_t port_proto = (proto << 16) | port;
                                sess_agg_map[port_proto] = sess_agg_ip_map; 
                            }
            ...
            for (std::vector<SessionEndpoint>::iterator it = begin;
                ...
                SessionAggMap sess_agg_port_proto_map; 
                for (SessionAggMap::const_iterator it1 
                    = end_point.get_sess_agg_map().begin();
                    it1 != end_point.get_sess_agg_map().end(); ++it1) {
                    std::map<IpAddress, SessionAggInfo> sess_agg_ip_map;
                    for (std::map<IpAddress, SessionAggInfo>::const_iterator it2 = 
                        (it1->second).begin(); it2 != (it1->second).end(); ++it2) {
                        SessionAggInfo sess_agg_map;
                        sess_agg_map.set_sampled_tx_bytes(0);
                        sess_agg_map.set_sampled_tx_pkts(0);
                        sess_agg_map.set_sampled_rx_bytes(0);
                        sess_agg_map.set_sampled_rx_pkts(0);
                        std::map<uint32_t, SessionPortProtMap> session_map;
                        int ncport = mgen_->nport(mgen_->rgen_);
                        for (int i = 0; i < ncport; i++) {
                            uint32_t cport(mgen_->dPort(mgen_->rgen_));
                            uint32_t cprot(mgen_->dPort(mgen_->rgen_));
                            uint32_t cport_proto = (cprot << 16) | cport;
                            SessionPortProtMap sess_prot_port_map;
                            ...
                            for (int j = 0; j < nip; j++) {
                                ...
                                session_val.set_sampled_rx_bytes(rx_pkts *
                                    mgen_->dBytesPerPacket(mgen_->rgen_));
                                sess_agg_map.set_sampled_tx_pkts(
                                    sess_agg_map.get_sampled_tx_pkts() + tx_pkts);
                                sess_agg_map.set_sampled_tx_bytes(
                                    sess_agg_map.get_sampled_tx_bytes() +
                                        session_val.get_sampled_tx_bytes());
                                sess_agg_map.set_sampled_rx_pkts(
                                    sess_agg_map.get_sampled_rx_pkts() + rx_pkts);
                                sess_agg_map.set_sampled_rx_bytes(
                                    sess_agg_map.get_sampled_rx_bytes() +
                                ...
                                IpAddress ipaddr(Ip4Address(mgen_->ip_vns_[other_vn]
                                    + mgen_->ip_start_index_ + i));
                                sess_prot_port_map.insert(std::make_pair(ipaddr, session_val));
                            }
                            session_map.insert(std::make_pair(cport_proto, sess_prot_port_map));
                        }
                        sess_agg_map.set_sessionMap(session_map);
                        sess_agg_ip_map.insert(std::make_pair(it2->first, sess_agg_map));
                    }
                    sess_agg_port_proto_map.insert(std::make_pair(it1->first, sess_agg_ip_map)); 
                }
                end_point.set_sess_agg_map(sess_agg_port_proto_map);
                lsession_cnt++;
                mgen_->session_counter_++;
