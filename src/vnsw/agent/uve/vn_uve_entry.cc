...
void VnUveEntry::BuildNhStats(const StatsManager::VrfStats *s,
    ...
    tns.set_udp_tunnels(s->udp_tunnels);
    tns.set_mpls_over_udp_tunnels(s->udp_mpls_tunnels);
    tns.set_mpls_over_gre_tunnels(s->gre_mpls_tunnels);
    tns.set_vxlan_tunnels(s->vxlan_tunnels);
