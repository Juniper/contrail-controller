...
VnUveTableBase::VnUveTableBase(Agent *agent, uint32_t default_intvl)
    : uve_vn_map_(), agent_(agent),
      ...
      timer_restart_trigger_
         (new TaskTrigger
              (boost::bind(&VnUveTableBase::RestartTimer, this),
              TaskScheduler::GetInstance()->GetTaskId("db::DBTable"), 0)) {
...
bool VnUveTableBase::TimerExpiry() {
    ...
    if (it == uve_vn_map_.end()) {
        timer_last_visited_ = "";
        auto_restart = set_expiry_time(agent_->uve()->default_interval());
    } else {
        timer_last_visited_ = it->first;
        auto_restart = set_expiry_time(agent_->uve()->incremental_interval());
    }
    return auto_restart;
}

bool VnUveTableBase::RestartTimer() {
    timer_->Cancel();
    timer_->Start(expiry_time_,
                  boost::bind(&VnUveTableBase::TimerExpiry, this));
    return true;
...
bool VnUveTableBase::set_expiry_time(int time) {
