...
bool VrouterUveEntry::SendVrouterMsg() {
    ...
    AgentStats::FlowCounters &created =  agent_->stats()->created();
    AgentStats::FlowCounters &aged =  agent_->stats()->aged();

    VrouterFlowRate flow_rate;
    bool built = uve->stats_manager()->BuildFlowRate(created, aged, flow_info_,
                                                     flow_rate);
    if (built) {
...
bool VrouterUveEntry::BuildPhysicalInterfaceFlowRate
    (std::map<std::string, VrouterFlowRate> *rate) {
    bool changed = false;
    PhysicalInterfaceSet::const_iterator it = phy_intf_set_.begin();
    while (it != phy_intf_set_.end()) {
        const Interface *intf = *it;
        ...
        VrouterFlowRate flow_rate;
        bool built = uve->stats_manager()->BuildFlowRate(s->created, s->aged,
                                                         s->flow_info,
                                                         flow_rate);
        if (built) {
            flow_rate.set_active_flows(agent_->pkt()->get_flow_proto()->
                                       InterfaceFlowCount(intf));
