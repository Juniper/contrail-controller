...
//Flow parameters (vrouter, other_vrouter and tunnel_type) verification for
//local flow
TEST_F(StatsTestMock, Underlay_1) {
    ...
    EXPECT_STREQ(fe->other_vrouter().c_str(),
    ...
    EXPECT_STREQ(rfe->other_vrouter().c_str(),
                 Agent::GetInstance()->router_id().to_string().c_str());
...
TEST_F(StatsTestMock, Underlay_2) {
    ...
    EXPECT_STREQ(fe->other_vrouter().c_str(), remote_router_ip);
    EXPECT_EQ(fe->tunnel_type().GetType(), TunnelType::MPLS_GRE);
    EXPECT_EQ(fe->underlay_source_port(), 0);

    EXPECT_STREQ(rfe->other_vrouter().c_str(), remote_router_ip);
