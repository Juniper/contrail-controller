...
void StatsManager::AddFlow(const boost::shared_ptr<FlowAceStatsRequest> &req) {
...
void StatsManager::DeleteFlow(const boost::shared_ptr<FlowAceStatsRequest>
                              &req) {
    FlowAceTree::iterator it = flow_ace_tree_.find(req->uuid());
    if (it == flow_ace_tree_.end()) {
        return;
...
void StatsManager::EnqueueEvent(const FlowAceStatsRequest &req) {
    boost::shared_ptr<FlowAceStatsRequest> req2(new FlowAceStatsRequest(req));
    request_queue_.Enqueue(req2);
}
