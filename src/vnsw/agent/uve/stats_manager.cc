...
bool StatsManager::BuildFlowRate(AgentStats::FlowCounters &created,
    ...
    if (flow_info.prev_time_) {
        ...
        if (diff_secs) {
            ...
            assert(max_add_rate != AgentStats::kInvalidFlowCount);
            assert(min_add_rate != AgentStats::kInvalidFlowCount);
            assert(max_del_rate != AgentStats::kInvalidFlowCount);
            assert(min_del_rate != AgentStats::kInvalidFlowCount);
