        ...
        if (diff_secs) {
            ...
            if (created_flows) {
                ...
                if (max_add_rate == AgentStats::kInvalidFlowCount) {
                    LOG(WARN, "Invalid max_flow_adds_per_second " <<
                        max_add_rate);
                    max_add_rate = 0;
                }
                if (min_add_rate == AgentStats::kInvalidFlowCount) {
                    LOG(WARN, "Invalid min_flow_adds_per_second " <<
                        min_add_rate);
                    min_add_rate = 0;
                }
            }
            if (aged_flows) {
                ...
                if (max_del_rate == AgentStats::kInvalidFlowCount) {
                    LOG(WARN, "Invalid max_flow_deletes_per_second " <<
                        max_del_rate);
                    max_del_rate = 0;
                }
                if (min_del_rate == AgentStats::kInvalidFlowCount) {
                    LOG(WARN, "Invalid min_flow_deletes_per_second " <<
                        min_del_rate);
                    min_del_rate = 0;
                }
            }
