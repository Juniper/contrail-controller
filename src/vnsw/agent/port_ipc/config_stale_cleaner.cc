...
DBTable::DBTableWalkRef InterfaceConfigStaleCleaner::WalkRef() {
    if ((walk_ref_ != NULL) || (agent_->interface_config_table() == NULL))
        return walk_ref_;

    walk_ref_ = agent_->interface_config_table()->AllocWalker(
