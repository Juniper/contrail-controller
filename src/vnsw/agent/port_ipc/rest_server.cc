...
public:
    ...
    virtual bool Run() {
        std::string info;
        std::string context = data_.session->get_context();
        if (context.empty())
            return true;
        if (pih_->GetVmVnCfgPort(vm_uuid_, info)) {
            REST::SendResponse(data_.session, info, 200, context);
        } else {
            REST::SendErrorResponse(data_.session, "{ Not Found }", 404, context);
        }
        return true;
    }
