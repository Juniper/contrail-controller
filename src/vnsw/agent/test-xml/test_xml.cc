...
void NovaIntfAdd(bool op_delete, const uuid &id, const Ip4Address &ip,
    ...
    InterfaceConfigVmiKey *key = new InterfaceConfigVmiKey(id);
    DBRequest req;
    req.key.reset(key);
    ...
    InterfaceConfigVmiData *data =
        new InterfaceConfigVmiData(InterfaceConfigVmiEntry::VM_INTERFACE, name,
                                   vm_uuid, vn_uuid, MakeUuid(0), ip,
