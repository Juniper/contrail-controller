...
bool FlowMgmtTree::TryDelete(FlowMgmtKey *key) {
    ...
    entry->PreFree(key, mgr_->agent()->pkt()->flow_table());
    tree_.erase(it);
    delete entry;
    delete it->first;
...
void FlowMgmtEntry::PreFree(FlowMgmtKey *key, FlowTable *table) {
    FlowTableRequest::Event event = key->FreeFlowTableEvent();
    if (event == FlowTableRequest::INVALID)
        return;

    if (oper_state_ != OPER_DEL_SEEN)
        return;
