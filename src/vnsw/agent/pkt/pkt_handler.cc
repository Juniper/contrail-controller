...
void PktHandler::CalculatePortIP(PktInfo *pkt) {
    ...
    pkt_in_intf = Agent::GetInstance()->interface_table()->FindInterface(pkt->agent_hdr.ifindex);
    ...
    ret = intf->IsFatFlowPrefixAggregation(ingress, pkt->ip_proto, (uint16_t *)&pkt->sport, 
                                     (uint16_t *) &pkt->dport, 
                                     &pkt->same_port_number, &pkt->ip_ff_src_prefix, 
                                     &pkt->ip_ff_dst_prefix,
                                     &pkt->is_fat_flow_src_prefix, 
                                     &pkt->is_fat_flow_dst_prefix,
                                     &pkt->ignore_address);
    if (ret) {
        LOG(ERROR, "IsFatFlowPrefix true for pkt " << (void*) pkt);
    }
}
...
PktInfo::PktInfo(const PacketBufferPtr &buff) :
    ...
    is_fat_flow_src_prefix(false), ip_ff_src_prefix(), is_fat_flow_dst_prefix(false),
    ip_ff_dst_prefix(), 
