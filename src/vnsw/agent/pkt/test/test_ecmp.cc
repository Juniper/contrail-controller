...
class EcmpTest : public ::testing::Test {
    virtual void SetUp() {
        ...
        bgp_peer = new BgpPeer("BGP peer 1", NULL, -1);
        ...
        Agent::GetDefaultInet4UcRouteTable()->AddRemoteVmRoute(NULL, "vrf2", 
                                                               remote_vm_ip1_,
                                                               32, remote_server_ip_,
                                                               TunnelType::AllType(),
                                                               30, "vn2");

        Agent::GetDefaultInet4UcRouteTable()->AddRemoteVmRoute(NULL, "vrf3", 
                                                               remote_vm_ip2_,
                                                               32, remote_server_ip_,
                                                               TunnelType::AllType(),
                                                               30, "vn3");
        Agent::GetDefaultInet4UcRouteTable()->AddRemoteVmRoute(NULL, "vrf4", 
                                                               remote_vm_ip3_,
                                                               32, remote_server_ip_,
                                                               TunnelType::AllType(),
                                                               30, "vn4");
    ...
    virtual void TearDown() {
        ...
        Agent::GetDefaultInet4UcRouteTable()->DeleteReq(NULL, "vrf2", 
                remote_vm_ip1_, 32);
        Agent::GetDefaultInet4UcRouteTable()->DeleteReq(NULL, "vrf3", 
                remote_vm_ip2_, 32);
        Agent::GetDefaultInet4UcRouteTable()->DeleteReq(NULL, "vrf4", 
                remote_vm_ip3_, 32);
...
public:
    void AddRemoteEcmpRoute(const string vrf_name, const string ip,
        ...
        for(int i = 0; i < count; i++) {
            ComponentNHData comp_nh(label,Agent::GetDefaultVrf(),
                    Agent::GetRouterId(), 
        ...
        Agent::GetDefaultInet4UcRouteTable()->AddRemoteVmRoute(bgp_peer,
    ...
    void AddLocalVmRoute(const string vrf_name, const string ip, uint32_t plen,
        ...
        const VmPortInterface *vm_intf = static_cast<const VmPortInterface *>
            (VmPortGet(intf_uuid));
        Agent::GetDefaultInet4UcRouteTable()->AddLocalVmRoute(bgp_peer,
    ...
    void DeleteRemoteRoute(const string vrf_name, const string ip,
        ...
        Agent::GetDefaultInet4UcRouteTable()->DeleteReq(bgp_peer, vrf_name,
                                                        server_ip, plen);
    } 
...
TEST_F(EcmpTest, EcmpTest_1) {
    ...
    FlowEntry *entry = FlowGet(VrfGet("vrf2")->GetVrfId(),
...
TEST_F(EcmpTest, EcmpTest_3) {
    ...
    FlowEntry *entry = FlowGet(VrfGet("vrf3")->GetVrfId(),
...
TEST_F(EcmpTest, EcmpTest_4) {
    ...
    strcpy(router_id, Agent::GetRouterId().to_string().c_str());
...
TEST_F(EcmpTest, EcmpTest_5) {
    ...
    FlowEntry *entry = FlowGet(VrfGet("vrf4")->GetVrfId(),
            remote_vm_ip, vm_ip, 1, 0, 0);
    EXPECT_TRUE(entry != NULL);
    EXPECT_TRUE(entry->data.component_nh_idx != 
            CompositeNH::kInvalidComponentNHIdx);
...
TEST_F(EcmpTest, EcmpTest_8) {
    ...
    ComponentNHData comp_nh_data1(16, Agent::GetDefaultVrf(), 
                                  Agent::GetRouterId(), server_ip1, false,
                                  TunnelType::AllType());
    comp_nh.push_back(comp_nh_data1);

    ComponentNHData comp_nh_data2(17, Agent::GetDefaultVrf(), Agent::GetRouterId(),
                                  server_ip2, false, TunnelType::AllType());
    comp_nh.push_back(comp_nh_data2);

    ComponentNHData comp_nh_data3(18, Agent::GetDefaultVrf(), Agent::GetRouterId(),
    ...
    Agent::GetDefaultInet4UcRouteTable()->DeleteReq(NULL, "vrf2", ip, 24);
    client->WaitForIdle();
}
...
TEST_F(EcmpTest, EcmpReEval_1) {
    ...
    EXPECT_TRUE(entry->data.component_nh_idx != ecmp_index);
}
...
TEST_F(EcmpTest, EcmpReEval_2) {
    ...
    Agent::GetDefaultInet4UcRouteTable()->AddRemoteVmRoute(bgp_peer, "vrf2",
...
TEST_F(EcmpTest, EcmpReEval_3) {
    ...
    TxIpPacketEcmp(VmPortGetId(1), "1.1.1.1", "3.1.1.10", 1);
    client->WaitForIdle();
    entry = FlowGet(VrfGet("vrf2")->GetVrfId(), "1.1.1.1", "3.1.1.10", 1, 0, 0);
    EXPECT_TRUE(entry != NULL);
    //Since flow already existed, use same old NH which would be at index 0
    EXPECT_TRUE(entry->data.component_nh_idx == 0);

    FlowEntry *rev_entry1 = entry->data.reverse_flow.get();
...
TEST_F(EcmpTest, ServiceVlanTest_1) {
    ...
    uint32_t vrf_id = Agent::GetVrfTable()->FindVrfFromName("vrf11")->GetVrfId();
    TxIpPacket(VmPortGetId(11), "11.1.1.253", "11.1.1.252", 1, 10, vrf_id);
    client->WaitForIdle();

    FlowEntry *entry = FlowGet(VrfGet("vrf11")->GetVrfId(),
            "11.1.1.253", "11.1.1.252", 1, 0, 0);
    EXPECT_TRUE(entry != NULL);
    EXPECT_TRUE(entry->data.component_nh_idx ==
            CompositeNH::kInvalidComponentNHIdx);
...
TEST_F(EcmpTest, ServiceVlanTest_2) {
    ...
    uint32_t vrf_id = Agent::GetVrfTable()->FindVrfFromName("vrf10")->GetVrfId();
    TxIpPacket(VmPortGetId(10), "10.1.1.1", "11.1.1.252", 1, 10, vrf_id);
    client->WaitForIdle();

    FlowEntry *entry = FlowGet(VrfGet("vrf10")->GetVrfId(),
            "10.1.1.1", "11.1.1.252", 1, 0, 0);
    EXPECT_TRUE(entry != NULL);
    EXPECT_TRUE(entry->data.component_nh_idx !=
            CompositeNH::kInvalidComponentNHIdx);
    EXPECT_TRUE(entry->key.vrf == vrf_id);
    ...
    EXPECT_TRUE(rev_entry->data.component_nh_idx == 
            CompositeNH::kInvalidComponentNHIdx);
    EXPECT_TRUE(rev_entry->key.vrf == vrf_id);
...
TEST_F(EcmpTest, ServiceVlanTest_3) {
    ...
    uint32_t service_vrf_id = 
        Agent::GetVrfTable()->FindVrfFromName("service-vrf1")->GetVrfId();
    ...
    for (uint32_t i = 0; i < 100; i++) {
        ...
        if (entry->data.component_nh_idx == 1) {
            ...
            EXPECT_TRUE(rev_entry->key.vrf == service_vrf_id);
        }
        EXPECT_TRUE(rev_entry->data.dest_vrf == vrf_id);
        EXPECT_TRUE(rev_entry->data.source_vn == "vn11");
        EXPECT_TRUE(rev_entry->data.dest_vn == "vn10");
    ...
    for (uint32_t i = 0; i < 100; i++) {
        ...
        FlowEntry *entry = FlowGet(VrfGet("service-vrf1")->GetVrfId(),
                "10.1.1.3", "11.1.1.252", IPPROTO_TCP, sport, dport);
        EXPECT_TRUE(entry != NULL);
...
TEST_F(EcmpTest, ServiceVlanTest_4) {
    ...
    for (uint32_t i = 0; i < 100; i++) {
        ...
        EXPECT_TRUE(entry->data.component_nh_idx !=
                CompositeNH::kInvalidComponentNHIdx);
        EXPECT_TRUE(entry->key.vrf == service_vrf_id);
        //Packet destined to service interface, vrf has to be 
        //service vlan VRF
...
TEST_F(EcmpTest, ServiceVlanTest_6) {
    ...
    for (uint32_t i = 0; i < 100; i++) {
        ...
        FlowEntry *entry = FlowGet(service_vrf_id,
                "11.1.1.1", "10.1.1.1", IPPROTO_TCP, sport, dport);
        EXPECT_TRUE(entry != NULL);
        EXPECT_TRUE(entry->data.component_nh_idx !=
                CompositeNH::kInvalidComponentNHIdx);
        EXPECT_TRUE(entry->key.vrf == service_vrf_id);
