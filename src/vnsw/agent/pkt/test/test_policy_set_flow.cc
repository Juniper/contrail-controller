...
TestTag src[] = {
    {"App1",  1, (TagTable::APPLICATION << TagTable::kTagTypeBitShift) | 0x1},
    {"Tier1", 2, (TagTable::TIER << TagTable::kTagTypeBitShift) | 0x1},
    {"Site1", 3, (TagTable::SITE << TagTable::kTagTypeBitShift) | 0x1},
};

TestTag dst[] = {
    {"App2",  4, (TagTable::APPLICATION << TagTable::kTagTypeBitShift) | 0x2},
    {"Tier2", 5, (TagTable::TIER << TagTable::kTagTypeBitShift) | 0x2},
    {"Site2", 6, (TagTable::SITE << TagTable::kTagTypeBitShift) | 0x2},
};
...
TEST_F(TestPolicySet, Unidirectional_Reverse_Udp) {
    ...
    TxUdpPacket(vnet[1]->id(), "1.1.1.2", "1.1.1.1", 10, 1000, rflow->flow_handle());
    client->WaitForIdle();
    EXPECT_TRUE(flow->IsShortFlow() == false);
    EXPECT_TRUE(flow->match_p().action_info.action & (1 << TrafficAction::PASS));
