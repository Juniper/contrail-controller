...
FlowEntry::~FlowEntry() {
    // Sanity check on the ref-count. Catches any explicit free of flow entry
    assert(refcount_ == 0);
    // Close linklocal socket if allocated
...
void FlowEntry::InitFwdFlow(const PktFlowInfo *info, const PktInfo *pkt,
    ...
    data_.vm_cfg_name = InterfaceIdToVmCfgName(intf_in);
...
VmInterfaceKey FlowEntry::InterfaceIdToKey(uint32_t id) {
    if (id != Interface::kInvalidIndex) {
        Interface *itf = InterfaceTable::GetInstance()->FindInterface(id);
...
const std::string FlowEntry::InterfaceIdToVmCfgName(uint32_t id) {
    if (id != Interface::kInvalidIndex) {
        const VmInterface *itf = dynamic_cast <const VmInterface *>
            (InterfaceTable::GetInstance()->FindInterface(id));
        if (itf) {
