...
static std::vector<std::string> MakeList(const std::set<std::string> &ilist) {
    std::vector<std::string> olist;
    for (std::set<std::string>::const_iterator it = ilist.begin();
         it != ilist.end(); ++it) {
        olist.push_back(*it);
    }
    return olist;
...
bool FlowEntry::DoPolicy() {
    ...
    data_.source_vn_match = nw_acl_info.src_policy;
    data_.dest_vn_match = nw_acl_info.dst_policy;
