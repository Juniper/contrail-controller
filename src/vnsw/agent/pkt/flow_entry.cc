...
void MatchPolicy::Reset() {
    ...
    m_application_policy_set_l.clear();
    application_policy_set_action = 0;
    m_out_application_policy_set_l.clear();
    out_application_policy_set_action = 0;
    sg_action_summary = 0;
    action_info.Clear();
}
...
void FlowEntry::ResetPolicy() {
    ...
    data_.match_p.m_application_policy_set_l.clear();
    data_.match_p.m_out_application_policy_set_l.clear();
}
...
void FlowEntry::GetApplicationPolicySet(const Interface *intf,
    ...
    if (vm_port != NULL) {
        ...
        for(; it != vm_port->fw_policy_list().end(); it++) {
            acl.acl = *it;
            data_.match_p.m_application_policy_set_l.push_back(acl);
        }
    }
    ...
    if (vm_port != NULL) {
        ...
        for(; it != vm_port->fw_policy_list().end(); it++) {
            acl.acl = *it;
            data_.match_p.m_out_application_policy_set_l.push_back(acl);
        }
    }
}
...
bool FlowEntry::DoPolicy() {
    ...
    data_.match_p.application_policy_set_action =
        MatchAcl(hdr, data_.match_p.m_application_policy_set_l,
                 true, true, &fw_acl_info);
    if (ShouldDrop(data_.match_p.application_policy_set_action)) {
        goto done;
    }

    data_.match_p.out_application_policy_set_action =
        MatchAcl(hdr, data_.match_p.m_out_application_policy_set_l,
                true, true, &fw_acl_info);
    if (ShouldDrop(data_.match_p.out_application_policy_set_action)) {
        goto done;
    }
...
void FlowEntry::SetAclAction(std::vector<AclAction> &acl_action_l) const {
    ...
    const std::list<MatchAclParams> &application_policy_set_l =
        data_.match_p.m_application_policy_set_l;
    acl_type = "fw acl";
    SetAclListAclAction(application_policy_set_l, acl_action_l, acl_type);

    const std::list<MatchAclParams> &out_application_policy_set_l =
        data_.match_p.m_out_application_policy_set_l;
    acl_type = "reverse fw acl";
    SetAclListAclAction(out_application_policy_set_l,
...
void FlowEntry::SetAclFlowSandeshData(const AclDBEntry *acl,
    ...
    SetAclListAceId(acl, data_.match_p.m_vrf_assign_acl_l,
                    fe_sandesh_data.ace_l);
    SetAclListAceId(acl, data_.match_p.m_application_policy_set_l,
                    fe_sandesh_data.ace_l);
    SetAclListAceId(acl, data_.match_p.m_out_application_policy_set_l,
                    fe_sandesh_data.ace_l);
