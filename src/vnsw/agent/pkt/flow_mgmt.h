...
public:
    FlowMgmtKeyNode() : flow_ptr_(NULL) { }
    FlowMgmtKeyNode(FlowEntry *fe) : flow_ptr_(fe) { }
    FlowEntry *flow_entry() { return flow_ptr_;}
private:
    friend class FlowMgmtEntry;
    FlowEntry *flow_ptr_;
    boost::intrusive::list_member_hook<> flow_node_hook_;
};
...
typedef std::map<FlowMgmtKey *, std::vector<FlowMgmtKeyNode *>, FlowMgmtKeyCmp> FlowMgmtKeyTree;
...
public:
    ...
    typedef boost::intrusive::member_hook<FlowMgmtKeyNode,
            ...
            &FlowMgmtKeyNode::flow_node_hook_> Node;
    typedef boost::intrusive::list<FlowMgmtKeyNode, Node> FlowNodeList;
    ...
    virtual ~FlowMgmtEntry() {
        assert(flow_node_list_.size() == 0);
    }

    uint32_t Size() const { return flow_node_list_.size(); }
