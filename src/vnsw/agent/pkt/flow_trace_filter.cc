...
void FlowTraceFilter::Reset(bool enable, Address::Family family) {
    ...
    if (family == Address::INET) {
        src_addr_ = dst_addr_ = Ip4Address::from_string("0.0.0.0");
        src_mask_ = dst_mask_ = Ip4Address::from_string("0.0.0.0");
    } else {
        src_addr_ = dst_addr_ = Ip6Address::from_string("::0");
        src_mask_ = dst_mask_ = Ip6Address::from_string("::0");
    }
    proto_start_ = 0;
    proto_end_ = 0xFF;
...
void FlowTraceFilter::SetFilter(bool enable, Address::Family family,
    ...
    src_addr_ = Ip4Address::from_string(src_addr);
    if (src_addr_.is_v4()) {
    ...
    dst_addr_ = Ip4Address::from_string(dst_addr);
    if (dst_addr_.is_v4()) {
