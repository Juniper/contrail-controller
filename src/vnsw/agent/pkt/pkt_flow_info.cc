...
static bool PickEcmpMember(const NextHop **nh, const PktInfo *pkt,
    ...
    info->ecmp = true;
    // If pkt->type is MESSAGE, it means flow was already setup. Once flows
    // are programmed to VRouter, VRouter ensures flow stickiness. So, dont
    // change ECMP member
    //
    // VRouter also handle transition from non-ECMP to ECMP as well
    if (pkt->type == PktType::MESSAGE) {
        return true;
    }

    // Compute hash to be used for picking ECMP member
    uint32_t hash = pkt->hash(ecmp_load_balance);

    // Pick ECMP member based on hash
    info->out_component_nh_idx = comp_nh->hash(hash);
