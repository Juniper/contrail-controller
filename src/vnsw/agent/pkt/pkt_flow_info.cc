...
void PktFlowInfo::FloatingIpSNat(const PktInfo *pkt, PktControlInfo *in,
    ...
    bool change = false;
    std::string out_dest_vn = "";
    std::string in_dest_vn = "";

    /* Get the dest_vn for in and out prefix */
    if (out->rt_ && out->rt_->GetActivePath()) {
        out_dest_vn = out->rt_->GetActivePath()->dest_vn_name();
    }
    if (in->rt_ && in->rt_->GetActivePath()) {
        in_dest_vn = in->rt_->GetActivePath()->dest_vn_name();
    }
    ...
    for ( ; it != fip_list.end(); ++it) {
        ...
        if (out->rt_ == NULL || rt_match_plen > out_rt_plen) {
            ...
            if (it->port_nat()) {
               change = false;
            } else if (!in_dest_vn.empty() && !out_dest_vn.empty()
                       && (in_dest_vn != out_dest_vn)) {
                change = true;
