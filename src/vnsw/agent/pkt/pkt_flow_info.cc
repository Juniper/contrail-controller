...
void PktFlowInfo::FloatingIpDNat(const PktInfo *pkt, PktControlInfo *in,
    ...
    if (nat_done == false) {
        if (in->rt_ != NULL) {
            flow_source_plen_map[in->rt_->vrf_id()] = in->rt_->plen();
        }
        in->rt_ = ftable->GetUcRoute(it->vrf_.get(), pkt->ip_saddr);
        if (in->rt_ == NULL) {
            flow_source_plen_map[it->vrf_.get()->vrf_id()] = 0;
        }
        nat_dest_vrf = it->vrf_.get()->vrf_id();
    }
    if (out->rt_ != NULL) {
        flow_dest_plen_map[out->rt_->vrf_id()] = out->rt_->plen();
    }
...
void PktFlowInfo::FloatingIpSNat(const PktInfo *pkt, PktControlInfo *in,
    ...
    for ( ; it != fip_list.end(); ++it) {
        ...
        if (change) {
            ...
            flow_dest_plen_map[rt_match->vrf_id()] = rt_match->plen();
        }
    }
    ...
    if (in->rt_ == NULL) {
        flow_source_plen_map[fip_it->vrf_.get()->vrf_id()] = 0;
        return;
    }
...
void PktFlowInfo::VrfTranslate(const PktInfo *pkt, PktControlInfo *in,
    ...
    if (match_acl_param.action_info.vrf_translate_action_.vrf_name() != "") {
        ...
        if (vrf) {
            ...
            out->rt_ = vrf->GetUcRoute(Ip4Address(pkt->ip_daddr.to_v4()));
            if (out->rt_ == NULL) {
                flow_dest_plen_map[vrf->vrf_id()] = 0;
            }
            if (vm_intf->vrf_assign_acl()) {
                if (in->rt_ != NULL) {
                    flow_source_plen_map[in->rt_->vrf_id()] = in->rt_->plen();
                }
                in->rt_ = vrf->GetUcRoute(Ip4Address(pkt->ip_saddr.to_v4()));
                if (in->rt_ == NULL) {
                    flow_source_plen_map[vrf->vrf_id()] = 0;
                }
            }
        }
    }
...
void PktFlowInfo::Add(const PktInfo *pkt, PktControlInfo *in,
    ...
    if (pkt->type != PktType::MESSAGE) {
        ...
        flow = Agent::GetInstance()->pkt()->flow_table()->Find(key);
        if (flow.get() == NULL || flow->deleted()) {
            return;
        }
