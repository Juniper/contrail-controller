...
void PktFlowInfo::UpdateEvictedFlowStats(const PktInfo *pkt) {
    ...
    if (flow.get() && flow->deleted() == false) {
        mgr->FlowStatsUpdateEvent(flow.get(), pkt->agent_hdr.cmd_param_2,
                                  pkt->agent_hdr.cmd_param_3, pkt->agent_hdr.cmd_param_4,
                                  pkt->agent_hdr.cmd_param_5, pkt->agent_hdr.cmd_param_6,
                                  pkt->agent_hdr.cmd_param_7, pkt->agent_hdr.cmd_param_8,
                                  pkt->agent_hdr.cmd_param_9, pkt->agent_hdr.cmd_param_10, flow->uuid());
    }
}
