...
void FlowTable::Add(FlowEntry *flow, FlowEntry *rflow) {
    ...
    FlowEntryPtr fe_ptr(flow);
    agent_->flow_stats_manager()->AddEvent(fe_ptr);

    FlowEntryPtr r_fe_ptr(rflow);
    agent_->flow_stats_manager()->AddEvent(r_fe_ptr);
...
void FlowTable::DeleteInternal(FlowEntryMap::iterator &it, uint64_t time)
{
    ...
    FlowEntryPtr fe_ptr(fe);
    agent_->flow_stats_manager()->DeleteEvent(fe_ptr);
}
