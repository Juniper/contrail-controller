...
void FlowTable::Add(FlowEntry *flow, FlowEntry *rflow) {
    ...
    bool rev_flow_update = false;
    if (rflow && rflow != new_rflow) {
        if (new_rflow->reverse_flow_entry() &&
                new_rflow->reverse_flow_entry() != flow) {
            //In case of ECMP flow, reverse flow could be reused,
            //when flow transition happens from one SI to other SI.
            //For example take a case where traffic is flowing from
            //remote SI to local destination VM, if remote SI is deleted
            //and flow swictes to local SI, reverese flow would be reused.
            rev_flow_update = true;
        }
    }

    FLOW_LOCK(new_flow, new_rflow);
    AddInternal(flow, new_flow, rflow, new_rflow, false, rev_flow_update);
}
...
void FlowTable::Update(FlowEntry *flow, FlowEntry *rflow) {
    ...
    if (new_flow == NULL) {
        uint64_t time = UTCTimestampUsec();
        new_flow = Locate(flow, time);
        fwd_flow_update = false;
    }
