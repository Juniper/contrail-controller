...
void ProtoHandler::EthHdr(const unsigned char *src, const unsigned char *dest, 
    ...
    memcpy(eth->h_dest, dest, MAC_ALEN);
    memcpy(eth->h_source, src, MAC_ALEN);
...
void ProtoHandler::TcpHdr(in_addr_t src, uint16_t sport, in_addr_t dst, 
                          uint16_t dport, bool is_syn, uint32_t seq_no, uint16_t len) {
...
void ProtoHandler::Send(uint16_t len, uint16_t itf, uint16_t vrf, 
                        uint16_t cmd, PktHandler::ModuleName mod) {
    // update the outer header
    struct ethhdr *eth = (ethhdr *)pkt_info_->pkt;
    std::string tmp_str((char *)eth->h_source, MAC_ALEN);
    memcpy(eth->h_source, eth->h_dest, MAC_ALEN);
    memcpy(eth->h_dest, tmp_str.data(), MAC_ALEN);
    ...
    if (PktHandler::GetPktHandler() == NULL)  {
        delete [] pkt_info_->pkt;
    } else {
        PktHandler::GetPktHandler()->Send(pkt_info_->pkt, len, mod);
    }
...
void ProtoHandler::SwapL4() {
    if (pkt_info_->ip_proto == TCP_PROTOCOL) {
    ...
    } else if(pkt_info_->ip_proto == UDP_PROTOCOL) {
        udphdr *udp = pkt_info_->transp.udp;
...
void ProtoHandler::SwapIpHdr() {
    //IpHdr expected IP address to be in network format
    iphdr *ip = pkt_info_->ip;
