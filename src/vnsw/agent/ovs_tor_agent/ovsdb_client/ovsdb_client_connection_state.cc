...
ConnectionStateEntry::ConnectionStateEntry(ConnectionStateTable *table,
    ...
    replicator_table_(NULL) {
    refcount_ = 0;
}

ConnectionStateEntry::~ConnectionStateEntry() {
    if (replicator_table_ != NULL) {
        replicator_table_->DeleteTable();
        replicator_table_ = NULL;
    }
}
...
void ConnectionStateTable::PhysicalDeviceNotify(DBTablePartBase *part,
    ...
    if (dev->IsDeleted()) {
        if (state) {
            ...
            if (state->replicator_table_ != NULL) {
                state->replicator_table_->DeleteTable();
                state->replicator_table_ = NULL;
            }
        }
        return;
    }
