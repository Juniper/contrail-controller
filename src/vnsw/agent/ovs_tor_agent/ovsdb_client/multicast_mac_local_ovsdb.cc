...
MulticastMacLocalEntry::MulticastMacLocalEntry(MulticastMacLocalOvsdb *table,
                                         std::string logical_switch_name) :
    OvsdbEntry(table),
    logical_switch_name_(logical_switch_name) {
}

void MulticastMacLocalEntry::SetLogicalSwitch() {
    if (!logical_switch_name_.empty()) {
        ...
        logical_switch_ = static_cast<LogicalSwitchEntry *>
            (l_table->GetReference(&ls_key));
    } else {
...
bool MulticastMacLocalEntry::Add() {
    ...
    if (logical_switch()) {
        ...
        vxlan_id_ = logical_switch()->vxlan_id();
        table->peer()->AddOvsPeerMulticastRoute(vrf_.get(), vxlan_id_,
                                                table_->client_idl()->tsn_ip(),
                                                logical_switch()->tor_ip().to_v4());
    } else {
        OVSDB_TRACE(Trace, "Deleting Route as logical switch not found for "
                    "VN uuid " + logical_switch_name_);
...
bool MulticastMacLocalEntry::Delete() {
    MulticastMacLocalOvsdb *table = static_cast<MulticastMacLocalOvsdb *>(table_);
    OVSDB_TRACE(Trace, "Deleting Route VN uuid " + logical_switch_name_);
    table->peer()->DeleteOvsPeerMulticastRoute(vrf_.get(), vxlan_id_);
...
KSyncEntry *MulticastMacLocalEntry::UnresolvedReference() {
    ...
    if (!vn_entry->IsResolved()) {
        OVSDB_TRACE(Trace, "Skipping route add " +
