...
#ifndef __MAC_LEARNING_H__
#define __MAC_LEARNING_H__
...
public:
    MacLearningEntry(MacLearningTable *table, uint32_t vrf_id,
                     const MacAddress &mac, uint32_t index);
    virtual void Add() = 0;
    virtual void Delete();
    virtual void Resync() { Add(); }

    MacLearningTable* mac_learning_table() {
...
protected:
    MacLearningTable *mac_learning_table_;
    MacLearningKey key_;
    uint32_t index_;
...
public:
     MacLearningEntryLocal(MacLearningTable *table, uint32_t vrf_id,
                           const MacAddress &mac, uint32_t index,
...
public:
    MacLearningEntryRemote(MacLearningTable *table, uint32_t vrf_id,
                           const MacAddress &mac, uint32_t index,
...
public:
    MacLearningEntryPBB(MacLearningTable *table, uint32_t vrf_id,
                        const MacAddress &mac, uint32_t index,
...
class MacLearningTable {
public:
    typedef std::pair<const MacLearningKey,
                      MacLearningEntryPtr> MacLearningEntryPair;
    typedef std::map<const MacLearningKey,
                     MacLearningEntryPtr,
                     MacLearningKeyCmp> MacLearningEntryTable;
    typedef WorkQueue<MacLearningEntryRequestPtr> MacLearningProtoQueue;

    MacLearningTable(Agent *agent, uint32_t id);
    ~MacLearningTable();
...
private:
    ...
    MacLearningProtoQueue request_queue_;
    boost::shared_ptr<MacAgingPartition> aging_partition_;
    DISALLOW_COPY_AND_ASSIGN(MacLearningTable);
};
#endif
