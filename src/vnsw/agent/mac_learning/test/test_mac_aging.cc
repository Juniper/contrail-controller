...
TEST_F(MacAgingTest, Test1) {
    ...
    MacLearningTable *table = agent_->mac_learning_proto()->Find(table_id);
...
TEST_F(MacAgingTest, Test2) {
    ...
    MacLearningTable *table1 = agent_->mac_learning_proto()->Find(table_id);

    const VrfEntry *vrf2 = VrfGet("vrf2");
    uint32_t table_id2 = agent_->mac_learning_proto()->Hash(vrf2->vrf_id(), smac);
    MacLearningTable *table2 = agent_->mac_learning_proto()->Find(table_id2);
...
TEST_F(MacAgingTest, Test4) {
    ...
    EXPECT_TRUE(EvpnRouteGet("vrf1", smac, Ip4Address(0), 0) != NULL);
    vr_bridge_entry *entry = KSyncSockTypeMap::GetBridgeEntry(1);
    entry->be_packets += 1;
