...
public:
    ...
    MacLearningMgmtRequest(Event event, MacLearningEntryPtr ptr):
        event_(event), mac_learning_entry_(ptr), key_(0, MacAddress::ZeroMac()){
    }

    MacLearningMgmtRequest(Event event, const DBEntry *db_entry,
                           uint32_t gen_id):
        event_(event), db_entry_(db_entry), key_(0, MacAddress::ZeroMac()),
        gen_id_(gen_id) {
    }

    MacLearningMgmtRequest(Event event, MacLearningKey &key):
        event_(event), key_(key) {}
    MacLearningEntryPtr mac_learning_entry() {
