...
public:
    TestRouteKState(bool vrfy, int count, KRouteResp *obj, std::string resp_ctx,
                    vr_route_req &encoder, int id, int family_id, sandesh_op::type op_code, int marker_size): RouteKState(obj, resp_ctx,
                    encoder, id, family_id, op_code, marker_size), TestKStateBase(vrfy, count, id) {}
    virtual void SendResponse() {
        UpdateFetchCount();
