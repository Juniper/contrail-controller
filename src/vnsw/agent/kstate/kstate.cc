...
void KState::IfMsgHandler(vr_interface_req *r) {
    ...
    if(ist->TypeToString(r->get_vifr_type()) == "PHYSICAL")
    {
        ...
        entry.set_child_bond_members_info("Master Interface");

        if(!r->get_vifr_fab_name().empty())
        {
            ...
            entry.set_child_bond_interface_name(str_bond_interface_name);
        }

        entry.set_child_bond_interface_status(bond_link[(r->get_vifr_intf_status() & 0x01)]);

        if(!r->get_vifr_fab_drv_name().empty())
        {
            ...
            entry.set_child_bond_interface_drv_name(str_bond_interface_drv_name);
        }
        ...
        for (int i = 0; i < r->get_vifr_num_bond_slave(); i++) {
            BondInterfaceInfo entry;
            entry.set_child_bond_members_info("Slave Interface");

            if(slave_intf_name || slave_intf_drv_name)
            {
