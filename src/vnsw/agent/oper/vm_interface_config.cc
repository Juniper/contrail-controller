...
static void ReadIgmpConfig(Agent *agent, IFMapNode *vn_node,
                            VirtualMachineInterface *cfg,
                            VmInterfaceConfigData &data) {

    VirtualNetwork *vn = NULL;
    if (vn_node) {
        vn = static_cast<VirtualNetwork *>(vn_node->GetObject());
    }
    ...
    if (cfg->IsPropertySet(VirtualMachineInterface::IGMP_ENABLE)) {
        ...
        data.igmp_enable_ = vn->igmp_enable();
    } else {
        data.igmp_enable_ = agent->oper_db()->global_system_config()->igmp_enable();
    }
}
