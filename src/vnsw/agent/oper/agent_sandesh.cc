...
void SandeshDBEntryIndex::HandleRequest() const {
    ...
    if (args.Get("table", &table_name) == false) {
        SandeshError(NULL, "Invalid request", context());
        return;
    }

    int first = 0;
    if (args.Get("begin", &first) == false) {
        SandeshError(NULL, "Invalid request", context());
        return;
    }

    int last = 0;
    if (args.Get("end", &last) == false) {
        SandeshError(NULL, "Invalid request", context());
        return;
    }
    ...
    if (table == NULL) {
        SandeshError(NULL, "Invalid DBTable", context());
        return;
    }

    AgentDBTable *agent_table = dynamic_cast<AgentDBTable *>(table);
    if (agent_table) {
        AgentSandeshPtr sandesh(agent_table->GetAgentSandesh(context()));
        if (sandesh) {
            sandesh->DoSandesh(first, last);
        } else {
            SandeshError(table, "Pagination not supported", context());
        }
        return;
    }
    ...
    if (route_table) {
        AgentSandeshPtr sandesh(route_table->GetAgentSandesh(context()));
        if (sandesh) {
            sandesh->DoSandesh(first, last);
        } else {
            SandeshError(table, "Pagination not supported", context());
