...
bool AgentSandeshManager::Run(PageRequest req) {
    ...
    if (agent_table) {
        AgentSandesh *sandesh = agent_table->GetAgentSandesh(&args,
                                                            req.context_);
        if (sandesh) {
    ...
    if (route_table) {
        AgentSandesh *sandesh = route_table->GetAgentSandesh(&args,
                                                             req.context_);
        if (sandesh) {
            sandesh->DoSandesh(first, last);
        } else {
...
void AgentSandesh::DoSandesh(int first, int last) {
    ...
    PageReqData req;
    MakeSandeshPageReq(&req, table, first, (last - first + 1), -1,
