...
RemoveWalkReferencesInRoutetable(RouteWalkerDBState::AgentRouteWalkerRefMapIter &it,
    ...
    for (uint8_t table_type = Agent::ROUTE_TABLE_START;
        ...
        DBTable::DBTableWalkRef ref = it->second[table_type];
        if (ref == DBTable::DBTableWalkRef())
            continue;
    }
}
...
AgentRouteWalker::LocateRouteTableWalkRef(const VrfEntry *vrf,
    ...
    if (it != state->walker_ref_map_.end()) {
        for (uint8_t table_type = Agent::ROUTE_TABLE_START;
            ...
            DBTable::DBTableWalkRef route_table_walk_ref =
                it->second[table_type];
            if (route_table_walk_ref == DBTable::DBTableWalkRef())
                break;
            if (route_table_walk_ref->table() != table)
                continue;
            return route_table_walk_ref;
        }
    }
    ...
    if (it == state->walker_ref_map_.end()) {
        ...
        route_walk_ref[table->GetTableType()] = rt_table_walk_ref;
        state->walker_ref_map_.insert(std::pair
                      <AgentRouteWalkerPtr, RouteWalkerDBState::RouteWalkRef>
                      (walker_ptr, route_walk_ref));
    } else {
        it->second[table->GetTableType()] = rt_table_walk_ref;
    }
