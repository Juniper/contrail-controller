...
void GlobalVrouter::UpdateFlowAging(autogen::GlobalVrouterConfig *cfg) {
    ...
    while (new_list_it != cfg->flow_aging_timeout_list().end()) {
        ...
        if (flow_aging_timeout_map_.find(key) !=
                flow_aging_timeout_map_.end()) {
            oper_->agent()->flow_stats_manager()->Add(
                    FlowAgingTableKey(new_list_it->protocol, new_list_it->port), 0,
                    new_list_it->timeout * 100000);
    ...
    while (old_list_it != flow_aging_timeout_map_.end()) {
        oper_->agent()->flow_stats_manager()->Delete(
                FlowAgingTableKey(old_list_it->first.protocol,
