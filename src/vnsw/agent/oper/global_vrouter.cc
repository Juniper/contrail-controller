...
void GlobalVrouter::AddCryptTunnelEndpoint(const CryptTunnelsMap::iterator &it) {
    ...
    bool crypt_path_available = agent()->crypt_tunnel_table()->IsCryptPathAvailable(it->first);
    if (it->second.mode == GlobalVrouter::CRYPT_ALL_TRAFFIC)
        crypt_traffic = true;
    agent()->crypt_tunnel_table()->Create(it->first, crypt_path_available, crypt_traffic);
}
