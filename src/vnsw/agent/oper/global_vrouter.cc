...
GlobalVrouter::GlobalVrouter(OperDB *oper)
    : oper_(oper), linklocal_services_map_(),
      ...
      ecmp_load_balance_(), resync_global_cfg_change_(false) {
...
void GlobalVrouter::GlobalVrouterConfig(DBTablePartBase *partition,
    ...
    if (resync_vn) {
        ...
        resync_global_cfg_change_ = true;
    }
}

void GlobalVrouter::ResyncGlobalCfgChange(const std::string name) {
    Agent *agent = oper_->agent();
    IFMapTable *iftable  = IFMapTable::FindTable(agent->db(), name);
    DBTableWalkMgr *walkermgr = agent->db()->GetWalkMgr();
    DBTableWalkMgr::DBTableWalkRef walk_ref =  walkermgr->AllocWalker
        (iftable, boost::bind(&GlobalVrouter::GlobalCfgChangeWalk,
                                    this, _1, _2),
         boost::bind(&GlobalVrouter::GlobalCfgChangeWalkDone, this));
    walkermgr->WalkTable(walk_ref);
}
...
void GlobalVrouter::GlobalCfgChangeWalkDone() {
}
