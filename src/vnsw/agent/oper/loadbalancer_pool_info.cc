...
LoadBalancerPoolInfo::LoadBalancerPoolInfo()
...
int LoadBalancerPoolInfo::CompareTo(const LoadBalancerPoolInfo &rhs) const {
    ...
    result = CustomAttributesCompare(custom_attributes_,
                                     rhs.custom_attributes_);
    return result;
}
...
std::string LoadBalancerPoolInfo::DiffString(
    const LoadBalancerPoolInfo *current) const {
    std::stringstream ss;
    if (current == NULL) {
        ss << "previous: NULL";
        return ss.str();
