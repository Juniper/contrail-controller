...
static void ComputeTypeInfo(Agent *agent, VmInterfaceConfigData *data,
    ...
    if (entry != NULL) {
        ...
        if (data->bridge_domain_list_.list_.size()) {
            //HACK remove, no instance interface would be
            //PBB interface eventually
            data->vmi_type_ = VmInterface::PBB;
        } else {
...
void VmInterface::ApplyConfig(bool old_ipv4_active, bool old_l2_active,
    ...
    if (vrf_ && vmi_type() == GATEWAY) {
        vrf_->CreateTableLabel(false, false);
    }
...
void AddVmiQosConfig::HandleRequest() const {
    ...
    req.data.reset(new InterfaceQosConfigData(NULL, NULL, qos_config_uuid, nil_uuid()));
    table->Enqueue(&req);
