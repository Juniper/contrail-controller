...
uint8_t GetEcmpHashToUse(const VmInterface *vmi) {
    if (vmi->ecmp_load_balance().is_set())
        return vmi->ecmp_load_balance().hash_fields_to_use();
    if (vmi->agent()->oper_db()->global_vrouter()->
        ecmp_load_balance().is_set())
        return vmi->agent()->oper_db()->global_vrouter()->
            ecmp_load_balance().hash_fields_to_use();
...
bool VmInterfaceGlobalVrouterData::OnResync(const InterfaceTable *table,
    ...
    if ((vmi->ecmp_load_balance().is_set() == false) &&
        global_ecmp_load_balance_changed_) {
        *force_update = true;
        ret = true;
    }
...
void VmInterface::UpdateMetadataRoute(bool old_metadata_ip_active,
    ...
    mdata_ip_->set_ecmp_hash_fields_to_use(ecmp_load_balance_.hash_fields_to_use());
}
