...
VmInterface::VmInterface(const boost::uuids::uuid &uuid) :
    ...
    do_dhcp_relay_(false), proxy_arp_mode_(PROXY_ARP_NONE), vm_name_(),
...
VmInterface::VmInterface(const boost::uuids::uuid &uuid,
    ...
    mirror_entry_(NULL), mirror_direction_(MIRROR_RX_TX), cfg_name_(""),
    fabric_port_(true), need_linklocal_ip_(false), dhcp_enable_(true),
    do_dhcp_relay_(false), proxy_arp_mode_(PROXY_ARP_NONE), vm_name_(vm_name),
...
void VmInterface::DeleteL2NextHop() {
    InterfaceNH::DeleteL2InterfaceNH(GetUuid(), vm_mac_);
    if (l2_interface_nh_policy_.get() != NULL)
        l2_interface_nh_policy_.reset();
    if (l2_interface_nh_no_policy_.get() != NULL)
        l2_interface_nh_no_policy_.reset();
...
VmInterface::InstanceIp::InstanceIp() :
    ListEntry(), ip_(), ecmp_(false), l2_installed_(false),
    is_primary_(false),  tracking_ip_(),
    l3_ecmp_mode_changed_(false), l2_ecmp_mode_changed_(false){
}
...
VmInterface::InstanceIp::InstanceIp(const InstanceIp &rhs) :
    ...
    l3_ecmp_mode_changed_(rhs.l3_ecmp_mode_changed_),
    l2_ecmp_mode_changed_(rhs.l2_ecmp_mode_changed_) {
}
...
VmInterface::InstanceIp::InstanceIp(const IpAddress &addr,
    ...
    l2_installed_(false), is_primary_(is_primary),
    tracking_ip_(tracking_ip), l3_ecmp_mode_changed_(false),
    l2_ecmp_mode_changed_(false) {
}
...
void VmInterface::InstanceIpList::Update(const InstanceIp *lhs,
    ...
    if (lhs->ecmp_ != rhs->ecmp_) {
        lhs->ecmp_ = rhs->ecmp_;
        lhs->l3_ecmp_mode_changed_ = true;
        lhs->l2_ecmp_mode_changed_ = true;
    }
