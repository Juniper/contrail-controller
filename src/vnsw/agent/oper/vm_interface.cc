...
bool VmInterface::CopyConfig(const InterfaceTable *table,
    ...
    } else if (primary_ip_addr_ != Ip4Address(0)) {
        InstanceIp iip(primary_ip_addr_, false, false);
        InstanceIpSet::const_iterator it =
                        data->instance_ipv6_list_.list_.find(iip);
        if (it != data->instance_ipv4_list_.list_.end() &&
                it->is_primary_ == true) {
            ipaddr = primary_ip_addr_;
        }
    }
    ...
    } else if (primary_ip6_addr_ != Ip6Address()) {
        InstanceIp iip(primary_ip6_addr_, false, false);
        InstanceIpSet::const_iterator it =
            data->instance_ipv6_list_.list_.find(iip);
        if (it != data->instance_ipv6_list_.list_.end() &&
                it->is_primary_ == true) {
            ip6_addr = primary_ip6_addr_;
        }
    }
