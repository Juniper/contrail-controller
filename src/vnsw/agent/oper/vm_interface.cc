...
VmInterface::VmInterface(const boost::uuids::uuid &uuid,
                         ...
                         Interface *intf, const Ip6Address &a6,
...
const MacAddress& VmInterface::GetVifMac(const Agent *agent) const {
    if (physical_interface()) {
        return physical_interface()->mac();
    } else if (parent_vmi_.get()) {
        const VmInterface *vmi =
            static_cast<const VmInterface *>(parent_vmi_.get());
...
bool VmInterface::CopyConfig(const InterfaceTable *table,
    ...
    *sg_changed =
    ...
    if (data->physical_interface_ != Agent::NullString()) {
        PhysicalInterfaceKey key(data->physical_interface_);
        physical_interface_ = static_cast<Interface *>
...
//   (We dont track parent_vmi and activate sub-interfaces. So, we only check 
...
bool VmInterface::IsIpv4Active() const {
    ...
    if (subnet_.is_unspecified() == false && physical_interface_ == NULL) {
        return false;
    }
