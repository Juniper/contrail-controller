...
void VmInterface::AllocateResources() {
    for (uint8_t type = (uint8_t)(MplsLabels::MIN + 1);
        ...
        labels_[type] = static_cast<IndexResourceData *>(
...
void VmInterface::AllocL3MplsLabel(bool force_update, bool policy_change,
    ...
    if (policy_change && (label != MplsTable::kInvalidLabel)) {
...
void VmInterface::AllocL2MplsLabel(bool force_update,
    ...
    if (l2_label_ == MplsTable::kInvalidLabel) {
        Agent *agent = static_cast<InterfaceTable *>(get_table())->agent();
        l2_label_ = mpls_labels_.labels_[L2];
        new_entry = true;
    }
...
void VmInterface::AllowedAddressPair::CreateLabelAndNH(Agent *agent,
    ...
    //rewrite information
    uint32_t new_label = policy_enabled_ ?
        mpls_labels_.labels_[MplsLabels::AAP_POLICY] :
        mpls_labels_.labels_[MplsLabels::AAP_NO_POLICY];
    if (policy_change) {
        old_label = label_;
    }
    label_ = new_label;
...
void VmInterface::ServiceVlan::Activate(VmInterface *interface,
    ...
    if (label_ == MplsTable::kInvalidLabel) {
        ...
        label_ = mpls_labels_.labels_[MplsLabels::SERVICE_VLAN];
...
VmInterface::MplsLabels::MplsLabels() {
    l2_ = l3_no_policy_ = l3_policy_ = aap_no_policy_ = aap_policy_ =
        service_vlan_ = MplsTable::kInvalidLabel;
}
