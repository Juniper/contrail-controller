...
void ProfileData::Get(Agent *agent) {
    ...
    for ( ;itr != itrend; ++itr) {
        if(itr->first.rfind(DB_IPV4_UNICAST_ROUTE_TABLE) != std::string::npos) {
           inet4_routes_.Accumulate(itr->second);
        } else if (itr->first.rfind(DB_IPV6_UNICAST_ROUTE_TABLE) != 
                   std::string::npos) {
           inet6_routes_.Accumulate(itr->second);
        } else if (itr->first.rfind(DB_L2_ROUTE_TABLE) != std::string::npos) {
           bridge_routes_.Accumulate(itr->second);
        } else if (itr->first.rfind(DB_MCAST_ROUTE_TABLE) != 
                   std::string::npos) {
           multicast_routes_.Accumulate(itr->second);
        } else if (itr->first.rfind(DB_EVPN_ROUTE_TABLE) != std::string::npos) {
...
static void GetDBTableStats(SandeshDBTableStatsInfo *stats, int index,
    ...
    while (itr != data->profile_stats_table_.end()) {
        if(itr->first.find(DB_INTERFACE_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Interface", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_MPLS_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Mpls", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_LOADBALNCE_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Loadbalancer", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_VN_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Vn", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_VM_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Vm", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_VRF_TABLE) != std::string::npos) {
           DBStatsToSandesh(&db_stats, "Vrf", itr->second);
           db_stats_list.push_back(db_stats);
        } else if (itr->first.find(DB_ACL_TABLE) != std::string::npos) {
