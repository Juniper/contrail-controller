...
InstanceManager::InstanceManager(Agent *agent)
        : si_listener_(DBTableBase::kInvalidId),
          ...
          if (agent->isMockMode()) {
              ...
              loadbalancer_config_path_ = "/tmp/" + std::string(getenv("LOGNAME"))+"/"+  pidstring + loadbalancer_config_path_default;
              namespace_store_path_ = "/tmp/" + std::string(getenv("LOGNAME"))+"/"+ pidstring + namespace_store_path_default;
          }
