...
bool VnTable::HandleIpamRoutesForForwardingModeChange(bool old_layer3_forwarding,
                                                      bool old_bridging,
                                                      bool *resync_routes,
                                                      VnData *data,
                                                      VnEntry *vn)
{
    ...
    if (vn->layer3_forwarding_ && old_layer3_forwarding) {
        ...
        //If there is no change in layer3_forwarding,
        //check if bridging has toggled. In case it has
        //happened resync routes in associated vrf.
        //This helps in fixing proxy, flood flags, mac binding.
        if (vn->bridging_ != old_bridging)
            *resync_routes = true;
    } else {
    ...
    if (old_bridging && vn->bridging_) {
        //No change in bridging but layer3_forwarding is toggled.
        //Enable flood flag for all IP routes by resync of routes.
        if (old_layer3_forwarding != vn->layer3_forwarding_)
            *resync_routes = true;
    }
    return ret;
}
...
bool VnTable::ChangeHandler(DBEntry *entry, const DBRequest *req) {
    ...
    ret |= HandleIpamRoutesForForwardingModeChange(old_layer3_forwarding,
                                                   old_bridging,
                                                   &resync_routes,
                                                   data,
                                                   vn);
...
VnData *VnTable::BuildData(IFMapNode *node) {
    ...
    bool bridging = GetBridgingConfig(derived_forwarding_mode);
    bool layer3_forwarding = GetLayer3ForwardingConfig(derived_forwarding_mode);
