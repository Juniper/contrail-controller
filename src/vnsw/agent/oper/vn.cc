...
VnEntry::VnEntry(Agent *agent, uuid id) :
    ...
    old_vxlan_id_(0), forwarding_mode_("") {
}
...
(const std::string &forwarding_mode) const {
...
bool VnTable::GetBridgingConfig(const std::string &forwarding_mode) const {
    if (forwarding_mode == "l3") {
        return false;
    }
    return true;
}
...
bool VnTable::EvaluateForwardingMode(VnEntry *vn) {
    bool ret = false;
    std::string forwarding_mode = vn->forwarding_mode();
    //Evaluate only if VN does not have specific forwarding mode change.
    if (forwarding_mode.length() == 0) {
        std::string global_forwarding_mode =
...
VnData *VnTable::BuildData(IFMapNode *node) {
    ...
    std::string forwarding_mode = properties.forwarding_mode;
    std::string derived_forwarding_mode = forwarding_mode;
    if (derived_forwarding_mode.length() == 0) {
