...
void Layer2AgentRouteTable::AddEvpnRoute(AgentRoute *rt) {
    EvpnRouteEntry *evpn_rt =
        static_cast<EvpnRouteEntry *>(rt);
    DBRequest req(DBRequest::DB_ENTRY_ADD_CHANGE);
    req.key.reset(new Layer2RouteKey(evpn_rt->evpn_peer(),
                                     evpn_rt->vrf()->GetName(),
                                     evpn_rt->mac(), 0));
    req.data.reset(new EvpnPathData(evpn_rt->evpn_peer_ref(),
                                    evpn_rt->GetActivePath(),
                                    evpn_rt->ip_addr(), true,
                                    evpn_rt->ToString()));
    Layer2TableProcess(agent(), vrf_name(), req);
}

void Layer2AgentRouteTable::DeleteEvpnRoute(AgentRoute *rt) {
    EvpnRouteEntry *evpn_rt =
        static_cast<EvpnRouteEntry *>(rt);
    Layer2AgentRouteTable::Delete(evpn_rt->evpn_peer(),
                                  evpn_rt->vrf()->GetName(),
                                  evpn_rt->mac(),
                                  0);
}
...
void Layer2AgentRouteTable::DeleteReq(const Peer *peer, const string &vrf_name,
    ...
    req.key.reset(new Layer2RouteKey(peer, vrf_name, mac,
                                     ethernet_tag));
    req.data.reset(NULL);
...
bool Layer2RouteEntry::DBEntrySandesh(Sandesh *sresp, bool stale) const {
    ...
    for (Route::PathList::const_iterator it = GetPathList().begin();
        ...
        const EvpnPath *evpn_path = NULL;
        if (path->PathType() == AgentPath::EVPN) {
            evpn_path = static_cast<const EvpnPath *>(path);
