...
bool MirrorTable::OnChange(DBEntry *entry, const DBRequest *req) {
    ...
    agent_->nexthop_table()->Process(nh_req);
    ...
    if (mirror_entry->nh_ != nh) {
        mirror_entry->nh_ = nh;
        mirror_entry->vrf_ =
            agent_->vrf_table()->FindVrfFromName(data->vrf_name_);
        ret = true;
    }
    return ret;
}
...
void MirrorTable::AddMirrorEntry(const std::string &analyzer_name,
    ...
    req.data.reset(NULL);
    mirror_table_->agent_->nexthop_table()->Enqueue(&req);
...
DBTableBase *MirrorTable::CreateTable(DB *db, const std::string &name) {
    ...
    mirror_table_->Init();
    mirror_table_->agent_ = Agent::GetInstance();
    mirror_table_->VrfListenerInit();
    return mirror_table_;
};

void MirrorTable::VrfListenerInit() {
    vrf_listener_id_ = agent_->vrf_table()->
...
void MirrorTable::MirrorSockInit(void) {
    ...
    assert(ec.value() == 0);
    agent_->set_mirror_port(sock_ep.port());
...
VrfEntry *MirrorTable::FindVrfEntry(const string &vrf_name) const {
    return agent_->vrf_table()->FindVrfFromName(vrf_name);
}
