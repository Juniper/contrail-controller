...
PhysicalInterface *LogicalInterface::physical_interface() const {
    return static_cast<PhysicalInterface *>(physical_interface_.get());
}
...
//////////////////////////////////////////////////////////////////////////////
LogicalInterfaceData::LogicalInterfaceData(const std::string &fq_name,
                                           const std::string &port,
                                           const boost::uuids::uuid &vif) :
    InterfaceData(), fq_name_(fq_name), physical_interface_(port),
    vm_interface_(vif) {
}
...
(const std::string &fq_name, const std::string &physical_interface,
 const boost::uuids::uuid &vif, uint16_t vlan) :
    LogicalInterfaceData(fq_name, physical_interface, vif), vlan_(vlan) {
}
...
static LogicalInterfaceData *BuildData(const Agent *agent, IFMapNode *node,
    ...
    return new VlanLogicalInterfaceData(node->name(), physical_interface,
...
bool InterfaceTable::LogicalInterfaceIFNodeToReq(IFMapNode *node,
    ...
    if (adj_node != NULL) {
        DBRequest req;
        if (VmiIFNodeToReq(adj_node, req)) {
            Enqueue(&req);
        }
    }
