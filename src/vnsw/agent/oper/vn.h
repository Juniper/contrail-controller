...
struct VnData : public AgentOperDBData {
    ...
    VnData(const string &name, const uuid &acl_id, const string &vrf_name,
           ...
           int vxlan_id, int vnid, bool bridge_forwarding,
        ...
        vxlan_id_(vxlan_id), vnid_(vnid), bridge_forwarding_(bridge_forwarding),
...
public:
    VnEntry(const boost::uuids::uuid &id) :
        ...
        bridge_forwarding_(true), layer3_forwarding_(true), admin_state_(true),
        table_label_(0) { }
    virtual ~VnEntry() { }
    ...
    bool bridge_forwarding() const {return bridge_forwarding_;};
