...
struct VnIpam {
    ...
    ConfigDhcpOptions config_dhcp_options;
    ...
    VnIpam(const std::string& ip, uint32_t len, const std::string& gw,
        ...
        config_dhcp_options.set_options(dhcp);
    }
...
struct VnIpamLinkData {
    ConfigDhcpOptions config_dhcp_options_;

    bool operator==(const VnIpamLinkData &rhs) const {
        if (config_dhcp_options_.host_routes() ==
            rhs.config_dhcp_options_.host_routes())
            return true;
        return false;
    }
};
