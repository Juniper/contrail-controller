...
struct VnData : public AgentOperDBData {
    ...
    VnData(const Agent *agent, const string &name, const uuid &acl_id,
           ...
           bool flood_unknown_unicast, const std::string &forwarding_mode) :
...
public:
    ...
    bool GetLayer3ForwardingConfig(const std::string &forwarding_mode) const;
    bool GetBridgingConfig(const std::string &forwarding_mode) const;
