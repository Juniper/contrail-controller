...
public:
    ...
    virtual void SetUp() {
        ...
        VmInterface *interface_ = static_cast<VmInterface *>(VmPortGet(1));
        MacAddress mac;
        VlanNH::CreateReq(interface_->GetUuid(), 1, "vrf1", mac, mac);
        client->WaitForIdle();
        VlanNHKey key(interface_->GetUuid(), 1);
    ...
    virtual void TearDown() {
        ...
        trigger_.Set();
        client->WaitForIdle();
        VlanNH::DeleteReq(interface_->GetUuid(), 1);
...
TEST_F(VrfAssignTest, Check_key_manipulations) {
    ...
    Interface *interface_ = VrfAssignTable::FindInterface(MakeUuid(1));
    EXPECT_TRUE(interface_ != NULL);
    //Verify key
