...
struct VlanTableReqData : public AgentOperDBData {
    VlanTableReqData(std::string desc) : AgentOperDBData(Agent::GetInstance(), NULL), description_(desc) {};
    ...
    uint64_t FindScale(int id, uint32_t count, bool use_key, bool do_lock){
        ...
        if (use_key) {
            for (uint32_t i = 0; i < count; i++) {
                ...
                assert(DBTable::Find(&key, do_lock) != NULL);
            }
        } else {
            for (uint32_t i = 0; i < count; i++) {
                Vlan entry(u);
                assert(DBTable::Find(&entry, do_lock) != NULL);
            }
        }
...
public:
    ScaleTask(VlanTable *table, bool use_key, bool do_lock, int thread_count,
              uint32_t find_count) :
        Task(TaskScheduler::GetInstance()->GetTaskId("DBScaleTest"), -1),
...
TEST_F(DBTest, Find) {
    ...
    EXPECT_TRUE(table_->Find(&key, true) != NULL);
    EXPECT_TRUE(table_->Find(&key, false) != NULL);

    Vlan entry(101);
    EXPECT_TRUE(table_->Find(&entry) != NULL);
    EXPECT_TRUE(table_->Find(&entry, true) != NULL);
    EXPECT_TRUE(table_->Find(&entry, false) != NULL);
}

// Find routine tests
TEST_F(DBTest, VlanScaleNoTask) {
    uint32_t count = 400*1000;
...
static uint64_t FindInterfaceScale(DBTable *table, uint32_t id,
    ...
    if (use_key) {
        for (uint32_t i = 0; i < count; i++) {
            ...
            assert(table->Find(&key, do_lock) != NULL);
        }
    } else {
        for (uint32_t i = 0; i < count; i++) {
            VmInterface entry(u);
            assert(table->Find(&entry, do_lock) != NULL);
        }
    }
