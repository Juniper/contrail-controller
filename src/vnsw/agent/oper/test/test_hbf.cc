...
#define MAX_VNET 10
int tap_fd[MAX_VNET];
static void CreateCustomerVMIInternal(struct PortInfo *input, int count, int acl_id,
...
void DeleteCustomerVMI(struct PortInfo *input, int count, bool del_vn, int acl_id,
                     const char* proj, const char *vn, const char *vrf,
...
TEST_F(HbfTest, basic_1) {
    ...
    vrf = static_cast<VrfEntry *>(Agent::GetInstance()->vrf_table()->FindActiveEntry(&vkey));
    ...
    VrfKSyncEntry *temp_ksync_entry = static_cast<VrfKSyncEntry *>(vrf_ksync_obj->DBToKSyncEntry(vrf));
    VrfKSyncEntry *ksync_entry = static_cast<VrfKSyncEntry *>(vrf_ksync_obj->Find(temp_ksync_entry));
    ...
    VmInterface *hbfr = static_cast<VmInterface *>(Agent::GetInstance()->interface_table()->Find(&key1, true));
    ...
    VmInterface *hbfl = static_cast<VmInterface *>(Agent::GetInstance()->interface_table()->Find(&key2, true));
    ...
    WAIT_FOR(100, 1000, (Agent::GetInstance()->interface_table()->Find(&key3, true)
                == NULL));
    ...
    WAIT_FOR(100, 1000, (Agent::GetInstance()->interface_table()->Find(&key4, true)
                == NULL));
    ...
    WAIT_FOR(100, 1000, (Agent::GetInstance()->interface_table()->Find(&key5, true)
                == NULL));
