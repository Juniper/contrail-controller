                                         ...
                                         uint64_t peer_identifier)
{
    ...
    /*
    TunnelOlist derived_olist;
    if (!delete_op && agent_->tsn_no_forwarding_enabled()) {
        for (TunnelOlist::const_iterator it = olist.begin(); it != olist.end(); it++)
        {
            ManagedPhysicalDevicesList::iterator pd_it =
                std::find(physical_devices_.begin(), physical_devices_.end(),
                          (*it).daddr_);
            if (pd_it != physical_devices_.end()) {
                derived_olist.push_back(*it);
            }
        }
    } else {
        derived_olist = olist;
