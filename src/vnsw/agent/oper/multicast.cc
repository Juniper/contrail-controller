                                           ...
                                           uint32_t ethernet_tag)
{
    ...
    ComponentNHKeyList component_nh_key_list =
        GetInterfaceComponentNHKeyList(obj, InterfaceNHFlags::EVPN);
...
{
    bool new_evpn_forwarding = vn->evpn_forwarding();
    ...
    for (std::set<MulticastGroupObject *>::iterator it =
        ...
        //L2 family disabled
        if (!(new_evpn_forwarding) && (*it)->evpn_forwarding()) {
            (*it)->SetEvpnForwarding(new_evpn_forwarding);
