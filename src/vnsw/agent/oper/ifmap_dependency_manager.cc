...
void IFMapDependencyManager::PropogateNodeChange(IFMapNode *node) {
    ChangeContext(IFMapNodeGet(node), true, false);
}
...
void IFMapDependencyManager::SetObject(IFMapNode *node, DBEntry *entry) {
    ...
    ChangeContext(state, propagate_change, true);
}
...
bool IFMapDependencyManager::ContextChangerCallback(ContextChangerDataType data) {
    if (data->propagate_change_) {
        tracker_->NodeEvent(data->node_->node(), data->add_node_);
        trigger_->Set();
    }
    return true;
}
