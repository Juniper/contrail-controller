...
IFMapNode *vn_test_node = NULL;
IFMapNode *vmi_test_node = NULL;
void IFMapDependencyManager::LinkObserver(
    ...
    bool set = false;
    if (left) {
        if (link->metadata() == "virtual-machine-interface-virtual-network") {
            if (((left->name().find("__right__3")) !=
                        std::string::npos)) {
                vn_test_node = right;
                vmi_test_node = left;
            }
...
static void RegisterConfigHandler(Agent *agent, IFMapDependencyManager *dep,
...
void IFMapDependencyManager::InitializeDependencyRules(Agent *agent) {
    ...
    RegisterConfigHandler(agent, this, "virtual-machine",
                          agent ? agent->vm_table() : NULL);

    RegisterConfigHandler(agent, this, "access-control-list",
    ...
    AddDependencyPath("virtual-network",
                      MakePath("virtual-network-network-ipam",
                               ...
                               "virtual-network-network-ipam",
                               "network-ipam", false));
    RegisterConfigHandler(agent, this, "virtual-network",
    ...
    RegisterConfigHandler(agent, this, "routing-instance",
    ...
    AddDependencyPath("security-group",
                      MakePath("security-group-access-control-list",
                               "access-control-list", true));
    RegisterConfigHandler(agent, this, "security-group",
    ...
    RegisterConfigHandler(agent, this, "virtual-machine-interface",
    ...
    AddDependencyPath("physical-interface",
                      MakePath("physical-router-physical-interface",
                               "physical-router", true));
    RegisterConfigHandler(agent, this, "physical-interface",
                          agent ? agent->interface_table() : NULL);
    ...
    AddDependencyPath("logical-interface",
                      MakePath("physical-router-logical-interface",
                               "physical-router", true));
    RegisterConfigHandler(agent, this, "logical-interface",
                          agent ? agent->interface_table() : NULL);
    ...
    AddDependencyPath("physical-router",
                      MakePath("physical-router-physical-interface",
                               "physical-interface", true));
    RegisterConfigHandler(agent, this, "physical-router",
