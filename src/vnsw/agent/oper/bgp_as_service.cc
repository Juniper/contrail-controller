    ...
    for (DBGraphVertex::adjacency_iterator it = bgp_as_a_service_node->begin(table->GetGraph());
        ...
        if (strcmp(adj_node->table()->Typename(), BGP_ROUTER_CONFIG_NAME) == 0) {
            ...
            if (vmi_service_port_index == BitSet::npos) {
                ...
                ss << bgp_router->parameters().source_port;
                BGPASASERVICETRACE(Trace, ss.str().c_str());
                continue;
            }
            source_port = EncodeBgpaasServicePort(bgp_router->parameters().source_port,
...
uint32_t BgpAsAService::EncodeBgpaasServicePort(const uint32_t sport,
                                                const size_t   index) {
    uint32_t max_session = agent_->params()->bgpaas_max_shared_vmis();
    const std::vector<uint16_t> &ports = agent_->params()->bgp_as_a_service_port_range_value();
    const std::vector<uint16_t> derived_ports =
        MiscUtils::GetDerivedBgpaasServicePort(ports, max_session);

    if (!index) {
        return sport;
    }

    uint16_t bgpaas_der_port_start = derived_ports[0];
    uint16_t bgpaas_der_port_end = derived_ports[1];
    uint16_t port_range_start = ports[0];
    if (bgpaas_der_port_start > bgpaas_der_port_end) {
...
std::pair<uint32_t, size_t> BgpAsAService::DecodeBgpaasServicePort(const uint32_t sport) {
    size_t   index = 0;
    uint32_t max_session = agent_->params()->bgpaas_max_shared_vmis();
    ...
    if (bgpaas_der_port_start > bgpaas_der_port_end) {
        original_sport = ((bgpaas_der_port_start - sport) / (max_session - 1))
                            + port_range_start;
        index = ((bgpaas_der_port_start - sport) % (max_session -1)) + 1;
    } else {
...
void BgpAsAService::FreeBgpVmiServicePortIndex(const uint32_t sport) {
    std::pair<uint32_t, size_t> portinfo = DecodeBgpaasServicePort(sport);
...
size_t BgpAsAService::AllocateBgpVmiServicePortIndex(const uint32_t sport) {
    ...
    if (port_map_it == bgp_as_a_service_port_map_.end()) {
        bgp_as_a_service_port_map_[sport] = new IndexAllocator(
                                            agent_->params()->bgpaas_max_shared_vmis());
    }
