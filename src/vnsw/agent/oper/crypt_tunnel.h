...
#ifndef vnsw_agent_crypt_tunnel_hpp
#define vnsw_agent_crypt_tunnel_hpp
...
struct CryptTunnelData : public AgentData {
    CryptTunnelData() : AgentData(), tunnel_available_(false), 
                        vr_to_vr_crypt_(false) { };
    CryptTunnelData(bool tunnel_available, bool vr_crypt) : 
                    AgentData(), tunnel_available_(tunnel_available),  
                    vr_to_vr_crypt_(vr_crypt) { }
    virtual ~CryptTunnelData() { }
...
public:
    ...
    bool CryptTunnelPathAvailable() const { return tunnel_available_;}
    bool CryptTraffic() const { return vr_to_vr_crypt_;}
    const IpAddress *GetTunnelRemoteIp() const { return &remote_ip_;}
    virtual void SetCryptTunnelPathAvailability(bool available) { tunnel_available_ = available;}
    virtual void SetVRToVRCrypt(bool crypt) { vr_to_vr_crypt_ = crypt;}
...
public:
    ...
    void CreateTunnels(const TunnelEndpointList &ep_list);
    void Create(const std::string &remote_ip, bool tunnel_available, bool crypt);
...
public:
    ...
    virtual bool RunTunnelTask();
    virtual bool StopTunnelTask();
    virtual bool IsRunning() const;

private:
    friend class CryptTunnelTable;
    void UpdateTunnelTaskCommand();
...
#endif // vnsw_agent_crypt_tunnel_hpp
