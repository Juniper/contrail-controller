...
(const AgentRoute *rt) {
    ...
    const VxlanRoutingVmiState *vmi_state =
        dynamic_cast<const VxlanRoutingVmiState *>(vmi->
                           GetAgentDBEntryState(mgr_->vmi_listener_id()));
    if (vmi_state == NULL) {
        return nil_uuid();
    }

    const VxlanRoutingVnState *vn_state =
        dynamic_cast<const VxlanRoutingVnState *>(vmi_state->vn_entry_->
                           GetAgentDBEntryState(mgr_->vn_listener_id()));
    if ((vn_state == NULL) || (vn_state->vmi_list_.size() == 0)) {
        return nil_uuid();
    }

    if (vmi_state->logical_router_uuid_ != vn_state->logical_router_uuid_) {
        return nil_uuid();
    }

    return vmi_state->logical_router_uuid_;
}
