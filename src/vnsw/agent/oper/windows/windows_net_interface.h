...
boost::optional<NET_LUID> GetVmInterfaceLuidFromName(
    const std::string& name, const size_t max_retries = 10);

/* This function assumes that name will be an `ifName` of the interface */
boost::optional<NET_LUID> GetPhysicalInterfaceLuidFromName(
    const std::string& name);

boost::optional<NET_IFINDEX> GetInterfaceIndexFromLuid(
    const NET_LUID intf_luid);

boost::optional<InterfaceOsParams> GetRealIdsFromOptionalLuid(
