...
int CustomAttributeCompare(const std::vector<autogen::KeyValuePair> &lhs,
    ...
    while (iter1 != lhs.end() && iter2 != rhs.end()) {
        ...
        if ((ret = iter2->value.compare(iter2->value)) != 0) {
            return ret;
        }
        ++iter1;
        ++iter2;
...
void CustomAttributeDiff(const std::vector<autogen::KeyValuePair> &lhs,
                         const std::vector<autogen::KeyValuePair> &rhs,
                         std::stringstream *ss) {
    autogen::KeyValuePairs::const_iterator iter1 = lhs.begin();
    autogen::KeyValuePairs::const_iterator iter2 = rhs.begin();
...
int LoadbalancerProperties::CompareTo(const LoadbalancerProperties &rhs) const {
    ...
    result = CustomAttributeCompare(custom_attributes_, rhs.custom_attributes_);
    return result;
}
#undef COMPARE_PROPERTY
...
std::string LoadbalancerProperties::DiffString(
    ...
    CustomAttributeDiff(current->custom_attributes_, custom_attributes_, &ss);
