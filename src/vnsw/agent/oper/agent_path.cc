...
bool SubnetRoute::UpdateRoute(AgentRoute *rt) {
    bool ret = true;
...
/*
 * IpamSubnetData is the data passed for resyncing route for updating flood flag
 * in all paths of route.
 * This is called when either Ipam subnet is deleted or added which is acting
 * as supernet for smaller subnet.
 * In case of addition of Ipam subnet route mark all smaller subnet coming in
 * same range with flood flag and remove it once Ipam route is withdrawn.
 */
bool IpamSubnetData::UpdateRoute(AgentRoute *rt) {
    bool ret = false;
    InetUnicastRouteEntry *uc_rt =
        static_cast<InetUnicastRouteEntry *>(rt);

    if (ipam_subnet_add_ != uc_rt->ipam_associated()) {
        uc_rt->set_ipam_associated(ipam_subnet_add_);
        ret = true;
    }
    return ret;
}
