...
const NextHop* AgentPath::GetNextHop(Agent *agent) const {
    if (nh_) {
        return nh_.get();
    }
...
void EvpnDerivedPath::set_nexthop(NextHop *nexthop) {
    nexthop_ = nexthop;
}

const NextHop *EvpnDerivedPath::GetNextHop(Agent *agent) const {
    return nexthop();
}

NextHop *EvpnDerivedPath::nexthop() const {
    return nexthop_.get();
}
...
bool LocalVmRoute::AddChangePath(Agent *agent, AgentPath *path,
    ...
    if (path->GetNextHop(agent) && path->GetNextHop(agent)->PolicyEnabled())
        old_policy = true;
...
bool AgentPath::ReorderCompositeNH(Agent *agent,
    ...
    composite_nh_key->Reorder(agent, label_, GetNextHop(agent));
