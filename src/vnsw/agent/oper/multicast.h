...
public:
    MulticastGroupObject(const std::string &vrf_name, 
        ...
        evpn_mpls_label_(0), vxlan_id_(0), evpn_forwarding_(true),
    ...
    void SetEvpnForwarding(bool enable) {evpn_forwarding_ = enable;};
    bool CanUnsubscribe() const {return (deleted_ || !evpn_forwarding_);}
