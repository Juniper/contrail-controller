...
public:
    MulticastGroupObject(const std::string &vrf_name, 
                         ...
                         const VnEntry *vn) :
        vrf_name_(vrf_name), grp_address_(grp_addr), vn_name_(vn->GetName()),
        vxlan_id_(0), peer_identifier_(0), deleted_(false), vn_(vn) {
    ...
    MulticastGroupObject(const std::string &vrf_name, 
        ...
        vxlan_id_(0), peer_identifier_(0), deleted_(false) {
        local_olist_.clear();
    };     
