...
public:
    ...
    virtual bool AutoAllocateLabel() = 0;
protected:
...
public:
    ...
    const Interface::Type &GetType() const {return intf_key_->type_;}
    const InterfaceKey *GetIntfKey() const { return intf_key_.get(); }
    const uint8_t &GetFlags() const { return flags_; }
...
public:
    ...
    virtual KeyPtr GetDBRequestKey() const;
    virtual bool CanAdd() const;
    virtual bool AutoAllocateLabel();
    virtual void SendObjectLog(const NextHopTable *table,
                               AgentLogEvent::type event) const;
...
public:
    ...
    virtual bool AutoAllocateLabel() { return true; }
...
public:
    ...
    const std::string& name() const { return intf_key_->name_;}
    const uint16_t& GetVlanTag() const { return vlan_tag_; }
private:
    friend class VlanNH;
    boost::scoped_ptr<InterfaceKey> intf_key_;
    uint16_t vlan_tag_;
