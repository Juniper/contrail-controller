...
public:
    PBBNHKey(const string &vrf_name, const MacAddress &dmac, uint32_t isid):
        NextHopKey(NextHop::PBB, false), vrf_key_(vrf_name), dmac_(dmac),
        isid_(isid) {
    };
    virtual ~PBBNHKey() { };
    ...
    virtual NextHop *AllocEntry() const;
    virtual NextHopKey *Clone() const {
        return new PBBNHKey(vrf_key_.name_, dmac_, isid_);
    }
...
public:
    ...
    virtual bool MatchEgressData(const NextHop *nh) const {
        ...
        if (pbb_nh && vrf_ == pbb_nh->vrf_ && dmac_ == pbb_nh->dmac_) {
            return true;
        }
        return false;
    }
