...
bool EvpnRouteEntry::FloodDhcpRequired() const {
    ...
    if (vn) {
        ...
        if (ip.is_unspecified()) {
            ...
            const Interface *interface = agent->oper_db()->mac_vm_binding()->
                FindMacVmBinding(mac_, GetActivePath()->vxlan_id());
...
bool EvpnRouteEntry::ProcessPath(Agent *agent, DBTablePartition *part,
