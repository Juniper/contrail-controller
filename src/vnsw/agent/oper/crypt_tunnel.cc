...
void CryptTunnelTable::CryptAvailability(const std::string &remote_ip, 
    ...
    if (entry) {
        crypt_traffic = entry->CryptTraffic();
        crypt_path_available = entry->CryptTunnelPathAvailable();
    }
}
...
bool CryptTunnelTable::IsCryptPathAvailable(const std::string &remote_ip) {
    bool ret = false;
    CryptTunnelEntry *entry = Find(remote_ip);
    if (entry && entry->CryptTunnelPathAvailable())
        ret = true;
    return ret;
}
...
void CryptTunnelTable::CreateTunnels(const TunnelEndpointList &ep_list) {
    bool available = false;
    bool crypt = CryptTunnelTable::VRToVRCrypt();
    for (std::vector<std::string>::const_iterator it = ep_list.begin();
         it != ep_list.end(); it++) {
        Create(*it, available, crypt);
    }
    return;
}
...
void CryptTunnelTable::Create(const std::string &remote_ip, bool available, bool vr_to_vr_crypt) {
    ...
    req.key.reset(new CryptTunnelKey(ip));
    req.data.reset(new CryptTunnelData(available, vr_to_vr_crypt));
    Process(req);
}
...
bool CryptTunnelTable::ChangeHandler(CryptTunnelEntry *entry, const DBRequest *req) {
    bool ret = false;
    CryptTunnelData *data = static_cast<CryptTunnelData *>(req->data.get());
    if (data && data->tunnel_available_ != entry->tunnel_available_) {
        entry->tunnel_available_ = data->tunnel_available_;
        ret = true;
    }
    if (data && data->vr_to_vr_crypt_ != entry->vr_to_vr_crypt_) {
        entry->vr_to_vr_crypt_ = data->vr_to_vr_crypt_;
        ret = true;
    }
...
bool CryptTunnelTable::Delete(DBEntry *entry, const DBRequest *req) {
    ...
    crypt_tunnel_entry->SendObjectLog(GetOperDBTraceBuf(), AgentLogEvent::DELETE);
    return false;
}
...
void CryptTunnelTaskBase::UpdateTunnel(const CryptTunnelEntry *entry, bool available) const {
    ...
    CryptTunnelKey *key = new CryptTunnelKey(*entry->GetTunnelRemoteIp());
    key->sub_op_ = AgentKey::RESYNC;
    req.key.reset(key);
    CryptTunnelTable *ctable = static_cast<CryptTunnelTable *>(entry->get_table());
    req.data.reset(new CryptTunnelData(available, entry->CryptTraffic()));
    ctable->Process(req);
}
...
bool CryptTunnelTask::RunTunnelTask() {
    UpdateTunnelTaskCommand();
    return task_->Run();
}
...
void CryptTunnelTask::UpdateTunnelTaskCommand() {
    Agent *agent = static_cast<CryptTunnelTable *>(entry_->get_table())->agent();
    if (agent->test_mode()) {
...
bool CryptTunnelTable::TunnelEventProcess(CryptTunnelEvent *event) {
    ...
    case CryptTunnelEvent::TASK_EXIT:
        if (tunnel_task->deleted_) {
            CRYPT_TUNNEL_TASK_TRACE(Trace, "Exit Stopped " + tunnel_task->to_string());
                delete tunnel_task;
        }
        break;
