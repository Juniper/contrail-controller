...
BridgeDomainEntry::BridgeDomainEntry(const BridgeDomainTable *table,
    ...
    AgentOperDBEntry(), table_(table), uuid_(id), isid_(0), vrf_name_(""),
...
void BridgeDomainEntry::UpdateVrf(const BridgeDomainData *data) {
    ...
    table_->agent()->vrf_table()->CreateVrf(vrf_name_, data->vn_uuid_,
    ...
    vrf_->CreateTableLabel(data->learning_enabled_,
...
bool BridgeDomainEntry::Change(const BridgeDomainData *data) {
    ...
    if (vn_ && data->vrf_name_ != Agent::NullString()) {
        std::ostringstream str;
        str << data->vrf_name_ << ":" << isid_;

        if (vrf_name_ != str.str() ||
            mac_aging_time_ != data->mac_aging_time_ ||
    ...
    if (learning_enabled_ != data->learning_enabled_) {
        learning_enabled_ = data->learning_enabled_;
        if (vrf_.get() && vn_.get()) {
            vrf_->CreateTableLabel(learning_enabled_,
...
void BridgeDomainEntry::Delete() {
    if (vrf_name_ != Agent::NullString()) {
        table_->agent()->vrf_table()->DeleteVrf(vrf_name_,
                                                VrfData::PbbVrf);
    }
}
