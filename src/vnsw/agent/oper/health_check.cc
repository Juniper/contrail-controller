...
void HealthCheckInstanceBase::set_service(HealthCheckService *service) {
    ...
    // set the service in this case because it is not needed and may have
...
HealthCheckService::StopHealthCheckService(HealthCheckInstanceBase *instance) {
    if (!instance->DestroyInstanceTask()) {
        ...
        //  crashing while trying to access the instance. Furthermore, in the
        // healthCheckTask instances markted as deleted in
        // DestroyInstanceTask() should not attemot to use service_ as it may
        // have already been deleted from the health check DB table.
    ...
    case HealthCheckInstanceEvent::MESSAGE_READ:
        {
            ...
            // has ben queued, and moreover the service_ pointer may be invalid.
            if (inst->deleted_) {
