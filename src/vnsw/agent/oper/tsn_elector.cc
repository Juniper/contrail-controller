...
TsnElectorWalker::TsnElectorWalker(Agent *agent) :
...
TsnElector::TsnElector(Agent *agent) : agent_(agent),
    ...
    walker_.reset(new TsnElectorWalker(agent));
}
...
bool TsnElector::IsTsnNoForwardingEnabled() const {
    if ((agent_->tsn_enabled() == false) ||
        (agent_->forwarding_enabled() == true))
        return false;
    return true;
}
...
void TsnElector::RouteNotify(DBTablePartBase *partition, DBEntryBase *e) {
    ...
    std::vector<std::string>::iterator it =
        std::find(active_tsn_servers_.begin(), active_tsn_servers_.end(),
                  rt_addr_str);
    std::string master = active_tsn_servers_.front();
    if (rt->IsDeleted()) {
    ...
    std::string new_master = active_tsn_servers_.front();
