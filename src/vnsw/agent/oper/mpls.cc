...
void MplsLabel::SyncDependentPath() {
    for (DependentPathList::iterator iter =
        ...
        MPLS_TRACE(MplsTrace, "Syncing route" << rt->ToString());
...
MplsLabelData::MplsLabelData(COMPOSITETYPE type, bool policy,
    ComponentNHKeyList &component_nh_key_list, const std::string &vrf_name) :
    AgentData() {
...
void MplsTable::Delete(uint32_t label) {
    DBRequest req;
...
void MplsTable::CreateEcmpLabel(uint32_t label, COMPOSITETYPE type, bool policy,
                                ComponentNHKeyList &component_nh_key_list,
                                const std::string &vrf_name) {
    if (label == 0 || label == MplsTable::kInvalidLabel)
        return;
