...
void MplsTable::VrMplsLimitExceeded() {
    ...
    if (label_table_.IndexInUse() >= agent()->vrouter_max_labels()) {
        vr_limit_itr->second.assign(std::string("TableLimit"));
        LOG(ERROR, "Vrouter Mpls Lablels Table Limit Reached. Skip Label Add.");
    } else if ( label_table_.IndexInUse() >= ((agent()->vr_limit_high_watermark() *
        agent()->vrouter_max_labels())/100) ) {
        vr_limit_itr->second.assign(std::string("Exceeded"));
        LOG(ERROR, "Vrouter Mpls Labels Exceeded.");
    } else if ( (label_table_.IndexInUse() >= ((agent()->vr_limit_high_watermark() *
        agent()->vrouter_max_labels())/100) ) &&
        ( label_table_.IndexInUse() < ((agent()->vrouter_max_labels()*95)/100)) ) {
        vr_limit_itr->second.assign(std::string("Exceeded"));
        LOG(ERROR, "Vrouter Mpls Labels Exceeded.");
    } else if ( label_table_.IndexInUse() < ((agent()->vr_limit_low_watermark() *
        agent()->vrouter_max_labels())/100) ) {
