...
/*
 * This function is to update NH reference in mpls label, as at the
 * time of label creation, nh might not get created.
 */
void MplsLabel::Update(uint32_t label, NextHopKey *nh_key) {
    ...
    Agent::GetInstance()->mpls_table()->Process(req);
}

MplsLabelData::MplsLabelData(COMPOSITETYPE type, bool policy,
    ComponentNHKeyList &component_nh_key_list, std::string vrf_name) :
    AgentData() {
    nh_key_.reset(new CompositeNHKey(type, policy, component_nh_key_list,
                  vrf_name));
}
...
//Free label from resource manager
...
bool MplsTable::IntfLabelAllocRequired(const InterfaceNHKey *key) {
    // Label is required only for VMI and Inet interfaces
    if (dynamic_cast<const VmInterfaceKey *>(key->GetIntfKey()) ||
        dynamic_cast<const InetInterfaceKey *>(key->GetIntfKey())) {
        return true;
    }
...
MplsLabel *MplsTable::AllocLabel(const NextHopKey *nh_key) {
    ...
    // Return newly created MplsLabel db entry
