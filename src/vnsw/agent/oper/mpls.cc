...
void MplsTable::ReserveLabel(uint32_t start, uint32_t end) {
    ...
    for (unsigned int i = start; i < end; i++) {
        ResourceManager::KeyPtr key(new EdgeMulticastMplsResourceKey(agent()->
                                        resource_manager(), i));
        agent()->resource_manager()->Allocate(key);
    }
}
...
void MplsTable::FreeReserveLabel(uint32_t start, uint32_t end) {
    ...
    for (unsigned int i = start; i < end; i++) {
        ResourceManager::KeyPtr key(new EdgeMulticastMplsResourceKey(agent()->
                                        resource_manager(), i));
        agent()->resource_manager()->Release(key);
    }
}
...
void MplsTable::FreeLabel(ResourceManager::KeyPtr key) {
    agent()->resource_manager()->Release(key);
}
...
MplsLabel *MplsTable::FindMplsLabel(size_t label) {
    DBRequest req;
    req.oper = DBRequest::DB_ENTRY_ADD_CHANGE;
