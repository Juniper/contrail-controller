...
PBBNH::PBBNH(VrfEntry *vrf, const MacAddress &dmac, uint32_t isid):
    NextHop(NextHop::PBB, true, false), vrf_(vrf, this), dmac_(dmac),
...
NextHop *PBBNHKey::AllocEntry() const {
    VrfEntry *vrf = static_cast<VrfEntry *>
        (Agent::GetInstance()->vrf_table()->Find(&vrf_key_, true));
    return new PBBNH(vrf, dmac_, isid_);
}
...
PBBNH::KeyPtr PBBNH::GetDBRequestKey() const {
    NextHopKey *key = new PBBNHKey(vrf_->GetName(), dmac_, isid_);
    return DBEntryBase::KeyPtr(key);
}
...
bool PBBNH::Change(const DBRequest *req) {
    ...
    BridgeRouteEntry *rt = rt_table->FindRouteNoLock(dmac_);
...
void NextHop::SetNHSandeshData(NhSandeshData &data) const {
    ...
    switch (type_) {
        ...
        case PBB: {
             ...
             data.set_mac(pbb_nh->dmac().ToString());
