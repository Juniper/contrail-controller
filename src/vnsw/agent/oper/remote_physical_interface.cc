...
RemotePhysicalInterfaceData::RemotePhysicalInterfaceData
    ...
    InterfaceData(), display_name_(display_name), device_uuid_(device_uuid) {
...
(const Agent *agent, IFMapNode *node, const autogen::PhysicalInterface *port) {
    ...
    return new RemotePhysicalInterfaceData(port->display_name(),
...
static void SetReq(DBRequest *req, const string &fqdn,
    ...
    req->data.reset(new RemotePhysicalInterfaceData(display_name, vrf_name,
                                                    device_uuid));
}
