                                                      ...
                                                      bool add_change)
{
    ...
    //Skip gateway subnet.
    const NextHop *parent_nh =
        (parent_route &&  (parent_route->addr()) != IpAddress()) ?
        parent_route->GetActiveNextHop() : NULL;
    if (parent_nh && parent_nh->GetType() == NextHop::RESOLVE)
        return false;
...
void InetUnicastAgentRouteTable::AddEvpnRoute(const AgentRoute *route) {
    ...
    if (parent_key &&
        (((parent_key->addr() == ip_addr) &&
          parent_key->IsHostRoute()) || 
         (parent_key->GetActiveNextHop()->GetType()
                                      == NextHop::RESOLVE)))
        parent_key = NULL;
