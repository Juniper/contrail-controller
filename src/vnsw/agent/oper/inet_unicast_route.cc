...
bool Inet4UnicastArpRoute::AddChangePath(Agent *agent, AgentPath *path,
    ...
    if (nh) {
        ...
        if (path->mac() != arp_nh->GetMac()) {
            path->set_mac(arp_nh->GetMac());
            ret = true;
        }

        if (path->interface() != arp_nh->GetInterface()) {
            path->set_interface(arp_nh->GetInterface());
            ret = true;
        }

        if (path->valid() != arp_nh->IsValid()) {
            path->set_valid(arp_nh->IsValid());
            ret = true;
        }
    }
