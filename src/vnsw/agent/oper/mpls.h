...
#define MPLS_TRACE(obj, arg)                                                  \
do {                                                                          \
    std::ostringstream _str;                                                  \
    _str << arg;                                                              \
    obj::TraceMsg(MplsTraceBuf, __FILE__, __LINE__, _str.str());              \
} while (false)
...
public:
    ...
    void CreateEcmpLabel(uint32_t label, COMPOSITETYPE type, bool policy,
                         ComponentNHKeyList &component_nh_key_list,
                         const std::string &vrf_name);
