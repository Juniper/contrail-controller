...
public:
    struct RoutedVrfInfo {
        ...
        RoutedVrfInfo() : routing_vrf_(NULL), bridge_vn_list_() {
        }
        virtual ~RoutedVrfInfo() {
            assert((parent_vn_entry_ == NULL) &&
                   (routing_vrf_ == NULL) &&
                   (bridge_vn_list_.size() == 0));
        }
    ...
    //Using VMI from path nexthop update VN and LR for bridge VRF.
    void UpdateRoutedVrfInfo(const EvpnRouteEntry *rt);
...
private:
    ...
    RoutedVrfInfo& GetRoutedVrfInfo(const VnEntry *vn);
