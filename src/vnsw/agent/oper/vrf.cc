...
VrfEntry::VrfEntry(const string &name, uint32_t flags, Agent *agent) : 
        ...
        global_cfg_current_resync_seqno_(0) {
}
...
bool VrfTable::OperDBOnChange(DBEntry *entry, const DBRequest *req) {
    ...
    if (vrf->GlobalCfgCurrentResyncSeqNo() !=
        global_vrouter->ResyncNotificationSeqNo()) {
        vrf->vn_.reset(vn);
        vrf->ResyncRoutes();
        vrf->SetGlobalCfgCurrentResyncSeqNo(global_vrouter->ResyncNotificationSeqNo());
    }
