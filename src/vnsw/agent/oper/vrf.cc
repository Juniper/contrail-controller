...
void VrfTable::CreateVrfReq(const string &name, uint32_t flags, uint32_t isid,
                            std::string bmac_vrf_name, bool learning_enabled) {
    DBRequest req(DBRequest::DB_ENTRY_ADD_CHANGE);
    req.key.reset(new VrfKey(name));
    req.data.reset(new VrfData(agent(), NULL, flags,
                               nil_uuid(), isid,
                               bmac_vrf_name, 0, learning_enabled));
    Enqueue(&req);
}
...
void VrfTable::CreateVrfReq(const string &name,
                            ...
                            std::string bmac_vrf_name,
                            bool learning_enabled) {
    DBRequest req(DBRequest::DB_ENTRY_ADD_CHANGE);
    req.key.reset(new VrfKey(name));
    req.data.reset(new VrfData(agent(), NULL, flags, vn_uuid,
                               isid, bmac_vrf_name, 0, learning_enabled));
    Enqueue(&req);
}
