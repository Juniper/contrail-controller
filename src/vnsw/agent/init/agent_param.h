...
public:
    ...
    bool atf_is_agent_mocked() const {
        return AgentMock_;
    }

    bool atf_is_dpdk_mocked() const {
        return atf_MockDPDK_;
    }

    std::string aft_ksocketdir() const {
        return atf_kSocketDir_;
    }
...
protected:
    ...
    (const boost::program_options::variables_map &var_map, Ip4Address *server1,
     ...
     void ParseTestFrameworkArguments(const boost::program_options::variables_map &var_map);//sagarc
