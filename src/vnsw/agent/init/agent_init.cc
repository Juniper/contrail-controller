...
int AgentInit::Start() {
    ...
    std::vector<std::string> v_slo_destinations = agent_param_->
                                                   get_slo_destination();
    bool sample_2_syslog = false;
    if (std::find(v_slo_destinations.begin(), v_slo_destinations.end(), "syslog"            ) != v_slo_destinations.end()) {
        sample_2_syslog = true;
    }
    ...
    if (log_property_file.size()) {
        ...
        LoggingInit(agent_param_->log_file(), agent_param_->log_file_size(),
                    agent_param_->log_files_count(), sample_2_syslog,
    ...
    Sandesh::set_sample_logger_appender(agent_param_->log_file(),
    ...
    Sandesh::set_slo_logger_appender(agent_param_->log_file(),
                                     agent_param_->log_file_size(),
                                     agent_param_->log_files_count(),
                                     agent_param_->syslog_facility(),
    ...
    Sandesh::set_send_2_collector_flags(agent_param_->get_sample_destination(),
                                        agent_param_->get_slo_destination());

    //Sandesh::set_send_2_logger_flags(agent_param_->get_sample_destination());
