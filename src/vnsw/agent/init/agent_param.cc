...
void AgentParam::ParseTestFrameworkArguments(const boost::program_options::variables_map &var_map) {
    ...
    if (AgentMock_)
    {
     ...
     GetOptValue<string>(var_map, atf_kSocketDir_, "AGENT-TEST-FRAMEWORK.ksocketdir");
     //sagarc  Pkt0Socket::InitDirectories(atf_kSocketDir_);
    }

   if (!AgentMock_ && atf_MockDPDK_){
        std::cout << "Please fix conf file parameters. If you want to mock dpdk, you have to mock agent" << std::endl;
        exit(1);
   }
...
AgentParam::AgentParam(bool enable_flow_options,
        ...
        mvpn_ipv4_enable_(false),AgentMock_(false), atf_MockDPDK_(false), atf_kSocketDir_("/tmp/") {
    ...
    uint16_t default_fabric_snat_table_size = Agent::kFabricSnatTableSize;
          ...
          restart_backup_dir_  = "/tmp/"+ stemp.str() + CFG_BACKUP_DIR;
    ...
    config.add_options()
        ...
        ("DEFAULT.agent_base_directory", opt::value<string>()->default_value("/tmp"),
        ...
        ("AGENT-TEST-FRAMEWORK.ksocketdir",  opt::value<string>()->default_value("/tmp/"), "ksocket directory") 
        ("DEFAULT.subnet_hosts_resolvable",
