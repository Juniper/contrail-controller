...
void AgentParam::ParseVirtualHostArguments
    ...
    cout << "vhost name " << GetOptValue<string>(var_map, vhost_.name_, "VIRTUAL-HOST-INTERFACE.name");
    string ip;
...
AgentParam::AgentParam(bool enable_flow_options,
    ...
    flow.add_options()
        ...
        ("FLOWS.tcp_flow_scan_interval", opt::value<uint16_t>(),
         " Tcp flow scan interval")
        ;
    options_.add(flow);
