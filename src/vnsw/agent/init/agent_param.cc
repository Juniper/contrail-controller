...
void AgentParam::ParseDefaultSection() {
    ...
    if (!GetValueFromTree<string>(xmpp_server_cert_, "DEFAULT.xmpp_server_cert")) {
        // set defaults
        xmpp_server_cert_ = "/etc/contrail/ssl/certs/control-node-cert.pem";
    }
    ...
    if (!GetValueFromTree<string>(xmpp_dns_server_cert_, 
                                  "DEFAULT.xmpp_dns_server_cert")) {
        // set defaults
        xmpp_dns_server_cert_ = "/etc/contrail/ssl/certs/dns-cert.pem";
    }
...
void AgentParam::ParseDefaultSectionArguments
    ...
    GetOptValue<bool>(var_map, xmpp_dns_auth_enable_, "DEFAULT.xmpp_dns_auth_enable");
    GetOptValue<string>(var_map, xmpp_dns_server_cert_, "DEFAULT.xmpp_dns_server_cert");
...
void AgentParam::LogConfig() const {
    ...
    if (xmpp_dns_auth_enable_) {
        LOG(DEBUG, "Xmpp Dns Server Certificate : " << xmpp_dns_server_cert_);
    }
...
AgentParam::AgentParam(Agent *agent, bool enable_flow_options,
        ...
        xmpp_dns_auth_enable_(false), xmpp_dns_server_cert_(""),
