...
void AgentParam::ParseQueue() {
    ...
    BOOST_FOREACH(const ptree::value_type &section, tree_) {
        ...
        uint16_t queue;
        std::string nic_queue = section.first;
        if (sscanf(nic_queue.c_str(), "QUEUE-%hu", &queue) != 1) {
                continue;
        }
        BOOST_FOREACH(const ptree::value_type &key, section.second) {
            if (key.first.compare("nic_queue") == 0) {
    ...
    GetValueFromTree<string>(input, "QUEUE-DEFAULT.nic_queue");
    boost::split(tokens, input, boost::is_any_of("[]"),
                         boost::token_compress_on);
    if(tokens.size() >3) {
        LOG(ERROR, "Error in parsing default nic queue ");
    } else {
        stringToInteger(tokens[1], default_nic_queue_);
    }
