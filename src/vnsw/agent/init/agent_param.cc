...
void AgentParam::ParseDefaultSectionArguments
    ...
    GetOptValue<bool>(var_map, log_local_, "DEFAULT.log_local");
    GetOptValue<bool>(var_map, debug_, "DEFAULT.debug");
    GetOptValue<bool>(var_map, log_flow_, "DEFAULT.log_flow");
...
void AgentParam::ParseFlowArguments
    ...
    GetOptValue<uint16_t>(var_map, flow_latency_limit_,
                          "FLOWS.latency_limit");
    GetOptValue<bool>(var_map, flow_trace_enable_,
                          "FLOWS.trace_enable");
    uint16_t val = 0;
...
void AgentParam::ParseServiceInstanceArguments
    ...
    GetOptValue<string>(var_map, si_lb_keystone_auth_conf_path_,
...
void AgentParam::ParseSimulateEvpnTor
    (const boost::program_options::variables_map &var_map) {
    GetOptValue<bool>(var_map, simulate_evpn_tor_,
...
AgentParam::AgentParam(bool enable_flow_options,
    ...
    config.add_options()
        ...
        ("DEFAULT.debug", opt::bool_switch(&debug_),
        ...
        ("DEFAULT.mirror_client_port",
         ...
         "Mirror client Port")
        ("DEFAULT.simulate_evpn_tor", opt::bool_switch(&simulate_evpn_tor_)->default_value(false),
         "Simulate Evpn Tor")
        ;
    ...
    if (enable_flow_options_) {
        ...
        flow.add_options()
            ...
            ("FLOWS.index_sm_log_count", opt::value<uint16_t>()->default_value(Agent::kDefaultFlowIndexSmLogCount),
             "Flow index-mgr sm log count")
            ("FLOWS.trace_enable", opt::bool_switch(&use_syslog_)->default_value(true),
             "Enable flow tracing")
            ("FLOWS.add_tokens", opt::value<uint32_t>(), "Number of add-tokens")
            ("FLOWS.ksync_tokens", opt::value<uint32_t>(), "Number of ksync-tokens")
            ("FLOWS.del_tokens", opt::value<uint32_t>(), "Number of delete-tokens")
            ("FLOWS.update_tokens", opt::value<uint32_t>(), "Number of update-tokens")
            ("FLOWS.flow_latency_limit", opt::value<uint16_t>(), "Flow latency limit")
            ;
    options_.add(flow);
    ...
    if (enable_vhost_options_) {
        ...
        vhost.add_options()
            ...
            ("VIRTUAL-HOST-INTERFACE.eth_port_no_arp", opt::bool_switch(&eth_port_no_arp_)->default_value(false),
    ...
    if (enable_service_options_) {
        ...
        service.add_options()
            ...
            ("SERVICE_INSTANCE.lbaas_auth_conf", opt::value<string>()->default_value("/etc/contrail/contrail-lbaas-auth.conf"),
             "Credentials fo ssl certificates and private-keys")
            ("SERVICE-INSTANCE.lb_keystone_auth_conf_path", opt::value<string>(),
             "Keystone config path for loadbalancer")
            ;
