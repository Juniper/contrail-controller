...
NexthopDBServer::NexthopDBServer(boost::asio::io_service &io)
  : io_service_(io), nexthop_table_(), client_table_()
{
...
NexthopDBServer::AddNexthop(NexthopDBEntry::NexthopPtr nh)
{
  for (ClientIterator iter = client_table_.begin();
       iter != client_table_.end(); ++iter) {
    LOG(DEBUG, "[NexthopServer] Add Nexthop " << nh->nexthop_string() <<
        " to client " << iter->second->session_->session_id());
    iter->second->AddNexthop(nh);
  }
}

NexthopDBEntry::NexthopPtr
NexthopDBServer::FindOrCreateNexthop(std::string nh_str)
{
  tbb::mutex::scoped_lock lock(mutex_);
...
void
NexthopDBServer::FindAndRemoveNexthop(std::string str)
{
  tbb::mutex::scoped_lock lock(mutex_);
...
NexthopDBServer::RemoveNexthop(NexthopDBEntry::NexthopPtr nh)
{
  ...
  for (ClientIterator iter = client_table_.begin();
       iter != client_table_.end(); ++iter) {
    iter->second->RemoveNexthop(nh);
    iter->second->AddNexthop(nh);
  }
  LOG(DEBUG, "[NexthopServer] Deletion event for nexthop "
      << nh->nexthop_string());
}
