...
class Debug(object):
    ...
    def get_vrouter_logs(self):
        ...
        commands = ['nh --list', 'vrouter --info', 'dropstats --log 0', 'dropstats -l 0', 'vif --list',
                        'mpls --dump', 'vxlan --dump', 'vrfstats --dump', 'vrmemstats', 'qosmap --dump-fc',
                         'qosmap --dump-qos', 'mirror --dump']
        ...
        for i in range(len(commands)):
            ...
            myCmd = 'docker exec vrouter_vrouter-agent_1 /bin/sh -c "%s" >> /tmp/vrouter_logs/%s.txt'%(str,str_file_name)
            cmd_op = self.get_ssh_cmd_output(myCmd)
        ...
        for i in range(len(values)):
            if values[i] != None :
                ...
                for i in range(len(commands)): 
                    ...
                    myCmd = 'docker exec vrouter_vrouter-agent_1 /bin/sh -c "rt --dump %d --family %s" >> /tmp/vrouter_logs/VRF_%d_Family%s'%(var,cmd,var,cmd)
                    cmd_op = self.get_ssh_cmd_output(myCmd)
                    source_path = '/tmp/vrouter_logs/VRF_%d_Family%s'%(var,cmd)
                    dest_path = '%s/vrouter_logs/VRF_%d_Family%s'%(self._parent_dir,var,cmd)
                    self.do_ftp(source_path,dest_path)
