...
""" python vrouter_agent_debug_info.py --host <host-ip> --port <port> --conf_file <path> -c """
...
class Debug(object):
    def __init__(self, dir_name, process_name, container_name, 
                log_path, lib_path, cli, config_file, host, port):
        ...
        self._ssh_client = self.create_ssh_connection(self._host, self._user, self._pwd)
    ...
    def create_directories(self):
        ...
        self._dir_name = '%s-%s-%s'%(self._dir_name, self._host, timestr)
    ...
    def copy_docker_logs(self):
        ...
        try:
            ...
            return
        # run below commands in node where agent is running and append the logs to file
    ...
    def copy_gcore(self):
        # since pid is appended to the core file, we need to find pif of process
        ...
        cmd = 'docker cp %s:%s %s'%(self._container, core_file_name, self._tmp_dir)
        ssh_stdin, ssh_stdout, ssh_stderr = self._ssh_client.exec_command(cmd)
        time.sleep(5)
    ...
    def copy_libraries(self, lib_list):
        for lib_name in lib_list:
            cmd = 'docker cp %s:%s%s %s' %(self._container, self._lib_path,
                                        lib_name, self._tmp_dir)
            ssh_stdin, ssh_stdout, ssh_stderr = self._ssh_client.exec_command(cmd)
            # need some delay to be sure that lib gets copied and then do ftp
            time.sleep(5)
    ...
    def copy_introspect(self):
        ...
        # executing above command will return list of all the sub-commands
        # which we will run in loop to collect all the agent introspect logs
    ...
    def copy_sandesh_traces(self, path):
        ...
        for i in range(len(trace_buffer_list)):
            ...
            try:
                ...
                print('\nCopying sandesh traces : Failed')
                return
            self._introspect.get('Snh_SandeshTraceRequest?x=' + trace_buffer_list[i])
            if self._introspect.output_etree is not None:
                for element in self._introspect.output_etree.iter('element'):
    ...
    def get_controller_ip_list(self):
        ...
        cmd = 'docker exec %s %s read xmpp connection status' %(self._container,
                                self._cli)
        cmd_op = self.get_ssh_cmd_output(cmd)
        ...
        for item in json_data['AgentXmppConnectionStatus']['peer']['list']:
            controller_ip_list.append(
                json_data['AgentXmppConnectionStatus']['peer']['list'][item]['controller_ip'])
        return controller_ip_list
    # end get_controller_ip_list

    def copy_controller_logs(self):
        for ip in self.get_controller_ip_list():
            # iterate through all the controller in the list  and collect logs
    ...
    def compress_folder(self):
        ...
        cmd = 'tar -zcf %s %s > /dev/null 2>&1' %(self._compress_file, self._parent_dir)
        subprocess.call(cmd, shell=True)
    #end compress_folder
...
def main():
    ...
    try:
        conf_file = argv[argv.index('--conf_file') + 1]
    except ValueError:
    ...
    lib_list = ['libc-2.17.so', 'libstdc++.so.6.0.19']
