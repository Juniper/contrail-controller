...
""" python vrouter_agent_debug_info.py --host <host-ip> --port <port>
        --conf_file <path> -c """
...
class Debug(object):
    ...
    def copy_sandesh_traces(self, path):
        if self._introspect.get(path) == 0:
            ...
            return
        trace_buffer_list = \
                self._introspect.getTraceBufferList('trace_buf_name')
    ...
    def get_controller_ip_list(self):
        ...
        controller_ip_list = []
        for item in \
                json_data['AgentXmppConnectionStatus']['peer']['list']:
    ...
    def copy_controller_logs(self):
        for ip in self.get_controller_ip_list():
            ...
            # and collect logs
            cntr_dir_path = '%s/controller/controller-%s'%( \
                    self._parent_dir, ip)
    ...
    def create_ssh_connection(self, ip, user, pw):
        try:
            ...
            print('\nError: ssh connection failed for ip %s: %s' \
                    %(ip,e))
            return None
...
class Introspect:
    ...
    def elementToStr(indent, etreenode):
        ...
        for element in etreenode:
            elementStr += Introspect.elementToStr(\
                    indent + '  ', element)
