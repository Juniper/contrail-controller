...
uint16_t DhcpHandler::AddConfigDhcpOptions(uint16_t opt_len,
    ...
    do {
        if (vm_itf_->config_dhcp_options().are_dhcp_options_set()) {
            options = vm_itf_->config_dhcp_options().dhcp_options();
            break;
        }
        ...
        if (vm_itf_->vn()) {
            ...
            if (index < vn_ipam.size() &&
                vn_ipam[index].config_dhcp_options.are_dhcp_options_set()) {
                options = vn_ipam[index].config_dhcp_options.dhcp_options();
                break;
            }
        }
...
uint16_t DhcpHandler::AddClasslessRouteOption(uint16_t opt_len) {
    std::vector<ConfigDhcpOptions::Subnet> host_routes;
    do {
        if (vm_itf_->config_dhcp_options().are_host_routes_set()) {
            host_routes = vm_itf_->config_dhcp_options().host_routes();
            break;
        }
        ...
        for (unsigned int i = 0; i < routes.size(); ++i) {
            ConfigDhcpOptions::Subnet subnet;
