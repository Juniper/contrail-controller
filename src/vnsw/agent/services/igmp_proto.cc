...
void IgmpProto::IgmpProtoInit(void) {
    ...
    gmp_sm_ = GmpSmManager::CreateGmpSm(agent_, task_name_, PktHandler::IGMP,
                                    io_);
    if (gmp_sm_) {
        gmp_sm_->Start();
    }
...
void IgmpProto::Shutdown() {
    ...
    if (gmp_sm_) {
        gmp_sm_->Stop();
        GmpSmManager::DeleteGmpSm(gmp_sm_);
    }
}
...
void IgmpProto::ItfNotify(DBTablePartBase *part, DBEntryBase *entry) {
    ...
    if (itf->IsDeleted()) {
        if (mif_state) {
            gmp_sm_->DeleteIntf(mif_state->gmp_intf_);
    ...
    if (mif_state == NULL) {
        ...
        mif_state->gmp_intf_ = gmp_sm_->CreateIntf();
        itf_attach_count_++;
...
IgmpInfo::McastInterfaceState::IgmpIfStats IgmpProto::GetIfStats(VmInterface *vm_itf) {
