            ...
            if (CheckPacket()) {
                ...
                if (vm_itf->vn() &&
                    ...
                    SendRAResponse(pkt_info_->GetAgentHdr().ifindex,
                                   ...
                                   pkt_info_->eth->h_source, prefix, plen);
...
bool Icmpv6Handler::RouterAdvertisement(Icmpv6Proto *proto) {
    ...
    for (Icmpv6Proto::VmInterfaceSet::const_iterator it = interfaces.begin();
        ...
        if ((*it)->IsIpv6Active()) {
            pkt_info_->pkt = new uint8_t[1024];
            memcpy(pkt_info_->pkt,
                   agent()->vhost_interface()->mac().ether_addr_octet,
                   ETH_ALEN);
            uint8_t mac[ETH_ALEN];
            ConvertMacToBytes((*it)->vm_mac(), mac);
            memcpy((pkt_info_->pkt + ETH_ALEN), mac, ETH_ALEN);
            pkt_info_->eth = (ethhdr *)(pkt_info_->pkt + IPC_HDR_LEN);
            pkt_info_->ip6 = (ip6_hdr *)(pkt_info_->pkt + IPC_HDR_LEN +
                                         sizeof(ethhdr));
            icmp_ = pkt_info_->transp.icmp6 =
                (icmp6_hdr *)(pkt_info_->pkt + IPC_HDR_LEN +
...
void Icmpv6Handler::ConvertMacToBytes(const std::string &mac,
                                      uint8_t *bytes) const {
    uint32_t data[ETH_ALEN];
    sscanf(mac.c_str(), "%x:%x:%x:%x:%x:%x", &data[0], &data[1], &data[2],
...
bool Icmpv6Handler::CheckPacket() {
    if (pkt_info_->len < (IPC_HDR_LEN + sizeof(ethhdr) + sizeof(ip6_hdr) +
...
uint16_t Icmpv6Handler::FillRouterAdvertisement(uint8_t *buf, uint8_t *src,
    ...
    nd_opt_prefix_info *prefix_info =
                    (nd_opt_prefix_info *)(buf + sizeof(nd_router_advert));
    ...
    icmp->nd_ra_cksum = Icmpv6Csum(src, dest, (icmp6_hdr *)icmp,
                        sizeof(nd_router_advert) + sizeof(nd_opt_prefix_info));

    return sizeof(nd_router_advert) + sizeof(nd_opt_prefix_info);
}
...
void Icmpv6Handler::SendPingResponse() {
    ...
    SendIcmpv6Response(pkt_info_->GetAgentHdr().ifindex, pkt_info_->vrf,
                       pkt_info_->ip_daddr.to_v6().to_bytes().data(),
                       pkt_info_->ip_saddr.to_v6().to_bytes().data(),
                       pkt_info_->eth->h_source, pkt_info_->ip6->ip6_plen);
}
...
void Icmpv6Handler::SendIcmpv6Response(uint16_t ifindex, uint16_t vrfindex,
    ...
    Send(len, ifindex, vrfindex, AGENT_CMD_SWITCH, PktHandler::ICMPV6);
}
