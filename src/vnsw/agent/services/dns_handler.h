...
private:
    ...
    bool IsLinkLocalResolveRequest(DnsItems::iterator &item,
                                   DnsItems *linklocal_items);
    bool ResolveLinkLocalRequests();
    bool HandleMessage();
    ...
    void UpdateStats();
    void GetDomainName(const VmInterface *vm_itf, std::string *domain_name);
    void GetBaseName(const std::string &name, std::string *base);
    std::string DnsItemsToString(DnsItems &items);
