...
TEST_F(IgmpTest, IgmpIntfConfig) {
    ...
    // Disable Snooping on idx = 0
    idx = 0;
    SetIgmpIntfConfig(input[idx].name, input[idx].intf_id, false, false);
    client->WaitForIdle();
    vm_itf = VmInterfaceGet(input[idx].intf_id);
    EXPECT_EQ(vm_itf->igmp_snooping_config(), false);
    ...
    // Enable Snooping on idx = 0
    idx = 0;
    SetIgmpIntfConfig(input[idx].name, input[idx].intf_id, true, true);
    client->WaitForIdle();
    vm_itf = VmInterfaceGet(input[idx].intf_id);
    EXPECT_EQ(vm_itf->igmp_snooping_config(), true);
...
TEST_F(IgmpTest, IgmpSystemConfig) {
    ...
    ClearIgmpIntfConfig(input[0].name);
    ClearIgmpConfig();
    client->WaitForIdle();
...
TEST_F(IgmpTest, IgmpQuerySend) {
    ...
    SetIgmpConfig(false, true);
    client->WaitForIdle();

    uint32_t idx = 0;
    for (idx = 0; idx < sizeof(input)/sizeof(PortInfo); idx++) {
        SetIgmpIntfConfig(input[idx].name, input[idx].intf_id, false, true);
        client->WaitForIdle();
        VmInterface *vm_itf = VmInterfaceGet(input[idx].intf_id);
        EXPECT_EQ(vm_itf->igmp_querying_config(), true);
        client->WaitForIdle();
    }
    ...
    usleep(sleep * qivl * MSECS_PER_SEC + 2000000);
    client->WaitForIdle();
...
TEST_F(IgmpTest, IgmpQueryDisabled) {
    ...
    for (idx = 0; idx < sizeof(input)/sizeof(PortInfo); idx++) {
        ...
        client->WaitForIdle();
        VmInterface *vm_itf = VmInterfaceGet(input[idx].intf_id);
        EXPECT_EQ(vm_itf->igmp_querying_config(), false);
        client->WaitForIdle();
    }
