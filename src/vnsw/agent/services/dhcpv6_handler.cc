...
bool Dhcpv6Handler::Run() {
    ...
    if (!vm_itf_->layer3_forwarding()) {
        dhcp_proto->IncrStatsError();
        DHCPV6_TRACE(Error, "DHCP request on VM port with disabled ipv4 service: "
                     << GetInterfaceIndex());
        return true;
    }
...
uint16_t Dhcpv6Handler::FillDhcpResponse(unsigned char *dest_mac,
    ...
    pkt_info_->ip6 = (ip6_hdr *)(pkt_info_->pkt + IPC_HDR_LEN + header_len);
...
void Dhcpv6Handler::SendDhcpResponse() {
    ...
    UpdateStats();
    uint16_t len = FillDhcpResponse(dest_mac, config_.gw_addr,
                                    pkt_info_->ip_saddr.to_v6());
    Send(len, GetInterfaceIndex(), pkt_info_->vrf,
         AGENT_CMD_SWITCH, PktHandler::DHCPV6);
}
