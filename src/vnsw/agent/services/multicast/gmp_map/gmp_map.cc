...
void igmp_notification_ready(void *context)
{
    ...
    while (notif_count-- > 0 || !gmpr_notification_last_sg(notification)) {
        ...
        bcopy(notification->notif_group_addr.gmp_addr, &g, sizeof(uint32_t));
        bcopy(notification->notif_source_addr.gmp_addr, &s, sizeof(uint32_t));
...
void igmp_host_notification_ready(void_t context)
{
    ...
    while (notif_count--) {
         ...
         */
        bcopy(gmpr_notif->host_notif_group_addr.gmp_addr, &g, sizeof(uint32_t));
        bcopy(gmpr_notif->host_notif_source_addr.gmp_addr, &s, sizeof(uint32_t));
        bcopy(gmpr_notif->host_notif_host_addr.gmp_addr, &h, sizeof(uint32_t));
