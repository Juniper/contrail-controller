...
bool DnsHandler::HandleDefaultDnsRequest(const VmInterface *vmitf) {
    ...
    while (curr_index_ < def_dns_resolvers_.size()) {
        if (!SendDnsQuery(def_dns_resolvers_[curr_index_], xid)) {
            curr_index_++;
        } else {
            return false;
        }
    ...
    if (handler) {
        ...
        if (valid_response) {
            ...
            delete handler;
                   ...
                   (!NeedRetryForNextServer(flags.ret) &&
                   handler->DefaultMethodInUse())) {
            HandleInvalidBindResponse(handler, flags, ques, ans, auth, add,
                                      xid);
        } else if (NeedRetryForNextServer(flags.ret) &&
                   handler->DefaultMethodInUse()) {
            if (!handler->TryNextDefaultServer()) {
                HandleInvalidBindResponse(handler, flags, ques, ans, auth, add,
                                          xid);
            }
        }
...
bool DnsHandler::TryNextDefaultServer() {
