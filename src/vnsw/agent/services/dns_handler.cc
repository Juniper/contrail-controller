...
bool DnsHandler::IsLinkLocalResolveRequest(DnsItems::iterator &item,
...
bool DnsHandler::ResolveLinkLocalRequests() {
    bool linklocal_request = false;
    for (DnsItems::iterator it = items_.begin(); it != items_.end(); ) {
        DnsItems linklocal_items;
        if (IsLinkLocalResolveRequest(it, &linklocal_items)) {
...
void DnsHandler::GetBaseName(const std::string &name, std::string *base) {
...
std::string DnsHandler::DnsItemsToString(DnsItems &items) {
    std::string str;
