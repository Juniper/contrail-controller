...
#include "services/multicast/gmp_map/gmp_sm.h"
...
public:
    struct IgmpIfStats {
        ...
        void Reset() {
            rx_unknown = 0;
            bzero(rx_badpacket, sizeof(uint32_t)*IGMP_MAX_TYPE);
            bzero(rx_okpacket, sizeof(uint32_t)*IGMP_MAX_TYPE);
        }
        uint32_t rx_unknown;
    ...
    void IncrRxBadPkt(unsigned long index) {
        stats_.rx_badpacket[index]++;
    }
    uint32_t GetRxBadPkt(unsigned long index) {
        return stats_.rx_badpacket[index];
    }
...
public:
    ...
    GmpSm *GetGmpSm() { return gmp_sm_; }
    IgmpInfo::McastInterfaceState::IgmpIfStats GetIfStats(VmInterface *vm_itf);
