...
public:
    ...
    const IgmpInfo::IgmpIfStats &GetIfStats() const { return stats_; }
    void ClearIfStats() { stats_.Reset(); }

    GmpIntf *gmp_intf_;
    std::string vrf_name_;
    IgmpInfo::IgmpIfStats stats_;
};
...
public:
    ...
    const bool GetIfStats(const VnEntry *vn, IpAddress gateway,
                            IgmpInfo::IgmpIfStats &stats);
    void ClearIfStats(const VnEntry *vn, IpAddress gateway);
    void IncrSendStats(const VmInterface *vm_itf, bool tx_done);

    DBTableBase::ListenerId VnListenerId ();
