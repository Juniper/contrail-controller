...
void ArpProto::VrfNotify(DBTablePartBase *part, DBEntryBase *entry) {
    ...
    if (!state){
        state = new ArpVrfState(agent_, this, vrf->GetInet4UnicastRouteTable());
...
ArpVrfState::ArpVrfState(Agent *agent_ptr, ArpProto *proto, AgentRouteTable *table):
    agent(agent_ptr), arp_proto(proto), rt_table(table),
