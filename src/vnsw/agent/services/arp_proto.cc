...
ArpDBState::ArpDBState(ArpVrfState *vrf_state, uint32_t vrf_id, Ip4Address ip) :
...
bool ArpDBState::SendArpRequest() {
    ...
    const unsigned char *smac = vrf_state_->agent->vrrp_mac();
    ...
    for (;it != wait_for_traffic_map_.end(); it++) {
        ...
        it->second++;
        arp_handler.SendArp(ARPOP_REQUEST, smac, gw_ip_.to_ulong(),
                            zero_mac, vm_ip_.to_ulong(),
                            it->first, vrf_id_);
...
void ArpDBState::SendArpRequestForAllIntf(const Inet4UnicastRouteEntry *route) {
