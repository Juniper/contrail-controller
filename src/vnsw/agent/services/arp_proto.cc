...
void ArpVrfState::RouteUpdate(DBTablePartBase *part, DBEntryBase *entry) {
    ...
    ArpEntry *arpentry = arp_proto->FindGratiousArpEntry(key);
    if (entry->IsDeleted() || deleted) {
        if (state) {
...
bool ArpProto::AddGratuitousArpEntry(ArpEntry *entry) {
    ...
    if (state == NULL || state->deleted == true)
        return false;
    gratuitous_arp_cache_.insert(ArpCachePair(entry->key(), entry));
    return true;
}
...
ArpEntry *ArpProto::FindGratiousArpEntry (const ArpKey &key) {
    ArpIterator it = gratuitous_arp_cache_.find(key);
