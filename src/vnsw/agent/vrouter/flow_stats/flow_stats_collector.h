...
public:
    ...
    void UpdateFloatingIpStats(const FlowEntry *flow, uint64_t bytes,
                               uint64_t pkts);
    void Shutdown();
    void AddEvent(const FlowEntryPtr &flow, uint32_t flow_handle, uint8_t g_id);
    void DeleteEvent(const FlowEntryPtr &flow, uint32_t flow_handle,
                     uint8_t gen_id, const RevFlowDepParams &params);
