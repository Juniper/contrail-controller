...
FlowExportInfo::FlowExportInfo() :
    flow_uuid_(boost::uuids::nil_uuid()), egress_uuid_(boost::uuids::nil_uuid())
    , rev_flow_uuid_(boost::uuids::nil_uuid()), source_vn_(), dest_vn_(),
    sg_rule_uuid_(), nw_ace_uuid_(),
...
FlowExportInfo::FlowExportInfo(FlowEntry *fe, uint64_t setup_time) :
    ...
    if (rflow) {
        ...
        rev_flow_uuid_ = boost::uuids::nil_uuid();
    }
    if (fe->intf_entry()) {
...
bool FlowExportInfo::IsEqual(const FlowExportInfo &rhs) const {
    ...
    if (rev_flow_uuid_ != rhs.rev_flow_uuid()) {
        return false;
    }
