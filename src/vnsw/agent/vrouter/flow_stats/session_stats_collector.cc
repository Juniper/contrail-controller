...
bool SessionStatsCollector::UpdateSloMatchRuleEntry(boost::uuids::uuid slo_uuid,
...
bool SessionStatsCollector::FindSloMatchRule(SessionSloRuleMap &map,
                                             const std::string match_uuid) {
    SessionSloRuleMap::iterator it;
    bool is_logged = false;
    it = map.find(match_uuid);
    if (it != map.end()) {
        is_logged = UpdateSloMatchRuleEntry(it->second.slo_uuid,
                                            match_uuid);
    }
    return is_logged;
}
    ...
    while (session_agg_map_iter != it->second.session_agg_map_.end()) {
        ...
        while (session_map_iter != session_agg_map_iter->second.session_map_.end()) {
            ...
            bool is_rev_logged = MatchSloForSession(
                                    ...
                                    match_policy_uuid);
            if (!(is_fwd_logged || is_rev_logged)) {
                session_map_iter++;
                continue;
            }
...
bool SessionSloState::UpdateSessionSloStateRuleRefCount(std::string uuid) {
    SessionSloRuleStateMap::iterator  it;
    it = session_rule_state_map_.find(uuid);
    if (it != session_rule_state_map_.end()) {
        it->second.ref_count++;
