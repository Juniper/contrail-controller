...
bool SessionStatsCollector::FindSloMatchRule(const SessionSloRuleMap &map,
    ...
    if (!fw_policy_uuid.empty()) {
        it = map.find(fw_policy_uuid);
        if (it != map.end()) {
            fw_logged = UpdateSloMatchRuleEntry(it->second.slo_uuid, fw_policy_uuid);
        }
    }

    if (!nw_policy_uuid.empty()) {
        it = map.find(nw_policy_uuid);
        if (it != map.end()) {
            nw_logged = UpdateSloMatchRuleEntry(it->second.slo_uuid, nw_policy_uuid);
        }
    }

    if (!sg_policy_uuid.empty()) {
        it = map.find(sg_policy_uuid);
        if (it != map.end()) {
            sg_logged = UpdateSloMatchRuleEntry(it->second.slo_uuid, sg_policy_uuid);
        }
    }
...
bool SessionStatsCollector::MatchSloForFlow(
    ...
    if (fe) {
        ...
        fw_policy_uuid = stats_info.export_info.
