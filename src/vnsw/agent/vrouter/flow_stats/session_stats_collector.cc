...
void SessionStatsCollector::AddSession(FlowEntry* fe, uint64_t setup_time) {
    ...
    if ((flow_session_map_iter != flow_session_map_.end()) &&
        (flow_session_map_iter->second.uuid() != fe_fwd->uuid()) &&
        (!(flow_session_map_iter->second.session_key().IsEqual(session_key))) &&
        (!(flow_session_map_iter->second.session_agg_key().IsEqual(session_agg_key))) &&
        (!(flow_session_map_iter->second.session_endpoint_key().IsEqual(session_endpoint_key)))) {
