...
TEST_F(FlowStatsManagerTest, FlowConfig) {
    boost::system::error_code ec;
    std::stringstream global_config;
    global_config << "<flow-aging-timeout-list>";
    global_config << "<flow-aging-timeout>";
    global_config << "<protocol>17</protocol>\n";
    global_config << "<port>0</port>\n";
    global_config << "<timeout>1</timeout>\n";
    global_config << "</flow-aging-timeout>";
    global_config << "</flow-aging-timeout-list>";
    ...
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0) != NULL);
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0)->flow_age_time_intvl_in_secs() == 1);
    client->WaitForIdle();

    global_config.str(std::string());
    global_config << "<flow-aging-timeout-list>";
    global_config << "<flow-aging-timeout>";
    global_config << "<protocol>17</protocol>\n";
    global_config << "<port>0</port>\n";
    global_config << "<timeout>2</timeout>\n";
    global_config << "</flow-aging-timeout>";
    global_config << "</flow-aging-timeout-list>";
    memset(buf, 0, 4096);
    AddXmlHdr(buf, len);
    ...
    client->WaitForIdle();
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0) != NULL);
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0)->flow_age_time_intvl_in_secs() == 2);
    client->WaitForIdle();
...
TEST_F(FlowStatsManagerTest, FlowConfig1) {
    boost::system::error_code ec;
    std::stringstream global_config;
    global_config << "<flow-aging-timeout-list>";
    global_config << "<flow-aging-timeout>";
    ...
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0) != NULL);
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 0)->flow_age_time_intvl_in_secs() == 1);
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 10) != NULL);
    EXPECT_TRUE(fsm_->Find(IPPROTO_UDP, 10)->flow_age_time_intvl_in_secs() == 1);
    client->WaitForIdle();
