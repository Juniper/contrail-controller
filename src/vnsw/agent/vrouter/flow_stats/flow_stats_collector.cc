...
FlowStatsCollector::FlowStatsCollector(boost::asio::io_service &io, int intvl,
        ...
        request_queue_(agent_uve_->agent()->task_scheduler()->
                       ...
                       boost::bind(&FlowStatsCollector::RequestHandler, this,
                                   _1)),
        flow_table_req_q_(agent_uve_->agent()->task_scheduler()->
                          GetTaskId(FlowTable::TaskName()), 1,
                          boost::bind(&FlowStatsCollector::FlowDelHandler,
                                      this, _1)),
...
void FlowStatsCollector::FlowDeleteEnqueue(const FlowKey &key, bool rev) {
    boost::shared_ptr<FlowTableReq>
        req(new FlowTableReq(FlowTableReq::DELETE_FLOW, key, rev));
    flow_table_req_q_.Enqueue(req);
}
...
void FlowStatsCollector::ExportFlow(const FlowKey &key,
    ...
    VmInterfaceKey ikey = info->intf_in();
    VmInterface *intf = dynamic_cast<VmInterface *>
        (agent_uve_->agent()->interface_table()->FindActiveEntry(&ikey));
    if (intf) {
...
/////////////////////////////////////////////////////////////////////////////
// FlowTable Delete message handler
/////////////////////////////////////////////////////////////////////////////
bool FlowStatsCollector::FlowDelHandler(boost::shared_ptr<FlowTableReq> req) {
    switch (req->event()) {
    case FlowTableReq::DELETE_FLOW: {
        agent_uve_->agent()->pkt()->flow_table()->
            Delete(req->key(), req->delete_rev_flow());
        break;
    }
    default:
        assert(0);
