...
void FlowStatsCollector::UpdateStatsAndExportFlow(FlowExportInfo *info,
    ...
    bool read_flow = true;
    if ((info->flow_handle() != fe->flow_handle()) ||
...
void FlowStatsCollector::AddFlow(FlowExportInfo info) {
    ...
    if (ret.second == false) {
        ...
        if ((prev.gen_id() != fe->gen_id()) ||
            ((prev.flow_handle() != FlowEntry::kInvalidFlowHandle) &&
             (prev.flow_handle() != fe->flow_handle()))) {
            reset_stats = true;
        }
        if (reset_stats) {
            /* Received ADD request for already added entry. Send FlowExport
             * to indicate delete for the flow. This export need not be sent
             * if teardown time is already set. Teardown time would be sent if
             * EvictedFlowStats updated request comes before this duplicate
             * Add.
             */
