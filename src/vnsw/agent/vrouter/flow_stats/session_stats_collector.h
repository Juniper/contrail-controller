...
public:
    ...
    size_t Size() const { return (client_session_agg_map_.size() +
                                  server_session_agg_map_.size()); }
...
private:
    ...
    int task_id_;
    SessionAggMap  client_session_agg_map_;
    SessionAggMap  server_session_agg_map_;
    Queue request_queue_;
    std::vector<SessionEndpoint> msg_list_;
