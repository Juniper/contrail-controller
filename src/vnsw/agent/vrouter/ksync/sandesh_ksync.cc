...
void KSyncSandeshContext::FlowMsgHandler(vr_flow_req *r) {
    ...
    FlowMgmtManager *mgr = flow_ksync_->ksync()->agent()->pkt()->
        flow_mgmt_manager();
    FlowEntry *evicted_flow = imgr->FindByIndex(r->get_fr_index());
    if (evicted_flow && evicted_flow->deleted() == false) {
        mgr->FlowStatsUpdateEvent(evicted_flow, r->get_fr_flow_bytes(),
