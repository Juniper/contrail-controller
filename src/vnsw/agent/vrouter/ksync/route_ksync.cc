...
VrfKSyncEntry::~VrfKSyncEntry() {
    LOG(ERROR, "Destroying VrfKSyncEntry");
}
...
bool VrfKSyncEntry::Sync(DBEntry* e) {
    ...
    if ((vrf->hbf_rintf() != hbf_rintf_) ||
        ...
        LOG(ERROR, "vrf->hbf_rintf() vrf->hbf_lintf() hbf_rintf_ hbf_lintf_ "
            << vrf->hbf_rintf() << " " << vrf->hbf_lintf() << " " << hbf_rintf_ << " " << hbf_lintf_);
...
int VrfKSyncEntry::Encode(sandesh_op::type op, uint8_t replace_plen,
    ...
    LOG(ERROR, "HBF enconding VrfKSyncEntry, vrf_id_ " << vrf_id_ <<
...
void VrfKSyncObject::VrfNotify(DBTablePartBase *partition, DBEntryBase *e) {
    ...
    std::stringstream ss;
    ss << "Vrf notify for vrf " << vrf->GetName();
    LOG(ERROR, ss.str());
