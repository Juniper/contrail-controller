...
bool RouteKSyncEntry::Sync(DBEntry *e) {
    ...
    if (rt_type_ == Agent::BRIDGE) {
        ...
        const AgentPath *path =
            l2_rt->FindV4DhcpPath(l2_rt->mac());
        //If not v4 search for v6
        if (path == NULL)
            path = l2_rt->FindV6DhcpPath(l2_rt->mac());
        const AgentPath *active_path = l2_rt->GetActivePath();
        //Only change flood_dhcp flag for EVPN programmed bridge entries.
        //Rest of the routes dont need this flag so keep it false.
        if (active_path->peer()->GetType() == Peer::EVPN_PEER)
            flood_dhcp = path ? false : true;
