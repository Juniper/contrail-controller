...
void KSyncBridgeMemory::CreateProtoAuditEntry(uint32_t idx, uint8_t gen_id) {
    ...
    vr_bridge_entry* entry = GetBridgeEntry(idx);
    BridgeRouteKSyncObject *obj = ksync_->bridge_route_ksync_obj();
    uint8_t *mac = entry->be_key.be_mac;
    MacAddress bmac(mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    //Create a BridgeRouteKSyncEntry. This will NOT send any message to vrouter
    BridgeRouteKSyncEntry key(obj, entry->be_key.be_vrf_id, bmac);
    KSyncEntry *kentry = obj->Create(&key);
    assert(kentry != NULL);

    //Delete BridgeRouteKSyncEntry to send delete message to Vrouter
    obj->Delete(kentry);
}
