...
void InterfaceKSyncObject::ProcessVrouterEntries(
                        KSyncRestoreData::Ptr restore_data) {
    if(dynamic_cast<InterfaceKSyncRestoreEndData *>(restore_data.get())) {
        //TODO set flag in ksync restoremanager and check if all flags are set 
        //then send restore ready event to agent init
        //TEMP:directly call function for now
        ksync()->agent()->agent_init()->SetKSyncRestoreDone();
        return;
    }
    ...
    if (dataPtr->GetIntfEntry()->get_type() == "VIRTUAL") {
        ...
        if (dataPtr->GetIntfEntry()->get_idx() != 0 )
        assert(ksync_entry_ptr == NULL);
