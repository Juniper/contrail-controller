...
void KSync::SetHugePages() {
    ...
    huge_fd_[0] = open(agent_->params()->huge_page_flow_file().c_str(),
                       O_RDWR, 0755);
    huge_fd_[1] = open(agent_->params()->huge_page_bridge_file().c_str(),
                       O_RDWR, 0755);
    if (huge_fd_[0] < 0 || huge_fd_[1] < 0) {
        fail = true;
        goto vrouter_send;
    }

    for (uint16_t i = 0; i < kHugePages; ++i) {
        huge_pages_[i] = (void *) mmap(NULL, 1024 * 1024 * 1024,
...
vrouter_send:
    ...
    for (uint16_t i = 0; i < kHugePages; ++i) {
        if (!fail) {
            ...
            huge_size.push_back(1024 * 1024 * 1024);
        } else {
