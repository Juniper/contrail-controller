...
const char *MirrorCfgTable::Add(const MirrorCreateReq &cfg) {
   ...
   uint8_t mirror_flag = AclEntrySpec::DecodeMirrorFlag (cfg.get_nhmode(),
   ...
   if (mirror_flag == MirrorEntryData::DynamicNH_With_JuniperHdr) {
       ...
       MacAddress mac = MacAddress::FromString(cfg.get_remote_vm_mac());
        ...
        IpAddress dst_ip = IpAddress::from_string(cfg.get_vtep_dst_ip());
        MirrorTable::AddMirrorEntry(entry->key.handle,entry->data.mirror_vrf,
                                    sip, agent_cfg_->agent()->mirror_port(),
...
const char *IntfMirrorCfgTable::Add(const IntfMirrorCreateReq &intf_mirror) {
    ...
    uint8_t mirror_flag = AclEntrySpec::DecodeMirrorFlag (intf_mirror.get_nhmode(),
    ...
    if (mirror_flag == MirrorEntryData::DynamicNH_With_JuniperHdr) {
        ...
        MacAddress mac = MacAddress::FromString(intf_mirror.get_remote_vm_mac());
        MirrorTable::AddMirrorEntry(entry->key.handle,
                                    entry->data.mirror_dest.mirror_vrf,
                                    entry->data.mirror_dest.sip,
                                    entry->data.mirror_dest.sport,
         ...
         IpAddress dst_ip = IpAddress::from_string(intf_mirror.get_vtep_dst_ip());
         MirrorTable::AddMirrorEntry(entry->key.handle,
                                     entry->data.mirror_dest.mirror_vrf,
                                     entry->data.mirror_dest.sip,
                                     entry->data.mirror_dest.sport,
