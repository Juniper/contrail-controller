...
bool DiagPktHandler::ParseIcmpData(const uint8_t *data, uint16_t data_len,
    ...
    uint8_t len;
    bool match = false;
    uint8_t ip_len; // this can be v4 or v6 hdr length
    struct ip *ip_hdr;
    ...
    if (is_v4) {
        ...
        *key = ntohs(ip_hdr->ip_id);
        ...
        if ((ip6_hdr->ip6_src.s6_addr == ipam->default_gw.to_v6().to_bytes().data()) ||
            ip6_hdr->ip6_dst.s6_addr == ipam->default_gw.to_v6().to_bytes().data()) {
            match = true;
        }
    ...
    if (match) {
        ...
        if (is_v4) {
            ...
            ip6_hdr = (struct ip6_hdr *)(data + len);
            len += sizeof(struct ip6_hdr);
            proto = ip6_hdr->ip6_ctlun.ip6_un1.ip6_un1_nxt;
        }
        switch (proto) {
...
void DiagPktHandler::Swapv6L4() {
    if (pkt_info_->ip_proto == IPPROTO_TCP) {
        tcphdr *tcp = pkt_info_->transp.tcp;
        TcpHdr(ntohs(pkt_info_->ip6->ip6_ctlun.ip6_un1.ip6_un1_plen) - sizeof(struct ip),
...
void DiagPktHandler::SwapIp6Hdr() {
    ...
    Ip6Hdr(ip6, ntohl(ip6->ip6_ctlun.ip6_un1.ip6_un1_plen), 255,
           ip6->ip6_ctlun.ip6_un1.ip6_un1_nxt, ip6->ip6_dst.s6_addr,
           ip6->ip6_src.s6_addr);
}
