...
void DiagEntry::FillOamPktHeader(OverlayOamPktData *pktdata, uint32_t vxlan_id,
    ...
    pktdata->oamtlv_.length_ = htons(sizeof(OamTlv::VxlanOamTlv));
    OamTlv::VxlanOamTlv *vxlan_tlv =
        (OamTlv::VxlanOamTlv *) pktdata->oamtlv_.data_;
    vxlan_tlv->vxlan_id_ = htonl(vxlan_id);
    vxlan_tlv->sip_ = htonl(sip_.to_ulong());
}
