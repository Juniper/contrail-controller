...
public:
    ...
    friend void intrusive_ptr_add_back_ref(const IntrusiveReferer ref,
                                           const Derived* p) {
        const AgentRefCount *entry = (const AgentRefCount *) (p);
    ...
    friend void intrusive_ptr_del_back_ref(const IntrusiveReferer ref,
                                           const Derived* p) {
        const AgentRefCount *entry = (const AgentRefCount *) (p);
        tbb::mutex::scoped_lock lock(entry->mutex_);
...
private:
    ...
    mutable std::set<IntrusiveReferer> back_ref_set_;
