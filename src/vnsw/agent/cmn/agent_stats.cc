...
void AgentStats::UpdateFlowMinMaxStats(FlowCounters &stat) {
    /* Copy the value of stat.total_flows so that we can use consistent value
     * through out this function because stat.total_flows may change during the
     * course of execustion of this funciton */
    uint64_t total_flows = stat.total_flows;
...
void AgentStats::ResetFlowMinMaxStats(FlowCounters &stat) {
