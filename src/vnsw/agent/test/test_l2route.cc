...
protected:
    ...
    void DeleteRoute(const Peer *peer, const std::string &vrf_name,
        ...
        BridgeAgentRouteTable::DeleteReq(peer, vrf_name_, remote_vm_mac,
                                        ip_addr, 0);
        client->WaitForIdle();
...
TEST_F(RouteTest, LocalVmRoute_1) {
    ...
    EXPECT_TRUE(obj->bridge_forwarding() == true);
    WAIT_FOR(1000, 100,
...
TEST_F(RouteTest, Enqueue_l2_route_add_on_deleted_vrf) {
    ...
    BridgeAgentRouteTable::AddRemoteVmRouteReq(agent_->local_vm_peer(),
...
TEST_F(RouteTest, Enqueue_l2_route_del_on_deleted_vrf) {
    ...
    client->WaitForIdle();
    TaskScheduler::GetInstance()->Stop();
    BridgeAgentRouteTable::DeleteReq(agent_->local_vm_peer(), vrf_name_,
                                     local_vm_mac_, local_vm_ip4_, 0);
    vrf_ref = NULL;
    TaskScheduler::GetInstance()->Start();
    client->WaitForIdle();
