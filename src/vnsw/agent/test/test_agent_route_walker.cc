...
public:    
    AgentRouteWalkerTest(AgentRouteWalker::WalkType walk_type,
...
public:    
    Test() {
        ...
        walker_ = Agent::GetInstance()->oper_db()->agent_route_walk_manager()->
            AllocWalker<AgentRouteWalkerTest>(AgentRouteWalker::ALL,
                                          "AgentRouteWalkerTest");
    };
    ~Test() { 
    }
...
TEST_F(Test, restart_walk_with_2_vrf) {
    ...
    WAIT_FOR(100, 1000, walker()->IsWalkCompleted() == true);
    EXPECT_TRUE(walker()->walk_task_context_mismatch_ == false);
    walker()->walk_task_context_mismatch_ = true;
    DeleteEnvironment(2);
}
