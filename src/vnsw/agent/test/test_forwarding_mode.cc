...
protected:
    ForwardingModeTest() : vrf_name_("vrf1"), eth_name_(eth_itf) {
    ...
    void VerifyVmInterfaceDhcp() {
        ...
        std::auto_ptr<InterfaceKSyncEntry> ksync(new InterfaceKSyncEntry(obj,
                                                                         vm_intf));
        ksync->Sync(vm_intf);
        if ((ksync->layer3_forwarding() == false) &&
            (ksync->bridging() == true)) {
    ...
    void VerifyL3OnlyMode() {
        ...
        WAIT_FOR(1000, 1000, (route_state->exported_ == false));
