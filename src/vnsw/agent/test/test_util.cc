...
InetUnicastRouteEntry* LPMRouteGet(const string &vrf_name, const Ip4Address &addr) {
    VrfEntry *vrf = Agent::GetInstance()->vrf_table()->FindVrfFromName(vrf_name);
    if (vrf == NULL) {
        return NULL;
    }
...
const NextHop* LPMRouteToNextHop(const string &vrf_name,
    ...
    InetUnicastRouteEntry* rt = LPMRouteGet(vrf_name, addr);
    if (rt == NULL)
        return NULL;
...
void CfgVxlanRouting(bool mode) {
    std::stringstream vxlan_routing_mode_str;
    if (mode) {
        vxlan_routing_mode_str << "<vxlan-routing>true</vxlan-routing>";
    } else {
        vxlan_routing_mode_str << "<vxlan-routing>false</vxlan-routing>";
    }
    AddNode("project", "project-admin", 1,
            vxlan_routing_mode_str.str().c_str());
