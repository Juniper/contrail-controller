...
bool VrfStatsMatch(int vrf_id, std::string vrf_name, bool stats_match,
    ...
    if (st->discards == req.get_vsr_discards() &&
        st->resolves == req.get_vsr_resolves() &&
        st->receives == req.get_vsr_receives() &&
        st->udp_tunnels == req.get_vsr_udp_tunnels() &&
        st->udp_mpls_tunnels == req.get_vsr_udp_mpls_tunnels() &&
        st->gre_mpls_tunnels == req.get_vsr_gre_mpls_tunnels() &&
        st->ecmp_composites == req.get_vsr_ecmp_composites() &&
        st->l2_mcast_composites == req.get_vsr_l2_mcast_composites() &&
        st->fabric_composites == req.get_vsr_fabric_composites() &&
        st->l2_encaps == req.get_vsr_l2_encaps() &&
        st->encaps == req.get_vsr_encaps() &&
        st->gros == req.get_vsr_gros() &&
        st->diags == req.get_vsr_diags() &&
        st->encap_composites == req.get_vsr_encap_composites() &&
        st->evpn_composites == req.get_vsr_evpn_composites() &&
        st->vrf_translates == req.get_vsr_vrf_translates() &&
        st->vxlan_tunnels == req.get_vsr_vxlan_tunnels() &&
        st->arp_virtual_proxy == req.get_vsr_arp_virtual_proxy() &&
        st->arp_virtual_stitch == req.get_vsr_arp_virtual_stitch() &&
        st->arp_virtual_flood == req.get_vsr_arp_virtual_flood() &&
        st->arp_physical_stitch == req.get_vsr_arp_physical_stitch() &&
        st->arp_tor_proxy == req.get_vsr_arp_tor_proxy() &&
        st->arp_physical_flood == req.get_vsr_arp_physical_flood() &&
        st->l2_receives == req.get_vsr_l2_receives() &&
        st->uuc_floods == req.get_vsr_uuc_floods()) {
        return true;
    }
    return false;
}
...
bool VrfStatsMatchPrev(int vrf_id, const vr_vrf_stats_req &req) {
    ...
    if (st->prev_discards == req.get_vsr_discards() &&
        st->prev_resolves == req.get_vsr_resolves() &&
        st->prev_receives == req.get_vsr_receives() &&
        st->prev_udp_tunnels == req.get_vsr_udp_tunnels() &&
        st->prev_udp_mpls_tunnels == req.get_vsr_udp_mpls_tunnels() &&
        st->prev_gre_mpls_tunnels == req.get_vsr_gre_mpls_tunnels() &&
        st->prev_ecmp_composites == req.get_vsr_ecmp_composites() &&
        st->prev_l2_mcast_composites == req.get_vsr_l2_mcast_composites() &&
        st->prev_fabric_composites == req.get_vsr_fabric_composites() &&
        st->prev_l2_encaps == req.get_vsr_l2_encaps() &&
        st->prev_encaps == req.get_vsr_encaps() &&
        st->prev_gros == req.get_vsr_gros() &&
        st->prev_diags == req.get_vsr_diags() &&
        st->prev_encap_composites == req.get_vsr_encap_composites() &&
        st->prev_evpn_composites == req.get_vsr_evpn_composites() &&
        st->prev_vrf_translates == req.get_vsr_vrf_translates() &&
        st->prev_vxlan_tunnels == req.get_vsr_vxlan_tunnels() &&
        st->prev_arp_virtual_proxy == req.get_vsr_arp_virtual_proxy() &&
        st->prev_arp_virtual_stitch == req.get_vsr_arp_virtual_stitch() &&
        st->prev_arp_virtual_flood == req.get_vsr_arp_virtual_flood() &&
        st->prev_arp_physical_stitch == req.get_vsr_arp_physical_stitch() &&
        st->prev_arp_tor_proxy == req.get_vsr_arp_tor_proxy() &&
        st->prev_arp_physical_flood == req.get_vsr_arp_physical_flood() &&
        st->prev_l2_receives == req.get_vsr_l2_receives() &&
        st->prev_uuc_floods == req.get_vsr_uuc_floods()) {
        return true;
    }
    return false;
}
