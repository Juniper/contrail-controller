...
class EventNotifyManagerTest : public ::testing::Test {
public:
    EventNotifyManagerTest() {
        agent_ = Agent::GetInstance();
        test_a_1_ = false;
        test_a_2_ = false;
        test_b_1_ = false;
    }
    virtual ~EventNotifyManagerTest() { }
    virtual void SetUp() {
    }
...
TEST_F(EventNotifyManagerTest, basic) {
}

TEST_F(EventNotifyManagerTest, notify) {
    EventNotifyHandle::Ptr ptr1 = agent_->event_notify_manager()->RegisterSubscriber
        ((new TestEventNotifyKey(TestEventNotifyKey::A)),
         boost::bind(&EventNotifyManagerTest::TestA_listener_1, this),
         TaskScheduler::GetInstance()->GetTaskId("test1"));
    EventNotifyHandle::Ptr ptr2 = agent_->event_notify_manager()->RegisterSubscriber
        ((new TestEventNotifyKey(TestEventNotifyKey::A)),
         boost::bind(&EventNotifyManagerTest::TestA_listener_2, this),
         TaskScheduler::GetInstance()->GetTaskId("test1"));
    EventNotifyHandle::Ptr ptr3 = agent_->event_notify_manager()->RegisterSubscriber
        ((new TestEventNotifyKey(TestEventNotifyKey::B)),
         boost::bind(&EventNotifyManagerTest::TestB_listener_1, this),
         TaskScheduler::GetInstance()->GetTaskId("test2"));
    ...
    agent_->event_notify_manager()->DeregisterSubscriber(ptr1);
    agent_->event_notify_manager()->
        Notify(new TestEventNotifyKey(TestEventNotifyKey::A));
    WAIT_FOR(100, 100, (test_a_2_ == true));
    EXPECT_TRUE(test_a_1_ == false);
    ...
    agent_->event_notify_manager()->DeregisterSubscriber(ptr2);
    agent_->event_notify_manager()->DeregisterSubscriber(ptr3);
}
