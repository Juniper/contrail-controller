    ...
    def fixup_contrail_vrouter_agent(self):
        ...
        if self.dev:
            ...
            if self._args.vrouter_1G_hugepages:
                if (os.path.isfile('/hugepage_1G/vrouter_1G_mem_0') and os.path.isfile('/hugepage_1G/vrouter_1G_mem_1')):
                    vrouter_kmod_1G_page = '/hugepage_1G/vrouter_1G_mem_0' + ' /hugepage_1G/vrouter_1G_mem_1'
            if self._args.vrouter_2M_hugepages:
                if (os.path.isfile('/hugepage_2M/vrouter_2M_mem_0') and os.path.isfile('/hugepage_2M/vrouter_2M_mem_1')):
                    vrouter_kmod_2M_page = '/hugepage_2M/vrouter_2M_mem_0' + ' /hugepage_2M/vrouter_2M_mem_1'
...
''' % (self.vhost_ip, self.netmask))
    ...
    def setup_vrouter_kmod_hugepages(self):
        ...
        line = "vrouter_kmod_1G_hugepages=false"
        insert_line_to_file(pattern=pattern, line=line,
        ...
        line = "vrouter_kmod_2M_hugepages=false"
        insert_line_to_file(pattern=pattern, line=line,
                            file_name='/etc/contrail/agent_param')

        # Configure vrouter kernel mode 1G hugepages
        if self._args.vrouter_1G_hugepages:
            if (int(self._args.vrouter_1G_hugepages) < 2):
                raise RuntimeError("Min 2 1G pages are required configured 1G pages are %s"
                                  % (self._args.vrouter_1G_hugepages))
            mounted = local("sudo mount | grep hugepage_1G | cut -d' ' -f 3",
                            capture=True, warn_only=False)
            if (mounted != ""):
                print "hugepages already mounted on %s" % mounted
            else:
                local("sudo mkdir -p /hugepage_1G", warn_only=False)
                local("sudo mount -t hugetlbfs -o pagesize=1G none /hugepage_1G", warn_only=False)
                if os.path.isdir('/hugepage_1G'):
                    for i in range(2):
                        local("sudo touch /hugepage_1G/vrouter_1G_mem_%s " % i, warn_only=False)
            pattern = "hugepage_1G"
            line = "hugetlbfs    "\
                   "/hugepage_1G    hugetlbfs defaults,pagesize=1G      0       0"
            insert_line_to_file(pattern=pattern, line=line,
                                file_name='/etc/fstab')
            pattern = "vrouter_kmod_1G_hugepages"
            line = "vrouter_kmod_1G_hugepages=true"
            insert_line_to_file(pattern=pattern, line=line,
                                file_name='/etc/contrail/agent_param')

        # Configure vrouter kernel mode 2M hugepages
        if self._args.vrouter_2M_hugepages and self._args.vrouter_1G_hugepages:
            mounted = local("sudo mount | grep hugepage_2M | cut -d' ' -f 3",
                            capture=True, warn_only=False)
            if (mounted != ""):
                print "hugepages already mounted on %s" % mounted
            else:
                local("sudo mkdir -p /hugepage_2M", warn_only=False)
                local("sudo mount -t hugetlbfs -o pagesize=2M none /hugepage_2M", warn_only=False)
                if os.path.isdir('/hugepage_2M'):
                    for i in range(2):
                        local("sudo touch /hugepage_2M/vrouter_2M_mem_%s " % i, warn_only=False)
            pattern = "hugepage_2M"
            line = "hugetlbfs    "\
                   "/hugepage_2M    hugetlbfs defaults,pagesize=2M      0       0"
            insert_line_to_file(pattern=pattern, line=line,
                                file_name='/etc/fstab')
            pattern = "vrouter_kmod_2M_hugepages"
            line = "vrouter_kmod_2M_hugepages=true"
            insert_line_to_file(pattern=pattern, line=line,
                                file_name='/etc/contrail/agent_param')
