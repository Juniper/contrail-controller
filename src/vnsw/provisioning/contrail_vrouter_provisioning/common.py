...
class CommonComputeSetup(ContrailSetup, ComputeNetworkSetup):
    ...
    def fixup_contrail_vrouter_agent(self):
        ...
        self.mac = None
        # Fresh install
        if self.dev and self.reprov == False:
            ...
            self.mac = self.get_value_from_agent_cfg_file(section, key)
            section = "VIRTUAL-HOST-INTERFACE"
            key = "ip"
            self.cidr = self.get_value_from_agent_cfg_file(section, key)
            section = "VIRTUAL-HOST-INTERFACE"
            key = "gateway"
            self.gateway = self.get_value_from_agent_cfg_file(section, key)
            self.netmaks = "255.255.255.0"
        ...
        if self.dev:
            ...
            if self._args.dpdk and self.reprov == False:
                iface = self.dev
            ...
            elif self._args.dpdk and self.reprov == True:
                section = "DEFAULT"
                key = "physical_interface_address"
                pci_dev = self.get_value_from_agent_cfg_file(section, key)
...
''' % (self.vhost_ip, self.netmask))
    ...
    def disable_nova_compute(self):
        ...
        if local("sudo service nova-compute status | grep running", warn_only=True).succeeded:
            ...
            # TODO Remove TSN node from nova manage service list
    ...
    def setup(self):
        ...
        if self._args.tsn_mode:
            ...
            self.run_services()
            if self._args.register and self.reprov == False:
                self.add_vnc_config()
