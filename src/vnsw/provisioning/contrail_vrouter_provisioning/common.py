    ...
    def fixup_contrail_vrouter_agent(self):
        ...
        if self.dev and self.dev != 'vhost0':
            ...
            if self._args.dpdk:
                ...
                # Add DPDK info in ctrl-details
                local("grep 'DPDK_MODE=TRUE' /etc/contrail/ctrl-details ||"\
                      " sed -i.bak -e '$a\DPDK_MODE=TRUE' "+\
                      "/etc/contrail/ctrl-details", warn_only=True)
                local("grep 'DPDK_INTERFACE=.*' /etc/contrail/ctrl-details ||"\
                      " sed -i.bak -e '$a\DPDK_INTERFACE=%s' "\
                      "/etc/contrail/ctrl-details" % (self.dev), warn_only=True)
