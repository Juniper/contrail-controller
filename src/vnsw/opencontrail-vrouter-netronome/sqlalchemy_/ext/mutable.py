...
.. versionadded:: 0.7 :mod:`sqlalchemy_.ext.mutable` replaces SQLAlchemy's
...
JSON strings before being persisted::
    ...
    from sqlalchemy_.types import TypeDecorator, VARCHAR
    import json
...
:mod:`sqlalchemy_.ext.mutable` extension can be used
with any type whose target Python type may be mutable, including
:class:`.PickleType`, :class:`.postgresql.ARRAY`, etc.

When using the :mod:`sqlalchemy_.ext.mutable` extension, the value itself
tracks all parents which reference it.  Below, we illustrate the a simple
version of the :class:`.MutableDict` dictionary object, which applies
the :class:`.Mutable` mixin to a plain Python dictionary::

    from sqlalchemy_.ext.mutable import Mutable
...
attribute. Such as, with classical table metadata::
    ...
    from sqlalchemy_ import Table, Column, Integer
...
mapping against the ``my_data`` table::
    ...
    from sqlalchemy_ import mapper
...
There's no difference in usage when using declarative::
    ...
    from sqlalchemy_.ext.declarative import declarative_base
...
will flag the attribute as "dirty" on the parent object::
    ...
    >>> from sqlalchemy_.orm import Session
...
The key to the :mod:`sqlalchemy_.ext.mutable` extension relies upon the
...
.. versionchanged:: 0.7
    ...
    propagate them to all owning parents. The :mod:`sqlalchemy_.ext.mutable`
...
:meth:`.MutableComposite.changed` method::
    ...
    from sqlalchemy_.ext.mutable import MutableComposite
...
objects to each of the ``Vertex.start`` and ``Vertex.end`` attributes::
    ...
    from sqlalchemy_.orm import composite, mapper
    from sqlalchemy_ import Table, Column
