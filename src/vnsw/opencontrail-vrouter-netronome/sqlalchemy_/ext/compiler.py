...
:class:`~sqlalchemy_.sql.expression.ClauseElement` subclasses and one or
more callables defining its compilation::

    from sqlalchemy_.ext.compiler import compiles
    from sqlalchemy_.sql.expression import ColumnClause
...
Above, ``MyColumn`` extends :class:`~sqlalchemy_.sql.expression.ColumnClause`,
...
invoked for the dialect in use::
    ...
    from sqlalchemy_.schema import DDLElement
...
:class:`~sqlalchemy_.engine.interfaces.Compiled` object in use. This object
...
:class:`~sqlalchemy_.sql.compiler.SQLCompiler` and
:class:`~sqlalchemy_.sql.compiler.DDLCompiler` both include a ``process()``
method which can be used for compilation of embedded attributes::

    from sqlalchemy_.sql.expression import Executable, ClauseElement
...
``autocommit`` flag::
    ...
    from sqlalchemy_.sql.expression import UpdateBase
...
an endless loop.   Such as, to add "prefix" to all insert statements::
    ...
    from sqlalchemy_.sql.expression import Insert
...
* :class:`~sqlalchemy_.sql.expression.ClauseElement` - This is the root
  expression class. Any SQL expression can be derived from this base, and is
  probably the best choice for longer constructs such as specialized INSERT
  statements.

* :class:`~sqlalchemy_.sql.expression.ColumnElement` - The root of all
  "column-like" elements. Anything that you'd place in the "columns" clause of
  a SELECT statement (as well as order by and group by) can derive from this -
  the object will automatically have Python "comparison" behavior.

  :class:`~sqlalchemy_.sql.expression.ColumnElement` classes want to have a
...
* :class:`~sqlalchemy_.sql.functions.FunctionElement` - This is a hybrid of a
  ...
  ``select()`` construct::
      ...
      from sqlalchemy_.sql.expression import FunctionElement
...
* :class:`~sqlalchemy_.schema.DDLElement` - The root of all DDL expressions,
...
* :class:`~sqlalchemy_.sql.expression.Executable` - This is a mixin which
...
accommodates two arguments::
    ...
    from sqlalchemy_.sql import expression
    from sqlalchemy_.ext.compiler import compiles
    from sqlalchemy_.types import Numeric
...
Example usage::
    ...
    from sqlalchemy_ import select, union_all
