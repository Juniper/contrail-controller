...
The dialect supports several :func:`~sqlalchemy_.create_engine()` arguments
...
specify sequences, use the sqlalchemy_.schema.Sequence object which is passed
...
* the NVARCHAR2 and NCLOB datatypes are no longer generated as DDL when
  the :class:`~sqlalchemy_.types.Unicode` is used - VARCHAR2 and CLOB are
  issued instead.   This because these types don't seem to work correctly on
  Oracle 8 even though they are available.  The
  :class:`~sqlalchemy_.types.NVARCHAR` and
  :class:`~sqlalchemy_.dialects.oracle.NCLOB` types will always generate
  NVARCHAR2 and NCLOB.
...
from sqlalchemy_ import util, sql
from sqlalchemy_.engine import default, reflection
from sqlalchemy_.sql import compiler, visitors, expression
from sqlalchemy_.sql import operators as sql_operators
from sqlalchemy_.sql.elements import quoted_name
from sqlalchemy_ import types as sqltypes, schema as sa_schema
from sqlalchemy_.types import VARCHAR, NVARCHAR, CHAR, \
