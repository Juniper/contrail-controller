...
All entities are subclasses of :class:`~sqlalchemy_.schema.SchemaItem`, and as
defined in this module they are intended to be agnostic of any vendor-specific
constructs.

A collection of entities are grouped into a unit called
:class:`~sqlalchemy_.schema.MetaData`. MetaData serves as a logical grouping of
schema elements, and can also be associated with an actual database connection
such that operations involving the contained elements can contact the database
as needed.

Two of the elements here also build upon their "syntactic" counterparts, which
are defined in :class:`~sqlalchemy_.sql.expression.`, specifically
:class:`~sqlalchemy_.schema.Table` and :class:`~sqlalchemy_.schema.Column`.
...
class Column(SchemaItem, ColumnClause):
    ...
    def __init__(self, *args, **kwargs):
        ...
        :param name: The name of this column as represented in the database.
          ...
          usage within the :mod:`~sqlalchemy_.ext.declarative` extension.

        :param type\_: The column's type, indicated using an instance which
          subclasses :class:`~sqlalchemy_.types.TypeEngine`.  If no arguments
        ...
        :param onupdate: A scalar, Python callable, or
            :class:`~sqlalchemy_.sql.expression.ClauseElement` representing a
        ...
        :param server_default: A :class:`.FetchedValue` instance, str, Unicode
            or :func:`~sqlalchemy_.sql.expression.text` construct representing
            ...
            A :func:`~sqlalchemy_.sql.expression.text` expression will be
...
class Sequence(DefaultGenerator):
    ...
    @util.dependencies("sqlalchemy_.sql.functions.func")
...
class MetaData(SchemaItem):
    ...
    @util.dependencies("sqlalchemy_.engine.url")
    def _bind_to(self, url, bind):
        """Bind this MetaData to an Engine, Connection, string or URL."""

        if isinstance(bind, util.string_types + (url.URL, )):
            self._bind = sqlalchemy_.create_engine(bind)
        else:
            self._bind = bind
    ...
    def _bind_to(self, url, bind):
        ...
        if isinstance(bind, util.string_types + (url.URL, )):
            try:
                ...
                e = sqlalchemy_.create_engine(bind)
