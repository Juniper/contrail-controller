...
def create_engine(*args, **kwargs):
    ...
    the URL can be an instance of :class:`~sqlalchemy_.engine.url.URL`.
    ...
    :param execution_options: Dictionary execution options which will
        ...
        :meth:`~sqlalchemy_.engine.Connection.execution_options`
    ...
    :param listeners: A list of one or more
        :class:`~sqlalchemy_.interfaces.PoolListener` objects which will
        receive connection pool events.

    :param logging_name:  String identifier which will be used within
        the "name" field of logging records generated within the
        "sqlalchemy_.engine" logger. Defaults to a hexstring of the
        object's id.

    :param max_overflow=10: the number of connections to allow in
        connection pool "overflow", that is connections that can be
        opened above and beyond the pool_size setting, which defaults
        to five. this is only used with :class:`~sqlalchemy_.pool.QueuePool`.
    ...
    :param pool=None: an already-constructed instance of
        :class:`~sqlalchemy_.pool.Pool`, such as a
        :class:`~sqlalchemy_.pool.QueuePool` instance. If non-None, this
        pool will be used directly as the underlying connection pool
        for the engine, bypassing whatever connection parameters are
        present in the URL argument. For information on constructing
        connection pools manually, see :ref:`pooling_toplevel`.

    :param poolclass=None: a :class:`~sqlalchemy_.pool.Pool`
    ...
    :param pool_logging_name:  String identifier which will be used within
       the "name" field of logging records generated within the
       "sqlalchemy_.pool" logger. Defaults to a hexstring of the object's
       id.

    :param pool_size=5: the number of connections to keep open
        inside the connection pool. This used with
        :class:`~sqlalchemy_.pool.QueuePool` as
        well as :class:`~sqlalchemy_.pool.SingletonThreadPool`.  With
        :class:`~sqlalchemy_.pool.QueuePool`, a ``pool_size`` setting
        of 0 indicates no limit; to disable pooling, set ``poolclass`` to
        :class:`~sqlalchemy_.pool.NullPool` instead.
    ...
    :param pool_timeout=30: number of seconds to wait before giving
        ...
        with :class:`~sqlalchemy_.pool.QueuePool`.
...
def engine_from_config(configuration, prefix='sqlalchemy_.', **kwargs):
    """Create a new Engine instance using a configuration dictionary.

    The dictionary is typically produced from a config file.
    
    The keys of interest to ``engine_from_config()`` should be prefixed, e.g.
    ``sqlalchemy_.url``, ``sqlalchemy_.echo``, etc.  The 'prefix' argument
    indicates the prefix to be searched for.  Each matching key (after the
    prefix is stripped) is treated as though it were the corresponding keyword
    argument to a :func:`.create_engine` call.

    The only required key is (assuming the default prefix) ``sqlalchemy_.url``,
