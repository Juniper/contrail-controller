...
class TestPortDB(unittest.TestCase):
    ...
    def test_Port_VF_interaction(self):
        ...
        v2 = s.query(vf.VF).filter(vf.VF.addr == addr2).one_or_none()
        ...
        v3 = s.query(vf.VF).filter(vf.VF.addr == addr3).one_or_none()
        ...
        #    VF
        p1_alias = (
            s.query(port.Port).filter(port.Port.uuid == u1).one_or_none()
        )
    ...
    def test_Port_PlugMode_interaction(self):
        ...
        pm = s.query(port.PlugMode).filter(
            port.PlugMode.neutron_port == mu[PM.SRIOV]).one_or_none()
...
class Test_VRT_604_gc(unittest.TestCase):
    ...
    def test_Port_PlugMode_gc(self):
        ...
        pm = (
            ...
            filter(port.PlugMode.neutron_port == u).one_or_none()
        )
        self.assertIsNone(pm)
        po = (
            s.query(port.Port).
            filter(port.Port.uuid == u).one_or_none()
        )
        self.assertIsNone(po)
