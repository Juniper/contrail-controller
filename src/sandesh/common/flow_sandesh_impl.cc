...
void SessionEndpointObject::LogUnrolled(std::string category,
    ...
    log4cplus::Logger Xlogger = Sandesh::sample_logger();
    log4cplus::Logger SLO_logger = Sandesh::slo_logger();
    bool is_send_sample_to_logger_enabled = Sandesh::is_send_slo_to_logger_enabled();
...
void SessionEndpointObject::adjust_session_end_point_objects(
    ...
    bool is_send_sample_to_collector = Sandesh::is_send_sampled_to_collector_enabled();
    ...
    for (sep_iter = session_data.begin(); sep_iter !=
        ...
        for (local_ep_iter = sep_iter->sess_agg_info.begin();
             ...
             local_ep_iter++) {
            // Adjust the aggreate logged and sampled info
