...
func checkFileOrDirExists(fname string) bool {
	if _, err := os.Stat(fname); err != nil {
		log.Infof("File/Dir - %s does not exist. Error - %s", fname, err)
		return false
	}
...
func getFilesinDir(dirName string) ([]os.FileInfo, error) {
	if checkFileOrDirExists(dirName) {
		...
		if err != nil {
			log.Errorf("Error in reading directory - %s", dirName)
			return nil, err
		}
		return files, nil
	}
...
func readContrailAddMsg(fname string) (contrailAddMsg, error) {
	...
	if checkFileOrDirExists(fname) {
		...
		if err != nil {
			...
			return msg, fmt.Errorf("Error reading file %s. Error : %s", fname, err)
		}
