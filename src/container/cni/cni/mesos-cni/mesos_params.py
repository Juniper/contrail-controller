...
class MESOSParams(Params.Params):
    ...
    def get_pod_info(self, container_id, pod_uuid=None):
        ...
        try:
            ...
            if docker_client == None:
                raise Params.ParamsError(Consts.PARAMS_ERR_DOCKER_CONNECTION,
            ...
            if self.pod_uuid == None:
                raise Params.ParamsError(Consts.PARAMS_ERR_GET_UUID,
    ...
    def get_stdin_params(self, stdin_json):
        self.mesos_params = stdin_json
        return

    def log(self):
        self.logger.debug('MESOS Params pod_uuid = ' + str(self.pod_uuid) +
