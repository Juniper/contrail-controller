...
class VncServiceTest(KMTestCase):
    ...
    def test_vnc_service_add_delete_service_with_isolated_namespace_with_cluster_defined(self):
        ...
        namespace = self._enqueue_add_isolated_namespace()
        network = self._create_virtual_network(cluster_project)
    ...
    def test_vnc_service_add_delete_loadbalancer_with_default_namespace_with_cluster_defined(self):
        ...
        ports, srv_meta, srv_type, srv_uuid = self._enqueue_add_loadbalancer()
    ...
    def test_vnc_service_kubernetes(self):
        ...
        ports, srv_meta, srv_type, srv_uuid = self._enqueue_add_kubernetes_service()
    ...
    def test_negative(self):
        ...
        ports, srv_meta, srv_type, _ = self._enqueue_add_service()

        self.kill_kube_manager()

        srv_spec = {'type': srv_type, 'ports': ports}
        srv_del_event = self.create_service_event('Service', srv_spec, srv_meta, 'DELETED')
        self.enqueue_event_without_waiting(srv_del_event)

        srv_uuid = str(uuid.uuid4())
        srv_meta = {'name': 'test-service', 'uuid': srv_uuid, 'namespace': self.namespace}
        ports = [{'name': 'http', 'protocol': 'TCP', 'port': 80, 'targetPort': 9376}]
        srv_spec = {'type': srv_type, 'ports': ports}
        srv_add_event = self.create_service_event('Service', srv_spec, srv_meta, 'ADDED')
        self.enqueue_event_without_waiting(srv_add_event)
    ...
    def _enqueue_add_default_namespace(self):
        ...
        ns_add_event = self.create_add_namespace_event(self.namespace, ns_uuid)
    ...
    def _enqueue_delete_namespace(self, ns_uuid):
        ns_delete_event = self.create_delete_namespace_event(self.namespace, ns_uuid)
        self.enqueue_event(ns_delete_event)

    def _enqueue_add_kubernetes_service(self):
        return self._enqueue_add_service(srv_name='kubernetes')

    def _enqueue_add_loadbalancer(self):
        return self._enqueue_add_service(srv_type='LoadBalancer',
    ...
    def _enqueue_add_service(self,
        ...
        srv_uuid = str(uuid.uuid4())
        srv_meta = {'name': srv_name, 'uuid': srv_uuid, 'namespace': self.namespace}
        ports = [{'name': 'http', 'protocol': 'TCP', 'port': 80, 'targetPort': 9376}]
