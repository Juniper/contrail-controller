...
class VncEndpointsTest(test_case.KMTestCase):
    ...
    @classmethod
    def spawn_kube_manager(cls, *args, **kwargs):
        with patch('test_common.launch_kube_manager',
                   side_effect=cls.launch_kube_manager):
            super(VncEndpointsTest, cls).spawn_kube_manager(*args, **kwargs)
    ...
    def _add_service(self, namespace, srv_name, srv_type='ClusterIP'):
        srv_meta = {
            'name': srv_name,
            'uid': str(uuid.uuid4()),
            'namespace': namespace
    ...
    def _check_lb_members(self, *members):
        lb_pool = self._vnc_lib.loadbalancer_pool_read(
            id=self.pool_uid, fields=('loadbalancer-pool-loadbalancer-member',))
        self.assertEqual(len(lb_pool.get_loadbalancer_members()), len(members))
        lb_members = [
            self._vnc_lib.loadbalancer_member_read(id=member['uuid'])
            for member in lb_pool.loadbalancer_members]
