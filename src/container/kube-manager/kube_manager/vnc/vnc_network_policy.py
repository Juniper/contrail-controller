...
class VncNetworkPolicy(object):
    ...
    def __init__(self, vnc_lib=None, label_cache=None, logger=None):
        ...
        self._policy_src_label_cache = {}
        self._policy_dest_label_cache = {}
    ...
    def _append_sg_rule(self, sg_obj, sg_rule):
        ...
        if rules is None:
            ...
            for sgr in rules.get_policy_rule() or []:
                ...
                if sg_rule == sgr_copy:
                    self._logger.info("SecurityGroupRuleExists %s" % sgr.rule_uuid)
                    return
    ...
    def _remove_sg_rule_by_id(self, sg_obj, rule_uuid):
        rules = sg_obj.get_security_group_entries()
        if rules is None:
            self._logger.info("No SecurityGroupRule matching rule_uuid %s, no rules found" % rule_uuid)
            return

        for sgr in rules.get_policy_rule() or []:
            if sgr.rule_uuid == rule_uuid:
                self._logger.info("Deleting SecurityGroupRule %s matching rule_uuid %s: " %
        ...
        if update_needed:
            ...
            self._logger.info("No SecurityGroupRule matching rule_uuid %s found" % rule_uuid)

    def _remove_sg_rule_by_src_address(self, sg_obj, src_address):
        rules = sg_obj.get_security_group_entries()
        if rules is None:
            self._logger.error("No SecurityGroupRules matching src_addr %s, no rules found" % src_address)
            return

        update_needed = False
        for sgr in rules.get_policy_rule() or []:
            for src in sgr.src_addresses:
                if src.subnet.ip_prefix == src_address:
                    self._logger.info("Deleting SecurityGroupRule %s matching src_addr %s: " %
        ...
        if update_needed:
            ...
            self._logger.error("No SecurityGroupRule matching src_addr %s found" % src_address)
        ...
        for rule in rules:
            ...
            for src_selector in src_selectors:
                ...
                for src_label in src_labels.items():
                    ...
                    self._label_cache._locate_label(key, self._policy_src_label_cache, src_label, uuid)
...
#            vmi = VirtualMachineInterfaceKM.get(vmi_id)
    ...
    def _apply_sg_2_pods(self, sg, event):
        ...
        for dest_label in dest_labels.items():
            ...
            self._label_cache._locate_label(key, self._policy_dest_label_cache, dest_label, uuid)
...
#        sg = SecurityGroupKM.get(uid)
    ...
    def _vnc_network_policy_add(self, event):
...
#            vmi = VirtualMachineInterfaceKM.get(vmi_id)
    ...
    def _vnc_network_policy_delete(self, event):
        uuid = event['object']['metadata'].get('uid')
        sg = self._vnc_lib.security_group_read(id=uuid)
        self._vnc_sg_clear_vmis(sg)
        self._vnc_lib.security_group_delete(id=uuid)
        self._policy_src_label_cache = self._label_cache._remove_value(self._policy_src_label_cache, uuid)
        self._policy_dest_label_cache = self._label_cache._remove_value(self._policy_dest_label_cache, uuid)
    ...
    def _add_src_pod_2_policy(self, pod_id, policy_id):
        try:
            ...
            self._logger.error("Cannot read security group with UUID %s" % id)
    ...
    def _vnc_pod_add(self, event):
        ...
        for label in labels.items():
            ...
            policy_ids = self._policy_src_label_cache.get(key, [])
            for policy_id in policy_ids:
                self._add_src_pod_2_policy(pod_id, policy_id)

            policy_ids = self._policy_dest_label_cache.get(key, [])
            for policy_id in policy_ids:
    ...
    def _delete_src_pod_from_policy(self, pod_id, policy_id):
        ...
        self._remove_sg_rule_by_src_address(sg, ip_addr)
    ...
    def _vnc_pod_delete(self, event):
        labels = event['object']['metadata']['labels']
        pod_id = event['object']['metadata']['uid']
    ...
    def process(self, event):
        if event['object'].get('kind') == 'NetworkPolicy':
            if event['type'] == 'ADDED' or event['type'] == 'MODIFIED':
                self._vnc_network_policy_add(event)
            elif event['type'] == 'DELETED':
                self._vnc_network_policy_delete(event)
        if event['object'].get('kind') == 'Pod':
            if event['type'] == 'ADDED' or event['type'] == 'MODIFIED':
                self._vnc_pod_add(event)
            elif event['type'] == 'DELETED':
                self._vnc_pod_delete(event)
