...
class VncNetwork(VncCommon):
    ...
    def process(self, event):
        ...
        if event['type'] == 'ADDED' or event['type'] == 'MODIFIED':
            ...
            if 'opencontrail.org/cidr' in annotations:
                ...
                # Create project and IPAM for given subnet if it does not exist
                self._logger.debug("%s - Create IPAM- Subnets %s" \
                                                %(self._name, subnets))
                subnet_data = self._create_subnet_data(subnets)
                proj_obj = self._create_project(namespace)
                ...
                cluster_pod_vn_obj = self._create_virtual_network(\
                    vn_name=vn_name, vn_type='pod-network', \
                    proj_obj=proj_obj, provider=provider, \
    ...
    def _make_vn_fq_name(self, ns_name, vn_name, domain_name='default-domain'):
        ...
        project_name = vnc_kube_config.cluster_name() + "-" + ns_name
        vn_fq_name.append(project_name)
        virtual_net_name = vnc_kube_config.cluster_name() + "-" + \
                                            vn_name + "-pod-network"
    ...
    def _create_virtual_network(self, vn_name, vn_type, proj_obj,
                ipam_obj, ipam_update, provider=None, \
                subnets=None, type='flat-subnet-only'):
