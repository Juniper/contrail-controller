...
class VncKubernetes(object):
    ...
    def _create_cluster_service_fip_pool(self, vn_obj, ipam_obj):
        ...
        try:
            ...
            fip_pool_vnc_obj = self.vnc_lib.floating_ip_pool_read(
                fq_name=fip_pool_obj.get_fq_name())
        except RefsExistError:
            ...
            prefixes = fip_pool_vnc_obj.get_floating_ip_pool_prefixes()
            if prefixes:
                subnet_list = prefixes.get_subnet_list()
                if subnet_list:
                    for subnet in subnet_list:
                        if subnet.get_subnet_uuid() == svc_subnet_uuid:
            ...
            # Update local cache.
            FloatingIpPoolKM.locate(fip_pool_vnc_obj.uuid)
