...
class KubeNetworkManager(object):
    ...
    def sandesh_handle_greenlet_stack_list_request(cls, request):
        x = [a for a in [traceback.format_stack(ob.gr_frame) for ob in
                         gc.get_objects() if
                         isinstance(ob, greenlet.greenlet) and ob]]
        resp = introspect.KubeGreenletStackListResp(greenlets=[])
        resp.greenlets = [introspect.KubeGreenletStackInstance(stack=i)
                          for i in x]
