...
def parse_config(args):
    ...
    cache = config.get("DEFAULT", "cache")
...
class Scheduler(object):
    ...
    def __init__(self, vnc_client, cache):
        self.cache = cache
        self._vnc_client = vnc_client
        self._cached_data = self._get_cached_data()
        self._send_freq = self._cached_data.get("send_freq",
                                                self._get_updated_send_freq())
        self._sched_job_ts = self._cached_data.get("sched_job_ts",
    ...
    def _save_cached_data(self):
        cached_data = shelve.open(self.cache, writeback=True)
    ...
    def is_job(self):
        ...
        elif ((self.sched_job_ts is not None) and
              (datetime.now() > self.sched_job_ts)):
            is_job = True
        return is_job
...
def main():
    ...
    scheduler = Scheduler(vnc_client=vnc_client, cache=config["cache"])
