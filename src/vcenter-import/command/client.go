...
// Token get the token either from ClusterToken or from Username
func (cc *ContrailCommand) Token(contrailClusterID, authToken string) error {
	var err error
	var jsonData []byte
	if contrailClusterID != "" && authToken != "" {
	...
	resp, _, err := cc.sendRequest(cc.AuthPath, string(jsonData), "POST")
	if err != nil {
		return err
	}
...
// tokenViaClusterToken gets the token from contrail-command using
// cluster-token and clusterId
func (cc *ContrailCommand) tokenViaClusterToken() (string, error) {
	authData := keyStoneAuthViaClusterToken{
		Auth: auth{
			Identity: identity{
				Methods: []string{"cluster-token"},
	...
	log.Print(" TOKEN ", cc.AuthToken)
	return cc.AuthToken, nil
}

// tokenViaUsername get the auth token from contrail-command using
// username/password and domain
func (cc *ContrailCommand) tokenViaUsername() (string, error) {
	var debug = true
	authData := keyStoneAuthViaUsername{
		Auth: userAuth{
			Identity: userIdentity{
	...
	jsonData, err := json.Marshal(authData)
	if err != nil {
		log.Print("json marshal failed for AuthViaUsername")
		return "", nil
	}
	resp, _, err := cc.sendRequest(cc.AuthPath, string(jsonData), "POST")
	if err != nil {
	...
	if resp.StatusCode != 200 && resp.StatusCode != 201 {
		log.Print("returning due to non-200")
...
func (cc *ContrailCommand) GetNodes() (map[string]CCNode, error) {
	_, body, err := cc.sendRequest("/nodes?detail=true", "", "GET")
	if err != nil {
		return nil, err
	}
...
func (cc *ContrailCommand) CreateNode(host vcenter.ESXIHost) error {
	var debug = true
	log.Print("Create Node:", cc.AuthToken)
	...
	if debug {
		log.Print("Got status : ", resp.StatusCode)
	}
	return nil
}
...
func (cc *ContrailCommand) CreatePort(port vcenter.ESXIPort, host vcenter.ESXIHost) error {
	...
	_, _, err = cc.sendRequest("/sync", string(jsonData), "POST")
	if err != nil {
		return err
	}
	return nil
