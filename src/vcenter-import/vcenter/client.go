...
func (c *Client) DataCenterHostSystems(ctx context.Context, datacenter string) ([]ESXIHost, error) {
	...
	for _, dc := range dcs {
		...
		log.Debug("Create View for ", datacenter)
...
func (c *Client) esxiPorts(ctx context.Context, hs mo.HostSystem) map[string]ESXIPort {
	...
	return c.addLLDPPortInfo(ctx, hs, esxiPorts)
}

func (c *Client) addLLDPPortInfo(ctx context.Context, hs mo.HostSystem , ports map[string]ESXIPort) map[string]ESXIPort {
	...
	if err != nil {
		// it may fail to query, just return ports we received
		log.Debug("Failed to get lldp info:", err)
		return ports
	}
