...
ExternalBase::Efn DbQueryUnit::QueryExec(uint32_t inst,
    ...
    if (current_row < inp.total_rows) {
        ...
        ip_ctx->inst = inst;
        QE_LOG(INFO,"start sub_query " << sub_query_id << " chunk " << ip_ctx->chunk_no << " instance " << ip_ctx->inst << " row " << ip_ctx->row_no);
        return boost::bind(&GenDb::GenDbIf::Db_GetRowAsync, m_query->dbif_,
                           cinp.cf_name, cinp.keys[current_row], cinp.cr,
                           cinp.cb, ip_ctx, _1);
    } else {
...
query_status_t DbQueryUnit::process_query()
{
    ...
    QE_LOG(INFO,"DbQueryUnit sub_query " << sub_query_id);
...
void DbQueryUnit::WPCompleteCb(QEPipeT *wp, bool ret_code) {
    ...
    QE_TRACE(DEBUG,  " Database query completed with Async"
            << size << " rows");
    AnalyticsQuery *m_query = (AnalyticsQuery *)main_query;
    QE_LOG(INFO,"Finished sub_query " << sub_query_id << " chunk " << m_query->parallel_batch_num);
...
void DbQueryUnit::cb(GenDb::DbOpResult::type dresult,
    ...
    //QE_LOG(INFO,"partial sub_query start" << sub_query_id );
    ...
    //QE_LOG(INFO,"partial sub_query end" << sub_query_id << " chunk " << gr->chunk_no << " instance " << gr->inst << " row " << gr->row_no);
    if (privdata) {
