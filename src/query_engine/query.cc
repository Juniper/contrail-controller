...
PostProcessingQuery::PostProcessingQuery(
    ...
    for (iter = json_api_data.begin(); iter != json_api_data.end(); iter++)
    {
        ...
        if (iter->first == QUERY_SORT_FIELDS)
        {
            ...
            const RAPIDJSON_NAMESPACE::Value& json_sort_fields =
                d["sort_fields"]; 
            QE_PARSE_ERROR(json_sort_fields.IsArray());
            QE_TRACE(DEBUG, "# of sort fields:"<< json_sort_fields.Size());
            for (RAPIDJSON_NAMESPACE::SizeType i = 0; i<json_sort_fields.Size(); i++) 
            {
        ...
        if (iter->first == QUERY_FILTER) {
            ...
            d.Parse<0>(const_cast<char *>(json_string.c_str()));
            const RAPIDJSON_NAMESPACE::Value& json_filters =
                d["filter"]; 
            QE_PARSE_ERROR(json_filters.IsArray());
            QE_TRACE(DEBUG, "# of filters:"<< json_filters.Size());
            bool single_list = false;
            if (json_filters.Size()) {
                RAPIDJSON_NAMESPACE::SizeType zeroth = 0;
                const RAPIDJSON_NAMESPACE::Value& json_filters_0 = json_filters[zeroth];
            ...
            if (single_list) {
                ...
                for (RAPIDJSON_NAMESPACE::SizeType j = 0; j<json_filters.Size(); j++) 
                  {
                    ...
                    const RAPIDJSON_NAMESPACE::Value& name_value = 
                        json_filters[j][WHERE_MATCH_NAME];
                    const RAPIDJSON_NAMESPACE::Value&  value_value = 
                        json_filters[j][WHERE_MATCH_VALUE];
                    const RAPIDJSON_NAMESPACE::Value& op_value = 
                ...
                for (RAPIDJSON_NAMESPACE::SizeType j = 0; j<json_filters.Size(); j++) {
                    std::vector<filter_match_t> filter_and;
                    const RAPIDJSON_NAMESPACE::Value& json_filter_and = json_filters[j];
                    QE_PARSE_ERROR(json_filter_and.IsArray());

                    for (RAPIDJSON_NAMESPACE::SizeType k = 0; k<json_filter_and.Size(); k++) {
                        filter_match_t filter;
                        QE_PARSE_ERROR((
...
static void get_uuid_from_json(const RAPIDJSON_NAMESPACE::Document &dd,
    boost::uuids::uuid *u) {
...
static void get_stats_from_json(const RAPIDJSON_NAMESPACE::Document &dd,
    flow_stats *stats) {
    const std::vector<std::string> &frnames(g_viz_constants.FlowRecordNames);
...
static void get_8tuple_from_json(const RAPIDJSON_NAMESPACE::Document &dd,
    flow_tuple *tuple) {
    const std::vector<std::string> &frnames(g_viz_constants.FlowRecordNames);
...
void get_uuid_stats_8tuple_from_json(const std::string &jsonline,
    ...
    RAPIDJSON_NAMESPACE::Document dd;
