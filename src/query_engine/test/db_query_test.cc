...
class DbQueryUnitTest: public ::testing::Test {
     public:
         bool GetRowAsyncSuccess(const std::string& cfname,
             ...
             GenDb::GenDbIf::DbGetRowCb cb, const void * ctx,
             void * privdata);
...
bool DbQueryUnitTest::GetRowAsyncSuccess(const std::string& cfname,
        const GenDb::DbDataValueVec& rowkey, const GenDb::ColumnNameRange &crange, GenDb::GenDbIf::DbGetRowCb cb, const void * ctx,
        void * privdata) {
    ...
    std::auto_ptr<GenDb::NewColVec> columns(new GenDb::NewColVec());
    int ttl = 5;
    *columns = boost::assign::ptr_list_of<GenDb::NewCol>
        (GenDb::NewCol(colname, colvalue, ttl));

    cb(GenDb::DbOpResult::OK, columns, ctx, privdata);
    return true;
}
...
bool DbQueryUnitTest::GetRowAsyncFailure(const std::string& cfname,
        ...
        void * privdata) {
        std::auto_ptr<GenDb::NewColVec> columns(new GenDb::NewColVec());
        cb(GenDb::DbOpResult::ERROR, columns, ctx, privdata);
        return true;
}
...
TEST_F(DbQueryUnitTest, DISABLED_ProcessQuery) {
    ...
    EXPECT_CALL(*(CqlIfMock *)(analytics_query_mock.dbif_.get()), Db_GetRowAsync(_,_,_,_,_,_)).Times(AnyNumber()).WillRepeatedly(Invoke(this, &DbQueryUnitTest::GetRowAsyncSuccess));
...
TEST_F(DbQueryUnitTest, DISABLED_ProcessQueryFailure) {
    AnalyticsQueryMock analytics_query_mock;
    AnalyticsQueryMock parent;
    analytics_query_mock.parallel_batch_num = 1;
    analytics_query_mock.query_id = "abcd";
    ...
    DbQueryUnit *dbq = new DbQueryUnit(&analytics_query_mock, &analytics_query_mock);
    EXPECT_CALL(*(CqlIfMock *)(analytics_query_mock.dbif_.get()), Db_GetRowAsync(_,_,_,_,_,_)).Times(AnyNumber()).WillRepeatedly(Invoke(this, &DbQueryUnitTest::GetRowAsyncFailure));
    EXPECT_CALL(analytics_query_mock, table()).Times(AnyNumber()).WillRepeatedly(Return("table1"));
    EXPECT_CALL(analytics_query_mock, end_time()).Times(AnyNumber()).WillRepeatedly(Return(1473385977637609));
    EXPECT_CALL(analytics_query_mock, from_time()).Times(AnyNumber()).WillRepeatedly(Return(1473384977637609));
...
TEST_F(DbQueryUnitTest, WhereQueryProcessing) {
    ...
    // check result of the AND operation
    EXPECT_EQ(1, wq->where_result_->size());
    delete q1;
    delete q2;
}
