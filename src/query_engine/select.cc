...
SelectQuery::SelectQuery(QueryUnit *main_query,
    ...
    for (contrail_rapidjson::SizeType i = 0; i < json_select_fields.Size(); i++) 
    {
        ...
        // processing other select fields
            ...
            if (json_select_fields[i].GetString() == g_viz_constants.UUID)
                uuid_key_selected = true;
#ifdef USE_SESSION
            std::vector<std::string>::iterator it =
                std::find(session_json_fields.begin(),
    ...
    if ((m_query->table() == g_viz_constants.FLOW_TABLE) && !uuid_key_selected) {
        select_column_fields.push_back(g_viz_constants.UUID);
        unroll_needed = true;
    }
        ...
        for (std::vector<query_result_unit_t>::const_iterator it = query_result.begin();
            ...
            for (unsigned int i = 0; i < it->info.size(); i++) {
                ...
                if (value.which() != GenDb::DB_VALUE_BLANK) {
                    ...
                    if (index_type) {
                        ...
                        QE_TRACE(DEBUG,  "value_str before T2:" << value_str);
                        static boost::regex expr("^[0-9]+:");
                        std::string value_str2 = boost::regex_replace(value_str, expr, "",
                                                        boost::match_default | boost::format_all);
                        value = value_str2;
                        QE_TRACE(DEBUG,  "value_str2 after T2:" << value_str2);
                    }

                    // Remove "<object-type>:" from OBJECT_TYPE_NAME[1..6] values
                    std::string object_type_name_pfx(g_viz_constants.OBJECT_TYPE_NAME_PFX);
                    object_type_name_pfx.append("[1-6]+");
                    static boost::regex object_type_name_expr(object_type_name_pfx);
                    if (boost::regex_match(query_column, object_type_name_expr)) {
                        std::string value_str(GenDb::DbDataValueToString(value));
                        QE_TRACE(DEBUG,  "value_str before OBJ-TYPE-NAME:" << value_str);
                        static boost::regex object_type_expr("^[A-Za-z]+:");
                        std::string value_str2 = boost::regex_replace(value_str, object_type_expr, "",
                                                        boost::match_default | boost::format_all);
                        value = value_str2;
                        QE_TRACE(DEBUG,  "value_str2 after OBJ-TYPE-NAME:" << value_str2);
                    }
                }

                col_res_map.insert(std::make_pair(query_column, value));
                QE_TRACE(DEBUG,  "col_res_map " << query_column << ":"
                         << GenDb::DbDataValueToString(value));
            }
            ...
            if (m_query->is_object_table_query(m_query->table())) {
                std::map<std::string, GenDb::DbDataValue>::iterator uuid_it =
                                            col_res_map.find(g_viz_constants.UUID);
            ...
            for (jt = select_column_fields.begin();
                ...
                } else if (*jt == g_viz_constants.UUID) {
