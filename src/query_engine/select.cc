...
SelectQuery::SelectQuery(QueryUnit *main_query,
    ...
    if (
            (m_query->table() == g_viz_constants.COLLECTOR_GLOBAL_TABLE2) ||
            (m_query->is_object_table_query(m_query->table()))
       )
    {
        cfname = g_viz_constants.COLLECTOR_GLOBAL_TABLE2;
        ...
        for (std::vector<query_result_unit_t>::const_iterator it = query_result.begin();
            ...
            for (unsigned int i = 0; i < it->info.size(); i++) {
                ...
                std::string column_name = 
                    (g_viz_constants._VIZD_TABLE_SCHEMA.find(cfname))\
                        ->second.column_to_query_column.find(columnN.c_str())->second;
                ...
                if (index_type) {
                    std::string value_str(GenDb::DbDataValueToString(it->info.at(i)));
                ...
                col_res_map.insert(std::make_pair(column_name, value));
            }
        }
