...
static std::string ToString(const RAPIDJSON_NAMESPACE::Value& value_value) {
    std::string svalue;
...
static GenDb::DbDataValue ToDbDataValue(const RAPIDJSON_NAMESPACE::Value& val) {
...
bool
WhereQuery::StatTermParse(QueryUnit *main_query, const RAPIDJSON_NAMESPACE::Value& where_term,
    ...
    if (!where_term.HasMember(WHERE_MATCH_NAME))
        return false;
    const RAPIDJSON_NAMESPACE::Value& name_value = where_term[WHERE_MATCH_NAME];
    if (!name_value.IsString()) return false;
    pname = name_value.GetString();

    const RAPIDJSON_NAMESPACE::Value& prval = where_term[WHERE_MATCH_VALUE];
    if (!((prval.IsString() || prval.IsNumber()))) return false;
    RAPIDJSON_NAMESPACE::Value prval2;
    ...
    if (!where_term.HasMember(WHERE_MATCH_OP))
        return false;
    const RAPIDJSON_NAMESPACE::Value& op_value = where_term[WHERE_MATCH_OP];
    ...
    if (where_term.HasMember(WHERE_MATCH_SUFFIX)) {
        const RAPIDJSON_NAMESPACE::Value& suffix = where_term[WHERE_MATCH_SUFFIX];
        if (suffix.IsObject()) {
            ...
            if (!suffix.HasMember(WHERE_MATCH_VALUE))
                return false;
            const RAPIDJSON_NAMESPACE::Value& svalue_value =
            ...
            RAPIDJSON_NAMESPACE::Value svalue2_value;
            ...
            if (!suffix.HasMember(WHERE_MATCH_OP))
                return false;
            const RAPIDJSON_NAMESPACE::Value& sop_value =
            ...
            if (!suffix.HasMember(WHERE_MATCH_NAME))
                return false;
            const RAPIDJSON_NAMESPACE::Value& sname_value =
    ...
    if (m_query->stats().is_stat_table_static()) {
        ...
        if (iter != aQuery->json_api_data_.end()) {
            ...
            const RAPIDJSON_NAMESPACE::Value& json_schema = d["schema"];
            // If schema is not passed, proceed without suffix information
            if (json_schema.Size() == 0) {
                return true;
            }
            for (RAPIDJSON_NAMESPACE::SizeType j = 0; j<json_schema.Size(); j++) {
                if (!(json_schema[j].HasMember(WHERE_MATCH_NAME) &&
                    ...
                    return false;
                const RAPIDJSON_NAMESPACE::Value& name = json_schema[j][WHERE_MATCH_NAME];
                const RAPIDJSON_NAMESPACE::Value&  datatype =
                        json_schema[j][QUERY_TABLE_SCHEMA_DATATYPE];
                const RAPIDJSON_NAMESPACE::Value& index =
                        json_schema[j][QUERY_TABLE_SCHEMA_INDEX];
                const RAPIDJSON_NAMESPACE::Value& suffixes =
                ...
                if (suffixes.IsArray() && suffixes.Size() > 0) {
                    for (RAPIDJSON_NAMESPACE::SizeType k = 0; k<suffixes.Size(); k++) {
                        const RAPIDJSON_NAMESPACE::Value& suffix_name = suffixes[k];
...
bool WhereQuery::StatTermProcess(const RAPIDJSON_NAMESPACE::Value& where_term,
...
WhereQuery::WhereQuery(const std::string& where_json_string, int direction,
    ...
    d.Parse<0>(const_cast<char *>(json_string.c_str()));
    const RAPIDJSON_NAMESPACE::Value& json_or_list = d["where"]; 
    ...
    for (RAPIDJSON_NAMESPACE::SizeType i = 0; i < json_or_list.Size(); i++) 
    {
        const RAPIDJSON_NAMESPACE::Value& json_or_node = json_or_list[i];
        ...
        for (RAPIDJSON_NAMESPACE::SizeType j = 0; j < json_or_node.Size(); j++)
        {
            ...
            if (op == IN_RANGE)
            {
                ...
                const RAPIDJSON_NAMESPACE::Value&  value_value2 = 
