    ...
    if (where_json_string == std::string(""))
    {
        ...
        || (m_query->table() == g_viz_constants.FLOW_SERIES_TABLE)) {
            ...
            DbQueryUnit *db_query_client = new DbQueryUnit(this, main_query);
            {
                ...
                db_query->row_key_suffix.push_back((uint8_t)0);
            ...
            }
            {
                ...
                db_query_client->row_key_suffix.push_back((uint8_t)1);
                // starting value for clustering key range
        ...
        for (contrail_rapidjson::SizeType j = 0; j < json_or_node.Size(); j++)
        {
            ...
            if (m_query->is_session_query(m_query->table())) {
                                ...
                                SessionRecordFields::SESSION_SPORT])
                {
                    ...
                    QE_INVALIDARG_ERROR(itr !=
                            (it->second.index_column_to_column.end()));
                    std::string columnN = itr->second;
                                ...
                                FlowRecordFields::FLOWREC_SOURCEIP])
                {
                    ...
                    additional_select.push_back(filter.name);
...
/*        if (!isSession) {
        ...
        if (isSession) {
            ...
            }
            {
                ...
                client_session_query->row_key_suffix.push_back((uint8_t)0);
                client_session_query->row_key_suffix.push_back((uint8_t)1);
                client_session_query->where_vec = where_vec;
                ...
                if ((direction_ing == 0 && dip_match) ||
                    ...
                    QE_ASSERT(it != g_viz_constants._VIZD_SESSION_TABLE_SCHEMA.end());
                    std::map<string, string>::const_iterator itr =
                        it->second.index_column_to_column.find("ip");
                    QE_INVALIDARG_ERROR(itr !=
                            (it->second.index_column_to_column.end()));
                    std::string columnN = itr->second;
                ...
                if ((direction_ing == 0 && dvn_match) ||
                    ...
                    std::string columnN =
                            ((g_viz_constants._VIZD_SESSION_TABLE_SCHEMA
                                .find(g_viz_constants.SESSION_TABLE))->second
                                .index_column_to_column.find("vn"))->second;
                ...
                if ((direction_ing == 0 && svn_match) ||
                    ...
                    std::string columnN =
                            ((g_viz_constants._VIZD_SESSION_TABLE_SCHEMA
                                .find(g_viz_constants.SESSION_TABLE))->second
                                .index_column_to_column.find("remote_vn"))->second;
            ...
            }
            {
                ...
                server_session_query->row_key_suffix.push_back((uint8_t)0);
                server_session_query->row_key_suffix.push_back((uint8_t)0);
                server_session_query->where_vec = where_vec;
                ...
                if ((direction_ing == 0 && sip_match) ||
                    (direction_ing == 1 && dip_match)) {
                    std::map<string, table_schema>::const_iterator it =
                        (g_viz_constants._VIZD_SESSION_TABLE_SCHEMA.find(
                            g_viz_constants.SESSION_TABLE));
                    QE_ASSERT(it != g_viz_constants._VIZD_SESSION_TABLE_SCHEMA.end());
                    ...
                    QE_INVALIDARG_ERROR(itr !=
                            (it->second.index_column_to_column.end()));
                    std::string columnN = itr->second;
                    int op = direction_ing?dip_op:sip_op;
                    std::string val = direction_ing?
                        (boost::get<std::string>(dip)):(boost::get<std::string>(sip));
                    GenDb::Op::type comparator;
                    if (op == PREFIX) {
                        val += "%";
                        comparator = GenDb::Op::LIKE;
