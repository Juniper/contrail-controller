...
public:
    ...
    void ConnUpPrePostProcess(uint8_t cnum) {
        ...
        cb_proc_fn_[cnum] = boost::bind(&QEOpServerImpl::AuthCallbackProcess,
                    this, cnum, _1, _2, _3);
        conns_[cnum].get()->SetClientAsyncCmdCb(cb_proc_fn_[cnum]);
        //Send AUTH command
    ...
    void AuthCallbackProcess(uint8_t cnum, const redisAsyncContext *c, void *r, void *privdata) {
        QE_LOG_NOQID(DEBUG,"In AuthCallbackProcess..");
        redisReply reply = *reinterpret_cast<redisReply*>(r);
        if (reply.type != REDIS_REPLY_ERROR) {
            qosp_->evm_->io_service()->post(
                    boost::bind(&QEOpServerImpl::ConnAuth,
                    this, cnum));
            return;

        }else {
            QE_LOG_NOQID(DEBUG,"In AuthCallbackProcess.. Error");
        }
    }
...
QEOpServerProxy::QEOpServerProxy(EventManager *evm, QueryEngine *qe,
            const string & hostname, uint16_t port, const string & redis_passwd,
            int max_tasks) :
        evm_(evm),
        qe_(qe),
        impl_(new QEOpServerImpl(hostname, port, redis_passwd, this, 
            max_tasks)) {}
