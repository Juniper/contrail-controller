    ...
    void JsonInsert(std::vector<query_column> &columns,
        ...
        for (size_t j = 0; j < columns.size(); j++)
        {
            if ((0 == map_it->first.compare(0,5,string("COUNT")))) {
                RAPIDJSON_NAMESPACE::Value val(RAPIDJSON_NAMESPACE::kNumberType);
                unsigned long num = 0;
                ...
                if (map_it->second.length() == 0) {
                    RAPIDJSON_NAMESPACE::Value val(RAPIDJSON_NAMESPACE::kNullType);
                    RAPIDJSON_NAMESPACE::Value vk;
                    dd.AddMember(vk.SetString(map_it->first.c_str(), dd.GetAllocator()), val, dd.GetAllocator());
                    found = true;
                    continue;
                ...
                {
                    RAPIDJSON_NAMESPACE::Value val(RAPIDJSON_NAMESPACE::kStringType);
