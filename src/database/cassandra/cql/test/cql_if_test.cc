...
TEST_F(CqlIfTest, DynamicCfGetResultAllRows) {
    ...
    EXPECT_EQ(expected_v_col_list.size(), actual_v_col_list.size());
    for (size_t i = 0; i < expected_v_col_list.size(); i++) {
        EXPECT_EQ(expected_v_col_list[i], actual_v_col_list[i]);
    }
}
