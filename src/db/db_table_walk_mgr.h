...
//      Application is suppose to release the walker allocated using AllocWalker
//      AllocWalker can be called from any task context.
...
//           context. This callback is not invoked in cases where application
...
// db::DBTable task with instance id set as partition index. Because of the
// default task policy only one DBTable partition task can walk the table at any
// given time. The advantage of running DBTable walk in serial manner is in
// clubbing multiple walk requests on a given table and serving such requests
// in one walk requests.
// On a given DBTable, if multiple application modules are requesting for table
// walk, they are clubbed in one iteration of DBTable walk.
//
...
// triggerred to trigger walk request from top of walk_request_list_.
//
