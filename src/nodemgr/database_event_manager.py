...
class DatabaseEventManager(EventManager):
    ...
    def process(self):
        ...
        if (linux_dist == 'Ubuntu'):
            ...
            popen_cmd = "set `df -Pk \`grep -A 1 'data_file_directories:'" + \
              ...
              " -f2 \`/ContrailAnalytics | grep %` && echo $3 | cut -d'%' -f1"
            (disk_space_used, error_value) = \
              Popen(popen_cmd, shell=True, stdout=PIPE).communicate()
            popen_cmd = "set `df -Pk \`grep -A 1 'data_file_directories:'" + \
              "  /etc/cassandra/conf/cassandra.yaml | grep '-' | cut -d'-'" + \
        ...
        if (disk_space_total / (1024 * 1024) < min_disk):
            ...
            (ret_value, error_value) = Popen(cmd_str,
    ...
    def runforever(self, test=False):
        ...
        while 1:
            ...
            headers, payload = self.listener_nodemgr.wait(self.stdin,
              self.stdout)
