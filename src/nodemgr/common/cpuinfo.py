...
class SystemMemCpuInfoData(object):
    ...
    def get_cpu_info(self, system=True):
        mem_cpu_info = SystemMemCpuInfo()
        num_cpu = self._get_num_cpu()
        if self._num_cpu != num_cpu:
            self._num_cpu = num_cpu
            mem_cpu_info.num_cpu = num_cpu
        if system:
            mem_cpu_info.sys_mem_info = self._get_sys_mem_info()
            mem_cpu_info.cpuload = self._get_cpu_load_avg()
        mem_cpu_info.meminfo = self._get_mem_info()
        mem_cpu_info.cpu_share = self._get_cpu_share()
        return mem_cpu_info
    #end get_cpu_info

#end class SystemMemCpuInfoData
