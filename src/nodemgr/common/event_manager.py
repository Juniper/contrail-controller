...
from nodemgr.common.sandesh.nodeinfo.supervisor_events.ttypes import *
...
class SupervisorEventListener(supervisor_event_listener_cls_type):
    def __init__(self):
        ...
        self.supervisor_events_timestamp = '0'
        self.supervisor_events_error_ctr = 0
        self.supervisor_events_error_timestamp = '0'
        ...
        while 1:
            if select.select([sys.stdin], [], [])[0]:
                ...
                if line is not None:
                    ...
                    self.supervisor_events_timestamp = str(int(time.time() * 1000000))
                    break
                else:
                    self.supervisor_events_error_ctr += 1
                    self.supervisor_events_error_timestamp = str(int(time.time() * 1000000))
...
class EventManager(object):
    ...
    def __init__(self, type_info, rule_file,
        ...
        supervisor_events_req.handle_request = self.sandesh_supervisor_handle_request
        event_handlers = {}
        ...
        if is_systemd_based():
            ...
            if not 'SUPERVISOR_SERVER_URL' in os.environ:
                self.msg_log('Node manager must be run as a supervisor event listener', SandeshLevel.SYS_ERR)
                sys.stderr.flush()
                exit(-1)
    ...
    def sandesh_supervisor_handle_request(self, req):
        sys.stderr.write("Got Introspect Request")
        supervisor_resp = SupervisorEventsResp()
        if not is_systemd_based():
            supervisor_resp.supervisor_events = SupervisorEvents(count = self.process_info_manager._event_listener.supervisor_events_ctr)
            supervisor_resp.supervisor_events_error = SupervisorEvents(count = self.process_info_manager._event_listener.supervisor_events_error_ctr)
            supervisor_resp.supervisor_events.last_time_updated_timestamp = self.process_info_manager._event_listener.supervisor_events_timestamp
            supervisor_resp.supervisor_events_error.last_time_updated_timestamp = self.process_info_manager._event_listener.supervisor_events_error_timestamp
    ...
    def get_build_info(self):
        ...
        if self.curr_build_info is None:
            ...
            if len(version_partials) < 3:
                self.msg_log('Not enough values to parse package version %s' % version, SandeshLevel.SYS_ERR)
                return ""
            else:
    ...
    def send_init_info(self):
        ...
        for group in self.group_names:
            ...
            if installed_package_version is None:
                self.msg_log('Error getting %s package version' % (self.get_package_name()), SandeshLevel.SYS_ERR)
                exit(-1)
            else:
        ...
        for key in self.process_state_db[group_name]:
            ...
            if (pstat.process_state == 'PROCESS_STATE_RUNNING'):
                try:
                    ...
                    self.msg_log('NoSuchProcess: process name: %s pid:%d' % (pstat.pname, pstat.pid), SandeshLevel.SYS_ERR)
                else:
    ...
    def run_periodically(self, function, interval, *args, **kwargs):
        while True:
            ...
            if duration < interval:
                ...
                self.msg_log('function %s duration exceeded %f interval (took %f)' % (function.__name__, interval, duration), SandeshLevel.SYS_ERR)
