...
class EventManager(object):
    ...
    def send_nodemgr_process_status_base(self, ProcessStateNames,
	    ...
	    if(state is ProcessStateNames[ProcessState.NON_FUNCTIONAL]):
		ConnectionState.update(conn_type=ConnType.COLLECTOR,
			name=self.module_id, status=ConnectionStatus.DOWN,
			message=description, server_addrs=["%s:%s" %
			    (self.discovery_server, self.discovery_port)])
	    else:
		ConnectionState.update(conn_type=ConnType.COLLECTOR,
			name=self.module_id, status=ConnectionStatus.UP,
			message=description, server_addrs=["%s:%s" %
			    (self.discovery_server, self.discovery_port)])
